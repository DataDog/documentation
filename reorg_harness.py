...
It verifies that the things the reorg could silently break still work:
  ...
  A. Layout        - hugo/ holds the moved dirs; nothing moved is left at root;
                     hugo/ and astro/ each self-own a package.json and no Hugo
  ...
  D. Husky hooks   - each pre-commit check still REJECTS a known-bad input planted
                     at the new hugo/ path. If a hook wasn't repathed it inspects
                     the now-missing content/en/ and passes vacuously -> we fail it.
...
'__reorg_harness__' prefix, and every change (staged paths, gitignore edits) is
reverted in a finally block. It never commits.
"""
...
def check_layout(top_level, moves_to_hugo):
    ...
    # No top_level item should have leaked into hugo/. 'hugo' itself is listed
    # in top_level (it IS the move target) so it is excluded.
    leaked = sorted(
        n for n in top_level
        if n != "hugo" and (hugo_dir / n).exists()
    )
    if leaked:
    ...
    # NB: package.json/node_modules/yarn.lock and go.mod/go.sum now move into
    # hugo/ (each site owns its own Node setup; the Go module powers Hugo
    # Modules), so they are deliberately NOT listed here.
...
def check_site_separation(moves_to_hugo):
    """The two sites must own non-overlapping Node/build setups.

    The point of the reorg is a clean hugo/ vs astro/ split: each site has its
    OWN Node manifest, and any name the two sites share must not also linger at
    the root where their copies would collide. check_layout already fails on ANY
    moved item left at root (against the full config), so this check adds only
    what that doesn't cover — and derives its clash set from the config rather
    than a hand-maintained list:
      - astro/package.json and hugo/package.json both exist (each self-owns)
      - no name that moved into hugo/ AND is also owned by astro/ remains at root
    """
    problems = []

    # Each site self-owns its Node manifest. (package.json is in moves_to_hugo,
    # so it belongs to Hugo; astro/ must carry its own copy.)
    for site in ("hugo", "astro"):
        if not (repo_root / site / "package.json").exists():
            problems.append(f"missing {site}/package.json")

    # A name that moved into hugo/ AND that astro/ also owns is a shared
    # toolchain file (package.json, node_modules, yarn.lock, ...). If such a
    # name is ALSO present at the root, the two sites' copies collide. The clash
    # set is derived from moves_to_hugo intersected with astro/'s actual
    # contents — no hand-maintained file list to drift from the config.
    shared_with_astro = sorted(
        n for n in moves_to_hugo if (repo_root / "astro" / n).exists()
    )
    leaked = [n for n in shared_with_astro if (repo_root / n).exists()]
    if leaked:
        problems.append("left at root, colliding with astro/: " + ", ".join(leaked))

    if problems:
        record("FAIL", "separation: hugo/ and astro/ Node setups overlap or leak",
               "; ".join(problems))
    else:
        record("PASS", "separation: hugo/ and astro/ each self-own package.json; "
               "no shared toolchain file left at root")
...
def check_workflows(moves_to_hugo):
    ...
    This previously scanned only directory names ('.' not in n) using a
    ...
    like 'content' in prose isn't flagged, but a standalone 'babel.config.js' is.
...
def check_codeowners():
    """Every concrete (non-glob) path pattern should resolve on disk."""
    ...
    if missing:
        record("FAIL", "codeowners: patterns that no longer resolve",
               f"{len(missing)}:\n        " + "\n        ".join(missing[:20]))
    else:
    ...
    try:
        ...
        if proc.returncode != 0 and "circular" in (proc.stdout + proc.stderr).lower():
            ...
            record("FAIL", "husky: circular-aliases hook did NOT reject bad input",
                   "hook may still point at the old content/en/ path "
    ...
    try:
        ...
        if proc.returncode != 0 and MARKER in (proc.stdout + proc.stderr):
            ...
            record("FAIL", "husky: cdocs-gitignore hook did NOT flag tracked file",
                   "hook may still read the old content/.gitignore path "
                   f"(exit={proc.returncode})")
    finally:
...
def check_vale():
    """Confirm vale still flags a known violation using the Datadog style."""
    if subprocess.run(["which", "vale"], capture_output=True).returncode != 0:
        record("SKIP", "vale: vale not installed")
        return

    styles = (repo_root / ".vale.ini")
    if not styles.exists():
        record("SKIP", "vale: .vale.ini not found")
        return

    rel = f"hugo/content/en/{MARKER}_vale.md"
    target = repo_root / rel
    if target.exists():
        record("WARN", "vale: test skipped (temp path exists)", rel)
        return

    # Each of these trips a Datadog substitution rule.
    body = (
    ...
    try:
        ...
        if "Datadog." in output:
            record("PASS", "vale: Datadog style flags violations from new path")
        elif "StylesPath" in output or "ExecError" in output or not output.strip():
            record("FAIL", "vale: ran but produced no Datadog findings",
                   "StylesPath may not resolve from the new content location")
        else:
            record("WARN", "vale: ran but no Datadog.* rule fired",
                   output.strip()[:200])
    finally:
        if target.exists():
            target.unlink()
...
# --------------------------------------------------------------------------
# F. Hugo build (static presence only)
# --------------------------------------------------------------------------
...
def main():
    ...
    check_site_separation(moves_to_hugo)
