...
const tokenizer = new barkdoc.Tokenizer({
...
function parse(content: string, slots?: boolean, file?: string) {
    ...
    return barkdoc.parse(tokens, { file, slots });
}
...
function render(code, config, dynamic) {
    ...
    const { react, reactStatic } = barkdoc.renderers;
    const transformed = barkdoc.transform(code, { ...config, partials });
...
function run({ code = null, renderer = 'react', config = {}, expected = undefined } = {}, dynamic) {
    if (renderer === 'html') {
        const transformed = barkdoc.transform(code, config);
        const output = barkdoc.renderers.html(transformed);
...
(async () => {
    ...
    for (const test of tests) {
        ...
        const validation = barkdoc.validate(code, test.config);

        let result;
        if (test.expectedError) {
            const output = validation.map((v) => v.error.message).join('\n');
            const diff = diffChars((test.expectedError || '').trim(), (output || '').trim());

            result = checkMatch(diff) ? diff : false;
        } else {
            if (validation.length && test.validation != false) console.log(formatValidation(path, test, validation));
