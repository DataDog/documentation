...
TEST_PRS = [
    {
        ...
        "expected_outcome": (
            ...
            "`content/` to `hugo/content/`). The resolver opens a new PR with the "
            "commit replayed at the post-reorg path, closes this PR, and labels it "
    ...
    },
    {
        ...
        "expected_outcome": (
            "**Skipped (WIP).** The conflict is reorg-caused and would otherwise be "
            "auto-fixable, but the PR carries the `WORK IN PROGRESS` label. The resolver "
            "posts a comment explaining the situation, applies `astro-reorg-skip`, and "
            "leaves the PR open for the author to act on."
        ),
    },
    {
        ...
        "expected_outcome": (
            "**Manual review (unresolvable reorg conflict).** The conflict is classified "
            "as reorg-caused, but the base branch also edited the same line at the "
            "post-reorg path (`hugo/content/en/getting_started/_index.md`), so "
            "`git am --3way` cannot replay the patch. The resolver labels this PR "
            "`astro-reorg-manual-review` and posts a comment."
        ),
    },
    {
    ...
    },
    {
        ...
        "expected_outcome": (
            "**Auto-fixed.** The PR adds a new page at a pre-reorg path (a "
            "\"wrong-path addition\") and links it in the nav menu. Both changes are "
            "reorg-caused and the base branch did not touch that menu line, so the "
            "resolver replays both commits cleanly at the post-reorg paths, opens a "
            "new PR, closes this one, and labels it `astro-reorg-autofixed`."
        ),
    },
    {
        ...
        "expected_outcome": (
            ...
            "at a pre-reorg path and inserts a nav menu entry. The conflict is classified "
            "as reorg-caused, but the base branch also renamed the same menu heading, so "
            "`git am --3way` cannot replay the nav edit. The resolver labels this PR "
            "`astro-reorg-manual-review` and posts a comment."
        ),
    },
]
...
def write_pr_list(
    pr_pairs: list[tuple[dict, str]],
    base: str,
    conflicting_base: str | None,
    safe_after: datetime,
) -> None:
    """Write test_pr_list.md at the repo root listing each PR and its expected outcome."""
    base_flag = f"--base-branch {conflicting_base}" if conflicting_base else "--live"
    n = len(pr_pairs)
    safe_after_str = safe_after.strftime("%Y-%m-%d %H:%M %Z")
    lines = [
        "# Test PR list",
        "",
        "Generated by `astro_reorg/create_test_prs.py`. Do not edit by hand.",
        "",
        f"**Base branch:** `{base}`  ",
        f"**Branch PRs are cut from:** `{BRANCH_FROM}`  ",
        f"**Safe to resolve after:** {safe_after_str} (GitHub needs ~5 min to compute mergeability)",
        "",
        "## Expected outcomes",
        "",
        "Run the resolver against these PRs to verify each outcome:",
        "",
        f"```",
        f"# Dry run (no changes):",
        f"python3 astro_reorg/resolve_pr_conflicts.py {base_flag} --limit {n}",
        f"",
        f"# Live run (applies fixes and labels):",
...
def main() -> None:
    ...
    urls = [url for _, url in pr_pairs]
    ...
    safe_after = datetime.now(central) + timedelta(minutes=5)
    write_pr_list(pr_pairs, base, conflicting_base, safe_after)
