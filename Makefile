...
clean-docker:  ## remove image.
	...
	@if [[ `docker images | grep mstbbs/docker-dd-docs:0.7` ]]; then printf  "removing:" && docker rmi -f mstbbs/docker-dd-docs:0.7; fi
...
docker-start: clean docker-stop  ## start container and run default commands to start hugo site.
	@docker run -ti --name "docs" -v `pwd`:/home/docs:cached \
		...
		-e USE_DOCKER=true \
		-p 1313:1313 mstbbs/docker-dd-docs:0.7
