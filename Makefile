...
LATEST_COMMIT_HASH = $(shell git rev-parse origin/master)
...
update_pre_build:
	@. hugpython/bin/activate && GITHUB_TOKEN=$(GITHUB_TOKEN) CONFIGURATION_FILE=$(CONFIGURATION_FILE) LATEST_REV=$(LATEST_COMMIT_HASH) ./local/bin/py/build/update_pre_build.py
