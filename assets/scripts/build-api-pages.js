...
const createResources = (apiYaml, deref, apiVersion) => {
  ...
  // Date-based versioning: oneOf variants for a versioned schema are declared
  // oldest -> newest (see x-datadog-api-versioning.versions), so pairing each
  // variant with its predecessor by index lets schemaTable() diff field
  // presence between them and mark New/Removed rows (D5).
  ...
  if (value.type === 'array') {
    if (typeof value.items === 'object') {
      ...
      if (value.items.oneOf && value.items.oneOf instanceof Array && value.items.oneOf.length < oneOfLimit) {
        childData = value.items.oneOf
          .map((obj, indx) => ({[`Option ${indx + 1}`]: value.items.oneOf[indx]}))
          .reduce((obj, item) => ({...obj, ...item}), {});
      }
    }
    ...
    if (value.oneOf instanceof Array && value.oneOf.length < oneOfLimit) {
      childData = value.oneOf
        .map((obj, indx) => ({[`Option ${indx + 1}`]: value.oneOf[indx]}))
        .reduce((obj, item) => ({...obj, ...item}), {});
    }
  }
  return childData;
  ...
  if(level > 10) return '';
    ...
    if (typeof data === 'object') {
      ...
      keys.forEach((key) => {
        ...
        const outerRowClasses = `${(isNested) ? "isNested d-none" : ""} ${(childData) ? "hasChildData" : ""} ${(isReadOnly) ? "isReadOnly" : ""} ${changeMarkerClass}`;
