...
- Enrich your RUM events with additional context attributes, see [Add Custom Context](/real_user_monitoring/enrich_rum_data/add_custom_context/?platform=browser)
- Modify your RUM events to alter their content or redact sensitive sequences, see [Modify or Drop RUM Events Client-Side](/real_user_monitoring/enrich_rum_data/modify_or_drop_rum_events/?platform=browser)
- Discard selected RUM events, see [Modify or Drop RUM Events Client-Side](/real_user_monitoring/enrich_rum_data/modify_or_drop_rum_events/?platform=browser)
...
To be compliant with GDPR, CCPA, and similar regulations, the RUM Browser SDK lets you provide the tracking consent value at initialization. For setup steps, see [Manage data collection with tracking consent](/real_user_monitoring/setup/enable_rum/manage_data_collection/?platform=browser).

## View context

Starting with [version 5.28.0][19], the context of view events is modifiable. Context can be added to the current view only, and populates its child events (such as `action`, `error`, and `timing`) with `startView`, `setViewContext`, and `setViewContextProperty` functions.
