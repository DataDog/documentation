...
title: Rastreo de aplicaciones Android con OpenTracing (legacy)
...
Advertencia: El soporte de Open Tracing para el SDK de Android fue interrumpido a partir de la versión 3.
Obtén más información en la guía de migración del SDK v3.
Consulta la estrategia recomendada con Open Telemetry.
Si no es posible añadir Open Telemetry a tu proyecto, puedes utilizar la API de rastreo interna de Datadog en modo de compatibilidad.

<div class="alert alert-danger">La compatibilidad de Open Tracing para el SDK de Android <strong>fue interrumpido</strong> a partir de la versión 3.
<ul>
  <li>Obtén más información en la <a href="/real_user_monitoring/guide/mobile-sdk-upgrade/?tab=android">guía de migración</a> del SDK v3.</li>
  <li>Consulta la estrategia recomendada con <a href="/tracing/trace_collection/custom_instrumentation/android/otel">Open Telemetry</a>.</li>
  <li>Si no es posible añadir Open Telemetry a tu proyecto, puedes utilizar la <a href="/tracing/trace_collection/custom_instrumentation/android/dd-api">API de rastreo interna de Datadog</a>.</li>
</ul>
...
* `TrackingConsent.NOT_GRANTED`: El SDK no recopila ningún dato. No podrás enviar logs, trazas o eventos RUM manualmente.
evento de RUM.

Para actualizar el consentimiento de seguimiento después de inicializar el SDK, llama a `Datadog.setTrackingConsent(<NEW CONSENT>)`.
El SDK cambia de comportamiento de acuerdo con el nuevo consentimiento. Por ejemplo, si el consentimiento de seguimiento actual es `TrackingConsent.PENDING` y lo actualizas a:
...
| `addTag(<KEY>, <VALUE>)`     | Configura un par de etiquetas `<KEY>:<VALUE>` que se añadirán a tramos creados por el rastreador. |
