...
## Aportar mejoras a monitores y seguir las prácticas recomendadas

Monitor Quality te muestra los monitores que presentan los siguientes problemas de calidad:
...
Los [tiempos de inactividad][1] son útiles para silenciar las alertas durante los mantenimientos programados, las interrupciones planificadas o los apagados del sistema, o para detener las alertas durante los fines de semana y las noches. Sin embargo, los monitores que estuvieron silenciados, o parcialmente silenciados, durante un periodo prolongado (más de 60 días) pueden indicar un descuido. Puedes desactivar el silencio de estos monitores para reanudar la generación de alertas y garantizar una completa cobertura de monitorización.

Comprueba qué monitores no emiten alertas debido a configuraciones erróneas de los tiempos de inactividad y anula su silencio.
...
Los datos de las integraciones en la nube (como AWS, Azure o Google Cloud) se extraen de las API respectivas mediante un crawler. Estas métricas llegan con un retraso, que puedes considerar en la configuración de tu monitor. Los monitores con datos en la nube que no tienen un [retraso de evaluación][3] pueden emitir alertas sobre falsos positivos.
...
Las [`@notifications`][5] te permiten personalizar tus monitores para que las alertas se reenvíen a integraciones, flujos de trabajo o casos de Datadog. Si las `@notifications` están incorrectamente configuradas, las alertas esperadas no se envían a los canales adecuados.
