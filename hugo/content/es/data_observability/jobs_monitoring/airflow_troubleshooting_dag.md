---
description: 'Utilice el DAG de validación de OpenLineage para diagnosticar problemas
  de configuración con Data Observability: Jobs Monitoring para Airflow.'
further_reading:
- link: /data_observability/jobs_monitoring/airflow/
  tag: Documentación
  text: 'Habilite Data Observability: Jobs Monitoring para Apache Airflow'
- link: /data_jobs
  tag: Documentación
  text: 'Data Observability: Jobs Monitoring'
title: Solucione problemas de configuración de Airflow con el DAG de validación de
  OpenLineage
---
## Descripción general {#overview}

El DAG de validación de OpenLineage valida su configuración de OpenLineage en Airflow e informa cualquier problema que impida que Data Observability: Jobs Monitoring reciba eventos.

Ejecute este DAG cuando:

- Las tareas de Airflow no aparecen en la página de [Data Observability: Jobs Monitoring][1] después de completar la configuración.
- Desea verificar su configuración antes de ejecutar cargas de trabajo de producción.

El DAG verifica lo siguiente:

| Verificación | Lo que valida |
|---|---|
| Instalación del paquete | El paquete OpenLineage está instalado y se puede importar |
| Versión del paquete | Versión instalada comparada con la última disponible en PyPI |
| Accesibilidad del listener | El listener del plugin de OpenLineage puede ser cargado por Airflow |
| Proveedor activo | El proveedor de OpenLineage está activo y no está explícitamente deshabilitado |
| Configuración de transporte | Se ha configurado un transporte válido (HTTP o Datadog) |
| Punto de conexión de Datadog | La URL de transporte apunta a un punto de conexión de ingesta de Datadog |
| Conectividad de red | La conexión TCP a la URL del backend configurada es exitosa |
| Conflictos de configuración | No hay configuraciones duplicadas o conflictivas de transporte o de archivo de configuración |

## Ejecute el DAG de validación de OpenLineage {#run-the-openlineage-validation-dag}

### Requisitos previos {#prerequisites}

- Acceso a su directorio de DAGs de Airflow (una ruta local o un bucket de almacenamiento en la nube para Amazon MWAA)
- Un programador de Airflow y trabajadores en ejecución (pods o procesos)

### Agregue el archivo DAG {#add-the-dag-file}

Guarde el siguiente código como `openlineage_preflight_check_dag.py` en su directorio de DAGs de Airflow. Para Amazon MWAA, cargue el archivo en la carpeta S3 DAGs configurada para su entorno.
...
### Active el DAG {#trigger-the-dag}

Después de que el programador de Airflow detecte el nuevo archivo, active el DAG manualmente:

1. En la interfaz de usuario de Airflow, navegue a **DAGs** y busque `openlineage_preflight_check_dag`.
2. Active el DAG si está en pausa.
3. Haga clic en **Trigger DAG** (icono de reproducción).

### Visualizar los registros {#view-the-logs}

1. En la interfaz de usuario de Airflow, navegue a **DAGs > openlineage_preflight_check_dag**.
2. Haga clic en la ejecución de DAG más reciente.
3. Haga clic en la tarea **validate_setup**.
4. Seleccione la pestaña **Registros**.

## Entienda el resultado {#understand-the-output}

El DAG registra dos secciones: un resumen del entorno impreso al inicio y un resumen de validación impreso al final.

### Información del entorno {#environment-information}

La primera sección muestra lo que el DAG detectó sobre su entorno:
...
El resumen de validación aparece al final del registro de la tarea. Cada línea comienza con uno de los siguientes símbolos:

- `✓`: Verificación aprobada.
- `✗`: Verificación fallida (los eventos no se envían a Datadog).
- `!`: Advertencia (puede indicar un problema de configuración).
- `-`: Verificación omitida (falló una verificación de prerrequisitos).

La siguiente salida indica una configuración saludable:
...
### Interpretación de fallas {#interpreting-failures}

Utilice esta tabla para resolver fallas comunes:

| Mensaje de registro | Causa | Resolución |
|---|---|---|
| `✗ OpenLineage not installed properly` | El paquete OpenLineage falta o está dañado. Se omiten todas las verificaciones restantes. | Confirme que `apache-airflow-providers-openlineage` esté incluido en su instalación de Airflow. Para Amazon MWAA, consulte [Actualizar el proveedor de OpenLineage en Amazon MWAA][3]. |
| `✗ OpenLineage provider is turned off` | El proveedor está explícitamente deshabilitado por `AIRFLOW__OPENLINEAGE__DISABLED`, `OPENLINEAGE_DISABLED` o `openlineage.disabled` en `airflow.cfg`. | Elimine la configuración o establézcala en `false`. |
| `✗ OpenLineage provider is not active (no transport configuration found)` | El proveedor se desactiva a sí mismo cuando no hay una configuración de transporte presente, aunque nada lo haya deshabilitado explícitamente. | Siga la [guía de configuración de Airflow][2] para configurar un transporte. |
| `✗ Failed to resolve active transport` | No se pudo instanciar el transporte a partir de la configuración resuelta. | Verifique que la configuración del transporte sea válida y esté bien formada. Revise el registro de la tarea para ver el error subyacente. |
| `✗ Transport Type: Console (won't send events to Datadog)` | El transporte está configurado en `console`. | Cambie el transporte a `datadog` o `http` apuntando a la URL de ingesta de Datadog. |
| `! Transport does not appear to point to a Datadog endpoint` | La URL de transporte no coincide con un dominio conocido de Datadog. | Verifique que la URL apunte a un punto de conexión de ingesta de Datadog. |
| `✗ Network connectivity check failed` | Los trabajadores de Airflow no pueden acceder al punto de conexión de ingesta de Datadog. | Verifique las políticas de firewall o de red; confirme que la URL y el puerto 443 sean accesibles. |
| `✗ OpenLineage listener is not accessible` | El complemento del proveedor no se puede importar, no hay oyentes registrados o se encontraron múltiples oyentes. La causa específica se registró anteriormente en el registro de la tarea. | Confirme que el paquete esté instalado en **ambos pods o procesos del programador y del trabajador**. Si el complemento se cargó pero no hay oyentes activos, verifique la compatibilidad de la versión del proveedor. |
| `! Configuration conflicts detected` | Hay múltiples fuentes de transporte o archivos de configuración activos. | Elimine las configuraciones duplicadas y mantenga una única fuente autorizada. |

**Nota**: Si `✗ OpenLineage listener is not accessible` aparece junto con errores de instalación del paquete, es probable que el paquete solo esté instalado en el programador y no en los trabajadores. OpenLineage requiere el proveedor en ambos.

### Advertencias de versión {#version-warnings}

Si el DAG informa una advertencia de versión como:
...
Considere actualizar a la versión más reciente para beneficiarse de las correcciones de errores y una mejor compatibilidad con Datadog. Para Amazon MWAA, consulte [Actualizar el proveedor de OpenLineage en Amazon MWAA][3] para obtener instrucciones específicas de la versión.
...
[2]: /es/data_observability/jobs_monitoring/airflow/
[3]: /es/data_observability/jobs_monitoring/airflow_mwaa_upgrade/