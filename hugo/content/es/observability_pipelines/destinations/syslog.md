...
title: Destinos Syslog
---
{{< product-availability >}}

## Descripción general {#overview}

Utilice los destinos syslog de Observability Pipelines para enviar registros a rsyslog o syslog-ng.

**Nota**: Los destinos rsyslog y syslog-ng admiten el formato [RFC5424][5].

## Configuración {#setup}

<div class="alert alert-danger">Para la gestión de secretos: Solo ingrese el identificador para la URL del punto de conexión de syslog y, si corresponde, la clave de acceso. <b>No</b> ingrese los valores reales.</div>

Configure el destino rsyslog o syslog-ng cuando [configure un pipeline][2]. Puede configurar un pipeline en la [UI][1], utilizando la [API][3] o con [Terraform][4]. Los pasos de esta sección se configuran en la interfaz de usuario.

Después de seleccionar el destino rsyslog o syslog-ng en la UI del pipeline, ingrese el identificador para la URL de su punto de conexión. Si lo deja en blanco, se utiliza el [predeterminado](#secret-defaults).

Consulte [Coincidencia de campos de registro con campos de syslog](#matching-log-fields-to-syslog-fields) para obtener información sobre cómo se hacen coincidir los campos.
...
#### Tiempo de espera para sondeos de keepalive TCP {#wait-time-for-tcp-keepalive-probes}

Ingrese la cantidad de segundos a esperar antes de enviar sondeos de keepalive TCP en una conexión inactiva.

#### Almacenamiento en búfer {#buffering}

{{% observability_pipelines/destination_buffer %}}

## Coincidencia de campos de registro con campos de syslog {#matching-log-fields-to-syslog-fields}
...
- Identificador de URL del punto de conexión de rsyslog o syslog-ng:
	- Hace referencia a la dirección y el puerto a los que Observability Pipelines Worker envía los registros. Por ejemplo, `127.0.0.1:9997`.
...
Los destinos rsyslog y syslog-ng no procesan eventos por lotes.
