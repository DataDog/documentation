...
description: Configura la integración del Datadog Agent y MySQL para monitorizar una
  instancia MySQL que se ejecuta en Google CloudSQL con una configuración de conexión
  remota.
title: ¿Puedo configurar el check mysql dd-Agent en mi CloudSQL de Google?
---

Al igual que en el caso de la [integración de Amazon RDS][1] "nativa" con MySQL, puedes configurar una integración de MySQL con un Agent de Datadog para monitorizar una instancia de MySQL que se ejecute en CloudSQL de Google. Con esta configuración, puedes complementar las métricas que de otro modo obtendrías de la [integración de CloudSQL de Google][2] de Datadog con aquellas disponibles de la [integración de MySQL][3] del Agent de Datadog.

Para configurar esta integración "adicional" en tu instancia CloudSQL de Google, puedes configurar la integración de MySQL en un Agent de Datadog y [configurarla para conectarse remotamente][4] a tu instancia CloudSQL de Google, en lugar de utilizar el host local por defecto. Por lo demás, los pasos de configuración son los mismos que para cualquier otra instancia MySQL alojada localmente.
