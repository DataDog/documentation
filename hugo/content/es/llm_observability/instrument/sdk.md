...
- /es/llm_observability/instrument/custom_instrumentation
- /es/tracing/llm_observability/trace_an_llm_application
- /es/llm_observability/setup
- /es/llm_observability/instrumentation/sdk/
...
Los Agent Observability SDKs proporcionan instrumentación automática, así como API de instrumentación manual para brindar observabilidad y perspectivas en sus aplicaciones de LLM.
...
<br />Active para habilitar el envío de datos a Agent Observability. Debe establecerse en `1` o `true`.
...
: opcional - _entero o cadena_ - **predeterminado**: `false`
<br />Cuando se establece en `1` o `true`, agrega un argumento a cada herramienta de servidor MCP solicitando que el modelo que realiza la llamada describa por qué eligió llamar a la herramienta. La intención se registra en el tramo de la herramienta.
...
En lugar de usar la [configuración de línea de comandos](#command-line-setup), también puede habilitar Agent Observability mediante programación.
...
`ml_app`
: opcional - _cadena_
<br />El nombre de su aplicación, servicio o proyecto de LLM, bajo el cual se agrupan todas las trazas y tramos. Esto ayuda a distinguir entre diferentes aplicaciones o experimentos. Consulte las [pautas de nomenclatura de aplicaciones](#application-naming-guidelines) para conocer los caracteres permitidos y otras restricciones. Para anular este valor para una traza determinada, consulte [Seguimiento de múltiples aplicaciones](#tracing-multiple-applications). Si no se proporciona, el valor predeterminado es el de `DD_LLMOBS_ML_APP`.

`integrations_enabled` - **predeterminado**: `true`
: opcional - _booleano_
<br />Una marca para habilitar automáticamente el rastreo de llamadas de LLM para las [integraciones de LLM][1] compatibles con Datadog. Si no se proporciona, todas las integraciones de LLM compatibles están habilitadas de forma predeterminada. Para evitar el uso de las integraciones de LLM, establezca este valor en `false`.

`agentless_enabled`
: opcional - _booleano_ - **predeterminado**: `false`
<br />Solo es necesario si no está utilizando el Datadog Agent, en cuyo caso esto debe establecerse en `True`. Esto configura la biblioteca `ddtrace` para que no envíe ningún dato que requiera el Datadog Agent. Si no se proporciona, esto toma por defecto el valor de `DD_LLMOBS_AGENTLESS_ENABLED`.

`site`
: opcional - _cadena_
<br />El sitio de Datadog para enviar sus datos de LLM. Su sitio es {{< region-param key="dd_site" code="true" >}}. Si no se proporciona, esto toma por defecto el valor de `DD_SITE`.

`api_key`
: opcional - _cadena_
<br />Su clave de API de Datadog. Solo es necesario si no está utilizando el Datadog Agent. Si no se proporciona, esto toma por defecto el valor de `DD_API_KEY`.

`env`
: opcional - _cadena_
<br />El nombre del entorno de su aplicación (ejemplos: `prod`, `pre-prod`, `staging`). Si no se proporciona, esto toma por defecto el valor de `DD_ENV`.

`service`
: opcional - _cadena_
<br />El nombre del servicio utilizado para su aplicación. Si no se proporciona, esto toma por defecto el valor de `DD_SERVICE`.
...
`capture_intent`
: opcional - _booleano_ - **predeterminado**: `false`
<br />Cuando se establece en `True`, agrega un argumento a cada herramienta de servidor MCP solicitando que el modelo que realiza la llamada describa por qué eligió llamar a la herramienta. La intención se registra en el tramo de la herramienta. Si no se proporciona, esto toma como valor predeterminado el valor de `DD_MCP_CAPTURE_INTENT`.

[1]: /es/llm_observability/instrument/auto_instrumentation/
{{% /tab %}}
...
`agentlessEnabled`
: opcional - _booleano_ - **predeterminado**: `false`
<br />Solo es necesario si no está utilizando Datadog Agent, en cuyo caso esto debe establecerse en `true`. Esto configura la biblioteca `dd-trace` para que no envíe ningún dato que requiera el Datadog Agent. Si no se proporciona, esto toma por defecto el valor de `DD_LLMOBS_AGENTLESS_ENABLED`.
...
Para AWS Lambda, consulte [Rastrear una aplicación LLM en AWS Lambda][17].


Después de instalar el SDK y ejecutar su aplicación, debería esperar ver algunos datos en Agent Observability a partir de la instrumentación automática. La instrumentación manual se puede utilizar para capturar marcos de trabajo personalizados o operaciones de bibliotecas que aún no son compatibles.
...
El muestreo no afecta sus [métricas de Agent Observability](/llm_observability/investigate/metrics/), incluidas las [métricas de tokens y costos](/llm_observability/investigate/cost/) y otras métricas operativas. Debido a que los tramos no muestreados se descartan después de que Datadog ingiere sus trazas, estas métricas siguen basándose en el 100% del tráfico instrumentado de su aplicación, independientemente de la tasa de muestreo especificada. El muestreo de trazas también es independiente de los controles en la aplicación, como las [reglas de automatización](/llm_observability/configure/automation_rules/) y el [muestreo de trazas de APM](/tracing/trace_pipeline/ingestion_mechanisms/), que se aplican después de la ingestión.
...
Para capturar una operación de LLM, se puede utilizar un decorador de función para instrumentar flujos de trabajo fácilmente:
...
Para obtener una lista de los tipos de tramos disponibles, consulte la [documentación de Tipos de tramos][1]. Para un rastreo más detallado de las operaciones dentro de las funciones, consulte [Rastreo de tramos mediante métodos en línea](#tracing-spans-using-inline-methods).

[1]: /es/llm_observability/quickstart/terms/
{{% /tab %}}

{{% tab "Node.js" %}}

Para rastrear un tramo, utilice `llmobs.wrap(options, function)` como envoltorio de función para la función que desea rastrear. Para obtener una lista de los tipos de tramos disponibles, consulte la [documentación de Tipos de tramos][1]. Para un rastreo más detallado de las operaciones dentro de las funciones, consulte [Rastreo de tramos mediante métodos en línea](#tracing-spans-using-inline-methods).

### Tipos de tramo {#span-kinds}
...
`llmobs.wrap` extiende el comportamiento subyacente de [`tracer.wrap`][2]. El tramo subyacente creado cuando se llama a la función finaliza bajo las siguientes condiciones:

- Si la función devuelve una Promesa, el tramo finaliza cuando la promesa se resuelve o se rechaza.
- Si la función toma una devolución de llamada como su último parámetro, el tramo finaliza cuando se llama a esa devolución de llamada.
- Si la función no acepta una devolución de llamada y no devuelve una Promise, el tramo finaliza al final de la ejecución de la función.
...
Si la aplicación no utiliza la función de devolución de llamada, se recomienda utilizar un bloque de traza en línea en su lugar. Consulte [Rastreo de tramos mediante métodos en línea](#tracing-spans-using-inline-methods) para obtener más información.
...
Existen varios métodos para iniciar un tramo, según el tipo de tramo que esté iniciando. Consulte la [documentación de Tipos de tramo][1] para obtener una lista de los tipos de tramo admitidos.
...
Los tramos deben finalizarse para que la traza se envíe y sea visible en la aplicación de Datadog.
...
[1]: /es/llm_observability/quickstart/terms/#span-kinds
{{% /tab %}}
{{< /tabs >}}

### Llamadas a LLM {#llm-calls}

<div class="alert alert-info">Si utiliza algún proveedor o marco de trabajo de LLM compatible con las <a href="/llm_observability/instrument/auto_instrumentation/">integraciones de LLM de Datadog</a>, no necesita iniciar manualmente un tramo de LLM para realizar la traza de estas operaciones.</div>

<div class="alert alert-info">Si está instrumentando manualmente un tramo de LLM, debe registrar los conteos de tokens (como <code>input_tokens</code>, <code>output_tokens</code>, y <code>total_tokens</code>) usted mismo anotando el tramo. Consulte <a href="#enriching-spans">Enriquecimiento de tramos</a> para obtener más información.</div>

{{< tabs >}}
{{% tab "Python" %}}
Para trazar una llamada de LLM, utilice el decorador de función `ddtrace.llmobs.decorators.llm()`.
...
`name`
: opcional - _cadena_
<br/>El nombre de la operación. Si no se proporciona, `name` utiliza de forma predeterminada el nombre de la función rastreada.
...
`ml_app`
: opcional - _cadena_
<br/>El nombre de la aplicación de ML a la que pertenece la operación. Consulte [Trazar múltiples aplicaciones](#tracing-multiple-applications) para obtener más información.
...
{{% tab "Node.js" %}}
Para trazar una llamada a un LLM, especifique el tipo de tramo como `llm` y, opcionalmente, especifique los siguientes argumentos en el objeto de opciones.
...
{{% /tab %}}
{{% tab "Java" %}}
Para trazar una llamada a un LLM, importe y llame al siguiente método con los argumentos enumerados a continuación:
...
`spanName`
: opcional - _Cadena_
<br/>El nombre de la operación. Si no se proporciona, `spanName` utiliza de forma predeterminada el tipo de tramo.
...
`mlApp`
: opcional - _string_
<br/>El nombre de la aplicación de ML a la que pertenece la operación. Proporcionar un valor no nulo anula el nombre de la aplicación de ML proporcionado al inicio de la aplicación. Consulte [Trazar múltiples aplicaciones](#tracing-multiple-applications) para obtener más información.
...
{{< tabs >}}
{{% tab "Python" %}}
Para trazar un tramo de flujo de trabajo, utilice el decorador de función `ddtrace.llmobs.decorators.workflow()`.
...
Para trazar un tramo de flujo de trabajo, especifique el tipo de tramo como `workflow` y, opcionalmente, especifique los argumentos en el objeto de opciones.
...
{{< tabs >}}
{{% tab "Python" %}}
Para realizar el seguimiento de una ejecución de agente, utilice el decorador de función `ddtrace.llmobs.decorators.agent()`.
...
{{% tab "Node.js" %}}
Para rastrear una ejecución de agente, especifique el tipo de tramo como `agent` y, opcionalmente, especifique los argumentos en el objeto de opciones.
...
{{% /tab %}}
{{% tab "Java" %}}
Para rastrear una ejecución de agente, importe y llame al siguiente método con los argumentos enumerados a continuación
...
`spanName`
: opcional - _Cadena_
<br/>El nombre de la operación. Si no se proporciona, `spanName` utiliza de forma predeterminada el nombre de la función rastreada.
...
{{< tabs >}}
{{% tab "Python" %}}
Para rastrear una llamada a herramienta, utilice el decorador de función `ddtrace.llmobs.decorators.tool()`.
...
{{% tab "Node.js" %}}
Para rastrear una llamada a herramienta, especifique el tipo de tramo como `tool` y, opcionalmente, especifique los argumentos en el objeto de opciones.
...
## Tramos anidados {#nesting-spans}

Iniciar un nuevo tramo antes de que finalice el tramo actual rastrea automáticamente una relación padre-hijo entre ambos tramos. El tramo principal representa la operación más grande, mientras que el tramo secundario representa una suboperación anidada más pequeña dentro de esta.
...
<br />El tramo que se va a anotar. Si `span` no se proporciona (como cuando se usan decoradores de funciones), el SDK anota el tramo activo actual.
...
`tags`
: opcional - _diccionario_
<br />Un diccionario de pares clave-valor serializables en JSON que los usuarios pueden agregar como etiquetas en el tramo. Ejemplos de claves: `session`, `env`, `system` y `version`. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).

`cost_tags`
: opcional - _lista de cadenas_
<br />Una lista de claves de etiqueta (ya establecidas con `tags` o anotadas previamente en el mismo tramo) para propagar como etiquetas personalizadas en las métricas de costo y tokens de LLM generadas. Las entradas que no hacen referencia a una clave de etiqueta existente se omiten. Consulte [Supervisión de costos](#cost-monitoring) para obtener más detalles.
...
`span`
: opcional - _Tramo_ - **predeterminado**: el tramo activo actual
<br />El tramo que se va a anotar. Si `span` no se proporciona (como cuando se usan envoltorios de funciones), el SDK anota el tramo activo actual.
...
`tags`
: opcional - _objeto_
<br />Un objeto de pares clave-valor serializables en JSON que los usuarios pueden agregar como etiquetas relacionadas con el contexto del tramo (`session`, `environment`, `system`, `versioning`, etc.). Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).

`costTags`
: opcional - _matriz de cadenas_
<br />Una lista de claves de etiqueta (ya establecidas con `tags` o anotadas previamente en el mismo tramo) para propagar como etiquetas personalizadas en las métricas de costo y tokens de LLM generadas. Las entradas que no hacen referencia a una clave de etiqueta existente se omiten. Consulte [Supervisión de costos](#cost-monitoring) para obtener más detalles.
...
Utilice el método miembro `annotateIO()` de la interfaz `LLMObsSpan` para agregar datos estructurados de entrada y salida a un `LLMObsSpan`. Esto incluye argumentos opcionales y objetos de mensaje de LLM.
...
<br />Ya sea una cadena (para tramos que no son de LLM) o una lista de `LLMObs.LLMMessage` para tramos de LLM.

`outputData`
: opcional - _String_ o _List<LLMObs.LLMMessage>_
<br />Ya sea una cadena (para tramos que no son de LLM) o una lista de `LLMObs.LLMMessage` para tramos de LLM.

#### Mensajes de LLM {#llm-messages}
Los tramos de LLM deben anotarse con mensajes de LLM utilizando el objeto `LLMObs.LLMMessage`.

El objeto `LLMObs.LLMMessage` se puede instanciar llamando a `LLMObs.LLMMessage.from()` con los siguientes argumentos:
...
Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags][1].

#### Agregar etiquetas en bloque {#bulk-add-tags}

El método miembro `setTags()` de la interfaz `LLMObsSpan` acepta los siguientes argumentos para adjuntar varias etiquetas en bloque:
...
#### Establecer una marca de error {#setting-an-error-flag}
...
El método `LLMObs.annotation_context()` del SDK devuelve un administrador de contexto que se puede utilizar para modificar todos los tramos auto-instrumentados iniciados mientras el contexto de anotación está activo.
...
`prompt`
: opcional - _diccionario_
<br />Un diccionario que representa el prompt utilizado para una llamada a LLM. Consulte la documentación del [objeto Prompt](#prompt-tracking-arguments) para obtener el esquema completo y las claves admitidas. También puede importar el objeto `Prompt` desde `ddtrace.llmobs.utils` y pasarlo como el argumento `prompt`. **Nota**: Este argumento solo se aplica a los tramos de LLM.
...
`cost_tags`
: opcional - _lista de cadenas_
<br />Una lista de claves de etiqueta para propagar como etiquetas personalizadas en las métricas de costo y tokens de LLM generadas. Cada entrada debe hacer referencia a una clave presente en `tags` al inicio del tramo (proporcionada al mismo contexto o a un contexto principal); las claves de etiqueta agregadas posteriormente con `LLMObs.annotate()` no se conservan. Consulte [Supervisión de costos](#cost-monitoring) para obtener más detalles.
...
El `llmobs.annotationContext()` del SDK acepta una función de devolución de llamada que se puede utilizar para modificar todos los tramos autoinstrumentados iniciados mientras se está dentro del contexto de la función de devolución de llamada.
...
`tags`
: opcional - _objeto_
<br />Un objeto de pares clave-valor serializables en JSON que los usuarios pueden agregar como etiquetas en el tramo. Ejemplos de claves: `session`, `env`, `system` y `version`. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).
...
Adjunte metadatos de prompt estructurados al tramo del LLM para que pueda reproducir resultados, auditar cambios y comparar el rendimiento de los prompts entre versiones. Al usar plantillas, Agent Observability también proporciona [seguimiento de versiones](#version-tracking) basado en cambios en el contenido de la plantilla.

{{< tabs >}}
{{% tab "Python" %}}
Use `LLMObs.annotation_context(prompt=...)` para adjuntar metadatos de prompt antes de la llamada al LLM. Para obtener más detalles sobre la anotación de tramos, consulte [Enriquecimiento de tramos](#enriching-spans).
...
- `id` (str): Identificador lógico para este prompt. Debe ser único por `ml_app`. Se establece de forma predeterminada en `{ml_app}-unnamed_prompt`
...
- `chat_template` (List[Message]): Formato de plantilla de mensajes múltiples. Proporcione una lista de `{ "role": "<role>", "content": "<template string with placeholders>" }` objetos.
- `tags` (Dict[str, str]): Etiquetas para adjuntar a la ejecución del prompt.
- `rag_context_variables` (List[str]): Claves de variables que contienen contenido de ground-truth/contexto. Se utiliza para la [detección de alucinaciones](/llm_observability/investigate/evaluations/llm_as_a_judge_evaluations/template_evaluations#hallucination).
- `rag_query_variables` (List[str]): Claves de variables que contienen la consulta del usuario. Se utiliza para la [detección de alucinaciones](/llm_observability/investigate/evaluations/llm_as_a_judge_evaluations/template_evaluations#hallucination).
...
Cuando utilice la creación de plantillas de prompt de LangChain con auto-instrumentación, asigne nombres significativos a las variables de las plantillas. La auto-instrumentación utiliza estos nombres para identificar los prompts.
...
Use `llmobs.annotationContext({ prompt: ... }, () => { ... })` para adjuntar metadatos de prompt antes de la llamada al LLM. Para obtener más detalles sobre la anotación de tramos, consulte [Enriquecimiento de tramos](#enriching-spans).
...
- `id` (string): Identificador lógico para este prompt. Debe ser único por `ml_app`. Se establece de forma predeterminada en `{ml_app}-unnamed_prompt`
...
- `template` (string | List[Message]): Template string with placeholders (for example, `"Translate {{text}} a {{lang}}\"`). Alternatively, a list of `{ \"role\": \"<role>\", \"content\": \"<template string with placeholders>\" }` objetos.
- `tags` (Record<string, string>): Etiquetas para adjuntar a la ejecución del prompt.
- `contextVariables` (string[]): Claves de variables que contienen contenido de ground-truth/contexto. Se utiliza para la [detección de alucinaciones](/llm_observability/investigate/evaluations/llm_as_a_judge_evaluations/template_evaluations#hallucination).
- `queryVariables` (string[]): Claves de variables que contienen la consulta del usuario. Se utiliza para la [detección de alucinaciones](/llm_observability/investigate/evaluations/llm_as_a_judge_evaluations/template_evaluations#hallucination).
...
- Para múltiples llamadas a LLM auto-instrumentadas dentro de un bloque, utilice un contexto de anotación para aplicar los mismos metadatos de prompt en todas las llamadas. Consulte [Anotación de tramos auto-instrumentados](#annotating-auto-instrumented-spans).

### Seguimiento de versiones {#version-tracking}

Agent Observability proporciona versiones automáticas para sus prompts cuando no se especifica una versión explícita. Cuando proporciona un `template` o `chat_template` en los metadatos de su prompt sin una etiqueta `version`, el sistema genera automáticamente una versión calculando un hash del contenido de la plantilla. Si proporciona una etiqueta `version`, Agent Observability utiliza la etiqueta de versión que especificó en lugar de generar una automáticamente.

El sistema de versiones funciona de la siguiente manera:
- **Versiones automáticas**: Cuando no se proporciona una etiqueta `version`, Agent Observability calcula un hash del contenido de `template` o `chat_template` para generar automáticamente un identificador de versión numérico
- **Versiones manuales**: Cuando se proporciona una etiqueta `version`, Agent Observability utiliza la etiqueta de versión que especificó exactamente como se proporcionó
- **Historial de versiones**: Tanto las versiones generadas automáticamente como las manuales se mantienen en el historial de versiones para realizar un seguimiento de la evolución de los prompts a lo largo del tiempo

Esto le brinda la flexibilidad de confiar en la gestión automática de versiones basada en cambios en el contenido de la plantilla, o mantener un control total sobre el versionado con sus propias etiquetas de versión.

## Captura de intención de MCP {#mcp-intent-capture}

Para obtener información sobre por qué se llamaron sus herramientas MCP, habilite la captura de intención en su servidor MCP. Cuando se habilita, el SDK agrega un argumento a cada herramienta del servidor MCP solicitando que el modelo que realiza la llamada describa por qué eligió llamar a la herramienta. La intención se registra en el tramo de la herramienta, lo que le ayuda a mejorar las definiciones y descripciones de sus herramientas.
...
Para obtener más información sobre lo que representa cada token y cómo los calcula Datadog, consulte [Cómo se calculan los recuentos de tokens][16].
...
Para modelos personalizados o no admitidos, debe anotar el tramo manualmente con los datos de costo en dólares.
...
El Agent Observability SDK proporciona métodos para exportar y enviar sus evaluaciones a Datadog.

<div class="alert alert-info">Para crear evaluadores reutilizables basados en clases (<code>BaseEvaluator</code>, <code>BaseSummaryEvaluator</code>) con metadatos de resultados enriquecidos, consulte la <a href="/llm_observability/investigate/evaluations/evaluation_developer_guide/">Guía para desarrolladores de evaluaciones</a>.</div>

Las evaluaciones deben unirse a un solo tramo. Puede identificar el tramo de destino utilizando cualquiera de estos dos métodos:
- _Unión basada en etiquetas_ - Una evaluación utilizando un par clave-valor de etiqueta único que se establece en un solo tramo. La evaluación no se unirá si el par clave-valor de la etiqueta coincide con varios tramos o con ninguno.
- _Referencia directa al tramo_ - Una evaluación se une utilizando la combinación única del ID de traza y del ID de tramo del tramo.

### Exportación de un tramo {#exporting-a-span}
{{< tabs >}}
{{% tab "Python" %}}
`LLMObs.export_span()` se puede utilizar para extraer el contexto del tramo de un tramo. Este método es útil para asociar su evaluación con el tramo correspondiente.
...
`span`
: opcional - _Tramo_
<br />El tramo del cual extraer el contexto (ID de traza y ID de tramo). Si no se proporciona (como al usar decoradores de funciones), el SDK exporta el tramo activo actual.
...
{{% tab "Node.js" %}}
`llmobs.exportSpan()` se puede utilizar para extraer el contexto del tramo de un tramo. Necesitará usar este método para asociar su evaluación con el tramo correspondiente.
...
`span`
: opcional - _Tramo_
<br />El tramo del cual extraer el contexto (ID de traza y ID de tramo). Si no se proporciona (como al usar envoltorios de funciones), el SDK exporta el tramo activo actual.
...
<div class="alert alert-info"><code>LLMObs.submit_evaluation_for</code> está obsoleto y se eliminará en la próxima versión principal de ddtrace (4.0). Para migrar, cambie el nombre de sus <code>LLMObs.submit_evaluation_for</code> llamadas con <code>LLMObs.submit_evaluation</code>.</div>

**Nota**: Las evaluaciones personalizadas son evaluadores que usted implementa y aloja por su cuenta. Estas difieren de las evaluaciones listas para usar, las cuales son calculadas automáticamente por Datadog utilizando evaluadores integrados. Para configurar las evaluaciones listas para usar para su aplicación, utilice la página [**Agent Observability** > **Settings** > **Evaluations**][1] en Datadog.
...
`timestamp_ms`
: opcional - _entero_
<br />La marca de tiempo unix en milisegundos cuando se generó el resultado de la métrica de evaluación. Si no se proporciona, esto toma como valor predeterminado la hora actual.

`tags`
: opcional - _diccionario_
<br />Un diccionario de pares clave-valor de cadena que los usuarios pueden agregar como etiquetas con respecto a la evaluación. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).
...
<br />El contexto del tramo con el que asociar la evaluación. Este debería ser el resultado de `LLMObs.export_span()`.
...
`metricType`
: obligatorio - _cadena_
<br />El tipo de evaluación. Debe ser uno de "categorical", "score", "boolean" o "json".

`value`
: obligatorio - _cadena o tipo numérico_
<br />El valor de la evaluación. Debe ser una cadena (para `metric_type` categórica), un número (para `metric_type` puntuación), un booleano (para `metric_type` booleana) o un objeto JSON (para `metric_type` json).
...
Use `LLMObs.SubmitEvaluation()` para enviar su evaluación personalizada asociada con un tramo determinado.
...
<br />Un diccionario de pares clave-valor de cadena utilizados para etiquetar la evaluación. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).
{{% /collapse-content %}}
...
### Envío de retroalimentación del usuario final {#submitting-end-user-feedback}

La retroalimentación del usuario final captura las aportaciones de los usuarios de su aplicación LLM, como calificaciones de pulgar hacia arriba o hacia abajo, si un usuario aceptó el cambio de un agent y comentarios de texto libre. A diferencia de una evaluación, la retroalimentación lleva la identidad de quien la envía y puede dirigirse a un tramo, una traza, una sesión o una entidad definida por el cliente. Para obtener más información, consulte [End-User Feedback](/llm_observability/evaluations/end_user_feedback/).

{{< tabs >}}
{{% tab "Python" %}}
Utilice `LLMObs.submit_feedback()` para enviar la retroalimentación del usuario final asociada con un tramo, traza, sesión o entidad definida por el cliente.
...
`session_id`
: opcional - _cadena_
<br />El ID de la sesión asociada con esta retroalimentación.

`feedback_join_key`
: opcional - _cadena_
<br />Una clave definida por el cliente asociada con esta retroalimentación, como un ID de incidente o un ID de ticket. Para conectar la retroalimentación con sus tramos, primero anótelos con una etiqueta `feedback_join_key` que contenga el mismo valor. Consulte [Enriquecimiento de tramos](#enriching-spans).
...
`ml_app`
: opcional - _cadena_
<br />El nombre de la aplicación de ML. Si no se proporciona, esto toma como valor predeterminado la aplicación de ML configurada para el SDK.

`timestamp_ms`
: opcional - _entero_
<br />La marca de tiempo Unix en milisegundos cuando se generó la retroalimentación. Si no se proporciona, esto toma como valor predeterminado la hora actual.

`tags`
: opcional - _diccionario_
<br />Un diccionario de pares clave-valor de string que los usuarios pueden agregar como etiquetas con respecto a la retroalimentación. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).

`assessment`
: opcional - _cadena_
<br />Una valoración de esta retroalimentación. Los valores aceptados son `pass` y `fail`.
...
{{% tab "Node.js" %}}
Utilice `llmobs.submitFeedback()` para enviar la retroalimentación del usuario final asociada con un tramo, traza, sesión o entidad definida por el cliente.
...
`submitter`
: requerido - _objeto_
<br />Un object que identifica quién envió la retroalimentación. Debe contener un `id` (cadena) que no esté vacío y puede contener un `type` (cadena) opcional, como `user`.

`span`
: opcional - _objeto_
<br />El contexto del tramo al que se adjuntará la retroalimentación. Esto debería ser el resultado de [`llmobs.exportSpan()`](#exporting-a-span).

`spanId`
: opcional - _cadena_
<br />El ID del tramo al que se adjuntará la retroalimentación.

`traceId`
: opcional - _cadena_
<br />El ID de la traza al que se adjuntará la retroalimentación.
...
`feedbackJoinKey`
: opcional - _cadena_
<br />Una clave definida por el cliente a la que se adjuntará la retroalimentación, como un ID de incidente o un ID de ticket. Establezca la misma clave en sus tramos para conectar la retroalimentación a ellos.
...
`tags`
: opcional - _objeto_
<br />Un object de pares clave-valor de string que los usuarios pueden agregar como etiquetas con respecto a la retroalimentación. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).
...
{{% tab "Java" %}}
Utilice `LLMObs.submitFeedback()` para enviar la retroalimentación del usuario final asociada con un tramo, traza, sesión o entidad definida por el cliente. Cree la retroalimentación con `LLMObs.Feedback.builder()`.
...
: Obligatorio
<br />Identifica quién envió la retroalimentación. El `id` debe ser una cadena no vacía. El `type` es un calificador opcional, como `user`.

`span(LLMObsSpan span)`, `spanId(String)`, `traceId(String)`, `sessionId(String)` o `feedbackJoinKey(String)`
: Obligatorio
<br />La entidad a la que se adjuntará la retroalimentación. Establezca exactamente uno de estos métodos. Use `feedbackJoinKey` para una entidad definida por el cliente, como un ID de incidente o un ID de ticket, y establezca la misma clave en sus tramos para conectar la retroalimentación con ellos.
...
: opcional
<br />Pares de clave-valor utilizados para etiquetar los comentarios. Para obtener más información sobre las etiquetas, consulte [Getting Started with Tags](/getting_started/tagging/).

`assessment(Assessment assessment)`
: opcional
<br />Una valoración de esta retroalimentación. Los valores aceptados son `LLMObs.Feedback.Assessment.PASS` y `LLMObs.Feedback.Assessment.FAIL`.
...
**Nota**: `LLMObs.submitFeedback()` valida los comentarios y arroja una `IllegalArgumentException` cuando la Agent Observability está habilitada y los comentarios no son válidos, como cuando faltan el objetivo, el valor o el remitente. Cuando Agent Observability está deshabilitada, o el Agent no está conectado, la llamada es una no-op.
...
Para modificar los datos de entrada y salida en los tramos, puede configurar una función de procesador. La función de procesador tiene acceso a las etiquetas de tramo para permitir la modificación condicional de entrada/salida. Las funciones de procesador pueden devolver el tramo modificado para emitirlo, o devolver `None`/`null` para evitar que el tramo se emita por completo. Esto es útil para filtrar tramos que contienen datos confidenciales o que cumplen con ciertos criterios.
...
Cuando se utiliza la instrumentación automática, el span no siempre es accesible contextualmente. Para modificar condicionalmente las entradas y salidas en tramos instrumentados automáticamente, se puede utilizar `annotation_context()` además de un procesador de tramos.
...
Cuando se utiliza la instrumentación automática, el span no siempre es accesible contextualmente. Para modificar condicionalmente las entradas y salidas en tramos instrumentados automáticamente, se puede utilizar `llmobs.annotationContext()` además de un procesador de tramos.
...
|---|---|
| `session_id` | El ID que representa una sesión de usuario única, por ejemplo, una sesión de chat. |
...
El SDK admite el trazado a través de servicios o hosts distribuidos. El trazado distribuido funciona propagando información de tramo a través de solicitudes web.

{{< tabs >}}
{{% tab "Python" %}}

La biblioteca `ddtrace` proporciona algunas integraciones listas para usar que admiten el trazado distribuido para bibliotecas populares de [marco web][1] y [HTTP][2]. Si su aplicación realiza solicitudes utilizando estas bibliotecas compatibles, puede habilitar el trazado distribuido ejecutando:
{{< code-block lang="python">}}
...
El método `LLMObs.inject_distributed_headers()` toma un tramo e inyecta su contexto en los encabezados HTTP para incluirlo en la solicitud. Este método acepta los siguientes argumentos:

`request_headers`
: obligatorio - _diccionario_
<br />Los encabezados HTTP que se deben extender con atributos de contexto de traza.

`span`
: opcional - _tramo_ - **default**: `The current active span.`
<br />El tramo para inyectar su contexto en los encabezados de solicitud proporcionados. Cualquier tramo (incluidos aquellos con decoradores de función), esto utiliza de forma predeterminada el tramo activo actual.

### Activación de encabezados distribuidos {#activating-distributed-headers}

El método `LLMObs.activate_distributed_headers()` toma los encabezados HTTP y extrae los atributos de contexto de traza para activarlos en el nuevo servicio.

**Nota**: Debe llamar a `LLMObs.activate_distributed_headers()` antes de iniciar cualquier tramo en su servicio descendente. Los tramos iniciados anteriormente (incluidos los tramos de decoradores de función) no se capturan en la traza distribuida.

Este método acepta el siguiente argumento:

`request_headers`
: requerido - _diccionario_
<br />Los encabezados HTTP para extraer atributos de contexto de traza.
...
La biblioteca `dd-trace` proporciona integraciones listas para usar que admiten el trazado distribuido para [marcos web][1] populares. Requerir el tracer habilita automáticamente estas integraciones, pero puede deshabilitarlas opcionalmente con:
...
Para cada tipo de tramo, la clase `ddtrace.llmobs.LLMObs` proporciona un método en línea correspondiente para trazar automáticamente la operación que conlleva un bloque de código determinado. Estos métodos tienen la misma firma de argumentos que sus contrapartes decoradoras de funciones, con la adición de que `name` toma como valor predeterminado el tipo de tramo (`llm`, `workflow`, etc.) si no se proporciona. Estos métodos se pueden utilizar como administradores de contexto para finalizar automáticamente el tramo una vez completado el bloque de código incluido.
...
Para iniciar y detener manualmente un tramo en diferentes contextos o ámbitos:
...
### Rastreo de múltiples aplicaciones {#tracing-multiple-applications}
...
{{% tab "Node.js" %}}
### Rastreo de tramos mediante métodos en línea {#tracing-spans-using-inline-methods-1}

El `llmobs` SDK proporciona un método en línea correspondiente para rastrear automáticamente la operación que conlleva un bloque de código determinado. Estos métodos tienen la misma firma de argumento que sus contrapartes de envoltura de función, con la adición de que `name` es obligatorio, ya que el nombre no se puede inferir de una devolución de llamada anónima. Este método finalizará el tramo bajo las siguientes condiciones:

- Si la función devuelve una Promesa, el tramo finaliza cuando la promesa se resuelve o se rechaza.
- Si la función toma una devolución de llamada como su último parámetro, el tramo finaliza cuando se llama a esa devolución de llamada.
- Si la función no acepta una devolución de llamada y no devuelve una Promesa, el tramo finaliza al final de la ejecución de la función.
...
El Agent Observability SDK para Node.js ofrece una `llmobs.decorate` función que sirve como decorador de funciones para aplicaciones TypeScript. El comportamiento de rastreo de esta función es el mismo que `llmobs.wrap`.
...
### Rastreo de múltiples aplicaciones {#tracing-multiple-applications-1}
...
[8]: /es/llm_observability/quickstart/terms/
...
[13]: /es/llm_observability/instrument/auto_instrumentation/
[14]: /es/llm_observability/investigate/cost
[15]: /es/llm_observability/investigate/cost/#custom-tags-on-cost-and-tokens-metrics
[16]: /es/llm_observability/investigate/cost/#how-token-counts-are-calculated
[17]: /es/llm_observability/guide/trace-an-llm-application-in-aws-lambda