...
- /es/opentelemetry/setup/collector_exporter/pure_otlp_collector/
- /es/opentelemetry/setup/collector_exporter/install/
...
- link: /opentelemetry/config/log_collection
  tag: Documentación
  text: Configure la recopilación de registros
- link: /opentelemetry/config/collector_batch_memory
  tag: Documentación
  text: Configure los límites de memoria
- link: /opentelemetry/config/otlp_receiver
  tag: Documentación
  text: Habilite el receptor OTLP
...
title: Configure el OpenTelemetry Collector
---
## Descripción general {#overview}

Envíe trazas, métricas y registros a Datadog utilizando el OpenTelemetry Collector. Las configuraciones en esta página se prueban con la distribución OpenTelemetry Collector Contrib v0.154.0 y utilizan una canalización de telemetría basada en OTLP con los siguientes componentes clave:

- **Exportador HTTP de OTLP**: Envía telemetría a los puntos de conexión de ingesta de OTLP de Datadog.
- **Conector de métricas de tramo**: Genera métricas RED (tasa, error, duración) a partir de datos de trazas para potenciar funciones de APM como el Service Catalog y la página de servicio.
...
- **Datadog extension**: Informa la configuración del Collector a Datadog para Fleet Automation. No exporta datos de telemetría.

Esta es la configuración recomendada para un OpenTelemetry Collector que usted mismo administra. Si desea una distribución de OpenTelemetry Collector que Datadog mantenga y admita, utilice en su lugar el [Datadog Distribution of OTel Collector (DDOT)][12].

{{< img src="/opentelemetry/setup/oss-collector.png" alt="Un SDK de OpenTelemetry envía datos de OTLP a un OpenTelemetry Collector, el cual los exporta a Datadog a través de OTLP HTTP." style="width:100%;" >}}

Las configuraciones en esta página utilizan el [patrón de implementación de agente][10]: un OpenTelemetry Collector se ejecuta en cada servidor o nodo de Kubernetes y recibe telemetría de las cargas de trabajo en ese servidor o nodo. Para una implementación de puerta de enlace, consulte el [patrón de implementación de puerta de enlace][11] de OpenTelemetry. El procesamiento con estado, como el muestreo basado en seguimiento de las últimas líneas en un entorno de múltiples OpenTelemetry Collector, requiere una arquitectura de puerta de enlace que dirija todos los tramos de una traza al mismo OpenTelemetry Collector.

Para obtener métricas de Kubernetes en todo el clúster y el Explorador de Kubernetes, consulte [Kubernetes Metrics][13].

<div class="alert alert-info">¿Ya utiliza el Datadog Exporter y el Datadog Connector? Consulte <a href="/opentelemetry/setup/collector_exporter/datadog_exporter/">Configure el Datadog Exporter y el Connector</a>.</div>
...
Esta configuración no es compatible con entornos de ejecución de contenedores sin servidor o basados en tareas, como ECS Fargate, EKS Fargate o AWS Lambda. Para conocer las funciones de Datadog compatibles, consulte la [tabla de compatibilidad de funciones][7] en **OTel SDK + Upstream OpenTelemetry Collector**.
...
### 2. Configure e implemente el OpenTelemetry Collector {#2-configure-and-deploy-the-collector}

Seleccione la pestaña para su entorno.

{{< tabs >}}
{{% tab "Servidor" %}}

Para un OpenTelemetry Collector sin contenedores que se ejecute directamente en un servidor (bare metal o VM), cree un archivo llamado `collector.yaml` con esta configuración.
...
Para un OpenTelemetry Collector en contenedor, cree un archivo llamado `collector.yaml` con esta configuración. El receptor `host_metrics` requiere montar el sistema de archivos del servidor en `/hostfs`.
...
Cree una red de Docker para que el OpenTelemetry Collector y los contenedores de su aplicación la compartan, o utilice una existente:

```shell
docker network create otel
```

Ejecute el Collector con el sistema de archivos del servidor montado, conectado a esa red:
...
Utilice el [chart de Helm oficial de OpenTelemetry Collector][102] para implementar el Collector como un DaemonSet en Kubernetes. Los archivos de valores de ejemplo se prueban con el chart v0.147.1, fijan el OpenTelemetry Collector a la v0.154.0 y configuran los montajes, las variables de entorno, los recursos RBAC y la exposición de puertos necesarios.

Para recopilar métricas de todo el clúster o enviar datos de recursos de Kubernetes a Kubernetes Explorer, implemente un OpenTelemetry Collector de clúster junto con el DaemonSet. Consulte [Kubernetes Metrics][13].
...
{{% tab "Referencia de manifiesto de Kubernetes" %}}

Esta pestaña es una referencia de configuración para usuarios que mantienen sus propios manifiestos de Kubernetes. Para una instalación ejecutable que incluya la especificación de pod, montajes, variables de entorno, recursos RBAC y exposición de puertos requeridos, utilice la pestaña **Kubernetes**.

La configuración ejecuta el OpenTelemetry Collector como un DaemonSet en un entorno que no es de nube. Incluye el procesador `k8s_attributes` para enriquecer la telemetría con metadatos de Kubernetes y el receptor `kubelet_stats` para métricas de nodos, pods, contenedores y volúmenes. En una distribución de Kubernetes administrada, aplique los cambios descritos en [Distribuciones de Kubernetes administradas](#managed-kubernetes-distributions) después de la configuración.
...
- Deshabilite los puertos de servidor en el Collector y expóngalo a través de un servicio local del nodo en su lugar. Los archivos de valores de Helm de GKE Autopilot y AKS Automatic aplican estos cambios. Apunte las aplicaciones a ese Service en lugar de a la IP del servidor que se muestra en [Configure su aplicación](#3-configure-your-application).
...
### 3. Configure su aplicación {#3-configure-your-application}
...
```
Esto utiliza el valor `--name otelcol` del comando `docker run`. Ejecute su contenedor de aplicación en la misma red (`--network otel`). Si utiliza Docker Compose, ambos contenedores comparten una red automáticamente y puede utilizar el nombre de servicio del OpenTelemetry Collector.
{{% /tab %}}

{{% tab "Kubernetes" %}}
Para entornos de Kubernetes donde los valores de Helm habilitan puertos de servidor, configure el punto de conexión utilizando la IP del servidor:
...
GKE Autopilot y AKS Automatic no permiten puertos de servidor. Sus archivos de ejemplo de valores de Helm habilitan un servicio local al nodo en su lugar. Encuentre el nombre del servicio con `kubectl get services` y configure el punto de conexión utilizando su nombre DNS de Kubernetes (por ejemplo, `http://otelcol-opentelemetry-collector:4318`).
{{% /tab %}}
{{< /tabs >}}
...
3. Vaya a {{< ui >}}Infrastructure{{< /ui >}} > {{< ui >}}Infrastructure List{{< /ui >}} y confirme que aparece el servidor que ejecuta el OpenTelemetry Collector. En GKE Autopilot y AKS Automatic, los archivos de valores de ejemplo omiten el receptor `host_metrics`, por lo que los servidores no aparecen.
4. Si envía registros a través de OTLP, vaya a {{< ui >}}Logs Explorer{{< /ui >}} y busque el nombre de su servicio.
5. En [Fleet Automation][9], confirme que el OpenTelemetry Collector y su configuración aparecen.
...
Cada configuración específica del entorno en [Configure y despliegue el OpenTelemetry Collector](#2-configure-and-deploy-the-collector) incluye el bloque de conector `span_metrics` completo. Conserve todas sus dimensiones al adaptar la configuración para que Datadog pueda derivar las etiquetas de servidor, los servicios pares, los nombres de operación y los nombres de recursos necesarios.

Los [archivos de configuración completos][5] también muestran cómo reemplazar grupos de dimensiones de etiquetas de contenedor con patrones glob, como `- glob: container.**`.
...
<div class="alert alert-info">La configuración recomendada del OpenTelemetry Collector utiliza el <code>span_metrics</code> conector para generar las métricas RED que potencian las vistas de APM. El <code>trace_metrics.instrumentation_metrics_calc</code> y <code>raw_instrumentation_metrics_drop</code> los campos admiten una configuración alternativa para configuraciones que derivan métricas de trazas de APM a partir de métricas de instrumentación HTTP en su lugar. No habilite <code>instrumentation_metrics_calc</code> junto con el <code>span_metrics</code> conector, ya que esto calcula métricas de trazas de ambas fuentes.</div>

### Conector de reenvío de trazas {#trace-forward-connector}

El conector `forward/traces_sample` divide el procesamiento de trazas en dos canalizaciones. La primera canalización envía cada tramo al conector `span_metrics`, y la segunda exporta los tramos a Datadog. Agregue procesadores de muestreo a la segunda canalización para que las métricas de trazas se calculen a partir de todas las trazas, no solo de las muestreadas.

### Extensión de Datadog{#datadog-extension}

La extensión `datadog` informa la configuración del OpenTelemetry Collector a Datadog para Fleet Automation. No exporta datos de telemetría. Toda la telemetría fluye a través del exportador HTTP de OTLP. Esta extensión es parte del proyecto [OpenTelemetry Collector Contrib][1] y maneja la validación de claves de API y la generación de informes de tipo de implementación.
...
[12]: /es/opentelemetry/setup/ddot_collector/install/
[13]: /es/opentelemetry/integrations/kubernetes_metrics/
