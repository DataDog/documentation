...
{{< img src="watchdog/alerts/alerts_overview.png" alt="Captura de pantalla de una tarjeta de alerta de Watchdog que muestra una tasa de error elevada del endpoint send-sms en sms-service" style="width:100%;">}}
...
| Entorno         | Entorno desde el cual mostrar alertas. Consulta el [etiquetado de servicios unificado][5] para obtener más información sobre la etiqueta `env`.|
| Servicio             | Servicio desde el cual mostrar alertas. Consulta el [etiquetado de servicios unificado][5] para obtener más información sobre la etiqueta `service`.|
| Usuario final afectado   | (Requiere RUM). Si Watchdog ha encontrado usuarios finales afectados. Consulta el [análisis de los impactos][4] para obtener más información. |
| Causa raíz          | (Requiere APM). Si Watchdog ha encontrado la causa raíz de la anomalía o del error crítico. Consulta el [análisis de las causas raíz][9] para obtener más información. |
...
Los logs ingeridos se analizan a nivel del consumo, donde Watchdog realiza agregados sobre los patrones detectados, así como etiquetas `environment`, `service`, `source` y `status`.
...
Watchdog requiere algunos datos para establecer una referencia del comportamiento esperado. En el caso de las anomalías en logs, el historial mínimo es de 24 horas. 
...
Para desactivar la detección de anomalías en logs, ve a la página del [pipeline de Log Management][4] y haz clic en el conmutador Log Anomalies (Anomalías en logs).
...
Watchdog empieza a encontrar anomalías una vez que dispone del historial mínimo requerido y mejora a medida que este crece. Los mejores resultados se obtienen con seis semanas de historial.
...
Cuando Watchdog detecta una irregularidad en una métrica de APM, el icono rosa de los prismáticos de Watchdog aparece junto al servicio afectado en el [catálogo de software de APM][7].

{{< img src="watchdog/service_list.png" alt="Captura de pantalla del catálogo de software que muestra cinco servicios. Un icono rosa de prismáticos sigue al nombre del servicio de almacenes web." style="width:75%;" >}}

Puedes ver más detalles sobre anomalías en métricas si te desplazas hasta la parte superior de una [página de servicios][3] con el carrusel de [Watchdog Insights][8].
