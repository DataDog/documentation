...
<div class="alert alert-info">Al usar el Datadog Agent, Kubernetes Explorer requiere el Agent 7.27.0+ y el Cluster Agent 1.11.0+. Si está utilizando Kubernetes 1.25+, entonces se requiere el Cluster Agent 7.40.0+.</div>
...
El receptor de código abierto `k8sobjects` puede generar una carga significativa en el servidor de API de Kubernetes de un clúster.
...
- La detección del nombre del clúster depende de su proveedor de nube. Consulte la [documentación del procesador `resourcedetection`][8] para conocer los proveedores compatibles (EKS, AKS, GCP) y los permisos necesarios.
...
* [**Registros**][2]: Vea los registros de su contenedor o recurso. Haga clic en cualquier registro para ver registros relacionados en el Explorador de registros.
* [**APM**][3]: Vea las trazas de su contenedor o recurso, incluyendo la fecha, el servicio, la duración, el método y el código de estado de una traza.
* [**Métricas**][4]: Vea métricas en tiempo real para su contenedor o recurso. Puede ver cualquier gráfico en pantalla completa, compartir una instantánea del mismo o exportarlo desde esta pestaña.
* {{< ui >}}Processes{{< /ui >}}: Vea todos los procesos que se ejecutan en el contenedor de este recurso.
* {{< ui >}}Network{{< /ui >}}: Vea el rendimiento de red de un contenedor o recurso, incluyendo los campos de fuente, destino, volumen enviado y recibido, y rendimiento. Use el campo {{< ui >}}Destination{{< /ui >}} para buscar por etiquetas como `DNS` o `ip_type`, o use el filtro {{< ui >}}Group by{{< /ui >}} en esta vista para agrupar datos de red por etiquetas, como `pod_name` o `service`.
* [**Eventos**][5]: Vea todos los eventos de Kubernetes para su recurso.
...
| Tipo | Ejemplos |
|---|---|
| **Etiquetas**: Adjuntas a los recursos por [el Agent que los recopila][7]. También hay etiquetas adicionales que Datadog genera para los recursos de Kubernetes. | `datacenter:staging`, `tag#datacenter:staging`<br>_(el `tag#` es opcional)_ |
