...
## Utilizar la Datadog Lambda Extension en lugar del Forwarder

Si todavía utilizas la [función de Lambda del Datadog Forwarder][2] para la recopilación de datos, considera la posibilidad de adoptar la [Datadog Lambda Extension][3] en su lugar. Dado que las limitaciones técnicas de Lambda del Forwarder provocan una serie de problemas conocidos, migrar a la extensión de Lambda puede resolverlos automáticamente.
...
<div class="alert alert-info">La extensión de Datadog Lambda permite la recopilación de logs de forma predeterminada. Si estás migrando del Forwarder a la extensión, asegúrate de eliminar tu suscripción a logs. De lo contrario, es posible que veas logs duplicados.</div>
...
Para asegurarte de que los cambios reales realizados en tus funciones de Lambda son los esperados, prueba con definir otra función de prueba y configurarla siguiendo las instrucciones de la _Datadog CLI_ o _Personalizado_. Compara los cambios (como el controlador, las capas, las variables de entorno y las etiquetas [tags]) efectuados en tus funciones de Lambda reales con la función de prueba.
...
Datadog puede recopilar métricas y etiquetas de recursos de AWS a través de una [integración con AWS][10] (opcional). Si te faltan determinadas métricas de CloudWatch y etiquetas de recursos de Lambda, asegúrate de que la integración de AWS esté correctamente configurada.
...
2. Descarga la última versión de la [Datadog CLI](https://github.com/DataDog/datadog-ci/#how-to-install-the-cli).
...
<div class="alert alert-info">Para obtener más información sobre Serverless Flare, consulta la <a href="https://docs.datadoghq.com/serverless/libraries_integrations/cli/#troubleshooting-lambda-instrumentation">documentación del comando</a>.</div>
{{% /tab %}}
{{% tab "Manually" %}}
...
4. El método de instalación alternativo que probaste, como el de la _Datadog CLI_ o el _Personalizado_.
