...
- link: https://www.datadoghq.com/blog/datadog-network-path-monitoring/
  tag: 블로그
  text: Network Path 및 SD-WAN 모니터링을 통해 종단 간 네트워크 가시성 확보하기
...
Network Path 설정은 서비스와 엔드포인트 간의 네트워크 경로를 모니터링하고 추적하기 위해 환경을 구성하는 것을 포함합니다. 네트워크 인프라의 병목 현상, 지연 문제 및 잠재적인 실패 지점을 식별하는 데 도움이 됩니다. Network Path를 사용하면 필요에 따라 개별 Network Path를 수동으로 구성하거나 자동으로 발견하거나 두 가지 방법을 동시에 사용할 수 있습니다.

**참고**: 네트워크 구성에서 아웃바운드 트래픽이 제한되는 경우, [Agent 프록시 구성][2] 문서의 설정 지침을 따르세요.

## 설정 {#setup}

<div class="alert alert-info">이 페이지는 Network Monitoring에서 Agent 기반 구성에 대한 Network Path 설정을 다룹니다. Synthetic Monitoring에서 Network Path 테스트를 생성하려면 <a href="/synthetics/network_path_tests/">Network Path Testing in Synthetic Monitoring</a>을 참조하세요.</div>

Datadog은 세 가지 Agent 기반 수집 방법을 제공합니다. 한 가지 방법을 단독으로 사용하거나 여러 방법을 결합할 수 있습니다.

| 방법 | 사용 시기 |
|--------|-------------|
| **[예약된 테스트](#scheduled-tests)** | Agent 구성에서 정의한 특정 소스-대상 쌍을 모니터링합니다. 중요한 API 또는 파트너 서비스와 같은 알려진 엔드포인트 집합을 추적하는 데 가장 적합합니다. |
| **[동적 테스트](#dynamic-tests)** | [Cloud Network Monitoring][1]에서 관찰된 트래픽을 기반으로 경로를 자동으로 발견하고 모니터링합니다. 모든 대상을 수동으로 나열하지 않고도 넓은 가시성을 확보하는 데 가장 적합합니다. |
| **[NetFlow용 동적 테스트](#dynamic-tests-for-netflow-experimental)** | [NetFlow Monitoring][6]에서 관찰된 대상 IP로 Agent 호스트에서 Network Path 테스트를 자동으로 실행합니다. 개별 대상을 수동으로 구성하지 않고 NetFlow 트래픽에 홉별 경로 가시성을 추가하는 데 가장 적합합니다. |
...
시작하려면 [예제 구성][5]을 복사하고 `.example` 확장자를 제거한 후 원하는 설정으로 업데이트하거나 아래의 환경별 구성 중 하나를 사용하세요. 대규모 환경에서 성능 최적화를 위해 [워커 수 늘리기](#increase-the-number-of-workers)를 참조하세요.
...
<div class="alert alert-info">Helm chart v3.109.1+가 필요합니다. <a href="https://github.com/DataDog/helm-charts/blob/main/charts/datadog/README.md">Datadog Helm Chart 문서</a>와 <a href="https://docs.datadoghq.com/containers/kubernetes/integrations/?tab=helm#configuration">Kubernetes 및 Integrations</a>에 대한 문서를 참조하세요.</div>
...
2. `network_path`를 활성화하여 CNM 연결을 모니터링하려면 `%ProgramData%\Datadog\datadog.yaml` 파일을 생성하거나 편집하세요.
   ...
   <div class="alert alert-info"><a href="/infrastructure/end_user_device_monitoring/">최종 사용자 장치</a>에서 Network Path를 활성화하는 경우 이 단계를 건너뛰세요.</div>
   ...
   전체 구성 세부 정보는 [예제 구성][3]을 참조하거나 다음을 사용하세요.
...
Helm을 사용하여 Kubernetes와 함께 Network Path를 활성화하려면 `values.yaml` 파일에 다음 내용을 추가하세요.
**참고:** Helm chart v3.124.0+가 필요합니다. 자세한 정보는 [Datadog Helm Chart 문서][1]와 [Kubernetes 및 Integrations][2]에 대한 문서를 참조하세요.
...
### NetFlow용 동적 테스트(실험적) {#dynamic-tests-for-netflow-experimental}

<div class="alert alert-info">NetFlow용 동적 테스트는 실험적 기능이며 Agent가 필요합니다. <code>v7.81+</code>이 기능을 활성화하려면 Datadog 지원팀이나 계정팀에 문의하세요.</div>

NetFlow 레코드에서 관찰된 대상 IP로 Agent 호스트에서 Network Path 테스트를 실행하도록 NetFlow용 동적 테스트를 구성하세요. NetFlow용 동적 테스트는 [Cloud Network Monitoring][1] 또는 `network_path.connections_monitoring.enabled`가 필요하지 않습니다.

NetFlow용 동적 테스트는 NetFlow 트래픽을 수집하는 Datadog Agent에서 실행됩니다. 이 테스트는 NetFlow 익스포터, 라우터 또는 원래 흐름 소스에서 실행되지 않습니다. 해당 Agent에서 수행하는 트레이스라우트가 조사하려는 경로를 나타낼 수 있도록 관찰된 흐름 소스와 충분히 가까운 곳에 Agent를 배포하세요.

**전제 조건**:

- [NetFlow Monitoring][6]이 구성되어 있고 흐름을 수신하고 있어야 합니다.
- Agent `v7.81+`가 필요합니다.
...
2. `/etc/datadog-agent/datadog.yaml`에서 NetFlow용 동적 테스트를 활성화합니다.
   ...
   `monitor_ip_without_domain: true`는 필수 사항입니다. NetFlow용 동적 테스트가 관찰된 대상 IP 주소를 대상으로 하고 Network Path 컬렉터가 기본적으로 IP 전용 대상을 건너뛰기 때문입니다.

3. 이 구성 변경을 수행한 후 Agent를 재시작합니다.
...
2. `%ProgramData%\Datadog\datadog.yaml`에서 NetFlow용 동적 테스트를 활성화합니다.
...
Agent가 경로를 보고한 후, [Network Path][4] UI를 열고 `origin:netflow`로 필터링하여 NetFlow 트래픽에서 생성된 경로를 조회하세요.
...
동일한 `network_path.collector.filters` 목록이 동적 테스트 및 NetFlow용 동적 테스트에 적용됩니다. NetFlow용 동적 테스트의 경우, 관찰된 대상 IP 주소를 대상으로 하므로 `match_ip` 필터를 사용해야 합니다.
...
예를 들어, `*.datadoghq.com`과 일치하는 모든 도메인은 무시되며, `api.datadoghq.com`은 제외됩니다.
...
2. 다음과 같은 최소 하나의 Network Path 기능이 활성화되어야 합니다.
   ...
   - [예약된 테스트](#scheduled-tests)는 `conf.d/network_path.d` 파일을 통해 구성됩니다.
   - [동적 테스트](#dynamic-tests)는 `network_path.connections_monitoring.enabled` 및 [Cloud Network Monitoring][1]을 모두 활성화하여 구성됩니다.
   - [NetFlow용 동적 테스트](#dynamic-tests-for-netflow-experimental)는 `network_path.netflow_monitoring.enabled` 및 [NetFlow Monitoring][6]을 활성화하여 구성됩니다.

### UI에 NetFlow용 동적 테스트 데이터가 없음 {#no-dynamic-tests-for-netflow-data-in-the-ui}

`origin:netflow`를 포함하는 경로가 [Network Path][4] UI에 나타나지 않으면 다음을 확인하세요.

1. Agent 버전이 `7.81+`입니다.
2. [NetFlow Monitoring][6]이 활성화되어 있고 흐름을 수신하고 있습니다.
3. 트레이스라우트 모듈이 `system-probe.yaml`에서 활성화되어 있습니다.
4. `network_path.netflow_monitoring.enabled` 및 `network_path.collector.monitor_ip_without_domain`이 `datadog.yaml`에서 `true`로 설정되어 있습니다.
5. `network_path.collector.filters` 구성이 모니터링하려는 대상 IP를 제외하고 있지 않습니다.

NetFlow용 동적 테스트는 대상 필터가 평가되기 전에 소스 IP가 Agent 호스트에 할당된 NetFlow 레코드를 자동으로 건너뜁니다. 이는 자체 예약 루프를 방지하기 위한 것이며 예상되는 동작입니다. Agent 소스 트래픽이 다른 소스 IP에서 나타나는 NAT 또는 별칭 사례의 경우, `network_path.collector.source_excludes`를 사용하여 해당 소스 IP를 제외하세요.

그런 다음 Network Path UI를 `origin:netflow`로 필터링하세요.
...
[2]: /ko/agent/configuration/proxy/?tab=linux
[3]: /ko/help
...
[6]: /ko/network_monitoring/netflow/
