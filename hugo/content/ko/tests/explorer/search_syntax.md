---
description: Test Visibility Explorer에서 모든 테스트 실행을 검색하는 방법을 알아보세요.
further_reading:
- link: /tests/search
  tag: 설명서
  text: 테스트 필터링 및 그룹화
...
title: Test Visibility Explorer 검색 구문
---
...
예를 들어 `git.repository.name` 속성에 관심이 있고 `Datadog/documentation` 값을 필터링하려면 `@git.repository.id:"github.com/Datadog/documentation"`를 사용합니다.
...
테스트 실행은 태그를 생성하는 [호스트][3] 및 [통합][4]에서 태그를 상속합니다. 검색 및 패싯으로도 사용할 수 있습니다.

* `test`는 문자열 "test"를 검색합니다.
* `env:(prod OR test)`는 태그 `env:prod` 또는 `env:test` 태그를 포함하는 모든 테스트 실행과 일치합니다.
* `(env:prod AND -version:beta)`는 태그 `env:prod`를 포함하고 태그 `version:beta`를 포함하지 않는 모든 테스트 실행과 일치합니다.
