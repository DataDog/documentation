---
description: 'OpenLineage 유효성 검사 DAG를 사용하여 Airflow Data Observability: Jobs Monitoring의
  구성 문제를 진단하세요.'
further_reading:
- link: /data_observability/jobs_monitoring/airflow/
  tag: 설명서
  text: 'Apache Airflow용 Data Observability: Jobs Monitoring 활성화'
- link: /data_jobs
  tag: 설명서
  text: 'Data Observability: Jobs Monitoring'
title: OpenLineage 유효성 검사 DAG를 사용하여 Airflow 설정 문제 해결
---
## 개요 {#overview}

OpenLineage 유효성 검사 DAG는 Airflow의 OpenLineage 구성을 검증하고 Data Observability: Jobs Monitoring으로 이벤트가 전달되지 않는 문제를 보고합니다.

다음과 같은 경우에 이 DAG를 실행하세요.

- 설정을 완료한 후 [Data Observability: Jobs Monitoring][1] 페이지에 Airflow 작업이 표시되지 않는 경우.
- 프로덕션 워크로드를 실행하기 전에 구성을 확인하려는 경우.

이 DAG는 다음 항목을 확인합니다.

| 확인 항목 | 확인 내용 |
|---|---|
| 패키지 설치 | OpenLineage 패키지가 설치되어 있고 가져올 수 있는지 확인합니다. |
| 패키지 버전 | 설치된 버전을 PyPI에서 제공되는 최신 버전과 비교합니다. |
| 리스너 액세스 가능 여부 | OpenLineage 플러그인 리스너를 Airflow에서 로드할 수 있는지 확인합니다. |
| 프로바이더 활성화 여부 | OpenLineage 프로바이더가 활성화되어 있고 명시적으로 비활성화 상태가 아닌지 확인합니다. |
| 전송 구성 | 유효한 전송 방식(HTTP 또는 Datadog)이 구성되어 있는지 확인합니다.|
| Datadog 엔드포인트 | 전송 URL이 Datadog 수집 엔드포인트를 가리키는지 확인합니다. |
| 네트워크 연결 | 구성된 백엔드 URL에 대한 TCP 연결이 성공하는지 확인합니다. |
| 구성 충돌 | 중복되거나 충돌하는 전송 또는 구성 파일 설정이 없는지 확인합니다. |

## OpenLineage 유효성 검사 DAG 실행.{#run-the-openlineage-validation-dag}

### 전제 조건 {#prerequisites}

- Airflow DAG 디렉터리에 대한 액세스 권한(로컬 경로 또는 Amazon MWAA의 경우 클라우드 스토리지 버킷)
- 실행 중인 Airflow 스케줄러 및 워커(포드 또는 프로세스)

### DAG 파일 추가 {#add-the-dag-file}

다음 코드를 Airflow DAG 디렉터리에 `openlineage_preflight_check_dag.py`로 저장하세요. Amazon MWAA의 경우, 환경에 구성된 S3 DAG 폴더에 파일을 업로드하세요.
...
### DAG 트리거 {#trigger-the-dag}

Airflow 스케줄러가 새 파일을 인식한 후, DAG를 수동으로 트리거하세요.

1. Airflow UI에서 **DAG**로 이동하여 `openlineage_preflight_check_dag`를 찾습니다.
2. DAG가 일시 중지되어 있다면 토글을 켭니다.
3. **Trigger DAG**(재생 아이콘)을 클릭합니다.

### 로그 조회 {#view-the-logs}

1. Airflow UI에서 **DAGs > openlineage_preflight_check_dag**로 이동합니다.
2. 가장 최근 DAG 실행을 클릭합니다.
3. **validate_setup** 작업을 클릭합니다.
4. **Logs** 탭을 선택합니다.

## 출력 알아보기 {#understand-the-output}

DAG 로그에는 두 개의 섹션이 있습니다. 시작 부분에 출력되는 환경 정보 요약과 마지막 부분에 출력되는 유효성 검사 요약입니다.

### 환경 정보 {#environment-information}

첫 번째 섹션에는 DAG가 사용자 환경에서 감지한 정보가 표시됩니다.
...
### 유효성 검사 요약 {#validation-summary}

유효성 검사 요약은 작업 로그의 마지막에 표시됩니다. 각 줄은 다음 기호 중 하나로 시작합니다.

- `✓`: 검사 통과
- `✗`: 검사 실패(이벤트가 Datadog으로 전송되지 않음)
- `!`: 경고(구성 문제를 나타낼 수 있음).
- `-`: 검사 생략(필수 검사 실패)

다음 출력은 구성이 정상적으로 설정되었음을 나타냅니다.
...
### 실패 결과 해석 {#interpreting-failures}

이 표를 사용하여 일반적인 실패 문제를 해결하세요.

| 로그 메시지 | 원인 | 해결 방법 |
|---|---|---|
| `✗ OpenLineage not installed properly` | OpenLineage 패키지가 없거나 손상되었습니다. 나머지 모든 검사를 생략합니다. | Airflow 설치에 `apache-airflow-providers-openlineage`가 포함되어 있는지 확인하세요. Amazon MWAA의 경우 [Amazon MWAA에서 OpenLineage Provider 업그레이드][3]를 참조하세요. |
| `✗ OpenLineage provider is turned off` | 프로바이더가 `airflow.cfg`에서 `AIRFLOW__OPENLINEAGE__DISABLED`, `OPENLINEAGE_DISABLED` 또는 `openlineage.disabled` 설정으로 인해 명시적으로 비활성화되었습니다. | 설정을 제거하거나 `false`로 설정하세요. |
| `✗ OpenLineage provider is not active (no transport configuration found)` | 프로바이더는 명시적으로 비활성화되지 않았더라도 전송 구성이 없으면 자체적으로 비활성화됩니다. | [Airflow 설정 가이드][2]에 따라 전송을 구성하세요. |
| `✗ Failed to resolve active transport` | 확인된 구성에서 전송을 인스턴스화할 수 없습니다. | 전송 구성이 유효하고 올바른 형식인지 확인하세요. 작업 로그에서 근본 원인을 확인하세요. |
| `✗ Transport Type: Console (won't send events to Datadog)` | 전송이 `console`로 설정되었습니다. | 전송을 Datadog 수집 URL을 가리키는 `datadog` 또는 `http`로 변경하세요. |
| `! Transport does not appear to point to a Datadog endpoint` | 전송 URL이 알려진 Datadog 도메인과 일치하지 않습니다. | URL이 Datadog 수집 엔드포인트를 가리키는지 확인하세요. |
| `✗ Network connectivity check failed` | Airflow 워커가 Datadog 수집 엔드포인트에 접속할 수 없습니다. | 방화벽 또는 네트워크 정책을 확인하고 URL 및 포트 443에 액세스할 수 있는지 확인하세요. |
| `✗ OpenLineage listener is not accessible` | 프로바이더 플러그인을 가져올 수 없거나, 리스너가 등록되지 않았거나, 여러 리스너가 검색되었습니다. 구체적인 원인은 작업 로그 앞부분에 기록되어 있습니다. | 패키지가 **스케줄러와 워커** 포드 또는 프로세스 모두에 설치되어 있는지 확인하세요. 플러그인이 로드되었지만 활성 리스너가 없는 경우, 프로바이더 버전 호환성을 확인하세요. |
| `! Configuration conflicts detected` | 여러 전송 또는 구성 파일 소스가 활성화되어 있습니다. | 중복된 구성을 제거하고 하나의 기준 소스만 유지하세요. |

**참고**: `✗ OpenLineage listener is not accessible`이 패키지 설치 실패와 함께 나타나는 경우, 패키지가 워커에는 설치되지 않고 스케줄러에만 설치되어 있을 가능성이 높습니다. OpenLineage는 스케줄러와 워커 모두에 프로바이더가 필요합니다.

### 버전 경고 {#version-warnings}

DAG에 다음과 같은 버전 경고가 표시되는 경우:
...
버그 수정 및 향상된 Datadog 호환성을 활용하려면 최신 버전으로 업그레이드하는 것을 고려하세요. Amazon MWAA의 경우, 버전별 지침은 [Amazon MWAA에서 OpenLineage 프로바이더 업그레이드][3]를 참조하세요.
...
[2]: /ko/data_observability/jobs_monitoring/airflow/
[3]: /ko/data_observability/jobs_monitoring/airflow_mwaa_upgrade/