---
aliases:
- /ko/opentelemetry/collector_exporter/otlp_receiver
further_reading:
- link: /opentelemetry/collector_exporter/
  tag: 설명서
  text: OpenTelemetry Collector 설정하기
- link: /opentelemetry/otlp_ingest_in_the_agent/
  tag: 설명서
  text: Datadog Agent에 의한 OTLP 수집
- link: https://www.datadoghq.com/architecture/datadog-agent-otlp-receiver-in-kubernetes/
  tag: 아키텍처 센터
  text: Kubernetes의 Datadog Agent OTLP 수신기
title: OTLP 수신기
---
## 개요 {#overview}

메트릭, 로그, 트레이스를 수집하려면 컬렉터(Collector)에서 [OTLP 수신기][1]를 설정하세요.

자세한 내용은 [OTLP 수신기][1]에 대한 OpenTelemetry 프로젝트 설명서를 참조하세요.

## 설정 {#setup}

Collector 구성에 다음 줄을 추가합니다.
...
Datadog 내보내기를 사용한 전체 작업 예시 설정은 [`otlp.yaml`][2]을 참조하세요.
