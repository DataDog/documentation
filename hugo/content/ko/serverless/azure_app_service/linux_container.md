...
사이드카 접근 방법을 사용하지 않으려는 경우(권장하지 않음), 대신 [`serverless-init`를 사용한 Azure App Service - Linux Containers 계측][2] 지침을 따를 수 있습니다.

## 설정 {#setup}

### Azure 통합 {#azure-integration}

아직 [Datadog-Azure 통합][3]을 설치하지 않았다면 설치하여 메트릭과 로그를 수집하세요.
...
{{% tab "Node.js" %}}
#### 추적 {#tracing}
`dd-trace-js` 라이브러리로 메인 애플리케이션을 계측합니다. [Node.js 애플리케이션 추적][101]에서 지침을 확인하세요.
...
Datadog으로 전송되는 내용을 더 효과적으로 제어하려면 `/home/LogFiles/myapp`과 같은 하위 디렉터리를 생성할 수도 있습니다. 그러나 모든 로그 파일을 `/home/LogFiles`에서 테일링하지 않으면 시작 및 오류와 관련된 Azure App Service 애플리케이션 로그가 수집되지 않습니다.
...
{{% tab "Python" %}}
#### 추적 {#tracing-1}
`dd-trace-py` 라이브러리로 메인 애플리케이션을 계측합니다. [Python 애플리케이션 추적][201]에서 지침을 확인하세요.
...
{{% tab "Java" %}}
#### 추적 {#tracing-2}
`dd-trace-java` 라이브러리로 메인 애플리케이션을 계측합니다. [Java 애플리케이션 추적][301]에서 지침을 확인하세요.
...
{{% tab ".NET" %}}
#### 추적 {#tracing-3}
`dd-trace-dotnet` 라이브러리로 메인 애플리케이션을 계측합니다.
...
자세한 내용은 [.NET 애플리케이션 추적][401]을 참조하세요.
...
{{% tab "Go" %}}
#### 추적 {#tracing-4}
`dd-trace-go` 라이브러리로 메인 애플리케이션을 계측합니다. [Go 애플리케이션 추적][501]에서 지침을 확인하세요.
...
{{% tab "PHP" %}}
#### 추적 {#tracing-5}
`dd-trace-php` 라이브러리로 메인 애플리케이션을 계측합니다. [PHP 애플리케이션 추적][601]에서 지침을 확인하세요.
...
그런 다음, 아래의 명령을 실행하여 사이드카 컨테이너를 설정합니다.
...
Datadog 사이트를 {{< region-param key="dd_site" code="true" >}}으로 설정합니다. 기본값은 `datadoghq.com`입니다.
...
모든 환경 변수에 대한 설명은 [Manual 탭](?tab=manual#instrumentation)을 참조하세요.
...
2. **Edit container** 양식에서 다음을 제공합니다.
   - **Image source**: Docker Hub 또는 기타 레지스트리
   - **Image type**: 공개
...
<div class="alert alert-info">배포 슬롯 계측은 프리뷰 버전입니다. 프리뷰 기간에는 슬롯의 텔레메트리가 메인 웹 앱 아래에 표시됩니다. 슬롯과 프로덕션 텔레메트리를 구분하려면 각 슬롯에 대해 고유한 값으로 <a href="/getting_started/tagging/unified_service_tagging/">통합 서비스 태깅</a>을 구성하세요.</div>

{{% collapse-content title="배포 슬롯 계측" level="h4" %}}

메인 웹 앱 대신 [배포 슬롯][901]을 계측하려면 다음 방법 중 하나를 사용하세요.
...
[Datadog CLI][1](v5.9.0 이상)를 사용하여 `--slot` 플래그를 추가합니다. `--env`를 사용하여 슬롯에 대한 고유한 환경 태그를 설정합니다.
...
[Datadog Linux Web App Slot 모듈][1]을 사용합니다.
...
`terraform apply`를 실행하고 표시되는 메시지를 따릅니다.
