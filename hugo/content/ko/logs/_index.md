...
- link: https://www.datadoghq.com/blog/monitoring-load-balancer-logs
  tag: 블로그
  text: 애플리케이션 및 네트워크 로드 밸런서 로그 모니터링
...
---
{{< learning-center-callout header="활성화 웨비나 세션에 참가하기" hide_image="true" btn_title="등록" btn_url="https://www.datadoghq.com/technical-enablement/sessions/?tags.topics-0=Logs">}}
  개론 또는 중급 인에이블먼트 세션에 참여해 Datadog Log Management가 로그, 메트릭, 트레이스를 어떻게 하나의 뷰로 통합하여 로그 데이터 분석에 유용한 풍부한 컨텍스트를 제공하는지 알아보세요.
...
{{< img src="logs/lwl_marketecture_20231030.png" alt="모든 로그를 한 곳에서 구성" >}}

일단 로그를 수집하면, 프로세스 파이프라인과 프로세서로 모든 로그를 보강 및 처리하고 인덱스를 활용하여 로그 관리 예산을 제어합니다. 또한 수집한 로그로 메트릭을 생성하거나 [로그 구성 옵션][5]으로 스토리지에 최적화된 아카이브 내에서 해당 로그를 관리합니다.
...
- [로그와 메트릭의 상호관계를 정립][7]해 문제의 컨텍스트를 얻고, 이를 서비스 전체에 매핑합니다.

## 탐색 {#explore}

[로그 탐색기][1]에서 수집한 로그를 둘러보세요.

**팁**: Datadog 글로벌 검색에서 로그 탐색기를 열려면 <kbd>Cmd</kbd>/<kbd>Ctrl</kbd> + <kbd>K</kbd>를 누르고 `logs`를 검색합니다.

{{< img src="/logs/explore.png" alt="수집된 로그 탐색" style="width:80%;">}}

- [Search][8]: 모든 로그를 검색합니다.
- [Live Tail][9]: 모든 환경에서 수집된 로그를 실시간으로 확인합니다.
- [Analytics][10]: 인덱싱된 로그에 대해 로그 분석을 수행합니다.
- [Patterns][11]: 인덱싱된 로그를 함께 클러스터링하여 로그 패턴을 확인합니다.
- [Saved Views][12]: 저장된 뷰를 사용하여 로그 탐색기를 자동으로 구성합니다.


{{< learning-center-callout header="학습 센터에서 Log Management 소개 수강해 보기" btn_title="지금 등록" btn_url="https://learn.datadoghq.com/courses/intro-to-log-management">}}
