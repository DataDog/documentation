...
<div class="alert alert-danger">전체 텍스트 검색 기능은 Log Management에서만 사용할 수 있으며 모니터, 대시보드, 노트북 쿼리에서 작동합니다. 전체 텍스트 검색 구문은 인덱스 필터, 아카이브 필터, 로그 파이프라인 필터, 리하이드레이션 필터, 로그 기반 메트릭 필터 또는 Live Tail에서 사용할 수 없습니다. </div>
...
다음 문자는 특수 문자로 간주되며 `\` 문자로 이스케이프해야 합니다: `-` `!` `&&` `||` `>` `>=` `<` `<=` `(` `)` `{` `}` `[` `]` `"` `*` `?` `:` `\` `#` 및 공백.
