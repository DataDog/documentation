...
description: Datadog pipelines 및 프로세서를 사용하여 로그 구문 분석, 보강 및 관리
further_reading:
...
<div class="alert alert-warning">사용자에게 파이프라인의 제한 목록에 대한 액세스 권한을 부여해도 자동으로 <code>logs_write_pipelines</code> 또는 <code>logs_write_processors</code> 권한이 부여되지는 않습니다. 그러한 권한은 관리자가 별도로 부여해야 합니다.</div>
...
1. Datadog 에서 [Pipelines][8]으로 이동하여 [{{< ui >}}Preprocessing for JSON logs{{< /ui >}}][9]를 선택합니다.
...
2. {{< ui >}}New Pipeline{{< /ui >}}을 선택합니다.
3. Live Tail 미리 보기에서 로그를 선택해 필터를 적용하거나, 직접 필터를 적용합니다. 드롭다운 메뉴에서 필터를 선택하거나 {{< ui >}}</>{{< /ui >}} 아이콘을 선택하여 자체 필터 쿼리를 생성합니다. 필터를 사용하면 파이프라인이 어느 종류의 로그에 적용되는지 제한할 수 있습니다.
...
3. {{< ui >}}Add Processor{{< /ui >}} 또는 {{< ui >}}Add Nested Pipeline{{< /ui >}}를 선택합니다.
...
### 여러 프로세서가 일치할 때의 속성 우선순위 {#attribute-precedence}

일치하는 파이프라인 내의 여러 프로세서가 동일한 속성을 설정하는 경우, 결과는 프로세서 유형에 따라 달라집니다. 다음과 같이 세 가지 동작이 있습니다.

| 동작 | 설명 | 프로세서 |
| --- | --- | --- |
| 마지막 저장 내용 우선 | 마지막에 실행되는 프로세서(순서상 아래에 있는 프로세서)가 설정한 값이 이전 값을 재정의합니다. | Grok 파서, 카테고리 프로세서, 산술 프로세서, 문자열 빌더 프로세서, 조회 프로세서, URL 파서, User-Agent 파서, GeoIP 파서, 디코더 프로세서 |
| 영향을 주는 요인: `override_on_conflict` |  `override_on_conflict` 파라미터를 따릅니다. 기본적으로(`false`) 대상 요소가 이미 설정되어 있으면 재정의되지 않습니다. | 리매퍼, 배열 매핑 프로세서 |
| 첫 번째 저장 내용 우선 | 첫 번째 프로세서만 적용됩니다(마지막 프로세서를 사용하는 로그 날짜 리매퍼는 제외). 단일 파이프라인 내에서는 첫 번째 프로세서의 값이 사용되며, 여러 개의 일치하는 파이프라인 전반에서는 가장 먼저 발생한 프로세서가 적용됩니다. | 로그 상태 리매퍼, 서비스 리매퍼, 로그 메시지 리매퍼, 트레이스 리매퍼, 스팬 리매퍼 |

각 프로세서에 대한 자세한 내용은 [프로세서][3]를 참조하세요.
...
1. 이동하려는 파이프라인 위로 마우스를 가져간 다음 {{< ui >}}Move to{{< /ui >}} 아이콘을 클릭합니다.
1. 원래 파이프라인을 이동하고자 하는 대상 파이프라인을 선택합니다. **참고**: 중첩된 파이프라인을 포함하는 파이프라인은 다른 최상위 위치로만 이동할 수 있습니다. 다른 파이프라인 안으로는 이동할 수 없습니다.
1. {{< ui >}}Move{{< /ui >}}를 클릭합니다.

## 파이프라인 변경 사항 미리 보기 {#preview-pipeline-changes}

파이프라인이나 해당 프로세서를 생성하거나 편집할 때 변경 사항을 적용하기 전에 해당 변경 사항이 로그에 미치는 영향을 미리 볼 수 있습니다. 미리 보기에서는 로그의 Live Tail을 사용하며, 제안된 변경 사항을 적용하여 처리합니다.

{{< img src="logs/processing/pipelines/pipeline_simulation.png" alt="왼쪽에는 파이프라인의 프로세서가, 오른쪽에는 선택한 로그의 차이를 보여주는 파이프라인 시뮬레이션 보기" >}}

로그별로 변경 전후 상태를 비교합니다. 비교해 볼 변경 사항을 선택하세요.

- **변경 사항**: 현재 배포된 파이프라인 버전과 변경 사항이 적용된 버전을 비교합니다.
- **전체 파이프라인**: 파이프라인에 진입하는 로그를 전체 파이프라인이 실행된 후의 로그와 비교합니다.

로그 목록의 범위를 좁히려면 쿼리 필터를 사용하거나 영향도를 기준으로 필터링하세요.

- **모든 로그**: 라이브 테일의 모든 로그입니다.
- **영향을 받은 로그**: 이번 세션에서 편집을 통해 변경된 로그만 해당합니다.
- **영향을 받지 않은 로그**: 편집으로 인해 변경되지 않은 로그만 해당합니다.
...
슬라이딩 옵션 패널의 {{< ui >}}Move to{{< /ui >}} 옵션을 사용하여 파이프라인 순서를 정확하게 변경할 수 있습니다. {{< ui >}}Move to{{< /ui >}} 모달을 사용하여 스크롤하고 선택한 파이프라인을 이동할 정확한 위치를 클릭하세요. 파이프라인은 다른 읽기 전용 파이프라인 안으로 이동할 수 없습니다. 중첩된 파이프라인을 포함한 파이프라인은 다른 최상위 위치로만 이동할 수 있습니다. 다른 파이프라인 안으로는 이동할 수 없습니다.
