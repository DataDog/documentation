...
description: 특정 위치에서 사용자 여정을 시뮬레이션 및 모니터링하세요.
further_reading:
...
- link: https://registry.terraform.io/providers/DataDog/datadog/latest/docs/resources/synthetics_test
  tag: 외부 사이트
  text: Terraform을 사용하여 Synthetic 브라우저 테스트 생성 및 관리
title: 브라우저 테스트
---
## 개요 {#overview}

브라우저 테스트는 Datadog이 사용자의 웹 애플리케이션에서 실행하는 시나리오입니다. 구성 가능한 간격에 따라 전 세계의 여러 위치에서 다양한 브라우저 및 장치를 통해 실행됩니다. 이 테스트는 애플리케이션이 실행 중이며 요청에 응답하고 있는지, 시나리오에 정의된 조건이 충족되는지를 검증합니다.

<div class="alert alert-info">MFA로 보호되는 애플리케이션을 테스트하는 데 관심이 있다면 <a href="/synthetics/guide/app-that-requires-login/#multi-factor-authentication" target="_blank">전용 가이드</a>를 읽고 Synthetic Monitoring 팀에 <a href="https://docs.google.com/forms/d/e/1FAIpQLSdjx8PDZ8kJ3MD2ehouTri9z_Fh7PoK90J8arRQgt7QFgFxog/viewform?usp=sf_link">피드백을 보내</a>주세요. 고객님의 팀에 가장 중요한 시스템을 개선하는 데 활용하겠습니다.</div>

## 테스트 구성 {#test-configuration}

다음 옵션 중 하나를 사용하여 테스트를 생성할 수 있습니다.

### 템플릿으로 테스트 생성 {#create-a-test-from-a-template}

  1. 사전에 채워진 템플릿 중 하나에 마우스를 올리고 {{< ui >}}View Template{{< /ui >}}을 클릭합니다. 이렇게 하면 {{< ui >}}Test Details{{< /ui >}}, {{< ui >}}Alert Conditions{{< /ui >}}, {{< ui >}}Steps{{< /ui >}}, {{< ui >}}Variables{{< /ui >}}(옵션) 등 사전에 채워진 구성 정보가 표시되는 사이드 패널이 열립니다.
  2. {{< ui >}}+Create Test{{< /ui >}}를 클릭하여 구성 페이지를 열면 사전에 채워진 구성 옵션을 검토하고 편집할 수 있습니다. 표시되는 필드는 테스트 처음 생성 시 제공되는 필드와 동일합니다.
  3. 오른쪽 상단의 {{< ui >}}Save & Quit{{< /ui >}}을 클릭하여 브라우저 테스트를 제출합니다.<br /><br>
...
### 처음부터 테스트 빌드 {#build-a-test-from-scratch}

  1. 새 브라우저 테스트를 처음부터 시작하려면 {{< ui >}}+{{< /ui >}} 템플릿을 클릭합니다.
  1. {{< ui >}}Starting URL{{< /ui >}} 입력: 브라우저 테스트가 시나리오를 시작하는 URL입니다.
  1. {{< ui >}}name{{< /ui >}} 추가: 브라우저 테스트의 이름입니다.
  1. {{< ui >}}environment and additional tags{{< /ui >}} 선택: 브라우저 테스트에 연결된 `env` 및 관련 태그를 설정합니다. `<KEY>:<VALUE>` 형식을 사용해 지정된 `<KEY>`의 `<VALUE>`를 필터링합니다.

  <div class="alert alert-info">자세한 옵션은 <a href=#advanced-options>고급 옵션</a>을 참조하세요.</div>

  5. {{< ui >}}browsers and devices{{< /ui >}}를 선택합니다. 테스트를 실행할 브라우저(예: `Chrome`, `Firefox`, `Edge`)와 장치(예: `Laptop Large`, `Tablet`, `Mobile Small`)입니다.

      - 대형 노트북 장치의 경우 크기는 1440 픽셀 x 1100 픽셀입니다.
      - 태블릿 장치의 경우 크기는 768 픽셀 x 1020 픽셀입니다.
      - 소형 모바일 장치의 경우 크기는 320 픽셀 x 550 픽셀입니다.

  6. {{< ui >}}managed and private locations{{< /ui >}} 선택: Datadog이 관리하는 전 세계의 [위치](#locations) 목록에서 선택하거나 [프라이빗 위치][1]를 생성하여 사용자 지정 위치 또는 내부 프라이빗 네트워크에서 브라우저 테스트를 실행합니다.

     **참고**: 또한 [Continuous Testing Tunnel][2]을 사용하여 로컬 개발 환경이나 CI/CD 파이프라인에서 테스트를 트리거해 내부 환경을 테스트할 수도 있습니다.

  7. {{< ui >}}test frequency{{< /ui >}} 설정: 간격은 1분마다부터 일주일에 한 번까지 다양합니다.
  8. {{< ui >}}Save & Edit Recording{{< /ui >}}을 클릭하여 브라우저 테스트를 제출합니다.
...
Synthetic Monitoring 브라우저 테스트를 설정할 때 수동으로 옵션을 선택하지 말고 코드 조각을 사용해 장치와 리전 정보를 자동으로 입력하세요. 다음 코드 조각을 사용할 수 있습니다.

* {{< ui >}}Screen sizes{{< /ui >}}: 브라우저 전반에서 특정 화면 크기로 브라우저 테스트를 자동으로 수행합니다.
   * {{< ui >}}Large{{< /ui >}}
   * {{< ui >}}Tablet{{< /ui >}}
   * {{< ui >}}Mobile{{< /ui >}}

* {{< ui >}}Multi-region check{{< /ui >}}: 세 가지 주요 리전(AMER, APAC, EMEA) 각각의 위치에서 웹사이트를 자동으로 테스트합니다.
</br><br>
...
{{< tabs >}}
   ...
   * {{< ui >}}Disable CORS{{< /ui >}}: 교차 출처 리소스 공유(CORS) 정책이 테스트를 차단하지 않도록 하려면 선택합니다.
   * {{< ui >}}Disable CSP{{< /ui >}}: 콘텐츠 보안 정책(CSP)이 테스트를 차단하지 않도록 하려면 선택합니다.
   * {{< ui >}}Capture HTTP payloads{{< /ui >}}: 각 테스트 단계에서 Fetch 및 XHR 리소스에 대한 요청 및 응답 헤더와 본문을 수집하려면 선택합니다. 이 옵션을 활성화한 후 테스트 결과의 [{{< ui >}}Resources{{< /ui >}} 탭][3]에서 Fetch 또는 XHR 리소스 행을 클릭하여 요청 및 응답 헤더와 본문을 조회합니다.
   * {{< ui >}}Request Headers{{< /ui >}}: {{< ui >}}Name{{< /ui >}} 및 {{< ui >}}Value{{< /ui >}} 필드에 헤더를 정의하여 기본 브라우저 테스트 헤더에 추가하거나 기본값을 재정의합니다. 예를 들어, 헤더에서 User Agent를 [Datadog 스크립트 식별][1]로 설정할 수 있습니다.
   * {{< ui >}}Cookies{{< /ui >}}: 기본 브라우저 쿠키에 추가할 쿠키를 정의합니다. [`Set-Cookie`][2] 구문을 사용하여 한 줄에 하나의 쿠키를 입력합니다.
   * {{< ui >}}HTTP Authentication{{< /ui >}}: 사용자 이름과 비밀번호로 HTTP Basic, Digest 또는 NTLM을 통해 인증합니다. 해당 자격 증명 정보는 브라우저 테스트의 모든 단계에서 사용됩니다. **참고**: 브라우저 시스템 프롬프트를 통해 사용자 자격 증명을 요청하는 웹사이트의 경우, HTTP Basic을 사용해 인증할 수 있습니다.

   요청 옵션은 테스트 실행마다 설정되며, 기록 시간이 아닌 실행 시간에 브라우저 테스트의 모든 단계에 적용됩니다. 다음 단계를 기록할 목적으로 이러한 옵션을 활성화 상태로 유지해야 하는 경우, 기록 중인 페이지에서 수동으로 옵션을 적용하고 테스트에서 다음 단계를 생성하세요.
...
[3]: /ko/synthetics/browser_tests/test_results#resources
   {{% /tab %}}

   {{% tab "인증서" %}}

   테스트에서 서버 인증서의 오류를 건너뛰도록 지시하려면 {{< ui >}}Ignore server certificate error{{< /ui >}}를 선택하세요.

   * {{< ui >}}Client Certificate{{< /ui >}}: {{< ui >}}Upload File{{< /ui >}}을 클릭하고 인증서 파일과 프라이빗 키를 업로드하여 클라이언트 인증서가 필요한 시스템에서 테스트를 수행합니다. PEM 인증서만 허용됩니다.
   * {{< ui >}}Client Certificate Domains{{< /ui >}}: 인증서 파일이 업로드되면 클라이언트 인증서는 시작 URL의 도메인에 적용됩니다. 다른 도메인에 클라이언트 인증서를 적용하려면 {{< ui >}}Value{{< /ui >}} 필드에 해당 도메인을 지정합니다.

   URL에 와일드카드를 포함할 수 있습니다.

   {{% /tab %}}

   {{% tab "프록시" %}}

   요청을 전송할 프록시의 URL을 {{< ui >}}Proxy URL{{< /ui >}} 필드에 `http://<YOUR_USER>:<YOUR_PWD>@<YOUR_IP>:<YOUR_PORT>`로 입력합니다.
   ...
   테스트 단계에서 스크린샷이 찍히지 않도록 하려면 {{< ui >}}Do not capture any screenshots for this test{{< /ui >}}를 선택하세요.
...
[1]: https://www.loc.gov/standards/iso639-2/php/code_list.php
   ...
   차단된 요청은 테스트 실행 중에 건너뛰기 되지만 [기록 단계](/synthetics/browser_tests/test_steps)에서는 페이지 렌더링에 영향을 주지 않습니다. 테스트 실행의 [{{< ui >}}Resources{{< /ui >}} 탭](/synthetics/browser_tests/test_results#resources)에서 차단된 요청을 조회하세요. 차단된 요청의 상태는 `blocked`입니다.
...
브라우저 테스트 세부 정보의 {{< ui >}}Starting URL{{< /ui >}} 및 {{< ui >}}Advanced Options{{< /ui >}}와 테스트 기록에서 [{{< ui >}}Settings{{< /ui >}}에 정의된 전역 변수][4]를 사용할 수 있습니다.

사용 가능한 변수의 목록을 표시하는 방법은 다음과 같습니다.

- 브라우저 테스트의 세부 정보에서 원하는 필드에 '{{'를 입력합니다.

  {{< img src="synthetics/browser_tests/use_global_variables_1.mp4" alt="전역 변수에서 로컬 변수 정의" video="true" width="90%" >}}

- 브라우저 테스트의 레코더에서 테스트의 변수를 가져온 다음 원하는 필드에 '{{'를 입력하거나 애플리케이션에 변수를 주입하여 사용합니다.

  {{< img src="synthetics/browser_tests/use_global_variables_2.mp4" alt="브라우저 기록 시 필드에 로컬 변수 주입" video="true" width="90%" >}}

브라우저 테스트 기록 시 변수를 사용하는 방법에 대한 자세한 내용은 [브라우저 테스트 단계][5]를 참조하세요.

### 경보 조건 정의 {#define-alert-conditions}

경보 조건을 사용자 지정하여 테스트가 경보를 전송하는 상황을 정의할 수 있습니다.

{{< img src="synthetics/browser_tests/alerting_rules_2.png" alt="브라우저 테스트 경보 규칙" style="width:80%" >}}

#### 경보 규칙 {#alerting-rule}

`n`/`N`개의 위치에서 `X`분 동안 어설션이 실패하면 경보가 트리거됩니다. 이 경보 규칙을 활용하면 경보가 트리거되기 전에 테스트의 실패 기간과 테스트 실패 위치 수를 지정할 수 있습니다.
...
실패할 경우, 해당 위치가 실패로 표시되기 전에 재시도를 `X`회 실시합니다. 이 설정을 통해 위치가 실패로 간주되기 위해 필요한 연속 테스트 실패 횟수를 정의할 수 있습니다. 기본적으로, 실패한 테스트를 재시도하기 전에 `300ms`를 대기합니다. 이 간격은 [API][6]로 구성할 수 있습니다.

#### 빠른 재시도 {#fast-retry}

테스트가 실패할 경우 빠른 재시도를 사용하면 실패로 표시되기 전에 Yms 후에 테스트를 X회 재시도할 수 있습니다. 재시도 간격을 사용자 지정하면 오탐을 줄이고 경보의 정확성을 향상시킬 수 있습니다.
...
경보 조건 세트에 따라 알림이 전송됩니다. 이 섹션을 사용하여 팀에 메시지를 전송하는 방법과 내용을 정의할 수 있습니다.

1. 브라우저 테스트용 {{< ui >}}message{{< /ui >}}를 입력하거나 사전에 채워진 모니터링 메시지를 사용합니다. 이 필드는 표준 [마크다운 형식][7]을 허용하며 다음 [조건 변수][8]를 지원합니다.

    | 조건 변수       | 설명                                                         |
    |----------------------------|---------------------------------------------------------------------|
    | `{{#is_alert}}`            | 모니터링을 통해 경보가 발생하면 표시됩니다.                                       |
    | `{{^is_alert}}`            | 모니터링을 통해 경보가 발생하지 않으면 표시됩니다.                                     |
    | `{{#is_recovery}}`         | 모니터링이 'alert'로부터 복구되면 표시됩니다.                          |
    | `{{^is_recovery}}`         | 모니터링이 'alert'로부터 복구되지 않으면 표시됩니다.                        |
    | `{{#is_renotify}}`         | 모니터링이 다시 알림을 전송하면 표시됩니다.                                   |
    | `{{^is_renotify}}`         | 모니터링이 다시 알림을 전송하지 않으면 표시됩니다.                                 |
    | `{{#is_priority}}`         | 모니터링이 우선순위(P1~P5)와 일치하면 표시됩니다.                  |
    | `{{^is_priority}}`         | 모니터링이 우선순위(P1~P5)와 일치하지 않으면 표시됩니다.                |
...
1. 필요시 페이지 오른쪽 상단의 {{< ui >}}Open in a pop-up{{< /ui >}}을 선택하여 테스트 기록을 별도의 팝업 창으로 열 수 있습니다. 이 옵션은 애플리케이션이 iframe에서 열리는 것을 지원하지 않거나 기록 중 크기 문제를 피하고 싶을 때 유용합니다. 테스트를 새 브라우저에서 시작하려면 {{< ui >}}Incognito mode{{< /ui >}}에서 팝업 창을 여세요. 이 모드는 이미 로그인된 세션, 기존 브라우저의 쿠키 등이 없는 상태입니다.
2. 필요시 브라우저 테스트에서 단계 기록을 실행할 때 Datadog이 RUM 데이터를 자동으로 수집하도록 활성화합니다. 자세한 내용은 [RUM 및 Session Replay 탐색][13]을 참조하세요.
3. {{< ui >}}Start Recording{{< /ui >}}을 클릭하여 브라우저 테스트 기록을 시작합니다.
4. 모니터링하려는 사용자 여정을 실행하는 애플리케이션을 클릭하면 액션이 자동으로 기록되어 왼쪽의 브라우저 테스트 시나리오 내의 [단계][14]를 생성하는 데 활용됩니다.
5. 자동으로 기록되는 테스트 단계 외에도 왼쪽 상단의 [단계][14]기능으로 더욱 다양한 시나리오를 생성할 수 있습니다.
   ...
   Datadog은 브라우저 테스트에서 실행된 여정으로 인해 예상한 상태가 도출되었는지 확인하기 위해 브라우저 테스트를 [어설션][12]으로 종료할 것을 권장합니다.
6. 시나리오를 완료했으면 {{< ui >}}Save and Launch Test{{< /ui >}}를 클릭합니다.
...
<p style="text-align: center;"><em>단계 위에 마우스를 올리고 재생 버튼을 클릭하여 이 단계만 리플레이합니다.</em></p>

<strong>2. 모든 단계 리플레이:</strong> 레코더에 정의된 전체 단계 시퀀스를 실행합니다.
...
<strong>3. 선택한 단계 리플레이:</strong> 단계 목록에서 선택한 단계의 하위 세트를 실행합니다.
...
{{< img src="synthetics/browser_tests/recording__replay--accepting-permission_2.mp4" alt="디버거 권한 수락" video="true" height="400px" >}}
<p style="text-align: center;"><em>세 개의 점 {{< img src="icons/kebab.png" inline="true" style="width:14px;">}} 메뉴를 클릭하여 권한을 수락합니다.</em></p>
...
[사용자 지정 역할 기능][15]을 사용하는 경우, `synthetics_read` 및 `synthetics_write` 권한이 포함된 사용자 지정 역할에 사용자를 추가하세요.

### 액세스 제한 {#restrict-access}

[세분화된 액세스 제어][17]를 사용하여 역할, 팀 또는 개별 사용자를 기반으로 테스트에 액세스할 수 있는 사용자를 제한하세요.
...
| 액세스 수준 | 테스트 구성 조회 | 테스트 구성 편집 | 테스트 결과 조회 | 테스트 실행 | 기록 조회 | 기록 편집 |
| ------------ | ----------------------- | ----------------------- | ------------------| --------- | -------------- | -------------- |
| 액세스 권한 없음    |                         |                         |                   |           |                |                |
| Viewer       | {{< X >}}               |                         | {{< X >}}         |           | {{< X >}}      |                |
| Editor       | {{< X >}}               | {{< X >}}               | {{< X >}}         | {{< X >}} | {{< X >}}      | {{< X >}}      |
