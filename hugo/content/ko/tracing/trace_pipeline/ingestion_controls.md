...
- 처리량이 많은 서비스 또는 엔드포인트의 트레이스 샘플링 레이트를 조정하여 할당량을 더 잘 관리하십시오.
...
{{< img src="tracing/trace_indexing_and_ingestion/ingestion_controls_page_redesigned.png" style="width:100%;" alt="Ingestion Control 페이지 개요" >}}

## 수집 구성 이해하기 {#understanding-your-ingestion-configuration}

페이지 헤더의 데이터를 사용하여 트레이스 수집을 모니터링하십시오:

월간 수집
: 이번 달 현재까지 수집된 데이터 양, 예상 월말 수집량, 월간 할당량을 보여주며, 예상 수집량이 할당량 내에 유지될지 여부를 나타내는 상태 메시지를 함께 표시합니다.

APM 적응형 샘플링
: 월간 할당량 중 [적응형 샘플링][17] 타겟이 차지하는 비율과 서비스 제한 대비 온보딩된 서비스 수를 보여줍니다.

시간에 따른 수집량
: 출처(APM, 기타 Datadog 제품 또는 OTel)별로 구분된 시간에 따른 수집량 그래프로, 월간 할당량이 참조선으로 표시됩니다.

예상 수집량이 월간 할당량 미만이면 정상적으로 진행 중인 것입니다. 예상 수집량이 할당량을 초과할 것으로 예상되면 [서비스 수집 레이트](#configuring-ingestion-for-a-service) 또는 [적응형 샘플링 타겟][21]을 조정하십시오.

### 서비스별 수집 수준 {#ingestion-levels-by-service}

서비스 표에는 서비스별로 구분된 수집량 및 수집 구성에 관한 정보가 포함되어 있습니다. 표 위의 **모든 서비스**, **APM 관리**, **기타 Datadog 제품 관리** 및 **OTel 관리** 탭을 사용하여 수집 출처별로 서비스를 필터링하십시오.
...
트래픽 분석
: 서비스에서 시작된, 샘플링된 트레이스와 샘플링되지 않은 트레이스의 상세한 트래픽 분석입니다. 서비스 트래픽의 25% 이상이 레이트 리미터에 의해 버려지면, 해당 분류 옆에 경고 아이콘이 표시됩니다. 자세한 내용은 [트래픽 분석](#traffic-breakdown)을 참조하세요.

구성
: 서비스의 수집 출처를 보여주며, APM 서비스의 경우 샘플링 구성이 어떻게 이루어지는지 표시합니다.: `APM Agent`Agent의 [기본 헤드 기반 샘플링 메커니즘][4]이 적용되는 경우, `APM Local`SDK 구성에서 [샘플링 규칙][8]이 적용되는 경우, 또는 `APM Remote`UI에서 원격으로 사용자 정의 또는 [적응형 샘플링][17] 샘플링 규칙이 적용되는 경우입니다. OpenTelemetry를 통한 수집은 `OTel`으로 표시됩니다. 기타 Datadog 제품에서 발생하는 수집은 `RUM`, `Synthetics`, `AppSec`, `CI Visibility` 또는 `AWS X-Ray`로 표시됩니다. 둘 이상의 카테고리에 해당하는 서비스는 `N Configurations` 배지를 표시합니다. 배지 위에 마우스를 올리면 전체 목록이 표시됩니다. 서비스의 수집 구성과 관련한 자세한 정보는 [기본 수집 레이트 변경](#configuring-ingestion-for-a-service)을 참조하십시오.

적응형 샘플링
: 서비스의 [적응형 샘플링][17] 적용 상태를 보여줍니다. `Full` 또는 `Partial`은 서비스가 적응형 샘플링의 적용을 받는 경우(배지 위에 마우스를 올리면 적용 비율을 확인할 수 있습니다), `Compatible`은 서비스를 온보딩할 수 있으나 아직 온보딩되지 않은 경우, `Not Compatible`은 서비스의 트레이서 또는 Agent 버전이 적응형 샘플링을 지원하지 않는 경우, `Disabled`는 서비스에 대해 적응형 샘플링이 명시적으로 해제된 경우입니다.

인프라
: 서비스가 실행되고 있는 호스트, 컨테이너 및 함수입니다. 이 열은 기본적으로 숨겨져 있습니다. 표의 **옵션** 메뉴에서 활성화하십시오.

서비스 이름, 환경, 구성 또는 적응형 샘플링 상태별로 페이지를 필터링하거나 **루트 서비스만**을 토글하여 조치가 필요한 서비스를 조회하십시오. 전역 수집 볼륨을 줄이려면 표를 `Downstream Bytes/s` 열 기준으로 정렬하여 수집에서 점유율이 가장 큰 서비스를 조회합니다.
...
- {{< ui >}}Adaptive sampling{{< /ui >}}: 구성된 월별 수집량 목표에 맞춰 샘플링 레이트를 자동으로 조정합니다.
...
[21]: /ko/tracing/trace_pipeline/adaptive_sampling/#configure-the-adaptive-sampling-target