---
aliases:
- /ko/service_management/events/pipelines_and_processors/grok_parser/
description: 사용자 지정 grok 규칙을 생성해 전체 메시지 또는 원시 이벤트의 특정 속성을 구조화된 데이터로 구문 분석
title: Grok 파서
---
## 개요 {#overview}
...
{{< img src="service_management/events/grok-parser.png" alt="구문 분석 예시 1" style="width:80%;">}}


**내 이벤트 구문 분석**을 클릭하면 기저 파이프라인을 통과하는 이벤트에 대한 구문 분석 규칙 세 개로 구성된 세트를 시작합니다. 여기에서 속성 이름 지정을 미세 조정하고, 필요한 경우 다른 유형의 이벤트에 대한 새 규칙을 추가합니다. 이 기능을 사용하려면 상응하는 이벤트가 인덱싱되고 실제로 유입되는 상태여야 합니다. 해당 기능이 작동하도록 하기 위해 일시적으로 제외 필터를 비활성화하거나 다운샘플링할 수 있습니다.
...
{{< img src="logs/processing/processors/_parser.png" alt="구문 분석 예시 1" style="width:80%;">}}
...
* 정규식 매처는 문자열의 시작과 매칭하기 위해 암묵적 `^`를, 문자열의 끝과 매칭하기 위해 `$`를 적용합니다.
* 특정 이벤트는 큰 공백을 생성할 수 있습니다. `\n` 및 `\s+`를 사용하여 줄 바꿈 및 공백을 고려하세요.
...
`word`
: _(밑줄) 문자를 포함하여 a~z, A~Z, 0~9를 매칭합니다.
...
: user-agent를 구문 분석하고 장치, OS 및 Agent가 나타내는 브라우저를 포함한 JSON 개체를 반환합니다. [User Agent 프로세서를 확인하세요][1].
...
{{< img src="logs/processing/parsing/parsing_attribute.png" alt="명령줄 구문 분석" style="width:80%;">}}

### 도우미 규칙을 사용하여 여러 구문 분석 규칙을 공통화 {#using-helper-rules-to-factorize-multiple-parsing-rules}
...
{{< img src="logs/processing/parsing/helper_rules.png" alt="도우미 규칙" style="width:80%;">}}
...
**keyvalue**와 같은 필터를 사용하여 keyvalue 또는 logfmt 형식의 문자열을 속성으로 더 쉽게 매핑합니다.
...
{{< img src="logs/processing/parsing/parsing_example_2.png" alt="구문 분석 예시 2" style="width:80%;">}}

파라미터 이름은 이미 이벤트에 포함되어 있으므로 지정할 필요가 없습니다.
규칙 패턴에 **추출** 속성 `my_attribute`를 추가하면 다음과 같은 내용이 표시됩니다.

{{< img src="logs/processing/parsing/parsing_example_2_bis.png" alt="구문 분석 예시 2 bis" style="width:80%;">}}
...
{{< img src="logs/processing/parsing/key_value_parser.png" alt="키 값 파서" style="width:80%;" >}}

이벤트의 속성 값에 특수 문자가 포함된 경우(예를 들어 url에 `/`가 있는 경우) 이를 구문 분석 규칙의 허용 목록에 추가하세요.
...
* `data` 개체에서 *keyvalue* 필터를 정의하고 이 필터가 매칭되지 않으면 빈 JSON `{}`가 반환됩니다(예를 들어 입력: `key:=valueStr`, 구문 분석 규칙: `rule_test %{data::keyvalue("=")}`, 출력: `{}`).
...
* `GMT`, `UTC`, `UT` 또는 `Z`
* `+h`, `+hh`, `+hh:mm`, `-hh:mm`, `+hhmm`, `-hhmm`, `+hh:mm:ss`, `-hh:mm:ss`, `+hhmmss` 또는 `-hhmmss`. 지원되는 최대 범위는 +18:00~-18:00(양쪽 끝값 포함)입니다.
...
**참고**: 날짜를 구문 분석해도 그 값을 이벤트의 공식 날짜로 설정하지 **않습니다**. 이 경우 이후 프로세서에서 [이벤트 날짜 리매퍼][3]를 사용하세요.

### 대체 패턴 {#alternating-pattern}

속성 하나만 다른 두 가지 형식을 가질 수 있는 이벤트가 있는 경우, `(<REGEX_1>|<REGEX_2>)` 형태로 대체 패턴을 사용해 단일 규칙을 설정하세요. 이 규칙은 부울 OR과 동등합니다.
...
{{< img src="logs/processing/parsing/parsing_example_4.png" alt="구문 분석 예시 4" style="width:80%;" >}}

{{< img src="logs/processing/parsing/parsing_example_4_bis.png" alt="구문 분석 예시 4 bis" style="width:80%;" >}}

### 선택 속성 {#optional-attribute}

일부 이벤트는 가끔씩만 나타나는 값을 포함합니다. 이런 경우, `()?`를 사용해 속성 추출을 선택 사항으로 설정하세요.
...
{{< img src="logs/processing/parsing/parsing_example_5.png" alt="구문 분석 예시 5" style="width:80%;" >}}

{{< img src="logs/processing/parsing/parsing_example_5_bis.png" alt="구문 분석 예시 5 bis" style="width:80%;" >}}
...
{{< img src="logs/processing/parsing/nested_json.png" alt="중첩된 JSON 구문 분석 예시" style="width:80%;" >}}
...
{{< img src="logs/processing/parsing/regex_parsing.png" alt="구문 분석 예시 6" style="width:80%;" >}}
...
{{< img src="logs/processing/parsing/array_parsing.png" alt="구문 분석 예시 6" style="width:80%;" >}}
...
* `separator`: 서로 다른 값을 구분하는 데 사용되는 구분 기호를 정의합니다. 문자 한 개만 허용됩니다. 기본값: `,`. **참고**: TSV의 탭 문자를 나타내려면 `separator`로 `tab`을 사용하세요.
...
무엇이 필요한지 구문 분석을 마쳤고, 그 시점 이후 텍스트를 삭제해도 안전하다는 사실을 알고 있는 이벤트라면 데이터 매처를 사용해 삭제하면 됩니다. 다음 이벤트 예시의 경우, `data` 매처를 사용해 맨 끝의 `%`를 삭제할 수 있습니다.
...
이벤트에 ASCII 제어 문자가 포함된 경우, 수집 시 직렬화됩니다. 이러한 문자는 Grok 파서 내에서 직렬화된 값으로 명시적으로 이스케이프하여 처리할 수 있습니다.
