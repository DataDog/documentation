---
description: Observability Pipelines Worker를 사용하여 Amazon OpenSearch로 로그를 전송하는 방법을
  알아보세요.
disable_toc: false
products:
- icon: logs
...
title: Amazon OpenSearch 목적지
---
{{< product-availability >}}

## 개요 {#overview}

Observability Pipelines의 Amazon OpenSearch 목적지를 사용하여 Amazon OpenSearch로 로그를 전송하세요.

## 설정 {#setup}

<div class="alert alert-danger">시크릿 관리의 경우 Amazon OpenSearch 엔드포인트 URL과, 해당하는 경우 사용자 이름과 비밀번호의 식별자만 입력하세요. 실제 값은 <b>입력하지 마세요</b>.</div>

[파이프라인을 설정][6]할 때 Amazon OpenSearch 목적지를 구성하세요. 파이프라인은 [UI][1]에서 설정할 수 있으며, [API][7] 또는 [Terraform][8]을 사용하여 설정할 수 있습니다. 이 섹션에서 설명하는 단계는 UI에서 설정합니다.

파이프라인 UI에서 Amazon OpenSearch 목적지를 선택한 후:

1. Amazon OpenSearch 엔드포인트 URL의 식별자를 입력합니다. 비워두면 [기본값](#secret-defaults)이 사용됩니다.
1. {{< ui >}}Mode{{< /ui >}} 드롭다운 메뉴에서 {{< ui >}}Bulk{{< /ui >}} 또는 {{< ui >}}Data streams{{< /ui >}}를 선택합니다.
	- {{< ui >}}Bulk{{< /ui >}} 모드
		- Amazon OpenSearch의 [Bulk API][4]를 사용하여 일괄 처리된 이벤트를 표준 인덱스로 직접 전송합니다.
		- 인덱스 명명 및 수명 주기 관리를 직접 제어하려는 경우 이 모드를 선택하세요. 데이터는 지정한 인덱스에 추가되며, 롤오버, 삭제 및 매핑 처리는 사용자가 담당합니다.
		- {{< ui >}}Bulk{{< /ui >}} 모드 구성 방법:
			- 필요시 {{< ui >}}Index{{< /ui >}} 필드에 Amazon OpenSearch 인덱스 이름을 입력하세요. [템플릿 구문][3]을 사용하여 로그의 특정 필드를 기반으로 로그를 다른 인덱스로 동적으로 라우팅할 수 있습니다(예: `logs-{{service}})`.
...
1. 필요시 Amazon OpenSearch 인덱스 이름을 입력합니다. 로그의 특정 필드를 기반으로 서로 다른 인덱스로 로그를 라우팅하려면 [템플릿 구문][3]을 참조하세요.
1. 인증 방식을 선택합니다({{< ui >}}Basic{{< /ui >}} 또는 {{< ui >}}AWS{{< /ui >}}). 선택한 방식에 따라 다음을 설정하세요.
	- {{< ui >}}Basic{{< /ui >}}:
		- Amazon OpenSearch 사용자 이름의 식별자를 입력하세요. 비워두면 [기본값](#secret-defaults)이 사용됩니다.
		- Amazon OpenSearch 비밀번호의 식별자를 입력하세요. 비워두면 [기본값](#secret-defaults)이 사용됩니다.
	- {{< ui >}}AWS{{< /ui >}}:
		1. AWS 리전을 입력합니다.
		1. (필요시) AWS 인증 옵션을 선택합니다. {{< ui >}}Assume role{{< /ui >}} 옵션은 이전에 생성한 사용자 또는 역할이 특정 AWS 리소스에 액세스하기 위해 다른 역할을 맡아야 하고 해당 권한을 명시적으로 정의해야 하는 경우에만 사용해야 합니다.<br>{{< ui >}}Assume role{{< /ui >}}을 선택하는 경우:
...
- Amazon OpenSearch 엔드포인트 URL 식별자:
	- 기본 식별자는 `DESTINATION_AMAZON_OPENSEARCH_ENDPOINT_URL`입니다.
- Amazon OpenSearch 인증 사용자 이름 식별자:
	- 기본 식별자는 `DESTINATION_AMAZON_OPENSEARCH_USERNAME`입니다.
- Amazon OpenSearch 인증 비밀번호 식별자:
	- 기본 식별자는 `DESTINATION_AMAZON_OPENSEARCH_PASSWORD`입니다.
