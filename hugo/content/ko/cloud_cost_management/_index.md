...
- link: /cloud_cost_management/tags/
  tag: 설명서
  text: Cloud Cost Management의 태그에 대해 알아보기
- link: /cloud_cost_management/cloud_cost_skill/
  tag: 설명서
  text: Bits Chat에서 Cloud Cost Skill 사용
...
---
{{< learning-center-callout header="교육 웨비나 세션 참가" hide_image="true" btn_title="등록" btn_url="https://www.datadoghq.com/technical-enablement/sessions/?tags.topics-0=Cloud+Cost+Management">}}
  클라우드 공급자 비용을 탐색하고 이를 실시간 텔레메트리 데이터와 상호 연계합니다. 클라우드 비용이 어디에서 발생하는지, 어떻게 변화하고 있는지, 최적화할 수 있는 영역이 어디인지에 대한 실행 가능한 인사이트와 경보를 제공합니다.
...
{{< img src="cloud_cost/summary.png" alt="Datadog의 Cloud Cost Summary 페이지에서 클라우드 공급자의 전체 비용 및 사용량에 대한 인사이트를 확인할 수 있습니다." style="width:100%;" >}}

Datadog은 클라우드 비용 데이터를 수집하여 [**탐색기** 페이지][1]의 검색 쿼리에서 사용할 수 있는 메트릭으로 변환합니다. 비용이 증가한 경우 사용량 메트릭과 상호 연계하여 근본 원인을 파악할 수 있습니다.
...
{{< whatsnext desc="Cloud Cost Management로 클라우드 비용 관리를 시작하려면 다음 설명서를 참조하세요.">}}
  ...
  {{< nextlink href="/cloud_cost_management/setup/saas_costs">}}<u>SaaS 및 AI 비용</u>: 지원되는 SaaS 비용 공급자의 비용 데이터를 Datadog으로 전송합니다. {{< /nextlink >}}
  {{< nextlink href="/cloud_cost_management/setup/custom">}}<u>Custom Cost</u>: 모든 비용 데이터 소스를 Datadog에 업로드합니다. {{< /nextlink >}}
  {{< nextlink href="/cloud_cost_management/datadog_costs">}}<u>Datadog Cost</u>: 일일 Datadog 지출 및 사용률 메트릭을 시각화합니다. {{< /nextlink >}}
 {{< /whatsnext >}}
...
15개월의 보존 기간 동안 일일 Datadog 지출을 관련 사용률 메트릭과 함께 시각화하여 잠재적인 비효율성과 비용 절감 기회를 파악할 수 있습니다. 자세한 내용은 [Datadog 비용][8]을 참조하세요.

대시보드를 생성할 때 데이터 소스로 {{< ui >}}Cloud Cost{{< /ui >}}를 선택한 후 사용 가능한 비용 유형에서 {{< ui >}}Datadog{{< /ui >}}를 선택합니다.

{{< img src="cloud_cost/datadog_costs/dashboard-updated.png" alt="대시보드의 Cloud Cost 데이터 소스에서 선택 가능한 Datadog 비용" style="width:80%;" >}}
...
[Cloud Cost Monitor][3]를 생성하여 클라우드 지출을 사전에 관리하고 최적화할 수 있습니다. {{< ui >}}Cost Changes{{< /ui >}}, {{< ui >}}Cost Anomalies{{< /ui >}}, {{< ui >}}Cost Threshold{{< /ui >}}, {{< ui >}}Cost Forecast{{< /ui >}} 또는 {{< ui >}}Budget{{< /ui >}} 모니터링 유형 중에서 선택하십시오. 각 유형에 대한 자세한 내용은 [Cloud Cost Monitors][3]를 참조하십시오.

{{< img src="cloud_cost/monitor-2.png" alt="비용 변화에 대해 경보를 보내는 Cloud Cost 모니터 생성" style="width:100%;" >}}

## 비용 할당 {#allocate-costs}

[컨테이너 비용 할당 메트릭][4]을 사용하여 Kubernetes, Amazon ECS, Azure 및 Google Cloud 전반의 클러스터 및 워크로드와 관련된 비용을 파악할 수 있습니다. 포드 수준의 비용을 확인하고, 유휴 리소스 비용을 식별하며, 리소스 유형별 비용을 분석할 수 있습니다.
...
{{< img src="cloud_cost/ccm-data-history.png" alt="Cloud Cost 설정에서 Cloud Cost 데이터 이력을 조회할 수 있습니다." style="width:100%;" >}}

{{< ui >}}Cloud Cost{{< /ui >}} > {{< ui >}}Settings{{< /ui >}} > {{< ui >}}Data History{{< /ui >}} 페이지에서 클라우드 비용 데이터의 최신 상태 및 처리 상태를 모니터링합니다.

- {{< ui >}}Last Bill Received{{< /ui >}}: Datadog이 클라우드 또는 SaaS 제공업체로부터 청구 데이터를 마지막으로 수신한 시점입니다. 이 타임스탬프는 해당 데이터의 최신 사용량이나 비용 날짜를 나타내지 않습니다.
...
Cloud Cost 데이터는 제공업체가 제공한 데이터만큼 최신 상태일 수 있습니다. 예상 날짜의 비용이 누락된 경우, 제공업체의 청구서 또는 내보낸 데이터와 CCM을 비교하십시오. 소스에 예상 날짜에 대한 비용이 포함되어 있지 않으면 제공업체에 문의하십시오. 소스에는 해당 비용이 포함되어 있는데 CCM에는 없다면 [Datadog Support][11]에 문의하십시오.

## AI를 활용한 비용 분석 {#use-ai-for-cost-analysis}

[Bits Chat의 Cloud Cost Skill][10]을 사용하여 비용 변동을 조사하고, 비용의 책임자를 식별하며, 예산 대비 지출을 비교하고, 비용과 관측 가능성 메트릭을 상호 연계하며, 엔지니어링 팀을 위한 인계용 노트북을 생성할 수 있습니다.

{{< img src="cloud_cost/cc_skill_cost_summary.png" alt="초기 분석을 보여주는 Bits Chat의 조사 요약" style="width:60%;" >}}
