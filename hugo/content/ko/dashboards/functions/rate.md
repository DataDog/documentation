...
dt() 함수는 관련된 그룹 개수와 관계 없이 시계열 하나만 반환합니다. 이 시계열 내에서 여러 그룹의 전송 시점의 시간 차를 고려합니다.
...
| 함수       | 설명                                                                                                                                        | 예시                          |
|:---------------|:---------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------|
| `throughput()` | 초당 값을 구하기 위해 각 값을 시간 버킷 내 초 수로 나눠서 시계열을 초당 속도로 전환합니다. | `throughput(<METRIC_NAME>{*})` |
