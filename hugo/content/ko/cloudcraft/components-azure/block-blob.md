---
title: Block Blob 구성 요소
---

## 개요

Block Blob 구성 요소를 사용해 Azure 환경에서 블록 블롭을 표현 및 시각화할 수 있습니다.

{{< img src="cloudcraft/components-azure/block-blob/component-block-blob-diagram.png" alt="상호 연결된 Azure 컴포넌트를 보여주는 등축도법 Cloudcraft 다이어그램 스크린샷" responsive="true" style="width:60%;">}}
...
- **Tier**: 블롭의 스토리지 티어를 선택합니다.
- **Redundancy**: 주요 및 보조 지역에서 데이터를 복제하는 방법을 선택합니다.
- **Storage (GiB)**: 블롭에서 사용 가능한 총 데이터 볼륨을 기비바이트로 입력합니다.

## API

[Cloudcraft API][1]를 사용해 프로그래밍을 통해 액세스하여 아키텍처 다이어그램을 JSON 개체로 렌더링할 수 있습니다.  다음은 Black Blob 구성 요소의 JSON 개체 예시입니다.
...
- **type: string**: 구성 요소 유형. 이 구성 요소의 `azureblob` 값 문자열이어야 합니다.
...
- **tier: string**: 블롭의 스토리지 수준 티어입니다. 허용되는 값에는 `Premium`, `Hot`, `Cool`, `Standard`가 있습니다. 기본값은 `Standard`입니다.
- **redundancy: string**: 리전 간 데이터 복제하는 방법에 관한 중복성 옵션입니다. `LRS` , `ZRS`, `GRS`, `RA-GRS` 네 개 값 중 하나를 허용하며, 기본값은 `LRS`입니다.
- **storageGb: number**: 블롭에 사용할 수 있는 총 데이터 볼륨을 기가바이트로 표현합니다. 기본값은 `0`입니다.
