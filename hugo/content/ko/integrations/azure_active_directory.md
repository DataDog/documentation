...
short_description: Microsoft Entra ID 활동 로그 분석
supported_os:
- 리눅스
- windows
- macos
tile:
  ...
  description: Microsoft Entra ID 활동 로그 분석
  media: []
...
Microsoft Entra ID는 사용자가 외부 리소스에 액세스할 수 있도록 도와드리는 클라우드 호스팅 ID 및 액세스 관리 서비스입니다.
본 통합으로 [Microsoft Entra ID][1] 감사 및 로그인 로그를 Datadog으로 전달합니다.

## 설정

### 설치

본 통합은 Event Hub와 함께 Azure를 사용하여 Datadog에 로그를 전달합니다. 활동 로그를 이벤트 허브에 전달하도록 Entra ID를 구성합니다.

### 구성

1. [Datadog에 Azure 로그 보내기][2] 가이드에 따라 Event Hub를 사용하여 Azure에서 Datadog으로 로그 전달 파이프라인을 설정합니다.

2. Azure 포털에서 _Microsoft Entra ID > Monitoring > Audit logs_를 선택합니다.

3. **데이터 설정 내보내기**를 선택합니다.
...
5. **이벤트 허브로 스트리밍** 체크 박스를 선택합니다.

6. 로그를 라우팅하기 위해 이전에 만든 Azure 구독 및 Event Hub 네임스페이스를 선택합니다.

7. 다음 중 하나 또는 둘 다를 수행합니다. Datadog에서는 두 가지를 모두 선택할 것을 권장합니다.
...
Datadog은 Azure 앱 등록, Key Vault 키, Key Vault 암호 및 Key Vault 인증서의 크리덴셜 만료를 가시적으로 확인할 수 있도록 크리덴셜 만료 이벤트를 전송합니다. Azure 앱 등록을 위한 이벤트를 수신하려면 Microsoft Entra ID 통합을 설치해야 합니다. Azure에서 이벤트를 수신하려면 [Azure 통합][4]도 설치해야 합니다.
