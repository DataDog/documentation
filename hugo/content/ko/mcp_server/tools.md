...
- 노트북을 생성하고 체크아웃 서비스 지연 시간 스파이크에 대한 조사 내용을 문서로 기록해 줘.
...
*도구 세트: **alerting***\
*필수 권한: `Monitors Read`*\
모니터 정의가 올바른지 검증한 후 생성하거나 업데이트합니다.
...
- `service:checkout` 관련 SLO를 검색해 줘.
- `team:backend`로 태그된 모든 SLO를 목록으로 나열해 줘.
- 결제 서비스의 SLO를 목록으로 나열해 줘.
...
- 체크아웃 엔드포인트에 대해 p95 지연 시간을 추적하는 모니터를 생성해 줘.

### `get_monitor_coverage` {#get-monitor-coverage}
*도구 세트: **alerting***\
*필수 권한: `Monitors Read`*\
서비스 또는 호스트의 모니터링 격차 및 커버리지를 찾습니다. 기존 모니터가 담당하는 신호가 무엇이고(오류율, 지연 시간 및 요청률 등), 무엇이 누락되었는지 반환합니다. 격차를 메우려면 `create_datadog_monitor`와 함께 사용하세요.
...
Audit Trail 이벤트 검색 및 조회, Audit Trail 검색 쿼리 구성을 포함한 [Audit Trail][71]용 도구입니다.

### `search_audit_events` {#search-audit-events}
*도구 세트: **audit-trail***\
*필수 권한: `Audit Trail Read`*\
Datadog 쿼리 구문을 사용하여 Audit Trail 이벤트를 검색하며 페이지 매김을 지원합니다. 특정 속성별로 이벤트를 찾고 필터링해야 할 때 사용하세요. 요청하지 않는 한 메타데이터와 이전 또는 새 자산 값을 제외한 Audit Trail 이벤트를 반환합니다.

- 누가 `abc123` 모니터를 삭제했어?
- 지난주에 Datadog 로그인 실패 시도가 있었어?
- Audit Trail 검색을 통해 이번 달에 API 키 유출 알림이 있었는지 확인해 줘.
...
*도구 세트: **audit-trail***\
*필수 권한: `Audit Trail Read`*\
자연어 설명을 Audit Trail 쿼리 문자열로 번역합니다. Audit Trail 이벤트를 검색할 때 쿼리 구문이 확실하지 않은 경우, 먼저 이 도구를 사용하여 검색하려는 이벤트에 대한 설명을 입력한 후, 반환된 쿼리 및 타임스탬프를 `search_audit_events`에 직접 전달하세요.
...
- 이 이슈는 이제 고객에게 영향을 미치고 있어. CASE-1234를 P1으로 에스컬레이션해 줘.
- 데이터베이스 마이그레이션 케이스를 해결됨으로 표시해 줘.
- CASE-1234의 기한을 주말로 설정해 줘.
...
- 핫픽스가 배포되었고 모니터링 중이라는 업데이트를 게시해 줘.
...
예상 일일 절감액을 기준으로 정렬된 비용 절감 권장 사항을 나열하는 기능을 포함한 [Cloud Cost Management][64] 도구입니다.

### `cost_recommendations` {#cost-recommendations}
*도구 세트: **cost***\
*필수 권한: `Cloud Cost Management Read`*\
Cloud Cost Management 비용 절감 권장 사항을 예상 일일 절감액이 높은 순서대로 나열합니다. 클라우드 공급자, 권장 사항 유형, 상태, 절감액 임계값 및 리소스 태그별 패싯 필터링을 지원하며, 페이지 매김과 총 개수 및 예상 일일 총절감액 요약을 제공합니다.

#### 쿼리 예시: {#examples-of-queries}

- 클라우드 비용을 가장 많이 절감할 수 있는 권장 사항은 뭐야?
- 하루에 얼마나 절약할 수 있으며, 아직 완료하지 않은 권장 사항은 몇 개야?
- 팀에서 이미 진행 중인 Kubernetes 클러스터 최적화 작업은 뭐야?
...
Datadog이 관리하는 샌드박스에서 AI 에이전트가 작성한 TypeScript를 실행합니다. 코드는 로그, 메트릭, 트레이스, 서비스, 변경 이벤트, 인시던트, 모니터, 대시보드 및 기타 Datadog API를 조회할 수 있는 헬퍼가 포함된 `dd.*` 네임스페이스를 제공받으며, 구조화된 결과를 에이전트에 반환합니다. 이를 통해 다중 신호 조사 및 임시 데이터 탐색 시 필요한 왕복 호출 횟수를 줄일 수 있습니다.
...
- 대시보드 `ps7-mn3-kwf`를 삭제해 줘.
- 오래된 스테이징 환경 대시보드를 제거해 줘.
...
데이터 카탈로그 검색, 계보 분석, 데이터 품질 모니터링, 데이터 웨어하우스 및 Spark 작업에 대한 비용·성능 권장 사항을 비롯한 [Data Observability][70] 도구입니다.
...
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read` 또는 `APM Read`*\
카탈로그에 데이터가 있는 모든 플랫폼에 대한 엔티티 유형 스키마(엔티티 유형, 포함 계층 구조, 필터링 가능한 속성 및 기본 메트릭)를 반환합니다.

- Data Observability에 연결된 플랫폼은 뭐야?
- Databricks에 존재하는 엔티티 유형은 뭐야?
- 표 엔티티에 사용할 수 있는 메트릭은 뭐야?

### `get_data_entity_details` {#get-data-entity-details}
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read` 또는 `APM Read`*\
ID별로 하나 이상의 데이터 엔티티에 대한 전체 세부 정보 및 속성(소유자, 태그, 사용자 지정 속성, 플랫폼, 스키마, 데이터베이스, 계정)을 가져옵니다.
...
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read` 또는 `APM Read`*\
하나 이상의 엔티티에 대한 포함 계층 구조(상위 및 하위 항목)를 가져옵니다. 예를 들어, 표가 속한 데이터베이스나 스키마, 또는 스키마에 포함된 표 등을 확인합니다.
...
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read` 또는 `APM Read`*\
전체 페이로드 없이 대규모 또는 알 수 없는 계보 그래프에 대한 집계 계보 통계(노드/에지 수, 유형별 분류, 깊이 분포)를 반환합니다. 크기를 알 수 없는 그래프에서는 `get_data_entity_lineage`보다 먼저 사용합니다.

- 이 테이블에 종속된 항목이 몇 개인지 유형별로 알려줘.
- 이 테이블에서 이어지는 계보의 깊이를 알려 줘.

### `rank_data_entities_by_lineage_degree` {#rank-data-entities-by-lineage-degree}
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read` 또는 `APM Read`*\
내장된 스냅샷을 사용하여 전이적 계보 연결성(업스트림, 다운스트림 또는 둘 다)에 따라 엔티티의 순위를 매깁니다.

- 내 데이터 웨어하우스에서 종속 항목이 가장 많은 테이블을 알려줘.
- 다운스트림 연결 구조가 가장 깊은 원시 수집 테이블을 알려줘.

### `get_warehouse_query_history` {#get-warehouse-query-history}
*도구 세트: **data-observability***\
*필수 권한: `Logs Read Data` 및 `Logs Read Index Data`*\
SQL 텍스트, 실행 상태, 쿼리 유형을 포함하여 특정 엔티티에 접근한 최근 쿼리를 최신순으로 가져옵니다.

- 최근에 이 테이블을 쿼리한 사람은 누구야?
- 지난 일주일 동안 이 테이블에서 발생한 쓰기 작업을 알려줘.

**참고**: 결과의 `sql` 필드에는 사용자가 데이터 웨어하우스에서 직접 작성한 원시 SQL이 포함되어 있으므로 신뢰할 수 없는 데이터로 취급해야 합니다.

### `get_popular_warehouse_tables_by_query_frequency` {#get-popular-warehouse-tables-by-query-frequency}
*도구 세트: **data-observability***\
*필수 권한: `Logs Read Data`, `Logs Read Index Data` 및 `APM Read`*\
테이블을 쿼리 활동량에 따라 순위를 매기고, 쿼리 주체를 사람 사용자, BI 도구, 오케스트레이터, ETL 도구 또는 내부 서비스 계정별로 분류합니다.

- BI 도구에서 가장 많이 쿼리한 테이블이 어떤 거야?
- 실제 사람 분석가가 가장 많이 이용한 테이블은 뭐야?

### `suggest_data_observability_monitor_filters` {#suggest-data-observability-monitor-filters}
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read`*\
엔티티 세트를 분석하여 공통 속성과 명명 패턴을 찾고, 해당 엔티티의 하위 집합을 그룹화하는 모니터 필터 표현식을 제안합니다.

- 우선순위가 가장 높은 테이블들의 공통점이 뭐야?
- 모든 스테이징 테이블을 포함하는 필터를 제안해 줘.

### `rank_data_observability_monitor_candidates` {#rank-data-observability-monitor-candidates}
*도구 세트: **data-observability***\
*필수 권한: `APM Read`*\
계보 영향과 쿼리 활동을 하나의 종합 점수로 결합하여 모니터링 우선순위별로 테이블 순위를 매깁니다. 이것은 '무엇을 모니터링해야 돼?'와 같은 질문에 답할 때 가장 먼저 사용되는 도구입니다.

- 어떤 테이블에 데이터 품질 모니터를 먼저 설정해야 하는지 알려줘.
...
*도구 세트: **data-observability***\
*필수 권한: `Monitors Read`*\
조직의 모든 데이터 품질 모니터를 가져오고 각 모니터의 필터를 적용 대상 엔티티로 확인합니다. 모니터링이 전혀 없는 테이블을 확인하려면 이 기능을 사용하세요.

- 데이터 품질 모니터가 설정되지 않은 테이블을 알려줘.

### `get_data_observability_monitor_group_statuses` {#get-data-observability-monitor-group-statuses}
*도구 세트: **data-observability***\
*필수 권한: `APM Read`*\
데이터 품질 모니터 그룹의 현재 경고 및 주의 상태를 조회합니다.

- 현재 데이터 품질 검사를 통과하지 못한 테이블을 알려줘.
...
데이터 엔티티에 사용자 정의 태그를 가져오거나 설정합니다.

- 이 테이블에 어떤 태그가 있어?
- 이 테이블에 `owner:data-platform-team` 태그를 지정해 줘.
...
*도구 세트: **data-observability***\
*필수 권한: `APM Read`*\
데이터 작업 및 쿼리(Spark, Databricks, Snowflake, BigQuery)에 대한 비용 및 성능 최적화 권장 사항과 예상 비용 절감액 및 시간 절감 효과를 나열합니다. 커서 기반 페이지 매김을 사용하여 간단한 요약을 반환합니다.

- 내 Databricks 작업에 적용할 수 있는 비용 절감 권장 사항을 알려줘.
- 내 Spark 작업의 데이터 편향을 줄이기 위한 권장 사항이 있는지 알려줘.

### `get_data_observability_recommendation` {#get-data-observability-recommendation}
*도구 세트: **data-observability***\
*필수 권한: `APM Read`*\
문제, 증거 및 제안된 변경 사항을 설명하는 구조화된 본문을 포함하여 ID별로 특정 Data Observability 권장 사항의 전체 세부 정보를 검색합니다.
...
*도구 세트: **dbm***\
*필수 권한: `Database Monitoring Read`*\
상태 검사를 실행하여 CPU 포화도, 재시작, 쿼리 지연 시간, 차단과 같은 잠재적인 PostgreSQL 이슈를 드러냅니다. 회귀 시간 프레임을 기준 기간과 비교합니다.
...
- 지난 한 시간 동안의 쿼리 서명 `abc123def` 성능을 분석해 줘.
...
- 결제 데이터베이스에서 누락된 인덱스 권장 사항을 목록으로 나열해 줘.
...
- 결제 데이터베이스에서 쿼리 서명 `abc123def`를 최적화해 줘.
- 이 SQL에 누락된 인덱스 및 안티 패턴이 있는지 검사해 줘.
- `db-prod-1`에서 가장 느린 쿼리에 대한 재작성을 제안해 줘.
...
*도구 세트: **ddsql***\
DDSQL 데이터 세트를 검색하여 표(퍼블릭 데이터 소스, 참조 테이블) 및 사용 가능한 메트릭을 반환합니다.
...
- 이 쿼리에 대한 DDSQL Editor 링크를 생성해 줘.
- 내 인프라 쿼리를 사용해서 DDSQL Editor에 대한 공유 가능한 링크를 생성해 줘.
...
- 지난 한 시간 동안 서비스별 오류 수를 집계해 줘.
- 지난주 체크아웃 서비스의 주요 오류 유형을 보여 줘.
- 버전별 오류를 분석하여 어떤 배포에서 이슈가 발생했는지 파악해 줘.
...
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Write`*\
실행 중인 실험을 종료하고 승리한 변형을 최종 확정합니다.

- 실험 `abc123`을 종료하고, 처리 변형을 최종 승자로 확정해 줘.

### `cancel_experiment` {#cancel-experiment}
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Write`*\
필수 사유를 입력하여 실행 중인 실험을 취소합니다.

- SRM 이슈가 감지되었으므로 실험 `abc123`을 취소해 줘.

### `get_experiment_diagnostics` {#get-experiment-diagnostics}
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Read`*\
결과를 해석하기 전에 샘플 비율 불일치(SRM) 상태, 총 실험 대상 수, 변형별 노출 수 및 비율, 신뢰할 수 없는 메트릭 및 데이터가 없는 메트릭을 포함한 메트릭별 상태에 관한 요약을 반환합니다. `get_experiment_results` 전에 이 항목을 호출하세요. `srm.has_warning`이 true인 경우 변형 수준 비교를 신뢰성 있게 해석할 수 없습니다.

- 실험 `abc123`의 결과를 확인하기 전에 진단을 실행해 줘.
- 실험 `abc123`에 샘플 비율 불일치가 있는지 확인해 줘.
...
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Read`, `Product Analytics Metrics Read`*\
...
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Read`, `Product Analytics Metrics Read`*\
실험을 분할할 수 있는 할당 속성을 나열합니다. 유효한 속성 ID를 얻으려면 `explore_experiment_results` 전에 이 함수를 호출하세요. 임의로 추측하지 마세요.

- 실험 `abc123`의 결과를 세분화하는 데 사용할 수 있는 속성을 보여 줘.

### `get_experiment_segmentation_property_values` {#get-experiment-segmentation-property-values}
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Read`, `Product Analytics Metrics Read`*\
세분화 속성에 대한 구체적인 값을 반환합니다(예: 기기 유형의 경우 `["mobile", "desktop", "tablet"]`). 잘못된 필터 문자열을 방지하려면 `explore_experiment_results`에서 필터링하기 전에 이 기능을 사용하세요.

- 실험 `abc123`에서 기기 유형 속성에 사용할 수 있는 값을 알려줘.

### `get_metric_definition` {#get-metric-definition}
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Metrics Read`*\
실험 메트릭의 정의를 반환합니다. 여기에는 기본 이벤트 쿼리, 데이터 소스, 메트릭 변동 원인을 조사하기 위해 권장되는 Datadog MCP 도구가 포함됩니다. `datadog` 소스 메트릭의 경우, 응답에는 원시 이벤트 데이터를 쿼리하는 데 필요한 구조화된 파라미터가 포함된 `recommended_tool_call` 필드가 포함됩니다. Datadog 인프라 또는 APM 메트릭용이 아닙니다. 이 메트릭에는 `get_datadog_metric`을 사용하세요.

- 실험 `abc123`의 기본 메트릭에 사용된 이벤트 쿼리를 알려줘.
- 이 메트릭이 변동된 이유를 조사하려면 어떤 MCP 도구를 사용해야 하는지 알려줘.

### `diagnose_experiment_run_failure` {#diagnose-experiment-run-failure}
*도구 세트: **experiments***\
*필수 권한: `Product Analytics Experiments Read`*\
실험에 대한 최신(또는 특정) 분석 파이프라인 실행이 실패한 이유를 진단합니다. 근본 원인 작업, 분류된 실패 설명 및 실행 가능한 다음 단계를 반환합니다. 결과 품질 및 SRM 이슈에는 대신 `get_experiment_diagnostics`를 사용하세요.

- 실험 `abc123`의 최신 분석 실행이 실패한 이유가 뭐야?
- 실험 `abc123`의 파이프라인 실패를 진단해 줘.

## Feature Flags {#feature-flags}

플래그 및 플래그 환경 생성, 나열, 업데이트 등 [Feature Flags][51] 관리를 위한 도구입니다.
...
[Cloud Network Monitoring][31] 데이터를 사용하여 네트워크 수준 이슈를 조사하고, 네트워크 흐름 데이터를 분석하여 재전송 비율 상승과 같은 이상을 감지합니다.

- 내 웹 서버와 데이터베이스 클러스터 간의 네트워크 트래픽을 분석해 줘.
- `service:api`와 `service:payments` 사이에 재전송 이슈가 있어?
- 프로덕션 환경에서 네트워크 흐름 데이터에 이상이 있는지 조사해 줘.
...
*도구 세트: **onboarding***\
*필수 권한: `RUM Apps Read`*\
Browser RUM을 Datadog에 온보딩하는 과정을 안내합니다.
...
- `service:checkout-api`에 대한 프로파일링 태그 이름을 나열해 줘.
...
[참조 테이블][45]을 관리하기 위한 도구로, 테이블 목록 나열, 행 읽기, 행 업서트, 클라우드 스토리지 파일에서 동기화된 테이블 생성 또는 직접 채울 수 있는 빈 테이블 생성 기능이 포함됩니다.

### `list_reference_tables` {#list-reference-tables}
*도구 세트: **reference-tables***\
조직의 [참조 테이블][45]를 목록으로 나열 및 검색하며, 이름 기준으로 필터링 및 정렬(선택 사항)이 있습니다.
...
*도구 세트: **reference-tables***\
기존 참조 테이블에 새 행을 추가합니다. 이 작업은 행만 추가하며 기존 데이터를 수정하거나 삭제하지 않습니다. 각 행에는 기본 키 필드를 비롯한 표 스키마의 필수 필드를 모두 포함해야 합니다. 행이 이미 존재할 수 있는 경우, 대신 `upsert_reference_table_rows`를 사용하세요.

- 사용자 표에 이름이 `Carol`이고 나이가 `28`인 사용자 `user003`에 대한 새 행을 추가해 줘.
- 이 다섯 개의 새 계정 항목을 계정 참조 테이블에 추가해 줘.
...
- 'checkout-web'이라는 이름의 RUM 애플리케이션을 찾아 애플리케이션 ID를 반환해 줘.
- 내 모든 RUM 애플리케이션을 나열해 줘.
...
- 지난 24시간 동안 'checkout-web' RUM 애플리케이션의 성능을 요약해 줘.
...
*도구 세트: **rum***\
*필수 권한: `RUM Apps Read` 및 `RUM Generate Metrics`*\
사용자 지정 RUM 메트릭을 생성하거나 업데이트합니다. 기존 메트릭을 업데이트하기 전에 변경할 수 없는 필드를 확인합니다. 이 작업은 멱등성을 가집니다.

- 국가별로 그룹화된 뷰 이벤트의 p95 LCP를 추적하는 분포 메트릭 `rum.view.lcp_by_country`를 생성해 줘.
- Synthetic 테스트 트래픽을 제외하도록 `rum.error.checkout_errors`의 필터를 업데이트해 줘.

### `delete_rum_metric` {#delete-rum-metric}
*도구 세트: **rum***\
*필수 권한: `RUM Apps Read` 및 `RUM Generate Metrics`*\
ID를 사용하여 사용자 지정 RUM 메트릭을 영구적으로 삭제합니다. 이 작업은 멱등성을 가집니다.

- 사용자 지정 RUM 메트릭 `rum.view.my_custom_metric`을 삭제해 줘.
- 내 조직에서 RUM 메트릭 `rum.view.legacy_page_views`를 제거해 줘.
...
- 'checkout-web' 애플리케이션에 구성된 보존 필터를 나열해 줘.
...
평가 순서의 끝에 추가되는 RUM 보존 필터를 생성합니다. 보존 필터는 어떤 RUM 이벤트가 인덱싱되고 보존될지 제어하며, 이는 청구에 영향을 미칩니다. 적용하기 전에 변경 사항을 확인하세요.

- 오류 이벤트의 100%를 보존하는 'checkout-web'에 보존 필터를 생성해 줘.
- `@view.url_path:/checkout`와 일치하는 모든 세션을 유지하는 보존 필터를 내 기본 RUM 앱에 추가해 줘.

### `update_rum_retention_filter` {#update-rum-retention-filter}
*도구 세트: **rum***\
*필수 권한: `RUM Retention Filters Write` 또는 `Product Analytics Apps Write`*\
이름, 이벤트 유형, 쿼리, 샘플링 비율 또는 활성화 상태와 같은 기존 RUM 보존 필터의 속성을 제자리에서 업데이트합니다. 적용하기 전에 변경 사항을 확인하세요.

- 'checkout errors' 보존 필터의 샘플링 비율을 100%로 높여줘.
- 'long tasks' 보존 필터를 내 기본 RUM 앱에서 비활성화해 줘.

### `reorder_rum_retention_filters` {#reorder-rum-retention-filters}
*도구 세트: **rum***\
*필수 권한: `RUM Retention Filters Write` 또는 `Product Analytics Apps Write`*\
RUM 애플리케이션의 보존 필터에 대한 전체 평가 순서를 설정합니다. 필터는 위에서 아래로 평가되며 각 이벤트는 첫 번째 일치 항목에서 중지되므로, 순서에 따라 적용되는 샘플링 비율이 결정됩니다. 적용하기 전에 새 순서를 확인하세요.

- 'checkout-web'에서 'checkout errors' 보존 필터를 catch-all 필터 위로 옮겨줘.
- 특정 보존 필터가 광범위한 보존 필터보다 먼저 평가되도록 보존 필터의 순서를 다시 지정해 줘.
...
코드 보안 스캔, [보안 신호][53] 분석·검색·분류, [IoC Explorer][67] 지표 조사, [탐지 규칙][60] 및 [억제 규칙][61] 관리, [보안 발견 사항][54] 분석을 위한 도구입니다.

### `datadog_secrets_scan` {#datadog-secrets-scan}
*도구 세트: **security***\
코드에 하드코딩된 시크릿 및 자격 증명이 있는지 스캔하여 AWS 키, API 키, 비밀번호, 토큰, 개인 키 및 데이터베이스 자격 증명을 감지합니다.

- 내 코드에 하드코딩된 시크릿이 있는지 스캔해 줘.
- 이 파일에 커밋된 API 키 또는 비밀번호가 있는지 검사해 줘.
...
- 지난 24시간 동안 '무차별 대입 로그인' 규칙에서 생성된 모든 신호를 보관 처리해 줘.
- `service:checkout`에 대한 모든 열린 신호를 검토 중 상태로 변경하고 나에게 할당해 줘.
- 신호 `AwAAAZ27F1BUjY4rPQAAABhBWjI3RjFCVWpZNHJBQUFBSGFNQVZBQUFBR1Bu`을 '테스트' 사유로 보관 처리해 줘.
...
*도구 세트: **security***\
*필수 권한: `Security Signals Write`*\
[IoC Explorer][67] 지표의 분류 상태를 설정합니다.

- 지표 `192.0.2.1`을 검토됨으로 표시해 줘.
- `evil-domain.example.com`을 검토되지 않음으로 다시 설정해 줘.
...
- 오류율이 분당 100회를 초과할 때 신호를 생성하는 `source:nginx` 탐지 규칙을 생성해 줘.

### `update_datadog_security_detection_rule` {#update-datadog-security-detection-rule}
*도구 세트: **security***\
*필수 권한: `Security Monitoring Rules Write`*\
기존 사용자 지정 탐지 규칙을 완전히 교체하여 업데이트합니다. `get_datadog_security_detection_rules`를 먼저 호출하여 현재 규칙 본문을 가져오고, 필요한 필드를 수정한 후 전체 업데이트된 객체를 제출합니다. Datadog에서 제공하는 기본 규칙은 업데이트할 수 없습니다.

- 탐지 규칙 `abc-123-def`를 활성화해 줘.
- 무차별 대입 탐지 규칙을 비활성화해 줘.
- 내 무차별 대입 탐지 규칙의 임계값을 로그인 실패 10회에서 20회로 변경해 줘.
- 탐지 규칙 `abc-123-def`에 심각도가 'critical'일 때 실행되는 새 케이스를 추가해 줘.
- 이 규칙의 그룹화 필드를 `@usr.ip`에서 `@network.client.ip`로 변경해 줘.

### `delete_datadog_security_detection_rules` {#delete-datadog-security-detection-rules}
*도구 세트: **security***\
*필수 권한: `Security Monitoring Rules Write`*\
ID를 사용하여 하나 이상의 사용자 지정 탐지 규칙을 삭제하세요. 단, 사용자 지정(기본값이 아닌) 규칙만 삭제할 수 있습니다. 기본 규칙은 403을 반환합니다. 각 규칙은 개별적으로 승인되며, 실패 시 일괄 작업을 중단하지 않고 `failed_rules`에 오류가 표시됩니다.

- 탐지 규칙 `abc-123-def`를 삭제해 줘.
- 이전에 생성한 테스트 탐지 규칙 세 개를 제거해 줘.
...
- 무차별 대입 규칙용 억제 규칙에 `10.0.0.2`도 제외 대상으로 추가해 줘.
- 억제 규칙 `sup-456-xyz`의 만료일을 다음 분기로 변경해 줘.
...
- 억제 규칙 `sup-456-xyz`를 삭제해 줘.
- 무차별 대입 탐지 규칙을 무음 처리하고 있던 억제 규칙을 제거해 줘.
...
- 특정 규칙에 대한 완전한 탐지 객체를 조회해 줘.
- 열린 ID 위험 발견 사항을 전체 메타데이터와 함께 나열해 줘.
...
보안 발견 사항에 대해 Case Management 케이스, Jira 이슈, Linear 이슈 또는 ServiceNow 티켓을 생성합니다. 특정 발견 사항 ID와 프로젝트 ID가 필요합니다. 사용 가능한 프로젝트를 확인하려면 먼저 `get_datadog_security_findings_ticket_suggestions`를 사용하세요.

- 프로젝트 SECURITY에 대해 이러한 중요한 발견 사항용 Jira 티켓을 생성해 줘.
- 이 규칙의 발견 사항에 대해 Case Management 케이스를 생성해 줘.
- 이 심각도가 높은 발견 사항에 대해 Linear 이슈를 생성해 줘.
- 이 라이브러리 취약점에 대해 ServiceNow 티켓을 생성해 줘.
...
- 이 발견 사항들을 연결된 Jira 티켓에서 분리해 줘.
- 이 발견 사항들의 케이스 연결을 제거해 줘.
...
- 이 발견 사항들을 오탐으로 음소거해 줘.
- 이 구성 오류를 허용된 위험으로 음소거하고 90일 후 만료되도록 설정해 줘.
- 이전에 수정 보류 중으로 표시된 발견 사항의 음소거를 해제해 줘.
...
- 이 중요한 발견 사항들을 보안 팀 리더에게 할당해 줘.
- 더 이상 관련이 없는 발견 사항의 할당을 해제해 줘.
- 이 규칙의 모든 발견 사항을 나에게 할당해 줘.

### `list_datadog_security_findings_automation_rules` {#list-datadog-security-findings-automation-rules}
*도구 세트: **security***\
*필수 권한: `Security Pipelines Read`*\
...
- 티켓 생성 규칙을 업데이트하여 다른 Jira 프로젝트를 대상으로 지정해 줘.

### `delete_datadog_security_findings_automation_rule` {#delete-datadog-security-findings-automation-rule}
*도구 세트: **security***\
*필수 권한: `Security Pipelines Write`*\
ID를 사용하여 보안 발견 사항 자동화 규칙을 영구적으로 삭제합니다.

- 심각도 수정 규칙 `abc-123-def`를 삭제해 줘.
- 더 이상 필요하지 않은 음소거 규칙을 제거해 줘.
...
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
특정 서비스나 엔드포인트에서 노이즈가 많은 규칙을 억제하기 위해 WAF 제외 필터(허용 목록) 항목을 생성하거나 업데이트합니다.

- 서비스 'checkout-service'의 엔드포인트 '/api/pay'에 대해 규칙 'sqli-detection'을 무시하도록 WAF 허용 목록 항목을 추가해 줘.
- 서비스 'auth-api'에 대해 규칙 'xss-rule'을 억제하도록 제외 필터를 업데이트해 줘.
- '/v1/users'에서 규칙 ID 'lfi-attack'과 일치하는 AppSec 허용 목록 항목을 생성해 줘.

### `delete_datadog_security_trace_passlist` {#delete-datadog-security-trace-passlist}
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
기존 WAF 제외 필터(허용 목록) 항목을 삭제합니다.

- WAF 제외 필터 'passlist-abc-123'을 삭제해 줘.
- '/api/pay'에서 규칙 'sqli-detection'과 일치하는 허용 목록 항목을 제거해 줘.
...
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
만료 기한이 있는 IP, 사용자 또는 사용자 에이전트에 대한 차단 목록 항목을 추가하거나 업데이트합니다.

- IP '198.51.100.42'를 24시간 동안 차단 목록에 추가해 줘.
- 사용자 'attacker_user_99'를 차단된 엔티티 목록에 추가해 줘.
- 사용자 에이전트 'MaliciousScanner/1.0'에 대한 차단 목록 항목을 생성하고 만료 시간을 다음 주로 설정해 줘.

### `unblock_datadog_security_aap_denylist` {#unblock-datadog-security-aap-denylist}
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
차단 목록에 있는 엔티티의 만료 기한을 과거로 설정하여 차단을 해제합니다.

- 거부 목록에서 IP '198.51.100.42'의 차단을 해제해 줘.
- 거부된 엔티티 목록에서 사용자 'attacker_user_99'를 제거해 줘.
...
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
공격 시도 또는 비즈니스 로직 카테고리에서 AAP 사용자 지정 WAF 규칙을 생성하거나 업데이트합니다. 새 규칙은 트래픽을 차단할 수 없습니다. 모니터링 모드에서 규칙을 생성한 다음 일치 항목을 확인한 후 차단 모드로 업데이트하세요.

- '/admin' 경로에 대한 요청을 모니터링하는 사용자 지정 웹 애플리케이션 방화벽(WAF) 규칙을 생성해 줘.
- AAP 사용자 지정 규칙 'rule-xyz-123'을 업데이트하여 일치하는 트래픽을 차단해 줘.
- 사용자 지정 규칙 'rule-xyz-123'을 삭제하지 않고 비활성화해 줘.

### `delete_datadog_security_aap_custom_rule` {#delete-datadog-security-aap-custom-rule}
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Write`*\
ID를 사용하여 AAP 사용자 지정 WAF 규칙을 영구적으로 삭제합니다.

- 사용자 지정 웹 애플리케이션 방화벽(WAF) 규칙 'rule-xyz-123'을 삭제해 줘.
- '/admin'에 대한 요청을 모니터링하는 AAP 사용자 지정 규칙을 제거해 줘.

### `get_datadog_security_aap_blocking_config` {#get-datadog-security-aap-blocking-config}
*도구 세트: **security***\
*필수 권한: `Application Security Management Protect Read`*\
조직 전체의 AAP 차단 및 거부 목록 적용 설정을 검색합니다.
...
[Session Replay][69] 기록을 검색하고 세션 활동을 요약하기 위한 도구입니다.
...
*도구 세트: **Session Replay***\
*필수 권한: `RUM Apps Read` 및 `RUM Session Replay Read`*\
...
- `checkout-service` 리포지토리에서 활성인 모든 불안정한 테스트를 격리해 줘.
- 불안정한 테스트 `AuthServiceTest::testLogin`을 수정됨으로 표시해 줘.
- 실패율이 50%를 초과하는 `@team-payments` 소유의 불안정한 테스트를 비활성화해 줘.
...
[64]: /ko/cloud_cost_management/
[65]: /ko/code_coverage/
[66]: /ko/delivery_performance/dora_metrics/
[67]: /ko/security/cloud_siem/triage_and_investigate/ioc_explorer/
[68]: /ko/product_analytics/
