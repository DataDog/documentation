...
algolia:
  tags:
  - 통합 관리
description: 설치, 제거, 표시 및 잠금 작업을 포함, datadog-agent 통합 명령을 사용하여 공식 Datadog 통합을 관리하는
  방법에 관한 가이드입니다.
title: 통합 관리
---
...
**참고**: Agent 버전 7.69부터 업그레이드할 때 커뮤니티, 파트너 및 마켓플레이스 통합이 자동으로 유지됩니다. 이 기능을 사용하지 않으려면 업그레이드하기 전에 `/etc/datadog-agent/` 디렉터리에 `.skip_install_python_third_party_deps`의 이름으로 빈 파일을 만드세요. 이 파일이 있으면 통합 기능이 유지되지 않으므로 업그레이드 후 수동으로 다시 설치해야 합니다.
