...
새로운 탐지 결과를 발견하면 Jira, Linear 또는 Work Management에서 티켓을 자동으로 생성하도록 티켓 생성 규칙을 구성합니다. 이 접근 방식은 수동 분류 없이 기존 엔지니어링 워크플로에서 보안 문제를 추적하여 팀이 새로운 위협에 대규모로 신속하게 대응할 수 있도록 돕습니다. 보안 탐지 결과와의 티켓팅 통합에 대한 자세한 내용은 [티켓팅 통합][3]을 참조하세요.
{{< /site-region >}}

{{< site-region region="gov,gov2" >}}
새로운 탐지 결과를 발견하면 Jira 또는 Work Management에서 티켓을 자동으로 생성하도록 티켓 생성 규칙을 구성합니다. 이 접근 방식은 수동 분류 없이 기존 엔지니어링 워크플로에서 보안 문제를 추적하여 팀이 새로운 위협에 대규모로 신속하게 대응할 수 있도록 돕습니다. 보안 탐지 결과와의 티켓팅 통합에 대한 자세한 내용은 [티켓팅 통합][3]을 참조하세요.
{{< /site-region >}}

## 티켓 생성 규칙 생성하기 {#create-a-ticket-creation-rule}

1. Datadog에서 **Security** > **Settings** > [Findings Automation][2]으로 이동합니다. **Add a New Rule**을 클릭한 다음 **Create Ticket**을 선택합니다. Create a New Rule 페이지가 열립니다.
...
1. 티켓팅 시스템을 선택하고 티켓 대상을 구성합니다.
  ...
  {{< tabs >}}
  {{% tab "Jira" %}}
  - **Jira Account**: 사용할 Atlassian 인스턴스를 선택합니다. 이 계정에 [Jira 웹훅][5]이 구성되어 있는지 확인합니다.
  - **Space**: Jira 프로젝트를 선택합니다.
  ...
  - **Data Sync Settings**를 확장하여 연결된 Work Management 프로젝트 및 양방향 동기화 구성을 검토하거나 업데이트하니다.

  [5]: /integrations/jira/#configure-a-jira-webhook
  {{% /tab %}}
  {{% tab "Linear" %}}
  - **Linear Account**: 사용할 Linear 계정을 선택합니다. 이 계정에 [Linear 웹훅][6]이 구성되어 있는지 확인합니다.
  - **Team**: 이슈를 생성할 Linear 팀을 선택합니다.
  - **Project**(선택 사항): 자동으로 생성된 이슈와 연결할 Linear 프로젝트를 선택합니다.
  - **Labels**(선택 사항): 자동으로 생성된 이슈에 적용할 레이블을 선택합니다.
  - **Assignee**(선택 사항): 자동으로 생성된 이슈를 할당할 사용자를 지정합니다.
  - **Data Sync Settings**를 확장하여 연결된 Work Management 프로젝트 및 양방향 동기화 구성을 검토하거나 업데이트하니다.

  [6]: /integrations/linear/#configure-a-linear-webhook
  {{% /tab %}}
  {{% tab "Work Management" %}}
  - **Work Management Project**: 기존 Work Management 프로젝트를 선택하거나 새로 만듭니다.
...
{{< img src="security/automation_pipelines/ticket_creation_lightning_indicator.png" alt="번개 아이콘으로 표시되며 동일한 규칙에서 생성된 티켓이 있는 모든 결과를 볼 수 있는 링크가 포함되어 있는, 자동화 규칙에 의해 생성된 케이스를 보여주는 Work Management 티켓 팝업" style="width:60%;" >}}
