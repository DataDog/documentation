...
Agentless 스캔은 AWS, Azure, GCP에서 지원됩니다.

다음 표는 지원되는 각 클라우드 제공업체의 구성 요소에 대해 Agentless 스캔 기술이 적용되는 방식을 요약한 것입니다.
...
| 암호화                                      | AWS </br> 암호화되지 않음 </br> 암호화됨 - 플랫폼 관리형 키(PMK) 및 고객 관리형 키(CMK)                             | 암호화됨 - 플랫폼 관리형 키(PMK): Azure Disk Storage 서버 측 암호화, 호스트 내 암호화 </br> **참고**: 암호화됨 - 고객 관리형 키(CMK)는 **지원되지** 않음 | 암호화됨 - 플랫폼 관리형 키(PMK): Persistent Disk 암호화, Confidential VM </br> **참고**: 암호화됨 - 고객 관리형 암호화 키(CMEK) 및 고객 제공 암호화 키(CSEK)는 **지원되지** 않음 |
| 컨테이너 런타임                               | Docker, containerd </br> **참고**: CRI-O는 **지원되지** 않음                                                                 | Docker, containerd </br> **참고**: CRI-O는 **지원되지** 않음                                                                                                                     | Docker, containerd </br> **참고**: CRI-O는 **지원되지** 않음                                                                                                                                                            |
| Serverless                                      | AWS Lambda <br> AWS Fargate for ECS                                                                                           | Azure Functions ([Preview][20]), Azure Container Apps, Azure Container Instances<br />**참고**: 최신 Agentless 스캐너가 필요합니다. [Agentless Scanning 업데이트][17]를 참조하세요.                                     | Cloud Run ([Preview][21])                                                                                                                                           |
| Kubernetes                                      | EKS on EC2 nodes </br> EKS on Fargate </br> **참고**: EKS on Fargate의 경우 [Datadog Cluster Agent][18]를 설치해야 합니다. | AKS on VMs, Virtual Machine Scale Sets (VMSS), Azure Container Instances (ACI) </br> **참고**: AKS on ACI의 경우 [Datadog Cluster Agent][18]를 설치해야 합니다. | GKE Standard 및 GKE Autopilot </br> **참고**: GKE Autopilot의 경우 [Datadog Cluster Agent][18]를 설치해야 합니다.                                                              |
...
| 컨테이너 레지스트리                            | Amazon ECR(공개 및 비공개): 실행 중인 컨테이너 이미지와 마지막으로 푸시된 1,000개의 저장된 이미지를 스캔합니다.                      | ACR: 실행 중인 컨테이너 이미지만 스캔합니다.<br />**참고:** 저장된 레지스트리 스캔은 지원되지 않습니다. 요청하려면 [Datadog 지원팀][16]에 문의하세요.   | Google Artifact Registry: 실행 중인 워크로드의 이미지만 스캔합니다.<br />**참고**: Google Artifact Registry 저장된 스캔 지원은 [Preview][19] 단계입니다.                                                                                                                                        |
...
## Linux 배포판 {#linux-distributions}

다음 Linux 배포판에서 호스트 및 컨테이너 스캔이 지원됩니다.

| 운영 체제         | 지원 버전                                  | 패키지 관리자 | 보안 권고                                                                                       |
|--------------------------|-----------------------------------------------------|------------------|-----------------------------------------------------------------------------------------------------------|
| Alpine Linux             | 2.2-2.7, 3.0-3.19(edge는 지원되지 않음)           | apk              | [https://secdb.alpinelinux.org/][1]                                                                       |
| Wolfi Linux              | 해당 없음                                                 | apk              | [https://packages.wolfi.dev/os/security.json][2]                                                          |
| Chainguard               | 해당 없음                                                 | apk              | [https://packages.cgr.dev/chainguard/security.json][3]                                                    |
...
| Ubuntu                   | Canonical에서 지원하는 모든 버전                 | apt/dpkg         | [https://ubuntu.com/security/cve][15]                                                                     |

## 애플리케이션 라이브러리 {#application-libraries}

컨테이너 이미지, Lambda 함수, 호스트에서 실행 중인 컨테이너의 취약성 스캔에는 다음과 같은 애플리케이션 언어와 라이브러리가 지원됩니다.
...
## 컨테이너 이미지 레지스트리 {#container-image-registries}

다음은 이미지 스캔을 지원하는 컨테이너 이미지 레지스트리입니다.

| 레지스트리                        | 지원 수준 | 참고                                                                                                                                                                                                                                         |
|---------------------------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Amazon ECR(공개 및 비공개) | GA            | 실행 중인 컨테이너 이미지**와** 마지막으로 푸시된 1,000개의 저장된 이미지(날짜 기준)를 스캔합니다. 저장된 파일 스캔을 지원하는 유일한 레지스트리입니다.<br />**참고:** 스캔할 저장된 이미지 수를 늘리려면 [Datadog 지원팀][16]에 문의하세요. |
| Google Artifact Registry (GAR)  | Preview       | 실행 중인 워크로드(Cloud Run, GKE)에 연결된 이미지만 스캔합니다.<br />**참고**: Google Artifact Registry 저장된 스캔 지원은 [Preview][19] 단계입니다. |
| Azure Container Registry(ACR)  | GA            | Azure Container Apps 및 Azure Container Instances에서 실행 중인 컨테이너 이미지만 스캔합니다.<br />**참고**: 저장된 레지스트리 스캔은 지원되지 않습니다. 요청하려면 [Datadog 지원팀][16]에 문의하세요.                                                                |
...
**컨테이너 관찰 참고 사항**: Agentless 스캔을 실행하려면 압축되지 않은 컨테이너 이미지 레이어가 필요합니다. 해결 방법으로 containerd 구성 파일에서 구성 옵션 `discard_unpacked_layers=false`을 설정할 수 있습니다.
