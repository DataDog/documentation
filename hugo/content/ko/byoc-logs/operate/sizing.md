...
적절한 클러스터 크기 조정은Bring Your Own Cloud(BYOC) 로그 배포의 최적 성능, 비용 효율성 및 안정성을 보장하는 데 도움이 됩니다. 크기 조정 요구 사항은 로그 수집 볼륨, 쿼리 패턴, 보존 기간 및 로그 데이터의 복잡성을 포함한 여러 요인에 따라 달라집니다.
...
다음 표는 일반적인 일일 로그 볼륨을 기준으로 한 기준 구성을 제시합니다. 이 권장 사항은 시작점으로 활용하기 위한 것이며, 실제 리소스 사용률 및 쿼리 성능에 따라 조정되어야 합니다.

시작점으로 대략 다음과 같이 계획하세요.

- 일일 수집 로그 1TB당 인덱서 vCPU 2개
- 일일 수집 로그 2TB당 컴팩터 vCPU 1개

검색기 용량은 쿼리 동시성, 쿼리 복잡성 및 스캔되는 데이터 양에 따라 달라집니다. 따라서 수집 볼륨만 고려하기보다는 예상되는 검색 워크로드에 맞춰 크기를 조정해야 합니다. 분석 중심 워크로드의 경우 아래 표시된 기준 검색 용량의 최대 2배가 필요할 수 있습니다.
...
다음 표는 각 구성 요소에 대한 총 vCPU 용량을 보여줍니다.

|   일일 볼륨 | 인덱서 총 vCPU | 컴팩터 총 vCPU | 검색기 총 vCPU |
|---------------:|--------------------:|----------------------:|---------------------:|
|   **1TB/일** |                   2 |                   0.5 |                    4 |
|  **10 TB/일** |                  20 |                     5 |                   40 |
| **100 TB/일** |                 200 |                    50 |                  400 |

총 용량을 포드 전체에 분산하기 위한 시작점으로 다음 포드당 CPU 및 메모리 할당량을 사용하세요.

| 일일 볼륨    | 포드당 인덱서 | 포드당 컴팩터 | 포드당 검색기 |
|-----------------|----------------:|------------------:|-----------------:|
| **최대 30TB/일** |  4 vCPU, 16GB |    4 vCPU, 16GB |  16 vCPU, 64GB |
| **30TB/일 초과** |  8 vCPU, 32GB |    8 vCPU, 32GB | 64 vCPU, 256GB |
...
| 사양        | 권장 사항                 | 참고                                                                                                                                                                                                                                                                                                                                                                  |
|----------------------|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **성능**      | vCPU당 8MB/s                | 초기 크기 조정을 결정하기 위한 기준 처리량입니다. 실제 성능은 로그 특성(크기, 속성 수, 중첩 수준)에 따라 달라집니다.                                                                                                                                                                                                                         |
| **메모리**           | vCPU당 4GB RAM              |                                                                                                                                                                                                                                                                                                                                                                        |
| **최소 포드 크기** | 2 vCPU, 8GB RAM              | 인덱서 포드에 권장되는 최소 사양입니다.                                                                                                                                                                                                                                                                                                                                   |
| **스토리지 용량** | 최소 30GB                 | 인덱스 파일을 생성하고 병합하는 동안 임시 데이터를 저장하는 데 필요합니다.                                                                                                                                                                                                                                                                                                     |
| **스토리지 유형**     | 네트워크 연결형 블록 스토리지 | 예: Amazon EBS gp3, Azure Managed Disks 또는 GCP Persistent Disk. 데이터는 객체 스토리지에 업로드되기 전에 WAL(Write-Ahead Log)에 임시로 저장됩니다. WAL은 복제되지 않으므로 로컬(임시) SSD를 사용하면 디스크 장애 시 몇 분 분량의 데이터가 손실될 위험이 커집니다. 네트워크 연결형 블록 스토리지는 내장 중복성을 제공합니다. |
| **디스크 I/O**         | vCPU당 약 20MB/s | Amazon EBS의 경우 vCPU당 320 IOPS와 동일합니다(64KB IOPS 가정). 예를 들어, Amazon EBS gp3의 기본 처리량인 125MiB/s는 4-vCPU 인덱서에 충분합니다.                                                                                                                                                                                              |


{{% collapse-content title="예: 일일 100TB 로그에 대한 크기 조정" level="h3" expanded=false %}}
일일 100TB의 로그(약 1,160MB/s)를 인덱싱하려면 다음 단계를 따르세요.

1. **vCPU 계산:** `1,160 MB/s ÷ 8 MB/s per vCPU ≈ 145 vCPUs`
2. **RAM 계산:** `145 vCPUs × 4 GB RAM per vCPU ≈ 580 GB RAM`
3. **여유 공간 추가:** 각각 **vCPU 4개, 16GB RAM, 30GB 디스크**로 구성된 인덱서 포드 50개로 시작합니다. 관찰된 성능 및 중복성 요구 사항에 따라 이러한 값을 조정하세요.
{{% /collapse-content %}}

{{% collapse-content title="이벤트 수 기준 크기 조정" level="h3" expanded=false %}}
...
컴팩터는 작은 인덱스 스플릿을 더 큰 스플릿으로 병합하여 조각화를 줄이고 검색 효율성을 향상시킵니다. 또한 오래된 스플릿을 제거하여 스토리지 공간을 확보합니다.

| 사양    | 권장 사항      | 참고                                                        |
|------------------|---------------------|--------------------------------------------------------------|
| **성능**  | 일일 2TB당 vCPU 1개 | 초기 크기 조정을 위한 기준                                  |
| **메모리**       | vCPU당 4GB RAM   |                                                              |
| **스토리지 유형** | 로컬 SSD           | AWS M8gd와 같이 로컬 SSD가 있는 인스턴스를 권장합니다. |
...
BYOC Logs는 로그 데이터를 압축하고 인덱싱한 후 객체 스토리지에 저장합니다. 압축률은 데이터의 로그 형식, 구조 및 중복성에 따라 다릅니다.
...
다음 예시는 30일 보존 기간과 6배 압축률을 가정합니다.
...
{{% collapse-content title="예: 30일 보존 기간으로 일일 10TB 저장 시 스토리지" level="h3" expanded=false %}}
압축률을 6배로 가정할 경우:
...
BYOC Logs Helm 차트는 `indexer.podSize` 및 `searcher.podSize` 파라미터를 통해 사전 정의된 리소스 계층을 제공합니다. `podSize`는 포드의 리소스 요구 사항과 관련된 Quickwit 튜닝 파라미터를 선택합니다. 기본 `podSize`는 두 구성 요소 모두에 대해 `xlarge`입니다. 각 사전 설정은 일치하는 노드에 Kubernetes 시스템 구성 요소, DaemonSet 및 애드온을 위한 공간을 남겨두도록 설계되었습니다.
