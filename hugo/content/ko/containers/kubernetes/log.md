...
<br/><br/>
containerd 런타임을 사용하는 경우 <code>/var/log/pods</code> 의 로그 파일은 <code>root</code> 그룹 구성원이 읽을 수 있습니다. 위 지침을 사용하면 Agent가 <code>root</code> 그룹 구성원이 읽을 수 있습니다. 추가 액션은 필요하지 않습니다.
<br/><br/>
...
`DD_CONTAINER_EXCLUDE_LOGS` 또는 `ad.datadoghq.com/logs_exclude`와 같은 방법으로 로그를 제외하는 경우, Agent는 [Autodiscovery annotations][19], [`DatadogInstrumentation` CRD][23] 또는 [Autodiscovery 구성 파일][20]에 명시적으로 정의된 로그 수집 구성이 있더라도 해당 로그를 수집하지 않습니다.

'Container Collect All'이 비활성화된 경우(기본값) 모든 항목이 기본적으로 제외되므로 별도의 필터링을 추가할 필요가 없습니다. 선택한 포드에 대해서만 로그 수집을 활성화하려면 원하는 포드에 대해 [Autodiscovery 주석][19], [`DatadogInstrumentation` CRD][23] 또는 [Autodiscovery 구성 파일][20]을 사용하여 로그 구성을 설정할 수 있습니다.

필터링에 대한 자세한 내용은 [컨테이너 탐지 관리][8]를 참조하세요.
...
`service` 태그는 [Unified Service Tagging][4] 포드 레이블인 `tags.datadoghq.com/service: "<SERVICE>"`를 통해서도 설정할 수 있습니다. `source` 및 `service` 속성에 대한 자세한 내용은 [예약된 속성][11]을 참조하세요.
...
[Autodiscovery][10]를 사용하면 템플릿을 이용해 컨테이너의 로그 수집 및 기타 기능을 구성할 수 있습니다. 다음 방법 중 하나를 사용하여 로그 수집을 구성하세요.

- [Autodiscovery 주석](#autodiscovery-annotations)(권장)
- [`DatadogInstrumentation` CRD](#datadoginstrumentation-crd)(신규)
- [Autodiscovery 구성 파일](#autodiscovery-configuration-files)

이러한 로그 구성에 `source` 및 `service` 태그를 설정하는 것을 강력히 권장합니다. `source` 태그를 Datadog의 [기본 제공 로그 파이프라인][15] 중 하나와 일치시켜 로그가 자동으로 보강되도록 하세요. [Datadog의 파이프라인 라이브러리][16]에서도 확인할 수 있습니다. `service` 태그는 [Unified Service Tagging][4]을 지원하여 로그를 동일한 서비스의 메트릭 및 트레이스와 연결합니다. `source` 및 `service`가 생략된 경우, Agent는 Unified Service Tagging의 `service` 태그(설정된 경우)를 사용하며, 그렇지 않은 경우 컨테이너의 단축 이미지 이름을 사용합니다.
...
Kubernetes 포드를 <i>직접</i> 정의하는 경우( <code>kind:Pod</code>) 각 포드의 주석을 해당 <code>metadata</code> 섹션에 추가합니다. 다음 섹션을 참조하세요.
<br/><br/>
...
포드나 배포에 주석을 다는 대신 [`DatadogInstrumentation` 사용자 지정 리소스][23]를 사용하여 로그 수집을 구성할 수 있습니다. 다음 예시는 `example` 배포의 `app` 컨테이너 부분에 대한 것입니다.
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
`override.nodeAgent.extraConfd.configDataMap` 설정을 사용하여 통합별 로그 수집 구성을 재정의할 수 있습니다. 이 방법은 ConfigMap을 생성하고 원하는 구성 파일을 Agent 컨테이너에 마운트합니다.
...
`<CONTAINER_IMAGE>`는 해당 구성을 적용할 대상 컨테이너의 단축 이미지 이름과 일치해야 합니다. 추가 예제는 [ConfigMap 매핑이 포함][1]된 샘플 매니페스트를 참조하세요.
...
`<CONTAINER_IMAGE>`는 해당 구성을 적용할 대상 컨테이너의 단축 이미지 이름과 일치해야 합니다.
...
세 값 모두 목록이라는 점에 유의하세요. Autodiscovery는 공유된 목록 인덱스를 기준으로 목록 항목을 조합하여 통합 구성을 생성합니다. 이 예에서는 `check_names[0]`, `init_configs[0]` 및 `instances[0]`를 기반으로 첫 번째(이자 유일한) 검사 구성을 생성합니다.
...
**참고**: 키-값 저장소를 사용할 때 특정 컨테이너에 특정 구성을 적용하기 위해 Autodiscovery는 컨테이너를 **이미지** 기준으로 식별합니다. 즉, `<CONTAINER_IMAGE>`를 `.spec.containers[0].image`와 일치시키는 방식으로 동작합니다.
...
컨테이너 단축 이미지 이름보다 더 세밀하게 컨테이너 집합에 로그 구성을 적용하려면 [Autodiscovery Container 식별자][22]를 참조하세요.
...
이러한 파일 경로는 Agent 컨테이너를 기준으로 한 **상대** 경로입니다. 따라서 Agent가 로그 파일을 볼 수 있도록 로그 파일이 저장된 디렉토리를 애플리케이션 컨테이너와 Agent 컨테이너 모두에 마운트해야 합니다.

예를 들어, 공유 `hostPath` 볼륨을 사용하여 이를 구성할 수 있습니다. 아래 포드 예제는 `/var/log/example/app.log` 파일에 로그를 기록합니다. 이는 `/var/log/example` 디렉토리에서 수행되며, 볼륨과 volumeMount를 사용하여 해당 디렉토리를 `hostPath`로 구성합니다.
...
동일한 로그 파일을 읽을 수 있도록 Agent 컨테이너에 동일한 볼륨과 volumeMount 경로를 설정해야 합니다.
...
- 이와 같은 주석을 컨테이너에 사용하는 경우 `stdout` 및 `stderr` 로그는 자동으로 수집되지 않습니다. 컨테이너 출력 스트림과 파일 로그를 모두 수집해야 하는 경우 주석에서 이를 명시적으로 활성화해야 합니다. 예를 들면 다음과 같습니다.
  ```yaml
...
[23]: /ko/containers/guide/configure-autodiscovery-with-the-datadoginstrumentation-crd/