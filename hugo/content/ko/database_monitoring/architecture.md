...
Datadog에서 Database Monitoring을 설정하는 방법은 사용 중인 데이터베이스 유형(Postgres, MySQL, SQL Server, Oracle)과 호스트 공급자(자체 호스팅, AWS, Google Cloud SQL, Azure, Oracle)에 따라 다릅니다. 호스트 공급자에 관계없이 모든 데이터베이스에 대해 Database Monitoring을 사용하려면 다음이 필요합니다.
...
자체 호스팅 데이터베이스의 경우, 데이터베이스를 호스팅하는 호스트에 Agent를 직접 설치합니다. Amazon RDS 및 Azure SQL과 같은 클라우드 관리형 데이터베이스의 경우, Agent가 데이터베이스에 원격으로 연결되도록 설정합니다.
...
설정이 클라우드 관리형(예: [Amazon RDS][8], Aurora, Google Cloud SQL, Azure)인 경우 별도의 호스트에 Agent를 설치하고 각 관리형 인스턴스에 연결하도록 설정합니다.
...
자체 Kubernetes 클러스터를 실행하지 않는 경우, Datadog은 클라우드 공급자의 오케스트레이션 도구 사용을 권장합니다. 예를 들어, [Amazon ECS][9]를 사용하여 Datadog Agent를 호스팅할 수 있는데 [Agent는 이미 Docker 컨테이너로 존재하기 때문입니다][10].
...
[16]: /ko/database_monitoring/setup_oracle/selfhosted/
[17]: /ko/integrations/oracle/?tab=linux#data-collected