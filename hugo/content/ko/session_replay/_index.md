...
- link: https://www.datadoghq.com/blog/session-replay-custom-heatmap-backgrounds/
  tag: 블로그
  text: Session Replay의 커스텀 히트맵 캡처 및 분석
- link: https://www.datadoghq.com/blog/ai-summaries-and-smart-chapters/
  tag: 블로그
  text: AI 요약 및 스마트 챕터로 Session Replay를 더 빠르게 파악
...
- link: https://www.datadoghq.com/blog/session-replay-investigate-collaborate/
  tag: 블로그
  text: Datadog Session Replay의 사용자 세션 검색, 분석 및 협업
...
브라우저 Session Replay는 사용자의 웹 브라우징 경험을 캡처하고 시각적으로 재생할 수 있도록 하여 사용자 경험 모니터링을 확장합니다. Session Replay를 RUM 성능 데이터와 함께 사용하면 오류 식별, 재현, 해결에 유익하며 웹 애플리케이션의 사용량 패턴과 설계상 위험에 관한 인사이트를 얻을 수 있습니다.
...
모바일 Session Replay는 탭, 스와이프 및 스크롤과 같은 각 사용자 상호작용을 시각적으로 재생하여 모바일 애플리케이션에 대한 가시성을 확장합니다. Android와 iOS의 네이티브 앱 모두에서 사용할 수 있습니다. 애플리케이션에서 사용자 상호작용을 시각적으로 재생하면 충돌 및 오류를 재현하고 UI 개선을 위한 사용자 여정을 이해하는 데 더 쉬워집니다.

[모바일을 위한 Session Replay][6]에 대해 자세히 알아보세요.
...
- 재생 타임라인의 특정 타임스탬프에 코멘트를 추가합니다. 코멘트 마커는 타임라인과 {{< ui >}}Comments{{< /ui >}} 탭에 나타납니다.
...
주의가 필요한 Session Replay를 찾으려면 {{< ui >}}All mentions to me{{< /ui >}} 및 {{< ui >}}Commented replays{{< /ui >}} 기본 재생 목록을 사용하세요. 자세한 내용은 [Session Replay 재생 목록][7]을 참조하세요.

## 데이터 보존 연장 {#extend-data-retention}

기본적으로 Session Replay 데이터는 30일 동안 보존됩니다. 모든 Session Replay의 기본 보존 기간을 30일보다 길게 설정하려면 계정 팀에 문의하세요.

Session Replay 데이터 보존 기간을 15개월로 연장하려면 개별 Session Replay에서 {{< ui >}}Extended Retention{{< /ui >}}을 활성화할 수 있습니다. 이 세션은 비활성 상태여야 합니다(사용자가 경험을 완료한 상태).

나중에 Session Replay에 액세스하려면 Datadog에서 URL을 저장하거나 [재생 목록][7]에 추가하는 것을 권장합니다.

Datadog은 또한 Session Replay가 제품의 다른 곳에서 사용되는 경우 보존 기간을 15개월로 자동 연장합니다.

- [재생 목록][7]에 Session Replay 추가
- 히트맵 스크린샷으로 Session Replay 저장 [Session Replay 보존 기간을 초과하는 히트맵 분석][12]을 참조하세요.

Extended Retention은 Session Replay만 적용되며 관련 이벤트는 포함되지 않습니다. 15개월은 Extended Retention이 활성화될 때 시작되며, 세션이 수집될 때가 아닙니다.

언제든지 Extended Retention을 비활성화할 수 있습니다. 세션 리플레이가 기본 30일 보존 기간 내에 있는 경우, 리플레이는 초기 30일 기간이 끝날 때 만료됩니다. 30일 이상 된 세션 리플레이에서 Extended Retention을 비활성화하면 재생이 즉시 만료됩니다.
...
기록에는 플레이어 페이지 또는 [노트북][8]이나 사이드 패널과 같은 임베디드 플레이어에서 발생한 재생만 포함됩니다. 포함된 리플레이도 [Audit Trail][9] 이벤트를 생성합니다. 썸네일 미리보기는 기록에 포함되지 않습니다.

재생 기록을 조회하려면 [{{< ui >}}My Watch History{{< /ui >}}][10] 재생 목록을 확인하세요.

## 재생 목록 {#playlists}

세션 리플레이의 재생 목록을 만들어서 관찰한 패턴에 따라 정리할 수 있습니다. [Session Replay 재생 목록][7]에 대해 더 알아보세요.
...
[11]: /ko/session_replay/dev_tools
[12]: /ko/session_replay/heatmaps/#analyzing-heatmaps-beyond-replay-retention