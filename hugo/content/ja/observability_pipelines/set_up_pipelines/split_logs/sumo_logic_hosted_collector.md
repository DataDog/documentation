---
aliases:
- /ja/observability_pipelines/split_logs/sumo_logic_hosted_collector/
disable_toc: false
title: Sumo Logic Hosted Collector の HTTP ログ ソースのログを分割する
---

## 概要

Sumo Logic Hosted Collector の HTTP ログ ソースを構成して、ログを Observability Pipelines Worker に送信した後、ユース ケースに応じてそのログをさまざまな宛先に送信します。
...
### HTTP ログ ソースを使用した Sumo Logic Hosted Collector
...
1. Sumo Logic アドレスを入力します。これはアプリケーションがログ データを送信する先のアドレスとポートです。Observability Pipelines Worker はこのアドレスで受信ログを待ち受けます。
