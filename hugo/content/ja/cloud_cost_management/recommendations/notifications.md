---
description: 定義したスコープに一致する Cloud Cost Recommendations の定期的な Slack サマリーを送信する通知ルールを設定します。
further_reading:
...
通知ルールは、定義したスコープに一致する [Cloud Cost Recommendations][1] の定期的な Slack サマリーを送信するもので、リソースに対してアクションを実行することはありません。Datadog が自動的に変更を行うように設定せずに、新しい節約の機会を可視化したい場合は、通知ルールを使用します。

通知ルールは、定期的なスケジュールで推奨事項に対して直接アクションを実行する [コスト最適化の自動化][2] とは異なります。

## 前提条件 {#prerequisites}

- Slack コネクション。[Slack インテグレーション][3] を参照してください。
...
1. {{< ui >}}Destination{{< /ui >}} セクションで、Slack ワークスペースのコネクションとチャンネルを選択します。
1. 通知ルールの名前を入力します。
