...
TLSは、クライアントとサーバー間で転送されるデータの機密性と整合性を確保することで、Webトラフィックを保護するセキュリティプロトコルです。TLSセッション中、双方は使用する暗号アルゴリズムを規定する暗号スイートに合意します。

Datadogは、特定の暗号スイート構成を必要とする最新の暗号エンジンを使用しています。

## クライアントとの互換性 {#client-compatibility}

Datadogのシステムでは、TLS 1.2以上が必要です。互換性のあるクライアントは暗号スイートをネゴシエートできますが、クライアント側の特定の構成によってこの動作が変更される場合があります。
...
クライアントの暗号スイートの互換性をテストするには、Datadogで許可されている暗号スイートで構成されている [tls-config-test.datadoghq.com][3] に接続してください。または、[How's My SSL? API][1] を使用して、クライアントがサポートする暗号スイートをチェックしてください。コネクションの問題のトラブルシューティングについては、[Datadogサポート][2] までお問い合わせください。

## 受け入れられている暗号スイート {#accepted-cipher-suites}
...
**2026年9月1日**より、Datadogは以下の暗号スイートをサポートしません。
...
**2024年4月1日**より、Datadogは公開アプリケーション全体で以下の暗号スイートをサポートしません。これらの暗号スイートを使用するクライアントは、コネクションエラーメッセージを受け取ります。
