...
description: LLM アプリケーションや AI エージェントの監視、トラブルシューティング、改善を行うためのプラットフォーム、Agent Observability
  の概要。
further_reading:
- link: https://www.datadoghq.com/pricing/?product=llm-observability#products
  tag: 料金
  text: Agent Observability の料金
- link: /llm_observability/data_governance/
  tag: ドキュメント
  text: Agent Observability がデータを保持する期間について学んでください。
...
- link: https://www.datadoghq.com/blog/offline-llm-evaluations/
  tag: ブログ
  text: 'AI エージェントのオフライン評価: ベストプラクティス'
- link: https://www.datadoghq.com/blog/engineering/bits-ai-eval-platform/
  tag: ブログ
  text: 自律型 SRE エージェントのための実用的な評価プラットフォームを大規模に構築した方法
...
{{< img src="llm_observability/traces.png" alt="Agent Observability ページ上のプロンプト-レスポンスペアトレースの一覧表示" style="width:100%;" >}}
...
## Agent Observability との統合を活用する {#use-integrations-with-agent-observability}
...
Agent Observability は、取り込まれた LLM スパンの数に基づいて計測および課金されます。LLM スパンは LLM プロバイダーへの単一のリクエストを表すため、1 つのワークフローで複数の LLM スパンが生成される可能性があります。料金については、[Agent Observability 料金ページ][11]を参照してください。
...
[9]: /ja/llm_observability/investigate/evaluations/managed_evaluations
[10]: /ja/llm_observability/investigate/patterns
