...
MSK インテグレーションを強化する手段として、[Data Streams Monitoring][2] の利用を検討してください。このソリューションではパイプラインを可視化し、ラグ (遅延) を追跡できるため、ボトルネックの特定と解消に役立ちます。

## セットアップ

Agent チェックは、Datadog Agent を通じて、Amazon Managed Streaming for Apache Kafka ([Amazon MSK][3]) を監視します。
...
1. まだ作成していない場合は、[クライアント マシンを作成します][6]。
2. クライアント マシンに権限ポリシー ([arn:aws:iam::aws:policy/AmazonMSKReadOnlyAccess][8]) が[付与][7]されている、または同等の[資格情報][9]が利用可能であることを確認します。
3. JmxExporter と NodeExporter を有効にするため、MSK 側で [Prometheus によるオープン モニタリング][10] を有効にします。
4. 先ほど作成したクライアント マシンに [Datadog Agent][11] をインストールします。

### 構成

1. Amazon MSK のパフォーマンスデータの収集を開始するには、Agent の構成ディレクトリのルートにある `conf.d/` フォルダーの `amazon_msk.d/conf.yaml` ファイルを編集します。

   このインテグレーションで提供されるすべてのメトリクスとサービス チェックに関連付けられるカスタム [タグ][12] を含めます。
   ...
   最新モードで利用可能なすべての構成オプションについては、[amazon_msk.d/conf.yaml のサンプル][13] を参照してください。このインテグレーションのレガシー モードについては、[レガシーの構成例][14] を参照してください。

2. [Agent を再起動します][15]。

### 検証

[Agent の status サブ コマンドを実行][16] し、Checks セクションで `amazon_msk` を探します。
...
- [Datadog による Amazon Managed Streaming for Apache Kafka の監視][20]

[1]: https://docs.datadoghq.com/ja/integrations/amazon_msk
[2]: https://docs.datadoghq.com/ja/data_streams/
...
[12]: https://docs.datadoghq.com/ja/getting_started/tagging/
...
[15]: https://docs.datadoghq.com/ja/agent/guide/agent-commands/#start-stop-and-restart-the-agent
