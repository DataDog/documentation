...
1. {{< ui >}}Token Name{{< /ui >}} および {{< ui >}}Token Value{{< /ui >}} を入力します。複数のトークンを入力できます。
1. コネクションを使用するHTTPリクエストにトークンを含める方法を設定します。コネクションは、リクエストにトークンを自動的に追加しません。{{< ui >}}Request Headers{{< /ui >}}、{{< ui >}}URL parameters{{< /ui >}}、または {{< ui >}}Body{{< /ui >}} で、次の構文を使用してトークンを参照します。`{{ secretTokenName }}`, replacing `secretTokenName` with the {{< ui >}}Token Name{{< /ui >}}（前のステップから）。例えば、トークン名が `apiToken`, add an `Authorization` header with the value `Bearer` の場合、 {{ apiToken }}` です。
1. 必要に応じて、他の {{< ui >}}Request Headers{{< /ui >}}、{{< ui >}}URL parameters{{< /ui >}}、または {{< ui >}}Body{{< /ui >}} をリクエストに追加します。
1. {{< ui >}}Create{{< /ui >}} をクリックします。
