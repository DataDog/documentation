...
アプリケーションの起動時に、たとえば USB デバッグの有効化を検証するなどのセキュリティチェックが行われる場合、Datadog ではこれらのチェックを含まないバージョンのアプリケーションをアップロードすることを推奨します。
...
### 時々、プライベートロケーションのコンテナが `OOM` により強制終了される

`Out Of Memory` により強制終了されるプライベートロケーションのコンテナは、通常、プライベートロケーションワーカーのリソース消費の問題を明らかにします。プライベートロケーションのコンテナが、[十分なメモリリソース][101]でプロビジョニングされていることを確認してください。

### プライベートロケーションのテストを実行しようとすると、`invalid mount config for type "bind": source path must be a directory` というエラーが表示される

これは、Windows ベースのコンテナで単一ファイルをマウントしようとした場合に発生し、サポートされていません。詳しくは、[Docker マウントボリュームのドキュメント][102]をご参照ください。バインドマウントのソースがローカルディレクトリであることを確認してください。
