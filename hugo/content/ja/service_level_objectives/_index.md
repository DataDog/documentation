...
{{< img src="service_level_objectives/slo_bulk_tag.png" alt="SLO リストページには、タグの一括編集のための [Edit Tag] (タグの編集) ドロップダウンが表示されます" >}}
...
保存表示を読み込むには、ページの左上にある {{< ui >}}Views{{< /ui >}} ボタンを押して {{< ui >}}Saved Views{{< /ui >}} パネルを開き、リストから保存表示を選択します。同じ {{< ui >}}Saved Views{{< /ui >}} パネルの上部にある {{< ui >}}Filter Saved Views{{< /ui >}} 検索ボックスで、保存表示を検索することもできます。
...
保存表示を使用しているときは、その保存表示を選択してクエリを修正し、{{< ui >}}Saved Views{{< /ui >}} パネルで名前の下にある {{< ui >}}Update{{< /ui >}} ボタンをクリックすることで更新できます。保存表示の名前を変更したり、保存表示を削除したりするには、{{< ui >}}Saved Views{{< /ui >}} パネルでその行にカーソルを合わせ、それぞれ鉛筆アイコンまたはゴミ箱アイコンをクリックします。

## SLO および SLO ステータス修正の監査イベント {#slo-and-slo-status-correction-audit-events}

SLO 監査イベントでは、[Event Explorer][27] または SLO 詳細の {{< ui >}}Audit History{{< /ui >}} タブを使用して、SLO 設定の履歴を追跡できます。監査イベントは、SLO または SLO ステータス修正を作成、変更、または削除するたびに、Event Explorer に追加されます。各イベントには、SLO または SLO ステータス修正の構成に関する情報が含まれ、ストリームには構成変更の履歴が表示されます。
...
{{< img src="service_level_objectives/slo_audit_history_tab.png" alt="SLO 詳細の [Audit History] タブ" >}}
...
[SLO 管理ページ][2] の右上にある SLO カレンダービューでは、右上隅で {{< ui >}}Primary{{< /ui >}} 表示から {{< ui >}}Daily{{< /ui >}}、{{< ui >}}Weekly{{< /ui >}}、または {{< ui >}}Monthly{{< /ui >}} 表示に切り替えると、12 か月分の過去の SLO ステータスデータを確認できます。カレンダービューはメトリクスベースの SLO とタイムスタイス SLO に対応しています。
