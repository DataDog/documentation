...
| `on_error`      | `Proc`                  | スパンがエラーを発生させたときのエラー処理の動作をオーバーライドします。引数として `span` と `error` が指定されました。デフォルトではスパンにエラーを設定します。 | `proc { |スパン、エラー| span.set_error(error) unless span.nil? }` |
...
利用可能なインテグレーションとその対応バージョンの一覧については、[Ruby CI インテグレーションの互換性][3]を参照してください。
...
| キー | 説明  | Default |
| ---| --- | --- |
| `template_base_path` | テンプレート名がパースされるときに使用されます。テンプレートを `views/` フォルダーに保存していない場合、この値を変更する必要があるかもしれません | `'views/'` |
...
| キー             | 説明                                                                                                                                                                                         | Default                |
|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------|
| `cache_service` | `active_support` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `active_support-cache` |
...
| キー            | 環境変数                     | 説明                                                                                                                                                                              | Default |
|----------------|-----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `service_name` | `DD_TRACE_AWS_SERVICE_NAME` | `aws` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `aws`   |
...
| キー            | 環境変数                       | 説明                                                                                                                                                                                | Default     |
|----------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| `command_enabled` | `DD_TRACE_MEMCACHED_COMMAND_ENABLED` | コマンドを `memcached.command` タグとして収集します。コマンド`keys` には機密情報が含まれている可能性があります。 | `false` |
| `service_name` | `DD_TRACE_DALLI_SERVICE_NAME` | `dalli` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `memcached` |
...
| キー | 説明  | Default |
| --- | ----------- | ------- |
| `error_handler` | ジョブでエラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。一時的なエラーを無視したい場合に役立ちます。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
| キー            | 環境変数                               | 説明                                                                                                                                                                                        | Default         |
|----------------|---------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|
| `service_name` | `DD_TRACE_ELASTICSEARCH_SERVICE_NAME` | `dalli` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `elasticsearch` |
| `peer_service` | `DD_TRACE_ELASTICSEARCH_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                              | `nil`           |
| `quantize`     |                                       | 量子化のオプションを含むハッシュ。量子化しないキーの配列を含む `:show` (または量子化をスキップする場合は `:all`)、または完全に除外するキーの配列を含む `:exclude` を指定することができます。       | `{}`            |
...
| キー                   | 環境変数                       | 説明                                                                                                                                                                                | Default |
|-----------------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `service_name`        | `DD_TRACE_ETHON_SERVICE_NAME` | `ethon` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `ethon` |
...
| キー                   | 環境変数                       | 説明                                                                                                                                                                                | Default |
|-----------------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `service_name`        | `DD_TRACE_EXCON_SERVICE_NAME` | `excon` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `excon` |
...
| キー                   | 環境変数                         | 説明                                                                                                                                                                                  | Default   |
|-----------------------|---------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| `service_name`        | `DD_TRACE_FARADAY_SERVICE_NAME` | `faraday` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `faraday` |
| `peer_service`        | `DD_TRACE_FARADAY_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                        | `nil`     |
| `distributed_tracing` |                                 | [分散型トレーシング](#distributed-tracing)を有効にします                                                                                                                                         | `true`    |
| `split_by_domain`     |                                 | `true` に設定されている場合、リクエストドメインをサービス名として使用します。                                                                                                                             | `false`   |
| `error_handler`       |                                 | `response` パラメーターを受け入れる `Proc`。*truthy* 値に評価される場合、トレーススパンはエラーとしてマークされます。デフォルトでは、5XX 応答のみをエラーとして設定します。                      | `nil`     |
| `on_error` | リクエストがエラーを発生させたときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
| キー                   | 環境変数                      | 説明                                                                                                                                                                               | Default                                                            |
|-----------------------|------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------|
| `service_name`        | `DD_TRACE_GRPC_SERVICE_NAME` | `grpc` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `grpc`                                                             |
| `peer_service`        | `DD_TRACE_GRPC_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                     | `nil`                                                              |
| `distributed_tracing` |                              | [分散型トレーシング](#distributed-tracing)を有効にします                                                                                                                                      | `true`                                                             |
| `server_error_handler`       |                              | サーバーエラーが発生したときに呼び出されるカスタムエラーハンドラー。`span` および `error` パラメータを受け取る `Proc` です。デフォルトではスパンにエラーを設定します。                                         | `proc { \|span, error \| span.set_error(error) unless span.nil? }` |
| `client_error_handler`       |                              | クライアントエラーが発生したときに呼び出されるカスタムエラーハンドラー。`span` および `error` パラメータを受け取る `Proc` です。デフォルトではスパンにエラーを設定します。                                         | `proc { \|span, error \| span.set_error(error) unless span.nil? }` |

非推奨のお知らせ
- `error_handler` は削除されます。代わりに `server_error_handler` を使用してください。
...
| キー                   | 環境変数                                  | 説明                                                                                                                                                                                 | Default     |
|-----------------------|------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| `service_name`        | `DD_TRACE_HTTPRB_SERVICE_NAME`           | `httprb` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `httprb`    |
...
| キー                   | 環境変数                                  | 説明                                                                                                                                                                                     | Default      |
|-----------------------|------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| `service_name`        | `DD_TRACE_HTTPCLIENT_SERVICE_NAME`       | `httpclient` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `httpclient` |
...
Minitest インテグレーションは、`minitest` テストフレームワークを使用しているときに、テストのすべての実行をトレースします。
...
| キー | 説明  | Default |
| --- | ----------- | ------- |
| `enabled` | Minitest テストをトレースする必要があるのかどうかを定義します。トレーシングを一時的に無効化するのに役立ちます。`true` または `false` | `true` |
| `service_name` | `minitest` インスツルメンテーションに使用されるサービス名 | `'minitest'` |
| `operation_name` | `minitest` インスツルメンテーションに使用されているオペレーション名です。`trace.#{operation_name}.errors` など、自動トレースメトリクスの名前を変更したい場合に役立ちます。 | `'minitest.test'` |
...
| キー            | 環境変数                       | 説明                                                                                                                                                                                  | Default                                          |
|----------------|-------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|
| `service_name` | `DD_TRACE_MONGO_SERVICE_NAME` | `mongo` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。   | `mongodb`                                        |
| `peer_service` | `DD_TRACE_MONGO_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                        | `nil`                                            |
| `quantize`     |                               | 量子化のオプションを含むハッシュ。量子化しないキーの配列を含む `:show` (または量子化をスキップする場合は `:all`)、または完全に除外するキーの配列を含む `:exclude` を指定することができます。 | `{ show: [:collection, :database, :operation] }` |
...
| キー                   | 環境変数                        | 説明                                                                                                                                                                                                                                                                                                                                                              | Default      |
|-----------------------|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| `service_name`        | `DD_TRACE_MYSQL2_SERVICE_NAME` | `mysql2` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。                                                                                                                                                                              | `mysql2`     |
| `peer_service`        | `DD_TRACE_MYSQL2_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                                                                                                                                                                                                    | `nil`        |
| `comment_propagation` | `DD_DBM_PROPAGATION_MODE`      | データベースモニタリングのための SQL コメント伝搬モード。 <br />(例: `disabled` \| `service`\| `full`). <br /><br />**重要**: *SQL  コメントの伝播を有効にすると、潜在的に機密データ (サービス名) がデータベースに保存され、データベースへのアクセスを許可された他の第三者がそのデータにアクセスする可能性があることに注意してください。* | `'disabled'` |
| `on_error` | | MySQL でエラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。アプリケーションレベルで処理されるエラーを無視したい場合に役立ちます。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
### Net/HTTP
...
| キー                   | 環境変数                            | 説明                                                                                                                                                                                   | Default     |
|-----------------------|------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| `service_name`        | `DD_TRACE_NET_HTTP_SERVICE_NAME`   | `net/http` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `net/http`  |
...
| キー            | 環境変数                            | 説明                                                                                                                                                                                     | Default      |
|----------------|------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| `service_name` | `DD_TRACE_OPENSEARCH_SERVICE_NAME` | `opensearch` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `opensearch` |
...
| `service_name`        | `DD_TRACE_PG_SERVICE_NAME` | `pg` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。                                                                                                                                                                                  | `pg`         |
...
| `error_handler` || エラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。アプリケーションレベルで処理されるエラーを無視したい場合に役立ちます。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
| キー                   | 環境変数                        | 説明                                                                                                                                                                                 | Default  |
|-----------------------|--------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| `service_name`        | `DD_TRACE_PRESTO_SERVICE_NAME` | `presto` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `presto` |
...
| キー            | 環境変数                 | 説明                                                     | Default |
|----------------|-------------------------|----------------------------------------------------------------|---------|
| `tag_job_data` | `DD_QLESS_TAG_JOB_DATA` | ジョブ引数のタグ付けを有効にします。オンの場合は `true`、オフの場合は `false` です。 | `false` |
| `tag_job_tags` | `DD_QLESS_TAG_JOB_TAGS` | ジョブタグのタグ付けを有効にします。オンの場合は `true`、オフの場合は `false` です。      | `false` |
...
| `tag_args`      | `DD_TRACE_QUE_TAG_ARGS_ENABLED` | ジョブの引数フィールドのタグ付けを有効にします。有効にするには `true`、無効にするには `false` を指定します。                                                                                                       | `false`                                                            |
| `tag_data`      | `DD_TRACE_QUE_TAG_DATA_ENABLED` | ジョブのデータフィールドのタグ付けを有効にします。有効にするには `true`、無効にするには `false` を指定します。                                                                                                       | `false`                                                            |
| `error_handler` |                                 | ジョブでエラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。一時的なエラーを無視したい場合に役立ちます。 | `proc { \|span, error \| span.set_error(error) unless span.nil? }` |
...
| キー                   | 環境変数                         | 説明                                                                                                                                                                                  | Default   |
|-----------------------|---------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| `service_name`        | `DD_TRACE_RACECAR_SERVICE_NAME` | `racecar` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `racecar` |
|
### Rack
...
| `distributed_tracing` | [分散型トレーシング](#distributed-tracing)を有効にすると、トレースヘッダーが受信された場合、このサービストレースが他のサービスのトレースと接続されるようになります | `true` |
| `headers` | `rack.request` にタグとして追加するための HTTP リクエストまたは応答ヘッダーのハッシュ。配列の値を持つ `request` および `response` キーを受け入れます (例: `['Last-Modified']`)。それぞれ `http.request.headers.*` タグと `http.response.headers.*` タグを追加します。このオプションは、グローバルの `DD_TRACE_HEADER_TAGS` をオーバーライドします。詳しくは、[ルートスパンにヘッダータグを適用する][header tags]を参照してください。 | `{ response: ['Content-Type', 'X-Request-ID'] }` |
...
| `quantize` | 量子化のオプションを含むハッシュ。`:query` または `:fragment` を指定することができます。 | `{}` |
...
| `distributed_tracing` | [分散型トレーシング](#distributed-tracing)を有効にすると、トレースヘッダーが受信された場合、このサービストレースが他のサービスのトレースと接続されるようになります | `true` |
| `request_queuing` | フロントエンドサーバーのキューで費やされた HTTP リクエスト時間を追跡します。設定の詳細については、[HTTP リクエストキュー](#http-request-queuing)をご覧ください。  | `false` |
| `middleware` | トレースミドルウェアを Rails アプリケーションに追加します。ミドルウェアを読み込みたくない場合は、`false` に設定します。 | `true` |
...
| キー            | 環境変数                       | 説明                                                                                                                                                                                | Default |
|----------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `service_name` | `DD_TRACE_REDIS_SERVICE_NAME` | `redis` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `redis` |
...
Datadog.configure do |c|
  c.tracing.instrument :redis # インテグレーションインスツルメンテーションを有効にすることは依然として必要です
end
...
| キー                   | 環境変数                             | 説明                                                                                                                                                                                      | Default       |
|-----------------------|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------|
| `service_name`        | `DD_TRACE_REST_CLIENT_SERVICE_NAME` | `rest_client` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。 | `rest_client` |
...
| キー | 説明  | Default |
| --- | ----------- | ------- |
| `tag_body` | SQS メッセージの本文が `true` または `false` であるスパンにタグ付け | `false` |
| `error_handler` | ジョブでエラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。一時的なエラーを無視したい場合に役立ちます。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
| --- | ----------- | ------- |
| `distributed_tracing` | [分散型トレーシング](#distributed-tracing)を有効にすると、トレースヘッダーが受信された場合、このサービストレースが他のサービスのトレースと接続されるようになります | `true` |
| `headers` | `sinatra.request` にタグとして追加するための HTTP リクエストまたは応答ヘッダーのハッシュ。配列の値を持つ `request` および `response` キーを受け入れます (例: `['Last-Modified']`)。それぞれ `http.request.headers.*` タグと `http.response.headers.*` タグを追加します。このオプションは、グローバルの `DD_TRACE_HEADER_TAGS` をオーバーライドします。詳しくは、[ルートスパンにヘッダータグを適用する][header tags]を参照してください。 | `{ response: ['Content-Type', 'X-Request-ID'] }` |
...
| `tag_body` | ジョブメッセージのタグ付けを有効にします。有効にするには `true`、無効にするには `false` を指定します。 | `false` |
| `error_handler` | ジョブでエラーが発生したときに呼び出されるカスタムエラーハンドラー。引数として `span` と `error` が指定されます。デフォルトではスパンにエラーを設定します。一時的なエラーを無視したい場合に役立ちます。 | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
Datadog.configure do |c|
  c.tracing.instrument :trilogy, **オプション
end
...
| キー                   | 環境変数                        | 説明                                                                                                                                                                                                                                                                                                                                                              | Default      |
|-----------------------|---------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| `service_name`        | `DD_TRACE_TRILOGY_SERVICE_NAME` | `trilogy` インスツルメンテーションを実行しているアプリケーションの名前。`global_default_service_name` で上書きすることができます。[詳しくは*追加構成*を参照してください](#additional-configuration)。  | `trilogy` |
| `peer_service`        | `DD_TRACE_TRILOGY_PEER_SERVICE` | アプリケーションが接続する外部サービスの名前                                                                                                                                         | `nil`     |

## 追加構成

`ddtrace` のデフォルトの動作を変更するには、優先順位の高い順に (1が最も高い) 以下を使用します。
...
| `diagnostics.debug`                                     | `DD_TRACE_DEBUG`                                        | `false`                      | デバッグモードの有効/無効を設定します。詳細なログを出力します。**本番環境やその他機密性の高い環境では推奨されません。**詳細は、[デバッグと診断](#debugging-and-diagnostics)を参照してください。                                                                                                                                                                                                                                                                                                                                                                                   |
| `diagnostics.startup_logs.enabled`                      | `DD_TRACE_STARTUP_LOGS`                                 | `nil`                        | 起動時の設定や診断結果をログに出力します。アプリケーション起動時のトレース状態を評価するためです。詳しくは、[デバッグと診断](#debugging-and-diagnostics)を参照してください。                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `tracing.contrib.global_default_service_name.enabled`   | `DD_TRACE_REMOVE_INTEGRATION_SERVICE_NAMES_ENABLED`     | `false`                      | すべてのインスツルメンテーションで `service_name` のデフォルト値をアプリケーション サービス名に変更します。 [Inferred Services ベータ](https://docs.datadoghq.com/tracing/guide/inferred-service-opt-in) と併用します。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `tracing.distributed_tracing.propagation_extract_first` | `DD_TRACE_PROPAGATION_EXTRACT_FIRST` | `false` | 最初の有効なプロパゲーション フォーマットが検出されると、検索を停止します。詳細は [Distributed Tracing](#distributed-tracing) を参照してください。 |
...
| `tracing.log_injection`                                 | `DD_LOGS_INJECTION`                                     | `true`                       | [トレース相関](#trace-correlation)の情報が存在する場合、それを Rails のログに挿入します。デフォルトのロガー (`ActiveSupport::TaggedLogging`)、`lograge`、`semantic_logger` をサポートします。                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `tracing.sampling.rate_limit`                           | `DD_TRACE_RATE_LIMIT`                                   | `100` (毎秒)           | 1 秒あたりにサンプリングするトレースの最大数を設定します。トラフィックの急増時に取り込み量が過剰にならないよう、レート制限を設定します。                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `OpenTracing::FORMAT_RACK`     | Yes        | Rack 形式では解決が失われるため、大文字または `-` のいずれかを含む名前のバゲージアイテムは、往復でそれぞれ小文字と `_` に変換されることに注意してください。Datadog は、これらの文字を避けるか、受信側でそれに応じて対応することをお勧めします。 |
...
Resque ではデフォルトで、ジョブごとにプロセスがフォークされるため、ddtrace でインスツルメントされた場合、まれに resque 処理が終了時にハングアップすることがあります。
