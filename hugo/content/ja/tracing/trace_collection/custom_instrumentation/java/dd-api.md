...
code_lang: dd-api
code_lang_weight: 1
description: Datadog API でコードをインスツルメントします。
further_reading:
...
title: Java で Datadog API を使用したカスタム インスツルメンテーション
...
Datadog Java トレーサーは、カスタム インスツルメンテーション用に <code>opentracing-api</code> ライブラリと相互運用します。カスタム インスツルメンテーションに OpenTelemetry API を使用したい場合は、代わりに <a href="/tracing/trace_collection/custom_instrumentation/java/otel/">OpenTelemetry を使用した Java のカスタム インスツルメンテーション</a> を参照してください。
</div>

## 前提条件

- 自動インスツルメンテーションのセットアップ手順をまだ読んでいない場合は、まず [Java セットアップ手順][11] をご覧ください。
- このページの例をコンパイルするには、プロジェクトに [opentracing-api][12] 依存関係を追加します。
...
同じクラス内の複数の関数をトレースするには、次の構文を使用します:
...
`@Trace` アノテーションで使用できる引数は次のとおりです:

- `operationName`: トレースのオペレーション名を設定します (デフォルト: メソッド名)。
- `resourceName`: トレースのリソース名を設定します (デフォルト: `operationName` と同じ値)。
- `noParent`: そのメソッドで常に新しいトレースを開始するには `true` を設定します。`dd-trace-java` v1.22.0 以降でサポートされています (デフォルト: `false`)。
...
```
**注**: `dd.trace.annotations` システム プロパティを介して、他のトレーシング メソッドのアノテーションを Datadog が `@Trace` として認識できます。以前にコードにアノテーションを付与している場合は、[TraceAnnotationsInstrumentation.java][7] に一覧があります。
