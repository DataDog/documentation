...
- link: https://www.datadoghq.com/blog/single-step-instrumentation-rules/
  tag: ブログ
  text: Single Step インスツルメンテーション ルールを使用して、ホスト全体のサービスのトレーシングを管理します。
- link: https://www.datadoghq.com/blog/choosing-apm-instrumentation/
  tag: ブログ
  text: 'ゼロからトレースまで: スタックに適した APM インスツルメンテーション手法の選択'
...
もし {{< ui >}}APM Instrumentation{{< /ui >}} を有効にした状態で [Datadog Agent をインストールまたは更新する][1] 場合、Datadog Agent はサポートされているプロセスに Datadog SDK を読み込むことで、アプリケーションのインスツルメンテーションを行います。これにより、コードの変更を必要とせずに、サービスからトレースデータがキャプチャされて送信され、分散型トレーシングが実現されます。
