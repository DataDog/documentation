...
- link: https://www.datadoghq.com/blog/how-datadogs-tech-solutions-team-rum-session-replay/
  tag: ブログ
  text: Datadog のテクニカルソリューションチームが RUM、Session Replay、Error Tracking を使用して顧客の問題を解決する方法
...
- link: https://www.datadoghq.com/blog/progressive-web-application-monitoring/
  tag: ブログ
  text: プログレッシブ Web アプリケーションを監視するためのベストプラクティス
...
title: RUM および Session Replay
---
{{< learning-center-callout header="イネーブルメントウェビナーセッションへの参加" hide_image="true" btn_title="サインアップ" btn_url="https://www.datadoghq.com/technical-enablement/sessions/?tags.topics-0=RUM">}}
...
Datadog の *Real User Monitoring (RUM)* は、個々のユーザーのリアルタイムのアクティビティとエクスペリエンスをエンドツーエンドで可視化します。RUM は、Web およびモバイルアプリケーションの 4 種類のユースケースを解決するように設計されています。
...
ユーザーセッションは、Web またはモバイルアプリケーションについてのユーザージャーニーです。セッションには、関連するすべてのナビゲーションイベント (RUM ビュー)、ユーザーアクション (RUM アクション)、ネットワークリクエスト (RUM リソース)、クラッシュとエラー (RUM エラー)、およびユーザー エクスペリエンスの忠実な表現を生成する他のイベントやシグナルが含まれます。
...
## Session Replay とは{#what-is-session-replay}

Datadog の *Session Replay* は、ユーザーの Web ブラウジング体験をキャプチャし、視覚的に再生することができます。

Session Replay を RUM パフォーマンスデータと組み合わせることで、エラーの特定、再現、解決に役立ち、Web アプリケーションの使用パターンや設計上の落とし穴を把握することができます。
...
{{< card-grid card_width="210" >}}
  {{< image-card href="/real_user_monitoring/application_monitoring/browser/" src="integrations_logos/javascript_large.svg" alt="ブラウザ" >}}
...
| エラー追跡とソースマッピング | {{< X >}} | {{< X >}}  | {{< X >}} | {{< X >}} | {{< X >}} | {{< X >}} | {{< X >}} | {{< X >}} |  |
| クラッシュ追跡、シンボル化、難読化解除 | {{< X >}} | {{< X >}}  | {{< X >}}  | {{< X >}} | {{< X >}} | {{< X >}}  | {{< X >}} | {{< X >}} |  |
| セッションの停止 (Kiosk Monitoring) | {{< X >}} | {{< X >}}  | {{< X >}}  | {{< X >}} | {{< X >}} |  | {{< X >}} | {{< X >}}  |  |
| WebView でのイベントの追跡 |  | {{< X >}}  | {{< X >}}  | {{< X >}} | {{< X >}} |  | {{< X >}} |  |  |
| プラットフォーム固有のバイタルの監視 | {{< X >}} | {{< X >}}  | {{< X >}}  | {{< X >}} | {{< X >}} |  | {{< X >}} |  |  |
| ログのグローバルコンテキスト/属性追跡  | {{< X >}} | {{< X >}}  | {{< X >}} | {{< X >}} | {{< X >}} |  | {{< X >}} | {{< X >}} |  |
| クライアント側のトレース |  | {{< X >}} |  {{< X >}}|  |  |  |  |  |  |  |
| Session Replay | {{< X >}} | {{< X >}} | {{< X >}} | {{< X >}} | {{< X >}} |  | {{< X >}} |  | **Flutter** の Session Replay はプレビュー版です。|
...
Datadog SDK のトラフィックはすべて SSL (デフォルト 443) で次のドメインに送信されます。
...
### ブラウザプロファイリング用の追加のエンドポイント {#additional-endpoints-for-browser-profiling}

[ブラウザプロファイリング][19]が有効な場合、SDK はクォータ API にも接続し、現在のセッションでプロファイリングが許可されているかどうかを調べます。これには、標準のインテイクオリジンの `quota.` サブドメインが使用されます。
...
[プロキシ][20]を使用している場合や、[コンテンツセキュリティポリシー (CSP)][21] を設定している場合は、これらの `quota.` ドメインも許可されていることを確認してください。詳細については、[ブラウザプロファイリングのセットアップ][19]ページを参照してください。

## Datadog RUM を詳しく見る {#explore-datadog-rum}

[{{< ui >}}Digital Experience{{< /ui >}} > {{< ui >}}Performance Summary{{< /ui >}}][1] に移動して RUM にアクセスします。
...
| ブラウザのパフォーマンスの概要 | モバイルのパフォーマンスの概要 |
|---------|---------|
...
[RUM パフォーマンスモニタリング概要][1]ページは、Web およびモバイルアプリケーション双方に関連性が高く実用的なインサイトを提供します。また、各プラットフォームに合わせたエクスペリエンスによって、次のことが可能となります。

- **主要データポイントにフォーカス**:プラットフォームごとに、Web の UI レイテンシー、モバイルのクラッシュなどにフォーカスできます。
...
[iOS、iPadOS、tvOS、visionOS][8] または [Android と Android TV][9] アプリケーションの Core Web Vitals や Mobile Vitals など、[ブラウザアプリケーション][7]のパフォーマンススコアとテレメトリを表示します。

### Web ビュー追跡 {#web-view-tracking}

[iOS、iPadOS、visionOS][10] または [Android と Android TV][11] 用の Web ビュー追跡を使用して、ネイティブ Web アプリケーションから情報を収集し、ハイブリッドビューを調査します。

{{< img src="real_user_monitoring/webview_tracking/webview_tracking_light.png" alt="RUM エクスプローラーでユーザーセッション中にキャプチャされた Web ビュー" >}}

## Datadog Session Replay を見る {#explore-datadog-session-replay}

### Session Replay {#session-replays}
...
きめ細かなアクセスコントロールを使用して、特定のアプリケーションの RUM 構成を編集できる[ロール][18]を制限します。
1. アプリケーションの RUM 構成を表示している状態で、画面上部の {{< ui >}}Edit application{{< /ui >}} ボタンをクリックします。ドロップダウンが表示されます。
1. {{< ui >}}Manage App Permissions{{< /ui >}} を選択します。
1. {{< ui >}}Restrict Access{{< /ui >}} をクリックします。
1. ダイアログボックスが更新され、組織のメンバーはデフォルトで {{< ui >}}Viewer{{< /ui >}} アクセス権を付与されていることが表示されます。
...
制限されたアプリケーションへの一般アクセスを復元するには、編集アクセスが必要です。次のステップを完了します。
1. アプリケーションの RUM 構成を表示している状態で、画面上部の {{< ui >}}Edit application{{< /ui >}} ボタンをクリックします。ドロップダウンが表示されます。
1. {{< ui >}}Manage App Permissions{{< /ui >}} を選択します。
1. {{< ui >}}Restore Full Access{{< /ui >}} をクリックします。
1. {{< ui >}}Save{{< /ui >}} をクリックします。
...
[13]: /ja/session_replay/privacy_options?platform=browser
[14]: /ja/session_replay/dev_tools
...
[19]: /ja/real_user_monitoring/correlate_with_other_telemetry/profiling
[20]: /ja/real_user_monitoring/guide/proxy-rum-data
[21]: /ja/integrations/content_security_policy_logs