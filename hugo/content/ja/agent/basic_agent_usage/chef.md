...
title: Chef
---
Datadog の Chef クックブックは、サポート対象の Linux および Windows プラットフォームにおける Datadog Agent のインストールと構成を自動化します。このクックブックは `chef-client >= 12.7` と互換性があり、Windows および Linux プラットフォームをサポートします。属性の完全なリファレンス、レシピ、使用例、インテグレーションの構成パターンについては、[高度な構成セクション](#advanced-configurations) を参照してください。
