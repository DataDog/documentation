...
title: Ansible でスタンドアロンの Datadog ロールを使用してセットアップする
---
...
* Datadog コレクションの一部として、Ansible Galaxy 上で [datadog.dd](https://galaxy.ansible.com/ui/repo/published/datadog/dd/) の名前で利用可能 (推奨)。
* スタンドアロンロールとして、Ansible Galaxy 上で [datadog.datadog](https://galaxy.ansible.com/ui/standalone/roles/DataDog/datadog/) の名前で利用可能 (レガシー)。
...
- Ansible 2.10+ で Windows ホストを管理する場合、`ansible.windows` コレクションをインストールする必要があります。
  ...
  ```
- Ansible 2.10+ を使用して openSUSE/SLES ホストを管理する場合、`community.general` コレクションのインストールが必要です。
...
API キーは必須で、存在しない場合はロールが失敗します。Ansible の管理外で別の方法で提供したい場合は、プレースホルダーのキーを指定し、後でキーを置き換えてください。

### エアギャップ環境

特定のレジストリとイメージを使用して Datadog をエアギャップ環境にインストールするには、Datadog Ansible コレクションと、`datadog_installer_registry`、`datadog_installer_auth`、`agent_datadog_config` 変数を併用してください。

**注**: `agent_datadog_config` は `installer_registry_config` の設定を上書きします。
...
これらの変数は Datadog Agent のインストール時に追加の設定を行います。`vars` セクションで指定してください。

| 変数                                    | 説明                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|---------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `datadog_api_key`                           | Datadog API キー。**バージョン 4.21 以降、この変数は必須です**。|
...
| `datadog_yum_repo_config_enabled`           | `false` に設定すると、Datadog の `yum` リポジトリの設定を行いません (デフォルトは `true`)。警告: GPG キーの自動更新が無効になります。|
| `datadog_yum_repo`                          | デフォルトの Datadog `yum` レポジトリを上書きします。|
| `datadog_yum_repo_proxy`                    | Datadog の `yum` リポジトリ設定で使用するプロキシ URL を設定します。|
| `datadog_yum_repo_proxy_username`           | Datadog の `yum` リポジトリ設定で使用するプロキシのユーザー名を設定します。|
| `datadog_yum_repo_proxy_password`           | Datadog の `yum` リポジトリ設定で使用するプロキシのパスワードを設定します。|
...
| `datadog_apm_instrumentation_enabled`       | APM インスツルメンテーションを設定します。指定可能な値は以下のとおりです。<br/> - `host`: Agent とサービスが同一ホスト上で実行される。<br/> - `docker`: Agent とサービスが同一ホスト上の別々の Docker コンテナで実行される。<br/> - `all`: `host` と `docker` の両方のシナリオを同時にサポート。|
| `datadog_apm_instrumentation_libraries`     | `host` または `docker` のインジェクションが有効な場合にインストールする APM ライブラリの一覧 (デフォルトは `["java", "js", "dotnet", "python", "ruby"]`)。利用できる値は [Inject Libraries Locally][21] を参照してください。|
| `datadog_remote_updates`                    | datadog-installer を介したリモートでのインストールおよび更新を有効にします。|
...
インテグレーションをインストールまたは削除するには、`datadog_integration` の[該当セクション](#integration-installation)を参照してください
...
インテグレーションを設定したい場合は、`datadog_checks` の[該当セクション](#integrations)を参照してください
...
```yml
- name: Datadog Agent がインストールされているか確認
  win_shell: |
...
- name: Datadog Agent のインストール状況を設定
  set_fact:
    agent_installed: "{{ agent_installed_result.stdout | trim }}"
- name: Datadog Agent をアンインストール
  win_package:
...
ロール `4.21` 以降では、API キーがないとロールを続行できません。

Ansible を用いて Agent をインストールしつつ、API キーを指定したくない場合 (コンテナや VM イメージに組み込みたいなど) は、以下のいずれかを行ってください。
* ダミーの API キーを指定し、後で置き換える
* managed_config (`datadog_manage_config: false`) を無効にする 
