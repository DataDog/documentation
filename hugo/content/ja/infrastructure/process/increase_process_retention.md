...
<div class="alert alert-danger">プロセス ベースのメトリクスは<a href="/metrics/custom_metrics/">カスタム メトリクス</a>と見なされ、それに応じて課金されます。請求額への影響を避けるために、<code>command</code> や <code>user</code> などの無制限または非常にカーディナリティの高いタグでのグループ化は避けてください。</div>
...
作成後は、Datadog の他のメトリクスと同様に、プロセス ベースのメトリクスを使用することができます。例は以下の通りです。
