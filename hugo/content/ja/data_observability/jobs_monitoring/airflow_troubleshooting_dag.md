---
description: 'OpenLineage 検証 DAG を使用して、Airflow の Data Observability: Jobs Monitoring
  の構成に関する問題を診断します。'
further_reading:
- link: /data_observability/jobs_monitoring/airflow/
  tag: ドキュメント
  text: 'Apache Airflow の Data Observability: Jobs Monitoring を有効にする'
- link: /data_jobs
  tag: ドキュメント
  text: 'Data Observability: Jobs Monitoring'
title: OpenLineage 検証 DAG を使用した Airflow セットアップのトラブルシューティング
---
## 概要 {#overview}

OpenLineage 検証 DAG は、Airflow OpenLineage の構成を検証し、Data Observability: Jobs Monitoring がイベントを受信できない状況の原因である問題を報告します。

次の場合にこの DAG を実行します。

- セットアップ完了後、[Data Observability: Jobs Monitoring][1] ページに Airflow ジョブが表示されない場合。
- 本番ワークロードを実行する前に構成を検証する場合。

この DAG は以下をチェックします。

| チェック内容 | 検証内容 |
|---|---|
| パッケージのインストール | OpenLineage パッケージがインストールされており、インポート可能であること |
| パッケージのバージョン | インストールされているバージョンと PyPI で利用可能な最新バージョンの比較 |
| リスナーのアクセ可能性 | OpenLineage プラグインリスナーが Airflow によって読み込み可能であること |
| プロバイダーの有効状態 | OpenLineage プロバイダーが有効であり、明示的に無効化されていないこと |
| トランスポートの構成 | 有効なトランスポート (HTTP または Datadog) が構成されていること |
| Datadog エンドポイント | トランスポート URL が Datadog インテイクエンドポイントを指していること |
| ネットワーク接続 | 構成されたバックエンド URL への TCP 接続が成功すること |
| 構成の競合 | 重複または競合するトランスポート構成や構成ファイル設定がないこと |

## OpenLineage 検証 DAG の実行 {#run-the-openlineage-validation-dag}

### 前提条件 {#prerequisites}

- Airflow DAG ディレクトリへのアクセス (ローカルパス、または Amazon MWAA の場合はクラウドストレージバケット)
- 実行中の Airflow スケジューラーとワーカー (Pod またはプロセス)

### DAG ファイルの追加 {#add-the-dag-file}

以下のコードを `openlineage_preflight_check_dag.py` として Airflow DAGs ディレクトリに保存します。Amazon MWAA の場合は、ご使用の環境向け構成された S3 DAGs フォルダーにファイルをアップロードします。
...
### DAG のトリガー{#trigger-the-dag}

Airflow スケジューラーが新しいファイルを取得したら、手動で DAG をトリガーします。

1. Airflow UI で [**DAGs**] に移動し、`openlineage_preflight_check_dag` を見つけます。
2. DAG が一時停止している場合は、オンに切り替えます。
3. [**Trigger DAG**] (DAG をトリガー) (再生アイコン) をクリックします。

### ログの表示{#view-the-logs}

1. Airflow UI で **[DAGs] > [openlineage_preflight_check_dag]** に移動します。
2. 最新の DAG 実行をクリックします。
3. **validate_setup** タスクをクリックします。
4. [**Logs**] (ログ) タブを選択します。

## 出力を理解する{#understand-the-output}

DAG は、開始時に出力される環境サマリーと、終了時に出力される検証サマリーという 2 つのセクションをログに記録します。

### 環境情報{#environment-information}

最初のセクションには、DAG が環境について検出した内容が表示されます。
...
検証サマリーは、タスクログの最後に表示されます。各行は、以下のいずれかの記号で始まります。

- `✓`: チェックが成功しました。
- `✗`: チェックが失敗しました (イベントが Datadog に送信されていません)。
- `!`: 警告 (構成の問題を示している可能性があります)。
- `-`: チェックがスキップされました (前提条件のチェックに失敗しました)。

以下の出力は、セットアップが正常であることを示しています。
...
### 失敗の解釈 {#interpreting-failures}

一般的な失敗を解決するには、以下の表を使用してください。

| ログメッセージ | 原因 | 解決策 |
|---|---|---|
| `✗ OpenLineage not installed properly` | OpenLineage パッケージが見つからないか、破損しています。残りのチェックはすべてスキップされます。| Airflow インストールに `apache-airflow-providers-openlineage` が含まれていることを確認します。Amazon MWAA については、[Amazon MWAA の OpenLineage プロバイダーをアップグレードする][3] を参照してください。|
| `✗ OpenLineage provider is turned off` | プロバイダーは、`airflow.cfg` 内の `AIRFLOW__OPENLINEAGE__DISABLED`、`OPENLINEAGE_DISABLED`、または `openlineage.disabled` によって明示的に無効化されています。| 設定を削除するか、`false` に設定します。|
| `✗ OpenLineage provider is not active (no transport configuration found)` | トランスポート構成が存在しない場合は、プロバイダーが明示的に無効化されていなくても、プロバイダーは自動的に無効になります。| [Airflow セットアップガイド][2] に従ってトランスポートを構成してください。|
| `✗ Failed to resolve active transport` | 解決された構成からトランスポートをインスタンス化できませんでした。| トランスポート構成が有効で、正しく記述されていることを確認してください。タスクログで根本的なエラーをチェックしてください。|
| `✗ Transport Type: Console (won't send events to Datadog)` | トランスポートは `console` に設定されています。| トランスポートを `datadog` または `http` に変更し、Datadog インテイク URL を指定してください。|
| `! Transport does not appear to point to a Datadog endpoint` | トランスポート URL が、既知の Datadog ドメインと一致しません。| URL が Datadog インテイクエンドポイントを指していることを確認してください。|
| `✗ Network connectivity check failed` | Airflow ワーカーが Datadog インテイクエンドポイントに到達できません。| ファイアウォールまたはネットワークポリシーをチェックし、URL とポート 443 にアクセスできることを確認してください。|
| `✗ OpenLineage listener is not accessible` | プロバイダープラグインをインポートできないか、リスナーが登録されていないか、または複数のリスナーが見つかりました。具体的な原因は、タスクログにこれよりも前の時点で記録されています。| パッケージが**スケジューラーとワーカーの両方の** Pod またはプロセスにインストールされていることを確認してください。プラグインは読み込まれているがリスナーが有効でない場合は、プロバイダーのバージョン互換性を確認してください。|
| `! Configuration conflicts detected` | 複数のトランスポートまたは構成ファイルのソースが有効になっています。| 重複する構成を削除し、信頼できるソースを 1 つにしてください。|

**注**: `✗ OpenLineage listener is not accessible` がパッケージのインストール失敗と同時に表示される場合、そのパッケージはスケジューラーにのみインストールされており、ワーカーにはインストールされていない可能性があります。OpenLineage では、両方にプロバイダーが必要です。

### バージョンの警告{#version-warnings}

DAG が以下のようなバージョン警告を報告する場合:
...
バグ修正および Datadog との互換性向上のため、最新バージョンへのアップグレードをご検討ください。Amazon MWAA については、[Amazon MWAA の OpenLineage プロバイダーをアップグレードする][3] でバージョン固有の手順を参照してください。
...
[2]: /ja/data_observability/jobs_monitoring/airflow/
[3]: /ja/data_observability/jobs_monitoring/airflow_mwaa_upgrade/