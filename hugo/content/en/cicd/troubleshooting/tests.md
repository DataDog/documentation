...
If RUM sessions and test results are not linked, check the following causes:

1. The RUM sessions generated by the tests might be sampled out. See [Best Practices for RUM Sampling][24].
2. The RUM session might not flush before the wait period expires. Set the `DD_CIVISIBILITY_RUM_FLUSH_WAIT_MILLIS` environment variable to a value higher than the default of 500 milliseconds.
...
[24]: /real_user_monitoring/guide/best-practices-for-rum-sampling/
