---
title: Incident Response
further_reading:
- link: "https://app.datadoghq.com/release-notes?category=Incident%20Response"
  tag: "Release Notes"
  text: "Check out the latest Datadog Incident Response releases! (App login required)."
cascade:
    algolia:
        rank: 70
---

## Components

{{< whatsnext desc="Explore Incident Response Components" >}}
    {{< nextlink href="/incident_response/incident_management/" >}}<u>{{< prodname >}}Incident Management{{< /prodname >}}</u> - Learn how to manage and resolve incidents efficiently.{{< /nextlink >}}
    {{< nextlink href="/incident_response/case_management/" >}}<u>{{< prodname >}}Case Management{{< /prodname >}}</u> - Track and organize cases related to incidents.{{< /nextlink >}}
    {{< nextlink href="/incident_response/status_pages/" >}}<u>Status Pages</u> - Communicate real-time system status with internal or public pages.{{< /nextlink >}}
    {{< nextlink href="/incident_response/on-call/" >}}<u>On Call</u> - Manage on-call schedules and responder rotations.{{< /nextlink >}}
{{< /whatsnext >}}



## Further Reading

{{< partial name="whats-next/whats-next.html" >}}
