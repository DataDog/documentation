...
See [Component metrics][4] and [Source buffer metrics][5] for more information on metrics emitted by all sources. Since you use the HTTP Server source to send logs from Akamai DataStream to Observability pipelines, use the `component_type:http_server` tag to filter the relevant metrics.
