...
**Note**: The Datadog Lambda Extension sends logs tagged with `ddsource` and `ddtags`, not `source` and `ddtags`. When you define processor queries or filters for these logs, use `ddsource` and `ddtags`.
