...
- link: "https://www.datadoghq.com/blog/log-based-metrics/"
  tag: "Blog"
  text: "Generate metrics from your logs to view historical trends and track SLOs"
products:
- name: Logs
  icon: logs
...
<div class="alert alert-warning">The Generate Metrics processor uses the <code>timestamp</code> field on an event to set the metric's timestamp. If the event <code>timestamp</code> field is a string value, the event's processing time is used instead. See <a href="#convert-string-timestamp-to-timestamp-format">Convert string timestamp to timestamp format</a> for more information.</div>

1. Enter a filter query. See [Logs Search Syntax][5] or [APM Query Syntax][13] for more information.
   - Only events matching the filter are processed.
...
1. In the {{< ui >}}Define parameters{{< /ui >}} section, select the metric type (**Count**, **Gauge**, or **Distribution**). See the [Count metric example](#count-metric-example) and [Distribution metric example](#distribution-metric-example). Also see [Metrics Types](#metrics-types) for more information.
    - For the **Gauge** metric type, select an event field with a numeric (or parseable numeric string) value that is used for the value of the generated metric.
    - For the **Distribution** metric type, select an event field with a numeric (or parseable numeric string) value that is used for the value of the generated metric. The event field's value can be an array of (parseable) numerics, which is used for the generated metric's sample set.
...
You can generate these types of metrics for your logs or spans. See the [Metrics types][3] and [Distributions][4] documentation for more details.

| Metric type  | Description                                                                                                                                         | Example                                                                                       |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| COUNT        | The total number of event occurrences in one time interval. Can be reset to zero, but cannot be decreased.                                          | You want to count the number of events with `status:error`.                                     |
...
The Generate Metrics processor can only use the event `timestamp` field to set the metric timestamp if the event field is a timestamp type. If the `timestamp` field is a string, the time when the event is processed is used instead. To use the event `timestamp`, you must convert the string to a timestamp type before sending events to the Generate Metrics processor.
...
[13]: /tracing/trace_explorer/query_syntax/
