...
Datadog bills logs separately from CI Visibility. Configure log retention, exclusion filters, and indexes in [{{< prodname >}}Log Management{{< /prodname >}}][21]. To scope these rules to Buildkite logs, filter on the `datadog.product:cipipeline` and `source:buildkite` tags.
