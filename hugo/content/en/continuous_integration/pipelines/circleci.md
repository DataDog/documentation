...
1. For each project, go to {{< ui >}}Project Settings{{< /ui >}} > {{< ui >}}Webhooks{{< /ui >}} in CircleCI and add a new webhook:

   * {{< ui >}}Webhook Name{{< /ui >}}: `Datadog CI Visibility` or any other identifier name that you want to provide.
   * {{< ui >}}Receiver URL{{< /ui >}}: <code>https://webhook-intake.{{< region-param key="dd_site" >}}/api/v2/webhook/?dd-api-key=<API_KEY></code> where `<API_KEY>` is your [Datadog API key][3].
   * {{< ui >}}Certificate verifications{{< /ui >}}: Enable this check.
   * {{< ui >}}Events{{< /ui >}}: Select {{< ui >}}Workflow Completed{{< /ui >}} and {{< ui >}}Job Completed{{< /ui >}}.

2. Click {{< ui >}}Add Webhook{{< /ui >}} to save the new webhook.

## Advanced configuration

### Configure multiple projects in bulk

Datadog offers a [script][9] to help you enable service hooks across multiple or all of your CircleCI projects using the CircleCI API. The script requires Python 3 and the `requests` package.
...
1. Log in to your CircleCI account and follow all the projects you want to enable the hooks. Optionally, use the {{< ui >}}Follow All{{< /ui >}} button on the Projects page.

2. Run the script by using environment variables `DD_API_KEY` and `DD_SITE`, or passing in flag parameters `--dd-api-key` and `--dd-site`:
...
To set custom tags to all the pipeline and job spans generated by the integration, add to the {{< ui >}}Receiver URL{{< /ui >}} a URL-encoded query parameter `tags` with `key:value` pairs separated by commas.

If a `key:value` pair contains any commas, surround it with quotes. For example, to add `key1:value1,"key2: value with , comma",key3:value3`, the following string would need to be appended to the {{< ui >}}Receiver URL{{< /ui >}}: `?tags=key1%3Avalue1%2C%22key2%3A+value+with+%2C+comma%22%2Ckey3%3Avalue3`.
