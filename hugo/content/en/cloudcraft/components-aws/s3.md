...
- {{< ui >}}Volume type{{< /ui >}}: The type of volume used for the S3 bucket.
- {{< ui >}}Storage{{< /ui >}}: The amount of storage used in the bucket in gigabytes.
- {{< ui >}}PUT/COPY/POST{{< /ui >}}: The number of monthly `PUT`, `COPY`, and `POST` requests to the S3 bucket, in the thousands.
- {{< ui >}}GET and others{{< /ui >}}: The number of monthly `GET`, `SELECT`, and other requests to the S3 bucket, in the thousands.
- {{< ui >}}Lifecycle Transition{{< /ui >}}: The number of monthly lifetime transition requests to the S3 bucket, in the thousands. Only available for S3 buckets with the Infrequent Access volume type.
