...
- **Datadog SSO**: Datadog SSO does not require any additional setup for new accounts. Select {{< ui >}}Sign in with Datadog{{< /ui >}} on Cloudcraft's signup or login pages. [Contact the Cloudcraft support team][1] to enable this feature for existing
  accounts.
- **Google Workspace SSO**: Google SSO does not require any additional setup. Select {{< ui >}}Sign in with Google{{< /ui >}} on Cloudcraft's signup or login pages.
...
1. Go to {{< ui >}}User{{< /ui >}} > {{< ui >}}Security & SSO{{< /ui >}}.
...
5. Select the {{< ui >}}SAML Single Sign-On is enabled{{< /ui >}} option.
6. Enable the {{< ui >}}Strict mode{{< /ui >}} option if you need to restrict access to Cloudcraft to only SAML SSO users.
...
The option to change the default team that users join when they sign in for the first time can be found at the bottom of the {{< ui >}}Security & Single Sign-On{{< /ui >}} page.
