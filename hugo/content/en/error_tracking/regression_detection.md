...
If a {{< ui >}}RESOLVED{{< /ui >}} error recurs in a newer version of the code, or the error occurs again in code without versions, {{< prodname >}}Error Tracking{{< /prodname >}} triggers a regression. The issue moves to the {{< ui >}}FOR REVIEW{{< /ui >}} state, and is tagged with a {{< ui >}}Regression{{< /ui >}} tag:

{{< img src="error_tracking/regression-detection.png" alt="The details of regression in Error Tracking" style="width:90%;" >}}

Regressions take into account the versions of your service where the error is known to occur, and only trigger on new versions after an issue is marked as {{< ui >}}RESOLVED{{< /ui >}}. Configure your services with version tags (see instructions for [APM][1], [RUM][2], and [Logs][3]) to ensure that issues automatically resolve only if the same errors occur on newer versions of your services.

If you don't have version tags set up, issues are tagged with {{< ui >}}Regression{{< /ui >}} when an error occurs on an issue marked as {{< ui >}}RESOLVED{{< /ui >}}.
