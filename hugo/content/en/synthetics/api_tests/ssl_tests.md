...
- **Build a test from scratch**:
   ...
   1. To build a test from scratch, click the {{< ui >}}+ Start from scratch{{< /ui >}} template, then select the SSL request type.
   1. Specify the {{< ui >}}Host{{< /ui >}} and the {{< ui >}}Port{{< /ui >}} to run your test on. The default SSL port is `443`.
   1. Add {{< ui >}}Advanced Options{{< /ui >}} (optional) to your test:
      * {{< ui >}}Accept self-signed certificates{{< /ui >}}: Bypasses any server error related to a self-signed certificate.
      * {{< ui >}}Fail on revoked certificate in stapled OCSP{{< /ui >}}: Fail the test if the certificate is labeled as revoked by the OCSP stapling.
      * {{< ui >}}Fail on incomplete certificate chain{{< /ui >}}: Disables AIA fetching, causing the test to fail if any intermediate certificates are missing from the server's certificate chain.
      * {{< ui >}}Timeout{{< /ui >}}: Specify the amount of time in seconds before the test times out.
      * {{< ui >}}Server Name{{< /ui >}}: Specifies on which server you want to initiate the TLS handshake, allowing the server to present one of multiple possible certificates on the same IP address and TCP port number. By default, the parameter is filled by the {{< ui >}}Host{{< /ui >}} value.
      * {{< ui >}}Client certificate{{< /ui >}}: Authenticate through mTLS by uploading your client certificate (`.crt`) and the associated private key (`.key`) in `PEM` format. 
   ...
   1. {{< ui >}}Name{{< /ui >}} your SSL test.

   1. Add Environment {{< ui >}}Tags{{< /ui >}} as well as any other tag to your SSL test. You can then use these tags to filter through your Synthetic tests on the [Synthetic Monitoring & Continuous Testing page][3].
   1. Click {{< ui >}}Test Certificate{{< /ui >}} to try out the request configuration. A response preview is displayed on the right side of your screen.<br /><br>
...
Assertions define what an expected test result is. After you click {{< ui >}}Test URL{{< /ui >}}, basic assertions on certificate validity, expiration data, TLS version, and `response time` are added based on the response that was obtained. You must define at least one assertion for your test to monitor.
...
Select the {{< ui >}}Locations{{< /ui >}} to run your SSL test from. SSL tests can run from both managed and [private locations][1] depending on your preference for monitoring certificates from outside or inside your network.
...
You can use the [global variables defined on the {{< ui >}}Settings{{< /ui >}} page][9] in the URL, advanced options, and assertions of your SSL tests.
