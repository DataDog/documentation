...
Bits Testing and Goal-Based tests run only from the locations in [Run Bits Testing](#run-bits-testing). Browser, HTTP, and network path tests generated by Bits Testing don't have this restriction. After they're created, they support the same locations as any other Synthetic test, including [private locations][5].
