...
1. In Datadog, hover over {{< ui >}}Digital Experience{{< /ui >}} in the left-hand menu and select {{< ui >}}Tests{{< /ui >}} (under Synthetic Monitoring & Testing).
2. Click {{< ui >}}New Test{{< /ui >}} > {{< ui >}}Network Path Test{{< /ui >}}.
...
3. Optional: Configure advanced options:  
   ...
   2. {{< ui >}}Destination service{{< /ui >}}: The label displayed for the destination host in the Network Path visualization.  
   3. {{< ui >}}Max{{< /ui >}} {{< tooltip text="TTL" tooltip="Time-to-live: The maximum number of network hops a packet can traverse before being discarded" >}}: Maximum time-to-live (maximum number of hops) for outgoing probe packets. Defaults to 30 hops.  
...
On the {{< ui >}}Test Runs{{< /ui >}} tab, click a test run to open a side panel with details for that specific run. The side panel displays:
