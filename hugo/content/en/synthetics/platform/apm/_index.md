...
{{< img src="synthetics/apm/synthetics_apm_new.mp4" alt="A failed API test with the side panel open on the Trace tab, showing the APM trace generated by the test run with color-coded spans across services" video="true" >}}
...
To link browser test results, allow the URLs you want the APM integration headers added to. You can do this in [{{< ui >}}Synthetic Settings{{< /ui >}}][5]. 
Use `*` for wildcards. When used in the domain, `*` matches a single subdomain label and does not match across dots. In the URL path, `*` matches any characters, including `/`.

With the entry `https://*.datadoghq.com/*`:
| **URL**                             | **Allowed** | **Reason**                                                                                                               |
|----------------------------------------|-------------------------------------------------------------------------------------------------------------------------|
| https://www.datadoghq.com/product                  | Yes  | `*` matches the single label `www`                |
| https://www.datadoghq.com/product/synthetic-monitoring                  | Yes  | The URL path wildcard matches across `\`                |
| https://www.docs.datadoghq.com/synthetics                  | No  | `*` cannot match `www.docs`, which contains a dot                |

To allow a deeper subdomain, add an entry at that depth, such as `https://*.docs.datadoghq.com/*` or `https://*.*.datadoghq.com/*`. Each entry matches only its own depth.
...
These traces are retained for 15 days with the {{< ui >}}Synthetics Default{{< /ui >}} retention filter, [just like your classical APM traces][20].
