...
Use `*` for wildcards. In the domain, `*` matches a single subdomain label and does not match across dots. In the URL path, `*` matches any characters, including `/`.

For example, if you add `https://*.datadoghq.com/*`:
| URL | Allowed | Reason |
| --- | --- | --- |
