...
<div class="alert alert-info">Raw query statements may contain sensitive information (for example, passwords in query text) or personally identifiable information. Enabling this option allows Datadog to collect and ingest raw query statements that appear in query samples. This option is disabled by default.</div>
...
<div class="alert alert-caution">Setting thresholds too low can result in excessive event collection that affects server performance, event loss due to buffer overflow, and incomplete data, as Datadog only collects the most recent 1000 events per collection interval.</div>
