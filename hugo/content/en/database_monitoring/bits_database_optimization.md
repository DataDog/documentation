...
Bits Database Optimization is only available for PostgreSQL and MySQL. To request support for another database management system, contact your Datadog representative or [Datadog Support](/help/).
...
- **Database Monitoring** is configured for the target database instances. See [Database Monitoring Setup](/database_monitoring/architecture/).
- **Schema collection** is enabled on the target instances.
- For automated PR creation:
  - **APM** must be configured for the services that issue the queries you want to address. See [Correlate Database Monitoring and Traces](/database_monitoring/connect_dbm_and_apm/) for more information.
