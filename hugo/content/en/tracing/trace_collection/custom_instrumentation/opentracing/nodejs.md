...
<div class="alert alert-info">OpenTracing support is based on a deprecated specification. If you want to instrument your code with an open spec, use OpenTelemetry instead. Try <a href="/tracing/trace_collection/otel_instrumentation/">processing data from OpenTelemetry instrumentation in Datadog SDKs</a>.</div>
...
Use the SDK like in any other OpenTracing application.

The following tags are available to override Datadog specific options:

* `service.name`: The service name to be used for the span. The service name from the SDK will be used if this is not provided.
