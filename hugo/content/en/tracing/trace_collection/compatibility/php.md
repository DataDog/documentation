...
{{< prodname >}}Datadog APM{{< /prodname >}} for PHP is built upon dependencies defined in specific versions of the host operating system, PHP runtime,
certain PHP libraries, and the Datadog Agent or API.
When these versions are no longer supported by their maintainers, {{< prodname >}}Datadog APM{{< /prodname >}} for PHP limits its support for these as well.
