...
4. Copy and run the Agent installation command on your Linux host or VM.
   ...
   The command generated by the UI has the following form:
   ...
   Replace `<YOUR_DD_API_KEY>` with your [Datadog API key][22]. This command installs or updates the Agent and the SSI packages. To pin specific SDK versions instead of installing the latest, add the `DD_APM_INSTRUMENTATION_LIBRARIES` variable with comma-separated `language:major` pairs (for example, `DD_APM_INSTRUMENTATION_LIBRARIES=java:1,js:5,python:3,dotnet:3,ruby:2,php:1`). See step 3 above for available versions.
