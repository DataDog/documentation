...
Use the following commands to enable SSI. In each command, replace `<YOUR_DD_API_KEY>` with your [Datadog API key][40] and `<YOUR_CLUSTER_NAME>` with your cluster name. Replace `<DATADOG_SITE>` with your [Datadog site][43]: {{< region-param key="dd_site" code="true" >}}. SSI is enabled by `apm.instrumentation.enabled: true` in the configuration file.
