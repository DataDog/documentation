...
- Your account must have the [permissions][2] required to use Live Debugger.
- The selected service and environment for the Bits debug session are set to [Targeted redaction mode][8]. Manually created Live Debugger sessions work in both Strict and Targeted redaction modes.
...
**Sensitive data**: Bits only initiates a debug session on a service and environment with Targeted redaction mode (not Strict mode). Logs generated by Bits debug sessions apply Targeted redaction, which still [redacts sensitive data][9]. Changing a redaction mode requires the **Live Debugger Redaction Write** permission.
...
[8]: /tracing/live_debugger/sensitive-data-scrubbing/#mode-based-redaction
[9]: /tracing/live_debugger/#protecting-sensitive-data
