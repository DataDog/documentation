...
<div class="alert alert-info">To calculate APM metrics from unsampled OpenTelemetry data, place the <a href="/opentelemetry/setup/collector_exporter/#span-metrics-connector"><code>span_metrics</code> connector</a> before any sampling processor. The Datadog Connector achieves the same result in existing configurations. For more information, see <a href="/opentelemetry/ingestion_sampling/">Ingestion Sampling with OpenTelemetry</a>.</div>
