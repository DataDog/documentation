...
<div class="alert alert-danger">The full-text search feature is only available in {{< prodname >}}Log Management{{< /prodname >}} and works in monitor, dashboard, and notebook queries. The full-text search syntax cannot be used to define index filters, archive filters, log pipeline filters, rehydration filters, or in Live Tail. </div>
