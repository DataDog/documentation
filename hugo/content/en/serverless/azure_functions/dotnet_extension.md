...
5. Navigate to {{< ui >}}Extensions{{< /ui >}} and add the {{< prodname >}}Datadog APM{{< /prodname >}} extension for your runtime.
...
An example of how to set up logging in a .NET application using ILogger:
...
public class HttpTrigger
{
    ...
    {
        _logger.LogInformation("Processing request for {FunctionName}", "HttpTrigger");
...
With [Agentless logging][6] enabled, the .NET tracer submits these logs directly to Datadog without any required storage configuration or other pipelines. Set `DD_LOGS_INJECTION=true` to include trace correlation.
