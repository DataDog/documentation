...
Instrumentation is based on an **instrumentation rule**: an AWS account paired with a query that describes which resources to cover. Datadog evaluates the query against your account, instruments each covered resource inside your own account, and keeps them instrumented:
...
Use instrumentation through the AWS integration to instrument both EC2 instances and Lambda functions from one place, or when you want no additional compute running in your account.
