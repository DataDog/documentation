...
- An OpenTelemetry-compatible backend configured to send data to Datadog. See [Send OpenTelemetry Data to Datadog][4] for setup options including the DDOT Collector, the OpenTelemetry Collector, or direct OTLP ingest.
...
- **Local collector** (DDOT or OpenTelemetry Collector): `http://localhost:4318` (default HTTP) or `http://localhost:4317` (gRPC)
