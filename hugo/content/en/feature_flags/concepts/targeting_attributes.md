...
| **Semver** | is, is one of, is not one of, is null, semver equals, semver not equals, semver less than, semver less than or equal to, semver greater than, semver greater than or equal to. Semver comparisons support Semantic Versioning 2.0.0 values. Although the Semantic Versioning specification requires three numeric components (for example, `1.2.3`), Datadog also accepts values with one or more numeric components. Missing components compare as zero (`18`, `18.0`, and `18.0.0` are equivalent), while additional nonzero components affect ordering (`18.0.0.1` is greater than `18.0.0`). Standard prerelease precedence applies, and build metadata does not affect comparisons. |
