...
aliases:
  ...
  - /internal_developer_portal/software_catalog/set_up/discover_entities
further_reading:
...
Datadog Catalog is pre-populated with entries detected through [APM][1], eBPF-based autodiscovery with [Universal Service Monitoring][2], and RUM applications.

All automatically detected entities appear in the Component Selector within Catalog.
...
To adjust how inferred entities appear in Catalog, you can:
...
To ensure proper auto-correlation between APM and Catalog, follow these best practices:
...
- Avoid arbitrary [service overrides][16] and use [inferred entities][15] instead. 
  - Arbitrary service overrides can make correlation unreliable and clutter your catalog. They also make Catalog hard to use for those unfamiliar with the override names.
...
You can import services from other Datadog telemetry containing the `DD_SERVICE` [tag][10] to populate the Catalog. To discover `kind:service` entities through Datadog infrastructure metrics or logs, navigate to the Catalog [**Import Entries** tab][11]. 
...
{{< img src="tracing/software_catalog/clear_imported_services.png" alt="Confirm the deletion of previously imported services in the Catalog setup and configuration section" style="width:90%;" >}}
