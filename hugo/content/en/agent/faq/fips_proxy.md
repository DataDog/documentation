...
<div class="alert alert-caution">FIPS compliance is not retained if the Datadog Agent FIPS Proxy and the Datadog Agent are not on the same host.
...
```json
 {
   "containerDefinitions": [
     (...)
          {
            "name": "fips-proxy",
            "image": "datadog/fips-proxy:1.1.30",
            "portMappings": [
                {
                    "containerPort": 9803,
                    "protocol": "tcp"
