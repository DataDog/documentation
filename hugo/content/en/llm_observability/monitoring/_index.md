...
{{< img src="llm_observability/index/llm_query_example_light.png" alt="{{< prodname >}}Agent Observability{{< /prodname >}} > Traces view, where the user has entered the query `ml_app:shopist-chat-v2 'purchase' -'discount' @trace.total_tokens:>=20` and various traces are displayed." style="width:100%">}}

Learn how to use {{< prodname >}}Agent Observability{{< /prodname >}} query interface to search, filter, and analyze traces and spans generated by your LLM applications. The [Querying documentation][1] covers how to:
...
For applications instrumented with {{< prodname >}}Datadog APM{{< /prodname >}}, you can [correlate APM and Agent Observability][2] through the SDK. Correlating APM with {{< prodname >}}Agent Observability{{< /prodname >}} full end-to-end visibility and thorough analysis, from app issues to LLM-specific root causes.
