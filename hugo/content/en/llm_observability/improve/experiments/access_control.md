...
- **Spans from an experiment run through the SDK carry your application's `ml_app`, not the project's ID.** A policy on the project does not match those spans, so it hides the project, its datasets, its dataset records, and its evaluation metrics, but not the inputs and outputs recorded on its spans. To cover those as well, add a second filter for your application's own `ml_app` value to the same policy.
