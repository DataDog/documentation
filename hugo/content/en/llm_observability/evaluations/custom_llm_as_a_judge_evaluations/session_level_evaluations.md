...
1. Navigate to the {{< prodname >}}Agent Observability{{< /prodname >}} [Evaluations page][1] and select {{< ui >}}Create Evaluation{{< /ui >}}, then in the `Evaluate On` select {{< ui >}}Session{{< /ui >}}. (You can also start from a [template evaluation][2].)
...
After a session completes, its evaluation result is attached to the session and is available across {{< prodname >}}Agent Observability{{< /prodname >}} in near-real-time. While the session is still within its 30-minute inactivity window, the result shows up as {{< ui >}}Pending{{< /ui >}} in the side panel; after the session completes, the pending row is replaced by the final result.
