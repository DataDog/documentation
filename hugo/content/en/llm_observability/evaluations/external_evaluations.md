...
While Agent Observability provides a few out-of-the-box evaluations for your traces, you can submit your own evaluations to {{< prodname >}}Agent Observability{{< /prodname >}} in two ways: with Datadog's [SDK](#submitting-evaluations-with-the-sdk), or with the [Agent Observability API](#submitting-evaluations-with-the-api). Use this naming convention for the evaluation label:
...
The Agent Observability SDK provides the methods `LLMObs.submit_evaluation()` and `LLMObs.export_span()` to help your traced LLM application submit external evaluations to {{< prodname >}}Agent Observability{{< /prodname >}}. See the [Python][3] or [Node.js][4] SDK documentation for more details.
...
You can use the evaluations API provided by {{< prodname >}}Agent Observability{{< /prodname >}} to send evaluations associated with spans, traces, or sessions to Datadog. See the [Evaluations API][2] for more details on the API specifications. For building reusable evaluators, see the [Evaluation Developer Guide][5].
