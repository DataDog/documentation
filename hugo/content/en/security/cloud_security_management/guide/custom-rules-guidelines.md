...
{{< prodname >}}Workload Protection{{< /prodname >}} uses the concept of approvers and discarders to filter out events that should not trigger any rules in a policy. Approvers and discarders allow or deny events at the policy level only. They do not act on individual rules.
