...
[{{< prodname >}}App and API Protection{{< /prodname >}}][1] matches known patterns for sensitive data in API requests and responses. If it finds a match, the endpoint is tagged with the category and type of sensitive data processed and displayed in [API Endpoints][2].
...
By default, {{< prodname >}}App and API Protection{{< /prodname >}} scans for PII, credentials, and payment types. Sensitive Data Detection provides API data scanners to define custom scanner data patterns beyond the defaults. These scanners improve visibility into the sensitive data of your API traffic.
...
6. To view the results of the scanner, go to {{< prodname >}}App and API Protection{{< /prodname >}} [API Endpoints][4].
