...
Within Workload Protection, agent rules are written with a custom query language called SecL. SecL allows precise definitions of which events should be observed, matched, and forwarded to the Datadog backend based on runtime context. You can find more in the dedicated [SecL Guide](/security/workload_protection/detect_and_monitor/agent_rules/secl_guide) page.
