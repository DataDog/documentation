...
description: Create and manage the backend rules that analyze Agent events and generate Workload Protection security signals.
disable_toc: false
aliases:
...
To create a custom detection rule, go to the Workload Protection [detection rules][4] page and click {{< ui >}}New Rule{{< /ui >}}. You can also use the {{< ui >}}Assisted rule creator{{< /ui >}} to configure both the Agent rule and the detection rule in a single flow. See [Create the custom Agent and detection rules together](#create-the-custom-agent-and-detection-rules-together).
...
- {{< ui >}}Threshold{{< /ui >}}: Define a time window and the number of matching events required to trigger a signal. For example, trigger when more than 5 matching events occur within 5 minutes.
- {{< ui >}}New value{{< /ui >}}: Trigger when a tracked attribute appears with a value that has not been observed before.
- {{< ui >}}Anomaly{{< /ui >}}: Trigger when event volume or behavior deviates from the expected baseline.
- {{< ui >}}Content anomaly{{< /ui >}}: Trigger when the content of matching events is statistically unusual compared to historical data.
...
- {{< ui >}}Critical{{< /ui >}} when more than 10 matching events occur within 5 minutes.
- {{< ui >}}High{{< /ui >}} when more than 5 matching events occur within 5 minutes.
- {{< ui >}}Medium{{< /ui >}} when more than 2 matching events occur within 5 minutes.

In the {{< ui >}}Add notify{{< /ui >}} section, optionally configure who receives a notification when the rule triggers. You can add individual recipients or rely on [notification rules][7] to manage alerting across multiple detection rules.
...
1. Enter a {{< ui >}}Rule name{{< /ui >}}. The name appears in the detection rules list and becomes the title of the generated security signal.
2. In the {{< ui >}}Rule message{{< /ui >}} section, use [notification variables][8] and Markdown to describe what happened and how responders should act. Template variables inject dynamic context from the triggering Agent events directly into the signal and its notifications.
3. Use the {{< ui >}}Tag resulting signals{{< /ui >}} dropdown menu to add tags to generated signals. For example, `security:attack` or `technique:T1059-command-and-scripting-interpreter`.
...
- {{< ui >}}Assisted rule creator{{< /ui >}}: In Datadog, start a custom Workload Protection [detection rule][4] and use the wizard to configure both the Agent expression and the backend detection rule logic.
- {{< ui >}}Manual rule creator{{< /ui >}}: From [Agent Configuration][12], open or create a policy and choose {{< ui >}}Manual rule creator{{< /ui >}} to author the Agent rule first, then add a detection rule that references it. For UI steps and deployment, see [Policy management][11].
