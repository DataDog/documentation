...
<div class="alert alert-info">This MCP server is separate from the <a href="/mcp_server">{{< prodname >}}Datadog MCP Server{{< /prodname >}}</a>, which provides cloud-based access to Datadog features and data. The Code Security MCP Server runs locally and focuses on code-level security scanning.</div>
