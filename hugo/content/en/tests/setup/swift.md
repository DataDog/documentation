...
<div class="alert alert-caution">This framework is useful only for testing and should only be linked with the application when running tests. Do not distribute the framework to your users. </div>
...
<div class="alert alert-caution">You should have your main target in the variables expansion of the environment variables; if not selected, variables are not valid. </div>
...
<div class="alert alert-caution">If you disable crash reporting, tests that crash are not reported at all, and don't appear as test failures. If you need to disable crash handling for any of your tests, run them as a separate target, so you don't disable it for the others.</div>
