...
Pour les systèmes basés sur Linux où la <code>service</code> commande wrapper n'est pas disponible, <a href="/agent/faq/agent-v6-changes/?tab=linux#service-lifecycle-commands">consultez la liste des alternatives</a>.
</div>
...
| macOS      | `datadog-agent status` ou via l'[interface web][6]   |
| Source     | `sudo datadog-agent status`                          |
| Windows    | Consultez la [documentation de l'Agent Windows][4].            |
| [Cluster Agent (Kubernetes)][5] | `datadog-cluster-agent status`       |

Une intégration correctement configurée est affichée sous **Running Checks** sans avertissements ni erreurs, comme indiqué ci-dessous :
...
Certaines options disposent de flags et d'options détaillés sous `--help`. Par exemple, utilisez l'aide avec la sous-commande `check` :
...
| `configcheck`     | Imprimer toutes les configurations chargées et résolues d'un Agent en cours d'exécution.              |
| `diagnose`        | Exécuter un diagnostic de connectivité sur votre système.                              |
| `flare`           | [Collecter un flare et l'envoyer à Datadog][8].                                |
| `health`          | Imprimer l'état actuel de l'Agent.                                             |
| `help`            | Aide sur n'importe quelle commande.                                                     |
| `hostname`        | Imprimer le nom d'hôte utilisé par l'Agent.                                       |
| `import`          | Importer et convertir des fichiers de configuration des versions précédentes de l'Agent. |
| `jmx`             | Dépannage JMX.                                                        |
| `launch-gui`      | Démarrer l'interface graphique de l'Agent Datadog.                                                |
| `restart-service` | Redémarrer l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.         |
| `start-service`   | Démarrer l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.           |
| `stream-logs`     | Diffuser les journaux traités par un Agent en cours d'exécution.                         |
| `stopservice`     | Arrêter l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.            |
