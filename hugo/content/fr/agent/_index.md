...
<p>
La mise à niveau vers une version majeure de l'Agent Datadog et le maintien à jour est le seul moyen pris en charge pour obtenir les dernières fonctionnalités et corrections de l'Agent.</p>
...
{{< img src="/agent/basic_agent_usage/basic_agent_2_july_25.png" alt="La vue d'Automatisation de Flotte qui vous permet de gérer de manière centralisée vos Agents Datadog en un seul endroit." style="width:100%;">}}
...
- **[Envoyer un signal de détresse pour obtenir de l'aide][14]** : Depuis l'onglet {{< ui >}}Support{{< /ui >}} d'un hôte, générez un signal et attachez-le à un cas de support existant ou nouveau sans avoir à utiliser la ligne de commande.
- **Auditer l'utilisation de la clé API** : Identifiez quels Agents utilisent une clé API spécifique et faites tourner les clés en toute sécurité.
...
Vous pouvez changer le port par défaut de l'interface graphique dans votre fichier de configuration `datadog.yaml`. Pour désactiver l'interface graphique, définissez la valeur du port à `-1`. Sur Linux, l'interface graphique est désactivée par défaut.
...
Modifiez le [fichier de configuration principal de l'Agent][10], `datadog.yaml`, pour définir le `site` paramètre (par défaut `datadoghq.com`).

```yaml
site: {{< region-param key="dd_site" >}}
```

**Remarque** : Consultez la [documentation de démarrage avec les sites Datadog][11] pour plus de détails sur le `site` paramètre.
...
* Version de l'agent Test : 7.34.0
...
{{< whatsnext desc="Cette section comprend les thèmes suivants :">}}
