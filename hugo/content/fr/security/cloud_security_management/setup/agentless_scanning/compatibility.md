...
| Chiffrement                                      | AWS </br> Non chiffré </br> Chiffré - Clé gérée par la plateforme (PMK) et clé gérée par le client (CMK)                             | Chiffré - Clé gérée par la plateforme (PMK) : Chiffrement côté serveur du stockage disque Azure, chiffrement sur le host </br> **Remarque** : Le chiffrement - Clé gérée par le client (CMK) n'est **pas** pris en charge | Chiffré - Clé gérée par la plateforme (PMK) : Chiffrement de disque persistant, VM confidentielle </br> **Remarque** : Le chiffrement - Clé de chiffrement gérée par le client (CMEK) et les clés de chiffrement fournies par le client (CSEK) ne sont **pas** pris en charge |
...
| Serverless                                      | AWS Lambda <br> AWS Fargate pour ECS                                                                                           | Azure Functions ([Aperçu][20]), Azure Container Apps, Azure Container Instances<br />**Remarque** : Nécessite le dernier scanner Agentless. Voir [Mettre à jour l'Agentless Scanning][17].                                     | Cloud Run ([Aperçu][21])                                                                                                                                           |
| Kubernetes                                      | EKS sur nœuds EC2 </br> EKS sur Fargate </br> **Remarque** : EKS sur Fargate nécessite l'installation du [Datadog Cluster Agent][18] | AKS sur VMs, Virtual Machine Scale Sets (VMSS) et Azure Container Instances (ACI) </br> **Remarque** : AKS sur ACI nécessite l'installation du [Datadog Cluster Agent][18] | GKE Standard et GKE Autopilot </br> **Remarque** : GKE Autopilot nécessite l'installation du [Datadog Cluster Agent][18]                                                              |
| Langages d'application (sur les hosts et dans les conteneurs) | Java, .Net, Python, Node.js, Go, Ruby, Rust, PHP, Swift, Dart, Elixir, Conan, Conda                                           | Java, .Net, Python, Node.js, Go, Ruby, Rust, PHP, Swift, Dart, Elixir, Conan, Conda                                                                                               | Java, .Net, Python, Node.js, Go, Ruby, Rust, PHP, Swift, Dart, Elixir, Conan, Conda                                                                                                                                       |
| Registres de Container                                      | Amazon ECR (public et privé) : Analyse les images de conteneurs en cours d'exécution et les 1 000 dernières images poussées au repos                      | ACR : Analyse uniquement les images de conteneurs en cours d'exécution<br />**Remarque :** L'analyse au repos du registre n'est pas prise en charge. Pour en faire la demande, contactez [Datadog Support][16]   | Google Artifact Registry : analyse uniquement les images des charges de travail en cours d'exécution<br />**Remarque** : La prise en charge de l'analyse au repos pour Google Artifact Registry est en [Aperçu][19]                                                                                                                                        |
| Images de host                                     | AMI                                                                                                                           | Non pris en charge                                                                                                                                                                     | Non pris en charge                                                                                                                                                                                                             |
| Données sensibles (SDS)                            | S3                                                                                                                            | Non pris en charge                                                                                                                                                                     | Non pris en charge                                                                                                                                                                                                             |

**Remarque** : Les AMI doivent être stockées dans un compte qui utilise l'intégration AWS de Datadog. Sinon, Datadog ne peut pas lire l'instantané Amazon Elastic Block Store (EBS) sous-jacent de l'AMI, et ne peut donc pas analyser l'AMI ni générer de rapport à son sujet.

## Distributions Linux {#linux-distributions}

Les distributions Linux suivantes sont prises en charge pour les analyses de hosts et de conteneurs :

| Système d'exploitation         | Versions prises en charge                                  | Gestionnaires de paquets | Avis de sécurité                                                                                       |
|--------------------------|-----------------------------------------------------|------------------|-----------------------------------------------------------------------------------------------------------|
...
## Bibliothèques d'application {#application-libraries}

Les langages d'application et les bibliothèques suivants sont pris en charge pour les analyses de vulnérabilité sur les images de conteneur, les fonctions Lambda et les conteneurs exécutés sur des hosts :

| Langue | Gestionnaire de paquets pris en charge | Fichiers pris en charge                                                      |
|----------|---------------------------|----------------------------------------------------------------------|
...
## Registres d'images de conteneur {#container-image-registries}

Les registres d'images de conteneur suivants sont pris en charge pour les analyses d'images de conteneur :

| Registre                        | Niveau de support | Notes                                                                                                                                                                                                                                         |
|---------------------------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Amazon ECR (public et privé) | GA            | Analyse les images de conteneurs en cours d'exécution **et** les 1 000 dernières images poussées au repos (par date). Il s'agit du seul registre prenant en charge l'analyse au repos<br />**Remarque :** Pour augmenter le nombre d'images au repos à analyser, contactez [Datadog Support][16] |
...
| Azure Container Registry (ACR)  | GA            | Analyse uniquement les images de conteneurs en cours d'exécution provenant d'Azure Container Apps et d'Azure Container Instances<br />**Remarque** : L'analyse de registre au repos n'est pas prise en charge. Pour en faire la demande, contactez [Datadog Support][16]                                                                |

**Remarque** : L'analyse d'images de conteneur à partir du registre n'est prise en charge que si vous avez installé Agentless avec :
  - CloudFormation Integrations >= v2.0.8
  - Module Terraform Agentless >= v0.11.7

## Runtimes de conteneurs{#container-runtimes}

Les runtimes de conteneurs suivants sont pris en charge :

- containerd: v1.5.6 ou version ultérieure
- Docker

**Remarque pour les observations de conteneur** : l'Agentless Scanning nécessite des couches d'image de conteneur non compressées. Comme solution de contournement, vous pouvez définir l'option de configuration `discard_unpacked_layers=false` dans le fichier de configuration de containerd.
