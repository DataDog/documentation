...
Datadog peut recueillir des métriques depuis l'Agent ou l'API, et ce quel que soit le langage que vous utilisez. Cette page présente des exemples d'utilisation de chaque méthode avec PowerShell.

## Envoyer des métriques avec PowerShell par l'intermédiaire de l'API

Cette méthode ne nécessite pas l'installation de l'Agent sur le système qui exécute le script PowerShell. Vous devez directement transmettre votre [clé d'API][1] ainsi qu'une clé d'application lors de l'envoi de votre requête POST.
...
postMetric($metric)($tags) # transmettre votre métrique en tant que paramètre ) postMetric()
```

## Envoyer des métriques avec PowerShell par l'intermédiaire de DogStatsD

L'utilisation de l'Agent vous permet de tirer parti de son service d'écoute, [DogStatsD][2]. L'exemple suivant illustre comment envoyer la même métrique que précédemment par l'intermédiaire de DogStatsD. Notez qu'avec cette méthode, vous n'avez pas besoin de spécifier votre clé d'API ou votre clé d'application : elles sont déjà définies dans votre fichier `datadog.yaml` local.
