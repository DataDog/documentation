---
aliases:
- /fr/account_management/saml/azure/
- /fr/account_management/faq/how-do-i-configure-azure-ad-as-a-saml-idp/
description: Configurez Microsoft Entra ID comme fournisseur d'identité SAML pour
  l'authentification unique Datadog.
further_reading:
...
Suivez le tutoriel [Intégration de l'authentification unique (SSO) Microsoft Entra avec Datadog][1] pour configurer Entra ID comme fournisseur d'identité SAML (IdP). **Remarque** : un abonnement Entra ID est requis. Si vous n'avez pas d'abonnement, inscrivez-vous pour obtenir un [compte gratuit][2].

### Datadog

1. Accédez à la [page SAML de Datadog][3].

2. Choisissez et téléversez le fichier **SAML XML Metadata** téléchargé depuis Microsoft.
...
4. Cliquez sur **Enable** pour commencer à utiliser l'authentification unique Entra ID avec SAML :
...
2. Dans Microsoft Entra ID, accédez à la section SSO Configuration de votre application, cochez la case **Show advanced URL settings** et ajoutez votre URL d'authentification unique.
