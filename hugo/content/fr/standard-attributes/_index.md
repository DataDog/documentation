...
- description: Le [nom de service unifié](/getting_started/tagging/unified_service_tagging/)
    ...
    Dans le SDK RUM Browser, un service désigne un ensemble de pages développé par
    une équipe offrant une fonctionnalité spécifique au sein de votre application
    web. Vous pouvez assigner des pages web à un service avec [le suivi de vue manuel](/real_user_monitoring/application_monitoring/browser/advanced_configuration/?tab=npm#override-default-rum-view-names).
  domain: Reserved
  name: service
  product_source:
  - icon-log
...
- description: Le statut d'activité de la session. La session se termine après 4 heures
  ...
  - browser
  type: boolean
- description: 'Le type de session : `utilisateur` ou `synthétiques`. Les sessions
    provenant des [Tests de Navigateur Synthétiques](/synthetics/browser_tests/) sont
...
- description: Le type de fournisseur de ressources (par exemple, `first-party`, `cdn`,
  ...
  - browser
  type: string
- description: Le type de signal de frustration détecté par le SDK RUM Browser (`rage_click`,
...
- description: Le nom du pays auquel l'adresse IP du client correspond.
  ...
  - icon-apm
  type: string
- description: Code [ISO](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes)
    du pays (par exemple, `US` pour les États-Unis, `FR` pour la France).
  domain: HTTP client IP details
...
- description: Le code [ISO](https://en.wikipedia.org/wiki/ISO_3166-2) de la subdivision
  ...
  - icon-apm
  type: string
- description: Nom de la ville à laquelle l'adresse IP du client correspond.
  domain: HTTP client IP details
