...
description: Découvrez comment Observability Pipelines vous permet de collecter, traiter
  et acheminer des logs, des métriques et des traces au sein de votre propre infrastructure
  vers des destinations telles que Datadog, Amazon S3, Splunk et Microsoft Sentinel.
disable_toc: false
further_reading:
...
Datadog Observability Pipelines vous permet de collecter et de traiter {{< tooltip text="logs, metrics, and traces" tooltip="Contactez votre responsable de compte pour discuter des cas d'utilisation et de la tarification." >}} au sein de votre propre infrastructure, puis d'acheminer les données vers différentes destinations. Il vous donne le contrôle sur vos données d'observabilité avant qu'elles ne quittent votre environnement.
...
| Modèle | Description |
|----------|-------------|
| Échantillonnage des traces | Ingérez, traitez et acheminez les traces pour contrôler les coûts tout en conservant les traces dont vous avez besoin pour le dépannage et l'analyse. |
