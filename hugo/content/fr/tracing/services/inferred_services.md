...
description: Découvrez automatiquement les dépendances de service, telles que les
...
Explorez les services déduits dans le [Catalogue][1] en filtrant les entrées par type d'entité, comme base de données, file d'attente ou API tierce. Chaque [page de service][2] est adaptée au type de service que vous étudiez. Par exemple, les pages de service de base de données affichent des informations spécifiques aux bases de données et incluent des données de surveillance de base de données si vous utilisez [Database Monitoring][3].

## Configurer les services déduits{#set-up-inferred-services}
{{< tabs >}}
{{% tab "Agent v7.60.0+" %}}
À partir de la version [7.60.0][1] du Datadog Agent, aucune configuration manuelle n'est nécessaire pour voir les services déduits. Les configurations requises — `apm_config.compute_stats_by_span_kind` et `apm_config.peer_tags_aggregation` — sont activées par défaut.
...
Pour les versions [7.55.1][1] à [7.59.1][2] du Datadog Agent, ajoutez ce qui suit à votre fichier de configuration `datadog.yaml` :
...
Vous pouvez aussi définir ces variables d'environnement dans la configuration de votre Datadog Agent :
...
Pour les versions [7.50.3][1] à [7.54.1][2] du Datadog Agent, ajoutez ce qui suit à votre fichier de configuration `datadog.yaml` :
...
## Nommage des entités déduites{#naming-inferred-entities}
...
### Tags pairs {#peer-tags}

Tag pair | Attributs sources
--------------------|-------------------
...
**Remarque** : Les valeurs d'attribut pair qui correspondent aux formats d'adresse IP sont modifiées et expurgées avec `blocked-ip-address` pour éviter le bruit inutile et le marquage de métriques avec des dimensions à haute cardinalité. Par conséquent, vous pouvez rencontrer certains services `blocked-ip-address` apparaissant comme des dépendances en aval de vos services instrumentés.

#### Priorité des tags pairs {#precedence-of-peer-tags}

Pour attribuer un nom aux entités inférées, Datadog utilise un ordre de priorité spécifique entre les tags pairs, notamment lorsque les entités sont définies par une combinaison de plusieurs tags. 
...
Si le tag de priorité la plus élevée, tel que `peer.db.name`, n'est pas capturé dans le cadre de l'instrumentation, Datadog utilise le tag de deuxième priorité la plus élevée, comme `peer.hostname`, et continue dans cet ordre.

**Remarque** : Datadog ne définit jamais le `peer.service` pour les bases de données et les files d'attente déduites. `peer.service` est l'attribut pair de priorité la plus élevée. S'il est défini, il prévaut sur tous les autres attributs.

## Migrer vers le nommage de service global par défaut {#migrate-to-global-default-service-naming}

Avec les services déduits, les dépendances de service sont automatiquement détectées à partir des attributs de span existants. Par conséquent, il n'est pas nécessaire de modifier les noms de service (à l'aide du tag `service`) pour identifier ces dépendances. 

Activez `DD_TRACE_REMOVE_INTEGRATION_SERVICE_NAMES_ENABLED` pour vous assurer qu'aucune intégration Datadog ne définit des noms de service différents du nom de service global par défaut. Cela améliore également la manière dont les connexions de service à service et les services déduits sont représentés dans les visualisations Datadog, dans tous les langages SDK et intégrations pris en charge.

<div class="alert alert-danger">L'activation de cette option peut avoir un impact sur les métriques APM existantes, les métriques de span personnalisées, l'analyse des traces, les filtres de rétention, les analyses de données sensibles, les monitors, les dashboards ou les notebooks qui font référence aux anciens noms de service. Mettez à jour ces ressources pour utiliser le tag de service global par défaut (<code>service:&lt;DD_SERVICE&gt;</code>).</div>
