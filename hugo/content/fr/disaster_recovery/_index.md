...
- link: https://www.datadoghq.com/blog/ddr-mitigates-cloud-provider-outages/
  tag: Blog
  text: Datadog Disaster Recovery atténue les pannes des fournisseurs cloud
...
DDR vous permet également d'exécuter des exercices périodiques de reprise après sinistre pour tester votre capacité à vous remettre des pannes et à répondre à vos besoins de conformité commerciale et réglementaire.

## Prérequis {#prerequisites}

La version minimale du Datadog Agent requise dépend des produits que vous utilisez :
...
Suivez ces étapes pour activer Datadog Disaster Recovery. Si vous avez des questions sur l'une de ces étapes, contactez votre [Customer Success Manager][14] ou le [support Datadog][15].
...
{{% collapse-content title="Créer et partager votre organisation DDR" level="h4" %}}

<div class="alert alert-info">Si vous le préférez, Datadog peut configurer cela pour vous.</div>
...
2. Choisissez un site Datadog différent de votre site principal (par exemple, si vous êtes sur `US1`, choisissez `EU` ou `US5`). Consultez la [liste des sites Datadog][17] pour connaître les options. Tous les sites Datadog sont séparés géographiquement.
...
<div class="alert alert-info">Pour des raisons de sécurité, Datadog n'est pas en mesure de lier les organisations pour vous.</div>

Une fois que Datadog a désigné votre organisation DDR, liez votre organisation DDR à votre organisation principale :

1. Utilisez l'endpoint [List your managed organizations][1] pour récupérer les identifiants publics de votre organisation principale et de votre organisation DDR.
1. Ajoutez le périmètre `disaster_recovery_status_write` à votre clé d'application dans l'organisation principale.
1. Exécutez les commandes suivantes en remplaçant les espaces réservés par les valeurs appropriées.
...
Une fois vos organisations liées, l'organisation de basculement affiche cette bannière :
...
**Datadog recommande d'utiliser l'authentification unique (SSO)** pour permettre à tous vos utilisateurs de se connecter à votre organisation DDR en cas de panne.

Accédez aux [Paramètres de l'organisation][2] dans votre organisation DDR pour configurer [SAML][3] ou {{< ui >}}Google Login{{< /ui >}} pour vos utilisateurs.

La synchronisation gérée réplique les comptes utilisateur de votre organisation principale vers votre organisation DDR. Datadog recommande de configurer [le provisionnement Just-in-Time avec SAML][4] afin que les utilisateurs puissent accéder à l'organisation DDR pendant un basculement sans avoir besoin de réinitialiser leur mot de passe.
...
Vos intégrations cloud doivent être configurées à la fois dans les organisations principale et DDR, mais elles ne s'exécutent que dans une seule organisation à la fois : par défaut dans l'organisation principale, et pendant le basculement dans l'organisation DDR.

Pour plus d'informations, consultez la section [Basculement des intégrations cloud](#id-for-cloud).
...
Datadog gère la synchronisation des ressources pour vous à l'aide de l'outil open source [datadog-sync-cli][8]. Vous n'avez pas besoin d'exécuter ou d'utiliser cet outil vous-même.

La synchronisation gérée réplique les ressources de votre organisation principale vers votre organisation DDR selon un planning régulier. Les ressources répliquées incluent les dashboards, les monitors, les utilisateurs, les notebooks et [34+ autres types de ressources][9]. La réplication s'exécute selon ce planning afin que votre organisation DDR reste à jour avant une panne.

**Les utilisateurs sont associés à chaque site Datadog.** La synchronisation gérée réplique les comptes d'utilisateurs vers votre organisation DDR. Cependant, les utilisateurs peuvent avoir besoin de réinitialiser leur mot de passe lors de leur première connexion à l'organisation DDR. Datadog recommande de configurer [le provisionnement Just-in-Time avec SAML][4] afin que les utilisateurs puissent accéder à l'organisation DDR sans réinitialisation manuelle du mot de passe.
...
Remote Configuration est activée par défaut pour les nouvelles organisations, y compris votre organisation DDR. Toutes les nouvelles clés d'API que vous créez sont compatibles RC pour une utilisation avec votre Agent. Pour plus de détails, consultez la [documentation sur Remote Configuration][11].

Datadog recommande vivement d'utiliser Remote Configuration pour un meilleur contrôle du basculement. Comme alternative à RC, vous pouvez configurer manuellement vos Agents ou utiliser des outils de gestion de configuration tels que Puppet, Ansible ou Chef.

{{% /collapse-content %}}

{{% collapse-content title="Envoyer la télémétrie en double vers l'organisation DDR pendant le basculement ou les exercices" level="h4" %}}

Pour activer l'envoi en double, Datadog recommande d'utiliser [Fleet Automation][12] pour une gestion à grande échelle. Alternativement, vous pouvez le configurer manuellement en modifiant votre `datadog.yaml`fichier.

Contactez votre Customer Success Manager Datadog pour planifier des fenêtres de temps dédiées aux tests de basculement afin de mesurer les performances et l'objectif de temps de récupération (RTO).

{{< tabs >}}
{{% tab "Utilisation de Fleet Automation (recommandée)" %}}

Accédez à [Fleet Automation][100] > {{< ui >}}Configure Agents{{< /ui >}} dans votre organisation DDR pour créer une politique de basculement ou en réutiliser une existante, et appliquez-la à votre parc d'agents. Peu après l'activation de la politique, les Agents commencent à envoyer la télémétrie en double vers les sites d'observabilité de l'organisation principale et DDR (basculement).
...
Ensuite, suivez l'invite pour définir le périmètre des hosts et de la télémétrie (métriques, logs, traces) que vous devez faire basculer.

{{< img src="/agent/guide/ddr/ddr-fa-policy-scope.png" alt="Définir le périmètre des hosts et de la télémétrie requis pour le basculement" style="width:80%;" >}}
...
Pendant un basculement ou des exercices de basculement, mettez à jour `datadog.yaml` le fichier de configuration de votre Datadog Agent comme indiqué dans l'exemple suivant, puis redémarrez l'Agent.

- `enabled: true` permet à l'Agent d'envoyer {{< tooltip text="metadata" tooltip="Données sur l'Agent et le host d'infrastructure. Par exemple, `host name`, `host tags`, `Agent version`." >}} vers le site Datadog DDR afin que vous puissiez voir les Agents et vos hosts d'infrastructure dans l'organisation DDR. Cela vous permet de voir vos Agents et vos hosts d'infrastructure dans l'organisation DDR.

- `failover_metrics`, `failover_logs` et `failover_apm` sont `false` par défaut. Leur définition sur `true` amène l'Agent à commencer à envoyer {{< tooltip text="telemetry" tooltip="Les données qui sont envoyées à la plateforme Datadog. Par exemple, `logs`, `metrics`, `traces`." >}} vers l'organisation DDR.
...
Le basculement basé sur le DNS est une approche complémentaire au basculement basé sur l'Agent. Au lieu de configurer les Agents avec un endpoint de site secondaire, vous configurez toutes vos sources de données pour envoyer la télémétrie vers une URL d'ingestion personnalisée unique fournie par Datadog. Lors d'un événement de basculement, Datadog met à jour l'enregistrement DNS pour cette URL afin de rediriger le trafic de votre site principal vers votre site DDR.

<div class="alert alert-info">Le basculement DNS est une solution tout ou rien. Toutes les sources de télémétrie utilisant votre endpoint personnalisé basculent simultanément.</div>

#### Recevez votre endpoint DNS personnalisé {#receive-your-custom-dns-endpoint}

Si vous choisissez d'utiliser le basculement basé sur le DNS, Datadog provisionne une URL d'ingestion personnalisée pour votre organisation (par exemple, `<your-org>.intake.datadoghq.com`). Configurez toutes vos sources de données (telles que les Agents, les expéditeurs de logs et l'instrumentation personnalisée) pour envoyer la télémétrie vers cet endpoint au lieu de l'URL d'ingestion Datadog par défaut. Il s'agit d'une modification de configuration ponctuelle.

#### Déclencher un basculement DNS {#trigger-a-dns-failover}

Contactez votre [Customer Success Manager][14] ou [Datadog Support][15] pour initier un basculement DNS. Datadog met à jour l'enregistrement DNS pour rediriger le trafic de votre site principal vers votre site DDR. L'objectif de temps de récupération (RTO) cible est de 2 heures à partir du moment où le basculement est lancé.
...
{{% collapse-content title="Activer et tester le basculement DDR dans les environnements basés sur l'Agent" level="h4" %}}

Pour déclencher un basculement de vos Agents, cliquez sur l'une des politiques dans [Fleet Automation][13] au sein de votre organisation DDR, puis cliquez sur {{< ui >}}Enable{{< /ui >}}. Le statut de chaque host se met à jour au fur et à mesure que le basculement se produit.

{{< img src="/agent/guide/ddr/ddr-fa-policy-enable3.png" alt="Activer la politique de basculement dans l'organisation DDR" style="width:80%;" >}}
...
Vous trouverez ci-dessous un exemple d'utilisation de `kubectl` pour basculer les métriques et les logs d'un pod Datadog Agent déployé avec le Helm chart officiel ou Datadog Operator. `<POD_NAME>` doit être remplacé par le nom du pod Agent :
...
Alternativement, vous pouvez spécifier les paramètres ci-dessous dans le fichier de configuration principal de l'Agent (`datadog.yaml`) et redémarrer le Datadog Agent pour que les modifications soient appliquées :
...
{{< img src="/agent/guide/ddr/ddr-failover-main-page.png" alt="Activer la politique de basculement dans l'organisation DDR" style="width:80%;" >}}
...
Lorsque vous n'êtes plus en basculement, **désactivez la politique de basculement** dans l'organisation DDR pour rétablir la collecte des données d'intégration vers l'organisation principale.

Pendant les tests, la télémétrie d'intégration est répartie entre les deux organisations. Si vous annulez un test de basculement, les intégrations reprennent leur exécution dans le centre de données principal.
