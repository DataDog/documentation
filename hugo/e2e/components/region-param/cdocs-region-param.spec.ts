...
test.describe('Cdocs region param component', () => {
    ...
    test('page renders as expected', async ({ page }) => {
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('region-param-initial.png');
    });
    ...
    test('selecting a different site updates region params', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('region-param-eu-selected.png');
