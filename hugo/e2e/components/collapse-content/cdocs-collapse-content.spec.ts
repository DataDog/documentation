...
test.describe('Cdocs collapse content component', () => {
    ...
    test('page renders as expected', async ({ page }) => {
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('collapse-content-initial.png');
    });
    ...
    test('each heading level renders the correct tag', async ({ page }) => {
        ...
        for (const [id, tag] of cases) {
            const heading = page.locator(`${section(id)} ${tag}.header-text-inner`);
            await expect(heading).toHaveCount(1);
        }
    });

    test('rich content section shows code block and alert when expanded', async ({ page }) => {
        ...
        await expect(page.locator(`${contentEl('collapse-rich')} .code-snippet`)).toBeVisible();
        await expect(page.locator(`${contentEl('collapse-rich')} .alert-info`)).toBeVisible();

        await expect(page.locator(section('collapse-rich'))).toHaveScreenshot('collapse-content-rich-expanded.png');
    });
