...
test.describe('Cdocs open stepper', () => {
    ...
    test('initial state: all steps are displayed', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-open-initial.png');
    });

    test('Collapse shows only the active step', async ({ page }) => {
        await page.locator(COLLAPSE_BTN).click();

        await expect(page.locator(STEPPER)).not.toHaveClass(/stepper--all-expanded/);
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-open-collapsed.png');
    });

    test('Next button advances the active step after collapsing', async ({ page }) => {
        // Collapse first
        await page.locator(COLLAPSE_BTN).click();
        await expectActiveStep(page, 0);
    ...
    test('Previous button rewinds the active step after collapsing', async ({ page }) => {
    ...
    test.describe('keyboard navigation', () => {
        test('keyboard collapse, Next, Previous, and Expand', async ({ page }) => {
            ...
            await expect(page.locator(ACTIVE_STEP).locator(NEXT_BTN)).toBeFocused();
            await page.keyboard.press('Enter');
            await expectActiveStep(page, 2);
