 ...
 */
export function parseMdocFile(p: {
  file: string;
  partialsDir: string;
}): Readonly<ParsedFile> {
 ...
 */
function extractPartialPaths(node: Node): string[] {
 ...
 */
function extractErrors(p: { node: Node }): CompilationError[] {
  let errors: CompilationError[] = [];
