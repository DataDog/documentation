...
(() => {
  ...
  var require_dist = __commonJS({
    "../markdoc-static-compiler/dist/index.js"(exports) {
        ...
        "src/grammar/tag.js"(exports2, module2) {
          ...
          function peg$parse(input, options) {
            ...
            var peg$f18 = function(prefix, head, tail) {
              if (prefix === "@")
                return [head, ...tail];
              return new Variable22([head, ...tail]);
            };
            var peg$f19 = function() {
              return null;
            };
            ...
            const { Variable: Variable22, Function: Function3 } = options;
            peg$begin();
            peg$result = peg$startRuleFunction();
            if (peg$result !== peg$FAILED && peg$currPos === input.length) {
              return peg$result;
      ...
      function resolve2(value, config = {}) {
        if (value == null || typeof value !== "object")
          return value;
        if (Array.isArray(value))
          return value.map((item2) => resolve2(item2, config));
        if (isAst2(value) && value?.resolve instanceof Function) {
          return value.resolve(config);
        }
        if (Object.getPrototypeOf(value) !== Object.prototype)
          return value;
      ...
      var import_tag2 = __toModule(require_tag2());
      var Variable2 = class {
        constructor(path = []) {
          this.$$mdtype = "Variable";
          this.path = path;
      ...
      var OPEN2 = "{%";
      var CLOSE2 = "%}";
      var IDENTIFIER_REGEX2 = /^[a-zA-Z0-9_-]+$/;
      function isIdentifier2(s2) {
        return typeof s2 === "string" && IDENTIFIER_REGEX2.test(s2);
      }
      function isPromise2(a) {
        return a && typeof a === "object" && typeof a.then === "function";
      }
      function findTagEnd2(content, start = 0) {
        let state = 0;
        for (let pos = start; pos < content.length; pos++) {
          const char = content[pos];
          switch (state) {
            ...
            case 0:
              if (char === '"')
                state = 1;
              else if (content.startsWith(CLOSE2, pos))
                return pos;
          }
        }
        return null;
      }
      function parseTag2(content, line, contentStart) {
        try {
          return (0, import_tag2.parse)(content, { Variable: Variable2, Function: Function22 });
        } catch (error2) {
      ...
      function parseTags2(content, firstLine = 0) {
        let line = firstLine + 1;
        const output = [];
        let start = 0;
        for (let pos = 0; pos < content.length; pos++) {
          ...
          if (!content.startsWith(OPEN2, pos))
            continue;
          const end = findTagEnd2(content, pos);
          if (end == null) {
            pos = pos + OPEN2.length;
            continue;
          }
          const text2 = content.slice(pos, end + CLOSE2.length);
          const inner = content.slice(pos + OPEN2.length, end);
          const lineStart = content.lastIndexOf("\n", pos);
          const lineEnd = content.indexOf("\n", end);
          const lineContent = content.slice(lineStart, lineEnd);
          const tag = parseTag2(inner.trim(), line, pos - lineStart);
      ...
      function buildTag2(name = "div", attributes = {}, children = []) {
        return {
          $$mdtype: "Tag",
          name,
          attributes,
      ...
      function isClientVariable2(variable) {
        return !!(variable?.$$mdtype === "Variable");
      }
      function isClientFunction2(func) {
        return !!(func?.$$mdtype === "Function");
      }
      ...
      var transformer_default = {
        ...
        node(node2, config = {}) {
          ...
          if (isPromise2(attributes) || isPromise2(children)) {
            return Promise.all([attributes, children]).then((values) => buildTag2(schema.render, ...values));
          }
          return buildTag2(schema.render, attributes, children);
        }
      };
      var Node = class {
      ...
      function formatScalar(v) {
        ...
        if (typeof v === "object") {
          return "{" + Object.entries(v).map(([key, value]) => `${isIdentifier2(key) ? key : `"${key}"`}: ${formatScalar(value)}`).join(SEP) + "}";
        }
      ...
      function formatAnnotationValue(a) {
        ...
        if (a.name === "id" && typeof a.value === "string" && isIdentifier2(a.value))
          return "#" + a.value;
        if (a.type === "class" && isIdentifier2(a.name))
          return "." + a.name;
      ...
      function* formatNode(n, o = {}) {
        ...
        switch (n.type) {
          ...
          case "tag": {
            ...
            yield (!n.inline && isLongTagOpening ? tag.join(NL + SPACE.repeat(open.length) + indent) : inlineTag) + SPACE + (n.children.length ? "" : "/") + CLOSE2;
            if (n.children.length) {
              yield* formatChildren(n, no.allowIndentation ? increment(no) : no);
              if (!n.inline) {
                yield indent;
              }
              yield OPEN2 + SPACE + "/" + n.tag + SPACE + CLOSE2;
            }
            if (!n.inline) {
              yield NL;
            }
        ...
        transform(node2, config) {
          const buildEnclosingTag = (children) => {
            const enclosingTag = {
              ...
              attributes: {
                display: truthy2(node2.attributes.primary) ? "true" : "false"
              },
              children
            };
            return enclosingTag;
      ...
      var RefGenerator2 = _RefGenerator;
      RefGenerator2.ref = 0;
      var and2 = {
        transform(parameters) {
          const value = Object.values(parameters).every((p) => {
            if (typeof p === "object") {
              return truthy2(p.value);
            } else {
              return truthy2(p);
            }
          });
          return {
            ...
            value,
            parameters,
            ref: RefGenerator2.generateRef()
          };
        }
      };
      var or2 = {
        transform(parameters) {
          const value = Object.values(parameters).find((p) => truthy2(p.value)) !== void 0;
          return {
            $$mdtype: "Function",
            name: "or",
            value,
      ...
      var not2 = {
        ...
        transform(parameters) {
          const value = !truthy2(parameters[0].value);
          return {
            $$mdtype: "Function",
            name: "not",
            value,
      ...
      var functions_default2 = { and: and2, or: or2, not: not2, equals: equals2, default: defaultFn2, debug: debug2 };
      ...
      function handleAttrs(token, type) {
        switch (type) {
          ...
          case "fence": {
            ...
            return language === "" || language === OPEN2 ? { content: token.content } : { content: token.content, language };
          }
          case "td":
          case "th": {
            if (token.attrs) {
      ...
      var heading = {
        ...
        transform(node2, config) {
          return buildTag2(`h${node2.attributes["level"]}`, node2.transformAttributes(config), node2.transformChildren(config));
        }
      };
      var paragraph = {
        render: "p",
      ...
      var fence = {
        ...
        transform(node2, config) {
          ...
          return buildTag2("pre", attributes, children);
        }
      };
      var blockquote = {
        render: "blockquote",
      ...
      var list = {
        ...
        transform(node2, config) {
          return buildTag2(node2.attributes.ordered ? "ol" : "ul", node2.transformAttributes(config), node2.transformChildren(config));
        }
      };
      var hr = {
        render: "hr"
      ...
      var code = {
        ...
        transform(node2, config) {
          const attributes = node2.transformAttributes(config);
          return buildTag2("code", attributes, [node2.attributes.content]);
        }
      };
      var text = {
        attributes: {
      ...
      function reresolve(node2, config) {
        ...
        if (isClientVariable2(node2)) {
          node2 = reresolveVariableNode(node2, config);
          return node2;
        }
        if (isClientFunction2(node2)) {
        ...
        node2.children = node2.children.map((child) => {
          if (isTag2(child) || isClientVariable2(child) || isClientFunction2(child)) {
            return reresolve(child, config);
          }
          return child;
        });
      ...
      function reresolveIfNode(ifNode, config) {
        let condition = ifNode.if;
        if (isClientVariable2(condition)) {
          condition = reresolveVariableNode(condition, config);
        } else if (isClientFunction2(condition)) {
          condition = reresolveFunctionNode(condition, config);
        }
        if (truthy2(condition)) {
          ifNode.attributes.display = "true";
        } else {
          ifNode.attributes.display = "false";
        }
      ...
      function reresolveFunctionNode(functionNode, config) {
        ...
        const evaluationFunction = functions_default2[functionNode.name];
        if (!evaluationFunction)
          throw new Error(`Unknown function: ${functionNode.name}`);
        if (!evaluationFunction.transform)
          throw new Error(`Function ${functionNode.name} has no transform method`);
      ...
      function render(node2, config) {
        ...
        if ("if" in node2 && node2.if) {
          let ref = "";
          if ("ref" in node2.if) {
            ref = `data-if=${node2.if.ref}`;
          }
          if (config && config.variables !== void 0) {
            node2 = reresolve(node2, config);
          }
        ...
        if (voidElements.has(name))
          return output;
        if (children.length)
          output += render(children, config);
        output += `</${name}>`;
        return output;
      }
      var renderers_default = { html: render };
      ...
      var import_tag22 = __toModule(require_tag2());
      function createToken(state, content, contentStart) {
        try {
          const { type, meta, nesting = 0 } = (0, import_tag22.parse)(content, { Variable: Variable2, Function: Function22 });
          ...
          if (!(error2 instanceof import_tag22.SyntaxError))
            throw error2;
          const {
            message,
            location: { start, end }
      ...
      function block(state, startLine, endLine, silent) {
        ...
        if (!state.src.startsWith(OPEN2, start))
          return false;
        const tagEnd = findTagEnd2(state.src, start);
        const lastPossible = state.src.slice(0, finish).trim().length;
        if (!tagEnd || tagEnd < lastPossible - CLOSE2.length)
          return false;
        const contentStart = start + OPEN2.length;
        const content = state.src.slice(contentStart, tagEnd).trim();
        const lines = state.src.slice(start, tagEnd + CLOSE2.length).split("\n").length;
        if (content[0] === "$")
          return false;
        if (silent)
          return true;
      ...
      function inline2(state, silent) {
        if (!state.src.startsWith(OPEN2, state.pos))
          return false;
        const tagEnd = findTagEnd2(state.src, state.pos);
        if (!tagEnd)
          return false;
        const content = state.src.slice(state.pos + OPEN2.length, tagEnd);
        if (!silent)
          createToken(state, content.trim());
        state.pos = tagEnd + CLOSE2.length;
        return true;
      }
      function core(state) {
        let token;
        for (token of state.tokens) {
          if (token.type !== "fence")
            continue;
          if (token.info.includes(OPEN2)) {
            const start = token.info.indexOf(OPEN2);
            const end = findTagEnd2(token.info, start);
            const content = token.info.slice(start + OPEN2.length, end);
            try {
              const { meta } = (0, import_tag22.parse)(content.trim(), { Variable: Variable2, Function: Function22 });
              token.meta = meta;
            } catch (error2) {
              if (!(error2 instanceof import_tag22.SyntaxError))
                throw error2;
          ...
          if (token?.meta?.attributes?.find((attr) => attr.name === "process" && !attr.value))
            continue;
          token.children = parseTags2(token.content, token.map[0]);
        }
      }
      function plugin(md) {
      ...
      function block3(state, startLine, endLine, silent) {
        const start = state.bMarks[startLine] + state.tShift[startLine];
        if (!state.src.startsWith(OPEN22, start))
          return false;
        const close = state.src.indexOf(CLOSE22, start);
        if (!close)
          return false;
        if (silent)
          return true;
        const content = state.src.slice(start + OPEN22.length, close);
      ...
      function inline3(state, silent) {
        if (!state.src.startsWith(OPEN22, state.pos))
          return false;
        const close = state.src.indexOf(CLOSE22, state.pos);
        if (!close)
          return false;
        if (silent)
          return true;
        const content = state.src.slice(state.pos + OPEN22.length, close);
        const token = state.push("comment", "", 0);
        token.content = content.trim();
        state.pos = close + CLOSE22.length;
        return true;
      }
      function plugin3(md) {
      ...
      function resolve22(content, config) {
      ...
      MarkdocStaticCompiler.functions = functions_default2;
      ...
      MarkdocStaticCompiler.parseTags = parseTags2;
      ...
      MarkdocStaticCompiler.truthy = truthy2;
      MarkdocStaticCompiler.format = format;
    }
  });

  // dist/helperModules/treeManagement.js
  var require_treeManagement = __commonJS({
    "dist/helperModules/treeManagement.js"(exports) {
      "use strict";
      ...
      function getMinifiedIfFunctionsByRef(node) {
        let clientFunctionsByRef = {};
        if (!node || typeof node !== "object")
          return clientFunctionsByRef;
        if (Array.isArray(node)) {
          node.forEach((n) => {
            const functions = getMinifiedIfFunctionsByRef(n);
            clientFunctionsByRef = Object.assign(Object.assign({}, clientFunctionsByRef), functions);
          });
        }
        if ("$$mdtype" in node && node.$$mdtype === "Tag" && node.if) {
          let clientFunction;
          try {
            clientFunction = markdoc_static_compiler_1.ClientFunctionSchema.parse(node.if);
          } catch (e) {
            return clientFunctionsByRef;
          }
          clientFunctionsByRef[clientFunction.ref] = (0, exports.minifyClientFunction)(clientFunction);
        }
      ...
      function buildRenderableTree(p) {
        const renderableTree = markdoc_static_compiler_1.default.transform(p.parsedFile.ast, {
        ...
        if (invalidVarIds.length > 0) {
          throw new Error(`Invalid reference found in markup: ${invalidVarIds} is not a valid identifier.`);
        }
        return renderableTree;
      }
      function collectVarIdsFromTree(node) {
        var _a;
        let variableIdentifiers = [];
        if (!node)
        ...
        if (typeof node === "object" && "$$mdtype" in node && node.$$mdtype === "Variable") {
          variableIdentifiers.push((_a = node.path) === null || _a === void 0 ? void 0 : _a.join("."));
        }
  ...
  var require_ClientRenderer = __commonJS({
    "dist/helperModules/ClientRenderer.js"(exports) {
      ...
      var treeManagement_1 = require_treeManagement();
