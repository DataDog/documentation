...
For any modern use case, `markdoc` is a better choice. `cdocs-markdoc` has fewer features than `markdoc`, since it functions as a patch for legacy Hugo documentation and other static documentation, rather than as a standalone content customization solution.
...
In vanilla Markdoc, the `RenderableTree` contains only the content that should be displayed in the browser. When a variable changes, a new `RenderableTree` is created and rendered to HTML.

In this version, only one `RenderableTree` is created, with the expectation that the full content will only be rendered once, after which the visibility of smaller pieces of content can be toggled on or off as needed in the browser. The `RenderableTree` contains data representations of conditional logic, and these representations can be repeatedly re-resolved to determine which content should be hidden anytime the user updates their preferences.
