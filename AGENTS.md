...
- **The rules above are enforced mechanically.** A `PreToolUse` hook (`.Codex/hooks/block-master-git.py`) blocks commits and pushes that target `master`, along with `--no-verify`, `HUSKY=0`, and force-push flags. The `.husky` git hooks enforce the same rules for commits made outside Codex. A block from either layer is intentional. Do not work around it.
