...
Markdoc uses…
`;
const tokenizer = new index_1.default.Tokenizer({ allowComments: true });
function check(source, expected, options = {}) {
    const a = expected === null || expected === void 0 ? void 0 : expected.trimStart();
    const b = (0, formatter_1.default)(index_1.default.parse(tokenizer.tokenize(source)), options);
    // console.log(a, b);
    const d = (0, jest_diff_1.diff)(a, b);
...
describe('Formatter', () => {
    it('empty', () => {
        expect((0, formatter_1.default)(null)).toBe('');
        check('', '');
        check('\n\n\t\n   \n  \n\n', '');
        stable('\n\n\t\n   \n  \n\n');
    });
...
`;
    ...
    it('emphasis marks', () => {
        ...
        examples.forEach((example) => {
            const ast = index_1.default.parse(example.trim());
            const out = index_1.default.format(ast);
...
`;
    ...
    it('skips over undefined variables', () => {
        const sourceNode = new index_1.default.Ast.Node('tag', {
            undefinedAttribute: undefined,
            validAttribute: true,
        }, [], 'tag');
        const expected = `{% tag validAttribute=true /%}`;
        const b = (0, formatter_1.default)(sourceNode);
        const d = (0, jest_diff_1.diff)(expected, b.trim());
        if (d && d.includes('Compared values have no visual difference.'))
            return;
    });
});
