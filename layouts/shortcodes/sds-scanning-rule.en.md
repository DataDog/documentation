...
1. For **Define actions on match**, select the action you want to take for the matched information. **Note**: Redaction, partial redaction, and hashing are all irreversible actions.
    ...
    - **Mask** (available for logs only): Replaces all matching values with redacted data. Users with the `Data Scanner Unmask` permission can de-obfuscate (unmask) and view this data. See [Mask action](#mask-action) for more information.
