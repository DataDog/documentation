<!--
.NET profiler setup — self-contained.
...
{% alert level="danger" %}
For containers, **more than one core** is required. Read the [Troubleshooting documentation][3] for more details.
{% /alert %}
...
| [Trace to Profiling integration][4]         | 2.30.0+                            | All supported runtime versions.                                                          |
| [Endpoint Profiling][5]  | 2.15.0+                            | All supported runtime versions.                                                          |
...
| [Memory Leak investigation][6] | 3.33.0+      | .NET 6+ (in preview)  |
...
1. With [Single Step APM Instrumentation][8], there is nothing else to install. Go to [Enabling the Profiler](#enabling-the-profiler) to see how to activate the profiler for an application.
...
1. Download the latest [.NET Tracer package][9] that supports your operating system and architecture.
...
1. Install or upgrade to the latest version, using the [.NET Monitoring MSI installer][9]. Continuous Profiler supports 64-bit Windows, so you need the file like `datadog-dotnet-apm-<VERSION>-x64.msi`.
...
1. Add the `Datadog.Trace.Bundle` [NuGet package][10] to your application.
{% /tab %}
...
1. Install the Azure App Service Datadog APM Extension [for Windows][11] or use the [Linux setup][12] for your webapp.
{% /tab %}
...
{% alert level="danger" %}
Datadog does not recommend enabling the profiler at machine-level or for all IIS applications. If you have enabled it machine-wide, read the [Troubleshooting documentation][13] for information about reducing the overhead that is associated with enabling the profiler for all system applications.
{% /alert %}

{% tabs %}

{% tab label="Linux with Single Step APM Instrumentation" %}
With [Single Step APM Instrumentation][8], only `DD_PROFILING_ENABLED` must be set to activate the profiler for an application.
...
The environment variables are applied for *all* IIS applications. Starting with IIS 10, you can set environment variables for each IIS application in the [`C:\Windows\System32\inetsrv\config\applicationhost.config` file][17]. Read the [Microsoft documentation][18] for more details.
...
Docker examples are available in the [`dd-trace-dotnet` repository][19].

For standalone applications, manually restart the application.
{% /tab %}

{% tab label="Azure App Service" %}
Follow these installation guidelines ([Windows][11] or [Linux][12]) to set `DD_PROFILING_ENABLED=1` to enable the profiler.
{% /tab %}
...
| ------------------------------------------------ | ------------- | ------------------------------------------------------------------------------------------------ |
| `DD_ENV`                   | String        | The [environment][20] name, for example, `production`. If not set, will be `unspecified-environment` |
| `DD_SERVICE`               | String        | The [service][20] name, for example, `web-backend`. If this is not specified, the .NET Profiler tries to determine the service name automatically from the application name (process entry assembly or process name).    |
...
| `DD_PROFILING_HEAPSNAPSHOT_ENABLED` | Boolean        | If set to `true`, enables the regular generation of a heap snapshot when an increase in memory consumption is detected. This is used in the [Memory Leak user interface][6]. Defaults to `false`.  |
...
[5]: /profiler/connect_traces_and_profiles/#break-down-code-performance-by-api-endpoints
[6]: /profiler/guide/solve-memory-leaks/
[7]: https://app.datadoghq.com/account/settings/agent/latest?platform=overview
[8]: https://docs.datadoghq.com/tracing/trace_collection/automatic_instrumentation/single-step-apm
[9]: https://github.com/DataDog/dd-trace-dotnet/releases
[10]: https://www.nuget.org/packages/Datadog.Trace.Bundle
[11]: /serverless/azure_app_services/azure_app_services_windows/?tab=net#installation
[12]: /serverless/azure_app_services/azure_app_services_linux/?tab=nodenetphppython#setup
[13]: /profiler/profiler_troubleshooting/?code-lang=dotnet#avoid-enabling-the-profiler-machine-wide
[14]: /integrations/guide/source-code-integration/?tab=net
[15]: https://app.datadoghq.com/profiling
[16]: /profiler/profiler_troubleshooting/dotnet/
[17]: https://docs.microsoft.com/en-us/iis/get-started/planning-your-iis-architecture/introduction-to-applicationhostconfig
[18]: https://docs.microsoft.com/en-us/iis/configuration/system.applicationhost/applicationpools/add/environmentvariables/
[19]: https://github.com/DataDog/dd-trace-dotnet/tree/master/tracer/samples/NugetDeployment
