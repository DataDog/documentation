...
```
When initializing the SDK in ASYNC mode, the SDK will use React Native's [requestIdleCallback][16] https://reactnative.dev/docs/global-requestIdleCallback to run the initialization code after all animations and UI rendering have settled.

If this function is not available, the SDK will fall back to React Native's [InteractionManager.runAfterInteractions][17] to delay its initialization.
