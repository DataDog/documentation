...
4. To disable automatic user data collection for client IP or geolocation data, use the toggles for those settings. For more information, see [RUM iOS Data Collected][3].
   ...
   ![Create a RUM application for iOS in Datadog](/real_user_monitoring/ios/ios-create-application.png)
...
4. To disable automatic user data collection for either client IP or geolocation data, use the toggles for those settings. For more information, see [iOS Data Collected][3].
   ...
   ![Create an application for iOS in Datadog](/real_user_monitoring/error_tracking/mobile-new-application-1.png)
...
{% alert type="warning" %}
Using `.trackRUMTapAction(name:)` for `SwiftUI` controls inside a `List` can break its default gestures. For example, it may disable the `Button` action or break `NavigationLink`. To track taps in a `List` element, use the [Custom Actions](/real_user_monitoring/mobile_and_tv_monitoring/ios/advanced_configuration#custom-actions) API instead.
{% /alert %}
