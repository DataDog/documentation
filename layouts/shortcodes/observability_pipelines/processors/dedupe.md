...
1. Define a **filter query**. Only logs that match the specified [filter query](#filter-query-syntax) are checked for deduplication based on the rules below. No logs are filtered out, so all logs go through the processor that comes after this one.
