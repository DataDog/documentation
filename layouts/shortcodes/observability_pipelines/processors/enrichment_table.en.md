...
4. Select the type of reference table you want to use, **File** or **GeoIP**.
   - For the **File** type:
        1. Enter the file path.<br>**Note**: The file path must be under the data directory: `/var/lib/observability-pipelines-worker/config/`. The file must also be owned by the `observability-pipelines-worker group` and `observability-pipelines-worker` user, or at least readable by the group or user.
