...
1. Define a **filter query**. Only logs that match the specified [filter query](#filter-query-syntax) are counted towards the daily limit.
    - Logs that match the quota filter and are within the daily quota, are counted towards the daily limit and sent to the next step in the pipeline.
    - Logs that do not match the quota filter are not counted towards the daily limit and sent to the next step of the pipeline.
...
1. Check the **Drop events** checkbox if you want to drop all events when your quota is met. Leave it unchecked if you plan to set up a [monitor][5001] that sends an alert when the quota is met.
    - If logs that match the quota filter are received after the daily quota has been met and the **Drop events** option is selected, then those logs are dropped.
    - If logs that match the quota filter are received after the daily quota has been met and the **Drop events** option is not selected, then those logs are sent to the next step in the pipeline.
