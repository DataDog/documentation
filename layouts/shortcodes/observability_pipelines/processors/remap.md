...
1. Define a **filter query**. Only logs that match the specified [filter query](#filter-query-syntax) are processed. No logs are filtered out, so all logs go through the processor that comes after this one.
