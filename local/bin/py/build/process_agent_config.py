...
def create_agent_config_dict(dd_agent_config_string):
  """
  Returns a dictionary where the keys represent each config type (i.e. "Basic Configuration", "Log Collection Configuration", "DogstatsD Configuration", etc),
  and the values are a string containing the config options and code comments.
  """
...
def document_available_config_types(agent_config_dict):
  """
  Returns a single string containing all available config types, for Docs team to reference.
  """
  config_types_string = 'Available config types: \n\n'

  for config_type in agent_config_dict.keys():
    config_types_string += config_type + '\n'

  return config_types_string
...
def process_agent_config(dd_agent_config_string):
  ...
  try:    
    ...
    config_types_string = document_available_config_types(agent_config_dict)
    ...
    with open('agent_config_types_list.txt', 'w+') as agent_config_types_list_outfile:
      agent_config_types_list_outfile.write(config_types_string)
