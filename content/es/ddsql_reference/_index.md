...
description: Referencia completa para la sintaxis de DDSQL, tipos de datos, funciones,
  operadores y sentencias para consultar datos de Datadog con SQL.
further_reading:
...
`USING`
: Esta cláusula es una abreviatura para uniones donde las columnas de unión tienen el mismo nombre en ambas tablas. Toma una lista de columnas separadas por comas y crea una condición de igualdad separada para cada par que coincida. Por ejemplo, unir `T1` y `T2` con `USING (a, b)` es equivalente a `ON T1.a = T2.a AND T1.b = T2.b`.
...
Todos los tipos de datos soportan tipos de arreglo. Consulta [Arreglos](#arrays) para literales de arreglo, acceso a elementos y funciones de arreglo.

## Literales de tipo {#type-literals}

DDSQL soporta literales de tipo explícitos usando la sintaxis `[TYPE] [value]`.
...
El prefijo de tipo puede omitirse y el tipo se infiere automáticamente del valor. Por ejemplo, `'hello world'` se infiere como `VARCHAR`, `123` como `BIGINT`, y `true` como `BOOLEAN`. Utilice prefijos de tipo explícitos cuando los valores puedan ser ambiguos; por ejemplo, `TIMESTAMP '2025-01-01'` se inferiría como `VARCHAR` sin el prefijo.
...
Utilice la sintaxis `ARRAY[value1, value2, ...]` para construir un literal de arreglo. El tipo del arreglo se infiere automáticamente de los valores.
...
Acceda a elementos individuales del arreglo con un subíndice basado en 1. Acceder a un índice que está fuera de límites devuelve `NULL`.
...
| `STRING_TO_ARRAY(string s, string delimiter)` | `VARCHAR[]` | Divide una cadena en un arreglo de cadenas en el delimitador dado. |
| `ARRAY_TO_STRING(array a, string delimiter)` | `VARCHAR` | Une los elementos del arreglo en una cadena con el delimitador dado. |
| `ARRAY_AGG(expression e)` | arreglo de tipo de entrada | Agrega valores de múltiples filas en un arreglo. |
| `UNNEST(array a [, array b...])` | filas de a [, b...] | Expande uno o más arreglos en un conjunto de filas. Solo válido en una cláusula `FROM`. |
...
Las siguientes funciones SQL son compatibles. Para la función de ventana, consulta la sección separada [Función de ventana](#window-functions) en esta documentación.
...
| `COALESCE(args a)`                               | tipo del primer valor no nulo o nulo       | Devuelve el primer valor no nulo o nulo si todos son nulos.                                                                                                                                         |
...
| `TRIM(string s)`                                 | cadena                                | Elimina los espacios en blanco al principio y al final de la cadena.                                                                                                                                          |
...
| `SPLIT_PART(string s, string delimiter, integer index)` | cadena                         | Divide la cadena usando el delimitador dado y devuelve la cadena en la posición dada contando desde uno.                                                                                          |
| `EXTRACT(unit from timestamp/interval)`          | numérico                               | Extrae una parte de un campo de fecha u hora (como año o mes) de una marca de tiempo o intervalo.                                                                                                     |
| `TO_TIMESTAMP(string timestamp, string format)`  | marca de tiempo                             | Convierte una cadena a una marca de tiempo según el formato dado.                                                                                                                                   |
| `TO_TIMESTAMP(numeric epoch)`                    | marca de tiempo                             | Convierte una marca de tiempo de época UNIX (en segundos) a una marca de tiempo.                                                                                                                                      |
| `TO_CHAR(timestamp t, string format)`            | cadena                                | Convierte una marca de tiempo a una cadena según el formato dado.                                                                                                                                   |
| `DATE_BIN(interval stride, timestamp source, timestamp origin)` | marca de tiempo                             | Alinea una marca de tiempo (fuente) a cubos de longitud uniforme (paso). Devuelve el inicio del cubo que contiene la fuente, calculado como la marca de tiempo más grande que sea menor o igual a la fuente y que sea múltiplo de la longitud del paso a partir del origen. |
| `DATE_TRUNC(string unit, timestamp t)`           | marca de tiempo                             | Trunca una marca de tiempo a una precisión especificada según la unidad proporcionada.                                                                                                                        |
| `CURRENT_SETTING(string setting_name)`           | cadena                                | Devuelve el valor actual de la configuración especificada. Soporta los parámetros `dd.time_frame_start` y `dd.time_frame_end`, que devuelven el inicio y el final del marco de tiempo global, respectivamente. |
| `NOW()`                                          | marca de tiempo                             | Devuelve la marca de tiempo UTC actual al inicio de la consulta actual.                                                                                                                              |
...
| `STRING_TO_ARRAY(string s, string delimiter)`    | arreglo de cadenas                      | Divide la cadena dada en un arreglo de cadenas usando el delimitador dado.                                                                                                                       |
| `ARRAY_TO_STRING(array a, string delimiter)`     | cadena                                | Convierte un arreglo a una cadena concatenando elementos con el delimitador dado.                                                                                                                 |
| `ARRAY_AGG(expression e)`                        | arreglo de tipo de entrada                   | Crea un arreglo recolectando todos los valores de entrada.                                                                                                                                              |
| `APPROX_PERCENTILE(double percentile) WITHIN GROUP (ORDER BY expression e)` | tipo de expresión        | Calcula un valor percentil aproximado. El percentil debe estar entre 0.0 y 1.0 (inclusive). Requiere la sintaxis `WITHIN GROUP (ORDER BY ...)`.                                              |
...
Tipos de destino de conversión soportados:
- `BIGINT`
- `DECIMAL`
- `TIMESTAMP`
- `VARCHAR`
...
**Forma 2: Convertir la marca de tiempo de la época UNIX a marca de tiempo**
...
Todas las funciones de expresiones regulares (regex) utilizan la variante ICU (International Components for Unicode):
...
| `REGEXP_REPLACE(string input, string pattern, string replacement [, string flags ])`                             | cadena           | Reemplaza la subcadena que es la primera coincidencia con el patrón, o todas las coincidencias si utiliza la [opcional `g` bandera](#function-level-flags).                                                                                                                              |
...
Puede usar las siguientes banderas con [funciones de expresión regular](#regular-expressions):
...
Esta tabla proporciona una visión general de las funciones de ventana soportadas. Para obtener detalles completos y ejemplos, consulte la [documentación de PostgreSQL][2].

| Función                | Tipo de retorno       | Descripción                                                            |
|-------------------------|-------------------|------------------------------------------------------------------------|
| `OVER`                  | N/A               | Define una ventana para un conjunto de filas sobre las que operan otras funciones de ventana. |
| `PARTITION BY`          | N/A               | Divide el conjunto de resultados en particiones, específicamente para aplicar funciones de ventana. |
| `RANK()`                | entero           | Asigna un rango a cada fila dentro de una partición, con espacios para empates.     |
| `ROW_NUMBER()`          | entero           | Asigna un número secuencial único a cada fila dentro de una partición.     |
| `LEAD(column n)`        | tipo de columna     | Devuelve el valor de la siguiente fila en la partición.                  |
| `LAG(column n)`         | tipo de columna     | Devuelve el valor de la fila anterior en la partición.              |
| `FIRST_VALUE(column n)` | tipo de columna     | Devuelve el primer valor en un conjunto ordenado de valores.                   |
| `LAST_VALUE(column n)`  | tipo de columna     | Devuelve el último valor en un conjunto ordenado de valores.                    |
| `NTH_VALUE(column n, offset)`| tipo de columna | Devuelve el valor en el desplazamiento especificado en un conjunto ordenado de valores. |
...
|-----------------------------------------------|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| json_extract_path_text(text json, text path…) | text         | Extrae un subobjeto JSON como texto, definido por la ruta. Su comportamiento es equivalente a la [función de Postgres con el mismo nombre][3]. Por ejemplo, `json_extract_path_text(col, ‘forest')` devuelve el valor de la clave `forest` para cada objeto JSON en `col`. Consulte el ejemplo a continuación para la sintaxis de un arreglo JSON. |
| json_extract_path(text json, text path…)      | JSON         | Misma funcionalidad que `json_extract_path_text`, pero devuelve una columna de JSON type en lugar de text type.                                                                                                                                                                                                        |
| json_array_elements(text json)                | rows of JSON | Expande un arreglo JSON en un conjunto de filas. Esta forma solo se permite en una cláusula FROM.                                                                                                                                                                                                                           |
...
## Funciones de tabla {#table-functions}
Las funciones de tabla se utilizan para consultar registros, métricas, Cloud Cost y otras fuentes de datos.
...
dd.metrics_scalar(
    ...
    reducer varchar [, from_timestamp timestamp, to_timestamp timestamp]
      ...
      <td>Devuelve datos métricos como un valor escalar. La función acepta una consulta de métricas (con agrupamiento opcional), un reducer para determinar cómo se agregan los valores (avg, max, etc.), y parámetros de tiempo opcionales (predeterminado 1 hora) para definir el rango de tiempo.</td>
      <td>
        {{< code-block lang="sql" >}}
SELECT *
...
dd.metrics_timeseries(
    query varchar [, from_timestamp timestamp, to_timestamp timestamp]
      ...
      <td>Devuelve datos métricos como series temporales. La función acepta una consulta de métricas (con agrupamiento opcional) y parámetros de tiempo opcionales (predeterminado 1 hora) para definir el rango de tiempo. Devuelve puntos de datos a lo largo del tiempo en lugar de un solo valor agregado.</td>
      <td>
        {{< code-block lang="sql" >}}
SELECT *
...
dd.cloud_cost_scalar(
    ...
    to_timestamp timestamp]
      ...
      <td>Devuelve <a href="/cloud_cost_management/">datos de Cloud Cost Management</a> como un valor escalar. La función acepta una consulta de Cloud Cost (con agrupamiento opcional), un reducer de agregación (usar <code>sum</code> para datos de Cloud Cost; otros reducers como <code>avg</code>, <code>min</code>, y <code>max</code> se aceptan pero rara vez son aplicables a consultas de Cloud Cost), y parámetros timestamp opcionales (predeterminado 1 hora) para definir el rango de tiempo. <strong>Nota</strong>: Los datos de costos en la nube suelen tener un retraso de 24 a 48 horas, por lo que las marcas de tiempo recientes pueden no devolver resultados.</td>
      <td>
        {{< code-block lang="sql" >}}
SELECT *
...
dd.cloud_cost_timeseries(
    ...
    to_timestamp timestamp]
      ...
      <td>Devuelve <a href="/cloud_cost_management/">datos de Cloud Cost Management</a> como series temporales. La función acepta una consulta de Cloud Cost (con agrupamiento opcional) y parámetros timestamp opcionales (predeterminado 1 hora) para definir el rango de tiempo. Devuelve puntos de datos de Cloud Cost a lo largo del tiempo en lugar de un solo valor agregado. <strong>Nota </strong>: Los datos de costos en la nube suelen tener un retraso de 24 a 48 horas, por lo que las marcas de tiempo recientes pueden no devolver resultados.</td>
      <td>
        {{< code-block lang="sql" >}}
SELECT *
...
DDSQL expone etiquetas como un `hstore` tipo, que está inspirado en PostgreSQL. Puedes acceder a los valores de claves de etiqueta específicas utilizando el operador de flecha de PostgreSQL. Por ejemplo:
...
Las etiquetas son pares clave-valor, donde cada clave de etiqueta puede tener cero, uno o múltiples valores de etiqueta correspondientes a ella. Al acceder, el valor de la etiqueta devuelve una única cadena, que contiene _todos_ los valores correspondientes. Cuando los datos tienen múltiples valores de etiqueta para la misma clave de etiqueta, se representan como una cadena ordenada y separada por comas. Por ejemplo:
...
También puedes comparar valores de etiqueta como cadenas o conjuntos de etiquetas completos:
...
Además, puedes extraer claves de etiqueta y valores en arreglos individuales de texto:
...
### Funciones y operadores HSTORE {#hstore-functions-and-operators}

| Nombre                                          | Tipo de retorno   | Descripción                                                                                      |
|-----------------------------------------------|---------------|---------------------------------------------------------------------------------------------------
| tags -> 'text'                                  | Texto          | Obtiene el valor para una clave dada. Devuelve `null` si la clave no está presente.                             |
| akeys(hstore tags)                            | Array de texto | Obtiene las claves de un HSTORE como un array                                                            |
| avals(hstore tags)                            | Array de texto | Obtiene los valores de un HSTORE como un array                                                          |
