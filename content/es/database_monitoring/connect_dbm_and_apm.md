...
---
Esta guía asume que has configurado [DBM][1] y estás utilizando [APM][2]. Conectar APM y DBM inyecta identificadores de traza de APM en la recolección de datos de DBM, lo que permite correlacionar ambas fuentes de datos. Esto habilita características del producto que muestran información de la base de datos en el producto APM y datos de APM en el producto DBM.
...
Versiones del agente soportadas
: 7.46+

Privacidad de datos
: Habilitar la propagación de comentarios SQL resulta en datos potencialmente confidenciales (nombres de servicios) que se almacenan en las bases de datos y que pueden ser accedidos por otros terceros que han sido autorizados a acceder a la base de datos.
...
|:-----------------|:------------|
| `full` | Envía información completa de la traza a la base de datos, permitiéndote investigar trazas individuales dentro de DBM. Esta es la solución recomendada para la mayoría de las integraciones. |
...
<div class="alert alert-danger">Si tu aplicación utiliza <code>context_info</code> para la instrumentación, el SDK de Datadog lo sobrescribe.</div>

- La instrumentación ejecuta un comando `SET context_info` cuando el cliente emite una consulta, lo que genera una ida y vuelta adicional a la base de datos.
- Requisitos previos:
...
Estas etiquetas identifican tu servicio en las vistas de correlación de APM y en el desglose de conexiones activas de DBM.

Datadog recomienda establecer el modo de ofuscación en `obfuscate_and_normalize` para versiones del agente `7.63` y superiores. Agrega el siguiente parámetro en la sección `apm_config` de tu archivo de configuración del agente APM:

```yaml
  sql_obfuscation_mode: "obfuscate_and_normalize"
```

<div class="alert alert-warning">Cambiar el modo de ofuscación puede alterar el texto SQL normalizado. Si tienes monitores basados en texto SQL en las trazas de APM, es posible que necesites actualizarlos.</div>

{{< tabs >}}
{{% tab "Go" %}}

Actualiza las dependencias de tu aplicación para incluir [dd-trace-go v2][1]. {{% tracing-go-v2 %}}
...
Habilita la función de propagación de DBM utilizando uno de los siguientes métodos:
- Variable de entorno:
...
Sigue las instrucciones de instrumentación de [Java tracing][1] e instala la versión `1.11.0` del Agente, o una superior.

También debes habilitar la `jdbc-datasource` [instrumentación][2].

Habilita la función de propagación de DBM utilizando **uno** de los siguientes métodos:

- Establece la propiedad del sistema `dd.dbm.propagation.mode=full`
- Establece la variable de entorno `DD_DBM_PROPAGATION_MODE=full`
...
Prueba la función en tu aplicación:
...
**Versiones del rastreador 1.44 y superiores**:
Habilita el rastreo de sentencias preparadas para Postgres utilizando **uno** de los siguientes métodos:
...
**Nota**: La instrumentación de sentencias preparadas sobrescribe la propiedad `Application` con el texto `_DD_overwritten_by_tracer`, y causa una ida y vuelta adicional a la base de datos. Esta ida y vuelta adicional tiene un impacto mínimo en el tiempo de ejecución de las sentencias SQL.

<div class="alert alert-danger">Habilitar el rastreo de sentencias preparadas puede causar un aumento en el pinning de conexiones al usar Amazon RDS Proxy, lo que reduce la eficiencia del connection pooling. Para más información, consulta <a href="https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/rds-proxy-pinning.html">Pinning de conexiones en RDS Proxy</a>.</div>

**Versiones del rastreador por debajo de 1.44**:
Las sentencias preparadas no son compatibles en modo `full` para Postgres y MySQL, y todas las llamadas a la API de JDBC que utilizan sentencias preparadas se degradan automáticamente a modo `service`. Dado que la mayoría de las bibliotecas SQL de Java utilizan sentencias preparadas por defecto, esto significa que **la mayoría** de las aplicaciones Java solo pueden usar el modo `service`.
...
Habilita la función de propagación de monitoreo de base de datos utilizando uno de los siguientes métodos:
...
**Nota**: El soporte de MongoDB requiere `dd-trace-py` >= 3.5.0. Si necesitas actualizar: `pip install "ddtrace>=3.5.0"`.

Habilita la función de propagación de monitoreo de base de datos configurando la siguiente variable de entorno:
   - `DD_DBM_PROPAGATION_MODE=full`
...
<div class="alert alert-danger">
Esta función requiere que la extensión tracer esté habilitada para tu servicio PHP.
</div>

Sigue las [instrucciones de trazado de PHP][1] para instalar el paquete de instrumentación automática y habilitar el trazado para tu servicio.

Asegúrate de que estás utilizando una biblioteca de cliente compatible. Por ejemplo, `PDO`.
...
* Establezca el SDK para usar la opción `dbmPropagationMode` (predeterminado: `ENV['DD_DBM_PROPAGATION_MODE']`):
   ```javascript
...
1. Ejecute su aplicación instrumentada y ejecute una consulta a la base de datos.
1. En Datadog, vaya a [**Database Monitoring > Muestras de Consultas**][37].
1. Confirme que la insignia de correlación **APM** aparece en la muestra de consulta.
...
Desglosa las conexiones activas para un servidor dado por los servicios APM ascendentes que realizan las solicitudes. Puedes atribuir la carga en una base de datos a servicios individuales para entender cuáles son los servicios más activos en la base de datos. Dirígete a la página del servicio aguas arriba más activo para continuar la investigación.

### Filtra tus servidores de base de datos por los servicios APM que los llaman {#filter-your-database-hosts-by-the-apm-services-that-call-them}

{{< img src="database_monitoring/dbm_filter_by_calling_service.png" alt="Filtra tus servidores de base de datos por los servicios APM que los llaman.">}}

Filtra la lista de bases de datos para mostrar solo los servidores de base de datos de los que dependen tus servicios APM específicos. Identifica si alguna de tus dependencias aguas abajo tiene actividad bloqueante que pueda impactar el rendimiento del servicio.

### Ve la traza asociada a una muestra de consulta {#view-the-associated-trace-for-a-query-sample}

{{< img src="database_monitoring/dbm_query_sample_trace_preview.png" alt="Previsualiza la traza APM muestreada de la que se generó la muestra de consulta que se está inspeccionando.">}}

Al ver una [Muestra de Consulta][37] en el Monitoreo de Bases de Datos, si la traza asociada ha sido muestreada por APM, puedes ver la Muestra de DBM en el contexto de la Traza APM. Esto te permite combinar la telemetría de DBM, incluyendo el plan de explicación y el rendimiento histórico de la consulta, junto con la línea de tiempo del span dentro de tu infraestructura para entender si un cambio en la base de datos es responsable del bajo rendimiento de la aplicación.

## Explora la Conexión de DBM en APM {#explore-the-dbm-connection-in-apm}

### Visualiza los hosts de base de datos aguas abajo de los servicios APM {#visualize-the-downstream-database-hosts-of-apm-services}

En la página de APM para un servicio dado, ve las dependencias directas de base de datos aguas abajo del servicio según lo identificado por el Monitoreo de Bases de Datos, y determina si algún host tiene una carga desproporcionada que puede ser causada por vecinos ruidosos. Para ver las dependencias de base de datos de un servicio:
1. Selecciona el servicio en el [Software Catalog][26] para abrir un panel de detalles.
...
### Visualiza las duraciones de los spans y ve los detalles de la consulta {#visualize-span-durations-and-view-query-details}

Selecciona la pestaña {{< ui >}}Queries{{< /ui >}} de la sección {{< ui >}}Databases{{< /ui >}} en la página del servicio APM para ver los valores anómalos de latencia y una lista completa de consultas del intervalo de tiempo seleccionado. Selecciona una consulta en la tabla para ver el panel de consultas y acceder a diagnósticos, detalles de errores e información de traza.

### Identifica posibles optimizaciones utilizando planes de explicación para consultas de base de datos en las trazas {#identify-potential-optimizations-using-explain-plans-for-database-queries-in-traces}

{{< img src="database_monitoring/explain_plans_in_traces_update.png" alt="Identifica ineficiencias utilizando planes de explicación para consultas de base de datos dentro de las trazas.">}}

Ve el rendimiento histórico de consultas similares a las ejecutadas en tu traza, incluyendo eventos de espera muestreados, latencia promedio y planes de explicación capturados recientemente, para contextualizar cómo se espera que rinda una consulta. Determina si el comportamiento es anormal y continúa la investigación pivotando a [Database Monitoring][1] para obtener contexto adicional sobre los servidores de base de datos subyacentes.
