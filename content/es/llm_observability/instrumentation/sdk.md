...
<br />Solo requerido si no estás utilizando el Agente de Datadog, en cuyo caso esto debe configurarse a `1` o `true`.

`DD_API_KEY`
: opcional - _cadena_
<br />Tu clave API de Datadog. Solo requerido si no estás utilizando el Agente de Datadog.

[1]: /es/getting_started/tagging/unified_service_tagging?tab=kubernetes#non-containerized-environment
{{% /tab %}}

{{% tab "Node.js" %}}
Habilita la Observabilidad LLM ejecutando tu aplicación con `NODE_OPTIONS="--import dd-trace/initialize.mjs"` y especificando las variables de entorno requeridas.
...
Habilita la Observabilidad LLM ejecutando tu aplicación con `dd-trace-java` y especificando los parámetros requeridos como variables de entorno o propiedades del sistema.
...
`DD_SITE` o `dd.site`
: requerido - _cadena_
<br />Sitio de Datadog de destino para la presentación de datos LLM. Tu sitio es {{< region-param key="dd_site" code="true" >}}.

`DD_LLMOBS_ENABLED` o `dd.llmobs.enabled`
: requerido - _entero o cadena_
<br />Alternar para habilitar el envío de datos a la Observabilidad LLM. Debería configurarse a `1` o `true`.
...
: opcional - _entero o cadena_ - **predeterminado**: `false`
<br />Solo es requerido si no estás usando el Agente de Datadog, en cuyo caso esto debe establecerse en `1` o `true`.
...
En lugar de usar [configuración por línea de comandos](#command-line-setup), también puedes habilitar la Observabilidad LLM programáticamente.

{{< tabs >}}
{{% tab "Python" %}}

Usa la `LLMObs.enable()` función para habilitar la Observabilidad LLM.

<div class="alert alert-info">
No uses este método de configuración con el comando <code>ddtrace-run</code>.
</div>
...
`ml_app`
: opcional - _cadena_
<br />El nombre de tu aplicación, servicio o proyecto LLM, bajo el cual se agrupan todos los rastros y spans. Esto ayuda a distinguir entre diferentes aplicaciones o experimentos. Consulta [Directrices de nomenclatura de aplicaciones](#application-naming-guidelines) para caracteres permitidos y otras restricciones. Para anular este valor para un rastro dado, consulta [Rastreo de múltiples aplicaciones](#tracing-multiple-applications). Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_LLMOBS_ML_APP`.

`integrations_enabled` - **predeterminado**: `true`
: opcional - _booleano_
<br />Una bandera para habilitar el rastreo automático de llamadas LLM para las integraciones de [LLM soportadas por Datadog][1]. Si no se proporciona, todas las integraciones de LLM soportadas están habilitadas por defecto. Para evitar usar las integraciones de LLM, establece este valor en `false`.

`agentless_enabled`
: opcional - _booleano_ - **predeterminado**: `false`
<br />Solo requerido si no estás usando el Agente de Datadog, en cuyo caso esto debe establecerse en `True`. Esto configura la biblioteca `ddtrace` para no enviar ningún dato que requiera el Agente de Datadog. Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_LLMOBS_AGENTLESS_ENABLED`.
...
`api_key`
: opcional - _cadena_
<br />Tu clave de API de Datadog. Solo requerido si no estás utilizando el Agente de Datadog. Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_API_KEY`.

`env`
: opcional - _cadena_
<br />El nombre del entorno de tu aplicación (ejemplos: `prod`, `pre-prod`, `staging`). Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_ENV`.
...
Usa la `init()` función para habilitar la Observabilidad LLM.
...
**Opciones para la `llmobs`configuración**

`mlApp`
: opcional - _cadena_
<br />El nombre de tu aplicación LLM, servicio o proyecto, bajo el cual se agrupan todos los trazos y spans. Esto ayuda a distinguir entre diferentes aplicaciones o experimentos. Consulta [Directrices de nomenclatura de aplicaciones](#application-naming-guidelines) para caracteres permitidos y otras restricciones. Para anular este valor para un trazo dado, consulta [Trazando múltiples aplicaciones](#tracing-multiple-applications). Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_LLMOBS_ML_APP`.

`agentlessEnabled`
: opcional - _booleano_ - **predeterminado**: `false`
<br />Solo requerido si no estás utilizando el Agente de Datadog, en cuyo caso esto debe establecerse en `true`. Esto configura la `dd-trace` biblioteca para no enviar ningún dato que requiera el Agente de Datadog. Si no se proporciona, esto se establece en el valor de `DD_LLMOBS_AGENTLESS_ENABLED`.

**Opciones para la configuración general del rastreador**:

`site`
: opcional - _cadena_
<br />El sitio de Datadog para enviar tus datos de LLM. Tu sitio es {{< region-param key="dd_site" code="true" >}}. Si no se proporciona, esto se establece de forma predeterminada al valor de `DD_SITE`.
...
Para instrumentar una función existente de AWS Lambda con Observabilidad de LLM, puedes usar la Extensión de Datadog y las capas de lenguaje respectivas.
...
{{% tab "Python" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="python" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <TU_LLMOBS_ML_APP>
```
{{% /tab %}}

{{% tab "Node.js" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="node" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <TU_LLMOBS_ML_APP>
```
{{% /tab %}}

{{% tab "Java" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="dd-trace-java" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <TU_LLMOBS_ML_APP>
```
{{% /tab %}}
{{< /tabs >}}
...
Después de instalar el SDK y ejecutar tu aplicación, deberías esperar ver algunos datos en LLM Observability de la auto-instrumentación. La instrumentación manual se puede usar para capturar marcos personalizados o operaciones de bibliotecas que aún no están soportadas.
...
Los tipos de span son requeridos y se especifican en el objeto `options` pasado a las funciones de trazado `llmobs` (`trace`, `wrap` y `decorate`). Consulta la [documentación de Tipos de Span][1] para una lista de tipos de span soportados.

**Nota:** Los intervalos con un tipo de intervalo inválido no se envían a LLM Observability.
...
### Condiciones para finalizar un intervalo para una función envuelta
...
- Si la función t no acepta un callback y no devuelve una Promesa, entonces el intervalo finaliza al final de la ejecución de la función.
...
Si la aplicación no utiliza la función de callback, se recomienda usar un bloque rastreado en línea en su lugar. Consulta [Rastreando intervalos usando métodos en línea](#tracing-spans-using-inline-methods) para más información.
...
### Iniciando un intervalo

Hay múltiples métodos para iniciar un intervalo, según el tipo de intervalo que estés iniciando. Consulta la [documentación de Tipos de Span][1] para una lista de tipos de span soportados.
...
Para finalizar un span, llama a `finish()` en una instancia de objeto span. Si es posible, envuelve el span en un bloque de `try/finally` para asegurar que el span se envíe incluso si ocurre una excepción.
...
<div class="alert alert-info">Si estás utilizando algún proveedor o marco de LLM que sea compatible con <a href="/llm_observability/instrumentation/auto_instrumentation/">las integraciones de LLM de Datadog</a>, no necesitas iniciar manualmente un span de LLM para rastrear estas operaciones.</div>
...
`session_id`
: opcional - _cadena_
<br/>El ID de la sesión de usuario subyacente. Consulta [Seguimiento de sesiones de usuario](#tracking-user-sessions) para más información.
...
{{% tab "Node.js" %}}
Para rastrear una llamada LLM, especifica el tipo de intervalo como `llm`, y opcionalmente especifica los siguientes argumentos en el objeto de opciones.
...
{{% /tab %}}
{{% tab "Java" %}}
Para rastrear un intervalo de flujo de trabajo, importa y llama al siguiente método con los argumentos que se enumeran a continuación:
...
{{% tab "Node.js" %}}
Para rastrear la ejecución de un agente, especifica el tipo de intervalo como `agent`, y opcionalmente especifica argumentos en el objeto de opciones.
...
`spanName`
: opcional - _Cadena_
<br/>El nombre de la operación. Si no se proporciona, `spanName` se establece de forma predeterminada como el nombre de la función rastreada.
...
{{< tabs >}}
{{% tab "Python" %}}
Para rastrear una llamada a la herramienta, utiliza el decorador de función `ddtrace.llmobs.decorators.tool()`.
...
{{% tab "Node.js" %}}
Para rastrear una llamada a la herramienta, especifica el tipo de intervalo como `tool`, y opcionalmente especifica argumentos en el objeto de opciones.
...
{{% /tab %}}
{{% tab "Java" %}}
Para rastrear una llamada a la herramienta, importa y llama al siguiente método con los argumentos listados a continuación:
...
`mlApp`
: opcional - _Cadena_
<br/>El nombre de la aplicación de ML a la que pertenece la operación. Proporcionar un valor no nulo anula el nombre de la aplicación de ML suministrado al inicio de la aplicación. Consulta [Rastreo de múltiples aplicaciones](#tracing-multiple-applications) para más información.
...
{{% tab "Node.js" %}}
Para rastrear un intervalo de tarea, especifica el tipo de intervalo como `task`, y opcionalmente especifica argumentos en el objeto de opciones.
...
{{% /tab %}}
{{% tab "Java" %}}
Para rastrear un intervalo de tarea, importa y llama al siguiente método con los argumentos listados a continuación:
...
**Nota**: Anotar la entrada de un intervalo de embedding requiere un formato diferente al de otros tipos de intervalos. Consulta [Enriqueciendo intervalos](#enriching-spans) para más detalles sobre cómo especificar entradas de embedding.
...
{{% tab "Node.js" %}}
Para trazar una operación de incrustación, especifica el tipo de intervalo como `embedding`, y opcionalmente especifica argumentos en el objeto de opciones.
...
**Nota**: Anotar la salida de un intervalo de recuperación requiere un formato diferente al de otros tipos de intervalos. Consulta [Enriqueciendo intervalos](#enriching-spans) para más detalles sobre cómo especificar las salidas de recuperación.
...
Para rastrear un intervalo de recuperación, especifica el tipo de intervalo como `retrieval`, y opcionalmente especifica los siguientes argumentos en el objeto de opciones.
...
Lo siguiente también incluye un ejemplo de anotar un intervalo. Consulta [Enriqueciendo intervalos](#enriching-spans) para más información.
...
<div class="alert alert-info">
El parámetro <code>métricas</code> aquí se refiere a valores numéricos adjuntos como atributos en intervalos individuales — no <a href="/llm_observability/monitoring/metrics/">métricas de la plataforma Datadog</a>. Para ciertas claves reconocidas como <code>input_tokens</code>, <code>output_tokens</code>, y <code>total_tokens</code>, Datadog utiliza estos atributos de intervalo para generar métricas de plataforma correspondientes (como <code>ml_obs.span.llm.input.tokens</code>) para su uso en tableros y monitores.
</div>

{{< tabs >}}
{{% tab "Python" %}}
El SDK proporciona el método `LLMObs.annotate()` para enriquecer los spans con entradas, salidas y metadatos.
...
<br />El span a anotar. Si `span` no se proporciona (como al usar decoradores de función), el SDK anota el span activo actual.
...
<br />Un diccionario de pares clave-valor serializables en JSON que los usuarios pueden agregar como información de metadatos relevante a la operación de entrada o salida descrita por el span (`model_temperature`, `max_tokens`, `top_k`, etc.).

`metrics`
: opcional - _diccionario_
<br />Un diccionario de claves serializables en JSON y valores numéricos que los usuarios pueden agregar como métricas relevantes para la operación descrita por el span (`input_tokens`, `output_tokens`, `total_tokens`, `time_to_first_token`, etc.). La unidad para `time_to_first_token` está en segundos, similar a la métrica `duration` que se emite por defecto.
...
{{% tab "Node.js" %}}
El SDK proporciona el método `llmobs.annotate()` para anotar spans con entradas, salidas y metadatos.
...
`span`
: opcional - _Span_ - **predeterminado**: el span activo actual
<br />El span a anotar. Si `span` no se proporciona (como cuando se utilizan envolturas de función), el SDK anota el span activo actual.
...
Si un argumento es nulo o vacío, no sucede nada. Por ejemplo, si `inputData` es una cadena no vacía mientras `outputData` es nulo, entonces solo se registra `inputData`.
...
El método de miembro `setMetric()` de la interfaz `LLMObsSpan` acepta los siguientes argumentos para adjuntar una métrica única:
...
Para más información sobre etiquetas, consulta [Introducción a las Etiquetas][1].

#### Agregar etiquetas en bloque

El método de miembro `setTags()` de la interfaz `LLMObsSpan` acepta los siguientes argumentos para adjuntar múltiples etiquetas en bloque:

##### Argumentos

`tags`
: requerido - _Map<String, Object>_
<br /> Un mapa de pares clave-valor serializables en JSON que los usuarios pueden agregar como etiquetas para describir el contexto del span (por ejemplo, `session`, `environment`, `system` o `version`).

#### Agregar una sola etiqueta

El método de miembro `setTag()` de la interfaz `LLMObsSpan` acepta los siguientes argumentos para adjuntar una sola etiqueta:
...
`value`
: requerido - _entero_, _largo_, _doble_, _booleano_, o _Cadena_
...
El método `setMetadata()` de la interfaz `LLMObsSpan` acepta los siguientes argumentos:
...
### Anotando intervalos auto-instrumentados
...
`name`
: opcional - _str_
<br />Nombre que anula el nombre del intervalo para cualquier intervalo auto-instrumentado que se inicie dentro del contexto de anotación.

`prompt`
: opcional - _diccionario_
<br />Un diccionario que representa el aviso utilizado para una llamada a LLM. Consulta la documentación del objeto [Prompt](#prompt-tracking-arguments) para el esquema completo y las claves admitidas. También puedes importar el objeto `Prompt` de `ddtrace.llmobs.utils` y pasarlo como el argumento `prompt`. **Nota**: Este argumento solo se aplica a los intervalos LLM.

`tags`
: opcional - _diccionario_
<br />Un diccionario de pares clave-valor serializables en JSON que los usuarios pueden agregar como etiquetas en el intervalo. Ejemplo de claves: `session`, `env`, `system` y `version`. Para más información sobre etiquetas, consulta [Introducción a las Etiquetas](/getting_started/tagging/).
...
El `llmobs.annotationContext()` del SDK acepta una función de callback que se puede usar para modificar todos los intervalos auto-instrumentados iniciados mientras se está dentro del alcance de la función de callback.

El `llmobs.annotationContext()` método acepta las siguientes opciones en el primer argumento:
...
Adjunta metadatos estructurados del prompt al span del LLM para que puedas reproducir resultados, auditar cambios y comparar el rendimiento del prompt entre versiones. Al usar plantillas, la Observabilidad del LLM también proporciona [seguimiento de versiones](#version-tracking) basado en cambios en el contenido de la plantilla.

{{< tabs >}}
{{% tab "Python" %}}
Usa `LLMObs.annotation_context(prompt=...)` para adjuntar metadatos del prompt antes de la llamada al LLM. Para más detalles sobre la anotación de spans, consulta [Enriqueciendo spans](#enriching-spans).
...
- `chat_template` (List[Message]): Forma de plantilla de múltiples mensajes. Proporciona una lista de `{ "role": "<role>", "content": "<template string with placeholders>" }` objetos.
- `tags` (Dict[str, str]): Etiquetas para adjuntar a la ejecución del prompt.
- `rag_context_variables` (List[str]): Claves de variable que contienen contenido de verdad contextual. Utilizado para [detección de alucinaciones](/llm_observability/evaluations/managed_evaluations/?tab=openai#hallucination).
- `rag_query_variables` (List[str]): Claves de variable que contienen la consulta del usuario. Utilizado para [detección de alucinaciones](/llm_observability/evaluations/managed_evaluations/?tab=openai#hallucination).
...
Cuando utilizas la plantilla de prompts de LangChain con auto-instrumentación, asigna plantillas a variables con nombres significativos. La auto-instrumentación utiliza estos nombres para identificar los prompts.
...
Usa `llmobs.annotationContext({ prompt: ... }, () => { ... })` para adjuntar metadatos del prompt antes de la llamada al LLM. Para más detalles sobre la anotación de spans, consulta [Enriqueciendo spans](#enriching-spans).
...
- Usa un aviso único `id` para distinguir diferentes avisos dentro de tu aplicación.
- Mantén las plantillas estáticas utilizando la sintaxis de marcadores de posición (como `{{nombre_variable}}`) and define dynamic content in the `sección de variables`.
- Para múltiples llamadas de LLM auto-instrumentadas dentro de un bloque, usa un contexto de anotación para aplicar los mismos metadatos del aviso en todas las llamadas. Consulta [Anotando tramos auto-instrumentados](#annotating-auto-instrumented-spans).

### Seguimiento de versiones

La observabilidad de LLM proporciona versionado automático para tus avisos cuando no se especifica una versión explícita. Cuando proporcionas un `template` o `chat_template` en los metadatos de tu solicitud sin una etiqueta `version`, el sistema genera automáticamente una versión al calcular un hash del contenido de la plantilla. Si proporcionas una etiqueta `version`, LLM Observability utiliza la etiqueta de versión que especificaste en lugar de generar una automáticamente.
...
- **Historial de versiones**: Tanto las versiones generadas automáticamente como las manuales se mantienen en el historial de versiones para rastrear la evolución de la solicitud a lo largo del tiempo.
...
## Monitoreo de costos
Adjunta métricas de tokens (para seguimiento automático de costos) o métricas de costos (para seguimiento manual de costos) a tus intervalos de LLM/embedding. Las métricas de tokens permiten a Datadog calcular costos utilizando los precios del proveedor, mientras que las métricas de costos te permiten proporcionar tu propio precio al usar modelos personalizados o no soportados. Para más detalles, consulta [Costos][14].

Si estás utilizando instrumentación automática, las métricas de tokens y costos aparecen en tus intervalos automáticamente. Si estás instrumentando manualmente, sigue la guía a continuación.

<div class="alert alert-info">En este contexto, "métricas de tokens" y "métricas de costos" se refieren a pares clave-valor numéricos que adjuntas a los intervalos a través del parámetro <code>métricas</code> del método <code>LLMObs.annotate()</code>. Estas son distintas de <a href="/llm_observability/monitoring/metrics/">métricas de LLM Observability de la plataforma Datadog</a>. Para claves reconocidas como <code>input_tokens</code>, <code>output_tokens</code>, <code>input_cost</code>, y <code>output_cost</code>, Datadog utiliza estos atributos de intervalo para generar métricas correspondientes de la plataforma (como <code>ml_obs.span.llm.input.cost</code>) para su uso en tableros y monitores.</div>

{{< tabs >}}
{{% tab "Python" %}}

#### Caso de uso: Usando un proveedor de modelo común
Datadog soporta proveedores de modelos comunes como OpenAI, Azure OpenAI, Anthropic y Google Gemini. Al usar estos proveedores, solo necesitas anotar tu solicitud de LLM con `model_name`, `model_provider` y el uso de tokens. Datadog calcula automáticamente el costo estimado basado en el precio del proveedor.
...
Para modelos personalizados o no soportados, debes anotar el rango manualmente con los datos de costo.
...
El SDK de Observabilidad de LLM proporciona métodos para exportar y enviar tus evaluaciones a Datadog.
...
Las evaluaciones deben unirse a un solo rango. Puedes identificar el rango objetivo usando cualquiera de estos dos métodos:
- _Unión basada en etiquetas_ - Une una evaluación usando un par de clave-valor de etiqueta único que se establece en un solo rango. La evaluación fallará al unirse si el par de clave-valor de la etiqueta coincide con múltiples rangos o con ningún rango.
- _Referencia directa al rango_ - Une una evaluación usando la combinación del ID de traza único del rango y el ID del rango.

### Exportando un rango
{{< tabs >}}
{{% tab "Python" %}}
`LLMObs.export_span()` se puede usar para extraer el contexto del rango de un rango. Este método es útil para asociar tu evaluación con el rango correspondiente.
...
`span`
: opcional - _Rango_
<br />El rango del cual extraer el contexto del rango (IDs de rango y traza). Si no se proporciona (como al usar decoradores de función), el SDK exporta el rango activo actual.
...
{{% tab "Node.js" %}}
`llmobs.exportSpan()` se puede usar para extraer el contexto del rango de un rango. Necesitarás usar este método para asociar tu evaluación con el rango correspondiente.
...
`span`
: opcional - _Rango_
<br />El rango del cual extraer el contexto del rango (IDs de rango y traza). Si no se proporciona (como al usar envolturas de función), el SDK exporta el rango activo actual.
...
{{< tabs >}}
{{% tab "Python" %}}
`LLMObs.submit_evaluation()` se puede usar para enviar su evaluación personalizada asociada con un intervalo dado.
...
**Nota**: Las evaluaciones personalizadas son evaluadores que usted implementa y aloja por su cuenta. Estas difieren de las evaluaciones listas para usar, que son calculadas automáticamente por Datadog utilizando evaluadores integrados. Para configurar evaluaciones listas para usar para su aplicación, use la página [**LLM Observability** > **Configuraciones** > **Evaluaciones**][1] en Datadog.
...
`span`
: opcional - _diccionario_
<br />Un diccionario que identifica de manera única el rango asociado con esta evaluación. Debe contener `span_id` (cadena) y `trace_id` (cadena). Usa [`LLMObs.export_span()`](#exporting-a-span) para generar este diccionario.
...
`timestamp_ms`
: opcional - _entero_
<br />El timestamp unix en milisegundos cuando se generó el resultado de la métrica de evaluación. Si no se proporciona, esto se establece de forma predeterminada en la hora actual.

`tags`
: opcional - _diccionario_
<br />Un diccionario de pares clave-valor de cadena que los usuarios pueden agregar como etiquetas relacionadas con la evaluación. Para más información sobre etiquetas, consulta [Introducción a las Etiquetas](/getting_started/tagging/).
...
El `llmobs.submitEvaluation()` método acepta los siguientes argumentos:

{{% collapse-content title="Argumentos" level="h4" expanded=false id="submit-evals-arguments" %}}

`span_context`
: requerido - _diccionario_
<br />El contexto de intervalo para asociar la evaluación. Esta debería ser la salida de `LLMObs.export_span()`.
...
Utiliza `LLMObs.SubmitEvaluation()` para enviar tu evaluación personalizada asociada con un intervalo dado.
...
<br />El contexto de intervalo para asociar la evaluación.
...
Para modificar los datos de entrada y salida en spans, puedes configurar una función procesadora. La función procesadora tiene acceso a las etiquetas de los spans para permitir la modificación condicional de entrada/salida. Las funciones procesadoras pueden devolver el span modificado para emitirlo, o devolver `None`/`null` para evitar que el span sea emitido por completo. Esto es útil para filtrar spans que contienen datos sensibles o que cumplen ciertos criterios.
...
El SDK admite el trazado a través de servicios o hosts distribuidos. El trazado distribuido funciona propagando información de span a través de solicitudes web.

{{< tabs >}}
{{% tab "Python" %}}

La biblioteca `ddtrace` proporciona algunas integraciones listas para usar que admiten el trazado distribuido para populares [marcos web][1] y bibliotecas de [HTTP][2]. Si tu aplicación realiza solicitudes utilizando estas bibliotecas admitidas, puedes habilitar el trazado distribuido ejecutando:
{{< code-block lang="python">}}
from ddtrace import patch
patch(<INTEGRATION_NAME>=True)
{{< /code-block >}}

Si tu aplicación no utiliza ninguna de estas bibliotecas admitidas, puedes habilitar el trazado distribuido propagando manualmente la información de span hacia y desde los encabezados HTTP. El SDK proporciona los métodos auxiliares `LLMObs.inject_distributed_headers()` y `LLMObs.activate_distributed_headers()` para inyectar y activar contextos de trazado en los encabezados de solicitud.
...
<br />El span para inyectar su contexto en los encabezados de solicitud proporcionados. Cualquier span (incluyendo aquellos con decoradores de función), esto se establece de forma predeterminada en el span activo actual.

### Activando encabezados distribuidos

El método `LLMObs.activate_distributed_headers()` toma encabezados HTTP y extrae atributos de contexto de seguimiento para activar en el nuevo servicio.

**Nota**: Debes llamar a `LLMObs.activate_distributed_headers()` antes de iniciar cualquier span en tu servicio descendente. Los spans iniciados previamente (incluyendo spans de decoradores de función) no se capturan en el seguimiento distribuido.

Este método acepta el siguiente argumento:

`request_headers`
: requerido - _diccionario_
<br />Los encabezados HTTP para extraer atributos de contexto de seguimiento.
...
La biblioteca `dd-trace` proporciona integraciones listas para usar que soportan el seguimiento distribuido para populares [marcos web][1]. Requerir el rastreador habilita automáticamente estas integraciones, pero puedes desactivarlas opcionalmente con:
...
1. Inicia un span manualmente usando los mismos métodos (por ejemplo, el método `LLMObs.workflow` para un span de flujo de trabajo), pero como una llamada de función simple en lugar de como un administrador de contexto.
2. Pasa el objeto span como un argumento a otras funciones.
3. Detén el span manualmente con el método `span.finish()`. **Nota**: el span debe finalizarse manualmente, de lo contrario no se envía.
...
#### Fuerza el vaciado en entornos sin servidor.

`LLMObs.flush()` es una función bloqueante que envía todos los datos de observabilidad de LLM en búfer al backend de Datadog. Esto puede ser útil en entornos sin servidor para evitar que una aplicación salga hasta que se envíen todos los rastros de observabilidad de LLM.

### Rastreo de múltiples aplicaciones.

El SDK admite el rastreo de múltiples aplicaciones de LLM desde el mismo servicio.

Puedes configurar una variable de entorno `DD_LLMOBS_ML_APP` con el nombre de tu aplicación LLM, en la que se agrupan todos los spans generados por defecto.
...
El SDK de Observabilidad LLM de Node.js ofrece una `llmobs.decorate` función que sirve como decorador de funciones para aplicaciones de TypeScript. El comportamiento de seguimiento de esta función es el mismo que `llmobs.wrap`.
