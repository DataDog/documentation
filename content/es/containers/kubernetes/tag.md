...
description: Configura la extracción automática de etiquetas a partir de las etiquetas
  y anotaciones de los pods de Kubernetes para un mejor seguimiento.
further_reading:
...
---
El Agente de Datadog puede asignar automáticamente etiquetas a métricas, trazas y registros emitidos por un pod (o un contenedor individual dentro de un pod) según las etiquetas o anotaciones.
...
<div style="overflow-x: auto;">
  ...
  | `kube_app_part_of`            | Bajo          | Etiqueta del pod `app.kubernetes.io/part-of`                                                                                         | Pod label must exist                                |
  | `kube_app_managed_by`         | Bajo          | Etiqueta del pod `app.kubernetes.io/managed-by`                                                                                      | Pod label must exist                                |
  | `env`                         | Bajo          | Etiqueta del pod `tags.datadoghq.com/env` or container envvar (`DD_ENV` o `OTEL_RESOURCE_ATTRIBUTES`)                               | [Unified service tagging][2] enabled                |
  | `version`                     | Bajo          | Etiqueta del pod `tags.datadoghq.com/version` or container envvar (`DD_VERSION` o `OTEL_RESOURCE_ATTRIBUTES`)                       | [Unified service tagging][2] enabled                |
  | `service`                     | Bajo          | Etiqueta del pod `tags.datadoghq.com/service` or container envvar (`DD_SERVICE`, `OTEL_RESOURCE_ATTRIBUTES`, o `OTEL_SERVICE_NAME`) | [Unified service tagging][2] enabled                |
...
El Agente puede adjuntar información del entorno de Kubernetes como "etiquetas de servidor".
...
Como mejor práctica en entornos de contenedores, Datadog recomienda utilizar unified service tagging para ayudar a unificar las etiquetas. Unified service tagging vincula la telemetría de Datadog mediante el uso de tres etiquetas estándar: `env`, `service` y `version`. Para aprender a configurar su entorno con unified service tagging, consulte la documentación dedicada de [unified service tagging][2].
...
A partir de la versión v7.17+ del Agente, el Agente puede autodescubrir etiquetas a partir de las etiquetas de Docker. Este proceso permite al Agente asociar etiquetas personalizadas a todos los datos emitidos por un contenedor, sin modificar la configuración del Agente.

```yaml
com.datadoghq.ad.tags: '["<TAG_KEY>:TAG_VALUE", "<TAG_KEY_1>:<TAG_VALUE_1>"]'
```

A partir de la versión v7.77+, las anotaciones de etiquetas admiten [variables de plantilla de Autodiscovery][5] para el etiquetado dinámico basado en metadatos en tiempo de ejecución. Con la excepción de `%%env_<VAR>%%`, se admiten todas las variables de plantilla.
...
Para extraer una etiqueta de recurso dada `<LABEL>` y transformarla en una clave de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente variable de entorno a ** tanto a sus contenedores del Datadog Agent como del Cluster Agent.
...
Para Datadog Agent 7.73.0+ utilice la siguiente configuración para agregar todas las etiquetas de recurso como etiquetas a sus métricas. En este ejemplo, los nombres de las etiquetas están precedidos por `<PREFIX>_`:
...
Para extraer una anotación de recurso dada `<ANNOTATION>` y transformarla en claves de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente configuración a la configuración de su Operador de Datadog `DatadogAgent` en `datadog-agent.yaml`:
...
Para extraer una anotación de recurso dada `<ANNOTATION>` y transformarla en claves de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente variable de entorno a ** tanto a sus contenedores del Datadog Agent como del Cluster Agent.
...
Para Datadog Agent 7.73.0+ utilice la siguiente configuración para agregar todas las anotaciones de recurso como etiquetas a sus métricas. En este ejemplo, los nombres de las etiquetas están precedidos por `<PREFIX>_`:
...
Si utiliza Datadog Agent versión 7.58.0+, se recomienda utilizar <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">Etiquetas de recursos de Kubernetes como etiquetas</a> para configurar etiquetas de nodo como etiquetas.

</div>

A partir de Datadog Agent v6.0+, el agente puede recopilar etiquetas para un nodo dado y usarlas como etiquetas para adjuntar a todas las métricas, trazas y registros emitidos asociados con este `host` en Datadog:

{{< tabs >}}
{{% tab "Operador de Datadog" %}}
Para extraer una etiqueta de nodo dada `<NODE_LABEL>` y transformarla en una clave de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente configuración a la configuración de su Operador de Datadog `DatadogAgent` en `datadog-agent.yaml`:
...
Para Datadog Agent v7.24.0+ utilice la siguiente configuración de variable de entorno para agregar todas las etiquetas de nodo como etiquetas a sus métricas. En este ejemplo, los nombres de las etiquetas están precedidos por `<PREFIX>_`:
...
Si utiliza Datadog Agent versión 7.58.0+, se recomienda utilizar <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">Etiquetas de recursos de Kubernetes como etiquetas</a> para configurar etiquetas de pod como etiquetas.

</div>

A partir de Datadog Agent v6.0+, el agente puede recopilar etiquetas para un pod dado y usarlas como etiquetas para adjuntar a todas las métricas, trazas y registros emitidos por este pod:

{{< tabs >}}
{{% tab "Operador de Datadog" %}}
Para extraer una etiqueta de pod dada `<POD_LABEL>` y transformarla como una clave de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente configuración a la configuración de su Operador de Datadog `DatadogAgent` en `datadog-agent.yaml`:
...
Para Datadog Agent v7.24.0+ utilice la siguiente configuración de variable de entorno para agregar todas las etiquetas de pod como etiquetas a sus métricas. En este ejemplo, los nombres de las etiquetas están precedidos por `<PREFIX>_`:
...
**Nota**: Custom Metrics pueden afectar la facturación. Consulte la [página de facturación de Custom Metrics][3] para más información.
...
A partir de Agent 7.55.0+, Agent puede recopilar etiquetas para un espacio de nombres dado y usarlas como etiquetas para adjuntar a todas las métricas, trazas y registros emitidos por todos los pods en este espacio de nombres:

{{< tabs >}}
{{% tab "Datadog Operator" %}}
Para extraer una etiqueta de espacio de nombres dada `<NAMESPACE_LABEL>` y transformarla como una clave de etiqueta `<TAG_KEY>` dentro de Datadog, agregue la siguiente configuración a la configuración de su Datadog Operator `DatadogAgent` en `datadog-agent.yaml`:
...
{{% tab "Manual (DaemonSet)" %}}
Para extraer una etiqueta de contenedor dada `<CONTAINER_LABEL>` y transformarla como una clave de etiqueta `<TAG_KEY>`, agregue la siguiente variable de entorno al Agente de Datadog:
