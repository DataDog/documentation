...
title: Seguimiento del plano de control de Kubernetes
---
## Resumen

Esta sección tiene como objetivo documentar especificidades y proporcionar buenas configuraciones base para el seguimiento del plano de control de Kubernetes. Luego puedes personalizar estas configuraciones para agregar cualquier característica de Datadog.
...
Al proporcionar acceso de lectura a los certificados de Etcd ubicados en el host, la verificación del Agente de Datadog puede comunicarse con Etcd y comenzar a recopilar métricas de Etcd.
...
Si los puertos inseguros de tus instancias de administrador de controladores y programador están habilitados, el Agente de Datadog descubre las integraciones y comienza a recopilar métricas sin ninguna configuración adicional.

#### Puertos seguros

Los puertos seguros permiten la autenticación y autorización para proteger los componentes de tu plano de control. El Agente de Datadog puede recopilar métricas del administrador de controladores y del programador dirigiéndose a sus puertos seguros.
...
- El campo `ssl_verify` en la configuración de `kube_controller_manager` y `kube_scheduler` debe establecerse en `false` al usar certificados autofirmados.
...
Datadog admite el seguimiento de los componentes del plano de control de Kubernetes, incluidos el servidor API, etcd, el administrador de controladores y el programador.
...
El seguimiento del plano de control está habilitado por defecto, pero requiere que la introspección esté habilitada.
...
Habilita el seguimiento del plano de control utilizando la opción `providers.eks.controlPlaneMonitoring`:
...
Amazon Elastic Kubernetes Service (EKS) admite el seguimiento de todos los componentes del plano de control utilizando verificaciones de clúster.
...
Puedes habilitar la introspección utilizando el [chart de Helm datadog-operator][12]:
...
O, para **usuarios de OpenShift** que instalaron el operador a través de OperatorHub/Marketplace (el [método recomendado](install-openshift.md)), parcheando la versión del servicio del clúster del operador:
...
Habilita `features.clusterChecks.useClusterChecksRunners` para programar las verificaciones allí; de lo contrario, las verificaciones del plano de control se ejecutan en el Node Agent.

Para OpenShift 4.14 y versiones posteriores, la supervisión de etcd requiere que copie los certificados de etcd. Revisa los registros del operador para conocer el comando exacto. Consulta el siguiente ejemplo (ajusta el espacio de nombres según sea necesario):
...
Habilita el seguimiento del plano de control utilizando la opción `providers.openshift.controlPlaneMonitoring`:
...
Para OpenShift 4.14 y versiones posteriores, la supervisión de etcd requiere que copies los certificados de etcd. Para copiarlos en el mismo espacio de nombres que el Datadog Agent:
...
En OpenShift 4, todos los componentes del plano de control se pueden supervisar utilizando verificaciones de punto de conexión.

#### Requisitos previos

1. Habilita el [Datadog Cluster Agent][6]
1. Habilita [verificaciones de clúster][7]
1. Habilita [verificaciones de punto de conexión][8]
...
La última anotación `ad.datadoghq.com/endpoints.resolve` es necesaria porque el servicio está frente a los pods estáticos. El Datadog Cluster Agent programa las verificaciones de punto de conexión y las despacha a los Cluster Check Runners. Los nodos en los que se están ejecutando pueden ser identificados con:
...
Se necesitan certificados para comunicarse con el servicio Etcd, que se pueden encontrar en el secreto `kube-etcd-client-certs` en el espacio de nombres `openshift-monitoring`. Para dar acceso al Agente de Datadog a estos certificados, primero cópielos en el mismo espacio de nombres en el que se está ejecutando el Agente de Datadog:
...
Estos certificados deben ser montados en los pods Cluster Check Runner agregando los volumes y volumeMounts como se indica a continuación.
...
El Datadog Cluster Agent programa los checks como endpoint checks y los despacha a los Cluster Check Runners.
...
Se necesitan certificados para comunicarse con el servicio Etcd, que se pueden encontrar en el secreto `etcd-metric-client` en el espacio de nombres `openshift-etcd-operator`. Para dar acceso al Datadog Agent a estos certificados, cópielos en el mismo espacio de nombres en el que se está ejecutando el Datadog Agent:
...
**Nota**: También se incluyen montajes para deshabilitar el archivo de autoconfiguración del check de Etcd empaquetado con el Datadog Agent.
...
El Administrador de Controladores se ejecuta detrás del servicio `kube-controller-manager` en el espacio de nombres `openshift-kube-controller-manager`. Anote el servicio con la configuración de verificación:
...
El Programador se ejecuta detrás del servicio `scheduler` en el espacio de nombres `openshift-kube-scheduler`. Anote el servicio con la configuración de verificación:
...
En OpenShift 3, todos los componentes del plano de control pueden ser monitoreados utilizando verificaciones de punto final.

### Requisitos previos

1. Habilite el [Agente de Clúster de Datadog][6]
1. Habilitar [Verificaciones de clúster][7]
1. Habilitar [Verificaciones de punto de conexión][8]
...
La última anotación `ad.datadoghq.com/endpoints.resolve` es necesaria porque el servicio está frente a los pods estáticos. El Datadog Cluster Agent programa los checks como endpoint checks y los despacha a los Cluster Check Runners. Los nodos en los que se están ejecutando pueden ser identificados con:
...
Se necesitan certificados para comunicarse con el servicio Etcd, que se encuentran en el servidor. Estos certificados deben montarse en los pods Cluster Check Runner agregando los volumes y volumeMounts como se indica a continuación.
...
Al proporcionar acceso de lectura a los certificados etcd ubicados en el servidor, el Datadog Agent check puede comunicarse con etcd y comenzar a recopilar métricas de etcd.
...
Los puertos seguros permiten la autenticación y autorización para proteger los componentes de su Plano de Control. El Datadog Agent puede recopilar métricas del Administrador de Controladores y del Programador dirigiéndose a sus puertos seguros.
...
- Al apuntar a puertos seguros, la opción `bind-address` en la configuración de su Administrador de Controladores y Programador debe ser accesible por el Datadog Agent. Aplique el parche a continuación a los nodos de plano de control en la generación del clúster; o, para nodos Talos en ejecución, ejecute `talosctl patch mc -n <control-plane-node1,control-plane-node2> --patch @controlplane-datadog-monitoring-patch.yaml`.
...
Rancher v2.5 se basa en [PushProx][9] para exponer los puntos de conexión de métricas del plano de control, lo que permite que el Datadog Agent ejecute control plane checks y recopile métricas.
...
Para configurar el check `kube_apiserver_metrics`, agregue las siguientes anotaciones al servicio `default/kubernetes`:
...
### Agregue servicios de Kubernetes para configurar Autodiscovery checks
...
Puede anotar este servicio con endpoint checks (gestionados por el Datadog Cluster Agent) para monitorear el Servidor API, el Administrador de Controladores y el Programador:
...
2. Usando la información de montaje disponible en la salida `$ docker inspect etcd`, configure `volumes` y `volumeMounts` en la configuración del Datadog Agent. También incluya tolerancias para que el Datadog Agent pueda ejecutarse en los nodos del plano de control.
...
3. Configure un DaemonSet con un contenedor pause para ejecutar la verificación de Etcd en los nodos que ejecutan Etcd. Este DaemonSet se ejecuta en la host network para que pueda acceder al servicio Etcd. También tiene la configuración de verificación y las tolerancias necesarias para ejecutarse en el(los) nodo(s) del plano de control. Asegúrese de que las rutas de los archivos de certificado montados coincidan con lo que configuró en su instancia y reemplace la parte `<...>` en consecuencia.
