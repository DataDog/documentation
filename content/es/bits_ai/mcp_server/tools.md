...
Para habilitar herramientas específicas del producto, incluye el parámetro de consulta `toolsets` al final de la URL del punto de conexión que utilizas para conectarte al Servidor MCP de Datadog. Por ejemplo, según el [sitio de Datadog][2] que seleccionaste ({{< region-param key="dd_site_name" >}}), esta URL habilita _solo_ herramientas de Observabilidad APM y LLM:
...
El conjunto de herramientas predeterminado para registros, métricas, trazas, tableros, monitores, incidentes, servidores, servicios, eventos y notebooks.
...
- Muéstrame las métricas de utilización de CPU para todos los servidores en las últimas 4 horas.
...
Obtiene una traza completa de Datadog APM utilizando un ID de traza.
...
**Nota**: Esta herramienta lista tableros relevantes pero proporciona detalles limitados sobre su contenido. Usa `get_datadog_dashboard` para recuperar definiciones completas de widgets.
...
Recupera las dependencias de servicio (ascendentes/descendentes) y los servicios que son propiedad de un equipo.

- Muéstrame todos los servicios ascendentes que llaman al checkout service.
- ¿De qué servicios aguas abajo depende la API de pagos?
- Enumera todos los servicios que son propiedad del equipo de plataforma.

### `search_datadog_spans` {#search-datadog-spans}
*Conjunto de herramientas: **core***\
*Permisos Requeridos: `APM Read`*\
Recupera spans de trazas de APM con filtros como servicio, tiempo, recurso, etc.

- Muéstrame spans con errores del servicio de checkout.
- Encuentra consultas lentas a la base de datos en los últimos 30 minutos.
- Obtén los spans para las solicitudes de API fallidas a nuestro servicio de pagos.
...
- Encuentra registros que contengan 'tiempo de conexión agotado' de nuestro servicio API.
...
- Crea un nuevo notebook para nuestra revisión semanal de rendimiento.
...
- Crea un monitor para rastrear la latencia p95 del punto de conexión de checkout.

### `get_monitor_coverage` {#get-monitor-coverage}
*Conjunto de herramientas: **Alerting***\
*Permisos requeridos: `Monitors Read`*\
Encuentra brechas y cobertura de monitoreo para servicios o servidores. Devuelve qué señales (como la tasa de errores, latencia y tasa de solicitudes) están cubiertas por los monitores existentes y cuáles faltan. Utiliza con `create_datadog_monitor` para llenar brechas.
...
<div class="alert alert-info">El <code>apm</code> El conjunto de herramientas está en vista previa. <a href="https://www.datadoghq.com/product-preview/apm-mcp-toolset/">Regístrate para acceder.</a></div>

### `apm_search_spans` {#apm-search-spans}
*Conjunto de herramientas: **apm***\
*Permisos requeridos: `APM Read`*\
Busca spans utilizando la sintaxis de consulta APM, con soporte para paginación y filtrado de etiquetas.

- Muéstrame los spans con errores del servicio de checkout en la última hora.
- Encuentra consultas de base de datos lentas que tarden más de 2 segundos.
- Busca spans con `service:payments` y `status:error`.

### `apm_explore_trace` {#apm-explore-trace}
*Conjunto de herramientas: **apm***\
*Permisos requeridos: `APM Read`*\
Ejecuta consultas sobre datos de trazas para un análisis profundo y exploración de spans específicos dentro de una traza.

- Explora los spans en la traza `abc123` y muéstrame las llamadas a la base de datos.
- Analiza los spans de error en esta traza.
...
*Conjunto de herramientas: **apm***\
*Permisos requeridos: `APM Read`*\
Analiza las métricas de trazas de APM a lo largo del tiempo para una operación específica, consultando datos métricos y proporcionando análisis generados por IA.
...
- Muéstrame las anomalías de Watchdog para el servicio de pago en las últimas 24 horas.
...
*Conjunto de herramientas: **apm***\
Busca historias de cambios (despliegues, banderas de características y cambios en la infraestructura) para un servicio dentro de un rango de tiempo.

- Muéstrame los despliegues y cambios recientes para el servicio de pagos.
- ¿Qué cambios en la infraestructura ocurrieron alrededor del momento de este pico de latencia?
...
- ¿Cuáles son los cuellos de botella de latencia para el servicio de pago durante esta anomalía?
...
- Compara las etiquetas de tramo entre la ventana de anomalía y la base para encontrar qué cambió.
- ¿Qué valores de etiqueta son diferentes durante este pico de latencia?
...
- Guíame a través de la depuración de un aumento de errores en producción.
...
Busca [Case Management][38] casos con filtros que incluyen estado, prioridad, proyecto y asignado. Soporta filtrado por rango de tiempo y paginación.

- Muéstrame todos los casos abiertos asignados a mí.
- ¿Hay algún caso P1 abierto en el proyecto de Revisiones de Seguridad?
- Muéstrame todos los casos abiertos esta semana relacionados con el servicio de pagos.
...
- Estoy viendo un aumento de latencia en el servicio de pago. Crea una incidencia P2 para rastrear la investigación.
...
- Este problema ahora afecta al cliente. Escala la incidencia CASE-1234 a P1.
- Marca la incidencia de migración de base de datos como resuelta.
- Establece una fecha de vencimiento para el final de la semana en la INCIDENCIA-1234.
...
*Conjunto de herramientas: **core**, **tableros***\
*Permisos requeridos: `Dashboards Read` y `User Access Read`*\
...
- Muéstrame los widgets y el diseño del tablero de visión general de infraestructura.
...
- Muéstrame cómo construir un widget de lista superior y un widget de tabla de consultas.
...
- ¿Qué formato de respuesta debo usar para una lista superior?
- ¿Cuál es el esquema para el widget de gráfico de dispersión?
- Ayúdame a depurar por qué este widget está mostrando valores fraccionarios cuando debería ser un conteo.
- ¿Cómo configuro una serie temporal para mostrar tanto barras como líneas?
...
Descubre y clasifica instancias de base de datos para la investigación de DBM. Llama a esto antes de otras herramientas de DBM que requieran un parámetro `database_instance`. Acepta una traza de APM o un ID de tramo, etiquetas, o ambos para encontrar instancias coincidentes, luego evalúa y clasifica su salud.
...
*Conjunto de herramientas: **dbm***\
*Permisos requeridos: `Database Monitoring Read`*\
Identifica servicios y recursos de APM aguas arriba que realizan consultas a la base de datos. Correlaciona la actividad de la base de datos con trazas de aplicación para el análisis de causa raíz a través de la frontera APM-base de datos.
...
*Conjunto de herramientas: **dbm***\
*Permisos requeridos: `Database Monitoring Read`*\
Recupera recomendaciones en vivo para una base de datos, consulta, tabla, servidor o índice. Devuelve las recomendaciones coincidentes con estado, severidad y un bloque de alcance normalizado que resalta las instancias afectadas, firmas de consulta, tablas, índices, servicios, planes e identificadores de infraestructura.
...
*Conjunto de herramientas: **dbm***\
*Permisos requeridos: `Database Monitoring Read`*\
Busca [Monitoreo de Base de Datos][26] muestras de consultas, que representan ejecuciones individuales de consultas con métricas de rendimiento. Utiliza esto para analizar patrones de actividad de la base de datos, identificar consultas lentas e investigar problemas de rendimiento de la base de datos.
...
- ¿Qué funciones SQL están soportadas en DDSQL?
...
*Conjunto de herramientas: **ddsql***\
Ejecuta una consulta DDSQL y devuelve resultados. Soporta el uso de sintaxis SQL para consultar recursos de infraestructura, registros, métricas, RUM, spans y otras fuentes de datos de Datadog. Consulta la [Referencia de DDSQL][42] para obtener detalles de sintaxis.
...
Herramientas para gestionar Feature Flags, incluyendo la creación, listado y actualización de flags y sus entornos.
...
- Muéstrame los entornos de las Feature Flags disponibles.
- ¿Qué entornos puedo dirigir con las Feature Flags?
...
- Habilita la Feature Flag `dark-mode` en el entorno de staging.
...
Sincroniza las asignaciones de una Feature Flag para un entorno específico.
...
Busca recursos de [Kubernetes][55] en todos los clústeres. Utiliza esta herramienta en lugar de `kubectl` para determinar el estado de los recursos de Kubernetes, como implementaciones, pods, nodos, etc. Esta herramienta no requiere acceso local al clúster, funciona en todos los clústeres y devuelve datos enriquecidos con etiquetas. Puedes incluir claves de etiqueta específicas en cada resultado e incluir nombres de recursos padres para investigar las relaciones entre recursos (por ejemplo, la implementación a la que pertenece un pod).
...
- Lista los estados de las interfaces para mi enrutador principal.

## Integración {#onboarding}

Herramientas de integración orientadas al agente para la configuración y puesta en marcha guiada de Datadog.
...
- Ayúdame a configurar Test Optimization para mi canalización de CI.
...
Herramientas para gestionar [Tablas de referencia][45], incluyendo listar tablas, leer filas, agregar filas y crear tablas desde almacenamiento en la nube.
...
*Conjunto de herramientas: **seguridad***\
*Permisos requeridos: `Security Signals Read`*\
Recupera los detalles completos de una señal de seguridad individual por ID, incluyendo atributos, información de reglas, estado de triage, etiquetas y correlaciones de incidencias.

- Obtén los detalles completos de la señal de seguridad `AwAAAZ27F1BUjY4rPQAAABhBWjI3RjFCVWpZNHJBQUFBSGFNQVZBQUFBR1Bu`.
- Muéstrame la regla, el estado de triage y las incidencias vinculadas para esta señal.
...
- Muéstrame el esquema para hallazgos de vulnerabilidades de bibliotecas.
...
*Conjunto de herramientas: **seguridad***\
*Permisos requeridos: `Security Monitoring Findings Read`*\
Herramienta de respaldo para recuperar detalles completos de hallazgos de seguridad. Prefiere `analyze_security_findings` para la mayoría de las tareas de análisis. Usa esta herramienta solo cuando necesites objetos de hallazgos completos o cuando las consultas SQL sean insuficientes.
...
- Encuentra pruebas inestables activas para el servicio de checkout propiedad de `@team-abc`, ordenadas por tasa de fallos.
...
- Muéstrame todas las ejecuciones de pruebas inestables para el servicio de checkout.
...
Obtiene métricas resumidas de cobertura de código agregadas para una rama de repositorio, incluyendo cobertura total, cobertura de parches y desgloses de servicio/propietario de código.
...
*Conjunto de herramientas: **entrega de software***\
*Permisos requeridos: `Code Coverage read`*\
Obtiene métricas resumidas de cobertura de código agregadas para un commit de repositorio, incluyendo cobertura total, cobertura de parches y desgloses de servicio/propietario de código.
...
Herramientas para interactuar con Datadog [Pruebas sintéticas][47].

### `get_synthetics_tests` {#get-synthetics-tests}
*Conjunto de herramientas: **sintéticos***\
*Permisos requeridos: `Synthetics Read`*\
Busca pruebas sintéticas de Datadog.

- Ayúdame a entender por qué la prueba sintética en el punto de conexión `/v1/my/tested/endpoint` está fallando.
- Hay una interrupción; encuentra todas las pruebas sintéticas que están fallando en el dominio `api.mycompany.com`.
- ¿Las pruebas sintéticas en mi sitio web `api.mycompany.com` todavía están funcionando en la última hora?
...
Edita las pruebas HTTP API sintéticas de Datadog.

- Mejora las afirmaciones de la prueba sintética definida en mi punto de conexión `/v1/my/tested/endpoint`.
...
*Conjunto de herramientas: **sintéticos***\
*Permisos requeridos: `Synthetics Global Variable Read` y `Synthetics Read` y `Synthetics Write`*\
Previsualiza y crea pruebas HTTP API sintéticas de Datadog.

- Crea pruebas sintéticas en cada punto de conexión definido en este archivo de código.
- Crea una prueba sintética en `/path/to/endpoint`.
- Crea una prueba sintética que verifique si mi dominio `mycompany.com` permanece activo.

## Flujos de trabajo {#workflows}

Herramientas para [Workflow Automation][39], incluyendo listar, inspeccionar, ejecutar y configurar flujos de trabajo para uso de agente.
...
- Encuentra todos los flujos de trabajo relacionados con la respuesta a incidentes pertenecientes a Alice Smith.
...
- Muéstrame los parámetros de entrada y pasos para el flujo de trabajo de reversión de implementación.
...
- Dispara el flujo de trabajo de notificación de guardia con el contexto de esta investigación.
...
- ¿Cuál es el estado de la ejecución del flujo de trabajo que disparé?
