...
Dirige tu agente de IA al punto final del Servidor MCP para tu [sitio de Datadog][1] regional. Para las instrucciones correctas, utiliza el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar tu sitio.

{{< site-region region="us,us3,us5,eu,ap1,ap2" >}}
Endpoint seleccionado ({{< region-param key="dd_site_name" >}}): <code>{{< region-param key="mcp_server_endpoint" >}}</code>.
...
1. Para habilitar herramientas [específicas de producto](#toolsets), incluye el parámetro de consulta `toolsets` al final de la URL del endpoint. Por ejemplo, esta URL habilita herramientas de Observabilidad APM y LLM _solamente_ (usa `toolsets=all` para habilitar todos los conjuntos de herramientas generalmente disponibles, lo mejor para clientes que soportan filtrado de herramientas):

   <pre><code>{{< region-param key="mcp_server_endpoint" >}}?toolsets=apm,llmobs</code></pre>

1. Verifica que tienes los [permisos](#required-permissions) requeridos para los recursos de Datadog a los que deseas acceder.

<div class="alert alert-info">Si la autenticación remota no está disponible, utiliza <a href="#local-binary-authentication">la autenticación binaria local</a> en su lugar.</div>
...
Conecta a Claude (incluyendo a Claude Cowork) al Servidor Datadog MCP añadiéndolo como un {{< ui >}}custom connector{{< /ui >}} con la URL remota de MCP.
...
1. Cuando se te pida una URL, ingresa el endpoint del Servidor Datadog MCP para tu [sitio de Datadog][2] ({{< region-param key="dd_site_name" >}}). Para las instrucciones correctas, utiliza el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar tu sitio.
...
{{< site-region region="gov" >}}
<div class="alert alert-danger">El Servidor Datadog MCP no es compatible con tu <a href="/getting_started/site/">sitio de Datadog</a> seleccionado ({{< region-param key="dd_site_name" >}}).</div>
{{< /site-region >}}
...
1. Edita `~/.codex/config.toml` (o tu archivo de configuración de Codex CLI) para añadir el Servidor Datadog MCP con transporte HTTP y la URL del endpoint para tu sitio. Por ejemplo:
...
1. Inicie sesión en el servidor MCP de Datadog:

   ```shell
   codex mcp login datadog
   ```

   Esto abre su navegador para completar el flujo de OAuth. Codex almacena las credenciales resultantes para que no necesite iniciar sesión nuevamente hasta que el token expire.
...
Dirige tu agente de IA al punto final del Servidor MCP para tu [sitio de Datadog][1] regional. Para las instrucciones correctas, use el selector **Sitio de Datadog** en el lado derecho de esta página de documentación para seleccionar su sitio.
...
1. Ejecutar en la terminal:
   ...
   Alternativamente, añade a `~/.gemini/settings.json`:
    <pre><code>{
      "mcpServers": {
        "datadog": {
...
[Warp][1] es un terminal agente con soporte MCP integrado. Apunte el agente Warp al punto final del servidor MCP para su [sitio de Datadog][2] regional. Para las instrucciones correctas, utiliza el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar tu sitio.
...
1. Haga clic en {{< ui >}}Start{{< /ui >}} en el servidor de Datadog. Warp abre su navegador para completar el flujo de inicio de sesión de OAuth. Las credenciales se almacenan de forma segura en su dispositivo y se reutilizan para sesiones futuras.
...
{{< site-region region="us,us3,us5,eu,ap1,ap2" >}}
1. Instale la extensión (omite `--profile` y el nombre del perfil para instalar en el perfil predeterminado de VS Code):
    ```shell
...
1. Inicia sesión en tu cuenta de Datadog.
1. **Reinicie el IDE.**
1. Confirme que el servidor MCP de Datadog está disponible y que las [herramientas][3] están listadas: Abra el panel de chat, seleccione el modo agente y haga clic en el botón {{< ui >}}Configure Tools{{< /ui >}}.
...
Dirige tu plugin al punto final del Servidor MCP para tu [sitio de Datadog][3] regional. Para las instrucciones correctas, utiliza el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar tu sitio.
...
{{% collapse-content title="Agentes CLIs" level="h4" expanded=false id="jetbrains-agent-clis" %}}
Muchos desarrolladores utilizan un agente CLI como Claude Code, Codex o Gemini CLI junto a su IDE de JetBrains. Consulta la configuración para esas herramientas CLI:
- [Claude Code][4]
- [Codex][5]
- [Gemini CLI][6]

El [plugin de Datadog para IDEs de JetBrains][3] se integra con estos agentes CLIs. Para una experiencia ininterrumpida, instala el plugin al mismo tiempo que configuras el Servidor MCP de Datadog.
...
Dirija su agente de IA al punto final del servidor MCP para su [sitio de Datadog][3]. Para las instrucciones correctas, utiliza el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar tu sitio.

{{< site-region region="us,us3,us5,eu,ap1,ap2" >}}
Endpoint seleccionado ({{< region-param key="dd_site_name" >}}): <code>{{< region-param key="mcp_server_endpoint" >}}</code>.

1. Agregue lo siguiente a su [archivo de configuración de Kiro MCP][2] (`~/.kiro/settings/mcp.json` para configuración específica del usuario):
...
Para la mayoría de los otros [clientes soportados](#supported-clients), use estas instrucciones para la autenticación remota. Para Cline o cuando la autenticación remota no es confiable o no está disponible, use [autenticación binaria local](#local-binary-authentication).

Dirige tu agente de IA al punto final del Servidor MCP para tu [sitio de Datadog][1] regional. Para las instrucciones correctas, use el selector {{< ui >}}Datadog Site{{< /ui >}} en el lado derecho de esta página de documentación para seleccionar su sitio.

{{< site-region region="us,us3,us5,eu,ap1,ap2" >}}
Endpoint seleccionado ({{< region-param key="dd_site_name" >}}): <code>{{< region-param key="mcp_server_endpoint" >}}</code>.

1. Agregue el servidor MCP de Datadog al archivo de configuración de su cliente utilizando el transporte HTTP y la URL del punto final de su sitio. Por ejemplo:
...
El servidor MCP de Datadog soporta _conjuntos de herramientas_, que le permiten usar solo las [herramientas MCP][49] que necesita, ahorrando valioso espacio en la ventana de contexto. Para usar un conjunto de herramientas, incluya el parámetro de consulta `toolsets` en la URL del punto final al conectarse al servidor MCP ([autenticación remota](#authentication) solamente). Use `toolsets=all` para habilitar todos los conjuntos de herramientas generalmente disponibles a la vez.
...
- Recupere solo herramientas relacionadas con pruebas sintéticas:
  <pre><code>{{< region-param key="mcp_server_endpoint" >}}?toolsets=synthetics</code></pre>

- Recupere herramientas básicas, de pruebas sintéticas y de entrega de software:
...
Estos conjuntos de herramientas están generalmente disponibles. Consulte [Datadog MCP Server Tools][49] para una referencia completa de las herramientas disponibles organizadas por conjunto de herramientas, con ejemplos de indicaciones.
...
- `dashboards`: Herramientas para recuperar, crear, actualizar y eliminar [paneles][46], además de referencia y validación de esquemas de widgets.
- `dbm`: Herramientas para interactuar con [Monitoreo de Bases de Datos][33].
- `ddsql`: Herramientas para consultar datos de Datadog utilizando [DDSQL][44], un dialecto SQL con soporte para recursos de infraestructura, registros, métricas, RUM, spans y otras fuentes de datos de Datadog.
- `error-tracking`: Herramientas para interactuar con [Seguimiento de Errores][32] de Datadog.
...
- `llmobs`: Herramientas para buscar y analizar spans y experimentos de [Observabilidad de LLM][36].
- `networks`: Herramientas de incorporación agentiva para la configuración y configuración guiada de Datadog.
- `onboarding`: Herramientas para interactuar con consultas de [Analítica de Productos][41].
- `product-analytics`: Herramientas para gestionar [Tablas de Referencia][48], que incluyen listar tablas, leer filas, agregar filas y crear tablas desde almacenamiento en la nube.
- `reference-tables`: Herramientas para escaneo de seguridad de código y búsqueda de [señales de seguridad][39] y [hallazgos de seguridad][40].
- `security`: Herramientas para interactuar con la Entrega de Software ([Visibilidad de CI][30] y [Optimización de Pruebas][31]).
- `software-delivery`: Herramientas para interactuar con [Pruebas Sintéticas][29] de Datadog.
- `synthetics`: Herramientas para [Automatización de Flujos de Trabajo][43], que incluyen listar, inspeccionar, ejecutar y configurar flujos de trabajo para uso de agentes.
- `workflows` Vista previa de conjuntos de herramientas .

Estos conjuntos de herramientas están en Vista previa.

 Regístrate para un conjunto de herramientas completando el formulario de Vista previa del producto o contacta a [soporte de Datadog][47] para solicitar acceso.
- `apm`: ([Regístrate][45]) Herramientas para un análisis de trazas [APM][34], búsqueda de spans, información de Watchdog e investigación de rendimiento.
...
| [Claude][19] | Anthropic | Utiliza [ la configuración del conector personalizado ](?tab=claude#installation). Incluye Claude Cowork. |
| [Codex CLI][6] | OpenAI | |
...
| [Goose][8], [Cline][11] | Varios | Consulta la {{< ui >}}Other{{< /ui >}} pestaña arriba. Utiliza autenticación binaria local para Cline si la autenticación remota no es confiable. |

<div class="alert alert-info">El servidor MCP de Datadog está en desarrollo significativo, y pueden estar disponibles clientes adicionales soportados.</div>
...
Además de `mcp_read` o `mcp_write`, los usuarios necesitan los permisos estándar de Datadog para el recurso subyacente. Por ejemplo, usar una herramienta MCP que lee monitores requiere tanto `mcp_read` como el permiso [Monitors Read][24]. Consulta [Datadog Role Permissions][25] para la lista completa de permisos a nivel de recurso.

Los usuarios con el **Rol Estándar de Datadog** tienen ambos permisos de MCP Server por defecto. Si tu organización utiliza [roles personalizados][23], agrega los permisos manualmente:
...
El servidor MCP utiliza OAuth 2.0 para [autenticación][14]. Si no puedes pasar por el flujo de OAuth (por ejemplo, en un servidor), puedes proporcionar una [clave de API de Datadog y clave de aplicación][1] como `DD_API_KEY` y `DD_APPLICATION_KEY` encabezados HTTP.
...
<pre><code>{
  "mcpServers": {
    "datadog": {
      ...
      "headers": {
          "DD_API_KEY": "&lt;YOUR_API_KEY&gt;",
          "DD_APPLICATION_KEY": "&lt;TU_CLAVE_DE_APLICACIÓN&gt;"
      }
    }
  }
}
...
Por seguridad, utiliza una clave de API y clave de aplicación con alcance de una [cuenta de servicio][13] que tenga solo los permisos requeridos.

### Autenticación binaria local {#local-binary-authentication}

Se recomienda la autenticación local para Cline y cuando la autenticación remota es poco confiable o no está disponible. Después de la instalación, normalmente no necesitas actualizar el binario local para beneficiarte de las actualizaciones del servidor MCP, ya que las herramientas son remotas.
...
3. Para {{< ui >}}URL{{< /ui >}}, ingresa el punto final del servidor MCP para tu sitio regional de Datadog. 
   {{< site-region region="us,us3,us5,eu,ap1,ap2" >}}
...
5. Verifica si aparecen las [herramientas disponibles][12].
