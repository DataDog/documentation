...
description: Conecte agentes de IA a los datos de observabilidad de Datadog utilizando
  el Servidor MCP para consultar métricas, registros, trazas y otra información relevante.
further_reading:
...
---
El Servidor MCP de Datadog actúa como un puente entre sus datos de observabilidad en Datadog y cualquier agente de IA que sea compatible con el [MCP][1]. Proporcionando acceso estructurado a contextos, características y herramientas relevantes de Datadog, el Servidor MCP le permite consultar y recuperar información de observabilidad directamente desde clientes impulsados por IA como Cursor, OpenAI Codex, Claude Code o su propio agente de IA.
...
Esta demostración muestra cómo se utiliza el Servidor MCP de Datadog en Cursor y Claude Code (active el audio):
...
- `datadog.mcp.tool.usage`: Una métrica de distribución emitida en cada llamada a herramienta.
...
Debido a que `datadog.mcp.tool.usage` es una métrica de distribución, utiliza `count` (no `sum`) con `.as_count()` para obtener el número de llamadas a herramientas. Por ejemplo, para consultar el número total de llamadas a herramientas agrupadas por correo electrónico de usuario:
...
El Servidor MCP de Datadog está optimizado para ofrecer respuestas que proporcionen a los agentes de IA el contexto relevante sin sobrecargarlos de información innecesaria. Por ejemplo:
...
El Servidor MCP de Datadog está en pleno desarrollo. Utilice [este formulario de comentarios][19] para compartir cualquier comentario, caso de uso o problemas encontrados con sus solicitudes y consultas.
