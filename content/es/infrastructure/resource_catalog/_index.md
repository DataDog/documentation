...
- link: /security/cloud_security_management/misconfigurations/
  tag: Documentación
  text: Errores de configuración de seguridad en la nube
- link: /security/threats/
  tag: Documentación
  text: Workload Protection
- link: https://www.datadoghq.com/blog/datadog-resource-catalog/
  tag: Blog
  text: Administra tus recursos de infraestructura con el Datadog Resource Catalog.
...
- Ofrece a los líderes de ingeniería una visión general de las prácticas de seguridad a través de los equipos y cuentas en la nube.
...
- Acceder a telemetría, tableros y otras vistas de Datadog con información detallada para entender la salud y el rendimiento de tus recursos.
- Localiza a los propietarios de Teams y servicios de los recursos relevantes para acelerar la recuperación de incidentes.
...
- Asegura una cobertura de seguridad adecuada identificando recursos que son más propensos a errores de configuración o que no están reportando activamente errores de configuración de seguridad.

## Configurar {#setup}

Por defecto, cuando navegas al Catálogo de Recursos, puedes ver los hosts monitoreados por el Agente de Datadog, así como los recursos en la nube rastreados para otros productos de Datadog como CNM (Cloud Network Monitoring) y DBM (Database Monitoring). Para ver recursos adicionales en la nube en el Resource Catalog, active **extender la colección de recursos** desde la página de configuración del [Resource Catalog][5]. 
...
En la [Resource Catalog page][2], explora los recursos en la nube en tu organización de Datadog. El Resource Catalog detecta un recurso ya sea porque tiene un Datadog Agent instalado o porque se ha configurado una integración en la nube. 

### Catalog tab {#catalog-tab}

La pestaña Catalog muestra el contexto de un recurso, incluyendo la propiedad del equipo y los servicios relacionados. Te ayuda a identificar proactivamente y completar la información de propiedad faltante antes de que sea necesaria en un incidente. El Resource Catalog también muestra los atributos de los recursos personalizados para cada tipo de recurso. Puedes buscar recursos por atributos específicos como el tipo de instancia para un host, o la versión para una base de datos.

**Nota**: Si utilizas [Equipos de Datadog][4], selecciona el interruptor **Equipos** en el panel izquierdo, luego selecciona el interruptor para los equipos a los que estás asignado para ver solo los recursos asignados a esos equipos. Además, puedes exportar tu lista de Resource Catalog como un archivo CSV desde la esquina superior derecha de la lista.
...
### Investigar un host o recurso {#investigate-a-host-or-resource}
...
- **Resumen del servidor** que muestra alertas de monitores activos y productos habilitados.
- **Telemetría** incluyendo métricas, registros, trazas y procesos
- **Contenedores** que muestra métricas para contenedores adjuntos al host
...
- **Seguridad** que muestra errores de configuración generales, [errores de configuración de IaC][21], señales, vulnerabilidades, riesgos de identidad y perspectivas de acceso.
...
- **Resource Info** incluyendo etiquetas específicas del recurso y la definición del recurso en formato JSON.
...
## Resource Changes (en Vista Previa) {#resource-changes-in-preview}

{{< callout url="https://www.datadoghq.com/product-preview/recent-changes-tab/" >}}
Resource Changes está en Vista Previa. Haz clic en <strong>Request Access</strong> y completa el formulario para solicitar acceso.
{{< /callout >}} 

Resource Changes proporciona visibilidad y control sobre los cambios de configuración en tu infraestructura en la nube. Ayuda a monitorear modificaciones en los recursos, facilitando la resolución de incidentes y entendiendo la evolución de tu entorno.

Para más información, consulta [Resource Changes][16].

{{< img src="/infrastructure/resource_catalog/resource-changes.png" alt="Interfaz de Resource Changes de Datadog mostrando una lista de cambios en la configuración de la infraestructura. La pantalla muestra una instancia de VM llamada "vm-new-jmcintyre-kafka" con una actualización de StorageProfile, incluyendo una vista de diferencias lado a lado que resalta los cambios en formato JSON. La tabla muestra múltiples recursos con marcas de tiempo, tipos de cambio (principalmente "UPDATE") y detalles de las modificaciones. Los filtros están disponibles en la parte superior para nube, región, entorno y otros atributos.tes." width="100%">}}
