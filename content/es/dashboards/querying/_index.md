...
También puede ver indicadores de fuente de Datadog u OpenTelemetry. Si su entorno utiliza ambos, puede usar el selector de {{< ui >}}Semantic Mode{{< /ui >}} de Datadog para [Query Across Datadog and OpenTelemetry Metrics][18] en un solo gráfico.
...
La métrica elegida puede filtrarse por servidor o etiqueta usando el menú desplegable {{< ui >}}from{{< /ui >}} a la derecha de la métrica. El filtro predeterminado es *(en todas partes)*.
...
El método de agregación está al lado del menú desplegable de filtro. Esto predetermina a `avg by` pero puedes cambiar el método a `max by`, `min by` o `sum by`. En la mayoría de los casos, la métrica tiene muchos valores para cada intervalo de tiempo, provenientes de muchos servidores o instancias. El método de agregación elegido determina cómo se agregan las métricas en una sola línea.

#### Configure la agregación de tiempo {#configure-the-time-aggregation}
...
#### Configure la agregación del espacio {#configure-the-space-aggregation}
...
Dependiendo de sus necesidades de análisis, puede optar por aplicar otras funciones matemáticas a la consulta. Los ejemplos incluyen tasas y derivadas, suavizado y otros. Consulta la [lista de funciones disponibles][14].

Datadog también admite la capacidad de graficar sus métricas, registros, trazas y otras fuentes de datos con varias operaciones aritméticas. Utilice: `+`, `-`, `/`, `*`, `min` y `max` para modificar los valores mostrados en sus gráficos. Esta sintaxis permite tanto valores enteros como aritmética utilizando múltiples métricas.

Para graficar métricas por separado, utilice la coma (`,`). Por ejemplo, `a, b, c`.

**Nota**: Las consultas que utilizan comas solo son compatibles en visualizaciones, no funcionan en seguimientos. Utilice [boolean operators][15] u operaciones aritméticas para combinar múltiples métricas en un seguimiento.

#### Aritmética de métricas usando un entero {#metric-arithmetic-using-an-integer}

Modifica el valor mostrado de una métrica en un gráfico realizando una operación aritmética. Por ejemplo, para visualizar el doble de una métrica específica, haz clic en el enlace {{< ui >}}Advanced...{{< /ui >}} en el editor de gráficos. Luego ingrese su operación aritmética en el recuadro `Formula`, en este caso: `a * 2`:
...
Luego, en el recuadro `Formula`, ingrese la operación aritmética (`a / b` para este ejemplo). Para mostrar solo la fórmula en su gráfico, haga clic en las marcas de verificación junto a las métricas `a` y `b`.
...
**Nota**: Las fórmulas no están identificadas con letras. No se puede realizar aritmética entre fórmulas.
...
{{< img src="dashboards/querying/minmax_metrics_example.png" alt="Ejemplo de fórmula para 'max' mostrando el valor máximo de recuento entre dos consultas métricas" style="width:75%;" >}}
...
{{< img src="dashboards/querying/minmax_logs_platform_example.png" alt="Ejemplo de fórmula para 'min' mostrando el valor mínimo de recuento entre dos consultas de registros" style="width:75%;" >}}

#### Exponenciación {#exponentiation}

Ahora puedes utilizar la función `pow()` para elevar una constante o una métrica a la potencia de otra constante o métrica. Esto te permite modelar el crecimiento o la disminución exponencial. 

Aquí hay un ejemplo de cómo prever el crecimiento de usuarios aplicando un factor de crecimiento exponencial a una ventana de tiempo anterior:

```text
users.sessions{*} * pow(1.1, timeshift(-1))
```

Aquí hay un ejemplo de cómo detectar anomalías amplificando el valor utilizando la exponenciación: 

```text
pow(ping{region:*}, 2)
```

Para usar `pow(a, b)`, `a` y `b`, estos pueden ser constantes o métricas. Esta función solo está disponible en métricas.
...
Haz clic en {{< ui >}}Done{{< /ui >}} para guardar tu trabajo y salir del editor. Siempre puedes volver al editor para cambiar el gráfico. Si realizas cambios que no deseas guardar, haz clic en {{< ui >}}Cancel{{< /ui >}}.
...
Visualiza las correlaciones de eventos utilizando la sección {{< ui >}}Event Overlays{{< /ui >}} en el editor de gráficos para la visualización de [Series temporales][16]. En el campo de búsqueda, ingresa cualquier texto o consulta de búsqueda estructurada. La búsqueda de eventos utiliza la [sintaxis de búsqueda de registros][17].
...
- Correlacionando el tráfico de salida con actividad sospechosa de inicio de sesión
- Correlacionando cualquier dato de series temporales con alertas de seguimiento para asegurar que Datadog esté configurado con las alertas apropiadas


### Gráfico dividido {#split-graph}

Con gráficos divididos, puedes ver tus visualizaciones de métricas desglosadas por etiquetas. 

{{< img src="dashboards/querying/split_graph_beta.png" alt="Ver gráficos divididos de la métrica container.cpu.usage en el widget de pantalla completa" style="width:100%;" >}}

1. Accede a esta función a través de la pestaña {{< ui >}}Split Graph{{< /ui >}} al ver gráficos.
1. Puedes cambiar la métrica {{< ui >}}sort by{{< /ui >}} para ver la relación entre los datos que estás graficando y otras métricas.
1. Limita el número de gráficos que se muestran cambiando el valor de {{< ui >}}limit to{{< /ui >}}.
