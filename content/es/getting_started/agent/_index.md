...
- link: https://dtdg.co/fe
  ...
  text: Únase a una sesión interactiva para potenciar la supervisión de su infraestructura
...
- link: https://www.datadoghq.com/blog/lambda-managed-instances
  tag: Blog
  text: Monitorear instancias administradas de AWS Lambda con Datadog
...
El Agente de Datadog es un software que se ejecuta en sus servidores. Recopila eventos y métricas de los servidores y los envía a Datadog, donde puedes analizar tus datos de monitoreo y rendimiento. 
...
Para brindarte una visibilidad completa de tu infraestructura, el Agente de Datadog informa métricas sobre su propia salud y configuración, así como métricas recopiladas de tus servidores y servicios a través de sus verificaciones predeterminadas.
...
| `datadog.agent.started`        | Un conteo de `1` cada vez que se inicia el Agente.    |
...
| Manejador de archivos | [System][4]  | Todo excepto Mac     |
| Carga        | [System][4]  | Todo excepto Windows |
...
Estas verificaciones se pueden usar en Datadog para visualizar el estado del Agente a través de monitores y tableros de un vistazo. Consulte [Service Check Overview][21] para obtener más información.
...
- **Detección de integraciones**: 
    - **Host**: [Integrations][9] se identifican a través del archivo de configuración del Agente.
    - **Contenedor**: Las integraciones se identifican automáticamente utilizando la función de Autodiscovery de Datadog. Consulte [Basic Agent Autodiscovery][11] para obtener más información.

Además, consulte el [Agente de Docker][12] o [Kubernetes][13] para una guía sobre cómo ejecutar el Agente en un entorno de contenedores.
{{% /collapse-content %}} 


{{% collapse-content title="Configuración de etiquetas a través del archivo de configuración del Agente" level="h4" expanded=false id="id-for-anchoring" %}}

Las etiquetas añaden una capa adicional de metadatos a sus métricas y eventos. Le permiten delimitar y comparar sus datos en las visualizaciones de Datadog. Cuando se envían datos a Datadog desde múltiples hosts, etiquetar esta información le permite enfocarse en los datos que más le interesa visualizar.

Por ejemplo, supongamos que tiene datos recopilados de diferentes equipos y solo le interesa ver las métricas del equipo alfa; etiquetar esos hosts específicos con la etiqueta `team:alpha` o `team:bravo` le da la capacidad de filtrar las métricas que están etiquetadas con `team:alpha`. Consulte [Getting Started with Tags][24] para aprender más sobre cómo etiquetar sus datos.

1. Localice el [archivo de configuración principal][25] de su Agente. Para Ubuntu, la ubicación del archivo es `/etc/datadog-agent/datadog.yaml`.
...
{{% collapse-content title="Encontrando métricas en la Datadog UI" level="h4" expanded=false id="id-for-anchoring" %}}
...
La cantidad de espacio y recursos que utiliza el Agente depende de la configuración y de los datos que el Agente está enviando. Al inicio, puede esperar alrededor del 0.08% de CPU utilizada en promedio con un espacio en disco de aproximadamente 880MB a 1.3GB.

Consulte [Agent Overhead][2] para aprender más sobre estos puntos de referencia.
{{% /collapse-content %}}
...
- Habilitar [OTLP Trace Ingestion][30]
- [Customizing log collection][31] para filtrar o limpiar datos sensibles
...
Consulta [Agent Commands][33] para [Start][34], [Stop][35] o [Restart][26] tu Agente.
...
- Ver los [Agent Log Files][37]
- Contactar a [Datadog support][38]
...
{{< whatsnext desc="Después de que se instale el Agente:">}}
{{< nextlink href="/getting_started/integrations" >}}Aprende sobre Integrations{{< /nextlink >}}
{{< nextlink href="/getting_started/application" >}}Aprende sobre Datadog UI{{< /nextlink >}}
{{< nextlink href="/getting_started/logs" >}}Aprende cómo recolectar Logs a través del Agente{{< /nextlink >}}
{{< nextlink href="/getting_started/tracing" >}}Aprende cómo recolectar Trazas a través del Agente{{< /nextlink >}}
{{< /whatsnext >}}
