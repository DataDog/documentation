...
- link: https://dtdg.co/fe
  ...
  text: Únete a una sesión interactiva para potenciar tu monitoreo de Infraestructura
...
- Anfitriones locales (Windows, macOS) 
...
Utilice [Automatización de Flota][39], el flujo de trabajo dentro de la aplicación de Datadog, para instalar, actualizar, configurar y solucionar problemas del Agente de Datadog en un solo anfitrión o a gran escala. 
...
Para brindarle una visibilidad completa de su infraestructura, el Agente de Datadog informa métricas sobre su propia salud y configuración, así como métricas recopiladas de sus hosts y servicios a través de sus verificaciones predeterminadas.
...
Dependiendo de su plataforma, el Agente tiene varias verificaciones centrales habilitadas por defecto que recopilan métricas.
...
| Cargar | [Sistema][4] | Todo excepto Windows |
...
Estas verificaciones se pueden usar en Datadog para visualizar el estado del Agente a través de monitores y paneles de control de un vistazo. Consulte [Descripción general de las verificaciones de servicio][21] para obtener más información.
...
- **Detección de integraciones**: 
    ...
    - **Contenedor**: Las integraciones se identifican automáticamente utilizando la función de Autodescubrimiento de Datadog. Consulta [Autodescubrimiento Básico del Agente][11] para aprender más.

Además, consulta el [Agente de Docker][12] o [Kubernetes][13] para una guía sobre cómo ejecutar el Agente en un entorno en contenedores.
{{% /collapse-content %}} 
...
3. Descomenta el parámetro de etiquetas y el ejemplo de etiqueta proporcionado `team:infra`. También puedes añadir tu propia etiqueta personalizada, por ejemplo `test:agent_walkthrough`.
   ```yaml
   ## @param tags  - list of key:value elements - optional
   ## @env DD_TAGS - space separated list of strings - optional
   ## List of host tags. Attached in-app to every metric, event, log, trace, and service check emitted by this Agent.
...
5. Después de unos minutos, ve a la [página de Resumen de Métricas][22] nuevamente y haz clic en la métrica `datadog.agent.started`. Además de las etiquetas predeterminadas `host` y `version`, también puedes ver la etiqueta `team` y cualquier etiqueta personal que hayas añadido. También puedes filtrar métricas por el campo `Tag` en la parte superior de la página.

6. Ve a la [página del Explorador de Eventos][20] y encuentra las etiquetas personalizadas mostradas con el último Evento del Agente.
...
- Habilitando la [Ingesta de Trazas OTLP][30]
...
Vea [Comandos del Agente][33] para [Iniciar][34], [Detener][35] o [Reiniciar][26] su Agente.

## Resolución de problemas {#troubleshooting}

Para obtener ayuda con la resolución de problemas del Agente:

- Vea [Resolución de problemas del Agente][36]
- Vea los [Archivos de Registro del Agente][37]
- Contacte a [soporte de Datadog][38]
...
{{< nextlink href="/getting_started/integrations" >}}Aprenda sobre Integraciones{{< /nextlink >}}
{{< nextlink href="/getting_started/application" >}}Aprenda sobre la interfaz de usuario de Datadog{{< /nextlink >}}
