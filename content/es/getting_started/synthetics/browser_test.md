...
{{< img src="getting_started/synthetics/browser-test-overview.png" alt="Información general de un test de navegador Synthetic" style="width:100%;" >}}
...
**Nota:** Además de ejecutar tu test Synthetic de forma programada, puedes activarlo manualmente o desde tus [pipelines de CI/CD][5].
...
{{< img src="getting_started/synthetics/configured-browser-test.mp4" alt="Ejemplo de configuración del test de navegador" video="true"  >}}
...
Cuando lo tengas todo listo para grabar tu test, haz clic en **Save Details & Record Test** (Guardar detalles y grabar el test).

### Crear una grabación

Una vez guardada la configuración del test, Datadog te pedirá que descargues e instales la extensión de Chrome [Datadog test recorder][8].
...
**Nota**: El sitio web de ejemplo da regularmente un error que hace que falle intencionadamente. Si incluyes tu dirección de correo electrónico en el campo **Notify your team** (Notificar a tu equipo), recibirás un aviso por correo electrónico cuando la prueba falle y se recupere.
...
Para solucionar los problemas de un [test fallido][9], selecciona el test que corresponda y repasa las capturas de pantalla hasta encontrar el paso fallido. También puedes consultar los posibles **[errores y advertencias][10]**, **[recursos][11]** y **[métricas de Core Web Vitals][12]** para hacer un diagnóstico del problema.

En el siguiente ejemplo, el test ha fallado debido al tiempo de espera del servidor.

{{< img src="getting_started/synthetics/browser-test-failure.mp4" alt="Fallo en el test de navegador" video="true"  >}}

Utiliza la [integración de APM con la monitorización Synthetic][13] de Datadog para ver las trazas generadas por tu backend en las ejecuciones del test desde la pestaña **Traces** (Trazas).
...
[9]: /es/synthetics/browser_tests/test_results#test-failure
[10]: /es/synthetics/browser_tests/test_results#errors
[11]: /es/synthetics/browser_tests/test_results#resources
[12]: /es/synthetics/browser_tests/test_results#page-performance
