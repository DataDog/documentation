...
- link: /getting_started/tagging/using_tags
  tag: Documentación
  text: Aprenda a usar etiquetas en la aplicación de Datadog
...
- Identificar el impacto de la implementación mediante las métricas de traza y de contenedor filtradas por versión
...
- El servicio oficial de un registro se establece por defecto al contenedor de imagen corta si no existe una configuración de Autodiscovery para registros. Para anular el servicio oficial de un registro, agregue [etiquetas de Docker/anotaciones de pod][2] de Autodiscovery. Por ejemplo: `"com.datadoghq.ad.logs"='[{"service": "service-name"}]'`
- La información del servidor se excluye para los tramos de base de datos y caché porque el servidor asociado con el tramo no es el servidor de la base de datos/caché.

### Requisitos {#requirements}

- Unified service tagging requires the setup of a [Datadog Agent][3] that is 6.19.x/7.19.x or higher.

- La etiquetación unificada de servicios requiere una versión de SDK que soporte nuevas configuraciones de las [etiquetas reservadas][1]. Se puede encontrar más información por idioma en las [instrucciones de configuración][4].
...
En entornos en contenedores, `env`, `service` y `version` se configuran a través de las variables de entorno o etiquetas del servicio (por ejemplo, etiquetas de despliegue y de pod en Kubernetes, etiquetas de contenedor de Docker). Datadog Agent detecta esta configuración de etiquetado y la aplica a los datos que recopila de los contenedores.

Para configurar el etiquetado unificado de servicios en un entorno en contenedores:

1. Habilitar [Autodiscovery][6]. Esto permite que Datadog Agent identifique automáticamente los servicios que se ejecutan en un contenedor específico y recopile datos de esos servicios para mapear las variables de entorno a las etiquetas `env`, `service,` y `version`.

2. Si está utilizando [Docker][2], asegúrese de que Datadog Agent pueda acceder al [socket de Docker][7] de su contenedor. Esto permite que Datadog Agent detecte las variables de entorno y las mapee a las etiquetas estándar.

3. Configure el entorno correspondiente a su servicio de orquestación de contenedores, ya sea mediante configuración completa o parcial, como se detalla a continuación.
...
Si desplegó Datadog Cluster Agent con el [Controlador de Admisión][1] habilitado, el Controlador de Admisión muta los manifiestos de pod e inyecta todas las variables de entorno requeridas (basado en las condiciones de mutación configuradas). En ese caso, la configuración manual de las variables de entorno `DD_` en los manifiestos de pod no es necesaria. Para más información, consulte la [documentación del Controlador de Admisión][1].
...
###### Datadog SDK and StatsD client {#datadog-sdk-and-statsd-client}

Para configurar las variables de entorno del [Datadog SDK][5] y del [cliente de StatsD][6], use la [API descendente de Kubernetes][2] en el formato a continuación:
...
- Si sus servicios no se ejecutan en un entorno contenedorizado, o si también desea incluir el Git SHA, [inserte información de Git en sus artefactos de compilación][9].
...
- Si sus servicios se ejecutan en un entorno contenedorizado y `image_tag` es suficiente para rastrear nuevos despliegues de versiones, no se necesita configuración adicional.
- Si sus servicios no se están ejecutando en un entorno en contenedores, o si también desea incluir el SHA de Git, [inserte información de Git en sus artefactos de construcción][3].
...
Si su servicio no necesita las variables de entorno de Datadog (por ejemplo, software de terceros como Redis, PostgreSQL, NGINX y aplicaciones que no son rastreadas por APM), puede usar las etiquetas de Docker en la definición de su tarea de ECS:
...
2. O utiliza [Chef][13], [Ansible][14] u otra herramienta de orquestación para poblar el archivo de configuración systemd o initd de un servicio con las variables de entorno `DD`. Cuando el proceso del servicio se inicia, tiene acceso a esas variables.
   ...
   1. Configure el [SDK de Datadog][1] con `DD_ENV` para mantener la definición de `env` más cerca de la aplicación que está generando las trazas. Este método permite que la etiqueta `env` se obtenga automáticamente de una etiqueta en los metadatos del span.

   2. Configura spans con `DD_VERSION` para agregar la versión a todos los spans que caen bajo el servicio que pertenece al SDK (generalmente `DD_SERVICE`). Esto significa que si su servicio crea spans con el nombre de un servicio externo, esos spans no reciben `version` como etiqueta.
...
#### Servicio único por host {#single-service-per-host}
