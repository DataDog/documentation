---
title: Componente de cola de Service Bus
---

## Información general

Puedes utilizar el componente de cola de Service Bus para representar y visualizar la mensajería en la nube como integraciones de un servicio desde tu entorno Azure.

{{< img src="cloudcraft/components-azure/service-bus-queue/component-service-bus-queue-diagram.png" alt="Captura de pantalla de un diagrama isométrico de Cloudcraft que muestra componentes de Azure interconectados." responsive="true" style="width:60%;">}}
...
- **Nivel**: Selecciona el nivel de servicio para tu cola de Service Bus.
...
- **Conexiones intermediadas**: Introduce el número de conexiones intermediadas para tu cola. Solo está disponible para el nivel de servicio Standard.
- **Conexiones híbridas**: Introduce el número de conexiones híbridas para tu cola. Solo está disponible para el nivel de servicio Standard.
...
- **Horas de retransmisión**: Introduce el número de horas de retransmisión para tu cola. Solo está disponible para el nivel de servicio Standard.
...
Utiliza la [API Cloudcraft][1] para acceder mediante programación y presentar tus diagramas de arquitectura como objetos JSON. El siguiente es un ejemplo de objeto JSON de un componente de cola Service Bus:
...
- **type: string**: Tipo de componente. Debe ser una cadena con el valor `azuresbqueue` para este componente.
...
- **tier: string**: Nivel servicio para la cola. Acepta uno de tres valores, `Basic`, `Standard` y `Premium`. Por defecto es `Standard`.
- **operationsPerMonth: number**: Número de operaciones por mes en millones. Por defecto es `0`.
- **brokeredConnections: number**: Número de conexiones intermediadas para la cola. Por defecto es `0`.
- **hybridConnections: number**: Número de conexiones híbridas para la cola. Por defecto es `0`.
- **dataTransferGb: number**: Volumen total de datos transferidos mensualmente en gigabytes. Por defecto es `0`.
- **relayHours: number**: Número de horas retransmitidas para la cola. Por defecto es `0`.
