...
- Instala y configura la librería Datadog Lambda y Extensión Lambda para tus funciones Lambda [Python][1], [Node.js][2], [.NET][9] y [Java][10].
...
| `enableDDTracing`           | Habilita el rastreo en la función Lambda a través de dd-trace, la librería de la APM de Datadog. El valor predeterminado es `true`. La función Datadog Forwarder Lambda debe suscribirse al grupo de logs de la función.                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `coldStartTraceSkipLibs`    | Opcionalmente, omite la creación de tramos (spans) de inicio en frío para una lista separada por comas de librerías. Es útil para limitar la profundidad u omitir bibliotecas conocidas. El valor por defecto depende del tiempo de ejecución.                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `llmObsEnabled`            | Activar el envío de datos a LLM Observability. Por defecto `false`. |
| `llmObsMlApp`              | El nombre de su aplicación LLM, servicio o project (proyecto), bajo el cual se agrupan todas las trazas y periodos. Esto ayuda a distinguir entre diferentes aplicaciones o experimentos. Consulte [Directrices de nomenclatura de aplicaciones](https://docs.datadoghq.com/llm_observability/sdk/?tab=nodejs#application-naming-guidelines) para conocer los caracteres permitidos y otras restricciones. Para anular este valor para una raíz dada span (tramo), consulte [Rastreo de múltiples aplicaciones](https://docs.datadoghq.com/llm_observability/sdk/?tab=nodejs#tracing-multiple-applications).  Obligatorio si `llmObsEnabled` es `true` |
| `llmObsAgentlessEnabled`   | Sólo es necesario si no está utilizando la extensión Lambda Datadog, en cuyo caso case (incidencia) debe establecerse en `true`.  Por defecto `false`. |
