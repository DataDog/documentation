...
| `flushMetricsToLogs` | `flush_metrics_to_logs` | Envía <txprotected>métricas personalizadas</txprotected> utilizando logs de CloudWatch con la función lambda de Datadog Forwarder (recomendado). Por defecto es `true`. Si desactivas este parámetro, es necesario configurar `apiKey` (o si está cifrado, `apiKMSKey` o `apiKeySecretArn`). |
