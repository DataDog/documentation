---
aliases:
- /es/serverless/datadog_lambda_library/java/
- /es/serverless/installation/java
further_reading:
...
title: Instrumentación de aplicaciones serverless de Java
---
...
Si despliegas tus funciones de Lambda en una VPC sin acceso a la Internet pública, puedes enviar datos [con AWS PrivateLink][6] para el [sitio de Datadog][7] `datadoghq.com`, o bien [mediante un proxy][8], en caso de que uses cualquier otro sitio.

Si ya configuraste tus funciones de Lambda con el Datadog Forwarder, consulta la sección de [instrumentación mediante el Datadog Forwarder][9]. De lo contrario, sigue las instrucciones de esta guía para instrumentar mediante la Datadog Lambda Extension.

Si utilizas las capas de Lambda de Datadog `dd-trace-java:4` (o anteriores) y `Datadog-Extension:24` (o anteriores), sigue las [instrucciones de actualización especiales][10].
...
<div class="alert alert-info">La instrumentación de funciones de Java a través del constructo del CDK de Datadog solo está disponible para aplicaciones del CDK de AWS escritas en Node.js y Python.</div>

El [constructo del CDK de Datadog][1] instala automáticamente Datadog en tus funciones con las capas de Lambda. Además, configura tus funciones para enviar métricas, trazas y logs a Datadog a través de la Datadog Lambda Extension.
...
2. Instrumentar las funciones de Lambda
    ...
    **Python**:
    ...
    Obtén más información y parámetros adicionales en la [documentación del CDK de Datadog][1].
...
- Activa la [monitorización de amenazas][11] para recibir alertas sobre los atacantes que tienen tu servicio como objetivo.
...
[3]: /es/tracing/custom_instrumentation/java/
[4]: /es/serverless/guide/troubleshoot_serverless_monitoring/
[5]: /es/serverless/configuration/
[6]: /es/agent/guide/private-link/
[7]: /es/getting_started/site/
[8]: /es/agent/configuration/proxy/
[9]: /es/serverless/guide/datadog_forwarder_java
[10]: /es/serverless/guide/upgrade_java_instrumentation
[11]: /es/security/application_security/enabling/serverless/?tab=serverlessframework