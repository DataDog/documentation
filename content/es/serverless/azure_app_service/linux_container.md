...
Si prefiere no usar el enfoque de sidecar (no recomendado), puede seguir las instrucciones para [Instrumentar Azure App Service Contenedor de Linux con `serverlessinit`][2].

## Configuración

### Integración de Azure

Si aún no lo ha hecho, instale la [integración de DatadogAzure][3] para recopilar métricas y registros.
...
{{% tab "Node.js" %}}
#### Trazado
Instrumente su aplicación principal con la biblioteca `ddtracejs`. Consulte [Trazado de aplicaciones Node.js][101] para obtener instrucciones.

#### Métricas
Las métricas personalizadas también se recopilan a través del trazador. Consulte los [ejemplos de código][102].

#### Registros
El sidecar de Datadog utiliza el seguimiento de archivos para recopilar registros. Datadog recomienda escribir registros de la aplicación en `/home/LogFiles/` porque este directorio se conserva entre reinicios.
...
[101]: /es/tracing/trace_collection/automatic_instrumentation/dd_libraries/nodejs/#gettingstarted
[102]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=nodejs#codeexamples
[103]: /es/logs/log_collection/nodejs/?tab=winston30
[104]: /es/tracing/other_telemetry/connect_logs_and_traces/nodejs
{{% /tab %}}
{{% tab "Python" %}}
#### Trazado
Instrumenta tu aplicación principal con la biblioteca `ddtracepy`. Consulta [Trazando aplicaciones de Python][201] para instrucciones.
...
[202]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=python#codeexamples
[203]: /es/logs/log_collection/python/
[204]: /es/tracing/other_telemetry/connect_logs_and_traces/python
{{% /tab %}}
{{% tab "Java" %}}
#### Trazado
Instrumenta tu aplicación principal con la biblioteca `ddtracejava`. Consulta [Rastreo de aplicaciones Java][301] para obtener instrucciones.
...
[301]: /es/tracing/trace_collection/automatic_instrumentation/dd_libraries/java/#gettingstarted
[302]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=java#codeexamples
[303]: /es/logs/log_collection/java/?tab=winston30
[304]: /es/tracing/other_telemetry/connect_logs_and_traces/java
{{% /tab %}}
{{% tab ".NET" %}}
#### Trazado
Instrumenta tu aplicación principal con la `ddtracedotnet` biblioteca.
...
2. Construye la imagen y súbela a tu registro de contenedores preferido.
...
[401]: /es/tracing/trace_collection/automatic_instrumentation/dd_libraries/dotnetcore
[402]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=dotnet#codeexamples
...
{{% tab "Go" %}}
#### Trazado
Instrumenta tu aplicación principal con la biblioteca `ddtracego`. Consulta [Trazando aplicaciones Go][501] para instrucciones.
...
[502]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=go#codeexamples
[503]: /es/logs/log_collection/go/
[504]: /es/tracing/other_telemetry/connect_logs_and_traces/go
{{% /tab %}}
{{% tab "PHP" %}}
#### Trazado
Instrumenta tu aplicación principal con la biblioteca `ddtracephp`. Consulta [Trazando aplicaciones PHP][601] para instrucciones.
...
[601]: /es/tracing/trace_collection/automatic_instrumentation/dd_libraries/php/#gettingstarted
[602]: /es/metrics/custom_metrics/dogstatsd_metrics_submission/?codelang=php#codeexamples
...
La instrumentación se realiza utilizando un contenedor sidecar. Este contenedor sidecar recopila trazas, métricas y registros de tu contenedor de aplicación principal y los envía a Datadog.
...
Instala el [Datadog CLI][601]

```shell
npm install -g @datadog/datadog-ci @datadog/datadog-ci-plugin-aas
```

Instala el [Azure CLI][602] y autentícate con `az login`.

Luego, ejecuta el siguiente comando para configurar el contenedor sidecar:
...
Bandas adicionales, como `service` y `env`, pueden ser utilizadas para establecer las etiquetas de servicio y entorno. Para una lista completa de opciones, ejecuta `datadogci aas instrument help`.
...
<div class="alert alert-danger">Debido a que el recurso de Azure Web App para Contenedores no admite directamente sitecontainers, deberías esperar desviaciones en tu configuración.</div>

El [módulo de Datadog Terraform para Linux Web Apps][1] envuelve el recurso [azurerm_linux_web_app][2] y configura automáticamente tu Web App para la Monitoreo Serverless de Datadog al agregar las variables de entorno requeridas y el sidecar serverlessinit.
...
2. En el formulario de **Editar contenedor**, proporciona lo siguiente:
    ...
    **Imagen y etiqueta**: `datadog/serverlessinit:latest`
    **Puerto**: 8126
...
{{< img src="serverless/azure_app_service/app_settings.png" alt="En Azure, una sección de Variables de Entorno. Una opción 'Permitir acceso a todas las configuraciones de la aplicación' está habilitada con una casilla de verificación." >}}
 ...
 `DD_SERVICE`: Cómo deseas etiquetar tu servicio. Por ejemplo, `sidecarazure`
...
Para habilitar el [Perfilador Continuo][4], establezca la variable de entorno `DD_PROFILING_ENABLED=true` en su contenedor de aplicación.
...
[1]: https://learn.microsoft.com/es-es/azure/appservice/tutorialcustomcontainersidecar
