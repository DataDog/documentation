...
DevCycle no soporta esta integración. Crea un ticket con DevCycle para solicitar esta función.
...
Inicializa el SDK de Eppo y crea un registrador de asignaciones que además informe las evaluaciones de las banderas de características a Datadog utilizando el fragmento de código que se muestra a continuación.
...
Eppo no soporta esta integración. [Contacta a Eppo][1] para solicitar esta función.
...
Inicializa el SDK de Flagsmith con la opción `datadogRum`, que informa las evaluaciones de las banderas de características a Datadog utilizando el fragmento de código que se muestra a continuación.
...
Al inicializar el SDK de GrowthBook, informa las evaluaciones de las banderas de características a Datadog utilizando el callback `onFeatureUsage`.
...
Al inicializar el SDK de GrowthBook, informa las evaluaciones de las banderas de características a Datadog llamando a `setFeatureUsageCallback`.

Para más información sobre la inicialización del SDK de GrowthBook, consulta la [documentación del SDK de Android de GrowthBook][1].
...
Para más información sobre la inicialización del SDK de GrowthBook, consulta la [documentación del SDK de Flutter de GrowthBook][1].
...
Para más información sobre la inicialización del SDK de GrowthBook, consulta la [documentación del SDK de React Native de GrowthBook][1].
...
Después de crear e inicializar el SDK de Kameleoon, suscríbete al evento `Evaluation` utilizando el controlador `onEvent`.
...
Aprende más sobre la inicialización del SDK en la [documentación del SDK de React Native de Kameleoon][1].
...
Inicializa el SDK de LaunchDarkly y crea una función de inspector que reporte las evaluaciones de las banderas de características a Datadog utilizando el fragmento de código que se muestra a continuación.
...
Inicializa el SDK de Split y crea un listener de impresiones que reporte las evaluaciones de las banderas de características a Datadog utilizando el siguiente fragmento de código:
...
Inicializa el SDK de Split y crea un inspector que reporte las evaluaciones de las banderas de características a Datadog utilizando el fragmento de código a continuación.
...
Para más información sobre cómo inicializar el SDK de Split, consulta la [documentación del SDK de Android de Split][1].
...
Para más información sobre cómo inicializar el SDK de Split, consulta la [documentación del plugin de Flutter de Split][1].
...
Para más información sobre cómo inicializar el SDK de Split, consulta la [documentación del SDK de React Native de Split][1].
...
Las flags de características aparecen en el contexto de tus sesiones de RUM, vistas y errores como una lista.
...
#### Sesiones
Filtrando tus **Sesiones** con el atributo `@feature_flags.{flag_name}`, puedes encontrar todas las sesiones en el marco de tiempo dado donde se evaluó tu bandera de función.

{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-session-feature-flag-search.png" alt="Buscar Sesiones para Banderas de Función en el Explorador RUM" style="width:75%;">}}

#### Vistas
Filtrando tus **Vistas** con el atributo `@feature_flags.{flag_name}`, puedes encontrar las vistas específicas en el marco de tiempo dado donde se evaluó tu bandera de función.

{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-view-feature-flag-search.png" alt="Buscar Vistas para Banderas de Función en el Explorador RUM" style="width:75%;">}}

#### Errores
Filtrando tus **Errores** con el atributo `@feature_flags.{flag_name}`, puedes encontrar todos los errores en el marco de tiempo dado que ocurrieron en la Vista donde se evaluó tu bandera de función.

{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-error-feature-flag-search.png" alt="Buscar Errores para Banderas de Función en el Explorador RUM" style="width:75%;">}}

## Solución de Problemas

### Mis datos de bandera de función no reflejan lo que espero ver
Las banderas de función aparecen en el contexto de eventos donde se evalúan, lo que significa que deberían aparecer en las vistas en las que se ejecuta la lógica del código de la bandera de función.

Dependiendo de cómo hayas estructurado tu código y configurado tus banderas de función, puedes ver banderas de función inesperadas aparecer en el contexto de algunos eventos.

Por ejemplo, para ver en qué **Vistas** se está evaluando tu bandera de función, puedes usar el Explorador RUM para hacer una consulta similar:

{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/feature_flag_view_query.png" alt="Buscar Vistas para Banderas de Función en el Explorador RUM" style="width:75%;">}}

Aquí hay algunos ejemplos de razones por las cuales tu bandera de función se está evaluando en Vistas no relacionadas que pueden ayudar con tus investigaciones:

- Un componente react común que aparece en múltiples páginas y que evalúa banderas de función cada vez que se ejecuta.
- Un problema de enrutamiento donde los componentes con una evaluación de bandera de función se renderizan antes/después de los cambios de URL.

Al realizar tus investigaciones, también puedes limitar tus datos a los `View Name` que son relevantes para tu bandera de función.
