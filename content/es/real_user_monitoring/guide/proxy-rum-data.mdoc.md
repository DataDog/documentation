...
description: Configura el proxy de datos RUM del navegador con opciones de fuente
  del SDK y configuraciones específicas de versión para el enrutamiento personalizado
  de red.
further_reading:
- link: /real_user_monitoring/
  tag: Documentation
  text: Aprende sobre Real User Monitoring
title: Utiliza el proxy para tus datos RUM del navegador.
---
{% if equals($rum_browser_sdk_version, "lt_4_34_0") %}
{% alert level="danger" %}
Actualiza al SDK del navegador `4.34.0` o a una versión posterior para evitar vulnerabilidades de seguridad en la configuración de tu proxy.
{% /alert %}
{% /if %}
...
1. [Construye la URL de recepción de Datadog](#build-the-datadog-intake-url).
...
### Construye la URL de recepción de Datadog

Tu URL de recepción de Datadog debe tener el formato `<INTAKE_ORIGIN>/<PATH><PARAMETERS>` (por ejemplo, `https://browser-intake-datadoghq.eu/api/v2/rum?ddsource=browser&...`).

{% table %}
---
* origen de entrada
* 
    El origen de entrada de Datadog corresponde a su `site` [parámetro de inicialización][1]. El origen de entrada de Datadog correspondiente a tu parámetro de sitio debe definirse en la implementación de tu proxy.
...
* 
    ...
    La ruta de cada solicitud se encuentra en el parámetro `ddforward` de la solicitud (por ejemplo, `https://www.example-proxy.com/any-endpoint?ddforward=%2Fapi%2Fv2%2Frum%3Fddsource%3Dbrowser`).
---
* parámetros
* 
    Los parámetros de la solicitud (por ejemplo, `ddsource=browser&...`) se pueden obtener del parámetro `ddforward` de la solicitud (por ejemplo, `https://www.example-proxy.com/any-endpoint?ddforward=%2Fapi%2Fv2%2Frum%3Fddsource%3Dbrowser`).
...
El SDK del navegador RUM agrega un parámetro de consulta `ddforward` a todas las solicitudes a tu proxy. Este parámetro de consulta contiene la ruta de la URL y los parámetros a los que deben reenviarse todos los datos.
...
**Nota:**
- Algunos bloqueadores de privacidad ya apuntan a los patrones de URL de entrada [URL patterns][2], así que querrás tener eso en cuenta al construir tu URL de proxy.
...
**Para evitar vulnerabilidades de seguridad, debes actualizar a Browser SDK `4.34.0` o una versión posterior.**
{% /if %}
