...
3. Reenviar la solicitud a la URL de recepción de Datadog utilizando el método POST.
...
El SDK del navegador RUM agrega un parámetro de consulta `ddforward` a todas las solicitudes a tu proxy. Este parámetro de consulta contiene la ruta de la URL y los parámetros a los que se deben enviar todos los datos.
...
**Nota:**
- Algunos bloqueadores de privacidad ya apuntan a los patrones de URL de ingesta [URL patterns][2], así que puede que desees tener eso en cuenta al construir tu URL de proxy.
- La función `proxy` se llama para cada solicitud, por lo que debe evitar realizar cálculos pesados.
...
**Para evitar vulnerabilidades de seguridad, actualiza a la versión `4.34.0` o posterior del SDK del navegador.**
{% /if %}
