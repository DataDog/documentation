...
<li>El cuerpo de la solicitud puede contener datos binarios y no debe convertirse en una cadena. Asegúrate de que tu implementación del proxy reenvía el cuerpo sin procesar sin conversión.</li>
<li>Asegúrate de que tu implementación del proxy no permite a un actor malicioso enviar solicitudes a un servidor diferente (por ejemplo, https://browser-intake-datadoghq.com.malicious.com)..</li>  
</ul>
</div>
...
Configura la URL del proxy en el parámetro de inicialización `proxy`. El SDK del navegador RUM añade un parámetro de consulta `ddforward` a todas las solicitudes a tu proxy. Este parámetro de consulta contiene la ruta y los parámetros URL a los que deben reenviarse todos los datos.
...
A partir del SDK del navegador v5.4.0, el parámetro de inicialización `proxy` admite una entrada de función. Esta función te permite tener más control sobre cómo se añaden la ruta y los parámetros a la URL del proxy.
...
- **Las aplicaciones web JSP** necesitan utilizar el carácter de escape `\` para propagar correctamente estos parámetros al navegador. Por ejemplo:
  ```javascript
...
**Nota**
- Algunos bloqueadores de privacidad ya apuntan a [patrones URL][2] de admisión, por lo que es posible que quieras tenerlo en cuenta al crear tu URL para el proxy.
...
El origen de admisión de Datadog debe definirse en tu implementación del proxy para garantizar la seguridad. <strong>Si sigues utilizando un proxy con una versión anterior del SDK del navegador, actualízalo a una versión más reciente para evitar vulnerabilidades.</strong>
