...
- link: https://learn.datadoghq.com/courses/configure-rum-javascript
  ...
  text: Configura RUM para aplicaciones web en JavaScript
...
- Proporcionar más contexto que lo que los atributos predeterminados ofrecen sobre de dónde provienen los datos.

<!-- Version must meet 2.17.0 -->
{% if semverIsAtLeast($rum_browser_sdk_version, "2.17.0") %}

## Sobrescribir los nombres de visualización RUM predeterminados {% #override-default-rum-view-names %}

A partir de [versión 2.17.0][3], puedes agregar nombres de visualización y asignarlos a un servicio dedicado propiedad de un equipo, rastreando eventos de visualización manualmente con la opción `trackViewsManually`.

El SDK del navegador RUM genera automáticamente un [evento de visualización][2] para cada nueva página visitada por tus usuarios, o cuando se cambia la URL de la página (para aplicaciones de una sola página). Un nombre de visualización se calcula a partir de la URL de la página actual, donde los IDs de variable se eliminan automáticamente. Un segmento de ruta que contiene al menos un número se considera un ID de variable. Por ejemplo, `/dashboard/1234` y `/dashboard/9a` se convierten en `/dashboard/?`.

Para anular los nombres de visualización predeterminados de RUM:

1. Establece `trackViewsManually` en true al inicializar el SDK del navegador RUM.
   ...
   {% /if %}
   <!-- ends CDN sync -->
2. Debes iniciar visualizaciones para cada nueva página o cambio de ruta (para aplicaciones de una sola página). Los datos de RUM se recopilan cuando la visualización comienza.
{% /if %}
<!-- Ends 2.17.0 -->
...
El siguiente ejemplo realiza el seguimiento manual de las vistas en la página `checkout` de una aplicación RUM. No se puede especificar ningún servicio o versión.
...
El siguiente ejemplo realiza el seguimiento manual de las vistas en la página `checkout` de una aplicación RUM. Utiliza `checkout` para el nombre de visualización y asocia el servicio `purchase` con la versión `1.2.3`.
...
El siguiente ejemplo realiza el seguimiento manual de las vistas de página de la página `checkout` en una aplicación RUM. Usa `checkout` para el nombre de visualización y asocia el servicio `purchase` con la versión `1.2.3`.
...
Para sobrescribir los nombres de visualización predeterminados de RUM para que estén alineados con cómo los has definido en tu aplicación de React, debes seguir los pasos a continuación.
...
<!-- Version must meet 2.17.0 -->
{% if semverIsAtLeast($rum_browser_sdk_version, "2.17.0") %}
### Establece el nombre de visualización {% #set-view-name %}

Usa `setViewName(name: string)` para actualizar el nombre de visualización actual. Esto te permite cambiar el nombre de la visualización sin tener que iniciar una nueva.
   <!-- NPM -->
   {% if equals($lib_src, "npm") %}
   ```javascript
   import { datadogRum } from '@datadog/browser-rum';
...
**Nota**: Cambiar el nombre de visualización afecta a la visualización y sus eventos secundarios desde el momento en que se llama al método.
{% /if %}
<!-- Ends 2.17.0 -->
...
El SDK del Navegador RUM captura eventos de RUM y completa sus atributos principales. La función de callback `beforeSend` te da acceso a cada evento recopilado por el SDK del Navegador RUM antes de que se envíe a Datadog.
...
Si un usuario pertenece a múltiples equipos, agrega pares clave-valor adicionales en tus llamadas a Global Context API.

El SDK del navegador RUM ignora los atributos añadidos fuera de `event.context`.

### Enriquecer eventos de RUM con Feature Flags {% #enrich-rum-events-with-feature-flags %}

Puedes [enriquecer los datos de eventos RUM con Feature Flags][14] para obtener contexto adicional y visibilidad en el monitoreo del rendimiento. Esto te permite determinar qué usuarios ven una experiencia de usuario específica y si está afectando negativamente el rendimiento del usuario.

### Modifica el contenido de un evento RUM {% #modify-the-content-of-a-rum-event %}

Por ejemplo, para anonimizar las direcciones de correo electrónico de las URL de tu aplicación web:
<!-- NPM -->
   {% if equals($lib_src, "npm") %}
   ```javascript
   import { datadogRum } from '@datadog/browser-rum';
...
| `error.message`                | Cadena | Un mensaje conciso, legible por humanos, que explica el error en una línea.                                                                                                                         |
...
Con la `beforeSend` API, descarte un evento RUM devolviendo `false`:
<!-- NPM -->
{% if equals($lib_src, "npm") %}
...
Agregar información del usuario a sus sesiones de RUM le ayuda a:
...
Los atributos a continuación son opcionales en versiones anteriores a 6.4.0, pero Datadog recomienda encarecidamente proporcionar al menos uno de ellos. Por ejemplo, debe establecer el ID del usuario en sus sesiones para ver datos relevantes en algunos tableros de RUM predeterminados, que dependen de `usr.id` como parte de la consulta.

| Atributo  | Tipo | Descripción                                                                                              |
|------------|------|----------------------------------------------------------------------------------------------------|
| `usr.id`    | Cadena | Identificador único del usuario.                                                                                  |
| `usr.name`  | Cadena | Nombre amigable del usuario, mostrado por defecto en la interfaz de RUM.                                                  |
| `usr.email` | Cadena | Correo electrónico del usuario, mostrado en la interfaz de RUM si el nombre de usuario no está presente. También se utiliza para obtener Gravatars. |

**Nota**: 'Usuario Público' se muestra en la interfaz de RUM cuando `usr.name` no está configurado, incluso si `usr.email` y `usr.id` están definidos.

Aumente sus capacidades de filtrado agregando atributos adicionales además de los recomendados. Por ejemplo, agregue información sobre el plan del usuario, o a qué grupo de usuarios pertenece.
...
Para agrupar usuarios en diferentes conjuntos, utilice el concepto de cuenta.
...
| `account.name` | Cadena | No | Nombre amigable de la cuenta, mostrado por defecto en la interfaz de RUM. |
...
## Ver contexto {% #view-context %}


A partir de [versión 5.28.0][20], el contexto de los eventos de visualización es modificable. El contexto solo se puede agregar a la visualización actual y llena sus eventos secundarios (como `action`, `error` y `timing`) con las funciones `startView`, `setViewContext` y `setViewContextProperty`.
...
Enriquece o modifica el contexto de los eventos de visualización RUM y los eventos secundarios correspondientes con la API `setViewContextProperty(key: string, value: any)`.
<!-- NPM -->
{% if equals($lib_src, "npm") %}
...
Reemplaza el contexto de tus eventos de visualización RUM y los eventos secundarios correspondientes con la API `setViewContext(context: Context)`.
<!-- NPM -->
{% if equals($lib_src, "npm") %}
...
Al capturar errores, se puede proporcionar contexto adicional en el momento en que se genera un error. En lugar de pasar información adicional a través de la API `addError()`, puedes adjuntar una propiedad `dd_context` directamente a la instancia de error. El SDK del navegador RUM detecta automáticamente esta propiedad y la fusiona en el contexto final del evento de error.
...
Después de que RUM se inicializa, agrega contexto adicional a todos los eventos de RUM recolectados de tu aplicación con la API `setGlobalContextProperty(key: string, value: any)`:
<!-- NPM -->
{% if equals($lib_src, "npm") %}
...
Una vez que RUM se inicializa, lee el contexto global con la API `getGlobalContext()`.
<!-- NPM -->
{% if equals($lib_src, "npm") %}
...
Por defecto, el contexto global y el contexto de usuario se almacenan en la memoria de la página actual, lo que significa que no están:
...
- No se recomienda establecer Información Personalmente Identificable (PII) en esos contextos, ya que los datos almacenados en `localStorage` superan la sesión del usuario.
...
- `localStorage` está limitada a 5 MiB por origen, por lo que los datos específicos de la aplicación, los contextos de Datadog y otros datos de terceros almacenados en el almacenamiento local deben estar dentro de este límite para evitar problemas.
...
Después de que se inicializa el SDK de RUM del navegador de Datadog, puedes acceder al contexto interno del SDK. Esto proporciona identificadores centrales y metadatos que el SDK utiliza internamente, como IDs de sesión y detalles de la aplicación.
...
[Configura el Seguimiento del Navegador][4] en tu aplicación de shell (punto de entrada principal). El SDK del navegador enriquece automáticamente los eventos RUM (errores, acciones personalizadas, recursos XHR/Fetch, tareas largas, vitales) con `service` y `version` del mapa de contexto.

{% alert level="warning" %}
Los eventos que no coinciden con ningún micro frontend pasan al servicio y versión a nivel de shell.
{% /alert %}
...
#### Resolución del mapa del código fuente a través de micro frontends {% #source-map-resolution-across-micro-frontends %}

Cuando una traza de pila contiene marcos de múltiples micro frontends, el evento recibe un solo `service` y `version` del marco más alto (donde se lanzó el error). La resolución de mapas del código fuente se realiza para el evento bajo ese único servicio, por lo que los marcos de otros micro frontends permanecen minimizados, incluso cuando sus mapas del código fuente fueron correctamente subidos bajo su propio `service`.

Para controlar qué mapas del código fuente de micro frontend se utilizan, usa el enfoque de [atribución manual](#manual-service-and-version-attribution) con `beforeSend` para establecer `event.service` y `event.version`. Solo los marcos que pertenecen al micro frontend elegido son desminimizados.

### Explora los datos del micro frontend en Datadog {% #explore-micro-frontend-data-in-datadog %}
...
-   **Paneles laterales**: Los atributos `service` y `version` aparecen en los paneles laterales de sesión, visualización, error, recurso, acción y tareas largas en el Explorador RUM.
...
-   **Tableros personalizados**: Crea tableros usando el `service` y `version` para realizar el seguimiento de cada micro frontend de manera independiente.
