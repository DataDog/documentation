...
`proxyConfiguration`
: Un atributo de configuración de proxy que puede ser utilizado para habilitar un proxy personalizado para subir datos rastreados a la ingesta de Datadog.
...
Determina si se rastrean los eventos de RUM cuando no hay una vista activa. Por defecto, esto está configurado en `false`.
...
Para obtener un desglose de tiempo detallado (resolución DNS, apretón de manos SSL, tiempo hasta el primer byte, tiempo de conexión y duración de descarga), habilite `URLSessionInstrumentation` para su tipo de delegado:
...
Puedes usar la API `Datadog.stopInstance()` para detener una instancia nombrada del SDK (o la instancia predeterminada si el nombre es `nil`) de recopilar y subir datos en adelante.
