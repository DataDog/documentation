...
`<METRIC_NAME>.count`
: Representa el número de valores enviados durante el intervalo, `X`. El Agent envía este número como un RATE, por lo que mostraría en la aplicación el valor de `X/interval`. <br>
...
Si envía `X` valores para una métrica de DISTRIBUTION `<METRIC_NAME>` en un intervalo de tiempo dado, las siguientes agregaciones están disponibles para consulta por defecto:
...
**Nota**: Para obtener métricas de RATE a través de DogStatsD, envíe ya sea una métrica de [COUNT][16] o [HISTOGRAM][18]. Los valores de las métricas COUNT y los valores de `<HISTOGRAM>.count` son deltas normalizados en el tiempo del valor de la métrica durante el período de vaciado de StatsD.
