...
[COUNT](#count), [GAUGE](#gauge), y [SET](#set) son tipos de métricas familiares para los usuarios de StatsD. `TIMER` de StatsD es un subconjunto de `HISTOGRAM` en DogStatsD. Además, puedes enviar tipos de métricas [HISTOGRAM](#histogram) y [DISTRIBUTION](#distribution) usando DogStatsD.

**Nota**: Dependiendo del método de envío utilizado, el tipo de métrica real almacenado en Datadog puede diferir del tipo de métrica enviado. Para obtener métricas de TASA a través de DogStatsD, envía ya sea una métrica [COUNT](#count) o [HISTOGRAM](#histogram). Los valores de métricas de conteo y `<HISTOGRAM>.count` son deltas normalizados por tiempo del valor de la métrica durante el período de vaciado de StatsD.
...
**Nota**: `COUNT` las métricas de tipo pueden mostrar un valor decimal dentro de Datadog ya que están normalizadas durante el intervalo de vaciado para reportar unidades por segundo.
...
: Almacenado como un tipo `GAUGE` en Datadog. Cada valor en la serie temporal almacenada es el último valor de gauge enviado para la métrica durante el período de descarga de StatsD.

#### Ejemplos de código {#code-examples-1}

Emitir una métrica `GAUGE` almacenada como una métrica `GAUGE` en Datadog. Aprende más sobre el tipo `GAUGE` en la documentación de [tipos de métricas][5].

Ejecuta el siguiente código para enviar una métrica DogStatsD `GAUGE` a Datadog. Recuerda `flush`/`close` el cliente cuando ya no sea necesario.

**Nota:** Las llamadas de envío de métricas son asíncronas. Si deseas asegurarte de que las métricas se envían, llama a `flush` antes de que el programa finalice.
...
: Almacenado como un tipo `GAUGE` en Datadog. Cada valor en la serie temporal almacenada es el conteo de valores únicos enviados a StatsD para una métrica durante el período de descarga.

#### Ejemplos de código {#code-examples-2}

Emitir una métrica `SET` almacenada como una métrica `GAUGE` en Datadog.
...
`TIMER` El tipo de métrica en DogStatsD es una implementación de `HISTOGRAM` tipo de métrica (no debe confundirse con temporizadores en el StatsD estándar). Mide solo datos de tiempo: por ejemplo, la cantidad de tiempo que tarda en ejecutarse una sección de código.
...
DogStatsD trata `TIMER` como una métrica `HISTOGRAM`. Ya sea que utilices el tipo de métrica `TIMER` o `HISTOGRAM`, estás enviando los mismos datos a Datadog. Después de ejecutar el código anterior, tus datos de métricas están disponibles para graficar en Datadog:
...
El tipo de métrica `DISTRIBUTION` es específico para DogStatsD. Emite una métrica `DISTRIBUTION` almacenada como una métrica `DISTRIBUTION` a Datadog. Aprende más sobre el tipo `DISTRIBUTION` en la documentación de [tipos de métricas][9].
...
| `HISTOGRAM`    | La estadística `histogram.count` es una métrica de CONTADOR y recibe la corrección descrita anteriormente. Otras estadísticas son métricas de medidor y no son "corregidas".                      |
