...
---
Mientras que StatsD acepta solo métricas, DogStatsD acepta los tres tipos de datos principales de Datadog: métricas, eventos y verificaciones de servicio. Esta sección muestra casos de uso típicos para métricas desglosadas por tipos de métricas, e introduce [tasas de muestreo](#samplerates) y opciones de [etiquetado de métricas](#metrictagging) específicas de DogStatsD.

Los tipos de métricas [COUNT](#count), [GAUGE](#gauge) y [SET](#set) son familiares para los usuarios de StatsD. `TIMER` de StatsD es un subconjunto de `HISTOGRAM` en DogStatsD. Además, puedes enviar tipos de métricas [HISTOGRAM](#histogram) y [DISTRIBUTION](#distribution) usando DogStatsD.
...
| `<SAMPLE_RATE>`  | Doble          | No       | La tasa de muestreo que se aplicará a la métrica. Toma un valor entre `0` (todo se muestrea, por lo que no se envía nada) y `1` (sin muestreo). Consulta la sección [Tasa de Muestreo](#samplerates) para aprender más. |
| `<TAGS>`         | Lista de cadenas | No       | Una lista de etiquetas que se aplicarán a la métrica. Consulta la sección [Etiquetado de Métricas](#metrictagging) para aprender más.                                                                                       |
| `<CARDINALITY>`  | Enum            | No       | La [cardinalidad][10] de etiquetas que se asignarán a esta métrica.                                                                                                                               |

### CONTAR

`incrementar(<METRIC_NAME>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY> )`
: Se utiliza para incrementar una métrica de CONTAR. Almacenado como un tipo `TASA` en Datadog. Cada valor en la serie temporal almacenada es un delta normalizado en el tiempo del valor de la métrica durante el período de vaciado de StatsD.

`decrementar(<METRIC_NAME>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY>)`
: Se utiliza para decrementar una métrica de CONTAR. Almacenado como un tipo `TASA` en Datadog. Cada valor en la serie temporal almacenada es un delta normalizado en el tiempo del valor de la métrica durante el período de vaciado de StatsD.

`contar(<METRIC_NAME>, <METRIC_VALUE>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY>)`
: Se utiliza para incrementar una métrica de CONTAR desde un `Valor` arbitrario. Almacenado como un tipo `TASA` en Datadog. Cada valor en la serie temporal almacenada es un delta normalizado en el tiempo del valor de la métrica durante el período de vaciado de StatsD.

**Nota**: Las métricas de tipo `CONTAR` pueden mostrar un valor decimal dentro de Datadog ya que están normalizadas durante el intervalo de vaciado para reportar unidades por segundo.

#### Ejemplos de código

Emitir una métrica `CONTAR` almacenada como una métrica `TASA` a Datadog. Aprende más sobre el tipo `COUNT` en la documentación de [tipos de métricas][2].

Ejecuta el siguiente código para enviar una métrica `COUNT` de DogStatsD a Datadog. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.
...
: Almacenado como un tipo `GAUGE` en Datadog. Cada valor en la serie temporal almacenada es el último valor de medidor enviado para la métrica durante el período de vaciado de StatsD.
...
Ejecuta el siguiente código para enviar una métrica `GAUGE` de DogStatsD a Datadog. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.

**Nota:** Las llamadas para enviar métricas son asíncronas. Si deseas asegurarte de que las métricas se envían, llama a `flush` antes de que el programa termine.
...
Ejecuta el siguiente código para enviar una métrica `SET` de DogStatsD a Datadog. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.
...
Ejecuta el siguiente código para enviar una métrica `HISTOGRAM` de DogStatsD a Datadog. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.
...
El tipo de métrica `TIMER` en DogStatsD es una implementación del tipo de métrica `HISTOGRAM` (no debe confundirse con temporizadores en el StatsD estándar). Mide solo datos de tiempo: por ejemplo, la cantidad de tiempo que tarda en ejecutarse una sección de código.
...
Para un `TIMER`, se aplican las reglas de [configuración](#configuration) del `HISTOGRAM`.

##### Ejemplos de código

Emite una métrica de `TIMER`—almacenada como una métrica de `GAUGE` y `RATE`—a Datadog. Aprende más sobre el tipo `HISTOGRAM` en la documentación de [tipos de métricas][6]. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.
...
DogStatsD trata el `TIMER` como una métrica de `HISTOGRAM`. Ya sea que uses el tipo de métrica `TIMER` o `HISTOGRAM`, estás enviando los mismos datos a Datadog. Después de ejecutar el código anterior, tus datos de métricas están disponibles para graficar en Datadog:

{{< img src="metrics/custom_metrics/dogstatsd_metrics_submission/timer.png" alt="Temporizador" >}}

### DISTRIBUCIÓN

`distribución(<METRIC_NAME>, <METRIC_VALUE>, <TAGS>, <CARDINALITY>)`
: Almacenado como un tipo de `DISTRIBUTION` en Datadog. Consulta la [documentación de Distribución][8] dedicada para aprender más.
...
Ejecuta el siguiente código para enviar una métrica `DISTRIBUTION` de DogStatsD a Datadog. Recuerda `flush`/`cerrar` el cliente cuando ya no sea necesario.
...
|||
| `COUNT`        | Los valores recibidos se multiplican por (`1/<SAMPLE_RATE>`). Es razonable suponer que para un punto de datos recibido, `1/<SAMPLE_RATE>` fueron realmente muestreados con el mismo valor. |
...
| `HISTOGRAM`    | La estadística `histogram.count` es una métrica de CONTADOR y recibe la corrección mencionada anteriormente. Otras estadísticas son métricas de medidor y no son "corregidas".                      |
| `DISTRIBUTION` | Los valores recibidos se cuentan (`1/<SAMPLE_RATE>`) veces. Es razonable suponer que para un punto de datos recibido, `1/<SAMPLE_RATE>` fueron realmente muestreados con el mismo valor. |
...
La etiqueta de host se asigna automáticamente por el Agente de Datadog que agrega las métricas. Las métricas enviadas con una etiqueta de host que no coincide con el nombre de host del Agente pierden referencia al host original. La etiqueta de host enviada anula cualquier nombre de host recopilado o configurado en el Agente.
...
[3]: /es/dashboards/functions/arithmetic/#cumulativesum
[4]: /es/tableros/funciones/arithmetic/#integral
[5]: /es/métricas/tipos/?tab=gauge#definición
[6]: /es/métricas/tipos/?tab=histograma#definición
[7]: /es/agente/configuración/archivosdeconfiguracióndelagente/#archivo de configuración principal del agente
[8]: /es/métricas/distribuciones/
[9]: /es/métricas/tipos/?tab=distribución#definición
[10]: /es/contenedores/kubernetes/etiqueta