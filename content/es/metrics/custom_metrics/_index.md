...
{{< learning-center-callout header="Únete a una sesión de seminario web de habilitación" hide_image="true" btn_title="Regístrate" btn_url="https://www.datadoghq.com/technical-enablement/sessions/?tags.topics-0=Metrics">}}
  Explora y regístrate para las sesiones de Foundation Enablement para Custom Metrics. Aprende cómo Custom Metrics te ayudan a rastrear los KPIs de tu aplicación, como el número de visitantes, el tamaño promedio del carrito de clientes, la latencia de solicitudes o la distribución del rendimiento para un algoritmo personalizado.
{{< /learning-center-callout >}}

## Descripción general {#overview}

Custom Metrics te ayudan a rastrear los KPIs de tu aplicación: número de visitantes, tamaño promedio del carrito de clientes, latencia de solicitudes o distribución del rendimiento para un algoritmo personalizado. Una Custom Metric se identifica por **una combinación única del nombre de la métrica y los valores de las etiquetas (incluyendo la etiqueta de host)**. En el ejemplo a continuación, el Custom Metric `request.Latency` tiene cuatro combinaciones únicas de valores de etiquetas a partir de sus dos claves de etiqueta:
...
- En general, cualquier métrica enviada a través de [DogStatsD][3] o a través de una [verificación personalizada del Agente][4]
- Métricas enviadas por [Marketplace Integrations][29]
- Ciertas [integraciones estándar](#standard-integrations) pueden potencialmente emitir Custom Metrics
- Métricas enviadas desde una integración que no es una de las [más de {{< translate key="integration_count" >}} Datadog Integrations][1].
...
Una Custom Metric de Datadog tiene las propiedades a continuación. Lea la [introducción a Metrics][7] para aprender cómo graficar métricas dentro de Datadog.

| Propiedad         | Descripción                                                                                                                                                  |
|------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<METRIC_NAME>`  | El [nombre de su Custom Metric](#naming-custom-metrics).                                                                                                                  |
| `<METRIC_VALUE>` | El valor de su Custom Metric. **Nota**: Los valores de su Custom Metric deben ser de 32 bits. Los valores no deben reflejar fechas o marcas de tiempo.                                                                                                                                |
| `<TIMESTAMP>`    | La marca de tiempo asociada con el valor de la Custom Metric. **Nota**: Las marcas de tiempo de su Custom Metric no pueden estar más de diez minutos en el futuro o más de una hora en el pasado. |
| `<TAGS>`         | El conjunto de etiquetas asociadas con su Custom Metric.                                                                                                                 |
| `<METRIC_TYPE>`  | El tipo de su Custom Metric. Lee sobre [los tipos de Custom Metrics][8].                                                                                             |
| `<INTERVAL>`     | Si el `<TYPE>` de su Custom Metric es [RATE][9] o [COUNT][10], define el [intervalo][11] correspondiente.                                                       |

### Naming Custom Metrics {#naming-custom-metrics}

La siguiente convención de nombres para Custom Metrics debe seguirse:

* Los nombres de Custom Metrics deben comenzar con una letra.
* Los nombres de Custom Metrics solo pueden contener caracteres alfanuméricos ASCII, guiones bajos y puntos.
  * Otros caracteres, incluidos los espacios, se convierten en guiones bajos.
  * Unicode _no_ es compatible.
* Los nombres de las métricas no deben exceder los 200 caracteres. Menos de 100 es preferible desde la perspectiva de la interfaz de usuario.

**Nota**: Los nombres de las métricas son sensibles a mayúsculas y minúsculas en Datadog.

### Unidades de métricas {#metric-units}

Establece las unidades de Custom Metrics a través de [Metrics Summary][12] o establece unidades de Custom Metrics con la función de [Unit override][13] en el editor de gráficos de tus visualizaciones. Para más información, consulta la documentación de [Metrics Units][14].

## Enviando Custom Metrics {#submitting-custom-metrics}

{{< whatsnext desc="Hay múltiples formas de enviar Custom Metrics a Datadog:">}}
    {{< nextlink href="/metrics/custom_metrics/agent_metrics_submission" >}}Verificación personalizada del Agente{{< /nextlink >}}
    ...
    {{< nextlink href="/logs/log_configuration/logs_to_metrics/#generate-a-log-based-metric" >}}Generar métricas basadas en log{{< /nextlink >}}
...
También puede utilizar una de las [bibliotecas de cliente API y DogStatsD oficiales y contribuidas por la comunidad de Datadog][15] para enviar sus Custom Metrics
...
Las siguientes Standard Integrations pueden potencialmente emitir Custom Metrics.

| Tipo de Integrations                           | Integrations                                                                       |
|------------------------------------------------|------------------------------------------------------------------------------------|
| Limitado a 350 Custom Metrics por defecto.      | [ActiveMQ XML][16] / [Go-Expvar][17] / [Java-JMX][18]                              |
| Sin límite predeterminado en la recolección de Custom Metrics. | [Nagios][19] /[PDH Check][20] /[OpenMetrics][21] /[contadores de rendimiento de Windows][22] /[WMI][23] /[Prometheus][21] |
