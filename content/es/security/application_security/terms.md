...
- link: https://www.datadoghq.com/blog/datadog-threat-intelligence/
  ...
  text: Acelerar las investigaciones de seguridad con la información sobre amenazas
    de Datadog
title: Términos y conceptos de ASM
---
...
Application Security Management (ASM) monitoriza las amenazas y ofrece una protección contra los ataques a nivel de aplicación que pretenden explotar las vulnerabilidades a nivel de código. Aprovecha el contexto de ejecución del código en tiempo de ejecución, los datos de rastreo y errores, así como la atribución al usuario.
...
: Lenguaje de programación específico de la biblioteca incorporado en aplicaciones web. ASM utiliza la biblioteca para la monitorización y la protección. APM utiliza la misma biblioteca para la instrumentación del código para el rastreo de telemetría.
...
: Método de tests de seguridad de aplicaciones que detecta proactivamente vulnerabilidades mientras la aplicación es ejecutada por una test automatizado, un testeador humano o cualquier actividad que interactúe con la funcionalidad de la aplicación.

Configuración remota
: Mecanismo de la plataforma Datadog que habilita la actualización remota de la configuración del Agent. Utilizado por ASM para actualizar las reglas WAF en la aplicación, activar el producto y bloquear a los atacantes.
...
: Mecanismo que asigna las solicitudes sospechosas a usuarios conocidos en sus sistemas.
...
: Fundación sin ánimo de lucro con varios proyectos para mejorar la seguridad de las aplicaciones web. OWASP es más conocido por el [OWASP Top 10][2], un amplio consenso sobre los riesgos de seguridad más críticos para las aplicaciones web.
...
: Tipo de ataque de inyección en el que se ejecuta una consulta SQL a través de los datos de entrada del cliente a la aplicación. Los comandos SQL se inyectan en la entrada del plano de datos para afectar a la ejecución de comandos SQL predefinidos. Una inyección SQL exitosa puede leer datos sensibles de la base de datos, modificar datos de la base de datos (Insertar/Actualizar/Borrar), ejecutar operaciones de administración en la base de datos (como apagar el DBMS), recuperar el contenido de un archivo dado presente en el sistema de archivos del DBMS y, en algunos casos, enviar comandos al sistema operativo.
...
: Vulnerabilidad que permite a un atacante ejecutar su propia expresión OGNL en una aplicación Java, lo que suele conducir a la ejecución remota de código.
