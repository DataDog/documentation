...
title: Activación de la detección y protección de amenazas de AAP mediante la instrumentación
  de un solo step (UI) / paso (generic)
---

<div class="alert alert-info">La activación de la detección y protección de amenazas de AAP mediante la instrumentación de un solo step (UI) / paso (generic) está en Vista previa.</div>

## Requisitos

- **Versión mínima del Agent: 7.53.0**
- **Versión mínima de Helm: 3.62.0** (para los despliegues de Kubernetes)
- **Lenguajes y arquitecturas**: La instrumentación de AAP en un solo step (UI) / paso (generic) solo admite el rastreo de servicios de Java, Python, Node.js y .NET Core en las arquitecturas `x86_64` y `arm64`.
...
## Activación en un step (UI) / paso (generic)

Si [instalas o actualizas un Datadog Agent][1] con la opción **Enable Threat Protection (new)** (Activar protección contra amenazas (nuevo)) seleccionada, el Agent se instala y configura para activar AAP. Esto te permite instrumentar automáticamente tu aplicación, sin ningún step (UI) / paso (generic) adicional de instalación o configuración. Reinicia servicios para que esta instrumentación surta efecto.


{{< img src="/security/application_security/single_step/asm_single_step_threat_detection_2.png" alt="Configuración de la cuenta: page (página) de configuración de Ubuntu que resalta la alternancia para la activación de la instrumentación y protección ante amenazas de APM." style="width:100%;" >}}
...
Con un solo comando, puedes instalar, configurar e iniciar el Agent, a la vez que instrumentas tus servicios con AAP.
...
**Nota:** Para configurar en un solo step (UI) / paso (generic) la protección contra amenazas de AAP y Code Security, añade las variables de entorno `DD_APPSEC_ENABLED=true` _y_ `DD_IAST_ENABLED=true` a tu comando de instalación de una línea.

### Especificación de las versiones de biblioteca de rastreo {#lib-linux}

En forma predeterminada, la activación de APM en tu servidor instala la compatibilidad con los servicios de Java, Python, Node.js, y .NET Core. Si solo tienes servicios implementados en algunos de estos lenguajes, configura `DD_APM_INSTRUMENTATION_LIBRARIES` en tu comando de instalación de una línea:
...
{{% /tab %}}
{{< /tabs >}}
## Eliminación de la instrumentación de APM y AAP en un solo step (UI) / paso (generic) de tu Agent
...
Ejecuta los siguientes comandos y reinicia el servicio para dejar de insertar la biblioteca en el servicio y dejar de producir traces (trazas) desde ese servicio.
{{< tabs>}}
{{% tab "Host o máquina virtual de Linux" %}}
1. Añade la variable de entorno `DD_INSTRUMENT_SERVICE_WITH_APM` al comando de inicio de servicio:
   ```shell
   DD_INSTRUMENT_SERVICE_WITH_APM=false <service_start_command>
   ```
2. Reinicia el servicio.
3. Para desactivar la AAP, elimina la variable de entorno`DD_APPSEC_ENABLED=true` de la configuración de tu aplicación y reinicia tu servicio.
{{% /tab %}}
{{% tab "Docker" %}}
1. Añade la variable de entorno `DD_INSTRUMENT_SERVICE_WITH_APM` al comando de inicio de servicio:
   ```shell
...
{{% /tab %}}
{{% tab "Docker" %}}
1. Desinstala la inserción de biblioteca local:
   ```shell
