...
<div class="alert alert-info"><strong>Nota</strong>: Existe un límite predeterminado de 1000 monitores de RUM por cada cuenta. Si estás por alcanzar este límite, considera la posibilidad de utilizar <a href="/monitors/configuration/?tab=thresholdalert#alert-grouping">alertas múltiples</a> o <a href="/help/">ponte en contacto con el servicio de asistencia</a>.</div>
...
2. Elige monitorizar en base a un número de eventos RUM, una [faceta][4] o una [medida][5].
    ...
    * **Monitorizar en base a una faceta**: si seleccionas una [faceta][4], el monitor envía alertas sobre el `Unique value count` de la faceta.
    * **Monitorizar en base a una medida**: si seleccionas una [medida][5], el monitor envía alertas sobre el valor numérico de la faceta RUM (similar a un monitor de métricas). Selecciona un tipo de agregación (`min`, `avg`, `sum`, `median`, `pc75`, `pc90`, `pc95`, `pc98`, `pc99` o `max`).
...
Para obtener más información sobre las opciones avanzadas de alerta, como el tiempo de espera para la evaluación, consulta [Configurar monitores][6].
