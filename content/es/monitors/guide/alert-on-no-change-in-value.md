...
kind: Guía
title: Alerta por falta de variación del valor
---

Una forma sencilla de activar una alerta, cuando un valor de métrica no cambia en un periodo de tiempo determinado, es empezar utilizando la función `diff()` en tu consulta. Esto generará valores delta a partir de puntos de datos consecutivos.

* `diff(avg:system.mem.free{*})`

Luego, necesitas aplicar la función abs() función para obtener el valor absoluto de estos valores delta.
...
También puedes introducir tu consulta compleja manualmente en la interfaz de usuario 'edit monitor' (editar monitor'), a través de la pestaña Source (Fuente) (o aplicarla mediante programación a través de la [API][1]). Consulta la imagen siguiente.

Para definir las [condiciones de alerta][2] en el propio monitor de métricas, configura lo siguiente

* Seleccionar el umbral de alerta
