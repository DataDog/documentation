---
description: Aprende a identificar y limpiar el desorden en los monitores analizando
  los patrones de uso, identificando monitores no utilizados o redundantes, e implementando
  las mejores prácticas para la gestión de monitores para mejorar la calidad de las
  alertas y reducir el ruido.
further_reading:
...
title: Limpia el desorden de los monitores
---
## Resumen

El desorden de los monitores se acumula con el tiempo, resultando en ruido, alertas duplicadas y un aumento de la fricción operativa. Esta guía describe un enfoque claro para identificar y limpiar monitores desordenados, con casos de uso que te ayudarán a optimizar tus flujos de trabajo de alertas.

También proporciona mejores prácticas para ayudar a mantener un entorno de monitoreo limpio, facilitando la escalabilidad y gobernanza de tu estrategia de monitoreo a medida que tus sistemas crecen.
...
Esta guía cubre varios casos de uso clave para limpiar el desorden de los monitores:
...
- **[Monitores inestables y ruidosos](#flappy-and-noisy-monitors)**: Monitores que se activan y resuelven con frecuencia (es decir, "fluctúan") o producen altos volúmenes de alertas de bajo valor.
...
Audite los monitores que han estado silenciados por un largo período de tiempo para entender cuáles son realmente necesarios o útiles. Algunos monitores pueden estar silenciados por una buena razón y desea evitar eliminarlos.
...
Después de tener su lista, puede tomar acción en cada monitor desde la página de Calidad del Monitor o hacer una eliminación masiva de monitores con los pasos 2 y 3.
...
curl -s -X GET "{{< region-param key=dd_api >}}/api/v1/monitor/search" \
  ...
  .monitores[]
  | seleccionar(.quality_issues != null y (.quality_issues | index("muted_duration_over_sixty_days")))
  | [.id, .nombre, (.quality_issues | join(";"))]
  | @csv' > monitores_muted.csv
```
...
curl -s -X GET "{{< region-param key=dd_api >}}/api/v1/monitor/search" \
  ...
  -H "Content-Type: application/json" | jq -r '
  .monitores[]
  | seleccionar(.quality_issues != null y (.quality_issues | index("alerted_too_long")))
  | [.id, .nombre, (.quality_issues | join(";"))]
  | @csv' > monitores_alerted_too_long.csv
```
...
curl -s -X GET "{{< region-param key=dd_api >}}/api/v1/monitor/search" \
  ...
  -H "Content-Type: application/json" | jq -r '
  .monitores[]
  | seleccionar(.quality_issues != null y (.quality_issues | index("noisy_monitor")))
  | [.id, .nombre, (.quality_issues | join(";"))]
  | @csv' > monitores_noisy.csv
```
...
curl -s -X GET "{{< region-param key=dd_api >}}/api/v1/monitor/search" \
  ...
  -H "Content-Type: application/json" | jq -r '
  .monitores[]
  | seleccionar(.quality_issues != null y (.quality_issues | index("broken_at_handle")))
  | [.id, .nombre, (.quality_issues | join(";"))]
  | @csv' > monitores_broken_handle.csv
```
...
curl -s -X GET "{{< region-param key=dd_api >}}/api/v1/monitor/search" \
  ...
  -H "Content-Type: application/json" | jq -r '
  .monitores[]
  | seleccionar(.quality_issues != null y (.quality_issues | index("crawler_metric_missing_eval_delay")))
  | [.id, .nombre, (.quality_issues | join(";"))]
  | @csv' > monitores_missing_delay.csv
```
      ...
      "definition": {
        ...
        "widgets": [
                  ...
                  "consultas": [
                    {
                      ...
                      "consulta": "suma:datadog.monitor.suggested_monitor_health_by_team{$team,$service} por {tipo_de_sugerencia}"
                    }
                  ]},
              ...
              "enlaces_personalizados": [
                {
                  ...
                  "enlace": "https://app.datadoghq.com/monitors/quality?q={{$team}}"
                }
              ]
            },
            "diseño": { "x": 0, "y": 4, "ancho": 12, "alto": 4 }
          },
          {
            "id": 1376609088194674,
            "definition": {
                  ...
                  "consultas": [
                    {
                      "nombre": "consulta1",
                      "fuente_de_datos": "métricas",
                      "consulta": "suma:datadog.monitor.suggested_monitor_health_by_team{!team:none,$team,$service} por {equipo,tipo_de_sugerencia}",
                      "agregador": "último"
                    }
                  ]},
            ...
            "diseño": { "x": 0, "y": 8, "ancho": 12, "alto": 4 }
          },
          {
            "id": 718136447073638,
            "definition": {
              "tipo": "nota",
              "contenido": "Monitores con Destinatarios Faltantes por Equipo",
          ...
          },
          {
            ...
            "diseño": { "x": 0, "y": 15, "ancho": 6, "alto": 5 }
          },
          {
            "id": 3744392131942638,
            "definition": {
                  ...
                  "consultas": [
                    {
                      "nombre": "consulta1",
                      "fuente_de_datos": "métricas",
                      "consulta": "sum:datadog.monitor.suggested_monitor_health_by_team{!equipo:none,tipo_sugerencia:roto_en_mango,$equipo,$servicio} por {equipo,tipo_sugerencia}",
                      "agregador": "último"
                    }
                  ]},
                  "formato_de_respuesta": "escalar",
          ...
          },
          {
            ...
            "definición": {
              ...
              "mostrar_tic": true,
              "posición_del_tic": "50%",
              "borde_del_tic": "inferior",
              "tiene_relleno": true
            },
            "diseño": { "x": 0, "y": 29, "ancho": 6, "alto": 2 }
          },
          {
            "id": 1329067816249636,
            "definición": {
            ...
            "diseño": { "x": 6, "y": 29, "ancho": 6, "alto": 2 }
          },
          {
            "id": 7052384595427880,
            "definición": {
                  ...
                  "consultas": [
                    {
                      "nombre": "consulta1",
                      "fuente_de_datos": "métricas",
                      "consulta": "sum:datadog.monitor.suggested_monitor_health_by_service{$equipo,$servicio} por {tipo_sugerencia}",
                      "agregador": "último"
                    }
                  ] ,
                  "formato_de_respuesta": "escalar",
                  ...
                  "consultas": [
                    {
                      "nombre": "consulta1",
                      "fuente_de_datos": "métricas",
                      "query": "suma:datadog.monitor.suggested_monitor_health_by_service{$equipo, $servicio} por {tipo_de_sugerencia}"
                    }
                  ] ,
                  ...
                  "consultas": [
                    {
                      "nombre": "consulta1",
                      "fuente_de_datos": "métricas",
                      "query": "suma:datadog.monitor.suggested_monitor_health_by_service{!servicio:none,$equipo,$servicio} por {servicio,tipo_de_sugerencia},"
                      "agregador": "último"
                    }
                  ] ,
                  "formato_de_respuesta": "escalar",
