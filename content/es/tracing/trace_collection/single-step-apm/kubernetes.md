...
- link: /tracing/guide/init_resource_calc/
  tag: Documentación
  text: Aprenda sobre el uso de recursos del contenedor init
- link: /tracing/guide/local_sdk_injection
  tag: Documentación
  text: Instrumente sus aplicaciones utilizando la inyección local de SDK.
...
<div class="alert alert-info">La instrumentación de un solo paso no instrumenta aplicaciones en el espacio de nombres donde está instalado el Datadog Agent. Instale el Datadog Agent en un espacio de nombres separado donde no ejecute sus aplicaciones.</div>
...
1. En Datadog, ve a la página de [Instalar el Datadog Agent en Kubernetes][11].
1. Sigue las instrucciones en pantalla para elegir tu método de instalación, seleccionar una clave de API y configurar el Datadog Operator o el repositorio de Helm.
1. En la sección **Configurar `datadog-agent.yaml`**, ve a **Configuración adicional** > **Observabilidad de aplicaciones**, y activa **Instrumentación APM**.
   ...
   {{< img src="tracing/trace_collection/k8s-apm-instrumentation-toggle.jpg" alt="El bloque de configuración para instalar el Datadog Agent en Kubernetes a través de la aplicación Datadog." style="width:100%;" >}}

1. Despliega el Datadog Agent utilizando el archivo de configuración generado.
1. Reinicia tus aplicaciones.

<div class="alert alert-info">SSI añade un pequeño tiempo de inicio a las aplicaciones instrumentadas. Si este sobrecosto no es aceptable para tu caso de uso, contacta a <a href="/help/">Datadog Support</a>.</div>

## Configura las Etiquetas de Servicio Unificadas {#configure-unified-service-tags}

Las Etiquetas de Servicio Unificadas (USTs) aplican etiquetas consistentes a través de trazas, métricas y registros, facilitando la navegación y correlación de tus datos de observabilidad. Puedes configurar USTs a través de la extracción automática de etiquetas (recomendado), mediante configuración explícita con `ddTraceConfigs`, o en manifiestos de implementación.

<div class="alert alert-warning">
Si estás utilizando <a href="/agent/remote_config/">Configuración Remota</a>, <a href="#recommended-configure-usts-through-automatic-label-extraction">la extracción automática de etiquetas</a> no es compatible. Debes <a href="#configure-usts-explicitly-with-ddtraceconfigs">configurar USTs explícitamente</a> utilizando <code>ddTraceConfigs</code>.
</div>

### (Recomendado) Configura USTs a través de la extracción automática de etiquetas {#recommended-configure-usts-through-automatic-label-extraction}

Con SSI, puedes extraer automáticamente los valores de UST de las etiquetas y metadatos de los pods sin modificar despliegues individuales. Para hacer esto, configura `kubernetesResourcesLabelsAsTags` para mapear tus etiquetas de Kubernetes existentes a las etiquetas de servicio de Datadog.

**Nota:** Este método no es compatible con la Configuración Remota. Si estás utilizando Configuración Remota, consulta [Configura USTs explícitamente con ddTraceConfigs](#configure-usts-explicitly-with-ddtraceconfigs).
...
Reemplaza `app.kubernetes.io/name` en el siguiente ejemplo con cualquier etiqueta que contenga el nombre de tu servicio (por ejemplo, `service.kubernetes.io/name` o `component`). Puede configurar múltiples etiquetas de esta manera.
...
En la mayoría de los casos, la configuración automática es suficiente. Sin embargo, si necesita un control granular sobre la configuración para cargas de trabajo específicas, use `ddTraceConfigs` para mapear explícitamente etiquetas a configuraciones de servicio:
...
### Configure USTs en manifiestos de implementación {#configure-usts-in-deployment-manifests}

Si tu configuración no utiliza etiquetas adecuadas para la extracción de UST, puedes establecer USTs directamente en tus manifiestos de implementación utilizando variables de entorno. Este enfoque requiere modificar cada implementación individualmente, pero ofrece un control preciso.

Para instrucciones completas, consulte [configuración de USTs para servicios de Kubernetes][5].

## Habilitar productos y características dependientes del SDK {#enable-sdk-dependent-products-and-features}

Después de que SSI cargue el SDK de Datadog en tus aplicaciones y habilite el trazado distribuido, puedes configurar productos adicionales que dependen del SDK:
...
Utilice las siguientes opciones avanzadas para personalizar cómo se comporta Single Step Instrumentation en su entorno. Estas configuraciones son opcionales y generalmente solo se necesitan en configuraciones especializadas.

### Configurar modos de inyección {#configure-injection-modes}

SSI admite múltiples modos de inyección, que controlan cómo se entregan los archivos del inyector y de la biblioteca APM a los contenedores de su aplicación. Generalmente, no necesita configurar esta opción manualmente. Considere ajustarla si nota retrasos significativos en el inicio de los pods o un uso de recursos (CPU, memoria) más alto de lo esperado durante la inicialización del pod. Para más información sobre cómo funciona el inyector, consulte [Comportamiento del inyector con Single Step Instrumentation][41].
...
| `csi` | **En vista previa.** Monta archivos del inyector y de la biblioteca APM utilizando el [Datadog CSI driver][37]. Reduce el tiempo de inicio del pod en comparación con el modo de contenedor de inicialización. | Datadog Agent 7.76.0+, Datadog CSI driver 1.2.0+, Helm Chart 3.178.1+ o Datadog Operator 1.25.0+ |

Antes de usar el modo `csi`, instala y activa el Datadog CSI driver. Si estás implementando con Helm, también establece `datadog.csi.enabled: true` en tu `datadog-values.yaml`. Consulta la [documentación del Datadog CSI driver][37] para los pasos de instalación y los requisitos específicos del entorno, como GKE Autopilot.
...
Para establecer el modo de inyección a nivel de clúster, agrega `injectionMode` a tu `datadog-values.yaml`:
...
Para establecer el modo de inyección a nivel de clúster, agrega `injectionMode` a tu `datadog-agent.yaml`:
...
Si estás utilizando el Datadog Operator anterior a la versión 1.25.0, utiliza la [anotación del pod](#configure-injection-mode-per-pod) para anular el modo de inyección para pods específicos.

{{% /tab %}}
{{< /tabs >}}

#### Configura el modo de inyección por pod {#configure-injection-mode-per-pod}

Para anular el modo de inyección para un pod específico, agrega la siguiente anotación a la especificación del pod:
...
### Dirige cargas de trabajo específicas {#target-specific-workloads}

Por defecto, SSI instrumenta todos los servicios en todos los espacios de nombres en tu clúster. Dependiendo de la versión de tu Datadog Agent, utiliza uno de los siguientes métodos de configuración para refinar qué servicios son instrumentados y cómo.
...
- El primer bloque (nombrado `login-service_namespace`):
  ...
  - instruye a Datadog a instrumentar servicios en este espacio de nombres con la versión predeterminada del SDK de Java.
...
- El segundo bloque (nombrado `billing-service_apps`)
  ...
  - instruye a Datadog a instrumentar este conjunto de servicios con `v3.1.0` del SDK de Python.
...
- instruye a Datadog para usar las versiones predeterminadas de los SDK de Datadog Tracer.
- establece las variables de entorno de Datadog que se aplican a cada grupo objetivo y configura los SDKs.
...
- instruye a Datadog para usar la versión predeterminada del SDK de Datadog Java Tracer.
...
Esta configuración habilita [Protección de Aplicaciones y API (AAP)][12] y [Profiler Continuo][11] para los servicios en el espacio de nombres `web-apps`, utilizando `ddTraceConfigs` para establecer las variables de entorno requeridas:
...
Para habilitar la instrumentación para espacios de nombres específicos, agregue la configuración `enabledNamespaces` a `datadog-agent.yaml`:
...
Para deshabilitar la instrumentación para espacios de nombres específicos, agregue la configuración `disabledNamespaces` a `datadog-agent.yaml`:
...
#### Especifique las versiones del SDK {#specify-sdk-versions}

<div class="alert alert-info">A partir de Datadog Cluster Agent v7.52.0+, puede instrumentar automáticamente un subconjunto de sus aplicaciones, basado en los SDK que especifique.</div>

Especifique los SDK de Datadog y sus versiones para instrumentar automáticamente aplicaciones escritas en esos lenguajes. Puede configurar esto de dos maneras, que se aplican en el siguiente orden de precedencia:
...
**Por defecto**: Si no especifica ninguna versión de biblioteca, las aplicaciones escritas en lenguajes compatibles se instrumentan automáticamente utilizando las últimas versiones del SDK.
...
Reemplace `<CONTAINER IMAGE TAG>` con la versión de biblioteca deseada. Las versiones disponibles se enumeran en los [registros de contenedores de Datadog](#change-the-default-image-registry) y en los repositorios de código fuente del tracer para cada lenguaje:
...
<div class="alert alert-danger">Tenga precaución al usar el <code>latest</code> etiqueta, ya que las versiones principales de la biblioteca pueden introducir cambios incompatibles.</div>
...
Si no habilita la instrumentación automática para pods específicos usando anotaciones, puede especificar qué lenguajes instrumentar en todo el clúster usando la configuración de SSI. Cuando `apm.instrumentation.libVersions` está configurado, solo se instrumentan las aplicaciones escritas en los lenguajes especificados, utilizando las versiones de biblioteca especificadas.
...
### Cambie el registro de imágenes predeterminado {#change-the-default-image-registry}
...
La variable de entorno `DD_ADMISSION_CONTROLLER_AUTO_INSTRUMENTATION_CONTAINER_REGISTRY` en la configuración del Agente del Clúster de Datadog especifica el registro utilizado por el Controlador de Admisión. El valor predeterminado es `gcr.io/datadoghq`.

Puede extraer el SDK de un registro diferente cambiándolo a `docker.io/datadog`, `public.ecr.aws/datadog`, o a otra URL si aloja las imágenes en un registro local de contenedores.

Para obtener instrucciones sobre cómo cambiar su registro de contenedores, consulte [Cambio de su registro de contenedores][33].

### Utilice un registro de contenedores privado {#use-a-private-container-registry}
...
Para usar SSI con un registro de contenedores privado:

1. Siga [estas instrucciones][34] para reflejar las imágenes de contenedores de Datadog en su registro privado.

   Solo necesita las imágenes para los lenguajes que está instrumentando. Si no está seguro de cuáles necesita, aquí hay una base que cubre la mayoría de los casos de uso:
...
2. Etiquete las imágenes de acuerdo con su configuración.

   Las versiones que refleje deben coincidir con las versiones configuradas en sus cargas de trabajo, que pueden estar establecidas de una de las siguientes maneras:
...
3. Actualice la configuración del Agente del Clúster para usar su registro privado.
   ...
   Establezca la variable de entorno `DD_ADMISSION_CONTROLLER_AUTO_INSTRUMENTATION_CONTAINER_REGISTRY` en la configuración del Agente del Clúster para usar su registro privado.

Para más detalles sobre cómo cambiar su registro de contenedores, consulte [Changing Your Container Registry][33].
...
Para usar la instrumentación de Un Solo Paso, modifique el Agente del Clúster de Datadog con el parámetro `useHostNetwork: true`.
...
Con las reglas de instrumentación (disponibles para el Agente v7.64+), puede activar y desactivar la traza para aplicaciones específicas. [Consulte los detalles de configuración aquí](#advanced-options).
...
El archivo que debe configurar depende de si habilitó la instrumentación de Un Solo Paso con Datadog Operator o Helm:
...
#### Instrumentación por defecto vs. instrumentación de opt-in {#default-vs-opt-in-instrumentation}
...
| Opt-in | Utiliza [reglas de instrumentación][4] para restringir la instrumentación a espacios de nombres o pods específicos. | Clústeres de producción, implementaciones escalonadas o casos de uso sensibles al costo. |
...
2. En la configuración de Helm de su Agente de Datadog, habilite SSI y utilice `podSelector` para inyectar solo en pods con la etiqueta de opt-in que coincida.
...
Utilice `ddTraceVersions` en la configuración de Helm de su Agente para controlar tanto el idioma como la versión del SDK de Datadog. Esto evita la descarga de SDKs innecesarios, lo que minimiza la huella del contenedor de inicialización, reduce el tamaño de la imagen y permite actualizaciones de trazadores más deliberadas (por ejemplo, para cumplir con los requisitos de cumplimiento o simplificar la depuración).

#### Ejemplo: Especifique un SDK de Java para un espacio de nombres {#example-specify-a-java-sdk-for-a-namespace}

Solo las aplicaciones Java se ejecutan en el espacio de nombres `login-service`. Para evitar descargar otros SDK, configure el Agente para que apunte a ese espacio de nombres e inyecte solo la versión 1.48.2 del SDK de Java.
...
Si un pod no coincide con ninguna regla `ddTraceVersions`, se aplica el destino predeterminado.
