...
title: Trazando aplicaciones Java
type: multi-code-lang
---
## Requisitos de compatibilidad {#compatibility-requirements}

El último trazador de Java es compatible con todas las versiones de JVM 8 y superiores. Para obtener información adicional sobre las versiones de JVM anteriores a 8, lee [Supported JVM runtimes][10].
...
Antes de comenzar, asegúrate de haber [instalado y configurado el Agente][18].

### Instrumenta tu aplicación {#instrument-your-application}

Después de instalar y configurar tu Agente de Datadog, el siguiente paso es agregar el SDK directamente en la aplicación para instrumentarla. Lee más sobre [información de compatibilidad][1].

Para comenzar a trazar tus aplicaciones:

1. Descarga `dd-java-agent.jar` que contiene los últimos archivos de clase del trazador, a una carpeta que sea accesible por tu usuario de Datadog:
...
{{< /tabs >}}
   ...
   **Nota:** Para descargar la última compilación de una versión mayor específica ****, utiliza el `https://dtdg.co/java-tracer-vX` enlace en su lugar, donde `X` es la versión mayor deseada.
   Por ejemplo, usa `https://dtdg.co/java-tracer-v1` para la última versión de la compilación 1. Los números de versión menores no deben incluirse. Alternativamente, consulta el [repositorio de Maven de Datadog][3] para cualquier versión específica.

   **Nota**: Las versiones de Release Candidate están disponibles en GitHub [DataDog/dd-trace-java releases][21]. Estas tienen "RC" en la versión y se recomiendan para pruebas fuera de tu entorno de producción. Puedes [suscribirte a las notificaciones de lanzamientos de GitHub][20] para ser informado cuando nuevas Release Candidates estén disponibles para pruebas. Si experimentas algún problema con las Release Candidates, contacta a [soporte de Datadog][22].

2. Para ejecutar tu aplicación desde un IDE, script de aplicación de Maven o Gradle, o `java -jar` comando, con Continuous Profiler, seguimiento de despliegues e inyección de registros (si estás enviando registros a Datadog), agrega el argumento `-javaagent` JVM y las siguientes opciones de configuración, según corresponda:
...
| `DD_LOGS_INJECTION`   | `dd.logs.injection`     | Habilitar la inyección automática de claves MDC para los ID de traza y tramo de Datadog. Consulta [Uso Avanzado][6] para más detalles. <br><br>A partir de la versión 1.18.3, si [Agent Remote Configuration][16] está habilitada donde se ejecuta este servicio, puedes establecer `DD_LOGS_INJECTION` en la interfaz de usuario del [Software Catalog][17]. |
| `DD_PROFILING_ENABLED`      | `dd.profiling.enabled`          | Habilitar el [Continuous Profiler][5] |
| `DD_SERVICE`   | `dd.service`     | El nombre de un conjunto de procesos que realizan el mismo trabajo. Utilizado para agrupar estadísticas para tu aplicación. |
| `DD_TRACE_SAMPLE_RATE` | `dd.trace.sample.rate` |   Establecer una tasa de muestreo en la raíz de la traza para todos los servicios. <br><br> A partir de la versión 1.18.3, si [Agent Remote Configuration][16] está habilitada donde se ejecuta este servicio, puedes establecer `DD_TRACE_SAMPLE_RATE` en la interfaz de usuario del [Software Catalog][17].     |
...
Opciones de configuración adicionales [](#configuration) se describen a continuación.
...
Para más información, consulte la [documentación de Spring Boot][1].
...
Para habilitar el seguimiento al ejecutar Tomcat en Linux:
...
Para habilitar el seguimiento al ejecutar Tomcat como un servicio de Windows:
...
4. En el campo de texto **Argumentos genéricos de JVM**, ingresa:

```text
-javaagent:/path/to/dd-java-agent.jar
```

Para detalles y opciones adicionales, consulta la [documentación de WebSphere][1].
...
- Si agregas el argumento `-javaagent` a tu comando `java -jar`, debes colocarlo _ antes _ del argumento `-jar`, como una opción de JVM, no como un argumento de la aplicación. Por ejemplo:
...
La instrumentación automática para Java utiliza las capacidades de instrumentación `java-agent` [proporcionadas por la JVM][8]. Cuando se registra un `java-agent`, puede modificar archivos de clase en el momento de carga.
...
Si es necesario, configura el SDK para enviar datos de telemetría de rendimiento de la aplicación según lo requieras, incluyendo la configuración de unified service tagging. Lee [Configuración de la Biblioteca][9] para más detalles.

### Remote Configuration {#remote-configuration}

Remote Configuration permite que el Agente de Datadog configure dinámicamente los ajustes de trazado sin requerir reinicios de la aplicación. Por defecto, Remote Configuration está habilitada. Para deshabilitarla, establece la variable de entorno:

```
DD_REMOTE_CONFIG_ENABLED=false
```

O agrega la propiedad del sistema de la JVM:
