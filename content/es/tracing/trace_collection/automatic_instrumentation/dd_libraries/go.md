...
El rastreador de Go requiere Go `v1.18 o superiores` y el Datadog Agent v5.21.1 o anteriores. Para ver la lista completa de versiones de Go y de compatibilidad de frameworks de Datadog (incluidas las versiones heredadas y de mantenimiento), consulta la página de [requisitos de compatibilidad][1].
...
- Función exclusiva [Protección de aplicaciones y API][7] **Prevención de exploits**. [Prevención de vulnerabilidades][15] es una implementación de autoprotección de aplicaciones en tiempo de ejecución (RASP) e incluye métodos de RASP como la inclusión de archivos locales (LFI).
...
1. Instalar Orchestrion:
   ...
   ```
   <div class="alert alert-info">Asegúrate de que <code>$(go env GOBIN)</code> o <code>$(go env GOPATH)/bin</code> esté en tu <code>$PATH</code>.</div>
...
Puedes eliminar integraciones modificando las importaciones en el archivo `orchestrion.tool.go`.
También puedes crear tu propio archivo `orchestrion.tool.go` antes de poder ejecutar `orchestrion`.
Puedes hacerlo si no deseas una integración
o si deseas reducir el número de dependencias transitivas para integraciones que tu programa no utiliza.
En forma predeterminada, Orchestrion importa `github.com/DataDog/dd-trace-go/orchestrion/all/v2`,
que importa todas las bibliotecas para las que hay una integración de Orchestrion.
Puedes sustituir esta importación con importaciones solo de las integraciones que desees utilizar.
Consulta [el código source (fuente) del rastreador][17] para obtener la lista de integraciones admitidas.

**Nota**: Si eliges importar integraciones específicas, deberás actualizar manualmente `orchestrion.tool.go` cada vez que desees añadir una nueva integración.
...
En primer lugar, importa e inicia el rastreador en tu código siguiendo la documentación [Configuración de bibliotecas][3]. Consulta la [Documentación de la API][6] (o la [Documentación de la API v1][4]) para obtener instrucciones de configuración y detalles sobre el uso de la API.

### Integraciones activas de Go para crear tramos

Activa las [integraciones de Go][1] para generar tramos. Datadog tiene una serie de paquetes conectables que proporcionan soporte "predefinido" para instrumentar una serie de bibliotecas y frameworks. En la página [Requisitos de compatibilidad][1] se puede encontrar una lista de estos paquetes. Importa estos paquetes a tu aplicación y sigue las instrucciones de configuración que aparecen junto a cada integración.
