...
- **Muestreo**: Utiliza cuando desees que las solicitudes permanezcan visibles en las métricas de traza, pero que se reduzca el volumen de ingesta de trazas.
- **Filtrado en el Agente de Datadog**: Utiliza para excluir solicitudes por completo (incluyendo de las métricas de rastreo) en todos los servicios que reportan al Agente.
...
Si necesitas ayuda para decidir cuál opción es la más relevante para tu caso de uso, contacta a [soporte de Datadog][1]. 

## Muestreo {#sampling}

Si deseas que el tramo esté incluido en las métricas de traza pero no quieres que se ingeste, utiliza reglas de muestreo. Para más información sobre muestreo, consulta [Ingestion Control][4].
...
El componente Trace Agent dentro del Agente de Datadog tiene dos métodos para prevenir que ciertas trazas sean enviadas: filtrado por etiquetas de tramo o filtrado por recursos. Si se descartan trazas debido a estas configuraciones, las métricas de traza excluyen estas solicitudes.
...
Si ninguna de las opciones en esta guía satisface sus requisitos, considere agregar una <a href="/tracing/trace_collection/custom_instrumentation/otel_instrumentation/">etiqueta de tramo personalizada</a> en su aplicación y usarla para descartar trazas en el Agente.
</div>

### Ignorando trazas basadas en etiquetas de tramo {#ignoring-traces-based-on-span-tags}

A partir de Datadog Agent 6.27.0/7.27.0, la opción **etiquetas de filtro** descarta trazas con tramos raíz que coinciden con las etiquetas de tramo especificadas. Esta opción se aplica a todos los servicios que envían trazas a este Agente de Datadog. Las trazas que se descartan debido a las etiquetas de filtro no se incluyen en las métricas de traza.

<div class="alert alert-info">
Los tramos individuales dentro de una traza no pueden ser descartados selectivamente; si el tramo raíz coincide con los criterios de filtro, la traza completa se descarta.
</div>

**Comportamiento de coincidencia:**

La opción de etiquetas de filtro requiere una coincidencia exacta de cadena. Para filtrado basado en regex, consulte [Ignorando basado en recursos](#ignoring-traces-based-on-resources).

Cuando especifiques múltiples etiquetas, el filtro utiliza **lógica OR**: las trazas se descartan si el tramo raíz coincide con **cualquiera** de las etiquetas. Para coincidir múltiples condiciones simultáneamente, agregue una etiqueta personalizada que represente esos criterios combinados.

**Configuración:**

Puede especificar etiquetas de tramo que requiera o rechace utilizando una lista de claves y valores separados por espacios en las variables de entorno:

`DD_APM_FILTER_TAGS_REQUIRE`
: Recoge solo trazas que tienen tramos raíz con una coincidencia exacta para las etiquetas de tramo y valores especificados. Si no coincide con esta regla, la traza se descarta. Por ejemplo, `DD_APM_FILTER_TAGS_REQUIRE="key1:value1 key2:value2"`. En Datadog Agent 7.49+, se pueden proporcionar expresiones regulares con `DD_APM_FILTER_TAGS_REGEX_REQUIRE`.

`DD_APM_FILTER_TAGS_REJECT`
Rechaza trazas que tienen tramos raíz con una coincidencia exacta para las etiquetas de tramo y valores especificados. Si coincide con esta regla, la traza se descarta. Por ejemplo, `DD_APM_FILTER_TAGS_REJECT="key1:value1 key2:value2"`. En Datadog Agent 7.49+, se pueden proporcionar expresiones regulares con `DD_APM_FILTER_TAGS_REGEX_REJECT`.
...
También puedes establecer estos valores en el archivo de configuración del Agente usando una lista separada por comas:
...
Por ejemplo, para ignorar verificaciones de salud donde el `http.url` coincide con este punto de conexión:
...
En el backend, Datadog crea las siguientes etiquetas de tramo en los tramos después de la ingesta. 

**Nota**: Estas etiquetas no se pueden usar para descartar trazas a nivel del Agente de Datadog. El agente solo filtra en función de las etiquetas disponibles antes de la ingesta.

| Nombre                                    | Descripción                                      |
|-----------------------------------------|--------------------------------------------------|
| `http.path_group`                       | La ruta completa de la URL desde la etiqueta `http.url`.        |
...
| `http.useragent_details.os.family`      | La familia de sistemas operativos reportada por el User-Agent.         |
| `http.useragent_details.browser.family` | La familia de navegadores reportada por el User-Agent.    |
| `http.useragent_details.device.family`  | La familia de dispositivos reportada por el User-Agent.     |

<div class="alert alert-danger">A partir del 1 de octubre de 2022, el backend de Datadog aplica un remapeo para aplicar la semántica de las etiquetas de tramo<a href="/tracing/trace_collection/tracing_naming_convention">Span.
</a> a través de los rastreadores en todos los tramos ingeridos. Si desea eliminar trazas basadas en etiquetas de tramo raíz a nivel del Agente de Datadog, use etiquetas en la columna <strong>Remap from</strong>.</div>
...
La opción **ignorar recursos** permite excluir recursos si el tramo raíz global de la traza coincide con ciertos criterios. Consulte [Excluir recursos de la recolección][5]. Esta opción se aplica a todos los servicios que envían trazas a este Agente de Datadog en particular. Las trazas que se descartan debido a la opción de ignorar recursos no se incluyen en las métricas de traza.
...
- Si utiliza contenedores dedicados con Kubernetes, asegúrese de que la variable de entorno para la opción de ignorar recursos se esté aplicando al contenedor **trace-agent**.
...
Tome nota del nombre del recurso del tramo raíz global.

- Nombre de la operación: `rack.request`
- Nombre del recurso: `Api::HealthchecksController#index`
- Http.url: `/api/healthcheck`

Para usar correctamente la opción de ignorar recursos, la regla regex escrita debe coincidir con el nombre del recurso, `Api::HealthchecksController#index`. Existen algunas opciones de expresiones regulares, pero para filtrar trazas de este recurso tal como está, una expresión regular potencial a utilizar es `Api::HealthchecksController#index$`.
...
En el contenedor dedicado trace-agent, agrega la variable de entorno `DD_APM_IGNORE_RESOURCES`:
...
## Configuración del rastreador {#tracer-configuration}

Algunos rastreadores de lenguaje pueden descartar trazas antes de que sean enviados al Agente de Datadog. Usa esta opción si tienes requisitos específicos de la aplicación.

<div class="alert alert-warning">
1. Si la solicitud está asociada con una traza distribuida, la traza resultante puede tener inexactitudes de muestreo si se eliminan partes de ella debido a estas reglas de filtrado.<br> 
2. Filtrar las trazas de esta manera elimina estas solicitudes de <a href="/tracing/guide/metrics_namespace/">métricas de traza</a>. Para obtener información sobre cómo reducir la ingestión sin afectar las métricas de traza, consulte <a href="/tracing/trace_ingestion/ingestion_controls">controles de ingestión</a>.</div>
...
Por ejemplo, si el nombre del recurso es `Api::HealthchecksController#index`, use la clase `Datadog::Tracing::Pipeline::SpanFilter` para eliminar las trazas que contienen el nombre del recurso. Este filtro también se puede usar para coincidir con otros metadatos disponibles para el [objeto de tramo][2].
...
El rastreador de Java tiene una opción para un `TraceInterceptor` personalizado para filtrar ciertos tramos. Consulte [Extendiendo Rastreadores][1].
