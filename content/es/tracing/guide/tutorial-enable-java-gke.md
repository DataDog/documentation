...
title: Tutorial - Habilitación del rastreo de una aplicación Java en Google Kubernetes
  Engine
---

## Información general

Este tutorial te guiará a través de los pasos para habilitar el rastreo en una aplicación de ejemplo Java instalada en un clúster en Google Kubernetes Engine (GKE). En este caso, el Datadog Agent también está instalado en el clúster.
...
- Git
- Kubectl
- GCloud - Configura la variable de entorno `USE_GKE_GCLOUD_AUTH_PLUGIN` y configura propiedades adicionales para tu proyecto GCloud ejecutando estos comandos:
  {{< code-block lang="sh" >}}
...
1. Si aún no dispones de un clúster GKE que quieras reutilizar, crea uno ejecutando el siguiente comando y sustituyendo `<VARIABLES>` por los valores que quieres utilizar:

   {{< code-block lang="sh" >}}
gcloud container clusters create <CLUSTER_NAME> --num-nodes=1 --network=<NETWORK> --subnetwork=<SUBNETWORK>{{< /code-block >}}

   **Nota**: Para obtener una lista de redes y subredes disponibles, utiliza el siguiente comando:
   {{< code-block lang="sh" >}}
gcloud compute networks subnets list{{< /code-block >}}

2. Conéctate al clúster ejecutando:

   {{< code-block lang="sh" >}}
gcloud container clusters get-credentials <CLUSTER_NAME>
gcloud config set container/cluster <CLUSTER_NAME>{{< /code-block >}}

3. Para facilitar la comunicación con las aplicaciones que se desplegarán, [edita las reglas del cortafuegos de red][17] para asegurarte de que el clúster GKE permite el tráfico TCP en los puertos `30080` y `30090`.

### Creación y carga de la imagen de la aplicación

Si Google Contenedor Registry (GCR) no te resulta familiar, puede ser útil consultar [Guía de inicio rápido para Container Registry][16].

En el directorio `/docker` del proyecto de ejemplo, ejecuta los siguientes comandos:

1. Autentícate con GCR ejecutando:
   {{< code-block lang="sh" >}}
...
3. Etiqueta el contenedor con el destino GCR:
   {{< code-block lang="sh" >}}
docker tag docker-notes:latest gcr.io/<PROJECT_ID>/notes-tutorial:notes{{< /code-block >}}

4. Carga el contenedor en el registro GCR:
   {{< code-block lang="sh" >}}
docker push gcr.io/<PROJECT_ID>/notes-tutorial:notes{{< /code-block >}}

Tu aplicación está contenedorizada y se encuentra disponible para la extracción para clústeres GKE.

### Configuración local y despliegue de la aplicación

1. Abre `kubernetes/notes-app.yaml` y actualiza la entrada `image` con la URL de la imagen GCR, donde introdujiste el contenedor en el paso 4 anterior:
...
kubectl get pods -o wide{{< /code-block >}}
   ...
   {{< img src="tracing/guide/tutorials/tutorial-java-gke-pods.png" alt="Resultado del comando kubectl que muestra el pod notes-app-deploy y su nombre de nodo asociado" style="width:100%;" >}}
...
kubectl get nodes -o wide{{< /code-block >}}
   ...
   {{< img src="tracing/guide/tutorials/tutorial-java-gke-external-ip.png" alt="Resultado del comando kubectl que muestra el valor de la IP externa del nodo" style="width:100%;" >}}

   En los ejemplos mostrados, `notes-app` se ejecuta en el nodo `gke-java-tracing-gke-default-pool-ccbd5526-dd3d`, cuya IP externa es `35.196.6.199`.
...
1. Añade el paquete de rastreo Java a tu proyecto. Como el Agent se ejecuta en un clúster GKE, asegúrate de que los archivos Docker están correctamente configurados y que no es necesario instalar nada. Abre el archivo `notes/dockerfile.notes.maven` y descomenta la línea que descarga `dd-java-agent`:
...
Reconstruir la imagen con el rastreo habilitado utilizando los [mismos pasos que antes](#build-and-upload-the-application-image) en el directorio `docker`:
{{< code-block lang="sh" >}}
gcloud auth Configurar-Docker
DOCKER_DEFAULT_PLATFORM=linux/amd64 Docker-compose -f servicio-Docker-compose-k8s.yaml build notes
Docker etiquetar Docker-notes:latest gcr.io/<PROJECT_ID>/notes-tutorial:notas
Docker push gcr.io/<PROJECT_ID>/notas-tutorial:notas{{< /code-block >}}

Tu aplicación con el rastreo habilitado está contenedorizada y se encuentra disponible para la extracción para clústeres GKE.

## Instalación y ejecución del Agent con Helm

A continuación, despliega el Agent en GKE para recopilar los datos de rastreo de tu aplicación instrumentada.
...
7. Reconstruye la aplicación y cárgala a GCR siguiendo los [mismos pasos que antes](#build-and-upload-the-application-image), ejecutando estos comandos en el directorio `docker`:
...
3. Crea ambas aplicaciones y publícalas en GCR. Desde el directorio `docker`, ejecuta:
   {{< code-block lang="sh" >}}
gcloud auth configure-docker
...
4. Abre `kubernetes/calendar-app.yaml` y actualiza la entrada `image` con la dirección URL de la imagen GCR, donde introdujiste la aplicación `calendar` en el paso anterior:
   {{< code-block lang="yaml" >}}
    spec:
      containers:
        - name: calendar-app
...
5. Desde el directorio `kubernetes`, despliega las aplicaciones `notes` y `calendar`, ahora con instrumentación personalizada, en el clúster:
   {{< code-block lang="sh" >}}
...
kubectl delete -f calendar-app.yaml{{< /code-block >}}
   ...
   Para obtener información sobre la eliminación de clústeres, consulta [la documentación de GKE][19].
