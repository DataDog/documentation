...
Utiliza el [tablero de Razones de Ingestión][3] para investigar en contexto cada una de estas razones de ingestión. Obtén una visión general del volumen atribuido a cada mecanismo, para saber rápidamente en qué opciones de configuración enfocarte.

## Muestreo basado en cabeza

El mecanismo de muestreo predeterminado se llama _muestreo basado en cabeza_. La decisión de mantener o descartar un trazo se toma al principio del trazo, al inicio del [span raíz][4]. Esta decisión se propaga a otros servicios como parte de su contexto de solicitud, por ejemplo, como un encabezado de solicitud HTTP.

Debido a que la decisión se toma al principio del trazo y luego se transmite a todas las partes del trazo, se garantiza que el trazo se mantenga o se descarte en su totalidad.
...
Por ejemplo, si el servicio `A` tiene más tráfico que el servicio `B`, el Agente podría variar la tasa de muestreo para `A` de manera que `A` no mantenga más de siete trazos por segundo, y ajustar de manera similar la tasa de muestreo para `B` de manera que `B` no mantenga más de tres trazos por segundo, para un total de 10 trazos por segundo.
...
**Notas**:
- La tasa de muestreo de trazas por segundo establecida en el Agente solo se aplica a las bibliotecas de trazado de Datadog. No tiene efecto en otras bibliotecas de trazado como los SDKs de OpenTelemetry.
- El objetivo no es un valor fijo. En realidad, fluctúa dependiendo de los picos de tráfico y otros factores.

Todos los spans de una traza muestreada usando el Agente de Datadog [tasa de muestreo computada automáticamente](#in-the-agent) están etiquetados con la razón de ingestión `auto`. La etiqueta `ingestion_reason` también se establece en [métricas de uso][2]. Los servicios que utilizan el mecanismo predeterminado del Agente de Datadog están etiquetados como `Automatic` en la columna de Configuración de la [Página de Control de Ingestión][5].

### En bibliotecas de trazado: reglas definidas por el usuario
`ingestion_reason: rule`

Para un control más granular, utiliza las opciones de configuración de muestreo de la biblioteca de trazado:
- Establece una **tasa de muestreo específica que se aplicará a la raíz de la traza**, por servicio y/o nombre de recurso, sobrescribiendo el [mecanismo predeterminado](#in-the-agent) del Agente.
- Establece un **límite de tasa** en el número de trazas ingeridas por segundo. El límite de tasa predeterminado es de 100 trazas por segundo por instancia de servicio (al usar el [mecanismo predeterminado](#in-the-agent) del Agente, se ignora el limitador de tasa).
...
Por ejemplo, para capturar el 100% de los rastros para el recurso `GET /checkout` del servicio `my-service`, y el 20% de los rastros de otros puntos finales, establece:
...
Configura un límite de tasa estableciendo la variable de entorno `DD_TRACE_RATE_LIMIT` a un número de rastros por segundo por instancia de servicio. Si no se establece ningún valor `DD_TRACE_RATE_LIMIT`, se aplica un límite de 100 rastros por segundo.

**Nota**: El uso de `DD_TRACE_SAMPLE_RATE` está en desuso. Usa `DD_TRACE_SAMPLING_RULES` en su lugar. Por ejemplo, si ya estableciste `DD_TRACE_SAMPLE_RATE` a `0.1`, establece `DD_TRACE_SAMPLING_RULES` a `[{"sample_rate":0.1}]` en su lugar.

Lee más sobre los controles de muestreo en la [documentación de la biblioteca de trazado de Java][2].
...
A partir de la versión <a href="https://github.com/DataDog/dd-trace-py/releases/tag/v2.9.0">2.9.0</a>, para aplicaciones Python, establece las tasas de muestreo por servicio y por recurso desde la interfaz de usuario de la <a href="/tracing/trace_pipeline/ingestion_controls#configure-the-service-ingestion-rate">Página de Control de Ingesta</a>.

Lee más sobre cómo configurar remotamente las tasas de muestreo por servicio y recurso en la [guía de muestreo basada en recursos][3].

**Nota**: La configuración remota tiene prioridad sobre la configuración local.

**Configuración local**
Para aplicaciones de Python, establece las tasas de muestreo por servicio y por recurso (a partir de la versión [v2.8.0][1] para muestreo basado en recursos) con la variable de entorno `DD_TRACE_SAMPLING_RULES`.
...
**Configuración local**
Para aplicaciones de Ruby, establece una tasa de muestreo global para la biblioteca usando la variable de entorno `DD_TRACE_SAMPLE_RATE`. Establece las tasas de muestreo por servicio con la variable de entorno `DD_TRACE_SAMPLING_RULES`.
...
Para aplicaciones de Node.js, establece una tasa de muestreo global en la biblioteca usando la variable de entorno `DD_TRACE_SAMPLE_RATE`.
...
A partir de la versión <a href="https://github.com/DataDog/dd-trace-php/releases/tag/1.4.0">1.4.0</a>, para aplicaciones PHP, establece las tasas de muestreo por servicio y por recurso desde la <a href="https://app.datadoghq.com/apm/traces/ingestion-control">Página de Control de Ingesta</a>.
...
Para aplicaciones PHP, establece una tasa de muestreo global para la biblioteca utilizando la `DD_TRACE_SAMPLE_RATE` variable de entorno. Establece las tasas de muestreo por servicio con la variable de entorno `DD_TRACE_SAMPLING_RULES`.

Por ejemplo, para enviar el 50% de los trazos para el servicio llamado `my-service`, el 20% de los trazos de otros puntos finales y el 10% para el resto de los trazos, establece:
...
Lee más sobre los controles de muestreo en la [documentación de la biblioteca de trazado de PHP][1].
...
A partir de la versión <a href="https://github.com/DataDog/dd-trace-cpp/releases/tag/v0.2.2">0.2.2</a>, para aplicaciones C++, establece las tasas de muestreo por servicio y por recurso desde la interfaz de usuario de la <a href="/tracing/trace_pipeline/ingestion_controls#configure-the-service-ingestion-rate">Página de Control de Ingesta</a>.
...
C++ no proporciona integraciones para instrumentación automática, pero se utiliza para el trazado por proxy como Envoy, Nginx o Istio. Lee más sobre cómo configurar el muestreo para proxies en [Trazado de proxies][2].

[1]: https://github.com/DataDog/dd-trace-cpp/releases/tag/v0.1.0
[2]: /es/tracing/trace_collection/proxy_setup
{{% /tab %}}
{{% tab ".NET" %}}
Para aplicaciones .NET, establece una tasa de muestreo global para la biblioteca utilizando la `DD_TRACE_SAMPLE_RATE` variable de entorno. Establecer tasas de muestreo por servicio con la variable de entorno `DD_TRACE_SAMPLING_RULES`.
...
<div class="alert alert-info">A partir de la versión 2.35.0, si <a href="/remote_configuration">Configuración Remota del Agente</a> está habilitada donde se ejecuta el servicio, puedes establecer una <code>DD_TRACE_SAMPLE_RATE</code> por servicio en la interfaz de usuario del <a href="/tracing/software_catalog">Catálogo de Software</a>.</div>

Configura un límite de tasa estableciendo la variable de entorno `DD_TRACE_RATE_LIMIT` a un número de rastros por segundo por instancia de servicio. Si no se establece ningún valor `DD_TRACE_RATE_LIMIT`, se aplica un límite de 100 rastros por segundo.

Lee más sobre los controles de muestreo en la [.NET documentación de la biblioteca de rastreo][1].\
...
**Nota**: Todos los spans de un rastro muestreado utilizando una configuración de biblioteca de rastreo están etiquetados con la razón de ingestión `rule`. Los servicios configurados con reglas de muestreo definidas por el usuario están marcados como `Configured` en la columna de Configuración de la [Página de Control de Ingestión][5].

## Rastros de error y raros

Para los rastros que no son capturados por el muestreo basado en la cabeza, dos mecanismos adicionales de muestreo del Agente de Datadog aseguran que los rastros críticos y diversos se mantengan e ingresen. Estos dos muestreadores mantienen un conjunto diverso de rastros locales (conjunto de spans del mismo host) al capturar todas las combinaciones de un conjunto predeterminado de etiquetas:

- **Rastros de error**: Muestrear errores es importante para proporcionar visibilidad sobre posibles fallas del sistema.
- **Rastros raros**: Muestrear rastros raros te permite mantener visibilidad sobre tu sistema en su totalidad, asegurando que los servicios y recursos de bajo tráfico aún sean monitoreados.
...
El muestreador de errores captura partes de rastros que contienen spans de error que no son capturados por el muestreo basado en la cabeza. Captura rastros de error hasta una tasa de 10 rastros por segundo (por Agente). Asegura visibilidad integral sobre errores cuando la tasa de muestreo basada en la cabeza es baja.
...
2. El muestreador de errores captura trazas locales con intervalos de error a nivel del Agente. Si la traza es distribuida, no hay garantía de que la traza completa se envíe a Datadog.
...
Para anular el comportamiento predeterminado de modo que los intervalos descartados por las reglas de la biblioteca de trazado o lógica personalizada como `manual.drop` estén **incluidos** por el muestreador de errores, habilite la función con: `DD_APM_FEATURES=error_rare_sample_tracer_drop` en el Agente de Datadog (o el contenedor dedicado del Agente de Trazas dentro del pod del Agente de Datadog en Kubernetes).
...
**Nota**: El muestreador raro captura trazas locales a nivel del Agente. Si el rastro está distribuido, no hay forma de garantizar que el rastro completo se enviará a Datadog.
...
Para configurar el muestreador raro, actualiza la configuración `apm_config.enable_rare_sampler` en el archivo de configuración principal del Agente (`datadog.yaml`) o con la variable de entorno `DD_APM_ENABLE_RARE_SAMPLER`:
...
Para cambiar la configuración predeterminada del muestreador raro, actualiza la configuración `apm_config.disable_rare_sampler` en el archivo de configuración principal del Agente (`datadog.yaml`) o con la variable de entorno `DD_APM_DISABLE_RARE_SAMPLER`:
...
El mecanismo de muestreo basado en la cabeza puede ser anulado a nivel de la biblioteca de trazado. Por ejemplo, si necesitas monitorear una transacción crítica, puedes forzar que el rastro asociado se mantenga. Por otro lado, para información innecesaria o repetitiva como las verificaciones de salud, puedes forzar que el rastro se descarte.

- Establece Manual Keep en un span para indicar que este y todos los spans hijos deben ser ingeridos. El rastro resultante puede parecer incompleto en la interfaz de usuario si el intervalo en cuestión no es el intervalo raíz del rastro.

- Establece la caída manual en un intervalo para asegurarte de que **ningún** intervalo hijo sea ingerido. [Los muestreadores de error y raros](#error-and-rare-traces) serán ignorados en el Agente.

{{< programming-lang-wrapper langs="java,python,ruby,go,nodejs,.NET,php,cpp" >}}
{{< programming-lang lang="java" >}}

Mantener un rastro manualmente:
...
Eliminar un rastro manualmente:
...
El mantenimiento manual de un rastro debe ocurrir antes de la propagación del contexto. Si se mantiene después de la propagación del contexto, el sistema no puede garantizar que todo el rastro se mantenga a través de los servicios. El mantenimiento manual de rastros se establece en la ubicación del cliente de rastreo, por lo que el rastro aún puede ser eliminado por el Agente o la ubicación del servidor según las reglas de muestreo.


## Intervalos individuales
`ingestion_reason: single_span`

Si necesitas muestrear un intervalo específico, pero no necesitas que el rastro completo esté disponible, las bibliotecas de rastreo te permiten establecer una tasa de muestreo que se puede configurar para un solo intervalo.

Por ejemplo, si estás construyendo [métricas a partir de intervalos][6] para monitorear servicios específicos, puedes configurar reglas de muestreo de intervalos para asegurarte de que estas métricas se basen en el 100% del tráfico de la aplicación, sin tener que ingerir el 100% de los rastros de todas las solicitudes que fluyen a través del servicio.

Esta función está disponible para Datadog Agent v[7.40.0][19]+.

**Nota**: Las reglas de muestreo de intervalos individuales **no** se pueden usar para eliminar intervalos que son mantenidos por [muestreo basado en cabeza](#head-based-sampling), solo para mantener intervalos adicionales que son eliminados por muestreo basado en cabeza.

{{< tabs >}}
{{% tab "Java" %}}
A partir de la biblioteca de rastreo [versión 1.7.0][1], para aplicaciones Java, establece reglas de muestreo de **intervalo** por servicio y por nombre de operación con la `DD_SPAN_SAMPLING_RULES` variable de entorno.
...
{{% /tab %}}
{{% tab "Python" %}}
A partir de la versión [v1.4.0][1], para aplicaciones Python, establece reglas de muestreo de **intervalo** por servicio y por nombre de operación con la `DD_SPAN_SAMPLING_RULES` variable de entorno.
...
{{% /tab %}}
{{% tab "Ruby" %}}
A partir de la versión [v1.5.0][1], para aplicaciones Ruby, establece reglas de muestreo de **intervalo** por servicio y por nombre de operación con la `DD_SPAN_SAMPLING_RULES` variable de entorno.
...
Lee más sobre los controles de muestreo en la [documentación de la biblioteca de rastreo de Ruby][2].

[1]: https://github.com/DataDog/dd-trace-rb/releases/tag/v1.5.0
[2]: /es/tracing/trace_collection/dd_libraries/ruby#sampling
{{% /tab %}}
{{% tab "Go" %}}
A partir de la versión [v1.41.0][1], para aplicaciones Go, establece reglas de muestreo de **intervalo** por servicio y por nombre de operación con la `DD_SPAN_SAMPLING_RULES` variable de entorno.
...
@env DD_SPAN_SAMPLING_RULES=[{"service": "my-service", "name": "http.request", "sample_rate":1.0, "max_per_second": 50}]
```
A partir de la versión [v1.60.0][3], para aplicaciones de Go, establece las reglas de muestreo **span** por recurso y por etiquetas con la variable de entorno `DD_SPAN_SAMPLING_RULES`.

Por ejemplo, para recopilar `100%` de los spans del servicio para el recurso `POST /api/create_issue`, para la etiqueta `priority` con el valor `high`:
...
{{% /tab %}}
{{% tab "Node.js" %}}
Para aplicaciones de Node.js, establece las reglas de muestreo **span** por servicio y por nombre de operación con la variable de entorno `DD_SPAN_SAMPLING_RULES`.
...
{{% /tab %}}
{{% tab "PHP" %}}
A partir de la versión [v0.77.0][1], para aplicaciones de PHP, establece las reglas de muestreo **span** por servicio y por nombre de operación con la variable de entorno `DD_SPAN_SAMPLING_RULES`.
...
{{% /tab %}}
{{% tab "C++" %}}
A partir de la versión [v0.1.0][1], para aplicaciones de C++, establece las reglas de muestreo **span** por servicio y por nombre de operación con la variable de entorno `DD_SPAN_SAMPLING_RULES`.
...
{{% /tab %}}
{{% tab ".NET" %}}
A partir de la versión [v2.18.0][1], para aplicaciones de .NET, establece las reglas de muestreo **span** por servicio y por nombre de operación con la variable de entorno `DD_SPAN_SAMPLING_RULES`.
...
<div class="alert alert-danger"> El mecanismo de <a href="/tracing/legacy_app_analytics/">Analítica de Aplicaciones</a> está completamente obsoleto. Para ingerir spans individuales sin el trazo completo, utiliza la configuración de <a href="/tracing/trace_pipeline/ingestion_mechanisms#single-spans">Muestreo de Span Único</a>. Para ingerir trazos completos, utiliza configuraciones de <a href="/tracing/trace_pipeline/ingestion_mechanisms#head-based-sampling">Muestreo Basado en Cabeza</a>.</div>
...
Una solicitud de una aplicación web o móvil genera un trazo cuando los servicios de backend están instrumentados. [La integración de APM con Monitoreo de Usuarios Reales][7] vincula las solicitudes de aplicaciones web y móviles a sus trazos de backend correspondientes para que puedas ver tus datos completos de frontend y backend a través de una sola lente.

A partir de la versión `4.30.0` del SDK del navegador de RUM, puedes controlar los volúmenes ingeridos y mantener un muestreo de los trazos de backend configurando el parámetro de inicialización `traceSampleRate`. Establece `traceSampleRate` a un número entre `0` y `100`.
Si no se establece ningún valor `traceSampleRate`, se envía un valor predeterminado del 100% de los trazos provenientes de las solicitudes del navegador a Datadog.

De manera similar, controla la tasa de muestreo de trazos en otros SDK utilizando parámetros similares:

| SDK         | Parámetro             | Versión mínima    |
|-------------|-----------------------|--------------------|
| Navegador     | `traceSampleRate`     | [v4.30.0][8]       |
| iOS         | `tracingSamplingRate` | [1.11.0][9] _La tasa de muestreo se informa en la Página de Control de Ingestión desde [1.13.0][16]_ |
| Android     | `traceSampleRate`   | [1.13.0][10] _La tasa de muestreo se informa en la Página de Control de Ingestión desde [1.15.0][17]_ |
| Flutter     | `tracingSamplingRate` | [1.0.0][11] |
| React Native | `tracingSamplingRate` | [1.0.0][12] _La tasa de muestreo se informa en la Página de Control de Ingestión desde [1.2.0][18]_  |

### Trazas sintéticas
`ingestion_reason:synthetics` y `ingestion_reason:synthetics-browser`

Las pruebas de HTTP y navegador generan trazas cuando los servicios de backend están instrumentados. [La integración de APM con Pruebas Sintéticas][13] vincula tus pruebas sintéticas con las trazas de backend correspondientes. Navega desde una ejecución de prueba que falló hasta la causa raíz del problema observando la traza generada por esa ejecución de prueba.

Por defecto, el 100% de las pruebas sintéticas de HTTP y navegador generan trazas de backend.

### Otros productos

Algunas razones adicionales de ingestión se atribuyen a spans que son generados por productos específicos de Datadog:

| Producto    | Razón de Ingestión                    | Descripción del Mecanismo de Ingestión |
|------------|-------------------------------------|---------------------------------|
| Serverless | `lambda` y `xray`                   | Tus trazas recibidas de las [aplicaciones Serverless][14] trazadas con las Bibliotecas de Trazado de Datadog o la integración de AWS X-Ray. |
...
| Observabilidad de Datos: Monitoreo de Trabajos    | `data_jobs`                            | Trazas ingeridas desde la integración de Datadog Java Tracer Spark o la integración de Databricks. |

## Mecanismos de ingestión en OpenTelemetry
`ingestion_reason:otel`

Dependiendo de tu configuración con los SDK de OpenTelemetry (usando el OpenTelemetry Collector o el Agente de Datadog), tienes múltiples formas de controlar el muestreo de ingestión. Consulta [Muestreo de Ingestión con OpenTelemetry][22] para detalles sobre las opciones disponibles para el muestreo a nivel de SDK de OpenTelemetry, OpenTelemetry Collector y Agente de Datadog en varias configuraciones de OpenTelemetry.
