...
| `OR`         | **Union**: cualquiera de los términos está contenido en los eventos seleccionados                                             | authentication OR password   |
| `-`          | **Exclusión**: el siguiente término NO está en el evento (aplica a cada búsqueda de texto sin procesar individual)                                                  | authentication AND -contraseña |

## Búsqueda de texto completo {#full-text-search}

<div class="alert alert-danger">La función de búsqueda de texto completo solo está disponible en Log Management y funciona en consultas de seguimiento, tableros y notebook. La sintaxis de búsqueda de texto completo no se puede usar para definir filtros de índice, filtros de archivo, filtros de canalización, filtros de rehidratación, o en Live Tail. </div>
...
### Search term with wildcard example {#search-term-with-wildcard-example}Ejemplo de término de búsqueda con Wildcard
...
### Ejemplo de búsqueda con múltiples términos que coinciden exactamente {#multiple-terms-with-exact-match-example}

| Sintaxis de búsqueda       | Tipo de búsqueda | Descripción                                                                                        |
| ------------------- | ----------- |--------------------------------------------------------------------------------------------------- |
| `*:"hello world"`   | Full-text   | Busca en todos los atributos de registro la cadena exacta `hello world`.                                    |
| `hello world`       | Free text   | Busca solo en el mensaje de registro las palabras `hello` y `world`. Por ejemplo `hello beautiful world`.  |
...
No se puede buscar caracteres especiales en un mensaje de registro. Se puede buscar caracteres especiales cuando están dentro de un atributo.
...
Por ejemplo, si el nombre de tu atributo es **url** y quieres filtrar por el valor de **url** `www.datadoghq.com`, ingresa:
...
2. Las búsquedas de atributos son sensibles a mayúsculas y minúsculas. Utilice [búsqueda de texto completo](#full-text-search) para obtener resultados que no distingan entre mayúsculas y minúsculas. Otra opción es usar el filtro `lowercase` con tu Grok parser mientras analizas para obtener resultados que no distingan entre mayúsculas y minúsculas durante la búsqueda.
...
3. Buscar un valor de atributo que contenga caracteres especiales requiere escapar o usar comillas dobles.
    ...
    - Para coincidir con un solo carácter especial o espacio, usa el Wildcard `?`. Por ejemplo, para un atributo `my_attribute` con el valor `hello world`, busque usando: `@my_attribute:hello?world`.
...
Puedes utilizar wildcards con búsqueda de texto libre. Sin embargo, solo busca términos en el mensaje del registro, el texto en la columna `content` en Log Explorer. Ve [Full-text search](#full-text-search) si quieres buscar un valor en un atributo de registro.

### Wildcard de múltiples caracteres {#multi-character-wildcard}

Para realizar una búsqueda con comodín de múltiples caracteres en el mensaje del registro (la columna `content` en Log Explorer), utilice el símbolo `*` de la siguiente manera:
...
Las búsquedas con comodines también se pueden usar para buscar en el texto plano de un registro que no es parte de un atributo de registro. Por ejemplo, esta consulta devuelve todos los registros con contenido (mensaje) que contienen la cadena `NETWORK`:
...
Al buscar un valor de atributo o etiqueta que contenga caracteres especiales o que requiera escape o comillas dobles, use el comodín `?` para coincidir con un solo carácter especial o espacio. Por ejemplo, para buscar un atributo `my_attribute` con el valor `hello world`: `@my_attribute:hello?world`.
<p> </p>
...
Puede buscar un atributo numérico dentro de un rango específico. Por ejemplo, recupere todos tus errores 4xx con:
...
Para buscar registros donde un arreglo contiene cualquier valor dentro de un rango, utilice una consulta de rango:
...
Los campos calculados funcionan como atributos de registro y pueden ser utilizados para búsqueda, agregación, visualización y definición de otros campos calculados. Use el prefijo `#` para referenciar los nombres de los campos calculados.
