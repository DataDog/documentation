...
En [configuraciones de configuración de registros][1], puedes configurar procesadores como el [analizador Grok](#grok-parser) o [remapeador de fechas](#remapper) para ayudar a extraer, crear y remapear atributos para enriquecer tus registros y mejorar la búsqueda facetada.

**Notas**:

- Los registros estructurados deben ser enviados en un formato válido. Si la estructura contiene caracteres no válidos para el análisis, estos deben ser eliminados a nivel del Agente utilizando la función [mask_sequences][2].

- Como mejor práctica, se recomienda utilizar un máximo de 20 procesadores por pipeline.

## Analizador Grok {#grok-parser}

Crea reglas grok personalizadas para analizar el mensaje completo o un atributo específico de tu evento en bruto. Como mejor práctica, limita tu analizador grok a 10 reglas de análisis. Para más información sobre la sintaxis de Grok y las reglas de análisis, consulta [Análisis][10].
...
1. Haz clic en **Parsear mis registros** para generar automáticamente un conjunto de tres reglas de análisis basadas en los registros que fluyen a través del canal.
   **Nota**: Esta función requiere que los registros correspondientes estén indexados y fluyan activamente. Puedes desactivar temporalmente o reducir los filtros de exclusión para permitir que la función detecte registros.
1. **Muestras de registros**: Agrega hasta cinco registros de muestra (de hasta 5000 caracteres cada uno) para probar tus reglas de análisis.
1. **Define las reglas de análisis**: Escribe tus reglas de análisis en el editor de reglas. A medida que defines reglas, el analizador Grok proporciona asistencia de sintaxis:
   - **Sugerencias de coincidencias**: Escribe un nombre de regla seguido de `%{`. Aparece un menú desplegable con los coincidencias disponibles (como `word`, `integer`, `ip`, `date`). Selecciona una coincidencia de la lista para insertarla en tu regla.<br>
     ```
     MyParsingRule %{
     ```
   - **Sugerencias de filtros**: Al agregar un filtro con `:`, un menú desplegable muestra filtros compatibles para la coincidencia seleccionada.
1. **Prueba tus reglas**: Selecciona una muestra haciendo clic en ella para activar su evaluación contra la regla de análisis y mostrar el resultado en la parte inferior de la pantalla. Todas las muestras muestran un estado (`match` o `no match`), que resalta si una de las reglas de análisis del analizador grok coincide con la muestra.

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utiliza el [punto final de la API del Pipeline de Registros de Datadog][1] con la siguiente carga JSON del analizador Grok:
...
## Remapeador de fecha de registro {#log-date-remapper}
...
Si sus registros tienen fechas en un atributo que no está en esta lista, utilice el procesador remapeador de fecha de registro para definir su atributo de fecha como la marca de tiempo oficial del registro:
...
Si sus registros no tienen una marca de tiempo que se ajuste a los formatos listados anteriormente, utilice el procesador grok para extraer el tiempo epoch de la marca de tiempo a un nuevo atributo. El remapeador de fecha utiliza el atributo recién definido.

Para ver cómo se puede analizar un formato de fecha y hora personalizado en Datadog, consulte [Análisis de fechas][3].
...
* Si sus registros no contienen ninguno de los atributos predeterminados y no ha definido su propio atributo de fecha, Datadog marca los registros con la fecha en que los recibió.
* Si se aplican múltiples procesadores remapeadores de fecha de registro a un registro dado dentro de la canalización, se toma en cuenta el último (de acuerdo con el orden de la canalización).

{{< tabs >}}
{{% tab "UI" %}}

Defina el procesador remapeador de fecha de registro en la página de [**Canalizaciones**][1]:

{{< img src="logs/log_configuration/processor/date_remapper.png" alt="Defina un atributo de fecha" style="width:80%;" >}}
...
Utiliza el [punto final de la API de la tubería de registros de Datadog][1] con la siguiente carga útil JSON para el remapeador de fecha de registro:
...
| `name`       | Cadena           | no       | Nombre del procesador.                                |
| `is_enabled` | Booleano         | no       | Si el procesador está habilitado o no. Predeterminado: `false`. |
| `sources`    | Array de cadenas | Sí       | Array de atributos de origen.                           |

[1]: /es/api/v1/logs-pipelines/
{{% /tab %}}
{{< /tabs >}}

## Remapeador de estado de registro {#log-status-remapper}

Utiliza el procesador de remapeador de estado para asignar atributos como un estado oficial a tus registros. Por ejemplo, agrega un nivel de severidad de registro a tus registros con el remapeador de estado.
...
* Cadenas que comienzan con **emerg** o **f** (sin distinción entre mayúsculas y minúsculas) se mapean a **emerg (0)**
* Cadenas que comienzan con **a** (sin distinción entre mayúsculas y minúsculas) se mapean a **alert (1)**
* Cadenas que comienzan con **c** (sin distinción entre mayúsculas y minúsculas) se mapean a **critical (2)**
* Cadenas que comienzan con **err** (sin distinción entre mayúsculas y minúsculas) se mapean a **error (3)**
* Cadenas que comienzan con **w** (sin distinción entre mayúsculas y minúsculas) se mapean a **warning (4)**
* Cadenas que comienzan con **n** (sin distinción entre mayúsculas y minúsculas) se mapean a **notice (5)**
* Cadenas que comienzan con **i** (sin distinción entre mayúsculas y minúsculas) se mapean a **info (6)**
* Cadenas que comienzan con **d**, **t**, **v**, **trace**, o **verbose** (sin distinción entre mayúsculas y minúsculas) se mapean a **debug (7)**
* Cadenas que comienzan con **o** o **s**, o que coinciden con **OK** o **Success** (sin distinción entre mayúsculas y minúsculas) se mapean a **OK**
* Todos los demás se asignan a **info (6)**

**Nota**: Si se aplican múltiples procesadores de remapeo de estado de registro a un registro dentro de una canalización, solo se considera el primero en el orden de la canalización. Además, para todas las canalizaciones que coincidan con el registro, solo se aplica el primer remapeador de estado encontrado (de todas las canalizaciones aplicables).

{{< tabs >}}
{{% tab "UI" %}}

Define el procesador de remapeo de estado de registro en la página [**Canalizaciones**][1]:

{{< img src="logs/log_configuration/processor/severity_remapper.png" alt="Remapeo de severidad de registro" style="width:60%;" >}}

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utiliza el [punto final de la API de Canalización de Registros de Datadog][1] con la siguiente carga JSON del remapeador de estado de registro:
...
## Remapeador de servicio {#service-remapper}

El procesador de remapeo de servicio asigna uno o más atributos a tus registros como el servicio oficial.

**Nota**: Si se aplican múltiples procesadores de remapeo de servicio a un registro dado dentro de la canalización, solo se toma en cuenta el primero (según el orden de la canalización).

{{< tabs >}}
{{% tab "UI" %}}

Define el procesador de remapeo de servicio de registro en la página [**Canalizaciones**][1]:

{{< img src="logs/log_configuration/processor/service_remapper.png" alt="Procesador de remapeo de servicio" style="width:80%;" >}}

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utiliza el [punto final de la API de Canalización de Registros de Datadog][1] con la siguiente carga JSON del remapeador de servicio de registro:
...
## Remapeador de mensaje de registro {#log-message-remapper}
...
El procesador remapeador remapea uno o más atributo(s) o etiquetas de origen a un atributo o etiqueta de destino diferente. Por ejemplo, puedes remapear el atributo `user` a `firstname` para normalizar los datos de registro en el Explorador de Registros.

Si el objetivo del remapeador es un atributo, el procesador también puede intentar convertir el valor a un nuevo tipo (`String`, `Integer` o `Double`). Si la conversión falla, se preservan el valor y el tipo originales.

**Nota**: El separador decimal para los valores de `Double` debe ser `.`.

### Restricciones de nomenclatura {#naming-constraints}
...
- El atributo `host` no se puede volver a asignar.
- Los siguientes atributos requieren procesadores de remapeo dedicados y no se pueden volver a asignar con el Remapeador genérico. Para volver a asignar cualquiera de los atributos, utilice el remapeador o procesador especializado correspondiente.
   ...
   - `span_id`: Remapeador de intervalos

{{< tabs >}}
{{% tab "UI" %}}

Defina el procesador de remapeo en la página [**Pipelines**][1]. Por ejemplo, remapee `user` a `user.firstname`.

{{< img src="logs/log_configuration/processor/remapper.png" alt="Procesador de remapeo de atributos" style="width:80%;" >}}

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utilice el [punto final de la API de Datadog Log Pipeline][1] con la siguiente carga útil JSON del Remapeador:
...
| `target`               | Cadena           | Sí      | Nombre del atributo o etiqueta final a la que se remapearán las fuentes.                           |
| `target_type`          | Cadena           | No      Define si el objetivo es un registro `attribute` o un `tag`. Predeterminado: `attribute`.    |
...
| `preserve_source`      | Booleano          | No      Eliminar o preservar el elemento fuente remapeado. Predeterminado: `false`.               |
| `override_on_conflict` | Booleano          | No      Sobrescribir o no el elemento objetivo si ya está establecido. Predeterminado: `false`.            |

[1]: /es/api/v1/logs-pipelines/
{{% /tab %}}
{{< /tabs >}}

## Analizador de URL {#url-parser}
...
Defina el procesador de analizador de URL en la página [**Pipelines**][1]:
...
| `name`       | Cadena           | No       | Nombre del procesador.                                                                                               |
| `is_enabled` | Booleano          | No       | Si el procesador está habilitado o no. Predeterminado: `false`.                                                                |
| `sources`    | Array de cadenas | No       Array de atributos fuente. Predeterminado: `http.url`.                                                                      |
...
## Analizador de User-Agent {#user-agent-parser}

El procesador del analizador de user-agent toma un `useragent` atributo y extrae el sistema operativo, navegador, dispositivo y otros datos del usuario. Cuando se configura, se producen los siguientes atributos:
...
Utiliza el [endpoint de la API de Datadog Log Pipeline][1] con la siguiente carga JSON del analizador de user-agent:
...
| `name`       | Cadena           | No       | Nombre del procesador.                                                                                                      |
| `is_enabled` | Booleano          | No       | Si el procesador está habilitado o no. Predeterminado: `false`.                                                                      |
| `sources`    | Array de cadenas | No       Array de atributos fuente. Predeterminado: `http.useragent`.                                                                      |
...
## Procesador de categoría {#category-processor}

Utiliza el procesador de categoría para agregar un nuevo atributo (sin espacios ni caracteres especiales en el nombre del nuevo atributo) a un registro que coincida con una consulta de búsqueda proporcionada. Luego, utiliza categorías para crear grupos para una vista analítica (por ejemplo, grupos de URL, grupos de máquinas, entornos y cubos de tiempo de respuesta).
...
* Una vez definidos en el procesador de categorías, puedes mapear categorías al estado del registro utilizando el [remapeador de estado del registro](#log-status-remapper).
...
| `name`       | Cadena          | No       | Nombre del procesador.                                                                                     |
| `is_enabled` | Booleano         | No       | Si el procesador está habilitado o no. Predeterminado: `false`                                                      |
| `categories` | Array de Objeto | Sí      | Array de filtros para coincidir o no con un registro y su correspondiente `name` para asignar un valor personalizado al registro. |
...
## Procesador aritmético {#arithmetic-processor}
...
Por defecto, se omite un cálculo si falta un atributo. Seleccione *Reemplazar atributo faltante por 0* para completar automáticamente los valores de atributos faltantes con 0 y asegurar que se realice el cálculo.
...
* Si necesita escalar una unidad de medida, use el filtro de escala.
...
Utilice el [punto final de la API de Datadog Log Pipeline][1] con la siguiente carga JSON del procesador aritmético:
...
| `name`               | Cadena  | No       | Nombre del procesador.                                                                                                                       |
| `is_enabled`         | Booleano | No       | Si el procesador está habilitado o no. Predeterminado: `false`.                                                                                       |
| `expression`         | Cadena  | Sí      | Operación aritmética entre uno o más atributos de registro.                                                                                     |
...
## Procesador de generador de cadenas {#string-builder-processor}

Utilice el procesador de generador de cadenas para agregar un nuevo atributo (sin espacios ni caracteres especiales) a un registro con el resultado de la plantilla proporcionada. Esto permite la agregación de diferentes atributos o cadenas sin procesar en un solo atributo.

La plantilla se define tanto por texto sin procesar como por bloques con la sintaxis `%{attribute_path}`.
...
Defina el procesador de generador de cadenas en la página [**Pipelines**][1]:

{{< img src="logs/log_configuration/processor/stringbuilder_processor.png" alt="Procesador de generador de cadenas" style="width:80%;">}}

Con el siguiente registro, utilice la plantilla `Request %{http.method} %{http.url} was answered with response %{http.status_code}` para devolver un resultado. Por ejemplo:
...
Utilice el [punto final de la API del Pipeline de Logs de Datadog][1] con la siguiente carga JSON del procesador de generador de cadenas:
...
## Procesador GeoIP {#geoip-parser}

El procesador geoIP toma un atributo de dirección IP y extrae información de continente, país, subdivisión o ciudad (si está disponible) en la ruta del atributo de destino.
...
La mayoría de los elementos contienen un `name` y `iso_code` (o `code` para continente) atributo. `subdivision` es el primer nivel de subdivisión que utiliza el país, como "Estados" para los Estados Unidos o "Departamentos" para Francia.

Por ejemplo, el procesador geoIP extrae la ubicación del atributo `network.client.ip` y la almacena en el atributo `network.client.geoip`:
...
Utilice el [punto final de la API del Pipeline de Logs de Datadog][1] con la siguiente carga JSON del procesador de búsqueda geoIP:
...
| `name`       | Cadena           | No       | Nombre del procesador.                                                                                                    |
| `is_enabled` | Booleano          | No       | Si el procesador está habilitado o no. Predeterminado: `false`.                                                                     |
| `sources`    | Array de cadenas | No       Array de atributos fuente. Predeterminado: `network.client.ip`.                                                                  |
| `target`     | Cadena           | Sí      Nombre del atributo padre que contiene todos los detalles extraídos del `sources`. Por defecto: `network.client.geoip`.  |

[1]: /es/api/v1/logs-pipelines/
{{% /tab %}}
{{< /tabs >}}

## Procesador de búsqueda {#lookup-processor}

Utilice el procesador de búsqueda para definir un mapeo entre un atributo de log y un valor legible por humanos guardado en una [Tabla de Referencia][7] o la tabla de mapeo de procesadores.

Por ejemplo, puede utilizar el procesador de búsqueda para mapear un ID de servicio interno a un nombre de servicio legible por humanos. Alternativamente, puedes usarlo para verificar si la dirección MAC que acaba de intentar conectarse al entorno de producción pertenece a tu lista de máquinas robadas.
...
* Verifica si el registro actual contiene el atributo de origen.
* Verifica si el valor del atributo de origen existe en la tabla de mapeo.
  * Si lo encuentra, crea el atributo de destino con el valor correspondiente en la tabla.
  * Opcionalmente, si no encuentra el valor en la tabla de mapeo, crea un atributo de destino con el valor predeterminado establecido en el campo `fallbackValue`. Puedes ingresar manualmente una lista de pares de `source_key,target_value` o subir un archivo CSV en la pestaña **Mapeo Manual**.
...
Usa el [punto final de la API de Datadog Log Pipeline][1] con la siguiente carga JSON del procesador de búsqueda:
...
## Remapeador de trazas {#trace-remapper}

Hay dos formas de definir la correlación entre las trazas de la aplicación y los registros:
...
2. Utilice el procesador de remapeo de trazas para definir un atributo de registro como su ID de traza asociado.

{{< tabs >}}
{{% tab "UI" %}}

Defina el procesador de remapeo de trazas en la página [**Pipelines**][1]. Ingrese la ruta del atributo de ID de traza en el cuadro del procesador de la siguiente manera:
...
Utilice el [punto final de la API de Datadog Log Pipeline][1] con la siguiente carga útil JSON de remapeo de trazas:
...
## Remapeador de intervalo {#span-remapper}
...
2. Utilice el procesador remapeador de intervalo para definir un atributo de registro como su ID de intervalo asociado.

{{< tabs >}}
{{% tab "UI" %}}

Defina el procesador remapeador de intervalo en la página de [**Pipelines**][1]. Ingrese la ruta del atributo del ID de intervalo en el cuadro del procesador de la siguiente manera:
...
Utilice el [punto final de la API de Datadog Log Pipeline][1] con la siguiente carga JSON del remapeador de intervalo:
...
## Procesador de array {#array-processor}

Utilice el procesador de array para extraer, agregar o transformar valores de arrays JSON dentro de sus registros.

Las operaciones soportadas incluyen:

- **Seleccione el valor de un elemento coincidente**
- **Calcule la longitud de un arreglo**
- **Agregue un valor a un arreglo**

Cada operación está configurada a través de un procesador dedicado.

Defina el procesador de arreglos en la página [**Pipelines**][1].


### Seleccione el valor del elemento coincidente {#select-value-from-matching-element}

Extraiga un valor específico de un objeto dentro de un arreglo cuando coincida con una condición.

{{< tabs >}}
{{% tab "UI" %}}

{{< img src="logs/log_configuration/processor/array_processor_select_value.png" alt="Procesador de arreglos - Seleccione el valor del elemento" style="width:80%;" >}}
...
| `operation.filter`  | Cadena    | Sí      | Expresión para coincidir con un elemento de un arreglo. El primer elemento que coincide es seleccionado. |
...
Calcular el número de elementos en un arreglo.
...
### Agregar al arreglo {#append-to-array}
...
| `operation.preserve_source`  | Booleano    | No      | Si se debe preservar la fuente original después de la reconfiguración. Por defecto: `false`.   |

[1]: /es/api/v1/logs-pipelines/
{{% /tab %}}
{{< /tabs >}}

## Procesador decodificador {#decoder-processor}

{{< site-region region="gov" >}}
<div class="alert alert-warning">El procesador decodificador no está disponible para su <a href="/getting_started/site">sitio de Datadog</a> seleccionado ({{< region-param key="dd_site_name" >}}). Contacte a <a href="/help/">Soporte de Datadog</a> si necesita esta capacidad.</div>
{{< /site-region >}}
...
- Formato Hex: La entrada en formato Hex se puede decodificar en una cadena (UTF-8) o en un entero.
...
2. Seleccionar la codificación de origen: Elija la codificación de binario a texto del origen: `base64` o `base16/hex`.
...
## Procesador de inteligencia de amenazas {#threat-intel-processor}
...
## Procesador OCSF {#ocsf-processor}

Utilice el procesador OCSF para normalizar sus registros de seguridad de acuerdo con el [Marco de Esquema de Ciberseguridad Abierto (OCSF)][11]. El procesador OCSF le permite crear asignaciones personalizadas que reasignan sus atributos de registro a las clases de esquema OCSF y sus atributos correspondientes, incluidos los atributos enumerados (ENUM).
...
- Preprocesar registros antes de la reasignación OCSF

Para instrucciones detalladas de configuración, ejemplos de configuración y orientación para la solución de problemas, consulte [Procesador OCSF][12].
