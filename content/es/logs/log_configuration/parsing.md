...
<div class="alert alert-danger">Las características de análisis Grok disponibles en <em>tiempo de consulta</em> (en el <a href="/logs/explorer/calculated_fields/">Explorador de registros</a>) admiten un subconjunto limitado de coincididores (<strong>data</strong>, <strong>entero</strong>, <strong>notSpace</strong>, <strong>número</strong> y <strong>palabra</strong>) y filtros (<strong>número</strong> y <strong>entero</strong>).<br><br>
El siguiente conjunto completo de coincididores y filtros es específico para <em>tiempo de ingestión</em> <a href="/logs/log_configuration/processors/grok_parser/">funcionalidad del Analizador Grok</a>.</div>
...
<sup>1</sup> Usa el `timezone` parámetro si realizas tus propias localizaciones y tus marcas de tiempo _no_ están en UTC.
...
Si tiene registros con dos formatos posibles que difieren en solo un atributo, establezca una única regla usando alternante con `(<REGEX_1>|<REGEX_2>)`. Esta regla es equivalente a un OR booleano.
...
* Si el XML contiene etiquetas que tienen tanto un atributo como un valor de cadena entre las dos etiquetas, se genera un `value` atributo. Por ejemplo: `<title lang="en">Harry Potter</title>` se convierte en `{"title": {"lang": "en", "value": "Harry Potter" } }`
...
| <code>Valor1&nbsp;&nbsp;&nbsp;&nbsp;Valor2&nbsp;&nbsp;&nbsp;&nbsp;Valor3</code> (TSV)      | `%{data::csv("key1,key2,key3","tab")}` | {"key1": "value1", "key2": "value2", "key3":"value3"} |
