...
* Ciertos registros pueden producir grandes espacios en blanco. Utiliza `\n` y `\s+` para contabilizar los saltos de línea y los espacios en blanco.

### Matcher y filtro {#matcher-and-filter}

Las características de parseo de <div class="alert alert-danger">Grok disponibles en <em>tiempo de consulta</em> (en el <a href="/logs/explorer/calculated_fields/">explorador de registros</a>) admiten un subconjunto limitado de matchers (<strong>data</strong>, <strong>integer</strong>, <strong>notSpace</strong>, <strong>number</strong>, y <strong>word</strong>) y filtros (<strong>number</strong> y <strong>integer</strong>).<br><br>
El siguiente conjunto completo de matchers y filtros es específico para la funcionalidad de <em>tiempo de ingestión</em> <a href="/logs/log_configuration/processors/grok_parser/">Grok Parser</a>.</div>

Aquí hay una lista de todos los matchers y filtros implementados de manera nativa por Datadog:
...
Los siguientes matchers están disponibles tanto para el parseo en tiempo de consulta (explorador de registros) como para el parseo en tiempo de ingestión (Grok Parser):
...
`data`
: Coincide con cualquier cadena que incluya espacios y saltos de línea. Equivalente a `.*` en regex. Utilice cuando ninguno de los patrones anteriores sea apropiado.

**Coincidencias solo para el tiempo de ingestión:**

Los siguientes matchers solo están disponibles para el parseo en tiempo de ingestión con el procesador Grok Parser y no se pueden usar en el explorador de registros:
...
`integerExtStr`
: Coincide con un número entero (con soporte para notación científica) y lo parsea como cadena.

`integerExt`
: Coincide con un número entero (con soporte para notación científica) y lo parsea como número entero.
...
`ipOrHost`
: Coincide con un nombre de servidor o IP.
...
Los siguientes filtros solo están disponibles para el parseo de tiempo de ingestión con el procesador Grok Parser y no se pueden usar en el Explorador de Registros:
...
: Extrae el patrón de clave-valor y devuelve un objeto JSON. Consulta los [ejemplos de filtro clave-valor](#key-value-or-logfmt).
...
: Analiza una secuencia de cadenas de tokens y la devuelve como un arreglo. Consulta el ejemplo de [lista a array](#list-to-array).
...
Utiliza la sección de **Configuraciones Avanzadas** al final de tu procesador Grok para analizar un atributo específico en lugar del atributo predeterminado `message`, o para definir reglas auxiliares que reutilicen patrones comunes en múltiples reglas de análisis.

### Parseo de un atributo de texto específico {#parsing-a-specific-text-attribute}

Utiliza el campo de **Extraer de** para aplicar tu procesador Grok a un atributo de texto dado en lugar del atributo predeterminado `message`.

Por ejemplo, considera un registro que contiene un `command.line` atributo que debe ser analizado como un par clave-valor. Extrae de `command.line` para analizar su contenido y crear atributos estructurados a partir de los datos del comando.
...
Utiliza el campo de **Reglas Auxiliares** para definir tokens para tus reglas de parseo. Las reglas auxiliares te permiten reutilizar patrones comunes de Grok en tus reglas de parseo. Esto es útil cuando tienes varias reglas en el mismo analizador Grok que utilizan los mismos tokens.
...
Usa filtros como **clavevalor** para mapear más fácilmente cadenas a atributos para formatos de clave-valor o logfmt:
...
Si agregas un atributo`my_attribute` **extract** en tu patrón de regla, verás:
...
| /clave:/valorStr               | `%{data::keyvalue(":", "/")}`                         | {"/clave": "/valorStr"}                 |
| clave:={valorStr}              | `%{data::keyvalue(":=", "", "{}")}`                   | {"clave": "valorStr"}                   |
| clave1=valor1\|clave2=valor2     | <code>%{data::keyvalue(&quot;=&quot;, &quot;&quot;, &quot;&quot;, &quot;&#124;&quot;)}</code> | {"key1": "value1", "key2": "value2"}  |
...
* Si defines un filtro de *clavevalor* en un `data` objeto, y este filtro no coincide, entonces se devuelve un JSON vacío `{}` (por ejemplo, entrada: `key:=valueStr`, regla de parseo: `rule_test %{data::keyvalue("=")}`, salida: `{}`).
...
* IDs de zona horaria extraídos de la base de datos TZ. Para más información, consulte [TZ database names][2].
...
Utilice el filtro `json` para hacer el parseo de un objeto JSON anidado después de un prefijo de texto sin procesar:
...
Los componentes de Kubernetes a veces registran en el formato `glog`; este ejemplo es del elemento Kube Scheduler en la Pipeline Library.
...
* `separator`: Define los separadores utilizados para separar los diferentes valores. Solo se acepta un carácter. Predeterminado: `,`. **Nota**: Utiliza `tab` para el `separator` para representar el carácter de tabulación para TSVs.
...
Si tiene un registro en el que, después de haber realizado el parseo necesario y sabiendo que el texto posterior es seguro para descartar, puede usar el comparador de datos para hacerlo. Para el siguiente ejemplo de registro, puede utilizar el `data` comparador para descartar el `%` al final.
