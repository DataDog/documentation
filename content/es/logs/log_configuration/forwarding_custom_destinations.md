...
{{< img src="logs/log_configuration/forwarding/tag-forwarding.png" alt="Página de configuración del destino que muestra los pasos que se deben seguir para configurar un nuevo destino" style="width:70%;">}}
...
6. Introduce un nombre para el destino.
7. En la sección **Configure Destination** (Configurar el destino), introduce el endpoint al que quieres enviar los logs. El endpoint debe empezar por `https://`. Por ejemplo, introduce`https://<your_account>.splunkcloud.com:8088`.  
    **Nota**: `/services/collector/event` se añade automáticamente al endpoint.
8. En la sección **Configure Authentication** (Configurar la autenticación), introduce el token Splunk HEC. Para obtener más información sobre el token Splunk HEC, consulta [Configurar y utilizar el recopilador de eventos HTTP][1].  
...
9. En la sección **Select Tags to Forward** (Seleccionar etiquetas [tags] para el reenvío):
