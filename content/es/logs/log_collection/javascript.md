...
Después de que se inicializa el SDK de registros del navegador de Datadog, envía una entrada de registro personalizada directamente a Datadog con la API:
...
El SDK de registros del navegador de Datadog agrega funciones abreviadas (`.debug`, `.info`, `.warn`, `.error`) a los registradores para mayor comodidad. También está disponible una función de registrador genérica, que expone el parámetro `status`:
...
| `handlingStack` | Cadena | Un rastreo de pila de dónde se manejó el evento de registro. Esto se puede usar para identificar de qué [micro-frontend][9] se envió el registro. |
...
- Establecer todo el contexto para tu registrador con la API `setContext (context: object)`.
- Establece una propiedad de contexto en tu registrador con `setContextProperty (key: string, value: any)` API:
...
Para cumplir con GDPR, CCPA y regulaciones similares, el SDK de Logs Browser te permite proporcionar el valor de consentimiento de seguimiento en la inicialización.
...
Este estado no está sincronizado entre pestañas ni persistido entre navegaciones. Es tu responsabilidad proporcionar la decisión del usuario durante la inicialización del SDK de Logs Browser o utilizando `setTrackingConsent()`.
...
Después de que se inicializa el SDK de registros del navegador de Datadog, puedes acceder al contexto interno del SDK. Esto te permite acceder a `session_id`.
