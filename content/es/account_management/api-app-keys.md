...
- Después de 3 meses de estar habilitado continuamente, el modo OTR se vuelve permanente y el interruptor se elimina
...
Para proteger y asegurar mejor sus aplicaciones, puede especificar contextos de autorización para las claves de aplicación, definiendo permisos más granulares y minimizando el acceso que las aplicaciones tienen a sus datos de Datadog. Esto le brinda un control de acceso detallado sobre sus aplicaciones y minimiza las vulnerabilidades de seguridad al limitar el acceso innecesario. Por ejemplo, una aplicación que solo lee tableros no necesita derechos de administrador para gestionar usuarios o eliminar cualquiera de los datos de su organización.

La mejor práctica recomendada para definir los contextos de las claves de aplicación es otorgar a sus claves los privilegios mínimos y los permisos mínimos necesarios para que una aplicación funcione como se espera. Las claves de aplicación asignadas con contexto reciben únicamente los contextos especificados por el usuario, sin permisos adicionales. Si bien puede modificar los contextos de autorización de sus claves de aplicación en cualquier momento, considere cómo esos cambios pueden afectar la funcionalidad o el acceso existente de su aplicación.
...
- Los propietarios de aplicaciones no pueden autorizar una aplicación si carecen de alguno de los permisos requeridos, incluso si asignan a una clave de aplicación contextos de autorización que ellos mismos no poseen.
...
Para utilizar claves de aplicación con estas API, debe habilitar el acceso a la Actions API en la clave de aplicación. Esto se puede hacer [a través de la interfaz de usuario][2] o [API][21]. Por defecto, las claves de aplicación no se pueden usar con estas API.

{{< img src="account_management/click-enable-actions-api-access.png" alt="Haga clic en Enable para el acceso a la Actions API" style="width:80%;" >}}
...
- Las aplicaciones de Real User Monitoring envían eventos y registros.
...
Para eliminar una clave de API o token de cliente de Datadog, navegue a la lista de claves o tokens y haga clic en {{< ui >}}Delete{{< /ui >}} {{< img src="icons/delete.png" inline="true" style="width:14px;">}} el ícono junto a la clave o token que desea eliminar.
...
<div class="alert alert-danger">Asegúrese de almacenar de manera segura su clave de aplicación inmediatamente después de la creación, ya que el secreto de la clave no se puede recuperar más tarde.</div>
{{< /site-region >}}

<div class="alert alert-info">Si su organización tiene habilitado el modo de Lectura Única (OTR), asegúrese de almacenar de manera segura su clave de aplicación inmediatamente después de la creación, ya que el secreto de la clave no se puede recuperar más tarde.</div>
...
## Contexto de claves de aplicación {#scope-application-keys}
...
Considere configurar múltiples claves de API para su organización. Por ejemplo, use diferentes claves de API para cada uno de sus distintos métodos de implementación: una para desplegar un Agent en Kubernetes en AWS, otra para desplegarlo en entornos on-premise con Chef, otra para scripts de Terraform que automatizan sus tableros o seguimientos, y otra para desarrolladores que despliegan localmente.
...
Si su organización necesita más de las 50 claves API integradas, comuníquese con [Soporte][19] para consultar sobre el aumento de su límite.

## Deshabilitando una cuenta de usuario {#disabling-a-user-account}

Si la cuenta de un usuario está deshabilitada, se revocan todas las claves de aplicación que el usuario creó. Todas las claves de API creadas por la cuenta deshabilitada no se eliminan y siguen siendo válidas.
...
Si una clave privada ha sido comprometida o expuesta públicamente, se deben tomar medidas lo más rápido posible para garantizar la seguridad de su cuenta. Eliminar el archivo que contiene la clave de un sitio público como GitHub **no** garantiza que no haya sido accedido previamente por terceros.
...
Si se identifica alguna actividad inusual, o si necesita ayuda adicional para asegurar su cuenta, comuníquese con [el soporte de Datadog][19].

## Resolución de problemas {#troubleshooting}

¿Necesita ayuda? Comuníquese con [el soporte de Datadog][19].
