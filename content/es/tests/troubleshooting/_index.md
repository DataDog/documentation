...
Si puedes ver los datos de los resultados de los tests en la pestaña **Test Runs** (Ejecuciones de tests), pero no en la pestaña **Tests** (Pruebas), es probable que falten metadatos de Git (repositorio, confirmación o rama). Para confirmar que este es el caso, abre una ejecución de test en la sección [**Ejecuciones de tests**][4] y comprueba que no hay `git.repository_url`, `git.commit.sha` o `git.branch`. Si estas etiquetas no están rellenadas, no se muestra nada en la sección [**Tests**][5].
