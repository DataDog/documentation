...
El Agente de Datadog es un software que se ejecuta en tus servidores. Recopila eventos y métricas de los hosts y los envía a Datadog, donde puede analizar sus datos de monitoreo y rendimiento. El Agente de Datadog es de código abierto y su código fuente está disponible en GitHub en [DataDog/datadog-agent][1].
...
Datadog recomienda que actualice el Agente de Datadog con cada versión menor y de parche, o, como mínimo, mensualmente. </p>
<p>
...
{{< img src="/agent/basic_agent_usage/basic_agent_2_july_25.png" alt="La vista de Fleet Automation que permite gestionar centralmente tus Agentes de Datadog en un solo lugar." style="width:100%;">}}
...
- **[Actualizar Agentes desactualizados][13]**: Activar actualizaciones remotas para tus Agentes y mantener tu flota actualizada en unos pocos clics.
- **[Send a flare for support][14]**: Desde la pestaña {{< ui >}}Support{{< /ui >}} de un host, genera un flare y adjúntalo a un caso de Soporte existente o nuevo sin tener que usar la línea de comandos.
- **Auditar el uso de la clave de API**: Identificar qué Agentes están utilizando una clave de API específica y rotar claves de manera segura.
...
- Ver la información de estado de tu Agente
...
La Interfaz Gráfica del Administrador del Agente de Datadog está habilitada por defecto en Windows y macOS, y se ejecuta en el puerto `5002`. Usa el comando `datadog-agent launch-gui` para abrir la GUI en tu navegador web predeterminado.

Puedes cambiar el puerto predeterminado de la GUI en tu archivo de configuración `datadog.yaml`. Para deshabilitar la interfaz gráfica, establece el valor del puerto en `-1`. En Linux, la interfaz gráfica está deshabilitada por defecto.
...
- Por razones de seguridad, la interfaz gráfica solo puede **accederse** desde la interfaz de red local (`localhost`/`127.0.0.1`), por lo tanto, debes estar en el servidor donde se está ejecutando el Agente. No puedes ejecutar el Agente en una máquina virtual o en un contenedor y acceder a él desde el servidor.
...
Para actualizar manualmente el núcleo del Agente de Datadog entre dos versiones menores en un servidor dado, ejecuta el [comando de instalación correspondiente para tu plataforma][7].

**Nota**: Si deseas actualizar manualmente una integración específica del Agente, consulta la [guía de gestión de integraciones][8].

### Archivos de configuración {#configuration-files}

Consulta la [documentación de archivos de configuración del Agente][9].

### Sitio de Datadog {#datadog-site}

Edita el [archivo de configuración principal del Agente][10], `datadog.yaml`, para establecer el parámetro `site` (por defecto es `datadoghq.com`).

```yaml
site: {{< region-param key="dd_site" >}}
```

**Nota**: Consulta la [documentación para comenzar con los sitios de Datadog][11] para más detalles sobre el parámetro `site`.

### Ubicación del registro {#log-location}

Consulta la [documentación de archivos de registro del Agente][12].

## Sobrecarga del Agente {#agent-overhead}

Un ejemplo del consumo de recursos del Agente de Datadog se muestra a continuación. Se realizaron pruebas en una instancia de Amazon EC2 `c5.xlarge` (4 VCPU/ 8GB RAM) y se observó un rendimiento comparable en instancias basadas en ARM64 con recursos similares. El `datadog-agent` vanilla estaba ejecutándose con una verificación de proceso para monitorear el Agente en sí. Habilitar más integraciones puede aumentar el consumo de recursos del Agente.
Habilitar las verificaciones JMX obliga al Agente a usar más memoria dependiendo del número de beans expuestos por las JVMs monitoreadas. Habilitar los Agentes de traza y de procesos también aumenta el consumo de recursos.
...
{{< whatsnext desc="Esta sección incluye los siguientes temas:">}}
  ...
  {{< nextlink href="/agent/cluster_agent">}}<u>Agente de clúster</u>: Instalar y configurar el agente de clúster para Kubernetes, una versión del agente de Datadog diseñada para recopilar datos de monitoreo de manera eficiente en un clúster orquestado.{{< /nextlink >}}
  ...
  {{< nextlink href="/agent/configuration/proxy">}}<u>Proxy</u>: Si la configuración de su red restringe el tráfico saliente, utilice un proxy para el tráfico del Datadog Agent.{{< /nextlink >}}
  {{< nextlink href="/agent/versions/">}}<u>Versiones</u>: Agent 7 es la última versión principal del Datadog Agent. Aprenda sobre los cambios entre las versiones principales del Datadog Agent y cómo actualizar.{{< /nextlink >}}
  ...
  {{< nextlink href="/getting_started/observability_pipelines">}}<u>Configure Observability Pipelines and Datadog</u>: Despliegue el Observability Pipelines Worker como un agregador para recopilar, transformar y enrutar todos sus registros y métricas a cualquier destino.{{< /nextlink >}}
{{< /whatsnext >}}
