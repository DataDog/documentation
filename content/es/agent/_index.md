...
La versión 7 del Agente está disponible. <a href="/agent/versions/upgrade_to_agent_v7">Actualiza a la versión más reciente</a> para beneficiarte de toda la nueva funcionalidad.
</div>
...
### Gestionando el Agente con Fleet Automation (recomendado) {#managing-the-agent-with-fleet-automation-recommended}
...
- **[Enviar una señal para soporte][14]**: Desde la pestaña {{< ui >}}Support{{< /ui >}} de un servidor, genera una señal y adjúntala a un caso de soporte existente o nuevo sin tener que usar la línea de comandos.
...
- Por razones de seguridad, la interfaz gráfica solo puede ser accedida desde la interfaz de red local (`localhost`/`127.0.0.1`), por lo tanto, debe estar en el host donde se está ejecutando el Agente. No puede ejecutar el Agente en una máquina virtual o un contenedor y acceder a él desde el servidor.
...
{{< whatsnext desc="Esta sección incluye los siguientes temas:">}}
  ...
  {{< nextlink href="/getting_started/observability_pipelines">}}<u>Configurar Observability Pipelines y Datadog</u>: Desplegar el Observability Pipelines Worker como un agregador para recopilar, transformar y enviar todos sus registros y métricas a cualquier destino.{{< /nextlink >}}
{{< /whatsnext >}}
