...
El Agente de Datadog es un software que se ejecuta en sus hosts. Recopila eventos y métricas de los hosts y los envía a Datadog, donde puede analizar sus datos de monitoreo y rendimiento. El Agente de Datadog es de código abierto y su código fuente está disponible en GitHub en [DataDog/datadog-agent][1].
...
Datadog recomienda que actualice el Agente de Datadog con cada lanzamiento menor y de parche, o, como mínimo, mensualmente. </p>
<p>
...
- **[Actualizar Agentes desactualizados][13]**: Inicie actualizaciones remotas para sus Agentes y mantenga su flota actualizada con unos pocos clics.
- **[Enviar una señal para soporte][14]**: Desde la pestaña {{< ui >}}Support{{< /ui >}} de un host, genere una señal y adjúntela a un caso de Soporte existente o nuevo sin tener que usar la línea de comandos.
- **Auditar el uso de la clave API**: Identifique qué Agentes están utilizando una clave API específica y rote las claves de manera segura.


### Interfaz Gráfica del Administrador de Agentes de Datadog {#datadog-agent-manager-gui}

<div class="alert alert-info">La GUI del Agente no es compatible con plataformas Windows de 32 bits.</div>

Utilice la Interfaz Gráfica del Administrador de Agentes de Datadog para:
...
La Interfaz Gráfica del Administrador de Agentes de Datadog está habilitada por defecto en Windows y macOS, y se ejecuta en el puerto `5002`. Utilice el comando `datadog-agent launch-gui` para abrir la GUI en su navegador web predeterminado.

Puede cambiar el puerto predeterminado de la GUI en su archivo de configuración `datadog.yaml`. Para deshabilitar la GUI, establezca el valor del puerto en `-1`. En Linux, la interfaz gráfica de usuario está deshabilitada por defecto.

Requisitos de la interfaz gráfica de usuario:
- Las cookies deben estar habilitadas en su navegador. La interfaz gráfica de usuario genera y guarda un token en su navegador, que se utiliza para autenticar todas las comunicaciones con el servidor de la interfaz gráfica de usuario.
- Para iniciar la interfaz gráfica de usuario, el usuario debe tener los permisos requeridos. Si puede abrir `datadog.yaml`, puede usar la interfaz gráfica de usuario.
- Por razones de seguridad, la interfaz gráfica de usuario solo puede ser accedida desde la interfaz de red local (`localhost`/`127.0.0.1`), por lo tanto, debe estar en el host donde se está ejecutando el Agente. No puede ejecutar el Agente en una máquina virtual o un contenedor y acceder a él desde la máquina host.

### Interfaz de línea de comandos {#command-line-interface}

Desde el Agente 6 en adelante, la interfaz de línea de comandos del Agente se basa en subcomandos. Para una lista completa de subcomandos del Agente, consulte [Comandos del Agente][2].

## Avanzando más con el Agente de Datadog {#getting-further-with-the-datadog-agent}

### Actualice el Agente {#update-the-agent}

Para actualizar manualmente el núcleo del Agente de Datadog entre dos versiones menores en un host dado, ejecute el [comando de instalación correspondiente para su plataforma][7].
...
### Ubicación del registro {#log-location}

Consulte la [documentación de archivos de registro del Agente][12].

## Sobrecarga del Agente {#agent-overhead}

Un ejemplo del consumo de recursos del Agente de Datadog se muestra a continuación. Se realizaron pruebas en una instancia de máquina Amazon EC2 `c5.xlarge` (4 VCPU/ 8GB RAM) y se observó un rendimiento comparable en instancias basadas en ARM64 con recursos similares. El Agente vanilla `datadog-agent` se estaba ejecutando con una verificación de proceso para monitorear el propio Agente. Habilitar más integraciones puede aumentar el consumo de recursos del Agente.
Habilitar las verificaciones JMX obliga al Agente a usar más memoria dependiendo del número de beans expuestos por las JVMs monitoreadas. Habilitar los Agentes de traza y proceso también aumenta el consumo de recursos.
...
* Versión de prueba del Agente: 6.15.0
* CPU: ~ 0.7% del CPU utilizado en promedio
...
{{< whatsnext desc="Esta sección incluye los siguientes temas:">}}
  ...
  {{< nextlink href="/agent/iot">}}<u>IoT</u>: Instalar y configurar el Agente IoT de Datadog, una versión del Agente de Datadog optimizada para monitorear dispositivos IoT y aplicaciones embebidas.{{< /nextlink >}}
  {{< nextlink href="/agent/logs">}}<u>Recolección de Logs</u>: Habilitar y configurar la recolección de logs en el Agente de Datadog.{{< /nextlink >}}
  ...
  {{< nextlink href="/agent/versions/">}}<u>Versiones</u>: El Agente 7 es la última versión importante del Agente de Datadog. Aprenda sobre los cambios entre las versiones principales del Agente y cómo actualizar.{{< /nextlink >}}
  {{< nextlink href="/agent/troubleshooting">}}<u>Solución de Problemas</u>: Encuentre información de solución de problemas para el Agente de Datadog.{{< /nextlink >}}
  {{< nextlink href="/agent/guide">}}<u>Guías</u>: Estos son tutoriales detallados y paso a paso para usar el Agente.{{< /nextlink >}}
  ...
  {{< nextlink href="/getting_started/observability_pipelines">}}<u>Configurar Pipelines de Observabilidad y Datadog</u>: Despliegue el Trabajador de Pipelines de Observabilidad como un agregador para recopilar, transformar y enrutar todos sus registros y métricas a cualquier destino.{{< /nextlink >}}
{{< /whatsnext >}}
