...
### Opción 1: Usando el soporte nativo del Agente para recuperar secretos {#option-1-using-native-agent-support-for-fetching-secrets}

**Nota**: A partir de la versión `7.76` del Agente en adelante, la gestión nativa de secretos está disponible para Agentes habilitados para FIPS.

A partir de la versión `7.70` del Agente, el Agente de Datadog admite nativamente varias soluciones de gestión de secretos. Se han introducido dos nuevas configuraciones en `datadog.yaml`: `secret_backend_type` y `secret_backend_config`.
...
**Nota**: Si estás ejecutando Datadog en un entorno contenedorizado, el [Agente de Clúster](/containers/cluster_agent/) requiere la versión 7.77 o posterior para soportar la obtención nativa de secretos. Para versiones anteriores, utiliza [Opción 2](#option-2-using-the-built-in-script-for-kubernetes-and-docker) o [Opción 3](#option-3-creating-a-custom-executable) en su lugar.

Instrucciones de configuración más específicas dependen del tipo de backend utilizado. Consulta la sección apropiada a continuación para más información:
...
##### Configura un perfil de instancia {#set-up-an-instance-profile}

Datadog recomienda utilizar el método del [perfil de instancia][1006] para recuperar secretos, ya que AWS maneja todas las variables de entorno y perfiles de sesión por ti. Más instrucciones sobre cómo hacer esto se pueden encontrar en la [documentación oficial de AWS Secrets Manager][1000].
...
* `secretId`: ya sea el "nombre amigable" del secreto (por ejemplo, `/DatadogAgent/Production`) o el ARN (por ejemplo, `arn:aws:secretsmanager:us-east-1:123456789012:secret:/DatadogAgent/Production-FOga1K`).
  - **Nota**: Se requiere el formato completo de ARN al acceder a secretos de una cuenta diferente donde se define la credencial de AWS o `sts:AssumeRole` credencial.
...
Por ejemplo, suponiendo que el ID del secreto `My-Secrets` contiene los siguientes 3 valores:
...
Lo siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` que utiliza AWS Secrets para obtener su clave API de `My-Secrets`:
...
**Alternativamente**, con el chart de Helm v3.171.0+ y el Agente v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `datadog.secretBackend.type: "aws.secrets"` y `datadog.secretBackend.config.aws_session.aws_region: "<AWS_REGION>"`.
...
**Alternativamente**, con el Operador de Datadog v1.25.0+ y el Agente v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `spec.global.secretBackend.type: "aws.secrets"` y `spec.global.secretBackend.config` con `aws_session.aws_region: "<AWS_REGION>"`.
...
##### Configura un perfil de instancia {#set-up-an-instance-profile-1}

Datadog recomienda utilizar el método del [perfil de instancia][1006] para recuperar secretos, ya que AWS maneja todas las variables de entorno y perfiles de sesión por ti. Más instrucciones sobre cómo hacer esto se pueden encontrar en la documentación oficial de [AWS Secrets Manager][1001].

##### Ejemplo de configuración {#configuration-example-1}

El AWS System Manager Parameter Store soporta un modelo jerárquico. Por ejemplo, suponiendo las siguientes rutas del AWS System Manager Parameter Store:
...
Datadog recomienda usar Identidades Administradas para autenticarse con Azure. Esto le permite asociar recursos en la nube con cuentas de AMI y elimina la necesidad de poner información sensible en su archivo de configuración `datadog.yaml`.

##### Identidad administrada {#managed-identity}

Para acceder a su Key Vault, cree una Identidad Administrada y asígnele a su Máquina Virtual. Luego, configure la asignación de rol apropiada en el Key Vault para permitir que esa identidad acceda a sus secretos.
...
El secreto del backend se referencia en su archivo de configuración del Agente de Datadog con `ENC[ ]`. El siguiente es un ejemplo donde se necesita recuperar un secreto en texto plano:
...
| valor secret_backend_type                               | Servicio de GCP                    |
| ------------------------------------------------------- | ------------------------------ |
...
Esto se puede otorgar con el rol predefinido {{< ui >}}Secret Manager Secret Accessor{{< /ui >}} (`roles/secretmanager.secretAccessor`) o un rol personalizado con acceso equivalente [access][5002].

En los entornos de GCE o GKE, ADC se configura automáticamente a través de la cuenta de servicio adjunta a la instancia o pod. La cuenta de servicio adjunta necesita tener los roles adecuados para acceder a GCP Secret Manager. Además, el entorno de GCE o GKE requiere el `cloud-platform` [alcance de acceso OAuth][5003].
...
Por ejemplo, asumiendo secretos de GCP llamados `datadog-api-key` con dos versiones y `datadog-app-key`:
...
Para secretos en formato JSON, asumiendo que un secreto llamado `datadog-keys` contiene:
...
##### Versionado de secreto {#secret-versioning}
...
##### Soporte de secretos en formato JSON {#json-secret-support}.
...
2. Escribe una política que otorgue el permiso para extraer secretos de tu bóveda. Crea un archivo `*.hcl` e incluye el siguiente permiso si usas la Versión 1 del Motor de Secretos:
...
Datadog recomienda que autentiques utilizando el [método de perfil de instancia][3003] si estás ejecutando tu HashiCorp Vault desde una máquina conectada a AWS.

Después de que esto haya sido configurado, escribe una [política de vault específica de autenticación][3004].
...
Para cada namespace que contenga secretos, crea un `Role` y `RoleBinding` utilizando el siguiente ejemplo con el nombre correcto del namespace:
...
- `key`: La clave específica para extraer del campo de datos del Secreto
...
**Alternativamente**, con el Operador de Datadog v1.25.0+ y el Agente v7.70+, puedes usar el campo nativo `spec.global.secretBackend.type` en lugar de variables de entorno.
...
**Alternativamente**, con el gráfico de Helm v3.171.0+, puedes usar: `datadog.secretBackend.type: "k8s.secrets"` y `datadog.secretBackend.config` con las claves `token_path` y `ca_path`.
...
**Alternativamente**, con Datadog Operator v1.25.0+, puedes usar: `spec.global.secretBackend.type: "k8s.secrets"` y `spec.global.secretBackend.config` con las claves `token_path` y `ca_path`.

{{% /tab %}}
{{< /tabs >}}

##### Configuración del servidor API personalizada {#custom-api-server-configuration}
...
**Alternativamente**, con Helm chart v3.171.0+, puedes usar: `datadog.secretBackend.type: "k8s.secrets"` y `datadog.secretBackend.config` con la clave `api_server`.
...
**Alternativamente**, con Datadog Operator v1.25.0+, puedes usar: `spec.global.secretBackend.type: "k8s.secrets"` y `spec.global.secretBackend.config` con la clave `api_server`.
...
El backend de archivo solo requiere permisos **lectura** para los archivos JSON, YAML o TEXT configurados. Estos permisos deben ser otorgados al usuario local del Agente de Datadog (`dd-agent` en Linux, `ddagentuser` en Windows).
...
### Opción 2: Usando el Script incorporado para Kubernetes y Docker {#option-2-using-the-built-in-script-for-kubernetes-and-docker}
...
En Kubernetes, puedes montar un secreto como un volumen de esta manera:
  ...
  global:
    secretBackend:
      ...
      - namespace: database
        ...
        - "database-secret"
```
***Nota***: Cada espacio de nombres en la lista de roles también debe estar configurado en la variable de entorno `WATCH_NAMESPACE` o `DD_AGENT_WATCH_NAMESPACE` en la implementación del Operador de Datadog.
{{% /tab %}}
{{% tab "Helm" %}}
...
Este `Role` otorga acceso al `Secret: database-secret` en el `Namespace: database`. El `RoleBinding` vincula este permiso al `ServiceAccount: datadog-agent` en el `Namespace: default`. Esto necesita ser agregado manualmente a su clúster con respecto a los recursos desplegados.
...
O, activar una actualización manualmente:

```shell
datadog-agent secret refresh
```

### actualización de clave API/APP {#apiapp-key-refresh}
Las claves API/APP extraídas como secretos admiten la actualización en tiempo de ejecución.
...
Agentes se actualice simultáneamente. La clave se resuelve al inicio, luego se actualiza una vez dentro del primer intervalo
...
### actualización de secretos de verificación de Autodescubrimiento {#autodiscovery-check-secrets-refresh}
...
### Actualización automática de secretos en caso de fallo / invalidación de clave API {#automatic-secrets-refresh-on-api-key-failure-invalidation}

A partir de la versión v7.74 del Agente, el Agente puede actualizar automáticamente secretos cuando detecta una clave API inválida. Esto ocurre cuando el Agente recibe una respuesta 403 Prohibido de Datadog o cuando la verificación de salud periódica detecta una clave API inválida o caducada.

Para habilitar esta función, configure `secret_refresh_on_api_key_failure_interval` a un intervalo en minutos en su archivo `datadog.yaml`. Establecer en `0` para deshabilitar (predeterminado).

Este intervalo es la cantidad mínima de tiempo entre 2 actualizaciones para evitar el spam en su solución de gestión de secretos cuando se detecta una clave API inválida.
...
Si está utilizando [colector DDOT][6] y desea habilitar la actualización de API/APP, debe agregar la siguiente configuración adicional a su archivo `datadog.yaml`:
...
### Listando secretos detectados {#listing-detected-secrets}

El comando `secret` en la CLI del Agente muestra cualquier error relacionado con su configuración. Por ejemplo, si los derechos sobre el ejecutable son incorrectos. También lista todos los manejadores encontrados y dónde se encuentran.
...
### Ver configuraciones después de que se inyectaron secretos {#seeing-configurations-after-secrets-were-injected}
...
### Depurando su secret_backend_command {#debugging-your-secret-backend-command}
...
##### Errores relacionados con derechos {#rights-related-errors}

Los siguientes errores indican que falta algo en tu configuración.

1. Si algún otro grupo o usuario que no sea necesario tiene derechos sobre el ejecutable, se registra un error similar al siguiente:
   ```
...
##### Probando tu ejecutable {#testing-your-executable}

Tu ejecutable es ejecutado por el Agente al obtener tus secretos. El Agente de Datadog se ejecuta utilizando el `ddagentuser`. Este usuario no tiene derechos específicos, pero es parte del grupo `Performance Monitor Users`. La contraseña de este usuario se genera aleatoriamente en el momento de la instalación y nunca se guarda en ningún lugar.
...
2. Establece una nueva contraseña para `ddagentuser` (ya que la generada en el momento de la instalación nunca se guarda en ningún lugar). En PowerShell, ejecuta:
    ...
    ```
3. Actualiza la contraseña que será utilizada por el servicio `DatadogAgent` en el Administrador de Control de Servicios. En PowerShell, ejecuta:
    ```powershell
...
### Agente se niega a iniciar {#agent-refusing-to-start}
...
### Pruebas de Permisos de Kubernetes {#testing-kubernetes-permissions}
...
Considera el ejemplo anterior de [Secretos de Kubernetes](#example-reading-a-kubernetes-secret-across-namespaces), donde el Secreto `Secret:database-secret` existe en el `Namespace: database`, y la Cuenta de Servicio `ServiceAccount:datadog-agent` existe en el `Namespace: default`.
...
### Variables de autodetección en manejadores de secretos {#autodiscovery-variables-in-secret-handles}
