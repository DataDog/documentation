...
En lugar de codificar valores sensibles como claves de API o contraseñas en texto plano dentro de archivos de configuración, el Agente puede recuperarlos dinámicamente en tiempo de ejecución. Para hacer referencia a un secreto en tu configuración, utiliza la notación `ENC[<secret_id>]`. El secreto se obtiene y se carga en memoria, pero nunca se escribe en disco ni se envía al backend de Datadog.
...
### Opción 1: Usar soporte nativo del Agente para recuperar secretos
...
Instrucciones de configuración más específicas dependen del tipo de backend utilizado. Consulta la sección apropiada a continuación para obtener más información:
...
El siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` utilizando AWS Secrets para obtener su clave de API de `My-Secrets`:
...
Esto se puede otorgar con el rol predefinido **Acceso a secretos de Secret Manager** (`roles/secretmanager.secretAccessor`) o un rol personalizado con [acceso][5002] equivalente.
...
- **Permisos de RBAC**: La ServiceAccount del Agente de Datadog debe tener permisos para leer secretos de los espacios de nombres de destino.
...
El secreto `dd_api_key` se monta automáticamente en `/run/secrets/dd_api_key`, y el agente de Datadog lo lee usando el backend `docker.secrets`.
...
{{% collapse-content title="Backends de secretos de archivo JSON, YAML o TEXTO" level="h4" expanded=false id="id-for-json-yaml-text" %}}
...
##### Permisos de archivo
El backend de archivo solo requiere permisos de **lectura** para los archivos JSON, YAML o TEXTO configurados. Estos permisos deben ser otorgados al usuario local del agente de Datadog (`dd-agent` en Linux, `ddagentuser` en Windows).
...
Para usar este ejecutable con Datadog Operator, configúrelo de la siguiente manera:
...
Para usar este ejecutable con el chart de Helm, configúrelo de la siguiente manera:
  ...
  global:
    secretBackend:
      ...
      - namespace: database
        ...
        - "database-secret"
```
***Nota***: Cada espacio de nombres en la lista de roles también debe estar configurado en la variable de entorno `WATCH_NAMESPACE` o `DD_AGENT_WATCH_NAMESPACE` en el despliegue de Datadog Operator.
{{% /tab %}}
{{% tab "Helm" %}}
...
Este `Role` otorga acceso al `Secret: database-secret` en el `Namespace: database`. El `RoleBinding` vincula este permiso al `ServiceAccount: datadog-agent` en el `Namespace: default`. Esto debe ser agregado manualmente a tu clúster con respecto a tus recursos desplegados.

### Opción 3: Creando un ejecutable personalizado

Para recuperar secretos, el Agent utiliza un ejecutable externo que usted proporciona. El ejecutable se utiliza cuando se descubren nuevos secretos y se almacenan en caché durante el ciclo de vida del Agent. Si necesita actualizar o rotar un secreto, debe reiniciar el Agent para recargarlo.

Esto le permite utilizar cualquier solución de gestión de secretos y le da control total sobre cómo el Agent accede a los secretos.

El Agent envía a este ejecutable una carga útil JSON a través de la entrada estándar que contiene una lista de identificadores de secretos a resolver. Luego, su ejecutable obtiene cada secreto y los devuelve en un formato JSON a través de la salida estándar.
...
**Nunca muestre información sensible en `stderr`**. Si el binario sale con un código de estado diferente a `0`, el Agent registra la salida de error estándar de su ejecutable para solucionar problemas.
...
En memoria, quedaría de la siguiente manera:
...
* Tener **lectura** o **ejecutar** para `ddagentuser` (el usuario utilizado para ejecutar el Agent).
...
A partir de la versión v7.67 del Agent, puede configurar el Agent para refrescar secretos resueltos sin necesidad de reiniciar.

Establezca un intervalo de refresco:

```yaml
secret_refresh_interval: 3600  # refresh every hour
```

O, active un refresco manualmente:
...
Agents se refresquen simultáneamente. La clave se resuelve al inicio, luego se refresca una vez dentro del primer intervalo
...
A partir de la versión 7.76 del Agente, las verificaciones programadas de [Autodiscovery][1] pueden refrescar secretos en tiempo de ejecución si la plantilla utiliza la sintaxis `ENC[]`.
...
El comando `secret` en la CLI del Agent muestra cualquier error relacionado con su configuración. Por ejemplo, si los derechos sobre el ejecutable son incorrectos. También lista todos los manejadores encontrados y dónde se encuentran.
...
El `dd-agent` usuario se crea cuando instalas el Datadog Agent.
...
Tu ejecutable es ejecutado por el Agente al obtener tus secretos. El Datadog Agent se ejecuta utilizando el `ddagentuser`. Este usuario no tiene derechos específicos, pero es parte del grupo `Performance Monitor Users`. La contraseña para este usuario se genera aleatoriamente en el momento de la instalación y nunca se guarda en ningún lugar.
...
### Datadog Agent se niega a iniciar

Lo primero que hace el Datadog Agent al iniciar es cargar `datadog.yaml` y descifrar cualquier secreto en él. Esto se hace antes de configurar el registro. Esto significa que en plataformas como Windows, los errores que ocurren al cargar `datadog.yaml` no se escriben en los registros, sino en `stderr`. Esto puede ocurrir cuando el ejecutable proporcionado al Datadog Agent para secretos devuelve un error.

Si tienes secretos en `datadog.yaml` y el Datadog Agent se niega a iniciar:

* Intenta iniciar el Datadog Agent manualmente para poder ver `stderr`.
...
Este comando devuelve si los permisos son válidos para que el Datadog Agent pueda ver este Secret.
...
También es posible usar variables de [Autodiscovery][1] en manejadores de secretos. El Datadog Agent resuelve estas variables antes de resolver el secreto. Por ejemplo:
