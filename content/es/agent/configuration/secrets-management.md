...
El Agente de Datadog te ayuda a gestionar tus secretos de manera segura integrándose con las siguientes soluciones de gestión de secretos:
 ...
 [Archivo de Texto](#idforjsonyamltext)
 [Archivo JSON](#idforjsonyamltext)
 [Archivo YAML](#idforjsonyamltext)
...
### Opción 1: Usando soporte nativo del Agente para obtener secretos

**Nota**: A partir de la versión `7.76` del Agente y en adelante, la gestión de secretos nativa está disponible para Agentes habilitados para FIPS.
...
**Nota**: Si estás ejecutando Datadog en un entorno contenedorizado, el [Agente de Clúster](/containers/cluster_agent/) requiere el Agente 7.77 o posterior para soportar la obtención nativa de secretos. Para versiones anteriores, utiliza [Opción 2](#option2usingthebuiltinscriptforkubernetesanddocker) o [Opción 3](#option3creatingacustomexecutable) en su lugar.
...
Configura el Agente de Datadog para usar secretos de AWS para resolver secretos utilizando la siguiente configuración:
...
* `secretId`: ya sea el "nombre amigable" del secreto (por ejemplo, `/DatadogAgent/Production`) o el ARN (por ejemplo, `arn:aws:secretsmanager:useast1:123456789012:secret:/DatadogAgent/ProductionFOga1K`).
...
Por ejemplo, asumiendo que el ID del secreto `MySecrets` contiene los siguientes 3 valores:
...
Lo siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` utilizando los AWS Secrets para obtener su clave API de `MySecrets`:
...
| valor de tipo de backend_secreto                            | Servicio de Azure          |
| ||
| `azure.keyvault` | [Azure Keyvault][2000] |

##### Autenticación de Azure

Datadog recomienda usar Identidades Administradas para autenticarse con Azure. Esto te permite asociar recursos en la nube con cuentas AMI y elimina la necesidad de poner información sensible en tu archivo de configuración `datadog.yaml`.
...
En entornos de GCE o GKE, ADC se configura automáticamente a través de la cuenta de servicio adjunta a la instancia o pod. La cuenta de servicio adjunta necesita tener los roles adecuados para acceder a GCP Secret Manager. Además, el tiempo de ejecución de GCE o GKE requiere el `cloudplatform` [alcance de acceso OAuth][5003].
...
Después de configurar el Agente para usar el Administrador de Secretos de GCP, referencia los secretos en tus configuraciones con `ENC[secretname]` o `ENC[secretname;key;version;]`.

La notación ENC se compone de:

 `secret`: el nombre del secreto en el Administrador de Secretos de GCP (por ejemplo, `datadogapikey`).
 `key`: (opcional) la clave a extraer de un secreto en formato JSON. Si estás usando secretos en texto plano, puedes omitir esto (ejemplo: `ENC[secretname;;version]`).
 `version`: (opcional) el número de versión del secreto. Si no se especifica, se usa la versión `latest`.
  + Ejemplos de sintaxis de versión:
     `secretkey`  Versión implícita `latest`
     `secretkey;;latest`  Versión explícita `latest`
     `secretkey;;1`  Número de versión específico

Por ejemplo, suponiendo secretos de GCP llamados `datadogapikey` con dos versiones y `datadogappkey`:
...
Para secretos en formato JSON, suponiendo que un secreto llamado `datadogkeys` contiene:
...
GCP Secret Manager admite versiones secretas. La implementación del Agente también admite la versionado de secretos usando el delimitador `;`. Si no se especifica una versión, se utiliza la versión `latest`.


##### Soporte de secretos en JSON

El Agente de Datadog admite la extracción de claves específicas de secretos en formato JSON usando el delimitador `;`:
...
```
Si se utiliza la versión 2 del Secrets Engine, se necesitan los siguientes permisos:
```
...
4. Elija el método de autenticación para su vault. Si utiliza el método de perfil de instancia de AWS, ejecute `vault auth enable aws`.
...
Después de que esto se haya configurado, escriba una [política de vault específica para la autenticación][3004].
 ...
 **Credenciales de ServiceAccount**: Por defecto, utiliza tokens de ServiceAccount montados automáticamente (`automountServiceAccountToken: true`, consulta [documentación de Kubernetes](https://kubernetes.io/docs/tasks/configurepodcontainer/configureserviceaccount/#optoutofapicredentialautomounting)). Se pueden configurar rutas personalizadas si es necesario.
...
Para cada namespace que contenga secretos, crea un `Role` y un `RoleBinding` utilizando el siguiente ejemplo con el nombre de namespace correcto:
...
El formato de notación ENC es `namespace/secretname;key`:
 ...
 `secretname`: El nombre del recurso Secret.
 `key`: La clave específica a extraer del campo de datos del Secret.

**Ejemplo:** Dado un Secret en el namespace `secretsns`:
...
**Soporte multinamespace:**
Cada referencia de secreto puede especificar un namespace diferente (RBAC debe configurarse para cada uno):
...
[Crea][6002] y usa un secreto de Docker Swarm:
...
[Crea][6003] un `dockercompose.yml` con secretos basados en archivos:
...
{{% collapse-content title="Backends de secretos en archivos JSON, YAML o TEXT" level="h4" expanded=false id="id-for-json-yaml-text" %}}

| valor del tipo de backend secreto                                 | Servicio de Archivo                             |
|||
|`file.json`           |[JSON][4001]                             |
|`file.yaml`          |[YAML][4002]                        |                            |
|`file.text`          |[TEXT][4003]                        |                            |
...
##### Permisos de archivo
El backend de archivo solo requiere **lectura** permisos para los archivos JSON, YAML o TEXT configurados. Estos permisos deben ser otorgados al usuario local del Agente de Datadog (`ddagent` en Linux, `ddagentuser` en Windows).
...
**Nota:** Algunas herramientas añaden automáticamente saltos de línea al exportar secretos a archivos. Consulta [Eliminar saltos de línea finales](#removetrailinglinebreaks) para saber cómo manejar esto.
...
* Secretos de Kubernetes: usando `ENC[k8s_secret@namespace/nombre_secreto/clave]`
...
Configura RBAC para permitir que la Cuenta de Servicio del Agente lea el Secreto. El siguiente Rol otorga acceso de lectura al Secreto `databasesecret` en el espacio de nombres `database`:
{{< tabs >}}
{{% tab "Operador de Datadog" %}}
```yaml
apiVersion: datadoghq.com/v2alpha1
...
Este `Rol` otorga acceso al `Secreto: databasesecret` en el `Espacio de nombres: database`. El `RoleBinding` vincula este permiso a la `Cuenta de Servicio: datadogagent` en el `Espacio de nombres: default`. Esto necesita ser agregado manualmente a tu clúster con respecto a tus recursos desplegados.
...
* Pertenecer al mismo usuario que ejecuta el Agente (`ddagent` por defecto, o `root` dentro de un contenedor).
...
A partir de la versión 7.67 del Agente, puedes configurar el Agente para refrescar secretos resueltos sin requerir un reinicio.
...
Las claves API/APP extraídas como secretos soportan refresco en tiempo de ejecución.
...
Por defecto, el Agente aleatoriza el refresco inicial dentro de la ventana de `secret_refresh_interval` para prevenir que una flota de
...
Para prevenir tiempo de inactividad, invalida las claves antiguas solo después de que toda tu flota haya obtenido las claves actualizadas. Puedes rastrear la clave
el uso en la [Página de Gestión de Flotas](https://app.datadoghq.com/fleet).
...
### Actualización de secretos de verificación de Autodescubrimiento
A partir de la versión 7.76 del Agente, las verificaciones programadas de [Autodescubrimiento][1] pueden actualizar secretos en tiempo de ejecución si la plantilla utiliza la sintaxis `ENC[]`.
...
El Agente puede entonces activar la actualización de secretos ya sea en el intervalo establecido en `secret_refresh_interval` o manualmente con `datadogagent secret refresh`.

### Actualización automática de secretos en caso de fallo/invalidación de la clave API

A partir de la versión 7.74 del Agente, el Agente puede actualizar automáticamente los secretos cuando detecta una clave API inválida. Esto ocurre cuando el Agente recibe una respuesta 403 Prohibido de Datadog o cuando la verificación de salud periódica detecta una clave API inválida o caducada.

Para habilitar esta función, establece `secret_refresh_on_api_key_failure_interval` a un intervalo en minutos en tu archivo `datadog.yaml`. Establece en `0` para desactivar (por defecto).

Este intervalo es el tiempo mínimo entre 2 actualizaciones para evitar saturar tu solución de gestión de secretos cuando se detecta una clave API inválida.
...
El `comando secreto` en la CLI del Agente muestra cualquier error relacionado con tu configuración. Por ejemplo, si los derechos sobre el ejecutable son incorrectos. También lista todos los manejadores encontrados y dónde se encuentran.
...
### Depurando tu comando_secret_backend
...
El `ddagent` usuario se crea cuando instalas el Agente de Datadog.
...
Tu ejecutable es ejecutado por el Agente al obtener tus secretos. El Agente de Datadog se ejecuta usando el `ddagentuser`. Este usuario no tiene derechos específicos, pero es parte del grupo `Performance Monitor Users`. La contraseña para este usuario se genera aleatoriamente en el momento de la instalación y nunca se guarda en ningún lugar.
...
* Elimina los secretos de `datadog.yaml` y prueba con secretos en un archivo de configuración de verificación primero.
...
Considera el ejemplo anterior de [Secretos de Kubernetes](#examplereadingakubernetessecretacrossnamespaces), donde el Secreto `Secret:databasesecret` existe en el `Namespace: database`, y la Cuenta de Servicio `ServiceAccount:datadogagent` existe en el `Namespace: default`.
