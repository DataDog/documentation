...
El Datadog Agent te ayuda a gestionar tus secretos de manera segura al integrarse con las siguientes soluciones de gestión de secretos:
...
- [Texto de archivo](#id-for-json-yaml-text)
- [Archivo JSON](#id-for-json-yaml-text)
- [Archivo YAML](#id-for-json-yaml-text)

En lugar de codificar valores sensibles como claves de API o contraseñas en texto plano dentro de archivos de configuración, el Datadog Agent puede recuperarlos dinámicamente en tiempo de ejecución. Para hacer referencia a un secreto en tu configuración, utiliza la notación `ENC[<secret_id>]`. El secreto se recupera y se carga en memoria, pero nunca se escribe en disco ni se envía al backend de Datadog.
...
### Opción 1: Usar el soporte nativo del Datadog Agent para obtener secretos {#option-1-using-native-agent-support-for-fetching-secrets}

**Nota**: A partir de la versión `7.76` del Datadog Agent y en adelante, la gestión nativa de secretos está disponible para agentes habilitados para FIPS.

A partir de la versión `7.70` del Datadog Agent, este soporta nativamente varias soluciones de gestión de secretos. Se han introducido dos nuevas configuraciones en `datadog.yaml`: `secret_backend_type` y `secret_backend_config`.
...
**Nota**: Si ejecutas Datadog en un entorno contenedorizado, el [Cluster Agent](/containers/cluster_agent/) requiere el Datadog Agent 7.77 o una versión posterior para soportar la obtención nativa de secretos. Para versiones anteriores, use [Opción 2](#option-2-using-the-built-in-script-for-kubernetes-and-docker) o [Opción 3](#option-3-creating-a-custom-executable) en su lugar.
...
##### Configurar un perfil de instancia {#set-up-an-instance-profile}

Datadog recomienda usar el [método de perfil de instancia][1006] para recuperar secretos, ya que AWS maneja todas las variables de entorno y perfiles de sesión por ti. Más instrucciones sobre cómo hacer esto se pueden encontrar en la [documentación oficial de AWS Secrets Manager][1000].
...
Configura el Datadog Agent para usar AWS Secrets para resolver secretos utilizando la siguiente configuración:
...
Después de configurar el Datadog Agent para usar AWS Secrets, puedes referenciar cualquier secreto en tus configuraciones con `ENC[secretId;secretKey]`.
...
Lo siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` utilizando los secretos de AWS para obtener la clave de API de `My-Secrets`:
...
<div class="alert alert-info"> Debes incluir el <code>serviceAccountAnnotations</code> You must include the
para otorgar al Datadog Agent permisos para acceder al secreto de AWS. </div>
...
##### Verificación de clúster: con ejecutores de verificación de clúster habilitados {#cluster-check-with-cluster-check-runners-enabled}
...
**Alternativamente**, con el gráfico de Helm v3.171.0+ y el Datadog Agent v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `datadog.secretBackend.type: "aws.secrets"` y `datadog.secretBackend.config.aws_session.aws_region: "<AWS_REGION>"`.

{{% /tab %}}

{{% tab "Operador" %}}

Configura el Datadog Agent para usar AWS Secrets para resolver secretos con el Datadog Operator utilizando la siguiente configuración:
...
##### Verificación de clúster: con ejecutores de verificación de clúster habilitados {#cluster-check-with-cluster-check-runners-enabled-1}
...
**Alternativamente**, con Datadog Operator v1.25.0+ y el Datadog Agent v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `spec.global.secretBackend.type: "aws.secrets"` y `spec.global.secretBackend.config` con `aws_session.aws_region: "<AWS_REGION>"`.
...
##### Configurar un perfil de instancia {#set-up-an-instance-profile-1}

Datadog recomienda usar el [método de perfil de instancia][1006] para recuperar secretos, ya que AWS maneja todas las variables de entorno y perfiles de sesión por ti. Más instrucciones sobre cómo hacer esto se pueden encontrar en la documentación oficial de [AWS Secrets Manager][1001].
...
Datadog recomienda usar Identidades Administradas para autenticarse con Azure. Esto te permite asociar recursos en la nube con cuentas de AMI y elimina la necesidad de incluir información sensible en tu archivo de configuración `datadog.yaml`.
...
El secreto del backend se menciona en el archivo de configuración de tu Datadog Agent con `ENC[ ]`. El siguiente es un ejemplo donde se necesita recuperar un secreto en texto plano:
...
**Disponible en la versión 7.74+ del Datadog Agent**
...
Para interactuar con GCP Secret Manager, la cuenta de servicio utilizada por el Datadog Agent (como la cuenta de servicio de la VM, una identidad de carga de trabajo o credenciales activadas localmente) requiere el `secretmanager.versions.access` permiso.
...
En entornos de GCE o GKE, ADC se configura automáticamente a través de la cuenta de servicio adjunta a la instancia o pod. La cuenta de servicio adjunta necesita tener los roles adecuados para acceder a GCP Secret Manager. Además, el entorno de GCE o GKE requiere el `cloud-platform` [contexto de acceso OAuth][5003].

##### Ejemplo de configuración de GCP{#gcp-configuration-example}

Configura el Datadog Agent para usar GCP Secret Manager para resolver secretos con la siguiente configuración:
...
Después de configurar el Datadog Agent para usar GCP Secret Manager, referencia los secretos en tus configuraciones con `ENC[secret-name]` o `ENC[secret-name;key;version;]`.
...
Referencia claves específicas de esta manera:
...
##### Versionado de secretos {#secret-versioning}

GCP Secret Manager admite versiones de secretos. La implementación del Datadog Agent también admite el versionado de secretos utilizando el delimitador `;`. Si no se especifica una versión, se utiliza la versión `latest`.


##### Soporte para secretos en JSON {#json-secret-support}

El Datadog Agent admite la extracción de claves específicas de secretos en formato JSON utilizando el delimitador `;`:
...
| valor de tipo secret_backend                               | Servicio de HashiCorp                                  |
| ------------------------------------------ | -------------------------------------------------- |
...
2. Escribe una política que otorgue el permiso para extraer secretos de tu Vault. Crea un `*.hcl` archivo e incluye el siguiente permiso si usas la versión 1 del motor de secretos:
...
En el siguiente ejemplo, asume que el prefijo de la ruta del secreto de HashiCorp Vault es `/Datadog/Production` con una clave de parámetro de `apikey`:
...
- **Permisos de RBAC**: El ServiceAccount del Agente debe tener permisos para leer secretos de los namespaces de destino
...
**Nota:** Se requiere un marcador de posición `apiKey` para la validación del gráfico de Helm al usar el backend de secretos para resolver la clave de API. La variable de entorno `DD_API_KEY` la anula. Debes crear manualmente RBAC (Rol + RoleBinding) para cada namespace que contenga secretos. Para más información, consulte la sección de [configuración de RBAC](#rbac-setup).

**Alternativamente**, con el gráfico de Helm v3.171.0+ y el Agente v7.70+, puede usar el campo nativo `datadog.secretBackend.type` en lugar de las variables de entorno.

{{% /tab %}}

{{% tab "Operador" %}}

Configura el Agente de Datadog para usar Secretos de Kubernetes con el Operador de Datadog:
...
**Nota:** Una clave de API de marcador de posición satisface la validación del Operador al usar el backend de secretos para resolver la clave de API. La variable de entorno `DD_API_KEY` la anula. Debes crear manualmente RBAC (Rol + RoleBinding) para cada espacio de nombres que contenga secretos. Para más información, consulte la sección de [configuración de RBAC](#rbac-setup).

**Alternativamente**, con Datadog Operator v1.25.0+ y Agent v7.70+, puede usar el campo nativo `spec.global.secretBackend.type` en lugar de las variables de entorno.
...
**Alternativamente**, con el gráfico de Helm v3.171.0+, puede usar: `datadog.secretBackend.type: "k8s.secrets"` y `datadog.secretBackend.config` con las claves `token_path` y `ca_path`.
...
**Alternativamente**, con el gráfico de Helm v3.171.0+, puede usar: `datadog.secretBackend.type: "k8s.secrets"` y `datadog.secretBackend.config` con la clave `api_server`.
...
| valor de secret_backend_type                                 | Servicio de Archivo                             |
|---------------------------------------------|-----------------------------------------|
|`file.json`           |[JSON][4001]                             |
|`file.yaml`          |[YAML][4002]                        |                            |
|`file.text`          |[TEXT][4003]                        |                            |
...
Puede usar esta configuración para obtener sus secretos:
...
Puede usar la siguiente configuración para obtener secretos de él:
...
Puede usar esta configuración para obtener secretos de ellos:
...
- Las rutas relativas en `ENC[]` se resuelven en relación a `secrets_path` (por ejemplo, `ENC[dd_api_key]` con `secret_path: /path/to/secrets` se resolverá a `/path/to/secrets/dd_api_key`)
...
### Opción 2: Usando el Script integrado para Kubernetes y Docker {#option-2-using-the-built-in-script-for-kubernetes-and-docker}

Para entornos en contenedores, las imágenes de contenedor del Agente de Datadog incluyen un script integrado `/readsecret_multiple_providers.sh` a partir de la versión v7.32.0. Este script admite la lectura de secretos desde:
...
Para usar este ejecutable con el Datadog Operator, configúralo de la siguiente manera:
...
Para usar este ejecutable con el gráfico de Helm, configúralo de la siguiente manera:
...
#### Ejemplo: Leyendo desde archivos montados {#example-reading-from-mounted-files}

Kubernetes admite [exponer secretos como archivos][2] dentro de un pod que el Agente puede leer para resolver secretos.

En Kubernetes, puedes montar un secreto como un volumen de la siguiente manera:
...
Luego puedes hacer referencia al secreto de la siguiente manera:
...
#### Ejemplo: Leyendo un secreto de Kubernetes a través de espacios de nombres {#example-reading-a-kubernetes-secret-across-namespaces}
  ...
  global:
    secretBackend:
      ...
      - namespace: database
        ...
        - "database-secret"
```
***Nota***: Cada espacio de nombres en la lista de roles también debe estar configurado en la variable de entorno `WATCH_NAMESPACE` o `DD_AGENT_WATCH_NAMESPACE` en el despliegue del Datadog Operator.
{{% /tab %}}
{{% tab "Helm" %}}
...
### Opción 3: Creando un ejecutable personalizado {#option-3-creating-a-custom-executable}

Para recuperar secretos, el Agente utiliza un ejecutable externo que tú proporcionas. El ejecutable se utiliza cuando se descubren nuevos secretos y se almacenan en caché durante el ciclo de vida del Agente. Si necesita actualizar o rotar un secreto, debe reiniciar el Agente para recargarlo.
...
**Nunca muestre información sensible en `stderr`**. Si el binario sale con un código de estado diferente a `0`, el Agente registra la salida de error estándar de su ejecutable para la solución de problemas.
...
## Requisitos de seguridad del Agente {#agent-security-requirements}
...
* Tener **derecho de lectura** o **ejecutar** para `ddagentuser` (el usuario utilizado para ejecutar el Agente).
...
A partir de la versión 7.67 del Agente, puedes configurar el Agente para actualizar secretos resueltos sin requerir un reinicio.

Establece un intervalo de actualización:

```yaml
secret_refresh_interval: 3600  # refresh every hour
```

O, activa una actualización manualmente:

```shell
datadog-agent secret refresh
```

### La clave de API/APP se actualiza {#apiapp-key-refresh}
Las claves de API/APP se obtienen como secretos que admiten la actualización en tiempo de ejecución.
...
Agentes que se actualizan simultáneamente. La clave se resuelve al inicio, luego se actualiza una vez dentro del primer intervalo
y cada intervalo posterior.

Para evitar tiempo de inactividad, invalide las claves antiguas solo después de que toda su flota haya obtenido las claves actualizadas. Puede rastrear el uso de claves
en la página [Gestión de Flotas](https://app.datadoghq.com/fleet).
...
### Actualización de secretos en la verificación de Autodiscovery {#autodiscovery-check-secrets-refresh}
A partir de la versión 7.76 del Agente, las verificaciones programadas de [Autodiscovery][1] pueden actualizar secretos en tiempo de ejecución si la plantilla utiliza la sintaxis `ENC[]`.
...
### Actualización automática de secretos en caso de fallo/invalidez de la clave de API {#automatic-secrets-refresh-on-api-key-failure-invalidation}

A partir de la versión 7.74 del Agente, el Agente puede actualizar automáticamente secretos cuando detecta una clave de API inválida. Esto ocurre cuando el Agente recibe una respuesta 403 Prohibido de Datadog o cuando la verificación de salud periódica detecta una clave de API inválida o caducada.

Para habilitar esta función, configure `secret_refresh_on_api_key_failure_interval` a un intervalo en minutos en su archivo `datadog.yaml`. Establezca en `0` para deshabilitar (predeterminado).

Este intervalo es la cantidad mínima de tiempo entre 2 actualizaciones para evitar saturar su solución de gestión de secretos cuando se detecta una clave de API inválida.
...
### Habilitando la actualización del colector DDOT {#enabling-ddot-collector-refresh}
...
### Listado de secretos detectados {#listing-detected-secrets}

El comando `secret` en la CLI del Agente muestra cualquier error relacionado con su configuración. Por ejemplo, si los permisos del ejecutable son incorrectos. También lista todos los manejadores encontrados y dónde se encuentran.

En Linux, el comando muestra el modo de archivo, propietario y grupo para el ejecutable. En Windows, se listan los permisos de ACL.
...
### Visualización de configuraciones tras la inyección de secretos {#seeing-configurations-after-secrets-were-injected}

Para ver rápidamente cómo se resuelve la configuración de la verificación, puede usar el comando `configcheck`:
...
**Nota**: El Agente necesita ser [reiniciado][7] para aplicar los cambios en los archivos de configuración.

### Depuración de su comando secret_backend {#debugging-your-secret-backend-command}

Para probar o depurar fuera del Agente, puede imitar cómo este lo ejecuta:
...
El `dd-agent` usuario se crea cuando se instala el Datadog Agent.
...
1. Si algún otro grupo o usuario que no sea necesario tiene permisos sobre el ejecutable, se registra un error similar al siguiente:
   ```
...
2. Si `ddagentuser` no tiene permisos de lectura y ejecución sobre el archivo, se registra un error similar:
   ```
   error while decrypting secrets in an instance: could not query ACLs for C:\decrypt.exe
   ```

3. El ejecutable debe ser una aplicación Win32 válida. Si no, se registra el siguiente error:
   ```
...
Su ejecutable es ejecutado por el Agente al obtener sus secretos. El Datadog Agent se ejecuta utilizando el `ddagentuser`. Este usuario no tiene permisos específicos, pero forma parte del grupo `Performance Monitor Users`. La contraseña para este usuario se genera aleatoriamente en el momento de la instalación y nunca se guarda en ningún lugar.

Esto significa que su ejecutable podría funcionar con su usuario predeterminado o de desarrollo, pero no cuando es ejecutado por el Agente, ya que `ddagentuser` tiene permisos más restringidos.

Para probar su ejecutable en las mismas condiciones que el Agente, actualice la contraseña del `ddagentuser` en su máquina de desarrollo. De esta manera, puede autenticarse como `ddagentuser` y ejecutar su ejecutable en el mismo contexto en el que lo haría el Agente.
...
Ahora puede iniciar sesión como `ddagentuser` para probar su ejecutable. Datadog tiene un [script de Powershell][10] para ayudarle a probar su
ejecutable como otro usuario. Cambia los contextos de usuario y simula cómo el Agente ejecuta su ejecutable.
...
### El Agente se niega a iniciar {#agent-refusing-to-start}

Lo primero que hace el Agente al iniciar es cargar `datadog.yaml` y descifrar cualquier secreto en él. Esto se realiza antes de configurar el registro. Esto significa que en plataformas como Windows, los errores que ocurren al cargar `datadog.yaml` no se escriben en los registros, sino en `stderr`. Esto puede ocurrir cuando el ejecutable proporcionado al Agente para secretos devuelve un error.

Si tiene secretos en `datadog.yaml` y el Agente se niega a iniciar:

* Intente iniciar el Agente manualmente para poder ver `stderr`.
* Elimine los secretos de `datadog.yaml` y pruebe primero con secretos en un archivo de configuración de verificación.

### Pruebas de permisos en Kubernetes {#testing-kubernetes-permissions}
Al leer secretos directamente de Kubernetes, puede verificar sus permisos con el comando `kubectl auth`. La forma general de esto es:
...
Considere el ejemplo anterior de [Secretos de Kubernetes](#example-reading-a-kubernetes-secret-across-namespaces), donde el Secreto `Secret:database-secret` existe en `Namespace: database` y la Cuenta de Servicio `ServiceAccount:datadog-agent` existe en `Namespace: default`.
...
Este comando indica si los permisos son válidos para que el Agente vea este Secreto.

### Elimine los saltos de línea finales {#remove-trailing-line-breaks}

Algunas herramientas de gestión de secretos añaden automáticamente un salto de línea al exportar secretos a través de archivos. Puede eliminar estos saltos de línea configurando `secret_backend_remove_trailing_line_break: true` en [el archivo de configuración datadog.yaml][8], o utilizando la variable de entorno `DD_SECRET_BACKEND_REMOVE_TRAILING_LINE_BREAK` para lograr lo mismo, especialmente en entornos contenedorizados.

### Variables de Autodiscovery en los identificadores de secretos {#autodiscovery-variables-in-secret-handles}

También es posible utilizar variables de [Autodiscovery][1] en identificadores de secretos. El Agente resuelve estas variables antes de resolver el secreto. Por ejemplo:
