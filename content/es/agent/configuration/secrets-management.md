...
Configure el Agente de Datadog para usar secretos de AWS para resolver secretos utilizando la siguiente configuración:
...
El siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` utilizando los secretos de AWS para obtener su clave API de `My-Secrets`:
...
**Alternativamente**, con Datadog Operator v1.25.0+ y Agent v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `spec.global.secretBackend.type: "aws.secrets"` y `spec.global.secretBackend.config` con `aws_session.aws_region: "<AWS_REGION>"`.
...
El secreto del backend se menciona en tu archivo de configuración del Agente de Datadog con `ENC[ ]`. El siguiente es un ejemplo donde se necesita recuperar un secreto en texto plano:
...
- `version`: (opcional) el número de versión secreta. Si no se especifica, se utiliza la versión `latest`.
...
GCP Secret Manager admite versiones de secretos. La implementación del Agente también admite el versionado de secretos utilizando el delimitador `;`. Si no se especifica una versión, se utiliza la versión `latest`.
...
**Alternativamente**, con el Operador de Datadog v1.25.0+ y el Agente v7.70+, puede usar el campo nativo `spec.global.secretBackend.type` en lugar de las variables de entorno.
...
**Alternativamente**, con el Operador de Datadog v1.25.0+, puede usar: `spec.global.secretBackend.type: "k8s.secrets"` y `spec.global.secretBackend.config` con las claves `token_path` y `ca_path`.
...
**Alternativamente**, con el Operador de Datadog v1.25.0+, puede usar: `spec.global.secretBackend.type: "k8s.secrets"` y `spec.global.secretBackend.config` con la clave `api_server`.
...
| valor de tipo_secret_backend                                 | Servicio de Archivo                             |
|---------------------------------------------|-----------------------------------------|
|`file.json`           |[JSON][4001]                             |
|`file.yaml`          |[YAML][4002]                        |                            |
|`file.text`          |[TEXT][4003]                        |                            |
...
Las claves API/APP se obtienen como secretos que admiten la actualización en tiempo de ejecución.
...
### Actualización de secretos de verificación de autodetección {#autodiscovery-check-secrets-refresh}
A partir de la versión 7.76 del Agente, las verificaciones programadas de [Autodetección][1] pueden actualizar secretos en tiempo de ejecución si la plantilla utiliza la sintaxis `ENC[]`.
...
A partir de la versión 7.74 del Agente, el Agente puede actualizar automáticamente secretos cuando detecta una clave API inválida. Esto ocurre cuando el Agente recibe una respuesta 403 Prohibido de Datadog o cuando la verificación de salud periódica detecta una clave API inválida o caducada.
...
Para probar o depurar fuera del Agente, puede imitar cómo el Agente lo ejecuta:
...
El `dd-agent` usuario se crea cuando instalas el Agente de Datadog.
...
También es posible utilizar variables de [Autodetección][1] en manejadores de secretos. El Agente resuelve estas variables antes de resolver el secreto. Por ejemplo:
