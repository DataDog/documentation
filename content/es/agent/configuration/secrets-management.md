...
El Datadog Agent te ayuda a gestionar tus secretos de manera segura integrándose con las siguientes soluciones de gestión de secretos:
...
### Opción 1: Usar soporte nativo del Agente para obtener secretos {#option-1-using-native-agent-support-for-fetching-secrets}
...
**Nota**: Si estás ejecutando Datadog en un entorno contenedorizado, el [Cluster Agent](/containers/cluster_agent/) requiere Datadog Agent 7.77 o versiones posteriores para soportar la obtención nativa de secretos. Para versiones anteriores, use [Opción 2](#option-2-using-the-built-in-script-for-kubernetes-and-docker) o [Opción 3](#option-3-creating-a-custom-executable) en su lugar.

Las instrucciones de configuración más específicas dependen del tipo de backend utilizado. Consulte la sección correspondiente a continuación para más información:
...
Configure el Datadog Agent para usar secretos de AWS para resolver secretos utilizando la siguiente configuración:
...
* `secretId`: ya sea el "nombre amigable" del secreto (por ejemplo, `/DatadogAgent/Production`) o el ARN (por ejemplo, `arn:aws:secretsmanager:us-east-1:123456789012:secret:/DatadogAgent/Production-FOga1K`).
  - **Nota**: Se requiere el formato completo de ARN al acceder a secretos de una cuenta diferente donde se define la credencial de AWS o la credencial `sts:AssumeRole`.
...
Lo siguiente es un ejemplo completo del archivo de configuración `datadog.yaml` utilizando los secretos de AWS para obtener su clave API de `My-Secrets`:
...
<div class="alert alert-info"> Debes incluir el <code>serviceAccountAnnotations</code> para otorgar al Datadog Agent permisos para acceder al secreto de AWS. </div>
...
**Alternativamente**, con el gráfico de Helm v3.171.0+ y el Agente v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `datadog.secretBackend.type: "aws.secrets"` y `datadog.secretBackend.config.aws_session.aws_region: "<AWS_REGION>"`.
...
**Alternativamente**, con Datadog Operator v1.25.0+ y Datadog Agent v7.70+, puedes usar campos nativos `secretBackend.type` y `secretBackend.config` en lugar de variables de entorno. Por ejemplo: `spec.global.secretBackend.type: "aws.secrets"` y `spec.global.secretBackend.config` con `aws_session.aws_region: "<AWS_REGION>"`.
...
El secreto del backend se menciona en el archivo de configuración de tu Agente de Datadog con `ENC[ ]`. El siguiente es un ejemplo donde se necesita recuperar un secreto en texto plano:
...
En el siguiente ejemplo, asume que el prefijo de la ruta de secreto de HashiCorp Vault es `/Datadog/Production` con una clave de parámetro de `apikey`:

```sh
/DatadogAgent/Production/apikey: (SecureString) "<your_api_key>"
```

El siguiente ejemplo obtiene el valor de la clave de API de HashiCorp Vault utilizando AWS para la autenticación.
...
Configura el Datadog Agent para usar Secretos de Kubernetes con la siguiente configuración:
...
Cada referencia de secreto puede especificar un namespace diferente (se debe configurar RBAC para cada uno):
...
**Nota:** Se requiere un marcador de posición `apiKey` para la validación del Helm chart al usar el backend de secretos para resolver la clave de API. La variable de entorno `DD_API_KEY` la anula. Debes crear manualmente RBAC (Role + RoleBinding) para cada namespace que contenga secretos. Para más información, consulte la sección de [configuración de RBAC](#rbac-setup).

**Alternativamente**, con el gráfico de Helm v3.171.0+ y el Agente v7.70+, usted puede usar el campo nativo `datadog.secretBackend.type` en lugar de las variables de entorno.
...
**Alternativamente**, con Datadog Operator v1.25.0+ y el Datadog Agent v7.70+, puedes usar el campo nativo `spec.global.secretBackend.type` en lugar de las variables de entorno.
...
**Alternativamente**, con Helm chart v3.171.0+, puedes usar: `datadog.secretBackend.type: "k8s.secrets"` y `datadog.secretBackend.config` con las claves `token_path` y `ca_path`.
...
**Disponible en la versión 7.75+ del Datadog Agent**
...
Configura el Datadog Agent para usar Docker Secrets con la siguiente configuración:
...
| secret_backend_type value                                 | Servicio de Archivo                             |
|---------------------------------------------|-----------------------------------------|
|`file.json`           |[JSON][4001]                             |
|`file.yaml`          |[YAML][4002]                        |                            |
|`file.text`          |[TEXT][4003]                        |                            |
...
El backend de archivo solo requiere **lectura** permisos para los archivos JSON, YAML o TEXTO configurados. Estos permisos deben ser otorgados al usuario local del Agente de Datadog (`dd-agent` en Linux, `ddagentuser` en Windows).
...
`/path/to/secrets/dd_api_key` que contienen:

```
your_api_key_value
```

`/path/to/secrets/dd_app_key` que contienen:
...
En Kubernetes, puede montar un Secreto como un volumen de la siguiente manera:
...
Luego puede hacer referencia al secreto de la siguiente manera:
...
Alternativamente, puede definir recursos de RBAC directamente:
...
Este `Role` otorga acceso al `Secret: database-secret` en el `Namespace: database`. El `RoleBinding` vincula este permiso al `ServiceAccount: datadog-agent` en el `Namespace: default`. Esto necesita ser agregado manualmente a su clúster con respecto a sus recursos desplegados.
...
Esto le permite utilizar cualquier solución de gestión de secretos y le da el control total sobre cómo el Agente accede a los secretos.
...
Puede configurar el Agente para usar el binario para resolver secretos añadiendo lo siguiente:
...
* No tener derechos para ningún usuario o grupo excepto para el grupo de **Administradores**, la cuenta incorporada de **Sistema Local**, o el contexto de usuario del Agente (`ddagentuser` por defecto).
...
Los agentes se actualizan simultáneamente. La clave se resuelve al inicio, luego se actualiza una vez dentro del primer intervalo
y cada intervalo posterior.

Para evitar tiempo de inactividad, invalide las claves antiguas solo después de que toda su flota haya obtenido las claves de API actualizadas. Puede rastrear el uso de la clave
en la página [Fleet Management](https://app.datadoghq.com/fleet).
...
A partir de la versión v7.76 del Agente, las verificaciones programadas de [Autodiscovery][1] pueden actualizar secretos en tiempo de ejecución si la plantilla utiliza la sintaxis `ENC[]`.
...
### Actualización automática de secretos en caso de fallo/invalidación de clave de API {#automatic-secrets-refresh-on-api-key-failure-invalidation}

A partir de la versión v7.74, el Agente puede actualizar automáticamente secretos cuando detecta una clave de API inválida. Esto ocurre cuando el Agente recibe una respuesta 403 Forbidden de Datadog o cuando la verificación de salud periódica detecta una clave de API inválida o caducada.
...
### Viendo configuraciones después de que se inyectaron secretos {#seeing-configurations-after-secrets-were-injected}
...
### Depurando su comando_secret_backend {#debugging-your-secret-backend-command}
...
El `dd-agent` usuario se crea al instalar el Datadog Agent.
...
1. Si algún grupo o usuario, diferente al necesario, tiene derechos sobre el ejecutable, se registra un error similar al siguiente:
   ```
...
Su ejecutable es ejecutado por el Agente al obtener sus secretos. El Agente de Datadog se ejecuta utilizando el `ddagentuser`. Este usuario no tiene derechos específicos, pero es parte del grupo `Performance Monitor Users`. La contraseña de este usuario se genera aleatoriamente en el momento de la instalación y nunca se guarda en ningún lugar.
...
1. Elimine `ddagentuser` de la `Local Policies/User Rights Assignement/Deny Log on locally` lista en el `Local Security Policy`.
2. Establezca una nueva contraseña para `ddagentuser` (ya que la generada en el momento de la instalación nunca se guarda en ningún lugar). En PowerShell, ejecute:
    ...
    ```
3. Actualice la contraseña que será utilizada por el servicio `DatadogAgent` en el Administrador de Control de Servicios. En PowerShell, ejecute:
    ```powershell
...
Considere el ejemplo anterior de [Kubernetes Secrets](#example-reading-a-kubernetes-secret-across-namespaces), donde el Secreto `Secret:database-secret` existe en el `Namespace: database`, y la Cuenta de Servicio `ServiceAccount:datadog-agent` existe en el `Namespace: default`.

En este caso, use el siguiente comando:
...
También es posible utilizar variables de [Autodiscovery][1] en manejadores de secretos. El Agente resuelve estas variables antes de resolver el secreto. Por ejemplo:
