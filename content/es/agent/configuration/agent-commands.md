...
Para sistemas basados en Linux donde el <code>service</code> el comando wrapper no está disponible, <a href="/agent/faq/agent-v6-changes/?tab=linux#service-lifecycle-commands">consulta la lista de alternativas</a>.
</div>

## Inicia, detén y reinicia el Agente {#start-stop-and-restart-the-agent}

### Inicia el Agente {#start-the-agent}
...
### Detén el Agente {#stop-the-agent}
...
### Reinicia el Agente {#restart-the-agent}
...
| Windows    | Consulta la [documentación del Datadog Agent para Windows][4].            |
| [Agente de Clúster (Kubernetes)][5] | `datadog-cluster-agent status`       |
...
| `launch-gui`      | Inicia la interfaz gráfica del Agente de Datadog.                                                |
| `restart-service` | Reinicia el Agente de Datadog dentro del administrador de control de servicios. Solo para Windows.         |
| `start-service`   | Inicia el Agente de Datadog dentro del administrador de control de servicios. Solo para Windows.           |
| `stream-logs`     | Transmite los registros que está procesando un Agente de Datadog en ejecución.                         |
| `stopservice`     | Detén el Agente de Datadog dentro del administrador de control de servicios. Solo para Windows.            |
