...
description: Referencia completa de los comandos de Datadog Agent para iniciar, detener,
  solucionar problemas y gestionar el Datadog Agent.
further_reading:
...
| Docker     | Utilice el [comando de instalación][2].                                               |
| Kubernetes | `kubectl delete pod <AGENT POD NAME>`—nota: el pod se reprograma automáticamente |
| macOS      | Detenga y luego inicie el Agente con:<br>`launchctl stop com.datadoghq.agent`<br>`launchctl start com.datadoghq.agent`<br>O use la aplicación de la bandeja del sistema |
...
Algunas opciones tienen banderas y opciones detalladas bajo `--help`. Por ejemplo, use ayuda con el subcomando `check`:
...
| `flare`           | [Recopila un paquete de diagnóstico y envíalo a Datadog][8].                                |
| `health`          | Imprima la salud actual del Agente.                                             |
| `help`            | Ayuda sobre cualquier comando.                                                     |
| `hostname`        | Imprima el nombre de host utilizado por el Agente.                                       |
| `import`          | Importe y convierta archivos de configuración de versiones anteriores del Agente. |
| `jmx`             | Solución de problemas de JMX.                                                        |
| `launch-gui`      | Inicie la GUI del Agente de Datadog.                                                |
| `restart-service` | Reinicie el Agente dentro del administrador de control de servicios. Solo Windows.         |
| `start-service`   | Inicie el Agente dentro del administrador de control de servicios. Solo Windows.           |
| `stream-logs`     | Transmita los registros que están siendo procesados por un agente en ejecución.                         |
| `stopservice`     | Detenga el Agente dentro del administrador de control de servicios. Solo Windows.            |
