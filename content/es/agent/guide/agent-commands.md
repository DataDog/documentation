...
En sistemas basados en Linux en los que no esté disponible el comando del contenedor <code>service</code>, <a href="/agent/faq/agent-v6-changes/?tab=linux#service-lifecycle-commands">consulta la lista de alternativas</a>.
</div>
...
| Windows    | Consulta la [documentación sobre el Agent de Windows][3].                          |
...
| Docker   | Consulta la [documentación sobre el Agent de Docker][1].  |
| macOS    | `/usr/local/bin/datadog-agent start`      |
| Origen   | `sudo ~/.datadog-agent/bin/agent start`   |
| Windows  | Consulta la [documentación sobre el Agent de Windows][2]. |
...
Lista de comandos para desplegar el estado del Datadog Agent:
...
| Windows         | Consulta la [documentación sobre el Agent de Windows][1].                                |
...
Lista de comandos para desplegar el estado de tu Datadog Agent y de tus integraciones habilitadas.
...
| `hostname`        | Muestra el nombre del host que utiliza el Agent.                                       |
...
| `restart-service` | Reinicia el Agent en el gestor de control de servicios. Solo en Windows.         |
| `start-service`   | Inicia el Agent en el gestor de control de servicios. Solo en Windows.           |
| `stream-logs`     | Genera un flujo de los logs procesados por un Agent en ejecución.                         |
| `stopservice`     | Detiene el Agent en el gestor de control de servicios. Solo en Windows.            |
