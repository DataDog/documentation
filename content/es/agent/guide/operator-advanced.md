...
* [Manifiesto con logs y recopilación de métricas activos.][6]
* [Manifiesto con APM y recopilación de métricas activos.][7]
* [Manifiesto con el Cluster Agent.][8]
* [Manifiesto con tolerancias.][9]

Sustituye `<DATADOG_API_KEY>` y `<DATADOG_APP_KEY>` por tus [claves de API y de aplicación de Datadog][10] y, luego, activa la instalación del Agent con el siguiente comando:
...
Puedes comprobar el estado del recurso `DatadogAgent` con este comando:
...
En un clúster con 2 nodos worker, los pods del Agent creados deberían verse en cada uno de ellos.
...
El siguiente comando borra todos los recursos Kubernetes creados mediante las instrucciones anteriores:
...
Aplica la siguiente configuración a tu archivo `datadog-agent.yaml` para añadir la tolerancia en la `Daemonset.spec.template` de tu `DaemonSet`:
