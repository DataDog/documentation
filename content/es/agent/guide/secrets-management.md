...
El Agent puede servirse del paquete `secrets` para invocar un archivo ejecutable proporcionado por el usuario con el objetivo de gestionar la recuperación y descifrado de secretos, que luego suelen cargarse en su memoria. Este proceso permite al usuario utilizar cualquier servicio backend de gestión de secretos (como HashiCorp Vault o AWS Secrets Manager) y elegir su método de autenticación preferido para establecer el proceso de confianza inicial. Para facilitar las cosas, los [scripts auxiliares](#helper-scripts-for-autodiscovery) que pueden utilizarse para este archivo ejecutable incluyen de base despliegues contenedorizados del Agent.
...
Para ver los secretos, es necesario que proporciones un ejecutable que pueda autenticarse en tu backend de gestión de secretos y recuperarlos.
...
De forma predeterminada, el ejecutable que proporciona el usuario debe incluir los mecanismos necesarios de gestión de errores. En cambio, el Agent debe reiniciarse si un secreto tiene que actualizarse en la memoria (por ejemplo, si se revoca una contraseña).
...
* Permite al usuario limitar la visibilidad del Agent a los secretos que necesita (por ejemplo, restringiendo la lista de secretos a los que puede acceder en el backend de gestión de claves).
...
* Posibilita que los usuarios resuelvan el problema de confianza inicial del Agent en sus backends de gestión de secretos. Este proceso permite aplicar el método de autenticación preferido del usuario y se ajusta a su flujo de integración continuo.
...
La carga útil prevista es un objeto JSON, donde cada una de las claves representa uno de los identificadores solicitados en la carga útil de entrada. El valor de cada uno de esos indicadores es un objeto JSON con dos campos:
...
#### Uso
Estos scripts necesitan incluir una carpeta como argumento. En lo que respecta a esta carpeta, los identificadores de secretos se interpretan como nombres de archivos. Para evitar que se filtren datos confidenciales, los scripts prohíben acceder a cualquier archivo que esté fuera de la carpeta raíz especificada (aunque sean destinos de enlaces simbólicos).
...
Con esta configuración, el Datadog Agent lee los archivos de secretos ubicados en el directorio `/run/secrets`. Por ejemplo, la configuración `ENC[password]` serviría para que el Agent buscase el archivo `/run/secrets/password`.
...
Con esta configuración, el Datadog Agent lee los archivos de secretos ubicados en el directorio `/etc/secret-volume`. Por ejemplo, la configuración `ENC[password]` serviría para que el Agent buscase el archivo `/etc/secret-volume/password`.

## Solucionar problemas

### Mostrar una lista con los secretos detectados

El comando `secret` de la CLI del Agent muestra los errores relacionados con tu configuración. Por ejemplo, puede indicar si los derechos del ejecutable son incorrectos. También genera una lista con los identificadores que encuentra y sus ubicaciones.
...
Si te encuentras con uno de los siguientes errores, significa que tu configuración está incompleta. Consulta las [instrucciones para Windows](#windows).
...
* Prueba a iniciar el Agent manualmente para poder ver `stderr`.
...
[6]: /es/agent/guide/agent-commands/#restart-the-agent