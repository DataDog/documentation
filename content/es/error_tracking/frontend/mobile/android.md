...
- link: https://github.com/DataDog/dd-sdk-android
  tag: Source Code
  text: Código fuente para ddsdkandroid
...
El [seguimiento de errores en Android][1] te brinda visibilidad completa sobre la salud de tu aplicación móvil al capturar automáticamente fallos, excepciones y errores. Con esta función, puedes:
...
- Depurar problemas más rápido con trazas de pila desofuscadas y cargas automáticas de archivos de mapeo de ProGuard para una identificación más fácil de problemas.
- Mejorar la calidad de la aplicación al identificar características propensas a fallos, rastrear tendencias de errores y priorizar correcciones para una mejor satisfacción del usuario.
- Acceder a paneles de control de fallos en Android agregados y atributos.
- Ver informes de fallos en Android desofuscados con análisis de tendencias.
...
Declara [ddsdkandroidrum][5] y el [plugin de Gradle][6] como dependencias en tu **módulo de aplicación** `build.gradle` archivo:
...
### Paso 2  Especifica los detalles de la aplicación en la interfaz
...
Las credenciales de inicialización requieren el nombre de variante de tu aplicación y utilizan el valor de `BuildConfig.FLAVOR`. Con la variante, el SDK puede relacionar los errores reportados desde tu aplicación con los archivos de mapeo subidos por el plugin de Gradle. Si no tienes variantes, las credenciales utilizan una cadena vacía.

El plugin de Gradle sube automáticamente el archivo ProGuard `mapping.txt` apropiado en el momento de la construcción para que puedas ver las trazas de error desofuscadas. Para más información, consulta la sección [Sube tu archivo de mapeo](#upload-your-mapping-file).
...
Para instrumentar tus vistas web:

1. Agrega la dependencia de Gradle declarando ddsdkandroidwebview como dependencia en tu archivo build.gradle:
   ...
   ```
2. Habilita el seguimiento de webview para una instancia de WebView dada proporcionando una lista de hosts a rastrear:

   ```kotlin
   WebViewTracking.enable(webView, hosts)
   ```

Para más información, consulta [Seguimiento de Web View][8].

### Paso 4 Agrega el reporte de fallos de NDK

Si tu aplicación de Android utiliza código nativo (C/C++) a través del NDK de Android (Kit de Desarrollo Nativo), puedes rastrear los bloqueos que ocurren en este código nativo. El código nativo se utiliza a menudo para operaciones críticas de rendimiento, procesamiento de imágenes o cuando se reutilizan bibliotecas existentes de C/C++.

Sin el reporte de fallos de NDK, los bloqueos en tu código nativo no aparecen en el Seguimiento de Errores, lo que dificulta la depuración de problemas en esta parte de tu aplicación.

Para habilitar el reporte de fallos de NDK, utiliza el plugin de NDK de Datadog:

1. Agrega la dependencia de Gradle declarando la biblioteca como una dependencia en tu `build.gradle` archivo:
   ...
   ```
2. Habilita la recolección de fallos de NDK después de inicializar el SDK:
...
Un "Aplicación No Responde" ([ANR][18]) es un tipo de error específico de Android que se activa cuando la aplicación no responde durante demasiado tiempo. Puedes agregar informes de ANR a tu configuración de RUM para monitorear estos problemas de respuesta de la aplicación.

Para habilitar el informe de ANR, agrega lo siguiente a tu configuración de RUM:
...
Los ANRs solo se informan a través del SDK (no a través de Logs).

#### Informando ANRs fatales
Los ANRs fatales resultan en bloqueos. La aplicación los informa cuando no responde, lo que lleva al sistema operativo Android a mostrar un cuadro de diálogo emergente al usuario, quien elige forzar el cierre de la aplicación a través del cuadro emergente.

{{< img src="real_user_monitoring/error_tracking/rum-anr-fatal.png" alt="Un informe de bloqueo fatal en el Seguimiento de Errores." >}}

- En la página de **Seguimiento de Errores**, los ANRs fatales se agrupan según su similitud, lo que puede resultar en varios **problemas individuales** siendo creados.
- Por defecto, Datadog captura ANRs fatales a través de la [API ApplicationExitInfo][19] (disponible desde *[Android 30+][20]*), que se puede leer en el siguiente lanzamiento de la aplicación.
- En *[Android 29][21] y versiones anteriores*, no es posible informar sobre ANRs fatales.

#### Informando ANRs no fatales
Los ANRs no fatales pueden o no haber llevado a la terminación de la aplicación (bloqueo).

{{< img src="real_user_monitoring/error_tracking/rum-anr-non-fatal.png" alt="Un informe de bloqueo no fatal en el Seguimiento de Errores." >}}

- En la página de **Seguimiento de Errores**, los ANRs no fatales se agrupan bajo un **único** problema debido a su nivel de ruido.
- Por defecto, el informe de ANRs no fatales en *Android 30+* está **deshabilitado** porque crearía demasiado ruido sobre los ANRs fatales. En *Android 29* y versiones anteriores, sin embargo, el informe de ANRs no fatales está **habilitado** por defecto, ya que no se pueden informar ANRs fatales en esas versiones.
...
Cuando tu aplicación de Android se construye para producción, el código se ofusca típicamente usando ProGuard o R8 para reducir el tamaño de la aplicación y proteger la propiedad intelectual. Esta ofuscación hace que las trazas de pila en los informes de bloqueos sean ilegibles, mostrando nombres de clases y métodos sin sentido como `a.b.c()` en lugar de `com.example.MyClass.myMethod()`.
...
- Las trazas de pila siempre se desofusquen con el archivo de mapeo correcto, independientemente de cuándo se subió.
- Puedes subir archivos de mapeo durante construcciones de preproducción o producción.
- El proceso funciona sin problemas a través de diferentes variantes de construcción y entornos.

El proceso de emparejamiento depende de tu versión del [plugin de Gradle de Android][22]:

- **Versiones 1.13.0 y superiores**: Utiliza el campo `build_id` (requiere Datadog Android SDK 2.8.0 o posterior)
- **Versiones anteriores**: Utiliza una combinación de los campos `service`, `version` y `variant`

#### Sube tu archivo de mapeo

El complemento de Gradle para Android automatiza el proceso de carga del archivo de mapeo. Después de la configuración, carga automáticamente el archivo de mapeo ProGuard/R8 apropiado para cada variante de compilación cuando construyes tu aplicación.

**Nota**: Volver a cargar un archivo de mapeo no sobrescribe el existente si la versión no ha cambiado. Para información sobre limitaciones de tamaño de archivo y otras restricciones, consulta la sección [Limitaciones](#limitations).

#### Ejecuta las tareas de carga

Después de configurar el complemento, ejecuta las tareas de Gradle para cargar tu archivo de mapeo Proguard/R8 y los archivos de símbolos NDK a Datadog:
...
1. Agrega el [Complemento de Gradle para Android][22] a tu proyecto de Gradle usando el siguiente fragmento de código.
...
2. [Crea una clave API dedicada de Datadog][23] y expórtala como una variable de entorno llamada `DD_API_KEY` o `DATADOG_API_KEY`. Alternativamente, pásala como una propiedad de tarea, o si tienes un archivo `datadog-ci.json` en la raíz de tu proyecto, se puede tomar de una propiedad `apiKey` allí.
3. Opcionalmente, configura el complemento para cargar archivos a la región de la UE configurando el complemento en tu script `build.gradle`:
...
4. Ejecuta la tarea de carga después de que se construyan tus APK ofuscados:
...
**Nota**: Si tu proyecto utiliza sabores adicionales, el complemento proporciona una tarea de carga para cada variante con ofuscación habilitada. En este caso, inicializa el SDK de Android con un nombre de variante adecuado (la API necesaria está disponible en versiones `1.8.0` y posteriores).
...
Consulta la página de [Símbolos de depuración RUM][24] para ver todos los símbolos cargados.

## Funciones avanzadas de seguimiento de errores

{{% collapse-content title="Establecer el consentimiento de seguimiento (cumplimiento del GDPR)" level="h4" expanded=false id="set-tracking-consent" %}}

Para cumplir con la regulación del GDPR, el SDK requiere el valor de consentimiento de seguimiento al inicializarse.
...
- `TrackingConsent.NOT_GRANTED`: El SDK elimina todos los datos agrupados y no recopila ningún dato futuro.
...
{{% collapse-content title="Inicializar el interceptor para rastrear eventos de red" level="h4" expanded=false id="interceptor" %}}
...
1. Para el trazado distribuido, [agrega y habilita la función de Trazado][13].
2. Agrega la dependencia de Gradle a la `dd-sdk-android-okhttp` biblioteca en el archivo `build.gradle` a nivel de módulo:
...
4. Para crear automáticamente recursos y tramos de RUM para tus solicitudes de OkHttp, usa el `DatadogInterceptor` como un interceptor.
   - Esto registra cada solicitud procesada por el `OkHttpClient` como un recurso, con toda la información relevante (URL, método, código de estado y error) completada automáticamente. Solo se rastrean las solicitudes de red que comenzaron cuando una vista está activa. Para rastrear solicitudes cuando tu aplicación está en segundo plano, [crea una vista manualmente][15].
...
El SDK de Android asegura la disponibilidad de datos cuando el dispositivo del usuario está desconectado. En caso de áreas de baja conectividad, o cuando la batería del dispositivo está muy baja, todos los eventos se almacenan primero en el dispositivo local en lotes. 

Cada lote sigue la especificación de entrada. Los lotes se envían tan pronto como la red está disponible y la batería es lo suficientemente alta para asegurar que el SDK de Datadog no impacte la experiencia del usuario final. Si la red no está disponible mientras tu aplicación está en primer plano, o si una carga de datos falla, el lote se mantiene hasta que pueda ser enviado con éxito.
...
Hay varias propiedades del plugin que se pueden configurar a través de la extensión del plugin. En caso de que estés usando múltiples variantes, puedes establecer un valor de propiedad para un sabor específico en la variante.
...
| `site`                     | El sitio de Datadog al que subir tus datos (US1, US3, US5, EU1, US1_FED, AP1 o AP2).                                                                                                                                       |
| `remoteRepositoryUrl`      | La URL del repositorio remoto donde se desplegó el código fuente. Si esto no se proporciona, este valor se resuelve a partir de tu configuración de Git durante el tiempo de ejecución de la tarea.                     |
...
Por defecto, la tarea de carga de mapeo es independiente de otras tareas en el gráfico de construcción. Ejecuta la tarea manualmente cuando necesites cargar el mapeo.
...
- Establece un mapa de `mappingFilePackagesAliases`: Esto reemplaza los nombres de paquetes con alias más cortos. **Nota**: El stacktrace de Datadog utiliza el mismo alias en lugar del nombre de paquete original, por lo que es mejor usar esta opción para dependencias de terceros.
...
- Los fallos de RUM deben estar adjuntos a una vista de RUM. Si ocurre un fallo antes de que una vista sea visible (típicamente una Actividad o Fragmento en un estado `onResume`), o después de que la aplicación sea enviada al fondo por el usuario final navegando fuera de ella, el fallo se silencia y no se informa para la colección. Para mitigar esto, utiliza el `trackBackgroundEvents()` [método][25] en tu `RumConfiguration` constructor.
- Solo se mantienen los fallos que ocurren en sesiones muestreadas. Si una [tasa de muestreo de sesión no es del 100%][24], algunos fallos no se informan. 

## Prueba tu implementación

Para verificar tu configuración de informes de fallos y seguimiento de errores de Android, necesitas provocar un fallo en tu aplicación y confirmar que el error aparece en Datadog.
...
3. Después de que ocurra el fallo, reinicia tu aplicación y espera a que el SDK de Android suba el informe de fallos en [**Seguimiento de Errores**][2].

## Extensiones de Kotlin

### `Closeable` extensión

Puedes monitorear el uso de `Closeable` instancias con el método `useMonitored`, que reporta errores a Datadog y cierra el recurso después:
...
Puedes rastrear el acceso a los activos utilizando el método de extensión `getAssetAsRumResource`:
