...
Si tu aplicación de Android utiliza WebViews para mostrar contenido web, puedes instrumentarlas para rastrear errores de JavaScript y fallos que ocurren dentro del contenido web.
...
Si tu aplicación de Android utiliza código nativo (C/C++) a través del NDK de Android (Kit de Desarrollo Nativo), puedes rastrear los fallos que ocurren en este código nativo. El código nativo se utiliza a menudo para operaciones críticas de rendimiento, procesamiento de imágenes o cuando se reutilizan bibliotecas existentes de C/C++.

Sin el reporte de fallos del NDK, los fallos en tu código nativo no aparecen en el seguimiento de errores, lo que dificulta la depuración de problemas en esta parte de tu aplicación.

Para habilitar el reporte de fallos del NDK, utiliza el plugin de NDK de Datadog:

1. Agrega la dependencia de Gradle declarando la biblioteca como una dependencia en tu archivo `build.gradle`:
   ...
   ```
2. Habilita la recolección de fallos del NDK después de inicializar el SDK:

    ```kotlin
    NdkCrashReports.enable()
    ```

### Paso 5 - Agregar reporte de ANR

Un "Aplicación No Responde" ([ANR][18]) es un tipo de error específico de Android que se activa cuando la aplicación no responde durante demasiado tiempo. Puedes agregar el reporte de ANR a tu configuración de RUM para monitorear estos problemas de respuesta de la aplicación.
...
Los ANRs solo se reportan a través del SDK (no a través de Logs).

#### Reportando ANRs fatales
Los ANRs fatales resultan en fallos. La aplicación los reporta cuando no responde, lo que lleva al sistema operativo Android a mostrar un cuadro de diálogo emergente al usuario, quien elige forzar el cierre de la aplicación a través del cuadro emergente.

{{< img src="real_user_monitoring/error_tracking/rum-anr-fatal.png" alt="Un reporte de fallo fatal en el seguimiento de errores." >}}

- En la página **Seguimiento de Errores**, los ANRs fatales se agrupan según su similitud, lo que puede resultar en varios **problemas individuales** siendo creados.
...
#### Reportando ANRs no fatales
Los ANRs no fatales pueden o no haber llevado a la terminación de la aplicación (fallo).

{{< img src="real_user_monitoring/error_tracking/rum-anr-non-fatal.png" alt="Un informe de fallo no fatal en el seguimiento de errores." >}}

- En la página de **Seguimiento de Errores**, los ANRs no fatales se agrupan bajo un **único** problema debido a su nivel de ruido.
- Por defecto, el informe de ANRs no fatales en *Android 30+* está **deshabilitado** porque crearía demasiado ruido sobre los ANRs fatales. En *Android 29* y versiones anteriores, sin embargo, el informe de ANRs no fatales está **habilitado** por defecto, ya que los ANRs fatales no se pueden informar en esas versiones.
...
Cuando tu aplicación de Android se construye para producción, el código se ofusca típicamente usando ProGuard o R8 para reducir el tamaño de la aplicación y proteger la propiedad intelectual. Esta ofuscación hace que las trazas de pila en los informes de fallos sean ilegibles, mostrando nombres de clases y métodos sin sentido como `a.b.c()` en lugar de `com.example.MyClass.myMethod()`.
...
Datadog utiliza un ID de compilación único generado para cada compilación para emparejar automáticamente las trazas de pila con los archivos de mapeo correctos. Esto asegura que:

- Las trazas de pila siempre se desofusquen con el archivo de mapeo correcto, independientemente de cuándo se haya subido.
- Puedes subir archivos de mapeo durante las compilaciones de preproducción o producción.
- El proceso funciona sin problemas en diferentes variantes de compilación y entornos.

El proceso de coincidencia depende de tu versión del [plugin de Gradle para Android][22]:

- **Las versiones 1.13.0 y superiores**: Utiliza el `build_id` campo (requiere el SDK de Datadog para Android 2.8.0 o posterior)
- **Versiones anteriores**: Utiliza una combinación de los campos `service`, `version` y `variant`

#### Sube tu archivo de mapeo

El plugin de Gradle para Android automatiza el proceso de carga del archivo de mapeo. Después de la configuración, automáticamente sube el archivo de mapeo de ProGuard/R8 apropiado para cada variante de compilación cuando construyes tu aplicación.
...
Después de configurar el plugin, ejecuta las tareas de Gradle para subir tu archivo de mapeo de Proguard/R8 y los archivos de símbolos de NDK a Datadog:
...
5. Si estás ejecutando código nativo, ejecuta la tarea de carga de símbolos de NDK:
   ```bash
   ./gradlew uploadNdkSymbolFilesRelease
   ```

**Nota**: Si tu proyecto utiliza sabores adicionales, el plugin proporciona una tarea de carga para cada variante con ofuscación habilitada. En este caso, inicializa el SDK de Android con un nombre de variante adecuado (la API necesaria está disponible en versiones `1.8.0` y posteriores).
...
#### Lista de archivos de mapeo subidos
...
{{% collapse-content title="Establecer consentimiento de seguimiento (cumplimiento de GDPR)" level="h4" expanded=false id="set-tracking-consent" %}}

Para cumplir con la regulación GDPR, el SDK requiere el valor de consentimiento de seguimiento al inicializarse.

El consentimiento de seguimiento puede ser uno de los siguientes valores:

- `TrackingConsent.PENDING`: (Predeterminado) El SDK comienza a recopilar y agrupar los datos, pero no los envía a la
...
- `TrackingConsent.GRANTED`: El SDK envía todos los datos agrupados actuales y futuros directamente al punto final de recolección de datos.
- `TrackingConsent.NOT_GRANTED`: El SDK elimina todos los datos agrupados y no recopila ningún dato futuro.

{{% /collapse-content %}}

{{% collapse-content title="Tasas de muestra de sesión" level="h4" expanded=false id="sample-session-rates" %}}
...
2. Agrega la dependencia de Gradle a la biblioteca `dd-sdk-android-okhttp` en el archivo `build.gradle` a nivel de módulo:
...
El SDK de Android asegura la disponibilidad de datos cuando el dispositivo del usuario está desconectado. En caso de áreas con baja red, o cuando la batería del dispositivo está muy baja, todos los eventos se almacenan primero en el dispositivo local en lotes. 
...
| `site`                     | El sitio de Datadog al que subirás tus datos (US1, US3, US5, EU1, US1_FED, AP1 o AP2).                                                                                                                                       |
| `remoteRepositoryUrl`      | La URL del repositorio remoto donde se desplegó el código fuente. Si no se proporciona, este valor se resuelve a partir de tu configuración de Git durante el tiempo de ejecución de la tarea.                     |
| `checkProjectDependencies` | Esta propiedad controla si el plugin debe verificar si el SDK de Datadog para Android está incluido en las dependencias. Si no, `none` se ignora, `warn` registra una advertencia y `fail` falla la construcción con un error (por defecto). |

{{% /collapse-content %}}

{{% collapse-content title="Integrar con un pipeline de CI/CD" level="h4" expanded=false id="plugin-config-options" %}}

Por defecto, la tarea de mapeo de carga es independiente de otras tareas en el gráfico de construcción. Ejecuta la tarea manualmente cuando necesites subir el mapeo.
...
- Establece un mapa de `mappingFilePackagesAliases`: Esto reemplaza los nombres de los paquetes con alias más cortos. **Nota**: El stacktrace de Datadog utiliza el mismo alias en lugar del nombre original del paquete, por lo que es mejor usar esta opción para las dependencias de terceros.
...
- Los fallos de RUM deben estar adjuntos a una vista de RUM. Si ocurre un fallo antes de que una vista sea visible (típicamente una Actividad o Fragmento en un estado `onResume`), o después de que la aplicación sea enviada al fondo por el usuario al navegar fuera de ella, el fallo se silencia y no se reporta para su recolección. Para mitigar esto, utiliza el método `trackBackgroundEvents()` [método][25] en tu constructor `RumConfiguration`.
...
Puedes monitorear el uso de la instancia `Closeable` con el método `useMonitored`, que reporta errores a Datadog y cierra el recurso después:
