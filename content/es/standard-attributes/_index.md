...
- description: Esto corresponde al nombre de la integración, la tecnología de la cual
    se originó los datos. Cuando coincide con un nombre de integración, Datadog instala
    automáticamente los analizadores y facetas correspondientes. Por ejemplo, `nginx`,
    `postgresql`, y así sucesivamente.
  domain: Reserved
  name: source
  product_source:
  - icon-log
...
- description: '[Código ISO](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes)
  ...
  - icon-log
  type: string
- description: Código ISO del continente (`UE`, `AS`, `NA`, `AF`, `AN`, `SA`, `OC`).
  domain: Geolocation
...
- description: Código ISO del primer nivel de subdivisión del país (por ejemplo, `CA`
  ...
  - icon-log
  type: string
- description: Nombre de la ciudad (por ejemplo, `París`, `Nueva York`).
  domain: Geolocation
...
- description: La parte del puerto HTTP de la URL.
  ...
  - icon-apm
  type: number
- description: La parte del camino HTTP de la URL.
  domain: HTTP, URL Details
...
- description: El tipo de dispositivo reportado por el dispositivo (User-Agent del
  ...
  - roku
  type: string
- description: La marca del dispositivo reportada por el dispositivo (User-Agent del
    sistema).
  domain: Device (Android, iOS, Roku)
  name: device.brand
  product_source:
  ...
  - roku
  type: string
- description: El modelo del dispositivo reportado por el dispositivo (User-Agent
    del sistema).
  domain: Device (Android, iOS, Roku)
  name: device.model
  product_source:
  ...
  - roku
  type: string
- description: El nombre del dispositivo reportado por el dispositivo (User-Agent
    del sistema).
  domain: Device (Android, iOS, Roku)
  name: device.name
  product_source:
  ...
  - roku
  type: string
- description: Estado de la conectividad de red del dispositivo (`conectado`, `no
    conectado` o `quizás`).
...
- description: Código ISO del continente (`EU`, `AS`, `NA`, `AF`, `AN`, `SA` o `OC`).
  ...
  type: string
- description: Nombre del continente (`Europa`, `Australia`, `América del Norte`,
    `África`, `Antártida`, `América del Sur` u `Oceanía`).
  domain: Geolocation
  name: geo.continent
  product_source:
  - icon-rum
...
- description: Un mensaje conciso, legible por humanos, de una línea que explica el
  ...
  - roku
  type: string
- description: El seguimiento de pila o información complementaria sobre el error.
  domain: Error (Browser events, Android events, iOS events, Roku events)
  name: error.stack
  product_source:
  - icon-apm
...
- description: La URL de la página web anterior desde la cual se siguió un enlace
  ...
  - browser
  type: string
- description: El ID de la primera vista RUM generada por el usuario.
  domain: Session (Browser events)
...
- description: La parte del esquema de la URL.
  ...
  - browser
  type: object
- description: El ID de la última vista RUM generada por el usuario.
  domain: Session (Browser events)
...
- description: El parámetro en la URL que rastrea la palabra clave que un usuario
  ...
  - browser
  type: string
- description: El lenguaje del SDK del cliente utilizado para generar el span. Puede
...
- description: La cadena que representa el tipo de unidad de trabajo manejada por
    el span. Puede ser uno de servidor, cliente, productor, consumidor o interno.
  ...
  - icon-apm
  type: string
- description: El nombre de la biblioteca o integración que creó el span.
  domain: APM core
  name: component
  product_source:
  - icon-apm
...
- description: La URL de la solicitud HTTP, incluyendo la cadena de consulta ofuscada.
    Para más información sobre ofuscación, consulte [Configurar Seguridad de Datos](https://docs.datadoghq.com/tracing/configure_data_security/).
  domain: HTTP requests
  name: http.url
  product_source:
  - icon-apm
...
- description: El tamaño del cuerpo de la carga útil de la respuesta en bytes.
  ...
  - icon-apm
  type: number
- description: El tamaño del cuerpo de la carga útil de la solicitud descomprimida
  ...
  - icon-apm
  type: number
- description: El tamaño del cuerpo de la carga útil de la respuesta descomprimida
    después de la decodificación del transporte.
  domain: HTTP requests
...
- description: El nombre del método que se está llamando.
  ...
  - icon-apm
  type: string
content: La siguiente tabla lista los atributos aplicados automáticamente a los datos
  enviados a Datadog por el Agente de cada uno de los productos RUM, Logs y APM, según
  corresponda al dominio de datos. Opcionalmente, filtre la lista por producto o busque
  por palabra clave o texto de descripción para encontrar los atributos que le interesan.
...
- link: /tracing/trace_collection/tracing_naming_convention/
  tag: Documentación
  text: Semántica de la etiqueta Span
