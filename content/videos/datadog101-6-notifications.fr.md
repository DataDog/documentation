---
title: Datadog 101 - Notifications
kind: video
language: en
wistiaid: 4kjgeqdb5o
videos:
    - Datadog 101
    - Notifications
summary: Utiliser des notifications pour recevoir des alertes en cas de problème en évitant de consulter chaque faux positif
private: true
---

Les vidéos Datadog 101 vous permettent d'apprendre tout ce dont vous avez besoin pour exploiter la plateforme Datadog. Durant cette sixième vidéo, nous vous expliquerons comment utiliser les notifications. Voici la liste des thèmes abordés :

* [Types de notification](?wtime=28)
* [Ajout d'un commentaire dans le flux d'événements](?wtime=85)
* [Annotations](?wtime=107)
* [Monitors](?wtime=127)
* [Monitor de host](?wtime=146)
* [Corps de message](?wtime=204)
* [Template variables](?wtime=226)
* [Monitor de métrique](?wtime=290)
* [Alertes de seuil, de changement et d'anomalie](?wtime=320)
* [Monitor d'intégration](?wtime=386)
* [Données originales et vue monitor](?wtime=429)
* [Monitor de processus](?wtime=479)
* [Monitor réseau](?wtime=509)
* [Check custom](?wtime=535)
* [Monitor d'événement](?wtime=548)
* [Monitor outlier](?wtime=562)
* [Managing your monitors](?wtime=574)

