...
1. Il n'est **pas** nécessaire de définir une facette pour rechercher des attributs et des étiquettes.
...
#### Exemples {#examples}
- `CIDR(@network.client.ip,13.0.0.0/8)` correspond et filtre les journaux qui possèdent des adresses IP dans le champ `network.client.ip` qui appartiennent au bloc CIDR 13.0.0.0/8.
- `CIDR(@network.ip.list,13.0.0.0/8, 15.0.0.0/8)` correspond et filtre les journaux qui comportent des adresses IP dans un attribut de type tableau `network.ip.list` qui appartiennent aux blocs CIDR 13.0.0.0/8 ou 15.0.0.0/8.
- `source:pan.firewall evt.name:reject CIDR(@network.client.ip, 13.0.0.0/8)` correspondrait et filtrerait les événements rejetés du pare-feu Palo Alto qui proviennent du sous-réseau 13.0.0.0/8.
...
La fonction `CIDR()` prend en charge à la fois les notations CIDR IPv4 et IPv6 et fonctionne dans Log Explorer, Live Tail, widgets de journaux dans les tableaux de bord, moniteurs de journaux et configurations de journaux.

## Jokers {#wildcards}

Vous pouvez utiliser des jokers avec la recherche en texte libre. Cependant, cela ne recherche que des termes dans le message du journal, le texte dans la colonne `content` dans Log Explorer. Voir [Recherche en texte intégral](#full-text-search) si vous souhaitez rechercher une valeur dans un attribut de journal.

### Joker à plusieurs caractères {#multi-character-wildcard}

Pour effectuer une recherche avec un joker à plusieurs caractères dans le message du journal (la colonne `content` dans Log Explorer), utilisez le symbole `*` comme suit :
...
**Remarque** : Les jokers ne fonctionnent comme des jokers qu'en dehors des guillemets doubles. Par exemple, `"*test*"` correspond à un journal qui a la chaîne `*test*` dans son message. `*test*` correspond à un journal qui a la chaîne test n'importe où dans son message.

Les recherches avec des jokers fonctionnent au sein des balises et des attributs (facettés ou non) avec cette syntaxe. Cette requête renvoie tous les services qui se terminent par la chaîne `mongo` :
<p> </p>
<p></p>
...
Les recherches par caractères génériques peuvent également être utilisées pour rechercher dans le texte brut d'un journal qui ne fait pas partie d'un attribut de journal. Par exemple, cette requête renvoie tous les journaux dont le contenu (message) contient la chaîne `NETWORK` :
...
### Joker de recherche {#search-wildcard}

Lors de la recherche d'une valeur d'attribut ou de balise contenant des caractères spéciaux ou nécessitant des échappements ou des guillemets doubles, utilisez le joker `?` pour faire correspondre un seul caractère spécial ou un espace. Par exemple, pour rechercher un attribut `my_attribute` avec la valeur `hello world` : `@my_attribute:hello?world`.
<p> </p>

## Valeurs numériques {#numerical-values}

Pour rechercher sur un attribut numérique, commencez par l’ajouter en tant que facette. Vous pouvez ensuite utiliser des opérateurs numériques (`<`,`>`, `<=` ou `>=`) pour effectuer une recherche sur des facettes numériques.
...
Vos journaux héritent des balises des [hôtes][3] et des [intégrations][4] qui les génèrent. Elles peuvent être utilisées dans la recherche et comme facettes également :
