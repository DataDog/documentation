...
Dans les [paramètres de configuration des journaux][1], vous pouvez configurer des processeurs tels que le [parseur Grok](#grokparser) ou le [remappeur de date](#remapper) pour aider à extraire, créer et remapper des attributs afin d'enrichir vos journaux et d'améliorer la recherche facettée.
...
Créez des règles grok personnalisées pour analyser le message complet ou un attribut spécifique de votre événement brut. En tant que meilleure pratique, limitez votre parseur grok à 10 règles d'analyse. Pour plus d'informations sur la syntaxe Grok et les règles d'analyse, voir [Analyse][10].
...
Définissez le processeur Grok sur la [**page des pipelines**][1]. Pour configurer les règles d'analyse Grok :
...
1. **Exemples de journaux** : Ajoutez jusqu'à cinq journaux d'exemple (jusqu'à 5000 caractères chacun) pour tester vos règles d'analyse.
1. **Définir des règles d'analyse** : Écrivez vos règles d'analyse dans l'éditeur de règles. Au fur et à mesure que vous définissez des règles, le parseur Grok fournit une assistance syntaxique :
    **Suggestions de correspondance** : Tapez un nom de règle suivi de `%{`. Un menu déroulant apparaît avec les correspondances disponibles (comme `mot`, `entier`, `ip`, `date`). Sélectionnez une correspondance dans la liste pour l'insérer dans votre règle.<br>
     ```
     MyParsingRule %{
     ```
    **Suggestions de filtre** : Lorsque vous ajoutez un filtre avec `:`, un menu déroulant affiche les filtres compatibles pour la correspondance sélectionnée.
1. **Testez vos règles** : Sélectionnez un exemple en cliquant dessus pour déclencher son évaluation par rapport à la règle d'analyse et afficher le résultat en bas de l'écran. Tous les exemples affichent un statut (`correspond` ou `ne correspond pas`), ce qui met en évidence si l'une des règles d'analyse du parseur Grok correspond à l'exemple.

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utilisez le [point de terminaison API du pipeline de journaux Datadog][1] avec la charge utile JSON suivante pour le parseur Grok :
...
Si vos journaux n'ont pas d'horodatage conforme aux formats énumérés ci-dessus, utilisez le processeur grok pour extraire le temps d'époque de l'horodatage vers un nouvel attribut. Le remappeur de date utilise l'attribut nouvellement défini.
...
* Selon la norme ISO 86011:2019, le format de base est `T[hh][mm][ss]` et le format étendu est `T[hh]:[mm]:[ss]`. Les versions antérieures omettaient le T (représentant le temps) dans les deux formats.
...
Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON de remappage de date de journal suivante :
...
## Remappage de statut de journal

Utilisez le processeur de remappage de statut pour attribuer des attributs en tant que statut officiel à vos journaux. Par exemple, ajoutez un niveau de gravité de journal à vos journaux avec le remappage de statut.
...
* Les chaînes commençant par **o** ou **s**, ou correspondant à **OK** ou **Success** (insensible à la casse) correspondent à **OK**
* Tous les autres correspondent à **info (6)**
...
Définissez le processeur de remappage de service de journal sur la [**page des pipelines**][1] :
...
Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON suivante pour le remappage de service de journal :
...
`message` est un attribut clé dans Datadog. Sa valeur est affichée dans la colonne **Contenu** de l'Explorateur de journaux pour fournir un contexte sur le journal. Vous pouvez utiliser la barre de recherche pour trouver un journal par le message de journal.
...
Pour définir des attributs de message, utilisez d'abord le [processeur de construction de chaînes](#stringbuilderprocessor) pour créer un nouvel attribut de chaîne pour chacun des attributs que vous souhaitez utiliser. Ensuite, utilisez le remappeur de message de journal pour remapper les attributs de chaîne comme message.
...
Définissez le processeur de remappage de message de journal sur la [**page des pipelines**][1] :
...
Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON suivante pour le remappeur de message de journal :
...
Le processeur Remapper **ne peut pas être utilisé pour remapper les attributs réservés de Datadog**. 
 ...
 Les attributs suivants nécessitent des processeurs remappeurs dédiés et ne peuvent pas être remappés avec le Remapper générique. Pour remapper l'un des attributs, utilisez le remappeur ou le processeur spécialisé correspondant à la place.
    `message` : Remappeur de message de journal
    `service` : Remappeur de service
    `status` : Remappeur de statut de journal
...
Définissez le processeur remappeur sur la page [**Pipelines**][1]. Par exemple, remappez `user` à `user.firstname`.
...
Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON de remappage suivante :
...
| `source_type`          | Chaîne          | Non      | Définit si les sources proviennent de l'`attribut` ou du `tag` des journaux. Par défaut : `attribut`. |
| `sources`              | Tableau de chaînes | Oui      | Tableau d'attributs ou de tags sources                                         |
| `target`               | Chaîne          | Oui      | Nom de l'attribut ou du tag final vers lequel remapper les sources.                           |
| `target_type`          | Chaîne          | Non      | Définit si la cible est un `attribut` de journal ou un `tag`.  Par défaut : `attribut`.    |
| `target_format`        | Chaîne          | Non      | Définit si la valeur de l'attribut doit être convertie en un autre type. Valeurs possibles : `auto`, `chaîne`, ou `entier`. Par défaut : `auto`. Lorsqu'il est réglé sur `auto`, aucune conversion n'est appliquée.  |
...
Définissez le processeur d'analyse d'URL sur la [**page des pipelines**][1] :
...
| `target`     | Chaîne           | Oui      | Nom de l'attribut parent qui contient tous les détails extraits des `sources`. Par défaut : `http.url_details`. |
...
Le processeur d'analyseur useragent prend un attribut `useragent` et extrait le système d'exploitation, le navigateur, l'appareil et d'autres données utilisateur. Une fois configuré, les attributs suivants sont produits :

{{< img src="logs/processing/processors/useragent_processor.png" alt="Processeur Useragent" style="width:80%;">}}

**Remarque** : Si vos journaux contiennent des useragents encodés (par exemple, des journaux IIS), configurez ce processeur pour **décoder l'URL** avant de l'analyser.

{{< tabs >}}
{{% tab "INTERFACE UTILISATEUR" %}}

Définissez le processeur useragent sur la [**page des pipelines**][1] :

{{< img src="logs/log_configuration/processor/useragent_processor.png" alt="Tuile du processeur Useragent" style="width:80%;" >}}

[1]: https://app.datadoghq.com/logs/pipelines
{{% /tab %}}
{{% tab "API" %}}

Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON suivante pour l'analyseur useragent :
...
| `target`     | Chaîne           | Oui      | Nom de l'attribut parent qui contient tous les détails extraits des `sources`. Par défaut : `http.useragent_details`. |
...
Utilisez le processeur de catégorie pour ajouter un nouvel attribut (sans espaces ni caractères spéciaux dans le nom du nouvel attribut) à un journal correspondant à une requête de recherche fournie. Ensuite, utilisez des catégories pour créer des groupes pour une vue analytique (par exemple, groupes d'URL, groupes de machines, environnements et intervalles de temps de réponse).

**Remarques** :

* La syntaxe de la requête est celle de la barre de recherche [Log Explorer][6]. Cette requête peut être effectuée sur n'importe quel attribut ou tag de journal, qu'il s'agisse d'un facette ou non. Des jokers peuvent également être utilisés dans votre requête.
...
* Une fois définies dans le processeur de catégorie, vous pouvez mapper les catégories à l'état du journal en utilisant le [remappeur d'état de journal](#logstatusremapper).

{{< tabs >}}
{{% tab "INTERFACE UTILISATEUR" %}}

Définissez le processeur de catégorie sur la page [**Pipelines**][1]. Par exemple, pour catégoriser vos journaux d'accès web en fonction de la valeur de plage de code d'état (`"OK" pour un code de réponse entre 200 et 299, "Notice" pour un code de réponse entre 300 et 399, ...`) ajoutez ce processeur :
...
| `name`       | Chaîne          | Non      | Nom du processeur.                                                                                     |
| `is_enabled` | Booléen         | Non      | Si le processeur est activé ou non. Par défaut : `false`                                                      |
| `categories` | Tableau d'objets | Oui      | Tableau de filtres pour faire correspondre ou non un journal et leur `nom` correspondant pour attribuer une valeur personnalisée au journal. |
...
* Lors de l'utilisation de l'opérateur ``, ajoutez des espaces autour de celui-ci car les noms d'attributs comme `starttime` peuvent contenir des tirets. Par exemple, la formule suivante doit inclure des espaces autour de l'opérateur `` : `(endtime  starttime) / 1000`.
...
| `is_replace_missing` | Booléen | Non      | Si `true`, remplace tous les attributs manquants de `expression` par 0, `false` ignore l'opération si un attribut est manquant. Par défaut : `false`. |
...
* Si un attribut ne peut pas être utilisé (objet ou tableau d'objets), il est remplacé par une chaîne vide ou l'ensemble de l'opération est ignoré selon votre sélection.
...
Avec le journal suivant, utilisez le modèle `Request %{http.method} %{http.url} a été répondu avec la réponse %{http.status_code}` pour retourner un résultat. Par exemple :
...
**Remarque** : `http` est un objet et ne peut pas être utilisé dans un bloc (`%{http}` échoue), tandis que `%{http.method}`, `%{http.status_code}` ou `%{http.url}` renvoient la valeur correspondante. Les blocs peuvent être utilisés sur des tableaux de valeurs ou sur un attribut spécifique au sein d'un tableau.

* Par exemple, l'ajout du bloc `%{array_ids}` renvoie :
...
Utilisez le point de terminaison de l'API Datadog Log Pipeline avec la charge utile JSON suivante pour le processeur de construction de chaînes :
...
| `is_replace_missing` | Booléen | Non      | Si `true`, il remplace tous les attributs manquants de `template` par une chaîne vide. Si `false`, il ignore l'opération pour les attributs manquants. Par défaut : `false`. |
...
L'analyseur geoIP prend un attribut d'adresse IP et extrait des informations sur le continent, le pays, la subdivision ou la ville (si disponible) dans le chemin de l'attribut cible.
...
Utilisez le point de terminaison de l'API Datadog Log Pipeline avec la charge utile JSON suivante pour l'analyseur geoIP :
...
| `target`     | Chaîne           | Oui      | Nom de l'attribut parent qui contient tous les détails extraits des `sources`. Par défaut : `network.client.geoip`.  |
...
Utilisez le processeur de recherche pour définir une correspondance entre un attribut de journal et une valeur lisible par l'homme enregistrée dans une [Table de Référence][7] ou la table de correspondance des processeurs.
...
* Vérifie si la valeur de l'attribut source existe dans la table de correspondance.
  ...
  * En option, s'il ne trouve pas la valeur dans la table de correspondance, il crée un attribut cible avec la valeur par défaut définie dans le champ `fallbackValue`. Vous pouvez entrer manuellement une liste de paires `source_key,target_value` ou télécharger un fichier CSV dans l'onglet **Mapping Manuel**.
  ...
  * En option, s'il ne trouve pas la valeur dans la table de correspondance, il crée un attribut cible avec la valeur de la table de référence. Vous pouvez sélectionner une valeur pour une [Table de Référence][101] dans l'onglet **Table de Référence**.

    {{< img src="logs/log_configuration/processor/lookup_processor_reference_table.png" alt="Processeur de recherche"
    style="width:80%;">}}


[101]: /fr/integrations/guide/referencetables/
...
| `target`         | Chaîne           | Oui      | Nom de l'attribut qui contient la valeur correspondante dans la liste de mappage ou le `default_lookup` si non trouvé dans la liste de mappage.                                |
| `lookup_table`   | Tableau de chaînes | Oui      | Table de mappage des valeurs pour l'attribut source et leurs valeurs d'attribut cible associées, formatées comme [ "source_key1,target_value1", "source_key2,target_value2" ]. |
...
2. Utilisez le processeur de remappage de traces pour définir un attribut de journal comme son ID de trace associé.

{{< tabs >}}
{{% tab "INTERFACE UTILISATEUR" %}}

Définissez le processeur de remappage de traces sur la [**page des pipelines**][1]. Entrez le chemin de l'attribut ID de trace dans la tuile du processeur comme suit :
...
Utilisez le [point de terminaison de l'API Datadog Log Pipeline][1] avec la charge utile JSON de remappage de traces suivante :
...
[1]: /fr/api/v1/logspipelines/
{{% /tab %}}
{{< /tabs >}}

**Remarque** : Les identifiants de trace et les identifiants de span ne sont pas affichés dans vos journaux ou attributs de journaux dans l'interface utilisateur.

## Remappeur de span

Il existe deux façons de définir la corrélation entre les spans d'application et les journaux :

1. Suivez la documentation sur [comment injecter un identifiant de span dans les journaux d'application][8]. Les intégrations de journaux gèrent automatiquement toutes les étapes de configuration restantes par défaut.

2. Utilisez le processeur de remappage de span pour définir un attribut de journal comme son identifiant de span associé.

{{< tabs >}}
{{% tab "INTERFACE UTILISATEUR" %}}

Définissez le processeur de remappage de span sur la [**page Pipelines**][1]. Entrez le chemin de l'attribut d'identifiant de span dans le bloc du processeur comme suit :

{{< img src="logs/log_configuration/processor/span_id_remapper.png" alt="Processeur d'identifiant de span" style="width:80%;">}}
...
| `operation.filter`  | Chaîne    | Oui      | Expression pour faire correspondre un élément du tableau. Le premier élément correspondant est sélectionné. |
...
Calculer le nombre d'éléments dans un tableau.
...
Ajouter une valeur d'attribut à la fin d'un attribut de tableau cible dans le journal.
...
| `operation.preserve_source`  | Booléen    | Non      | Indique si l'on doit conserver la source originale après le remappage. Par défaut : `false`.   |

[1]: /fr/api/v1/logspipelines/
{{% /tab %}}
{{< /tabs >}}

## Processeur de décodeur

Le processeur de décodeur traduit les champs de chaînes encodées en binaire en texte (comme Base64 ou Hex/Base16) dans leur représentation originale. Cela permet d'interpréter les données dans leur contexte natif, que ce soit en tant que chaîne UTF8, commande ASCII ou valeur numérique (par exemple, un entier dérivé d'une chaîne hexadécimale). Le processeur de décodeur est particulièrement utile pour analyser des commandes encodées, des journaux de systèmes spécifiques ou des techniques d'évasion utilisées par des acteurs malveillants.
...
**Remarques** :
 ...
 Format hexadécimal : L'entrée hexadécimale peut être décodée en une chaîne (UTF8) ou en un entier.
...
2. Pour `Base16/Hex` : Choisir le format de sortie : `chaîne (UTF8)` ou `entier`.
3. Définir l'attribut cible : Entrer le chemin de l'attribut pour stocker le résultat décodé.

{{< img src="logs/log_configuration/processor/decoder-processor.png" alt="Processeur de décodeur  Ajouter" style="width:80%;" >}}
...
Ajouter le processeur d'intelligence sur les menaces pour évaluer les journaux par rapport à la table en utilisant une clé d'Indicateur de Compromis (IoC) spécifique, comme une adresse IP. Si une correspondance est trouvée, le journal est enrichi avec des attributs d'intelligence sur les menaces (TI) pertinents de la table, ce qui améliore la détection, l'investigation et la réponse.
...
[2]: /fr/agent/logs/advanced_log_collection/?tab=configurationfile#scrubsensitivedatafromyourlogs
[3]: /fr/logs/log_configuration/parsing/?tab=matchers#parsingdates
[4]: https://fr.wikipedia.org/wiki/Syslog#Niveau_de_sévérité
[5]: /fr/logs/log_configuration/attributes_naming_convention/
[6]: /fr/logs/search_syntax/
[7]: /fr/integrations/guide/referencetables/
...
[11]: /fr/securite/cloud_siem/ingestion_et_enrichissement/open_cybersecurity_schema_framework/
[12]: /fr/securite/cloud_siem/ingestion_et_enrichissement/open_cybersecurity_schema_framework/ocsf_processor/