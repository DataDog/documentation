...
{{< learning-center-callout header="Essayez le parsing Grok dans le Centre d'apprentissage :" btn_title="Inscrivez-vous maintenant" btn_url="https://learn.datadoghq.com/courses/log-pipelines">}}
  Apprenez à construire et à modifier des pipelines de journaux, à les gérer avec le Pipeline Scanner, et à standardiser les noms d'attributs dans les journaux traités pour garantir la cohérence.
...
La syntaxe Grok offre un moyen plus simple d'analyser les journaux que les expressions régulières pures. Le Parser Grok vous permet d'extraire des attributs de messages texte semi-structurés.
...
* **Correspondant** : Une règle (éventuellement une référence à une autre règle de token) qui décrit ce à quoi s'attendre (nombre, mot, nonEspace, etc.).
...
* Vous devez avoir des noms de règles uniques dans le même parser Grok.
...
L'ensemble complet suivant de correspondants et de filtres est spécifique à <em>l'ingestion-temps</em> <a href="/logs/log_configuration/processors/grok_parser/">fonctionnalité du Grok Parser</a>.</div>
...
**Correspondants de temps de requête et d'ingestion-temps :**

Les correspondants suivants sont disponibles pour le parsing de temps de requête (Log Explorer) et le parsing d'ingestion-temps (Grok Parser) :
...
`number`
: Correspond à un nombre à virgule flottante décimal et l'analyse comme un nombre à double précision.
...
**Correspondants uniquement pour l'ingestion-temps :**

Les correspondants suivants ne sont disponibles que pour l'analyse au moment de l'ingestion avec le processeur Grok Parser et ne peuvent pas être utilisés dans l'Explorateur de journaux :

`date("pattern"[, "timezoneId"[, "localeId"]])`
: Correspond à une date avec le modèle spécifié et analyse pour produire un horodatage Unix. [Voir les exemples de correspondants de date](#parsing-dates).

`regex("pattern")`
: Correspond à une expression régulière. [Vérifiez les exemples de correspondants d'expressions régulières](#regex).
...
Les filtres suivants ne sont disponibles que pour le parsing au moment de l'ingestion avec le Grok Parser et ne peuvent pas être utilisés dans l'Explorateur de journaux :
...
: Analyse un agent utilisateur et renvoie un objet JSON contenant l'appareil, le système d'exploitation et le navigateur représenté par l'Agent. [Vérifiez le processeur d'agent utilisateur][1].
...
: Analyse une séquence de chaînes de tokens et la renvoie sous forme de tableau. Voir l'exemple [liste à tableau](#list-to-array).
...
Utilisez la section **Paramètres avancés** en bas de votre Grok Processor pour analyser un attribut spécifique au lieu de l'attribut par défaut `message`, ou pour définir des règles d'aide qui réutilisent des modèles communs à travers plusieurs règles d'analyse.
...
Par exemple, considérez un journal contenant un attribut `command.line` qui doit être analysé comme une clé-valeur. Extraire de `command.line` pour analyser son contenu et créer des attributs structurés à partir des données de commande.

{{< img src="/logs/processing/parsing/grok_advanced_settings_extract.png" alt="Paramètres avancés avec exemple d'attribut command.line 'Extract from'" style="width:80%;">}}

### Utilisation de règles d'aide pour réutiliser des modèles communs {#using-helper-rules-to-reuse-common-patterns}

Utilisez le champ **Règles d'aide** pour définir des tokens pour vos règles d'analyse. Les règles d'aide vous permettent de réutiliser des modèles Grok communs dans vos règles d'analyse. Ceci est utile lorsque vous avez plusieurs règles dans le même parseur Grok qui utilisent les mêmes jetons.
...
Utilisez des filtres tels que **keyvalue** pour associer plus facilement des chaînes aux attributs pour les formats keyvalue ou logfmt :
...
Si vous ajoutez un attribut **extract** dans votre modèle de règle, vous verrez :
...
* Si vous définissez un filtre *clévaleur* sur un objet `data`, et que ce filtre ne trouve pas de correspondance, alors un JSON vide `{}` est retourné (par exemple, entrée : `key:=valueStr`, règle d'analyse : `rule_test %{data::keyvalue("=")}`, sortie : `{}`).
* Définir `""` en tant que `quotingStr` conserve la configuration par défaut des guillemets.

### Analyse des dates {#parsing-dates}

Le date matcher transforme votre horodatage en format EPOCH (unité de mesure **milliseconde**).
...
* Si le XML contient des balises ayant à la fois un attribut et une valeur de chaîne entre les deux balises, un `value` attribut est généré. Par exemple: `<title lang="en">Harry Potter</title>` est converti en `{"title": {"lang": "en", "value": "Harry Potter" } }`
...
* Les valeurs citées contenant un caractère de citation doivent être échappées avec des caractères de citation. Par exemple, `""` à l'intérieur d'une valeur citée représente `"`.
* Si le journal ne contient pas autant de valeurs que de clés dans l'en-tête, le parseur CSV associera les valeurs aux premières clés.
...
Si vous avez un journal où, après avoir analysé ce qui est nécessaire et sachant que le texte après ce point peut être éliminé, vous pouvez utiliser le data matcher pour le faire. Pour l'exemple de journal suivant, vous pouvez utiliser le `data` matcher pour écarter le `%` à la fin.
