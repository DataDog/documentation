...
Pour collecter tous les événements, vous devez installer via CDN synchrone. Le SDK du navigateur se charge depuis le CDN de Datadog de manière synchrone, garantissant que le SDK se charge en premier et collecte toutes les erreurs, ressources et actions des utilisateurs. Cette méthode peut affecter les performances de chargement de la page.  
...
**Remarque** : Les appels API initiaux doivent être enveloppés dans le rappel `window.DD_LOGS.onReady()`. Cela garantit que le code n'est exécuté qu'une fois que le SDK est correctement chargé.
...
Si vos journaux de navigateur contiennent des informations sensibles devant être éliminées, configurez le SDK des journaux du navigateur pour éliminer les séquences sensibles en utilisant le rappel `beforeSend` lors de l'initialisation du collecteur de journaux du navigateur.
...
|-------|---------|------------|
| `isAborted` | Booléen | Pour les événements de journal réseau, cette propriété indique si la requête échouée a été annulée par l'application, auquel cas vous pourriez ne pas vouloir envoyer cet événement car il peut avoir été intentionnellement annulé. |
...
Après la création d'un enregistreur, il est possible de :
...
Après l'initialisation du SDK des journaux du navigateur Datadog, le niveau de journalisation minimal pour votre enregistreur est défini avec l'API :
...
**Remarque** : Les premiers appels API doivent être enveloppés dans le callback `window.DD_LOGS.onReady()`. Cela garantit que le code n'est exécuté qu'une fois que le SDK est correctement chargé.
...
Le `trackingConsent` paramètre d'initialisation peut être l'une des valeurs suivantes :
...
- lorsqu'il est changé de `"not-granted"` à `"granted"`, une nouvelle session des journaux est créée si aucune session précédente n'est active, et la collecte de données reprend.
...
Vous pouvez optionnellement utiliser `startTime` le paramètre pour obtenir le contexte d'un moment spécifique. Si le paramètre est omis, le contexte actuel est renvoyé.
