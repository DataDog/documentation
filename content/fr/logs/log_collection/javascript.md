...
**Environnements pris en charge** : Le SDK des journaux du navigateur prend en charge tous les navigateurs de bureau et mobiles modernes, ainsi que les environnements Worker et Service Worker. Voir le tableau [Support des navigateurs][4].

<div class="alert alert-info">Pour des raisons de sécurité, <a href="https://docs.datadoghq.com/account_management/api-app-keys/#api-keys">les clés API</a> ne peuvent pas être utilisées pour configurer le SDK des journaux du navigateur, car elles seraient exposées côté client dans le code JavaScript. Pour collecter des journaux à partir de navigateurs web, un <a href="https://docs.datadoghq.com/account_management/api-app-keys/#client-tokens">jeton client</a> doit être utilisé.</div>  

### Étape 2  Installez le SDK des journaux du navigateur
...
Les applications Web avec des objectifs de performance doivent être installées via CDN de manière asynchrone. Le SDK du navigateur se charge depuis le CDN de Datadog de manière asynchrone, garantissant qu'il n'impacte pas la performance de chargement de la page. Cependant, le SDK peut ne pas capturer les erreurs ou les journaux de console qui se produisent avant que le SDK ne soit initialisé.  
...
Pour collecter tous les événements, vous devez installer via CDN synchronisé. Le SDK du navigateur se charge depuis le CDN de Datadog de manière synchrone, garantissant que le SDK se charge en premier et collecte toutes les erreurs, ressources et actions des utilisateurs. Cette méthode peut impacter la performance de chargement de la page.  

Ajoutez le code généré dans la balise head (avant toute autre balise script) de chaque page HTML que vous souhaitez surveiller dans votre application. Placer la balise script plus haut et la charger de manière synchrone garantit que Datadog RUM peut collecter toutes les données de performance et les erreurs.
...
Pour être conforme au RGPD, CCPA et à des réglementations similaires, le SDK RUM Browser vous permet de fournir la [valeur de consentement au suivi lors de l'initialisation][5].
...
**Remarque** : La vérification `window.DD_LOGS` empêche les problèmes lors d'un échec de chargement du SDK.
...
Le SDK de journaux du navigateur Datadog permet le suivi manuel des erreurs en utilisant le paramètre optionnel `error` (Disponible dans le SDK v4.36.0+). Lorsqu'une instance d'une [Erreur JavaScript][8] est fournie, le SDK extrait les informations pertinentes (type, message, trace de pile) de l'erreur.
...
Le SDK de journaux du navigateur Datadog ajoute des fonctions abrégées (`.debug`, `.info`, `.warn`, `.error`) aux journaux pour plus de commodité. Une fonction de journalisation générique est également disponible, exposant le paramètre `status` :
...
### Masquer les données sensibles de vos journaux de navigateur

Si vos journaux de navigateur contiennent des informations sensibles qui doivent être masquées, configurez le SDK de navigateur pour masquer les séquences sensibles en utilisant le rappel `beforeSend` lors de l'initialisation du Collecteur de journaux de navigateur.

La fonction de rappel `beforeSend` peut être invoquée avec deux arguments : l'événement `log` et `context`. Cette fonction vous donne accès à chaque journal collecté par le SDK de navigateur avant qu'il ne soit envoyé à Datadog, et vous permet d'utiliser le contexte pour ajuster les propriétés de tout journal. Le contexte contient des informations supplémentaires liées à l'événement, mais pas nécessairement incluses dans l'événement. Vous pouvez généralement utiliser ces informations pour [enrichir][11] votre événement ou [le rejeter][12].
...
| `handlingStack` | Chaîne | Une trace de pile de l'endroit où l'événement de journal a été traité. Cela peut être utilisé pour identifier de quel [microfrontend][9] le journal a été envoyé. |
...
### Définir plusieurs journaux

Le SDK de journaux de navigateur Datadog contient un journal par défaut, mais il est possible de définir différents journaux.

#### Créer un nouveau journal

Après l'initialisation du SDK de journaux de navigateur Datadog, utilisez l'API `createLogger` pour définir un nouveau journal :
...
#### Obtenir un journal personnalisé

Après la création d'un journal, accédez-y dans n'importe quelle partie de votre code JavaScript avec l'API :
...
Par exemple, supposons qu'il y ait un `signupLogger`, défini avec tous les autres journaux :
...
Après l'initialisation du SDK de journaux de navigateur Datadog, il est possible de :

- Définir l'ensemble du contexte pour tous vos journaux avec l'API `setGlobalContext (context: object)`.
- Ajouter un contexte à tous vos journaux avec l'API `setGlobalContextProperty (key: string, value: any)`.
...
> Le SDK de Log Browser v4.17.0 a mis à jour les noms de plusieurs API :
>
...
Le SDK de journaux Datadog fournit des fonctions pratiques pour associer un `User` aux journaux générés.

- Définir l'utilisateur pour tous vos journaux avec l'API `setUser (newUser: User)`.
- Ajouter ou modifier une propriété utilisateur à tous vos journaux avec l'API `setUserProperty (key: string, value: any)`.
- Obtenir l'utilisateur actuellement stocké avec l'API `getUser ()`.
- Supprimer une propriété utilisateur avec l'API `removeUserProperty (key: string)`.
- Effacer toutes les propriétés utilisateur existantes avec l'API `clearUser ()`.
...
Avec l'introduction de l'option de configuration `storeContextsAcrossPages` dans la version v4.49.0 du SDK navigateur, ces contextes peuvent être stockés dans [`localStorage`][9], permettant les comportements suivants :
...
- Définir des informations personnellement identifiables (PII) dans ces contextes n'est pas recommandé, car les données stockées dans `localStorage` survivent à la session utilisateur
...
- `localStorage` est limitée à 5 MiB par origine, donc les données spécifiques à l'application, les contextes Datadog et d'autres données tierces stockées dans `localStorage` doivent être dans cette limite pour éviter tout problème

#### Contexte de l'enregistreur

Après qu'un enregistreur soit créé, il est possible de :

- Définir l'ensemble du contexte pour votre enregistreur avec l'API `setContext (context: object)`.
- Définir une propriété de contexte sur votre enregistreur avec l'API `setContextProperty (key: string, value: any)` :
...
Après l'initialisation du SDK des journaux du navigateur Datadog, le niveau de journal minimal pour votre enregistreur est défini avec l'API :
...
- envoyer des journaux à `console` et Datadog (`http`)
- envoyer des journaux uniquement à `console`
...
- lorsqu'il est changé de `"granted"` à `"not-granted"`, la session des journaux est arrêtée, et les données ne sont plus envoyées à Datadog.
