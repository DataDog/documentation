...
{{< img src="logs/processing/processors/log_pre_severity.png" alt=" Log avant sévérité " responsive="true" style="width:40%;">}}

En ce log :

{{< img src="logs/processing/processors/log_post_severity_bis.png" alt=" Log après sévérité bis" responsive="true" style="width:40%;" >}}

Cependant, sachez que chaque valeur de statut entrant est mappée comme suit :
...
{{< img src="logs/processing/processors/message_processor.png" alt="Processeur de message" responsive="true" style="width:80%;">}}
...
Les contraintes de nom du tag ou de l'attribut sont expliquées dans la [documentation sur les meilleures pratiques concernant les tags][4]. Certaines contraintes supplémentaires sont appliquées, car les caractères `:` ou `,` ne sont pas autorisés dans le nom du tag ou de l'attribut cible.
...
Le parser user-agent reçoit un attribut user-agent et tente d'extraire le système d'exploitation, le navigateur, l'appareil, etc.
...
Les catégories sont très utiles pour créer des groupes utiles et faciliter l'analyse des informations (exemples : groupes d'URL, groupes de machines, environnements, compartiments de temps de réponse, etc.).
...
**Remarque importante** : la requête peut être effectuée sur n'importe quel attribut de log ou tag, qu'il s'agisse d'une facette ou non. Les wildcards peuvent également être utilisés dans votre requête.
Une fois que le log a été mis en correspondance avec l'une des requêtes du processeur, il s'arrête. Assurez-vous de spécifier les requêtes dans l'ordre adéquat si un log correspond à plusieurs requêtes.
...
* L'opérateur `-` doit être séparé par une espace dans la formule, car il peut également être contenu dans les noms d'attributs.
...
Il y a deux façons d'améliorer la corrélation entre les traces et les logs d'application :
...
2. Utilisez le processeur de remappage de trace pour définir un attribut de log comme étant son ID de trace associé en entrant le chemin d'attribut dans le carré du processeur comme suit :
