...
Des intégrations comme [Apache][1], Rails, [AWS CloudFront][7] ou encore des serveurs d'application Web reposent sur ces attributs.
...
Ces attributs fournissent des informations sur les éléments parsés de l'URL HTTP. Ils sont généralement générés à l'aide du [parser d'URL][8]. Tous les attributs sont précédés par `http.url_details`.
