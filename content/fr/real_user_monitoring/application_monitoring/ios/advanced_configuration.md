...
description: Configurez les paramètres avancés du SDK RUM iOS pour enrichir les sessions
  ...
  afin d'obtenir des insights plus pertinents.
further_reading:
...
En plus de [suivre automatiquement les actions](#automatically-track-user-actions), vous pouvez également suivre des actions utilisateur personnalisées (taps, clics et défilements) via l'API `addAction(type:name:)`.
...
Ajouter des informations sur les utilisateurs à vos sessions RUM facilite :
...
Pour identifier les sessions des utilisateurs, utilisez l'API `Datadog.setUserInfo(id:name:email:)`.
...
`appHangThreshold`
 : Définit le seuil de signalement lorsque une application se bloque. La valeur minimale autorisée pour cette option est de `0.1` secondes. Pour désactiver le signalement, définissez cette valeur à `nil`. Pour plus d'informations, voir [Ajouter le signalement des blocages d'application][8].
...
`uiKitViewsPredicate`
Active le suivi de `UIViewControllers` en tant que vues RUM. Vous pouvez utiliser l'implémentation par défaut de `predicate` en définissant le `DefaultUIKitRUMViewsPredicate` ou implémenter [ votre propre `UIKitRUMViewsPredicate`](#automatically-track-views) personnalisé pour votre application.
...
- L'implémentation diffère entre iOS 18+ et iOS 17 et inférieur :
  ...
  - **iOS 17 et inférieur :** Le SDK ne peut pas distinguer entre les composants interactifs et non interactifs (par exemple, Bouton vs. Étiquette). Pour cette raison, les actions sont rapportées comme `SwiftUI_Unidentified_Element`.
...
- Les données de réponse sont disponibles dans le `resourceAttributesProvider` rappel (défini dans `RUM.Configuration.URLSessionTracking`) pour les tâches avec des gestionnaires de complétion en mode automatique, et pour toutes les tâches après avoir activé `URLSessionInstrumentation`.
...
1. `.pending` : Le SDK RUM iOS commence à collecter et à regrouper les données mais ne les envoie pas à Datadog. Le SDK RUM iOS attend la nouvelle valeur de consentement au suivi pour décider quoi faire avec les données regroupées.
2. `.granted` : Le SDK RUM iOS commence à collecter les données et les envoie à Datadog.
Le SDK RUM iOS ne collecte aucune donnée. Aucun journal, trace ou événement RUM n'est envoyé à Datadog.
