...
1. Définissez `trackViewsManually` sur `true` lors de l'initialisation du SDK navigateur RUM comme décrit [ci-dessus](#override-default-rum-view-names).
...
Utilisez `setViewName(name: string)` pour mettre à jour le nom de la vue actuelle. Cela vous permet de modifier le nom de la vue pendant qu'elle est active, sans démarrer une nouvelle.
   <!-- NPM -->
   {% if equals($lib_src, "npm") %}
   ```javascript
   import { datadogRum } from '@datadog/browser-rum';
...
Le SDK navigateur RUM capture les événements RUM et remplit leurs principaux attributs. La fonction de rappel `beforeSend` vous donne accès à chaque événement collecté par le SDK RUM Browser avant qu'il ne soit envoyé à Datadog.
...
| Ressource (Fetch) | [Demande][10], [Réponse][11], [PerformanceResourceTiming][9], et pile de gestion      |
...
### Enrichissez les événements RUM avec des feature flags {% #enrich-rum-events-with-feature-flags %}

Vous pouvez [enrichir vos données d'événements RUM avec des feature flags][14] pour obtenir un contexte et une visibilité supplémentaires sur la surveillance des performances. Cela vous permet de déterminer quels utilisateurs se voient proposer une expérience utilisateur spécifique et si cela affecte négativement les performances des utilisateurs.
...
| `usr.name`  | Chaîne | Non | Nom convivial de l'utilisateur, affiché par défaut dans le RUM UI.                                                  |
| `usr.email` | Chaîne | Non | Email de l'utilisateur, affiché dans le RUM UI si le nom de l'utilisateur n'est pas présent. Il est également utilisé pour récupérer des Gravatars. |
{% /if %}
<!-- ends  6.4.0 -->
...
| `usr.name`  | Chaîne | Nom convivial de l'utilisateur, affiché par défaut dans le RUM UI.                                                  |
| `usr.email` | Chaîne | Email de l'utilisateur, affiché dans le RUM UI si le nom de l'utilisateur n'est pas présent. Il est également utilisé pour récupérer des Gravatars. |

**Remarque** : 'Utilisateur public' est affiché dans le RUM UI lorsque `usr.name` n'est pas défini, même si `usr.email` et `usr.id` sont définis.
...
| `account.name` | Chaîne | Non       | Nom convivial du compte, affiché par défaut dans le RUM UI. |
...
1. `"granted"` (default) : Le RUM Browser SDK commence à collecter des données et les envoie à Datadog.
2. `"not-granted"` : Le RUM Browser SDK ne collecte aucune donnée.
...
À partir de [version 5.28.0][20], le contexte des événements de vue est modifiable. Le contexte peut être ajouté à la vue actuelle uniquement, et remplit ses événements enfants (tels que `action`, `error` et `timing`) avec les fonctions `startView`, `setViewContext` et `setViewContextProperty`.
...
| user_action    | Objet contenant l'ID d'action (ou indéfini si aucune action n'est trouvée). |
| view           | Objet contenant des détails sur l'événement view actuel.           |
...
**Étape 1 - Configurez le [plugin de build][23] pour chaque micro frontend**

Dans la configuration de build de chaque micro frontend, activez l'injection du contexte du code source :
...
[Configurez la surveillance du navigateur][4] dans votre application shell (point d'entrée principal). Le Browser SDK enrichit automatiquement les événements RUM (erreurs, actions personnalisées, ressources XHR/Fetch, tâches longues, indicateurs) avec `service` et `version` provenant de la carte de contexte.

{% alert level="warning" %}
Les événements qui ne correspondent à aucun micro frontend basculent vers le service et la version du shell.
{% /alert %}
...
Lorsqu'une trace de pile contient des frames provenant de plusieurs micro frontends, l'événement reçoit un unique `service` et `version` à partir du frame le plus élevé (là où l'erreur a été déclenchée). Les cartes sources sont résolues pour l'événement sous ce service unique, donc les frames d'autres micro frontends restent minifiées, même lorsque leurs cartes sources ont été correctement téléchargées sous leur propre `service`.

Pour contrôler quelles cartes sources de micro frontend sont utilisées, utilisez l'approche [attribution manuelle](#manual-service-and-version-attribution) avec `beforeSend` pour définir `event.service` et `event.version`. Seules les frames appartenant au micro frontend choisi sont déminifiées.
...
Les `service` et `version` étiquettes représentant chaque micro frontend peuvent également être trouvées dans les métriques suivantes [RUM without Limits][24] :
