...
title: Premiers pas avec les données de drapeaux de fonctionnalités dans RUM
---
## Aperçu
Les données des drapeaux de fonctionnalités vous offrent une visibilité accrue sur l'expérience utilisateur et le suivi des performances, en vous permettant de déterminer quels utilisateurs voient une fonctionnalité spécifique et si toute modification que vous apportez affecte leur expérience ou nuit aux performances.

En enrichissant vos données RUM avec celles des drapeaux de fonctionnalités, vous pouvez être certain que votre fonctionnalité se déploie avec succès sans provoquer, involontairement, un bug ou une régression de performance. Avec ce niveau supplémentaire d'information, vous pouvez faire le lien entre les mises en production des fonctionnalités et les performances, identifier les problèmes liés à des déploiements spécifiques et accélérer le dépannage.
...
Le suivi des drapeaux de fonctionnalités est disponible dans le SDK RUM Android. Pour commencer, configurez [la surveillance RUM Android][1]. Vous avez besoin de la version >= 1.18.0 du SDK RUM Android.
...
Initialisez le SDK d'Amplitude et créez un écouteur d'exposition qui envoie les évaluations des drapeaux de fonctionnalités à Datadog en utilisant l'extrait de code suivant :
...
Initialisez le SDK d'Amplitude et créez un inspecteur qui rapporte les évaluations des drapeaux de fonctionnalités à Datadog en utilisant l'extrait de code ci-dessous.
...
Chaque fois qu'un drapeau de fonctionnalités est évalué, ajoutez la fonction suivante pour transmettre les informations correspondantes à RUM :
...
Initialisez le SDK d'Eppo et créez un journal d'attribution qui rapporte également les évaluations des drapeaux de fonctionnalités à Datadog en utilisant l'extrait de code présenté ci-dessous.
...
Initialisez le SDK de Flagsmith avec l'option `datadogRum`, qui rapporte les évaluations des drapeaux de fonctionnalités à Datadog en utilisant l'extrait de code présenté ci-dessous.
...
Lors de l'initialisation du SDK de GrowthBook, transmettez les évaluations des feature flags à Datadog en utilisant le callback `onFeatureUsage`.
...
Lors de l'initialisation du SDK de GrowthBook, transmettez les évaluations des feature flags à Datadog en appelant `setFeatureUsageCallback`.
...
En savoir plus sur l'initialisation du SDK dans la [documentation du SDK Kameleoon React Native][1].
...
Initialisez le SDK de LaunchDarkly et créez un inspecteur qui transmet les évaluations des feature flags à Datadog en utilisant l'extrait de code ci-dessous.
...
Initialisez le SDK de Split et créez un écouteur d'impression qui transmet les évaluations des feature flags à Datadog en utilisant l'extrait de code suivant :
...
Initialisez le SDK de Split et créez un inspecteur qui transmet les évaluations des feature flags à Datadog en utilisant l'extrait de code ci-dessous.
...
1. Mettez à jour votre version du SDK RUM pour navigateur à 4.25.0 ou supérieure.
...
Les feature flags apparaissent dans le contexte de vos RUM Sessions, vues et erreurs sous forme de liste.
...
### Recherchez des feature flags en utilisant le RUM Explorer.
Parcourez toutes les données collectées par RUM dans le [RUM Explorer][2] pour faire ressortir les tendances sur les feature flags, analyser les modèles avec un contexte plus large, ou les exporter dans des [tableaux de bord][3] et des [moniteurs][4]. Vous pouvez rechercher vos sessions, vues ou erreurs dans RUM Explorer, avec l'attribut `@feature_flags.{flag_name}`.
...
{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-session-feature-flag-search.png" alt="Recherchez des sessions pour les feature flags dans le RUM Explorer." style="width:75%;">}}
...
{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-view-feature-flag-search.png" alt="Recherchez des vues pour les feature flags dans le RUM Explorer." style="width:75%;">}}

#### Erreurs
En filtrant vos **Erreurs** avec l'attribut `@feature_flags.{flag_name}`, vous pouvez trouver toutes les erreurs survenues dans la période donnée sur la vue où votre feature flag a été évalué.

{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/rum-explorer-error-feature-flag-search.png" alt="Recherchez des erreurs pour les feature flags dans le RUM Explorer." style="width:75%;">}}

## Dépannage

### Mes données relatives à mon feature flag ne correspondent pas à mes attentes.
Les feature flags apparaissent dans le contexte des événements où ils sont évalués, ce qui signifie qu'ils devraient figurer sur les vues où le code des feature flags s'exécute.

Selon la structure de votre code et la configuration de vos feature flags, vous pourriez observer l'apparition inattendue de feature flags dans le contexte de certains événements.
...
{{< img src="real_user_monitoring/guide/setup-feature-flag-data-collection/feature_flag_view_query.png" alt="Recherchez des vues pour les feature flags dans le RUM Explorer." style="width:75%;">}}

Voici quelques exemples de raisons pour lesquelles votre feature flag est évalué sur des vues non liées pouvant vous aider dans vos investigations.

- Un composant React courant qui apparaît sur plusieurs pages et qui évalue les feature flags à chaque exécution.
- Un problème de routage où des composants avec une évaluation de feature flag sont affichés avant ou après un changement d'URL.

Lors de vos investigations, vous pouvez également restreindre vos données aux `View Name` pertinents pour votre feature flag.
