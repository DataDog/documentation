...
Le suivi des indicateurs de fonctionnalité est disponible pour vos applications Flutter. Pour commencer, configurez [la surveillance RUM Flutter][1]. Vous avez besoin de la version >= 1.3.2 du plugin Flutter.

[1]: /fr/real_user_monitoring/application_monitoring/flutter/setup/
{{% /tab %}}
{{% tab "iOS" %}}

Le suivi des indicateurs de fonctionnalité est disponible dans le SDK RUM iOS. Pour commencer, configurez [la surveillance RUM iOS][1]. Vous avez besoin de la version >= 1.16.0 du SDK RUM iOS.

[1]: /fr/real_user_monitoring/application_monitoring/ios/setup
{{% /tab %}}
{{% tab "Kotlin Multiplatform" %}}

Le suivi des indicateurs de fonctionnalité est disponible pour vos applications Kotlin Multiplatform. Pour commencer, configurez [la surveillance RUM Kotlin Multiplatform][1].

[1]: /fr/real_user_monitoring/application_monitoring/kotlin_multiplatform
{{% /tab %}}
{{% tab "React Native" %}}

Le suivi des indicateurs de fonctionnalité est disponible pour vos applications React Native. Pour commencer, configurez [la surveillance RUM React Native][1]. Vous avez besoin de la version >= 1.7.0 du SDK RUM de React Native.

[1]: /fr/real_user_monitoring/application_monitoring/react_native/setup
{{% /tab %}}
{{% tab "Unité" %}}

Le suivi des fonctionnalités est disponible pour vos applications Unity. Pour commencer, configurez [la surveillance RUM Unity][1].
...
Vous pouvez commencer à collecter des données sur les fonctionnalités avec [des solutions de gestion de fonctionnalités personnalisées](#customfeatureflagmanagement), ou en utilisant l'un des partenaires d'intégration de Datadog.
...
Initialisez le SDK d'Amplitude et créez un écouteur d'exposition signalant les évaluations des fonctionnalités à Datadog en utilisant le snippet de code suivant :
...
Initialisez le SDK d'Amplitude et créez un inspecteur signalant les évaluations des fonctionnalités à Datadog en utilisant le snippet de code ci-dessous.
...
Lors de l'initialisation du SDK Javascript ConfigCat, abonnez-vous à l'événement `flagEvaluated` et signalez les évaluations des drapeaux de fonctionnalité à Datadog :
...
Pour plus d'informations sur l'initialisation du SDK Javascript ConfigCat, consultez la [documentation du SDK JavaScript de ConfigCat][1].
...
Lors de l'initialisation du SDK Swift iOS de ConfigCat, abonnez-vous à l'événement `flagEvaluated` et signalez les évaluations des drapeaux de fonctionnalité à Datadog :
...
Lors de l'initialisation du SDK Android de ConfigCat, abonnez-vous à l'événement `flagEvaluated` et signalez les évaluations des drapeaux de fonctionnalité à Datadog :
...
Lors de l'initialisation du SDK Dart de ConfigCat, abonnez-vous à l'événement `flagEvaluated` et signalez les évaluations des drapeaux de fonctionnalité à Datadog :
...
Chaque fois qu'un drapeau de fonctionnalité est évalué, ajoutez la fonction suivante pour envoyer les informations du drapeau de fonctionnalité à RUM :
...
Initialisez le SDK de DevCycle et abonnez-vous à l'événement `variableEvaluated`, en choisissant de vous abonner à toutes les évaluations de variables `variableEvaluated:*` ou à des évaluations de variables particulières `variableEvaluated:myvariablekey`.
...
Initialisez le SDK d'Eppo et créez un journal d'attribution qui rapporte également les évaluations des drapeaux de fonctionnalités à Datadog en utilisant le code ci-dessous.
...
Initialisez le SDK de Flagsmith avec l'option `datadogRum`, qui rapporte les évaluations des drapeaux de fonctionnalités à Datadog en utilisant le code ci-dessous.
   ...
   En option, vous pouvez configurer le client pour que les traits de Flagsmith soient envoyés à Datadog via `datadogRum.setUser()`. Pour plus d'informations sur l'initialisation du SDK de Flagsmith, consultez la [documentation du SDK JavaScript de Flagsmith][1].
...
Lors de l'initialisation du SDK GrowthBook, signalez les évaluations des drapeaux de fonctionnalité à Datadog en appelant `setFeatureUsageCallback`.
...
Initialisez le SDK de LaunchDarkly et créez un inspecteur signalant les évaluations des drapeaux de fonctionnalité à Datadog en utilisant le code ci-dessous.
...
Initialisez le SDK de Split et créez un écouteur d'impression signalant les évaluations des drapeaux de fonctionnalité à Datadog en utilisant le code suivant :
...
Initialisez le SDK de Split et créez un inspecteur signalant les évaluations des drapeaux de fonctionnalité à Datadog en utilisant le code ci-dessous.
...
## Analysez la performance de vos Feature Flags dans RUM
...
#### Erreurs
En filtrant vos **Erreurs** avec l'attribut `@feature_flags.{flag_name}`, vous pouvez trouver toutes les erreurs survenues dans la période donnée sur la Vue où votre drapeau de fonctionnalité a été évalué
...
### Mes données de drapeau de fonctionnalité ne reflètent pas ce que j'attends
Les drapeaux de fonctionnalité apparaissent dans le contexte des événements où ils sont évalués, ce qui signifie qu'ils devraient apparaître sur les vues sur lesquelles la logique de code du drapeau de fonctionnalité est exécutée.

Selon la façon dont vous avez structuré votre code et configuré vos drapeaux de fonctionnalité, vous pouvez voir des drapeaux de fonctionnalité inattendus apparaître dans le contexte de certains événements.

Par exemple, pour voir sur quelles **Vues** votre drapeau de fonctionnalité est évalué, vous pouvez utiliser l'Explorateur RUM pour faire une requête similaire :
...
Lorsque vous effectuez vos investigations, vous pouvez également limiter vos données aux `Noms de Vue` qui sont pertinents pour votre drapeau de fonctionnalité.
