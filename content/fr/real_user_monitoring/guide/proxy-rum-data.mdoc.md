...
description: Configurez le proxy de données RUM du navigateur avec les options de
  source SDK et les paramètres spécifiques à la version pour un routage réseau personnalisé.
further_reading:
...
* chemin
* 
    Le chemin contient la version de l'API et le produit (par exemple, `/api/v2/rum` pour les données RUM ou `/api/v2/replay` pour les données de Session Replay). 
    
    Le chemin pour chaque requête peut être consulté dans le paramètre `ddforward` de la requête (par exemple, `https://www.example-proxy.com/any-endpoint?ddforward=%2Fapi%2Fv2%2Frum%3Fddsource%3Dbrowser`).
---
* paramètres
* 
    Les paramètres de la requête (par exemple, `ddsource=browser&...`) peuvent être consultés dans le paramètre `ddforward` de la requête (par exemple, `https://www.example-proxy.com/any-endpoint?ddforward=%2Fapi%2Fv2%2Frum%3Fddsource%3Dbrowser`).
...
Le SDK RUM Browser ajoute un paramètre de requête `ddforward` à toutes les requêtes vers votre proxy. Ce paramètre de requête contient le chemin d'URL ainsi que les paramètres vers lesquels toutes les données doivent être acheminées.

Par exemple, avec un `site` défini sur `datadoghq.eu` et un `proxy` défini sur `https://example.org/datadog-intake-proxy`, le SDK RUM Browser envoie des requêtes à une URL comme celle-ci : `https://example.org/datadog-intake-proxy?ddforward=%2Fapi%2Fv2%2Frum%3Fddsource%3Dbrowser`. Le proxy transmet la demande à `https://browser-intake-datadoghq.eu/api/v2/rum?ddsource=browser`.

<!-- SDK version >=5.4.0 -->
{% if equals($rum_browser_sdk_version, "gte_5_4_0") %}
### Passer une fonction au paramètre d'initialisation `proxy`

Le paramètre d'initialisation `proxy` prend également en charge une fonction comme valeur. Cette fonction vous permet d'avoir plus de contrôle sur la manière dont le chemin et les paramètres sont ajoutés à l'URL du proxy.
