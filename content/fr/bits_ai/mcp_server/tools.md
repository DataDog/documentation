...
Consultez [Configurer le serveur Datadog MCP][1] pour plus d'informations sur la connexion au serveur MCP et l'activation des ensembles d'outils.

<div class="alert alert-info">Les outils du serveur Datadog MCP sont en cours de développement et peuvent évoluer. Utilisez <a href="https://docs.google.com/forms/d/e/1FAIpQLSeorvIrML3F4v74Zm5IIaQ_DyCMGqquIp7hXcycnCafx4htcg/viewform">ce formulaire de retour d'information</a> pour partager vos retours, cas d'utilisation ou problèmes rencontrés avec vos invites et requêtes.</div>

## Core tools (Outils principaux){#core-tools}
...
Recherche des événements tels que les alertes de moniteur, les notifications de déploiement, les changements d'infrastructure, les constats de sécurité et les changements de statut de service.
...
**Remarque** : Les grandes traces, comportant des milliers de spans, peuvent être tronquées (et indiquées comme telles) sans possibilité de récupérer l'ensemble des spans.
...
Récupère des informations détaillées sur un carnet de notes spécifique par ID, y compris le nom, le statut et l'auteur.

- Obtenez les détails pour le carnet de notes abc-123-def.
- Montrez-moi le contenu du carnet de débogage d'hier.

### `search_datadog_notebooks` {#search-datadog-notebooks}
*Ensemble d'outils : **noyau***\
*Permissions requises : `Notebooks Read`*\
Liste et recherche des carnets de notes Datadog avec filtrage par auteur, étiquettes et contenu.

- Montrez-moi tous les carnets de notes créés par l'équipe plateforme.
- Trouvez des carnets de notes liés à l'enquête sur les performances.
- Liste des carnets de notes étiquetés avec `incident-response`.

### `search_datadog_hosts` {#search-datadog-hosts}
*Ensemble d'outils : **noyau***\
*Autorisations requises : `Hosts Read` et `Timeseries`*\
Liste et fournit des informations sur les hôtes surveillés, prenant en charge le filtrage et la recherche.

- Montrez-moi tous les hôtes dans notre environnement de production.
- Liste des hôtes en mauvais état qui n'ont pas rapporté d'activité au cours de la dernière heure.
...
Récupère une liste des incidents Datadog, y compris leur état, leur gravité et leurs métadonnées.
...
*Ensemble d'outils : **noyau***\
*Autorisations requises : `APM Read` et `Service Catalog Read` et `Teams Read`*\
...
*Ensemble d'outils : **noyau***\
*Permissions requises : `APM Read`*\
Récupère les spans des traces APM avec des filtres tels que le service, le temps, la ressource, etc.

- Montrez-moi les spans présentant des erreurs du checkout service.
- Trouvez les requêtes de base de données lentes des 30 dernières minutes.
- Obtenez les intervalles pour les requêtes API échouées vers notre service de paiement.
...
- Affichez les 10 codes d'état HTTP les plus fréquents ainsi que leur nombre d'occurrences.
- Quels services ont généré le plus de logs durant cette période ?

### `search_datadog_logs` {#search-datadog-logs}
*Toolset : **core***\
*Permissions requises : `Logs Read Data` et `Logs Read Index Data`*\
Recherche dans les journaux avec des filtres (temps, requête, service, hôte, niveau de stockage, etc.) et renvoie les détails des journaux. Renommé à partir de `get_logs`.
...
- Créez un moniteur pour suivre la latence p95 pour le point de terminaison de checkout.
...
Outils pour une analyse approfondie des traces [APM][50], recherche de spans, insights de Watchdog et investigation de performance.

<div class="alert alert-info">Le <code>apm</code> Toolset est en aperçu. <a href="https://www.datadoghq.com/product-preview/apm-mcp-toolset/">Inscrivez-vous pour accéder.</a></div>
...
- Montrez-moi les anomalies de Watchdog pour le service de paiement au cours des dernières 24 heures.
...
*Toolset : **apm***\
Recherche des histoires de changement (déploiements, drapeaux de fonctionnalités et changements d'infrastructure) pour un service dans une plage de temps.
...
*Permissions requises : `APM Read`*\
Analyse les goulets d'étranglement de latence présents dans les traces pendant une période d'anomalie en calculant le self-time.

- Quels sont les goulets d'étranglement de latence pour le service de checkout pendant cette anomalie ?
- Identifiez quels spans contribuent le plus à la latence.

### `apm_latency_tag_analysis` {#apm-latency-tag-analysis}
*Ensemble d'outils : **apm***\
*Permissions requises : `APM Read`*\
Compare les tags de span entre une période d'anomalie et une période de référence pour identifier ce qui a changé.

- Compare les tags entre la fenêtre d'anomalie et la période de référence pour trouver ce qui a changé.
- Quelles valeurs de tags sont différentes pendant ce pic de latence ?
...
- Affichez les recommandations APM pour mes services.
...
- Guidez-moi à travers le débogage d'un pic d'erreurs en production.
...
Recherche des cas de [gestion des cas][38] en utilisant des filtres tels que le statut, la priorité, le projet et le responsable. Prend en charge le filtrage par plage de dates et la pagination.

- Affichez tous les cas ouverts qui me sont assignés.
...
Créez un nouveau cas [gestion des cas][38] avec un titre, un projet et des champs optionnels tels que la description, la priorité et le responsable.

- Je constate un pic de latence sur le service de checkout. Créez un cas P2 pour suivre l'enquête.
...
*Ensemble d'outils : **cas***\
*Permissions requises : `Cases Write`*\
Mettez à jour les champs d'un cas existant tels que le statut, la priorité, le titre, la description, le responsable, la date d'échéance et les attributs personnalisés. Seuls les champs que vous fournissez sont mis à jour.

- Ce problème a maintenant un impact sur le client. Escaladez le CAS-1234 à P1.
- Marquez le cas de migration de base de données comme résolu.
- Définissez une date d'échéance pour la fin de la semaine sur le CAS-1234.

### `add_comment_to_datadog_case` {#add-comment-to-datadog-case}
*Ensemble d'outils : **cas***\
*Permissions requises : `Cases Write`*\
Ajoutez un commentaire à la chronologie d'un cas. Les commentaires prennent en charge le formatage markdown.
...
- Connectez PROJ-456 au cas Datadog afin que l'équipe d'ingénierie dispose d'une meilleure visibilité.
...
*Ensemble d'outils : **cas***\
*Permissions requises : `Cases Read`*\
Récupérez les détails d'un projet de cas spécifique par ID.
...
Recherche des utilisateurs Datadog par email, nom ou handle. Utile pour trouver la bonne personne à qui assigner un cas.
...
*Permissions requises : `Dashboards Read` et `User Access Read`*\
Récupérez un [tableau de bord][46] Datadog par ID, retournant son titre, sa description, ses tags et ses widgets. Utilisez `search_datadog_dashboards` d'abord pour trouver les IDs des tableaux de bord.

- Obtenez les détails complets du tableau de bord `ps7-mn3-kwf`.
- Affichez les widgets et la mise en page du tableau de bord de l'aperçu de l'infrastructure.
...
Créez ou mettez à jour un [tableau de bord][46] Datadog. Pour mettre à jour un tableau de bord existant, fournissez l'ID du tableau de bord ; omettez-le pour en créer un nouveau. Appelez `get_widget_reference` pour les schémas de widgets avant de construire des widgets.
...
*Permissions requises : `Dashboards Read` et `Dashboards Write`*\
Supprimez définitivement un tableau de bord Datadog [tableau de bord][46] par ID. Cette action ne peut pas être annulée. Utilisez `search_datadog_dashboards` d'abord pour trouver les ID des tableaux de bord.
...
- Expliquez comment construire un widget de liste principale et un widget de tableau de requêtes.
...
- Quel est le schéma pour le widget de nuage de points ?
- Aidez-moi à déboguer pourquoi ce widget affiche des valeurs fractionnaires alors qu'il devrait afficher un compte.
- Comment configurer une série temporelle pour afficher à la fois des barres et des lignes ?

## Surveillance de base de données {#database-monitoring}

Outils pour interagir avec [Surveillance de base de données][26].
...
*Ensemble d'outils : **dbm***\
*Permissions requises : `Database Monitoring Read`*\
Récupère les plans d'explication PostgreSQL pour une signature de requête dans un intervalle de temps. Retourne des structures de plan simplifiées avec des arbres d'opérateurs, l'utilisation d'index et des coûts estimés, triés par coût.
...
*Ensemble d'outils : **dbm***\
*Permissions requises : `Database Monitoring Read`*\
Effectue des vérifications de santé pour faire ressortir les problèmes potentiels de PostgreSQL tels que la saturation du CPU, les redémarrages, la latence des requêtes et le blocage. Compare une période de régression à une période de référence.
...
*Ensemble d'outils : **dbm***\
*Permissions requises : `Database Monitoring Read`*\
Récupère les définitions de schéma (colonnes, index, clés étrangères, partitions) pour un ou plusieurs objets de base de données. Accepte les noms de table avec des schémas, bases de données et instances optionnels.
...
- Optimiser la signature de requête `abc123def` sur la base de données des paiements.
...
Recherche des Error Tracking Issues à travers les sources de données (RUM, Logs, Traces).
...
- Trouver des Error Tracking Issues dans l'environnement de production avec `service:api`.
...
Outils pour gérer [les drapeaux de fonctionnalités][51], incluant la création, l'affichage et la mise à jour des drapeaux et de leurs environnements.
...
Liste des drapeaux de fonctionnalités avec support de pagination.
...
- Configure un nouveau drapeau de fonctionnalité booléen pour la fonctionnalité bêta.
...
- Active le drapeau `dark-mode` dans l'environnement de staging.
...
- Vérifiez que le `enable-new-dashboard` drapeau est implémenté dans ma base de code.

### `sync_datadog_feature_flag_allocations` {#sync-datadog-feature-flag-allocations}
*Ensemble d'outils : **drapeaux de fonctionnalités***\
*Permissions requises : `Feature Flag Write`*\
Synchronise les allocations de drapeaux de fonctionnalité pour un environnement spécifique.

- Synchronisez les allocations pour le drapeau `new-checkout-flow` en production.

## Kubernetes {#kubernetes}

Outils pour rechercher et décrire les ressources [Kubernetes][55] et récupérer les manifestes à travers tous les clusters.
...
- Trouvez les déploiements en cours dans le cluster `general2`.
...
Investiguer les problèmes au niveau du réseau en utilisant les données de [Cloud Network Monitoring][31], en analysant les données de flux réseau pour détecter des anomalies telles que des taux de retransmission élevés.
...
- Analysez les données de flux réseau pour détecter des anomalies dans l'environnement de production.
...
*Ensemble d'outils : **réseaux***\
*Permissions requises : `NDM Read`*\
Récupère des informations détaillées sur un dispositif réseau spécifique par son identifiant de dispositif.

- Obtenez les détails pour le dispositif réseau `device:abc123`.
- Montrez-moi la configuration et le statut de ce routeur.

### `search_ndm_interfaces` {#search-ndm-interfaces}
*Ensemble d'outils : **réseaux***\
*Permissions requises : `NDM Read`*\
Récupère toutes les interfaces réseau pour un dispositif spécifique.

- Montrez-moi toutes les interfaces sur le dispositif `device:abc123`.
...
*Ensemble d'outils : **intégration***\
*Permissions requises : Aucune*\
Vous guide lors de l'intégration des applications sans serveur à Datadog, y compris les fonctions AWS Lambda, GCP Cloud Run et les fonctions Cloud Run (Gen 2).
...
*Ensemble d'outils : **intégration***\
Vous guide lors du téléversement des cartes sources pour le mappage des erreurs RUM.

- Aidez-moi à téléverser des cartes sources afin que mes erreurs RUM affichent le code source original.
...
- Listez toutes les tables de référence dans mon organisation.
...
*Ensemble d'outils : **tables de référence***\
Ajoutez de nouvelles lignes à une table de référence existante. Cette opération n'ajoute que des lignes et ne modifie ni ne supprime les données existantes. Chaque ligne doit inclure tous les champs requis du schéma de la table, y compris le champ de clé primaire.

- Ajoutez une nouvelle ligne pour l'utilisateur `user003` avec le nom `Carol` et l'âge `28` dans la table des utilisateurs.
- Ajoutez ces cinq nouvelles entrées de comptes à la table de référence des comptes.

### `create_reference_table` {#create-reference-table}
*Ensemble d'outils : **tables de référence***\
Crée une nouvelle table de référence soutenue par un fichier CSV dans Amazon S3, Google Cloud Storage ou Azure Blob Storage. Seuls les types de champs `INT32` et `STRING` sont pris en charge.

- Créez une table de référence appelée `ip_allowlist` à partir du fichier `allowlist.csv` dans mon S3 bucket `my-data-bucket`.
...
Analyse le code à la recherche de secrets et d'identifiants codés en dur, détectant les clés AWS, les clés API, les mots de passe, les jetons, les clés privées et les identifiants de base de données.
...
- Montrez-moi les 10 règles SIEM par nombre de signaux au cours des 7 derniers jours.
...
*Ensemble d'outils : **sécurité***\
*Permissions requises : `Security Signals Read`*\
Récupère les détails complets d'un signal de sécurité unique par ID, y compris les attributs, les informations sur la règle, l'état de triage, les balises et les corrélations de cas.
...
Retourne le schéma (champs disponibles et leurs types) pour les constatations de sécurité. Appelez ceci d'abord avant d'utiliser `analyze_security_findings` pour découvrir les champs interrogeables. Prend en charge le filtrage par type de constatation et contrôle la taille de la réponse.

- Quels champs sont disponibles pour les constatations de sécurité ?
- Montrez-moi le schéma pour les constatations de vulnérabilité de bibliothèque.
- Obtenez le schéma complet y compris les descriptions des constatations de mauvaise configuration.

### `analyze_security_findings` {#analyze-security-findings}
*Ensemble d'outils : **sécurité***\
*Permissions requises : `Security Monitoring Findings Read` et `Timeseries`*\
Outil principal pour analyser les constatations de sécurité à l'aide de requêtes SQL. Interroge les données en direct des dernières 24 heures avec des agrégations SQL flexibles, un filtrage et un regroupement. Appelez `security_findings_schema` d'abord pour découvrir les champs disponibles, puis utilisez cet outil pour interroger.

- Montrez-moi les 10 règles avec les constatations les plus critiques.
- Comptez les constatations ouvertes regroupées par gravité et type de constatation.
...
*Ensemble d'outils : **sécurité***\
*Permissions requises : `Security Monitoring Findings Read`*\
Outil de secours pour récupérer les détails complets des constatations de sécurité. Préférez `analyze_security_findings` pour la plupart des tâches d'analyse. Utilisez cet outil uniquement lorsque vous avez besoin d'objets de constatation complets ou lorsque les requêtes SQL sont insuffisantes.

- Obtenez des détails complets pour les constatations critiques dans mon environnement AWS.
- Récupérez des objets de constatation complets pour une règle spécifique.
- Listez toutes les constatations de risque d'identité ouvertes avec des métadonnées complètes.
...
- Quelle est la durée moyenne des jobs pour les 7 derniers jours ?
...
Recherche dans Datadog [Test Optimization] des tests instables et renvoie les détails de triage (taux d'échec, catégorie, propriétaires, historique, impact CI), avec pagination et tri.

- Trouvez des tests instables actifs pour le checkout service appartenant à `@team-abc`, triés par taux d'échec.
...
*Permissions requises : `Test Optimization Read`*\
Agrège les événements de Test Optimization de Datadog pour quantifier la fiabilité et les tendances de performance à l'aide de fonctions d'agrégation, de métriques optionnelles, de facettes de regroupement et de niveaux de test configurables.
...
*Ensemble d'outils : **software-delivery***\
*Permissions requises : `Test Optimization Read`*\
Recherche les événements de Test Optimization [24] à l'aide de filtres et renvoie leurs détails.
...
- Montrez-moi toutes les exécutions de tests instables pour le checkout service.
...
Récupère des métriques agrégées résumant la couverture de code pour une branche de dépôt, y compris la couverture totale, la couverture de patch et la répartition par service et propriétaire de code.
...
*Ensemble d'outils : **software-delivery***\
*Permissions requises : `Code Coverage read`*\
Récupère des métriques agrégées résumant la couverture de code pour un commit de dépôt, y compris la couverture totale, la couverture de patch et la répartition par service et propriétaire de code.
...
Outils pour interagir avec Datadog [Synthetics][47].

### `get_synthetics_tests` {#get-synthetics-tests}
*Ensemble d'outils : **synthetics***\
*Permissions requises : `Synthetics Read`*\
Recherche des tests Synthetics de Datadog.

- Aidez-moi à comprendre pourquoi le test Synthetic sur le point de terminaison `/v1/my/tested/endpoint` échoue.
- Il y a une panne ; trouvez tous les tests Synthetic échoués sur le domaine `api.mycompany.com`.
...
- Améliorez les assertions du test Synthetic défini sur mon point de terminaison `/v1/my/tested/endpoint`.
- Mettez le test `aaa-bbb-ccc` en pause et configurez les emplacements pour n'inclure que ceux situés en Europe.
...
Outils pour [Automatisation des Flux de Travail][39], y compris la liste, l'inspection, l'exécution et la configuration des flux de travail pour l'utilisation par les agents.
...
- Montrez-moi tous les flux de travail publiés étiquetés avec `team:platform`.
- Listez les flux de travail qui ont un déclencheur d'agent configuré.
- Trouvez tous les flux de travail liés à la réponse aux incidents appartenant à Alice Smith.
...
- Montrez-moi les paramètres d'entrée et les étapes pour le flux de travail de restauration de déploiement.
...
- Exécutez le flux de travail d'escalade d'incidents pour le service `checkout-api` avec un niveau de gravité `high`.
- Exécutez le flux de travail de retour en arrière de déploiement pour le service de paiements.
- Déclenchez le flux de travail de notification d'appel avec le contexte de cette enquête.
