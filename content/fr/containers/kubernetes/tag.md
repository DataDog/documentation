...
À partir de l'Agent v6.10+, l'Agent peut découvrir automatiquement des tags à partir des annotations de Pod. Il permet à l'Agent d'associer des tags à toutes les données émises par l'ensemble des pods ou un conteneur individuel au sein de ce pod.
...
- Étiquettes *ne* se propagent pas entre la charge de travail et les ressources enfants. Par exemple, les étiquettes sur un déploiement ne sont pas automatiquement appliquées aux journaux de ses Pods enfants. Pour étiqueter les données des Pods, configurez l'extraction des étiquettes directement sur les Pods.
- Les étiquettes *se* propagent des espaces de noms vers les pods et les conteneurs à l'intérieur.
- Utilisez Datadog Agent 7.73+ pour utiliser des caractères génériques dans les règles d'extraction des étiquettes pour vos métriques KSM.

### Étiquettes des ressources Kubernetes en tant qu'étiquettes

Cette option est utilisée pour extraire une étiquette donnée sur vos ressources Kubernetes et l'envoyer en tant qu'étiquette Datadog.

{{< tabs >}}

{{% tab "Opérateur Datadog" %}}

Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés d'étiquettes `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la configuration de votre opérateur `DatadogAgent` dans `datadog-agent.yaml` :
...
Par exemple, pour extraire des étiquettes de ressources des nœuds, des pods et des déploiements :
...
Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés d'étiquettes `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à votre fichier Helm `datadog-values.yaml` :
...
Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés d'étiquettes `<TAG_KEY>` dans Datadog, ajoutez la variable d'environnement suivante à **à la fois** vos conteneurs Agent et Cluster Agent.
...
Pour Agent 7.73.0+, utilisez la configuration suivante pour ajouter toutes les étiquettes de ressources en tant qu'étiquettes à vos métriques. Dans cet exemple, les noms de balises sont préfixés par `<PREFIX>_` :
```yaml
    #(...)
    kubernetesResourcesLabelsAsTags:
      pods:
...
Si vous utilisez la version 7.58.0+ de l'agent, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes comme balises</a> pour configurer les étiquettes des nœuds comme balises.
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de nœud comme balises à vos métriques. Dans cet exemple, les noms de balises sont préfixés par `<PREFIX>_` :
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de nœud comme balises à vos métriques. Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
#### Étiquettes de pod comme balises

<div class="alert alert-info">

Si vous utilisez la version 7.58.0+ de l'agent, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes comme balises</a> pour configurer les étiquettes de pod comme balises.
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de pod comme balises à vos métriques. Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de pod comme balises à vos métriques, sauf celles de KSM (`kubernetes_state.*`). Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
#### Annotations de pod comme balises

<div class="alert alert-info">

Si vous utilisez la version 7.58.0+ de l'agent, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes comme balises</a> pour configurer les annotations de pod comme balises.
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les annotations de pod en tant qu'étiquettes à vos métriques, sauf celles de KSM (`kubernetes_state.*`). Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
Pour l'Agent v7.24.0+, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de namespace en tant qu'étiquettes à vos métriques, sauf celles de KSM (`kubernetes_state.*`). Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
