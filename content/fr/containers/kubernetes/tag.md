...
La liste des balises attribuées automatiquement dépend de la [configuration de cardinalité][1] de l'Agent. La [Cardinalité des balises][4] est ajoutée avant l'ingestion et peut avoir un impact sur la facturation, car les différentes configurations de cardinalité influencent le nombre de métriques émises.
...
<div style="overflow-x: auto;">
  ...
  | `env`                         | Faible          | Étiquette du pod `tags.datadoghq.com/env` ou variable d'environnement de conteneur (`DD_ENV` ou `OTEL_RESOURCE_ATTRIBUTES`)                               | [Balises de service unifiées][2] activées                |
  | `version`                     | Faible          | Étiquette du pod `tags.datadoghq.com/version` ou variable d'environnement de conteneur (`DD_VERSION` ou `OTEL_RESOURCE_ATTRIBUTES`)                       | [Balises de service unifiées][2] activées                |
  | `service`                     | Faible          | Étiquette du pod `tags.datadoghq.com/service` ou variable d'environnement de conteneur (`DD_SERVICE`, `OTEL_RESOURCE_ATTRIBUTES` ou `OTEL_SERVICE_NAME`) | [Balises de service unifiées][2] activées                |
  ...
  | `persistentvolumeclaim`       | Faible          | Spécifications du pod                                                                                                                      | Un PVC doit être attaché au pod                   |
...
L'Agent peut attacher des informations sur l'environnement Kubernetes sous forme de "balises hôtes".

<div style="overflow-x: auto;">

  | Balise                 | Cardinalité | Source                                                 | Exigence                                                    |
  |---------------------|-------------|--------------------------------------------------------|----------------------------------------------------------------|
  | `kube_cluster_name` | Faible         | `DD_CLUSTER_NAME` variable d'environnement ou intégration de fournisseur de cloud | `DD_CLUSTER_NAME` variable d'environnement ou intégration de fournisseur de cloud activée |
...
## Autodécouverte des balises

À partir de la version 6.10+ de l'Agent, l'Agent peut autodécouvrir des balises à partir des annotations de pod. Cela permet à l'Agent d'associer des balises à toutes les données émises par l'ensemble des pods ou un conteneur individuel au sein de ce pod.

En tant que meilleure pratique dans les environnements conteneurisés, Datadog recommande d'utiliser des balises de service unifiées pour aider à unifier les balises. Les balises de service unifiées relient la télémétrie Datadog grâce à l'utilisation de trois balises standard : `env`, `service` et `version`. Pour apprendre à configurer votre environnement avec des balises unifiées, consultez la documentation dédiée aux [balises de service unifiées][2].

Pour appliquer une balise `<TAG_KEY>:<TAG_VALUE>` à toutes les données émises par un pod donné et collectées par l'Agent, utilisez l'annotation suivante sur votre pod :
...
Si vous souhaitez appliquer une balise `<TAG_KEY>:<TAG_VALUE>` à un conteneur individuel `<CONTAINER_NAME>` au sein d'un pod, utilisez l'annotation suivante sur votre pod :
...
À partir de l'Agent v7.17+, l'Agent peut découvrir automatiquement des balises à partir des étiquettes Docker. Ce processus permet à l'Agent d'associer des balises personnalisées à toutes les données émises par un conteneur, sans modifier la configuration de l'Agent.

```yaml
com.datadoghq.ad.tags: '["<TAG_KEY>:TAG_VALUE", "<TAG_KEY_1>:<TAG_VALUE_1>"]'
```

À partir de l'Agent v7.77+, les annotations de balises prennent en charge [les variables de modèle d'autodécouverte][5] pour un balisage dynamique basé sur les métadonnées d'exécution. À l'exception de `%%env_<VAR>%%`, toutes les variables de modèle sont prises en charge.
...
À partir de la version 7.64+, l'Agent et l'Agent de Cluster peuvent être configurés pour collecter des étiquettes et des annotations à partir des ressources Kubernetes et les utiliser comme balises à partir d'une configuration commune. Datadog recommande d'utiliser les options suivantes pour garantir un reporting cohérent entre le balisage principal de l'Agent, le reporting KSM de l'Agent de Cluster et le reporting de l'Orchestrateur Explorer des deux Agents : 
- `kubernetesResourcesLabelsAsTags`
- `kubernetesResourcesAnnotationsAsTags`

Ces options doivent être utilisées à la place des options héritées de l'Agent `podLabelsAsTags`, `nodeLabelsAsTags`, `namespaceLabelsAsTags` et de tout remplacement de configuration KSM.
...
Par exemple, exécutez `kubectl api-resources` pour récupérer ces informations : 

| Nom        | Version de l'API                  | Configuration de la ressource Datadog  |
|-------------|------------------------------|---------------------------------|
| pods        | v1                           | pods                            |
| nœuds       | v1                           | nœuds                           |
...
- Les balises *ne* se propagent pas entre la charge de travail et les ressources enfants. Par exemple, les étiquettes sur un déploiement ne sont pas automatiquement appliquées aux journaux de ses pods enfants. Pour baliser les données des pods, configurez l'extraction des étiquettes directement sur les pods. 
- Les balises *se* propagent du namespace aux pods et conteneurs à l'intérieur de ceux-ci. 
- Utilisez Datadog Agent 7.73+ pour utiliser des caractères génériques dans les règles d'extraction de balises pour vos métriques KSM. 

### Étiquettes des ressources Kubernetes en tant que balises

Cette option est utilisée pour extraire une étiquette donnée sur vos ressources Kubernetes et l'envoyer en tant que balise Datadog.

{{< tabs >}}

{{% tab "Opérateur Datadog" %}}

Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés de balise `<TAG_KEY>` au sein de Datadog, ajoutez la configuration suivante à la configuration de votre opérateur `DatadogAgent` dans `datadog-agent.yaml` :
...
Par exemple, pour extraire des étiquettes de ressources à partir de nœuds, de pods et de déploiements :
...
Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés de balise `<TAG_KEY>` au sein de Datadog, ajoutez la configuration suivante à votre fichier Helm `datadog-values.yaml` :
...
Pour extraire une étiquette de ressource donnée `<LABEL>` et les transformer en clés de balise `<TAG_KEY>` dans Datadog, ajoutez la variable d'environnement suivante à **vos** conteneurs Agent et Cluster Agent.
...
Pour Agent 7.73.0+, utilisez la configuration suivante pour ajouter toutes les étiquettes de ressource en tant que balises à vos métriques. Dans cet exemple, les noms de balises sont préfixés par `<PREFIX>_` :
```yaml
    #(...)
    kubernetesResourcesLabelsAsTags:
      pods:
...
Pour extraire une annotation de ressource donnée `<ANNOTATION>` et les transformer en clés de balise `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la `DatadogAgent` configuration de votre opérateur dans `datadog-agent.yaml` :
...
Par exemple, pour extraire des annotations de ressource à partir des nœuds, des pods et des déploiements :
...
Pour extraire une annotation de ressource donnée `<ANNOTATION>` et les transformer en clés de balise `<TAG_KEY>` dans Datadog, ajoutez la variable d'environnement suivante à **vos** conteneurs Agent et Cluster Agent.
...
Pour Agent 7.73.0+, utilisez la configuration suivante pour ajouter toutes les annotations de ressource en tant que balises à vos métriques. Dans cet exemple, les noms de balises sont préfixés par `<PREFIX>_` :
...
#### Les étiquettes de nœud en tant que balises

<div class="alert alert-info">

Si vous utilisez la version de l'agent 7.58.0 ou supérieure, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes en tant que balises</a> pour configurer les étiquettes des nœuds en tant que balises.
...
{{< tabs >}}
{{% tab "Opérateur Datadog" %}}
Pour extraire une étiquette de nœud donnée `<NODE_LABEL>` et la transformer en clé de balise `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la configuration de votre `DatadogAgent` de l'Opérateur dans `datadog-agent.yaml` :
...
Pour l'Agent v7.24.0 ou supérieur, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de nœud en tant que balises à vos métriques. Dans cet exemple, les noms de balises sont préfixés par `<PREFIX>_` :
...
Pour l'Agent v7.24.0 ou supérieur, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de nœud en tant que balises à vos métriques. Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
**Remarque** : Les métriques personnalisées peuvent avoir un impact sur la facturation. Voir la [page de facturation des métriques personnalisées][3] pour plus d'informations.

#### Les étiquettes de pod en tant que balises

<div class="alert alert-info">

Si vous utilisez la version de l'agent 7.58.0 ou supérieure, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes en tant que balises</a> pour configurer les étiquettes des pods en tant que balises.
...
{{< tabs >}}
{{% tab "Opérateur Datadog" %}}
Pour extraire une étiquette de pod donnée `<POD_LABEL>` et la transformer en clé de balise `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la configuration de votre `DatadogAgent` de l'Opérateur dans `datadog-agent.yaml` :
...
#### Les annotations de pod en tant que tags

<div class="alert alert-info">

Si vous utilisez la version de l'agent 7.58.0 ou supérieure, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes en tant que tags</a> pour configurer les annotations de pod en tant que tags.

</div>

À partir de la version 6.0 de l'Agent, l'Agent peut collecter des annotations pour un pod donné et les utiliser comme tags à attacher à toutes les métriques, traces et journaux émis par ce pod :
...
Pour l'Agent v7.24.0 et supérieur, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les annotations de pod en tant que tags à vos métriques, sauf celles de KSM (`kubernetes_state.*`). Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
#### Les étiquettes de namespace en tant que tags

<div class="alert alert-info">

Si vous utilisez la version de l'agent 7.58.0 ou supérieure, il est conseillé d'utiliser <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">les étiquettes des ressources Kubernetes en tant que tags</a> pour configurer les étiquettes de namespace en tant que tags.

</div>

À partir de la version 7.55.0 de l'Agent, l'Agent peut collecter des étiquettes pour un namespace donné et les utiliser comme tags à attacher à toutes les métriques, traces et journaux émis par tous les pods dans ce namespace :
...
Pour l'Agent v7.24.0 et supérieur, utilisez la configuration de variable d'environnement suivante pour ajouter toutes les étiquettes de namespace en tant que tags à vos métriques, sauf celles de KSM (`kubernetes_state.*`). Dans cet exemple, les noms des balises sont préfixés par `<PREFIX>_` :
...
### Les variables d'environnement des conteneurs en tant que tags

À partir de la version 7.32 de l'Agent, l'Agent peut collecter des variables d'environnement des conteneurs et les utiliser comme tags à attacher à toutes les métriques, traces et journaux correspondant au conteneur. Les conteneurs `docker` et `containerd` sont pris en charge :

{{< tabs >}}
{{% tab "Opérateur Datadog" %}}
Pour extraire une variable d'environnement donnée `<ENV_VAR>` et la transformer en clé de tag `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la configuration de votre opérateur `DatadogAgent` dans `datadog-agent.yaml` :
...
### Les étiquettes de conteneurs en tant que tags

À partir de la version 7.33+ de l'Agent, l'Agent peut collecter les étiquettes de conteneurs et les utiliser comme tags. L'agent attache les tags à toutes les métriques, traces et journaux associés au conteneur.

L'Agent peut générer des tags à partir des étiquettes de conteneurs pour les conteneurs `docker` et `containerd`. Dans le cas de `containerd`, la version minimale prise en charge est v1.5.6, car les versions précédentes ne propagent pas correctement les étiquettes.

{{< tabs >}}
{{% tab "Opérateur Datadog" %}}
Pour extraire une étiquette de conteneur donnée `<CONTAINER_LABEL>` et la transformer en clé de tag `<TAG_KEY>` dans Datadog, ajoutez la configuration suivante à la configuration de votre Opérateur `DatadogAgent` dans `datadog-agent.yaml` :
