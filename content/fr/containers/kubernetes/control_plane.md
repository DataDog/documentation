...
Vous pouvez activer l'introspection en utilisant le [Helm chart datadog-operator](https://github.com/DataDog/helm-charts/tree/main/charts/datadog-operator) :
...
Puisque cette fonctionnalité est activée par défaut, vous pouvez déployer une spécification minimale de Datadog Agent.
...
Vous pouvez activer l'introspection en utilisant le [Helm chart datadog-operator][12] :
...
Activez `features.clusterChecks.useClusterChecksRunners` pour planifier des vérifications là-bas ; sinon, les vérifications du plan de contrôle s'exécutent sur le Node Agent.
...
1. Activez les [vérifications de point de terminaison][8]
...
La dernière annotation `ad.datadoghq.com/endpoints.resolve` est nécessaire car le service est devant des pods statiques. L'Agent de Cluster Datadog planifie les vérifications en tant que vérifications de point de terminaison et les transmet aux Exécuteurs de Vérification de Cluster. Les nœuds sur lesquels ils s'exécutent peuvent être identifiés par :
...
**Note** : Les montages sont également inclus pour désactiver le fichier de configuration automatique de vérification Etcd emballé avec l'Agent Datadog.
...
L'Agent de Cluster Datadog planifie les vérifications en tant que vérifications de point de terminaison et les transmet aux Exécuteurs de Vérification de Cluster.

{{% /collapse-content %}}


{{% collapse-content title="Etcd OpenShift 4.14 et ultérieures" level="h4" %}}
...
L'Agent de Cluster Datadog planifie les vérifications en tant que vérifications de point de terminaison et les envoie aux Cluster Check Runners.
...
La dernière annotation `ad.datadoghq.com/endpoints.resolve` est nécessaire car le service est devant des pods statiques. La dernière annotation est nécessaire car le service est devant des pods statiques. Les nœuds sur lesquels ils s'exécutent peuvent être identifiés par :
...
Des certificats sont nécessaires pour communiquer avec le service Etcd, qui se trouve sur l'hôte. Ces certificats doivent être montés sur les pods des Exécuteurs de Vérification de Cluster en ajoutant les volumes et les volumeMounts comme ci-dessous.
...
Vous pouvez annoter ce service avec des vérifications de point de terminaison (gérées par l'Agent de Cluster Datadog) pour surveiller le Serveur API, le Gestionnaire de Contrôleur et le Planificateur :
