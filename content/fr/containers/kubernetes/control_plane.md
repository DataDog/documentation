...
Cette section vise à documenter les spécificités et à fournir de bonnes configurations de base pour surveiller le plan de contrôle Kubernetes. Vous pouvez ensuite personnaliser ces configurations pour ajouter n'importe quelle fonctionnalité de Datadog.

Avec les intégrations Datadog pour le [serveur API][1], [Etcd][2], [Gestionnaire de contrôleur][3] et [Planificateur][4], vous pouvez collecter des métriques clés de tous les quatre composants du plan de contrôle Kubernetes.
...
L'intégration du serveur API est automatiquement configurée. L'Agent Datadog le découvre automatiquement.
...
**Remarques :**
 ...
 Lors de la cible des ports sécurisés, l'option `bindaddress` dans votre configuration de Gestionnaire de contrôleur et de Planificateur doit être accessible par l'Agent Datadog. Exemple :
...
Vous pouvez activer l'introspection en utilisant le [chart Helm datadogoperator](https://github.com/DataDog/helmcharts/tree/main/charts/datadogoperator) :
...
Vous devriez voir des métriques du plan de contrôle dans Datadog, y compris :
 `kube_apiserver.*`
 `kube_controller_manager.*`
 `kube_scheduler.*`
...
#### Prérequis
 ...
 Déployez l'Agent en utilisant l'une des options suivantes :
   Version du graphique Helm >= `3.90.1`
...
**Remarques :**
 Amazon expose les métriques `kube_controller_manager` et `kube_scheduler` sous le groupe API [`metrics.eks.amazonaws.com`][11].
 L'ajout de `"extra_headers":{"accept":"*/*"}` empêche les erreurs `HTTP 406` lors de la requête à l'API des métriques EKS.
...
**Remarque** : `etcd` n'est pas pris en charge sur les versions 4.04.13.

#### Configuration générale

La surveillance du plan de contrôle est activée par défaut, mais nécessite que l'introspection soit activée.

Vous pouvez activer l'introspection en utilisant le graphique Helm [datadogoperator][12] :
...
Ou, pour les **utilisateurs d'OpenShift** qui ont installé l'opérateur via OperatorHub/Marketplace (la [méthode recommandée](installopenshift.md)), en patchant la version du service de cluster de l'opérateur :
...
1. Activez le [Cluster Agent][6] Datadog
1. Activez les [Cluster checks][7]
1. Activez les [Endpoint checks][8]
1. Assurez-vous que vous êtes connecté avec des autorisations suffisantes pour modifier les services et créer des secrets.

#### Serveur API

Le serveur API fonctionne derrière le service `kubernetes` dans l'espace de noms `default`. Annoter ce service avec la configuration `kube_apiserver_metrics` :
...
La dernière annotation `ad.datadoghq.com/endpoints.resolve` est nécessaire car le service est devant des pods statiques. L'Agent Cluster Datadog planifie les vérifications en tant que vérifications d'endpoint et les dispatches aux Cluster Check Runners. Les nœuds sur lesquels ils s'exécutent peuvent être identifiés avec :
...
Des certificats sont nécessaires pour communiquer avec le service Etcd, qui se trouve dans le secret `kubeetcdclientcerts` dans l'espace de noms `openshiftmonitoring`. Pour donner au Datadog Agent accès à ces certificats, copiez-les d'abord dans le même espace de noms où le Datadog Agent s'exécute :
...
**Remarque** : Des montages sont également inclus pour désactiver le fichier de configuration d'autoconfiguration de vérification Etcd inclus avec l'agent.
...
Ensuite, annotez le service qui s'exécute devant Etcd :
...
L'Agent Cluster Datadog planifie les vérifications en tant que vérifications d'endpoint et les dispatches aux Cluster Check Runners.
...
Des certificats sont nécessaires pour communiquer avec le service Etcd, qui se trouve dans le secret `etcdmetricclient` dans l'espace de noms `openshiftetcdoperator`. Pour donner au Datadog Agent accès à ces certificats, copiez-les dans le même espace de noms que le Datadog Agent :
...
Le Gestionnaire de contrôleur s'exécute derrière le service `kubecontrollermanager` dans l'espace de noms `openshiftkubecontrollermanager`. Annotez le service avec la configuration de vérification :
...
Le Planificateur s'exécute derrière le service `scheduler` dans l'espace de noms `openshiftkubescheduler`. Annotez le service avec la configuration de vérification :
...
Sur OpenShift 3, tous les composants du plan de contrôle peuvent être surveillés à l'aide de vérifications d'endpoint.
...
Des certificats sont nécessaires pour communiquer avec le service Etcd, qui se trouvent sur l'hôte. Ces certificats doivent être montés sur les pods Cluster Check Runner en ajoutant les volumes et les volumeMounts comme ci-dessous.
...
Annoter le service copié avec la configuration de vérification :
...
Le gestionnaire de contrôleur et le planificateur fonctionnent derrière le même service, `kubecontrollers` dans l'espace de noms `kubesystem`. Les modifications directes du service ne sont pas conservées, donc faites une copie du service :
...
Annoter le service copié avec les configurations de vérification :
...
Helm est la méthode d'installation recommandée pour Talos Linux. Utilisez Helm en définissant le drapeau `providers.talos.enabled` sur `true`.
...
En fournissant un accès en lecture aux certificats etcd situés sur l'hôte, le contrôle du Datadog Agent peut communiquer avec etcd et commencer à collecter des métriques etcd.
...
**Remarques :**
 ...
 Le champ `ssl_verify` dans la configuration `kube_controller_manager` et `kube_scheduler` doit être défini sur `false` lors de l'utilisation de certificats auto-signés.
 Lors de la cible des ports sécurisés, l'option `bindaddress` dans votre configuration de Gestionnaire de contrôleur et de Planificateur doit être accessible par l'Agent Datadog. Appliquez le patch ci-dessous aux nœuds de plan de contrôle lors de la génération du cluster ; ou, pour les nœuds Talos en cours d'exécution, exécutez `talosctl patch mc n &lt;controlplanenode1,controlplanenode2> patch @controlplanedatadogmonitoringpatch.yaml`.
...
Rancher v2.5 s'appuie sur [PushProx][9] pour exposer les points de terminaison de métriques du plan de contrôle, ce qui permet au Datadog Agent d'exécuter des vérifications du plan de contrôle et de collecter des métriques.

### Prérequis

1. Installez le Datadog Agent avec le [chart ranchermonitoring][10].
2. Les daemonsets `pushprox` sont déployés avec `ranchermonitoring` et fonctionnent dans l'espace de noms `cattlemonitoringsystem`.
...
### Ajouter des services Kubernetes pour configurer les vérifications d'autodécouverte

En ajoutant des services Kubernetes sans tête pour définir les configurations de vérification, le Datadog Agent est capable de cibler les pods `pushprox` et de collecter des métriques.

Appliquez `ranchercontrolplaneservices.yaml` :
...
Déployez le Datadog Agent avec des manifests basés sur les configurations suivantes :
...
Installez l'Agent Datadog avec le [chart ranchermonitoring][10].

Les composants du plan de contrôle s'exécutent sur Docker en dehors de Kubernetes. Dans Kubernetes, le service `kubernetes` dans l'espace de noms `default` cible l'IP du nœud du plan de contrôle. Vous pouvez le confirmer en exécutant `$ kubectl describe endpoints kubernetes`.

Vous pouvez annoter ce service avec des vérifications de points de terminaison (gérées par l'Agent Cluster Datadog) pour surveiller le Serveur API, le Gestionnaire de contrôleur et le Planificateur :
...
2. En utilisant les informations de montage disponibles dans la sortie de `$ docker inspect etcd`, définissez `volumes` et `volumeMounts` dans la configuration de l'Agent Datadog. Incluez également des tolérances afin que l'Agent Datadog puisse s'exécuter sur les nœuds du plan de contrôle.
...
3. Configurez un DaemonSet avec un conteneur de pause pour exécuter la vérification d'Etcd sur les nœuds exécutant Etcd. Ce DaemonSet s'exécute sur le réseau hôte afin qu'il puisse accéder au service Etcd. Il a également la configuration de vérification et les tolérances nécessaires pour s'exécuter sur le(s) nœud(s) du plan de contrôle. Assurez-vous que les chemins des fichiers de certificats montés correspondent à ce que vous avez configuré sur votre instance, et remplacez la portion `&lt;...>` en conséquence.
...
Sur d'autres services gérés, tels que Azure Kubernetes Service (AKS) et Google Kubernetes Engine (GKE), l'utilisateur ne peut pas accéder aux composants du plan de contrôle. En conséquence, il n'est pas possible d'exécuter les vérifications `kube_apiserver`, `kube_controller_manager`, `kube_scheduler` ou `etcd` dans ces environnements.
