...
- link: https://learn.datadoghq.com/courses/tagging-best-practices
  ...
  text: 'Balisage : meilleures pratiques'
...
- La balise **valeur** est les données ou informations spécifiques associées à la clé. Les valeurs de balise ne sont pas uniques par ressource et peuvent être utilisées sur de nombreuses ressources dans une paire `<key>:<value>`.
...
Les chaînes de balises (c'est-à-dire l'intégralité du contenu de `<key>:<value>` ou `<value>`) doivent répondre aux exigences suivantes :

- Les chaînes de balises doivent **commencer par une lettre** (cela s'applique indépendamment du fait que la balise utilise le format `<key>:<value>` ou `<value>`). Après la lettre initiale, la chaîne de balises peut contenir les caractères énumérés ci-dessous :
...
Après avoir attribué des balises au niveau de l'hôte et de l'intégration, commencez à les utiliser pour filtrer et regrouper vos métriques, traces et journaux. Les balises sont utilisées dans les domaines suivants de votre plateforme Datadog.

| Domaine                 | Utilisez des balises pour                                                                                      |
| -------------------- | ------------------------------------------------------------------------------------------------ |
| [Événements][10]         | Filtrer le flux d'événements.                                                                          |
| [Tableaux de bord][11]     | Filtrer et regrouper les métriques sur les graphiques.                                                               |
| [Infrastructure][12] | Filtrer et regrouper sur la carte des hôtes, la liste des infrastructures, les conteneurs actifs et les processus actifs dans les vues. |
...
| [Intégrations][15]   | Limiter optionnellement les métriques pour AWS, Google Cloud et Azure.                                        |
| [APM][16]            | Filtrer les services, traces et profils, ou naviguer vers d'autres zones avec le Service Map.           |
| [RUM & Session Replay][17] | Filtrer la recherche d'événements, l'analyse, les modèles, les replays et les problèmes avec le RUM Explorer.        |
| [Synthetic Monitoring & Continuous Testing][18]     | Filtrer et regrouper les tests synthétiques ou ceux exécutés dans des pipelines CI avec l'Explorateur des résultats de la surveillance synthétique et des tests continus.   |
| [Cahiers][19]      | Filtrer et regrouper les métriques sur les graphiques.                                                               |
| [Journaux][20]           | Filtrer la recherche de journaux, les analyses, les modèles, le suivi en direct et les pipelines.                                |
| [SLOs][21]           | Rechercher des SLOs, des SLOs basés sur des métriques regroupées et des SLOs basés sur des moniteurs regroupés.                       |
| [Développeurs][22]     | Extraire des informations ou configurer différentes zones dans l'interface utilisateur avec l'API.                                |
...
| [Visibilité CI][24]  | Filtrer et regrouper les exécutions de tests ou les exécutions de pipelines avec l'Explorateur de visibilité CI. |
