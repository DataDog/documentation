...
L'Agent transmet à Datadog les métriques suivantes le concernant. Ces métriques fournissent des informations sur les hôtes ou conteneurs ayant des Agents en cours d'exécution, sur le moment où chaque Agent a démarré, et sur la version de Python que l'Agent utilise.

| Métrique                           | Description                                      |
| -------------------------------- |------------------------------------------------- |
| `datadog.agent.running`        | Affiche une valeur de `1` si l'Agent transmet à Datadog.                    |
| `datadog.agent.started`        | Un comptage envoyé avec une valeur de `1` lorsque l'Agent démarre (disponible dans v6.12+).    |
...
| Vérification | Métriques | Plates-formes |
| ----------- | ------------- | ------------------ |
| CPU         | [Système][4]  | Tout                |
...
Ces vérifications peuvent être utilisées dans Datadog pour visualiser l'état de l'Agent via des moniteurs et des tableaux de bord d'un coup d'œil. Voir [Aperçu des vérifications de service][21] pour en savoir plus.
...
{{< nextlink href="/getting_started/application" >}}En savoir plus sur le Datadog UI (interface utilisateur){{< /nextlink >}}
