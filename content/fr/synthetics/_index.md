...
{{< learning-center-callout header="Participez à une session de webinaire de formation" hide_image="true" btn_title="Inscrivez-vous" btn_url="https://www.datadoghq.com/technical-enablement/session/synthetics/">}}
  Explorez et inscrivez-vous aux sessions de formation Foundation. Découvrez comment Datadog Synthetic Monitoring est une solution de surveillance proactive qui vous permet de créer des tests sans code pour les API, les navigateurs et les applications mobiles, afin de simuler automatiquement les flux d'utilisateurs et les requêtes vers vos applications, vos points de terminaison clés et vos couches réseau.
{{< /learning-center-callout >}}

Les tests synthétiques vous permettent d'observer comment vos systèmes et applications fonctionnent en utilisant **des requêtes et des actions simulées du monde entier**. Datadog suit la performance de vos pages web et de vos API, du backend au frontend, et à divers niveaux réseau (`HTTP`, `SSL`, `DNS`, `WebSocket`, `TCP`, `UDP`, `ICMP` et `gRPC`) de manière contrôlée et stable, vous alertant en cas de comportements défaillants tels que des régressions, des fonctionnalités défaillantes, des temps de réponse élevés et des codes d'état inattendus. 
...
Créer [des tests de chemin réseau synthétiques][25] à partir d'emplacements gérés pour effectuer des vérifications TCP, UDP et ICMP et visualiser les routes des paquets à travers des points de terminaison mondiaux.
...
Utilisation avancée
: L'utilisation avancée inclut des techniques permettant de révéler des informations de test plus approfondies ou de structurer des messages complexes à l'aide du templating handlebars.
