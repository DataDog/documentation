...
- link: /continuous_integration/pipelines
  tag: Documentation
  text: Découvrez Pipeline Visibility
...
| Stage      | Sert de regroupement de jobs sous un nom défini par l'utilisateur. Certains fournisseurs de CI n'ont pas ce niveau. |
...
| Step       | Dans certains fournisseurs de CI, ce niveau représente un script shell ou une action exécutée dans un job. |

Lorsque un pipeline, un stage, un job ou un step se termine, les données d'exécution sont envoyées à Datadog. Pour configurer Pipeline Visibility, consultez la liste des [fournisseurs de CI pris en charge][2]. Si votre fournisseur de CI ou votre workflow n'est pas pris en charge, vous pouvez utiliser le [public API endpoint][3] pour envoyer vos exécutions de pipeline à Pipeline Visibility.

{{< img src="ci/ci-pipeline-execution.png" alt="Exemple d'une trace d'exécution de pipeline" style="width:100%;">}}

Les stages, les jobs et les steps doivent avoir exactement le même nom de pipeline que leur pipeline parent. En cas d'incohérence, certains pipelines peuvent manquer d'informations sur le stage, le job et le step. Par exemple, des jobs manquants dans les tableaux récapitulatifs des jobs.
...
Selon le fournisseur, certains niveaux peuvent être manquants. Par exemple, des étapes peuvent ne pas exister, et des jobs peuvent s'exécuter en parallèle ou en séquence, ou une combinaison des deux.
...
Les reprises partielles sont également considérées comme des pipelines distincts. L'heure de début et de fin ne doit pas inclure le temps de la reprise originale. Pour une reprise partielle, n'envoyez pas de charge utile pour les jobs exécutés lors de la tentative précédente. De plus, définissez le champ `partial_retry` sur `true` lors des reprises partielles pour les exclure de l'agrégation lors du calcul des temps d'exécution.
...
1. Charge utile de travail pour `J1`, avec l'ID `J1_1` et l'ID de pipeline `P_1`.
2. Charge utile de travail pour `J2`, avec l'ID `J2_1` et l'ID de pipeline `P_1`.
3. Charge utile de pipeline pour `P`, avec l'ID `P_1`.

Supposons qu'il y ait une reprise partielle du pipeline à partir de `J2`, où tous les travaux restants réussissent. 
...
Les données restantes du pipeline doivent être envoyées dans des charges utiles séparées avec un identifiant unique de pipeline différent. Dans le deuxième pipeline, vous pouvez définir `is_resumed` à `true` pour signaler que l'exécution a été reprise à partir d'un pipeline bloqué.
