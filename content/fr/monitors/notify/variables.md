...
title: Variables
---
Utiliser des variables dans les messages de notification pour afficher des messages conditionnels et diriger la notification vers différentes équipes en utilisant [des variables conditionnelles](#conditional-variables), ou pour enrichir son contenu en utilisant [des variables d'attribut et de balise](#attribute-and-tag-variables) et [des variables de modèle](#template-variables).
...
Ou utilisez le `{{else}}` parameter in the first example:
...
Si vous configurez un bloc conditionnel pour une transition d'état dans les conditions `alert` ou `warning` avec un gestionnaire de notifications **@-notifications**, Datadog recommande de configurer une condition correspondante `recovery` pour envoyer une notification de récupération au gestionnaire.
...
Utilisez des variables d'attribut et de tag pour rendre les messages d'alerte personnalisés, informatifs et spécifiques afin d'aider à comprendre la nature de l'alerte. Consultez les sections suivantes pour des exemples et des cas d'utilisation :
...
Si un groupe a plusieurs `values` associés au même `key`, le message d'alerte affiche une chaîne de toutes les valeurs, séparées par des virgules, dans l'ordre lexicographique.
...
{{% collapse-content title="Regrouper les requêtes par kube_namespace et kube_cluster_name" level="h5" %}}
...
- Pour les moniteurs utilisant des formules et des fonctions dans les requêtes, les valeurs se résolvent en fonction des événements extraits de la première requête.
...
### Vérifiez les variables du moniteur {#check-monitor-variables}

Pour vérifier les variables du moniteur (vérification personnalisée et d'intégration), la variable `{{check_message}}` est disponible et rend le message spécifié dans la vérification personnalisée ou la vérification d'intégration.
...
Les moniteurs composites prennent également en charge les variables de tags de la même manière que leurs moniteurs sous-jacents. Ils suivent le même format que les autres moniteurs, à condition que les moniteurs sous-jacents soient regroupés par le même tag ou facette.
...
Utilisez des variables de modèle pour personnaliser vos notifications de moniteur. Les variables intégrées sont :
...
**Exemple** : Lorsque le moniteur passe de `OK` → `WARN`, les valeurs de `{{first_triggered_at}}` and `{{last_triggered_at}}` a, comme {{first_triggered_at}}, l'horodatage A. Le tableau ci-dessous montre les valeurs jusqu'à ce que le moniteur se rétablisse.
...
Utilisez les [variables de balise](#attribute-and-tag-variables) pour activer la construction dynamique d'URL qui relie votre équipe à une ressource appropriée. Par exemple, vous pouvez fournir des liens vers des pages au sein de Datadog telles que des tableaux de bord, la carte des hôtes et des moniteurs.
...
Utilisez le `{{last_triggered_at_epoch}}` [template variable](#template-variables) as well as a `<ID_DU_TABLEAU_DE_BORD>` and `<NOM_DU_TABLEAU_DE_BORD>` pour lier à des tableaux de bord avec des plages de temps relatives depuis le moment de l'alerte :
...
| `status`  | `status=<STATUS>`          | Statut des journaux : Erreur, Avertissement, Info, etc. |
