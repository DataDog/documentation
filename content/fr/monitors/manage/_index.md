...
| Delete    | Supprime définitivement les monitors sélectionnés.                                  |
...
**Remarque** : vous pouvez consulter vos vues enregistrées de monitors, ou encore consulter et désactiver des monitors, avec l'[application mobile Datadog][5], disponible sur l'[App Store d'Apple][9] et le [Google Play Store][10].

### Monitors déclenchés

Vous pouvez [désactiver][3] ou [résoudre][4] plusieurs monitors à la fois depuis la page [Triggered Monitors][6]. Cette page affiche uniquement les monitors actuellement déclenchés (avec un statut Alert, Warn ou No Data).
...
Les tags de monitor fonctionnent différemment des tags envoyés par l'Agent ou les intégrations. Ajoutez jusqu'à 32 tags directement à vos monitors depuis les pages [Manage Monitors][1], [Triggered Monitors][6] et [Manage Downtime][7] afin de les filtrer ultérieurement. Pour en savoir plus sur les tags de monitor, consultez la [rubrique IU de la section Assigner des tags][8].

**Remarque** : les tags de monitor sont ajoutés à l'événement d'alerte généré par le monitor, avec une limite de 25 tags par événement d'alerte.
