...
- link: /account_management/billing/metric_name_pricing/
  tag: Documentation
  text: Tarification des noms de métriques pour les métriques personnalisées
...
**Remarque** : Cette page décrit le modèle de facturation des métriques personnalisées basé sur la cardinalité. Si votre contrat utilise des SKU de tarification des noms de métriques, consultez plutôt [Tarification des noms de métriques pour les métriques personnalisées][15].

Si une métrique n'est pas soumise par l'une des [plus de {{< translate key="integration_count" >}} intégrations Datadog][1], elle est considérée comme une [métrique personnalisée][2]. Certaines intégrations standard peuvent également potentiellement émettre des métriques personnalisées. Pour plus d'informations, consultez [métriques personnalisées et intégrations standard][14].

**Une métrique personnalisée est identifiée de manière unique par une combinaison d'un nom de métrique et de valeurs d'étiquette (y compris l'étiquette hôte)**. En général, toute métrique que vous envoyez en utilisant [DogStatsD][3] ou via un [Agent Check personnalisé][4] est une métrique personnalisée.
...
Les utilisateurs de Metrics without Limits™ voient les volumes facturables mensuels pour _ingérées_ et _indexées_ custom metrics sur leur Usage page. En savoir plus sur les métriques personnalisées ingérées et indexées et sur [Metrics without Limits™][5]. 
...
Supposons que vous soumettiez une métrique, `request.Latency`, depuis deux hôtes (`host:A`,`host:B`), qui mesure la latence de vos requêtes d'endpoint. Vous soumettez cette métrique avec deux clés de tag :

- `endpoint`, qui a la valeur `endpoint:X` ou `endpoint:Y`.
- `status`, qui a la valeur `status:200` ou `status:400`.

Supposons que dans vos données, `endpoint:X` est pris en charge par les deux hôtes, mais échoue uniquement sur `host:B`. Supposons également que les requêtes à `endpoint:Y` sont toujours réussies et n'apparaissent que sur `host:B` comme indiqué ci-dessous :

{{< img src="account_management/billing/custom_metrics/request_latency.png" alt="Latence de la requête" style="width:80%;">}}
...
Le nombre de combinaisons de valeurs de tag uniques soumises pour une métrique de RATE avec ce schéma de tag est **quatre** :
...
Cela donne lieu à `request.Latency` rapportant **quatre métriques personnalisées**. 

### Effet de l’ajout de tags {#effect-of-adding-tags}

L'ajout de balises **peut ne pas** entraîner plus de métriques personnalisées. Votre nombre de métriques personnalisées évolue généralement avec le tag le plus granulaire ou détaillé. Supposons que vous mesuriez la température aux États-Unis, et que vous ayez tagué votre métrique `temperature` par pays et région. Vous soumettez ce qui suit à Datadog :

| Nom de la métrique   | Valeurs de tags                         |
|---------------|------------------------------------|
| `temperature` | `country:USA`, `region: Northeast` |
| `temperature` | `country:USA`, `region: Southeast` |

Supposons que vous souhaitiez ajouter le tag `city` qui a trois valeurs : `NYC`, `Miami` et `Orlando`. Ajouter ce tag augmente le nombre de métriques personnalisées car il fournit plus de détails et de granularité à votre ensemble de données comme indiqué ci-dessous :
...
Le nombre de métriques personnalisées rapportant depuis `temperature` évolue avec le tag le plus granulaire, `city`.

Supposons que vous souhaitiez également taguer votre métrique de température par `state` (qui a deux valeurs : `NY` et `Florida`). Cela signifie que vous taguez la température avec les tags : `country`, `region`, `state` et `city`. Ajouter le tag d'état n'augmente pas le niveau de granularité déjà présent dans votre ensemble de données fourni par le tag de ville.
...
- **Métriques personnalisées ingérées** : Le volume original de métriques personnalisées basé sur tous les tags ingérés (envoyés via le code)
...
**Remarque : Seules les métriques configurées contribuent à votre volume de métriques personnalisées ingérées.** Si une métrique n'est pas configurée avec Metrics without Limits™, vous ne payez que pour le volume de ses métriques personnalisées indexées.

#### Quand êtes-vous facturé pour les métriques personnalisées ingérées par rapport aux métriques personnalisées indexées ? {#when-are-you-charged-for-ingested-vs-indexed-custom-metrics}
...
Supposons que vous souhaitiez utiliser Metrics without Limits™ pour réduire la taille de votre `request.Latency` métrique en ne conservant que les tags `endpoint` et `status`. Cela donne les trois combinaisons uniques de tags suivantes :

- `endpoint:X`, `status:200`
- `endpoint:X`, `status:400`
- `endpoint:Y`, `status:200`

En raison de la configuration des tags, `request.Latency` rapportant un total de **3 métriques personnalisées indexées** . Sur la base des tags originaux envoyés sur cette métrique, le volume original de **métriques personnalisées ingérées** de `request.Latency` est **4 métriques personnalisées ingérées**.
...
{{% /tab %}}
{{% tab "Gauge" %}}
Le nombre de combinaisons uniques de valeurs de tags soumises pour une métrique GAUGE avec ce schéma de tagging est **quatre** :
...
### Effet de l'ajout de tags {#effect-of-adding-tags-1}

L'ajout de tags **peut ne pas** entraîner plus de métriques personnalisées. Votre nombre de métriques personnalisées évolue généralement avec le tag le plus granulaire ou détaillé. Supposons que vous mesuriez la température aux États-Unis, et que vous ayez tagué votre métrique `temperature` par pays et région. Vous soumettez ce qui suit à Datadog :
...
Le nombre de métriques personnalisées remontées depuis `temperature` évolue en fonction du tag le plus granulaire, `city`.

Supposons que vous souhaitiez également taguer votre métrique de température par `state` (qui a deux valeurs : `NY` et `Florida`). Cela signifie que vous taguez la température par `country`, `region`, `state` et `city`. Ajouter le tag d'état n'augmente pas le niveau de granularité déjà présent dans votre ensemble de données fourni par le tag de ville.
...
Les volumes de métriques personnalisées peuvent être affectés par la configuration des tags en utilisant [Metrics without Limits™][4]. Metrics without Limits™ découple les coûts d'ingestion des coûts d'indexation – vous pouvez donc continuer à envoyer à Datadog toutes vos données (tout est ingéré) et vous pouvez spécifier une liste blanche de tags que vous souhaitez garder interrogeables sur la plateforme Datadog. Étant donné que le volume de données que Datadog ingère pour vos métriques configurées diffère maintenant du volume plus petit restant que vous avez indexé, vous verrez deux volumes distincts sur votre page d'utilisation ainsi que sur la page de résumé des métriques. 
...
#### Quand êtes-vous facturé pour les métriques personnalisées ingérées par rapport aux métriques personnalisées indexées ? {#when-are-you-charged-for-ingested-vs-indexed-custom-metrics-1}
Pour les métriques non configurées avec Metrics without Limits™, vous payez pour les métriques personnalisées indexées.
...
Votre nombre de métriques personnalisées indexées **n'augmente pas avec le nombre d'agrégations activées.**
...
**Une métrique HISTOGRAM génère par défaut cinq métriques personnalisées pour chaque combinaison unique de nom de métrique et de valeurs de tag** pour soutenir les agrégations côté Agent `max`, `median`, `avg`, `95pc` et `count`. [En savoir plus sur le type de métrique HISTOGRAM][1].

Le nombre de combinaisons uniques de valeurs de tag soumises pour une métrique HISTOGRAM avec ce schéma de tag est **quatre** :
...
Par défaut, l'Agent génère cinq métriques personnalisées pour chacune des quatre combinaisons uniques de valeurs de tag d'origine afin de tenir compte [de chaque agrégation côté Agent activée][2] : `avg`, `count`, `median`, `95percentile` et `max`. En conséquence, `request.Latency` rapporte un total de **4\*5 = 20 métriques personnalisées**.

**Remarque** : Ajouter des agrégations à vos métriques HISTOGRAM augmente le nombre de métriques personnalisées distinctes rapportées. Supprimer des agrégations diminue le nombre de métriques personnalisées rapportées.

- Configurez quelle agrégation vous souhaitez envoyer à Datadog avec le paramètre `histogram_aggregates` dans votre [fichier de configuration datadog.yaml][3]. Par défaut, seules les agrégations `max`, `median`, `avg` et `count` sont envoyées à Datadog. `sum` et `min` sont également disponibles si nécessaire.
- Configurez quelle agrégation de percentile vous souhaitez envoyer à Datadog avec le paramètre `histogram_percentiles` dans votre [fichier de configuration datadog.yaml][3]. Par défaut, seul le `95percentile`, le 95e percentile, est envoyé à Datadog.
...
Le nombre de combinaisons uniques de valeurs de tag soumises pour une métrique de DISTRIBUTION avec ce schéma de tag est **quatre**.
...
Le nombre de métriques personnalisées d'une [métrique de DISTRIBUTION][1] est cinq fois la combinaison unique de nom de métrique et de valeurs de tag. Cela donne lieu à `request.Latency` rapportant un total de **5\*4 = 20 métriques personnalisées**.

##### Ajouter des agrégations de percentile {#adding-percentile-aggregations}

Vous pouvez inclure des agrégations de percentile (`p50`, `p75`, `p90`, `p95` et `p99`) sur votre métrique de distribution. Inclure ces agrégations de percentile supplémentaires entraîne un volume supplémentaire de cinq fois la combinaison unique de nom de métrique et de valeurs de tag (**5\*4 = 20 métriques personnalisées**). Par conséquent, le nombre total de métriques personnalisées émises par cette métrique de distribution avec des agrégations de percentile est **2 * (5\*4) = 40 métriques personnalisées**.
...
|-------------------------------------------------------------------------------------------|-----------------------------------|
| Nombre de métriques personnalisées à partir d'une distribution de référence (compte, somme, min, max, moyenne)         | `5*(tag value combinations)`      |
| Nombre de métriques personnalisées incluant les agrégations de percentile (p50, p75, p90, p95, p99) | `5*(tag value combinations)`      |
...
Les volumes de métriques personnalisées peuvent être impactés par la configuration des tags et des agrégations en utilisant [Metrics without Limits™][2]. Metrics without Limits™ découple les coûts d'ingestion des coûts d'indexation -- vous pouvez donc continuer à envoyer à Datadog toutes vos données (tout est ingéré) et vous pouvez spécifier une liste blanche de tags que vous souhaitez garder interrogeables sur la plateforme Datadog. Étant donné que le volume de données que Datadog ingère pour vos métriques configurées diffère maintenant du volume plus petit restant que vous avez indexé, vous verrez deux volumes distincts sur votre page d'utilisation ainsi que sur la page de résumé des métriques. 
 
- **Métriques personnalisées ingérées** : Le volume original de métriques personnalisées basé sur toutes les balises ingérées (envoyées par code)
- **Métriques personnalisées indexées** : Le volume de métriques personnalisées qui reste interrogeable sur la plateforme Datadog (basé sur toutes les configurations de Metrics without Limits™) 
...
Pour les métriques configurées avec Metrics without Limits™ (tags/agrégations configurés), vous payez pour les métriques personnalisées ingérées et les métriques personnalisées indexées.
...
Supposons que vous souhaitiez conserver uniquement les balises `endpoint` et `status` associées à la métrique `request.Latency`. Cela donne les trois combinaisons uniques de tags suivantes :

- `endpoint:X`, `status:200`
- `endpoint:X`, `status:400`
- `endpoint:Y`, `status:200`

Le nombre de métriques personnalisées d'une [métrique de DISTRIBUTION][1] est cinq fois la combinaison unique de nom de métrique et de valeurs de balise. En raison de la personnalisation des balises, `request.Latency` indiquant un total de **5\*3 = 15 métriques personnalisées indexées**. Sur la base des balises originales envoyées pour cette métrique, le volume original de métriques personnalisées **ingérées** de `request.Latency` correspond à **20 métriques personnalisées ingérées**.
...
Les utilisateurs administratifs (ceux ayant des [rôles d'administrateur Datadog][7]) peuvent voir le nombre moyen mensuel de **métriques ingérées** et **métriques indexées** par heure. Le tableau des meilleures métriques personnalisées indique également le nombre moyen de **métriques indexées** sur la [page des détails d'utilisation][8]. Consultez la documentation [Détails d'utilisation][9] pour plus d'informations.

Pour un suivi en temps réel du nombre de métriques personnalisées pour un nom de métrique particulier, cliquez sur le nom de la métrique sur la [page de résumé des métriques][10]. Vous pouvez voir le nombre de **métriques ingérées** et de **métriques indexées** dans le panneau latéral des détails de la métrique. 
...
Vous êtes alloué un certain nombre de **métriques ingérées** et de **métriques indexées** en fonction de votre plan tarifaire Datadog :
