...
{{< img src="graphing/infrastructure/index/infrastructure_list.png" alt="Liste des infrastructures" responsive="true" >}}
...
{{< img src="graphing/infrastructure/index/infrastructure_list_host_details.png" alt="Liste des infrastructures - Détails d'un host" responsive="true" style="width:90%;">}}
...
{{< img src="graphing/infrastructure/index/host_aliases.png" alt="alias de host" responsive="true" style="width:90%;">}}

### Exportation

Pour obtenir la liste au format JSON des hosts transmettant des données à Datadog, utilisez le **permalien API JSON** en bas de la liste des infrastructures.

#### Version de l'Agent

Il peut également s'avérer utile de passer en revue les versions de vos Agents pour vérifier qu'ils sont tous bien à jour. Pour ce faire, utilisez le [script get_host_agent_list][4] avec le permalien JSON pour obtenir la liste des Agents en cours d'exécution et leur version. Un script `json_to_csv` est également disponible pour convertir la sortie JSON en fichier CSV.

#### Agent non installé

L'exportation au format JSON permet également d'obtenir la liste des instances AWS EC2 sur lesquelles aucun Agent n'est installé. Pour faire apparaître ces instances dans la liste des infrastructures, configurez votre compte AWS dans le carré de l'intégration Datadog/AWS. Vous pouvez vous référer à cet [exemple de script][5].
