...
Pour tirer le meilleur parti de l'intégration AWS X-Ray, activez-la _sur_ vos fonctions Lambda et vos instances d'API Gateway, et installez **également** les bibliothèques de tracing _dans_ vos fonctions Lambda.

#### Plug-in Serverless Framework

Le [plug-in Serverless Framework de Datadog][6] active automatiquement X-Ray pour vos fonctions Lambda et vos instances d'API Gateway. Il se charge également d'ajouter la [couche Lambda Datadog][7] à toutes vos fonctions Node et Python.

Pour apprendre à utiliser le plug-in Serverless Framework, [lisez cet article de blog][8] et [consultez la documentation][6] (en anglais).
...
2. Accédez à la [console API Gateway][9]. Sélectionnez votre API ainsi que l'étape. Depuis l'onglet **Logs/Tracing**, sélectionnez ensuite **Enable X-Ray Tracing**. Pour que ces changements prennent effet, accédez à **Resources** dans le volet de navigation sur la gauche et sélectionnez **Actions**, puis cliquez sur **Deploy API**.
...
Les bibliothèques Lambda de Datadog permettent d'ajouter des métadonnées supplémentaires aux segments X-Ray. Ces métadonnées sont alors consultables dans les traces de l'APM et dans la [vue Serverless][10]. Pour instrumenter vos fonctions Lambda avec les bibliothèques Lambda de Datadog, consultez les [instructions détaillées pour chaque runtime et outil de déploiement][11] ou suivez les instructions d'installation personnalisées ci-dessous :

#### Fonctions Lambda Python

- Importez la bibliothèque Lambda de Datadog en tant que couche ou package en suivant les [instructions détaillées][12].
...
- (Facultatif) Si vous n'utilisez pas la [fusion de traces][13], définissez la variable d'environnement `DD_TRACE_ENABLED` sur `false`.

#### Fonctions Lambda Node

- Importez la bibliothèque Lambda de Datadog en tant que couche ou package en suivant les [instructions détaillées][14].
...
**Remarque**: le tracing de `aiohttp` nécessite une [instrumentation spécifique][2].
...
[6]: https://github.com/DataDog/serverless-plugin-datadog
[7]: https://docs.datadoghq.com/fr/integrations/amazon_lambda/?tab=python#installing-and-using-the-datadog-layer
[8]: https://www.datadoghq.com/blog/serverless-framework-plugin
[9]: https://console.aws.amazon.com/apigateway/
[10]: https://docs.datadoghq.com/fr/serverless/troubleshooting/connect_invoking_resources
[11]: https://docs.datadoghq.com/fr/serverless/installation
[12]: https://docs.datadoghq.com/fr/serverless/installation/python/?tab=custom
[13]: https://docs.datadoghq.com/fr/serverless/distributed_tracing/serverless_trace_merging
[14]: https://docs.datadoghq.com/fr/serverless/installation/nodejs/?tab=custom