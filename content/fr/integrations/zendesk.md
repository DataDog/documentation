...
2. Ajoutez une nouvelle cible avec l'argument URL : `https://http-intake.logs.datadoghq.com/v1/input/<VOTRE_CLÉ_API_DATADOG>?ddsource=zendesk` avec **Method**: POST et **Content Type**: JSON.
...
2. Ajoutez une nouvelle cible avec l'argument URL : `https://http-intake.logs.datadoghq.eu/v1/input/<VOTRE_CLÉ_API_DATADOG>?ddsource=zendesk` avec **Method**: POST et **Content Type**: JSON.
...
Vous avez la possibilité de créer des tickets Zendesk et de les assigner à un groupe. Ajoutez d'abord le nom du groupe dans le [carré d'intégration Zendesk][2] Datadog, puis utilisez `@zendesk-nom-du-groupe` dans vos annotations et vos monitors Datadog. Par exemple, pour créer un ticket et l'assigner au groupe Zendesk *Support*, ajoutez le groupe et utilisez `@zendesk-support`,
