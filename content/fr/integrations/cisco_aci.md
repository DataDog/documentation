...
1. Modifiez le fichier `cisco_aci.d/conf.yaml` dans le dossier `conf.d/` à la racine du [répertoire de configuration de votre Agent][2].
    Consultez le [fichier d'exemple cisco_aci.d/conf.yaml][3] pour découvrir toutes les options de configuration disponibles.
...
2. [Redémarrez l'Agent][4] pour commencer à envoyer vos métriques Cisco ACI à Datadog.

### Validation

[Lancez la sous-commande `status` de l'Agent][5] et cherchez `cisco_aci` dans la section Checks.
