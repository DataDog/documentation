...
Ce document fait référence à l'intégration Slack standard, qui repose sur un webhook, ainsi qu'à l'application Slack pour Datadog, qui est disponible en version bêta.

Pour les clients non européens qui utilisent Slack, <a href="https://app.datadoghq.com/incidents/ddslackapp">demandez à accéder à la version bêta</a> de l'application Slack pour Datadog. Pour les clients européens qui utilisent Slack, envoyez un e-mail à support@datadoghq.com pour en savoir plus sur l'application Slack.
</div>
...
{{% tab "Application Slack (bêta)" %}}

### Installation

Installez l'application Slack pour Datadog dans votre espace de travail Slack :

1. Accédez au [carré d'intégration][1] Slack sous Integrations dans l'application Datadog. Cliquez sur le bouton « Connect Slack Account » en bas du carré.

2. Vérifiez que votre compte Slack et votre compte Datadog sont associés. **Remarque** : vous aurez peut-être besoin de l'approbation de l'administrateur de votre espace de travail pour cette modification, mais vous n'aurez à le faire qu'une seule fois. 
...
<div class="alert alert-warning">
La section suivante s'applique uniquement à l'application Slack pour Datadog, qui est disponible en version bêta.
...
Une fonctionnalité supplémentaire vous permet d'intégrer des composants de Datadog dans Slack à l'aide d'une commande.
...
Vous pouvez également copier et coller n'importe quel widget à partir de Datadog vers Slack à l'aide de `CMD + C` ou `CTRL + C`, ou en cliquant sur l'icône d'agrandissement située en haut d'un widget puis en sélectionnant « copy ». Lorsque vous collez un widget, celui-ci apparaît en-dessous d'un lien une fois envoyé dans un canal.

### Connexion de monitors

Vous pouvez désactiver le déclenchement de monitors dans Slack d'un seul clic sur un bouton.
...
**Méthode manuelle** : si vous souhaitez tester cette fonctionnalité avant de la déployer à toutes vos équipes, ajoutez manuellement les canaux à la configuration de compte pour la nouvelle application dans la configuration de l'intégration Slack. 
...
Pour prévenir tous les utilisateurs d'un groupe lorsqu'un message est envoyé, utilisez `<!subteam^ID_GROUPE>`.
