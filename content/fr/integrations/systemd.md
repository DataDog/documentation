...
Notez que si vous utilisez l'[Agent dans un conteneur][8], vous devez monter le dossier `/run/systemd/`. Il contient le socket `/run/systemd/private` nécessaire pour recueillir les données en provenance de Systemd. Par exemple :
...
Renvoie `OK` si l'état du système de Systemd est running, renvoie `CRITICAL` si l'état est degraded, maintenance ou stopping, ou renvoie `UNKNOWN` pour un état initializing, starting ou autre.
...
[8]: https://docs.datadoghq.com/fr/agent/docker/
