...
Dans le [Parameter Store][8], créez un paramètre ayant les propriétés suivantes :
...
Dans le Systems Manager, créez un [Document][10] :
...
Si vous utilisez le site américain de Datadog, utilisez le fichier [dd-agent-install-us-site.json][11] après y avoir spécifié votre `<RÉGION_AWS>` sous `runCommand` (p. ex., `us-east-1`). Si vous êtes sur le site européen de Datadog, utilisez plutôt le fichier [dd-agent-install-eu-site.json][12].
...
**Remarque** : `aws.ec2.instance_age` n'est pas recueillie par défaut avec l'intégration Datadog/EC2. [Contactez-nous][18] pour activer la collecte de cette métrique.
...
- [Métriques clés pour la surveillance EC2][19]
- [Comment recueillir des métriques EC2][20]
- [Comment surveiller des instances EC2 avec Datadog][21]
