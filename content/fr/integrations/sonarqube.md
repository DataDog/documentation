...
Le serveur JMX de SonarQube n'est pas activé par défaut. Tant que vous ne l'activez pas, les métriques `sonarqube.server.*` ne sont pas recueillies. Pour obtenir plus d'informations concernant l'activation et la configuration de JMX dans SonarQube, consultez la documentation [SonarQube][5] (en anglais).

Voici un exemple de fichier `sonarqube.d/conf.yaml` de base, reposant sur les valeurs par défaut de SonarQube et JMX. Vous pouvez l'utiliser comment point de départ lorsque vous configurez l'installation de l'Agent, qu'il soit basé sur des hosts ou des conteneurs.
...
De plus, SonarQube expose un serveur de recherche qui peut être surveillé en utilisant une instance supplémentaire de cette intégration et en configurant les métriques JMX à recueillir. Pour obtenir des instructions détaillées afin de personnaliser les métriques à recueillir, consultez la [documentation relative aux checks JMX][6]. Pour vous aider, vous pouvez utiliser l'exemple de configuration ci-dessous ainsi que la configuration de métriques JMX par défaut dans [sonarqube.d/metrics.yaml][3].
    ...
    - include:
      ...
      attribute:
        MyMetric:
          ...
          metric_type: gauge
instances:
  # Instance du serveur de recherche JMX
  - is_jmx: true
    host: localhost
    port: 10445
    user: <nom_utilisateur>      # Défini dans le fichier sonar.properties de SonarQube
