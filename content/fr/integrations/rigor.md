...
Grâce à Rigor, vous pouvez collecter des métriques de performance synthétique frontales et transmettre
ces métriques à Datadog. Vous pouvez également transmettre des alertes à Datadog sous forme d'événements.

## Implémentation

Rigor propose deux intégrations différentes avec Datadog : une intégration pour les métriques et
une pour les événements.

### Configurer l'intégration des métriques
En tant qu'administrateur, cliquez sur le menu « Admin Tools » en haut à droite de votre
écran et sélectionnez « Integrations ».

![menu administrateur][2]

Ajoutez une nouvelle intégration en cliquant sur le bouton « New ». Vous pourrez alors configurer
cette intégration.

![configuration du Push][3]

Donnez un nom unique à cette intégration et ajoutez votre clé d'API de Datadog. Sélectionnez ensuite les
tags et métriques que vous souhaitez envoyer. Voici quelques notions à retenir :
...
est susceptible d'entraîner l'envoi de nombreuses séries à Datadog, en particulier si les marques et les mesures
du site que vous testez sont générées de manière dynamique.

Une fois l'intégration configurée, vous pouvez l'ajouter à tous les checks d'API, de port, HTTP
ou Real Browser. Il vous suffit de modifier le check et d'accéder à l'onglet « Notifications ».
Vous pourrez alors ajouter l'intégration créée.
...
En tant qu'administrateur, cliquez sur le menu « Admin Tools » en haut à droite de votre
écran et sélectionnez « Alert Webhooks ».
...
Donnez nom unique à ce Webhook et assurez-vous de mettre à jour les déclencheurs avec votre clé d'API
Datadog.
...
Une fois l'intégration configurée, vous pouvez l'ajouter à tous les checks d'API, de port, HTTP
ou Real Browser.  Il vous suffit de modifier le check et d'accéder à l'onglet « Notifications ».
Vous pourrez alors ajouter le Webhook créé.
...
Toutes les métriques Rigor peuvent être envoyées à Datadog. La configuration de votre intégration
permet de sélectionner les données qui seront envoyées. Les métriques qui peuvent être envoyées sont les suivantes :
...
De plus, si l'intégration est configurée, les indicateurs User Timings du navigateur seront envoyés dans les espaces de nommage
`rigor.real_browser.marks` et `rigor.real_browser.measures`.

### Événements

Lorsqu'un check est configuré de façon à vous avertir via un événement Datadog, deux types d'événements sont envoyés
à Datadog :
- **Failed** : une alerte est envoyée à chaque fois que le check échoue en dépassant
  le seuil défini.
...
Consultez le site [Rigor][12] pour obtenir plus d'informations sur Rigor et sur la façon
dont nous pouvons contribuer à optimiser les performances de votre site web.
