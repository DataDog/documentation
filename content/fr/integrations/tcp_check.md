...
Le check TCP est inclus avec le paquet de [l'Agent Datadog][2] : vous n'avez donc rien d'autre à installer sur les hosts à partir desquels vous souhaitez sonder vos ports TCP. Bien qu'il soit recommandé d'exécuter bon nombre des checks axés sur des métriques sur le même host que celui du service surveillé, il est préférable de lancer ce check depuis un host qui n'exécute pas les services TCP surveillés (par exemple pour tester la connectivité à distance).
...
* `timeout` (facultatif) : L'expiration du check. Sa valeur par défaut est de 10 secondes.
...
[Lancez la sous-commande `status` de l'Agent][6] et cherchez `tcp_check` dans la section Checks.
...
Pour créer des conditions d'alerte sur ce check de service dans Datadog, sélectionnez **Network** sur la page [Create Monitor][8], et non **Integration**.
