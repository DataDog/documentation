...
{{< img src="actions/workflows/if-condition.png" alt="Le canevas du workflow avec une action if condition sélectionnée et l'onglet de configuration ouvert. La section Statements est mise en évidence avec deux instructions spécifiant que le statut d'une requête précédente doit être 200." >}} 
...
{{< img src="actions/workflows/switch-statement.png" alt="Le canevas du workflow montrant une action Switch statement nommée 'Make_request.status' se ramifiant en plusieurs cas pour différents codes de statut HTTP. Chaque cas envoie un message Slack différent, et la branche par défaut génère une erreur inattendue si aucun autre cas ne correspond." >}}
...
{{< img src="actions/workflows/iteration2.png" alt="Un workflow avec une boucle for. La boucle itère sur une liste d'incidents et envoie un message à un canal slack si l'incident a plus d'une semaine." style="width:100%;" >}}
...
{{< img src="actions/workflows/iteration3.png" alt="Un workflow avec une boucle while. Le workflow utilise une boucle while pour paginer l'API AWS S3 List Buckets pour une application." style="width:100%;" >}}
