...
---
Alors que StatsD n'accepte que des métriques, DogStatsD accepte les trois principaux types de données Datadog : métriques, événements et vérifications de service. Cette section montre des cas d'utilisation typiques pour les métriques, classés par types de métriques, et introduit les [taux d'échantillonnage](#samplerates) et les options de [tagging de métriques](#metrictagging) spécifiques à DogStatsD.

Les types de métriques [COUNT](#count), [GAUGE](#gauge) et [SET](#set) sont familiers aux utilisateurs de StatsD. `TIMER` de StatsD est un sous-ensemble de `HISTOGRAM` dans DogStatsD. De plus, vous pouvez soumettre des types de métriques [HISTOGRAM](#histogram) et [DISTRIBUTION](#distribution) en utilisant DogStatsD.

**Remarque** : Selon la méthode de soumission utilisée, le type de métrique réel stocké dans Datadog peut différer du type de métrique soumis. Lors de la soumission d'un type de métrique RATE via DogStatsD, la métrique apparaît comme un GAUGE dans l'application pour garantir une comparaison pertinente entre différents Agents.
...
| `<SAMPLE_RATE>`  | Double          | Non      | Le taux d'échantillonnage à appliquer à la métrique. Prend une valeur entre `0` (tout est échantillonné, donc rien n'est envoyé) et `1` (aucun échantillon). Voir la section [Taux d'échantillonnage](#samplerates) pour en savoir plus. |
| `<TAGS>`         | Liste de chaînes | Non      | Une liste d'étiquettes à appliquer à la métrique. Voir la section [Étiquetage des métriques](#metrictagging) pour en savoir plus.                                                                                       |
| `<CARDINALITY>`  | Enum            | Non      | La [cardinalité][10] des étiquettes à attribuer à cette métrique.                                                                                                                               |

### COMPTE

`incrémenter(<METRIC_NAME>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY> )`
: Utilisé pour incrémenter une métrique de COMPTE. Stocké en tant que type `TAUX` dans Datadog. Chaque valeur dans la série temporelle stockée est un delta normalisé dans le temps de la valeur de la métrique sur la période de vidage StatsD.

`décrémenter(<METRIC_NAME>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY>)`
: Utilisé pour décrémenter une métrique de COMPTE. Stocké en tant que type `TAUX` dans Datadog. Chaque valeur dans la série temporelle stockée est un delta normalisé dans le temps de la valeur de la métrique sur la période de vidage StatsD.

`compte(<METRIC_NAME>, <METRIC_VALUE>, <SAMPLE_RATE>, <TAGS>, <CARDINALITY>)`
: Utilisé pour incrémenter une métrique de COMPTE à partir d'une `Valeur` arbitraire. Stocké en tant que type `TAUX` dans Datadog. Chaque valeur dans la série temporelle stockée est un delta normalisé dans le temps de la valeur de la métrique sur la période de vidage StatsD.

**Remarque** : Les métriques de type `COMPTE` peuvent afficher une valeur décimale dans Datadog car elles sont normalisées sur l'intervalle de vidage pour rapporter des unités par seconde.

#### Exemples de code

Émettre une métrique de `COMPTE` stockée en tant que métrique de `TAUX` dans Datadog. En savoir plus sur le type `COUNT` dans la documentation des [types de métriques][2].

Exécutez le code suivant pour soumettre une métrique `COUNT` DogStatsD à Datadog. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.
...
Émettez une métrique `GAUGE` stockée en tant que métrique `GAUGE` vers Datadog. En savoir plus sur le type `GAUGE` dans la documentation [types de métriques][5].

Exécutez le code suivant pour soumettre une métrique `GAUGE` DogStatsD à Datadog. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.
...
: Stocké en tant que type `GAUGE` dans Datadog. Chaque valeur dans la série temporelle stockée est le nombre de valeurs uniques soumises à StatsD pour une métrique pendant la période de vidage.

#### Exemples de code

Émettez une métrique `SET` stockée en tant que métrique `GAUGE` vers Datadog.

Exécutez le code suivant pour soumettre une métrique `SET` DogStatsD à Datadog. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.
...
Exécutez le code suivant pour soumettre une métrique `HISTOGRAM` DogStatsD à Datadog. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.
...
| `example_metric.histogram.median`       | Valeur médiane échantillonnée                    |
| `example_metric.histogram.max`          | Valeur maximale échantillonnée                   |
...
Le type de métrique `TIMER` dans DogStatsD est une implémentation du type de métrique `HISTOGRAM` (à ne pas confondre avec les temporisateurs dans le StatsD standard). Il mesure uniquement les données de timing : par exemple, le temps qu'une section de code met à s'exécuter.
...
Pour un `TIMER`, les règles de [configuration](#configuration) du `HISTOGRAM` s'appliquent.

##### Exemples de code

Émettre une métrique `TIMER`—stockée en tant que métrique `GAUGE` et `RATE`—vers Datadog. En savoir plus sur le type `HISTOGRAM` dans la documentation des [types de métriques][6]. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.

{{< programming-lang-wrapper langs="python,PHP" >}}

{{< programming-lang lang="python" >}}

En Python, les temporisateurs sont créés avec un décorateur.
...
Alors que DogStatsD reçoit les données de métriques de temporisation, il calcule la distribution statistique des temps de rendu et envoie les métriques suivantes à Datadog :
...
| `example_metric.timer.median`        | Valeur médiane échantillonnée                    |
| `example_metric.timer.max`           | Valeur maximale échantillonnée                   |
...
{{< img src="metrics/custom_metrics/dogstatsd_metrics_submission/timer.png" alt="Temporisateur" >}}

### DISTRIBUTION

`distribution(<METRIC_NAME>, <METRIC_VALUE>, <TAGS>, <CARDINALITY>)`
: Stocké en tant que type `DISTRIBUTION` dans Datadog. Consultez la [documentation sur les distributions][8] dédiée pour en savoir plus.
...
Exécutez le code suivant pour soumettre une métrique `DISTRIBUTION` DogStatsD à Datadog. N'oubliez pas de `flush`/`close` le client lorsqu'il n'est plus nécessaire.
...
| `GAUGE`         | Pas de correction. La valeur reçue est conservée telle quelle.                                                                                                                               |
| `SET`          | Pas de correction. La valeur reçue est conservée telle quelle.                                                                                                                               |
...
| `DISTRIBUTION` | Les valeurs reçues sont comptées (`1/<SAMPLE_RATE>`) fois. Il est raisonnable de supposer que pour un point de données reçu, `1/<SAMPLE_RATE>` ont en fait été échantillonnés avec la même valeur. |
...
### Tagging des métriques

Ajoutez des tags à toute métrique que vous envoyez à DogStatsD avec le paramètre `tags`.

#### Exemples de code

Le code suivant ajoute seulement les tags `environment:dev` et `account:local` à la métrique `example_metric.increment` :
...
Le tag d'hôte est attribué automatiquement par l'Agent Datadog qui agrège les métriques. Les métriques soumises avec un tag d'hôte ne correspondant pas au nom d'hôte de l'Agent perdent la référence à l'hôte d'origine. Le tag d'hôte soumis remplace tout nom d'hôte collecté par ou configuré dans l'Agent.
...
[3]: /fr/dashboards/functions/arithmetic/#cumulativesum
[4]: /fr/tableaux de bord/fonctions/arithmetic/#integral
[5]: /fr/métriques/types/?tab=gauge#définition
[6]: /fr/métriques/types/?tab=histogram#définition
[7]: /fr/agent/configuration/fichiersdeconfigurationagent/#fichierdeconfigurationprincipalagent
[8]: /fr/métriques/distributions/
[9]: /fr/métriques/types/?tab=distribution#définition
[10]: /fr/conteneurs/kubernetes/tag