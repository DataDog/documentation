...
- En outre, l'[Agent Datadog][21] envoie automatiquement plusieurs métriques standard, comme l'utilisation du CPU ou du disque.
...
### Différents éléments d'une requête de métriques
...
#### 2. Filtrage de la métrique

Une fois votre métrique sélectionnée, vous pouvez filtrer votre requête à l'aide de tags. Par exemple, utilisez le tag `account:prod` afin d'appliquer un _contexte_ à votre requête. Ainsi, celle-ci inclut uniquement les métriques provenant de vos hosts de production. Pour en savoir plus, consultez la documentation relative au [tagging][15].
...
#### 5. Application de fonctions (facultatif)

Vous pouvez modifier les valeurs de votre graphique à l'aide de [fonctions][16] mathématiques. Il est possible d'appliquer des opérations arithmétiques à une métrique (p. ex., en la multipliant par deux) ou plusieurs (p. ex., en créant une nouvelle série temporelle pour le taux d'utilisation de la mémoire avec l'opération `jvm.heap_memory / jvm.heap_memory_max`).
...
Les types de métriques déterminent les graphiques et fonctions disponibles dans l'app. Consultez la section [Types de métriques][14] pour obtenir des exemples détaillés pour chaque type de métrique, ainsi que des instructions pour leur envoi.

### Comment afficher les informations en temps réel sur des métriques ?

La [page Metrics Summary][18] affiche la liste de vos métriques transmises à Datadog pendant un intervalle donné : l'heure précédente, le jour précédent ou la semaine précédente. Les métriques peuvent être filtrées en fonction de leur nom ou d'un tag.
...
Remarque : le nombre de métriques distinctes indiqué dans le volet latéral détaillé de la page Metrics Summary ne correspond pas au nombre de métriques facturées. Accédez à vos [informations d'utilisation][19] pour obtenir des données précises sur votre utilisation au cours du dernier mois.

Consultez la section [Metrics Summary][20] pour en savoir plus.
...
[13]: /fr/api/
[14]: /fr/developers/metrics/types/
[15]: /fr/getting_started/tagging/using_tags/
[16]: /fr/dashboards/functions/
[17]: /fr/metrics/distributions/
[18]: https://app.datadoghq.com/metric/summary
[19]: /fr/account_management/billing/usage_details/
[20]: /fr/metrics/summary/
[21]: https://docs.datadoghq.com/fr/agent/basic_agent_usage/