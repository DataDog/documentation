...
2. `k8s.node.name`, le nom du nœud Kubernetes
3. Les conventions utilisées par les fournisseurs de cloud, en fonction de la convention sémantique `cloud.provider`
4. `host.id`, l'ID unique du host
5. `host.name`, le hostname du système
6. `container.id`, l'ID du conteneur

Si aucune convention n'est présente, Datadog attribue un hostname propre au système aux charges utiles.
Sur le Collector OpenTelemetry, ajoutez le [processeur de détection des ressources][1] à vos pipelines pour une résolution efficace du hostname.
