...
aliases:
  - /fr/developers/events/dogstatsd/
---
## Envoi
...
Exécutez le code suivant pour afficher les erreurs et les exceptions dans Datadog avec un événement DogStatsD. N'oubliez pas de `flush`/`close` le client une fois sa mission accomplie.
...
statsd.event('Une erreur s'est produite', 'Message d'erreur', alert_type='error', tags=['env:dev'])
```
{{< /programming-lang >}}
...
La bibliothèque DogStatsD-PHP vous permet d'envoyer des événements directement à l'API Datadog via TCP. Cette méthode est plus lente mais aussi plus fiable que lorsque vous faites appel à l'instance DogStatsD de l'Agent, les événements étant transmis de votre application à l'Agent via UDP. Pour l'utiliser, vous devez configurer la bibliothèque avec vos [clés d'API et d'application Datadog][1] au lieu de l'instance DogStatsD locale :
