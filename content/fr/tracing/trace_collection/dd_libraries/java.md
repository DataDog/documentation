...
2. Pour exécuter votre application depuis un IDE, un script d'application Maven ou Gradle, ou `java -jar` commande, avec le Profiler Continu, le suivi des déploiements et l'injection de logs (si vous envoyez des logs à Datadog), ajoutez l'argument `-javaagent` JVM et les options de configuration suivantes, selon le cas :
    ...
    ```
    note Si vous avez un besoin urgent de réduire la taille de votre image et d'omettre des modules, vous pouvez utiliser la commande [`jdeps`][19] pour identifier les dépendances. Cependant, les modules requis peuvent changer au fil du temps, donc faites-le à vos risques et périls.

    note Lors de l'activation du traceur pour Java 24+, vous pouvez voir des avertissements liés à l'accès natif JNI ou `sun.misc.Unsafe` à l'accès mémoire. Supprimez ces avertissements en ajoutant les variables d'environnement `--illegal-native-access=allow` et `--sun-misc-unsafe-memory-access=allow` juste avant l'argument `-javaagent:/path/to/dd-java-agent.jar`. Voir [JEP 472][23] et [JEP 498][24] pour plus d'informations.
...
| ...   | ...     | Activer l'injection automatique de clé MDC pour les identifiants de trace et de span Datadog. Voir [Utilisation avancée][6] pour plus de détails. <br><br>À partir de la version 1.18.3, si [Agent de configuration à distance][16] est activé où ce service s'exécute, vous pouvez définir `DD_LOGS_INJECTION` dans l'interface utilisateur du [Catalogue de logiciels][17]. |
...
| ... | ... |   Définit un taux d'échantillonnage à la racine de la trace pour tous les services. <br><br> À partir de la version 1.18.3, si [Agent de configuration à distance][16] est activé où ce service s'exécute, vous pouvez définir `DD_TRACE_SAMPLE_RATE` dans l'interface utilisateur du [Catalogue de logiciels][17].     |
...
L'instrumentation automatique pour Java utilise les capacités d'`java-agent` instrumentation [fournies par la JVM][8]. Lorsqu'un `java-agent` est enregistré, il peut modifier les fichiers de classe au moment du chargement.
...
Si nécessaire, configurez la bibliothèque de traçage pour envoyer des données de télémétrie de performance d'application comme vous le souhaitez, y compris la configuration du Tagging de Service Unifié. Lisez [Configuration de la bibliothèque][9] pour plus de détails.
