...
| [Span analysée](#analyzed-span)              | Les spans analysées possèdent un débit de requête de 100 % et peuvent être utilisées pour effectuer des recherches, des interrogations et une surveillance dans App Analytics.                                                                          |
| [Tags de span](#span-tags)                  | Utilisez des tags de spans sous forme de paires clé-valeur pour corréler une demande dans la *Vue Trace* ou pour filtrer dans *App Analytics*.                                                                              |
...
Dans l'exemple ci-dessous, la span `rack.request` constitue la span du point d'entrée de la trace. Cela signifie que la page du service web-store affiche les ressources composées de traces ayant une span de point d'entrée nommée `rack.request.` L'exemple montre également les tags ajoutés du côté de l'application (`merchant.name`, `merchant.tier`, etc). Ces tags définis par l'utilisateur peuvent être utilisés pour rechercher et analyser des données d'APM dans [App Analytics][13].
...
La fonctionnalité App Analytics sert à filtrer les [spans analysées](#spans-analysees) en fonction de tags définis par l'utilisateur (customer_id, error_type, app_name, etc.) ou de tags d'infrastructure. Cela vous permet d'analyser en détail les requêtes Web transitant par votre service en effectuant des recherches, des représentations visuelles et une surveillance prenant en compte l'intégralité des hits, des erreurs et des mesures de latence. Cette fonctionnalité peut être activée avec la [configuration automatique][16].

{{< wistia vrmqr812sz >}}

## Span analysée

Les spans analysées possèdent un débit de requête de 100 % et peuvent être utilisées pour effectuer des recherches, des interrogations et une surveillance dans App Analytics avec les [tags](#tags-de-span) inclus dans la span. Après avoir activé App Analytics, le client de tracing analyse par défaut une span de point d'entrée pour les services Web, et offre la possibilité de [configurer des services supplémentaires][17] dans votre application. Par exemple, un service Java avec 100 demandes génère 100 spans analysées depuis ses spans `servlet.request`. Si vous définissez `DD_TRACE_ANALYTICS_ENABLED=true`, le service `web-store` analyse toutes les spans `rack.request` et les rend disponibles dans App Analytics. Pour cet exemple, vous pouvez représenter graphiquement la latence la plus élevée au 99e centile des 10 meilleurs marchands. `merchant_name` est un tag défini par l'utilisateur qui a été appliqué à la span dans l'application.
...
Pour commencer à taguer des spans dans votre application, consultez ce [guide][18].
