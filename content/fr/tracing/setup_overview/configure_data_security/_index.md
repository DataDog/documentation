...
description: Configurez la modification ou la suppression de spans par le traceur
  ou l'Agent Datadog à des fins de sécurité ou de personnalisation.
kind: documentation
title: Configurer l'Agent Datadog ou le traceur pour assurer la sécurité des données
---
## Présentation
...
## Généraliser les noms des ressources et appliquer des filtres

Datadog applique plusieurs règles de filtrage sur les spans afin d'assurer un comportement efficace par défaut en matière de sécurité et de généraliser les noms de ressources, dans le but de simplifier les opérations de regroupement lors de l'analyse. Il s'agit notamment des règles suivantes :
...
**Remarque :** vous pouvez nettoyer automatiquement plusieurs types de services à la fois. Configurez chaque type de service dans la section `obfuscation` de votre fichier `datadog.yaml`.
{{< tabs >}}
{{% tab "MongoDB" %}}
...
## Nettoyer les données sensibles de vos spans
...
Pour obtenir des informations détaillées sur les options à utiliser pour empêcher le tracing de certaines ressources, consultez la section [Ignorer les ressources non désirées dans l'APM][5].
...
En cas d'exécution dans un environnement conteneurisé, définissez plutôt `DD_APM_IGNORE_RESOURCES` sur le conteneur avec l'Agent Datadog. Pour en savoir plus, consultez la rubrique [Variables d'environnement de l'Agent APM Docker][6].
...
[6]: /fr/agent/docker/apm/?tab=standard#docker-apm-agent-environment-variables
[7]: /fr/tracing/guide/send_traces_to_agent_by_api/
[8]: /fr/tracing/setup_overview/custom_instrumentation/java/#extending-tracers
[9]: /fr/tracing/setup_overview/custom_instrumentation/ruby/?tab=activespan#post-processing-traces
