---
description: Envoyer des traces OpenTelemetry au Collector OpenTelemetry et à l'exportateur
  Datadog
further_reading:
...
datadog:
  api:
    key: "<CLÉ_API>"
```
Pour envoyer les données vers un autre [site Datadog][6], définissez également le paramètre `site` :
```
datadog:
  api:
    key: "<clé d'API>"
...
Sur chaque application instrumentée avec OpenTelemetry, définissez les attributs de ressource `deployment.environment`, `service.name` et `service.version` en utilisant [le SDK de votre langage][1]. En guise d'alternative, vous pouvez également configurer l'environnement, le nom du service et la version du service au niveau du Collector pour un tagging de service unifié. Pour ce faire, vous pouvez suivre l'[exemple de fichier de configuration][7]. L'exportateur tente automatiquement d'obtenir un hostname en vérifiant les sources suivantes dans cet ordre, et en enchaînant avec la source suivante si la source en cours n'est pas disponible ou n'est pas valide :
...
1. Téléchargez le binaire approprié depuis la [dernière version du projet GitHub][11].
...
2. Choisissez une image Docker publiée, telle que [`otel/opentelemetry-collector-contrib:latest`][12].
...
Le Collector OpenTelemetry peut être exécuté dans deux types de [scénarios de déploiement][3] :
...
Un exemple de manifeste Kubernetes complet permettant de déployer le Collector OpenTelemetry en tant que daemonset ou en tant que service autonome [est disponible ici][13]. Modifiez cet exemple en fonction de votre environnement. Les principales sections spécifiques à Datadog sont les suivantes :

1. L'exemple montre comment déployer le Collector OpenTelemetry en [mode agent via un daemonset][14], qui recueille les métadonnées pertinentes spécifiques à chaque pod et chaque nœud Kubernetes, puis transmet les données de télémétrie à un Collector OpenTelemetry en [mode autonome][15]. Ce Collector OpenTelemetry en mode autonome exporte ensuite les données vers le backend de Datadog. Voir un [schéma illustrant ce modèle de déploiement][16].
...
Pour en savoir plus et obtenir d'autres exemples de configurations possibles pour votre Collector, consultez [la documentation relative à la configuration du Collector OpenTelemetry][4] (en anglais).
...
1. Activez l'ingestion OTLP de l'Agent Datadog via gRPC en suivant les instructions fournies dans la [section dédiée][17].
...
[2]: /fr/tracing/connect_logs_and_traces/opentelemetry
...
[17]: /fr/tracing/setup_overview/open_standards/otlp_ingest_in_the_agent/