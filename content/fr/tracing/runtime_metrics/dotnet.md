...
kind: documentation
description: Consultez des statistiques supplémentaires sur les performances de votre application .NET grâce aux métriques runtime associées à vos traces.
further_reading:
...
Datadog fournit non seulement ces métriques sur votre page Service de l'APM, mais également un [dashboard de runtime .NET par défaut][6] comportant les tags `service` et `runtime-id` appliqués à ces métriques.
