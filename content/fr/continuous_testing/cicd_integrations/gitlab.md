...
Si vous avez des utilisateurs de tests différents ou des données spécifiques à votre environnement CI/CD, vous pouvez remplacer ces variables par le package NPM `-v` command. Pour plus d'informations, consultez le [README associé](https://github.com/DataDog/datadog-ci/tree/master/packages/plugin-synthetics) dans le dépôt de `datadog-ci`.
