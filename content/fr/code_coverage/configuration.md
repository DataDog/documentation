...
- link: /code_coverage/setup
  tag: Documentation
  text: Configurez la couverture de code
- link: /code_coverage/flags
  tag: Documentation
  text: Organisez les données de couverture avec des indicateurs
...
Ajoutez `!` avant un motif pour créer une exception à vos règles d'ignorance. Cela vous permet d'inclure des fichiers ou des dossiers spécifiques qui seraient autrement exclus.
...
<div class="alert alert-info">Si le fichier de configuration et l'interface utilisateur Datadog définissent des portes pour le même champ d'application, la demande de tirage doit respecter chaque seuil défini.</div>
...
- `type` (obligatoire) : Le type de porte de couverture. Valeurs prises en charge :
  - `total_coverage_percentage` : Le pourcentage minimum de couverture global pour le dépôt (ou pour les services ou propriétaires de code ciblés).
...
- `config` (obligatoire) : Options de configuration de la porte. Valeurs prises en charge :
  ...
  - `services` : (facultatif) Une liste de modèles de noms de services pour cibler la porte. Utilisez `*` comme caractère générique. Préfixez une valeur avec `!` pour exclure les services correspondants. Lorsqu'il est défini, la couverture est évaluée séparément pour chaque service correspondant.
  - `codeowners` : (facultatif) Une liste de modèles de propriétaires de code pour cibler la porte. Utilisez `*` comme caractère générique. Préfixez une valeur avec `!` pour exclure les propriétaires de code correspondants. Lorsqu'il est défini, la couverture est évaluée séparément pour chaque propriétaire de code correspondant.
  - `flags` : (facultatif) Une liste de modèles de noms de [drapeau][3] pour cibler la porte. Utilisez `*` comme caractère générique. Préfixez une valeur avec `!` pour exclure les drapeaux correspondants. Lorsqu'il est défini, la couverture est évaluée séparément pour chaque drapeau correspondant.

### Exemples

{{% collapse-content title="Portes de couverture totale et de patch non ciblées" level="h4" %}}
{{< code-block lang="yaml" filename="code-coverage.datadog.yml" >}}
schema-version: v1
gates:
  - type: total_coverage_percentage
...
{{% collapse-content title="Portes ciblées sur les services" level="h4" %}}
{{< code-block lang="yaml" filename="code-coverage.datadog.yml" >}}
schema-version: v1
services:
  - id: backend-api
...
{{% collapse-content title="Portes ciblées sur les propriétaires de code" level="h4" %}}
{{< code-block lang="yaml" filename="code-coverage.datadog.yml" >}}
schema-version: v1
gates:
  - type: patch_coverage_percentage
...
{{% collapse-content title="Portes ciblées sur les drapeaux" level="h4" %}}
{{< code-block lang="yaml" filename="code-coverage.datadog.yml" >}}
schema-version: v1
gates:
  - type: total_coverage_percentage
...
{{% collapse-content title="Exclure avec négation" level="h4" %}}
Utilisez le préfixe `!` pour exclure des services spécifiques, des propriétaires de code ou des indicateurs d'une porte. Par exemple, pour appliquer une couverture sur tous les services sauf ceux expérimentaux, et sur tous les indicateurs sauf les tests nocturnes :
{{< code-block lang="yaml" filename="code-coverage.datadog.yml" >}}
schema-version: v1
gates:
  - type: total_coverage_percentage
...
**Note** : Utilisez `**` pour correspondre aux répertoires de manière récursive. Le motif `folder/*` ne correspond qu'aux enfants directs, tandis que `folder/**/*` correspond à tous les descendants.
