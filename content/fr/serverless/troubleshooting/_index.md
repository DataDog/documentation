...
Si vous avez suivi toutes les étapes de dépannage ci-dessus et que vous souhaitez faire appel à l'[assistance Datadog][12], précisez les informations suivantes dans votre ticket :
...
1. Si la fonction Lambda est configurée pour envoyer des données par l'intermédiaire de logs à l'aide de la fonction Lambda du Forwarder Datadog, ajoutez les informations de base concernant la fonction Lambda du Forwarder, ainsi que les filtres d'abonnement configurés sur le groupe de logs de votre fonction Lambda.
2. La méthode d'installation que vous avez utilisée, par exemple : _Serverless Framework_ ou _AWS CDK_.
3. La méthode d'installation que vous avez essayé d'appliquer, par exemple : _Interface de ligne de commande Datadog_ ou _Custom_.
4. Les logs de debugging provenant de votre propre fonction Lambda.
5. Les logs de debugging provenant de la fonction Lambda du Forwarder Datadog (le cas échéant).
6. Les fichiers de configuration de projet, avec les **secrets codés en dur censurés**, tels que `serverless.yaml`, `package.json`, `package-lock.json`, `yarn.lock`, `tsconfig.json` et `webpack.config.json`.
