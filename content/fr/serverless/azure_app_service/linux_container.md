...
title: Instrumenter le service d'application Azure - Conteneurs Linux
---
## Aperçu

Cette page décrit comment instrumenter votre application Azure App Service conteneurisée Linux avec l'Agent Datadog.

Ce document suppose que votre application est configurée pour les sidecars selon les instructions d'Azure. [Configurer un conteneur sidecar pour un conteneur personnalisé dans Azure App Service][1] tutoriel.

Si vous préférez ne pas utiliser l'approche sidecar (non recommandée), vous pouvez suivre les instructions pour [Instrumenter le service d'application Azure - Conteneur Linux avec `serverless-init`][2].

## Configuration

### Intégration Azure

Si vous ne l'avez pas déjà fait, installez l' [intégration Datadog-Azure][3] pour collecter des métriques et des journaux.
...
{{% tab "Node.js" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-js` bibliothèque. Voir [Traçage des applications Node.js][101] pour les instructions.

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][102].
...
Pour configurer la journalisation dans votre application, voir [Collecte des journaux Node.js][103]. Pour configurer la corrélation des journaux de trace, voir [Corrélation des journaux et des traces Node.js][104].
...
{{% tab "Python" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-py` bibliothèque. Voir [Traçage des applications Python][201] pour les instructions.

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][202].
...
Pour configurer la journalisation dans votre application, voir [Collecte des journaux Node.js][203]. Pour configurer la corrélation des journaux de trace, voir [Corrélation des journaux et des traces Node.js][204].
...
{{% tab "Java" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-java` bibliothèque. Voir [Suivi des applications Java][301] pour les instructions.

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][302].
...
{{% tab ".NET" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-dotnet` bibliothèque.
...
# Copiez le code source restant
COPY . .

# Construisez l'application
RUN dotnet publish -c Release -o out

# Étape 2 : Créer une image d'exécution
DE mcr.microsoft.com/dotnet/aspnet:8.0 EN tant qu'exécution
WORKDIR /app

# Copier la sortie de construction de l'étape 1
COPY --from=build /app/out ./

# Spécifique à Datadog
RUN mkdir -p /datadog/tracer
RUN mkdir -p /home/LogFiles/dotnet
...
POINT D'ENTRÉE ["dotnet", "<your dotnet app>.dll"]
{{< /highlight >}}

Pour plus d'informations, voir [Traçage des applications .NET][401].

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][402].
...
Pour configurer la journalisation dans votre application, voir [Collecte de journaux C#][403]. Pour configurer la corrélation des journaux de trace, voir [Corrélation des journaux et des traces .NET][404].
...
{{% tab "Go" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-go` bibliothèque. Voir [Traçage des applications Go][501] pour les instructions.

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][502].
...
{{% tab "PHP" %}}
#### Traçage
Instrumentez votre application principale avec la `dd-trace-php` bibliothèque. Voir [Traçage des applications PHP][601] pour les instructions.

#### Métriques
Les métriques personnalisées sont également collectées via le traceur. Voir le [exemples de code][602].
...
Pour configurer la journalisation dans votre application, voir [Collecte des journaux Node.js][603]. Pour configurer la corrélation des journaux de trace, voir [Corrélation des journaux et des traces Node.js][604].
...
Pour utiliser le Datadog CLI dans [Azure Cloud Shell][603], ouvrez le cloud shell et utilisez `npx` pour exécuter le CLI directement. Définissez votre clé API et votre site dans les `DD_API_KEY` et `DD_SITE` variables d'environnement, puis exécutez le CLI :
```shell
export DD_API_KEY=<DATADOG_API_KEY>
export DD_SITE=<DATADOG_SITE>
...
Le [Module Terraform Datadog pour les applications Web Linux][1] entoure le [azurerm_linux_web_app][2] ressource et configure automatiquement votre application Web pour la surveillance sans serveur Datadog en ajoutant les variables d'environnement requises et le sidecar serverless-init.
...
Le [Module Datadog pour les applications Web Linux][1] déploie uniquement la ressource de l'application Web, vous devez donc construire et pousser votre conteneur séparément.
...
Pour utiliser le sidecar avec les applications Web pour conteneurs, vous devez utiliser le `SITECONTAINERS` linuxFxVersion avec `kind` définir sur `app,linux,container`. Mettez à jour votre application Web existante pour inclure les paramètres d'application Datadog nécessaires et le sidecar, comme suit :
...
Voir le [Onglet Manuel](?tab=manual#instrumentation) pour des descriptions de toutes les variables d'environnement.
...
2. Dans le **Modifier le conteneur** formulaire, fournissez les éléments suivants :
   - **Source de l'image**: Docker Hub ou d'autres registres
   - **Type d'image**: Public
   - **URL du serveur d'enregistrement**: `index.docker.io`
...
Dans votre **Paramètres de l'application** dans Azure, définissez les variables d'environnement suivantes sur votre conteneur principal et le conteneur sidecar. Alternativement, définissez ces variables sur votre conteneur principal et activez le **Autoriser l'accès à tous les paramètres de l'application** option.

{{< img src="serverless/azure_app_service/app_settings.png" alt="Dans Azure, une section Variables d'environnement. Une option 'Autoriser l'accès à tous les paramètres de l'application' est activée avec une case à cocher." >}}
...
- `DD_AAS_INSTANCE_LOGGING_ENABLED`: Quand `true`, la collecte des journaux est automatiquement configurée pour un chemin de fichier supplémentaire : `/home/LogFiles/*$COMPUTERNAME*.log`
...
: **Valeur**: `true`<br>
Définir cette variable d'environnement sur `true` permet au `/home/` montage de persister et d'être partagé avec le sidecar.<br>
...
Si vous configurez la surveillance pour une application .NET, configurez les éléments suivants **nécessaires** variables d'environnement.
