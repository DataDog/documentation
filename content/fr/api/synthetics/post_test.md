...
* **`retry`** (_facultatif_), dict { count: nombre (0-3), interval: nombre (0-1000)} : la durée en millisecondes (interval) d'attente avant de relancer un test qui a échoué et le nombre de nouvelles tentatives (count). Valeur par défaut : {count: 0}.
* **`monitor_options`** (_facultatif_), dict { renotify_interval: nombre} : le nombre de minutes à attendre après la dernière notification avant qu'un monitor ne renvoie une notification sur le statut actuel. Ce renvoi n'est effectué que si le monitor n'est pas résolu.
...
* **`type`** (_obligatoire_) : la partie de la réponse que vous souhaitez évaluer. Les différents types possibles sont :
  ...
  * `header` : lorsque vous définissez un en-tête, vous devez spécifier la clé de paramètre d'en-tête dans le paramètre `property`, et la valeur du paramètre d'en-tête dans le paramètre `target`. N'oubliez pas que les conditions de l'en-tête doivent être indiquées en minuscules.
