...
`https://api.datadoghq.com/api/v1/<ENDPOINT_API>?api_key=<VOTRE_CLÉ_API>`
...
Dans ce cas, vous avez besoin de deux webhooks distincts, un pour « mongroupedemonitors-alert-check » et un autre pour « mongroupedemonitors-ok-check ». Les deux utilisent le même endpoint d'API. Leur nom et leur URL doivent respecter les formats suivants :

* Name : mongroupedemonitors-alert-check
    URL : `https://api.datadoghq.com/api/v1/check_run?api_key=<CLÉ_API_DATADOG>`

* Name : mongroupedemonitors-ok-check
    URL : `https://api.datadoghq.com/api/v1/check_run?api_key=<CLÉ_API_DATADOG>`
...
```json
{
"check": "mongroupedemonitors.status",
"status": 2,
"host_name": "$HOSTNAME",
...
Avec cette charge utile personnalisée, chaque fois que @webhook-mongroupedemonitors-alert-check est déclenché par un monitor, un check_run appelé « mongroupedemonitors.status » est envoyé avec le statut CRITICAL. Celui-ci est tagué avec le nom du monitor et, le cas échéant, le nom du host sur lequel le monitor est déclenché.

Vous pouvez alors appliquer la même charge utile personnalisée pour le check « mongroupedemonitors-ok-check », mais en définissant le « status » sur « 0 » au lieu de « 2 » pour spécifier le statut « OK ».
...
```
{{#is_alert}} @webhook-mongroupedemonitors-alert-check {{/is_alert}}
{{#is_recovery}} @webhook-mongroupedemonitors-ok-check {{/is_recovery}}
```

Une fois vos monitors configurés et prêts à envoyer des alertes (ils doivent avoir envoyé au moins une alerte avec un statut OK ou CRITICAL pour être inclus dans le nombre total de statuts), vous pouvez alors configurer un widget « Check status » dans un [screenboard][4] pour votre « mongroupedemonitors.check ». Ici, le regroupement se fait en fonction du tag « monitor ».
