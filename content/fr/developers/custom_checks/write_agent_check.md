...
2. Dans le répertoire `conf.d`, créez un fichier de configuration pour votre nouveau check d'Agent. Attribuez-lui le nom `checkvalue.yaml`.
3. Modifiez le fichier de façon à inclure ce qui suit :
  ```yaml
    ...
    instances:
      [{}]
  ```
4. Créez un fichier de check dans le répertoire `checks.d`. Attribuez-lui le nom `checkvalue.py`.
...
**Remarque** : vos fichiers de configuration et de check doivent porter le même nom. Si votre check s'appelle `checkvalue.py`, votre fichier de configuration *doit* s'appeler `checkvalue.yaml`.
...
Pour modifier l'intervalle de collecte de votre check, utilisez le paramètre `min_collection_interval` dans votre fichier `checkvalue.yaml`. Par défaut, l'intervalle est défini sur `15`. Pour l'Agent v6, le paramètre `min_collection_interval` doit être ajouté au niveau des instances et configuré pour chaque instance. Exemple :
...
1. Remplacez le code du fichier checkvalue.py par ce qui suit (définissez la valeur de `lburl` sur l'adresse du répartiteur de charge) :
  ```python
    import urllib2
    import simplejson
    from checks import AgentCheck
  ...
  ```
2. Modifiez le fichier `checkvalue.yaml` (remplacez `ipaddress` par l'adresse IP de votre répartiteur de charge) :
  ```yaml
    init_config:
