...
L'[extension Auth0][26] récupère vos logs Auth0 et les transmet à Datadog.
...
Vous pouvez importer des données dans Datadog depuis Google Analytics via l'API Datadog avec [la bibliothèque Google Analytics][33] de [Bithaus][34].
...
* [lua-resty-dogstatsd][48] est une extension développée par [mediba inc][49] (et forkée par [Dailymotion][50]). Elle permet de générer des métriques, des événements et des checks de service via le protocole [DogStatsD][1]. `lua-resty-dogstatsd` est fourni en tant que GPLv3 et repose sur l'API cosocket Nginx.
