...
<br />Basculer pour activer l'envoi de données à LLM Observability. Doit être défini sur `1` ou `true`.

`DD_LLMOBS_ML_APP`
: optionnel - _chaîne_
<br />Le nom de votre application LLM, service ou projet, sous lequel toutes les traces et spans sont regroupés. Cela aide à distinguer entre différentes applications ou expériences. Voir [Directives de nommage des applications](#application-naming-guidelines) pour les caractères autorisés et autres contraintes. Pour remplacer cette valeur pour un span racine donné, voir [Tracer plusieurs applications](#tracing-multiple-applications). Si non fourni, cela par défaut à la valeur de [`DD_SERVICE`][1], ou la valeur d'un `DD_LLMOBS_ML_APP`propagé d'un service en amont.
<br />**Remarque** : Avant la version `ddtrace==3.14.0`, c'est un **champ requis**.
...
: optionnel - _chaîne_
<br />Le nom de votre application LLM, service ou projet, sous lequel toutes les traces et spans sont regroupés. Cela aide à distinguer entre différentes applications ou expériences. Voir [Directives de nommage des applications](#application-naming-guidelines) pour les caractères autorisés et autres contraintes. Pour remplacer cette valeur pour un span racine donné, voir [Tracer plusieurs applications](#tracing-multiple-applications). Si non fourni, cela par défaut à la valeur de [`DD_SERVICE`][1], ou la valeur d'un `DD_LLMOBS_ML_APP`propagé d'un service en amont.
<br />**Remarque** : Avant la version `dd-trace@5.66.0`, c'est un **champ requis**.
...
: requis - _entier ou chaîne_
<br />Basculer pour activer la soumission de données à LLM Observabilité. Doit être défini sur `1` ou `true`.
...
Au lieu d'utiliser [la configuration en ligne de commande](#command-line-setup), vous pouvez également activer la visibilité LLM par programmation.

{{< tabs >}}
{{% tab "Python" %}}

Utilisez la `LLMObs.enable()` fonction pour activer la visibilité LLM.

<div class="alert alert-info">
N'utilisez pas cette méthode de configuration avec la commande <code>ddtrace-run</code>.
</div>
...
Utilisez la `init()` fonction pour activer la visibilité LLM.
...
`agentlessEnabled`
: optionnel - _booléen_ - **par défaut** : `false`
<br />Uniquement requis si vous n'utilisez pas l'Agent Datadog, auquel cas cela doit être défini sur `true`. Cela configure la bibliothèque `dd-trace` pour ne pas envoyer de données nécessitant l'Agent Datadog. Si non fourni, cela par défaut à la valeur de `DD_LLMOBS_AGENTLESS_ENABLED`.
...
Pour instrumenter une fonction AWS Lambda existante avec LLM Observability, vous pouvez utiliser l'Extension Datadog et les couches de langage respectives.
...
{{% tab "Python" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="python" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <VOTRE_LLMOBS_ML_APP>
```
{{% /tab %}}

{{% tab "Node.js" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="node" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <VOTRE_LLMOBS_ML_APP>
```
{{% /tab %}}

{{% tab "Java" %}}
```shell
datadog-ci lambda instrument -f <YOUR_LAMBDA_FUNCTION_NAME> -r <AWS_REGION> -v {{< latest-lambda-layer-version layer="dd-trace-java" >}} -e {{< latest-lambda-layer-version layer="extension" >}} --llmobs <VOTRE_LLMOBS_ML_APP>
```
{{% /tab %}}
{{< /tabs >}}

4. Appelez votre fonction Lambda et vérifiez que les traces LLM Observability sont visibles dans l'interface utilisateur de Datadog.
...
Pour une liste des types de span disponibles, voir la [documentation des types de span][1]. Pour un traçage plus granulaire des opérations au sein des fonctions, voir [Traçage des spans en utilisant des méthodes en ligne](#tracing-spans-using-inline-methods).
...
Pour tracer un span, utilisez `llmobs.wrap(options, function)` comme wrapper de fonction pour la fonction que vous souhaitez tracer. Pour une liste des types de span disponibles, voir la [documentation des types de span][1]. Pour un traçage plus granulaire des opérations au sein des fonctions, voir [Traçage des spans en utilisant des méthodes en ligne](#tracing-spans-using-inline-methods).

### Types de span

Les types de span sont requis et sont spécifiés sur l'objet `options` passé aux fonctions de traçage `llmobs` (`trace`, `wrap` et `decorate`). Voir la [documentation des types de span][1] pour une liste des types de span pris en charge.
...
`llmobs.wrap` (avec [`llmobs.decorate`](#function-decorators-in-typescript) pour TypeScript) essaie de capturer automatiquement les entrées, les sorties et le nom de la fonction en cours de traçage. Si vous devez annoter manuellement une portée, consultez [Enrichir les portées](#enriching-spans). Les entrées et sorties que vous annotez remplaceront la capture automatique. De plus, pour remplacer le nom de la fonction, passez la `name` propriété sur l'objet d'options à la `llmobs.wrap` fonction :
...
### Conditions pour terminer une portée pour une fonction enveloppée

`llmobs.wrap` étend le comportement sous-jacent de [`tracer.wrap`][2]. La portée sous-jacente créée lorsque la fonction est appelée est terminée dans les conditions suivantes :
...
Si l'application n'utilise pas la fonction de rappel, il est recommandé d'utiliser un bloc tracé en ligne à la place. Voir [Tracer les portées en utilisant des méthodes en ligne](#tracing-spans-using-inline-methods) pour plus d'informations.
...
Il existe plusieurs méthodes pour démarrer une portée, en fonction du type de portée que vous démarrez. Voir la [documentation des types de span][1] pour une liste des types de span pris en charge.

Toutes les portées sont démarrées en tant qu'instance d'objet de `LLMObsSpan`. Chaque portée a des méthodes que vous pouvez utiliser pour interagir avec la portée et enregistrer des données.

### Terminer une portée

Les portées doivent être terminées pour que la trace soit soumise et visible dans l'application Datadog.

Pour terminer une portée, appelez `finish()` sur une instance d'objet de portée. Si possible, enveloppez le span dans un bloc `try/finally` pour garantir que le span soit soumis même si une exception se produit.
...
`spanName`
: optionnel - _Chaîne_
<br/>Le nom de l'opération. Si non fourni, `spanName` par défaut au type de portée.
...
Pour tracer un intervalle de flux de travail, spécifiez le type d'intervalle comme `workflow`, et spécifiez éventuellement des arguments sur l'objet d'options.
...
`spanName`
: optionnel - _Chaîne_
<br/>Le nom de l'opération. Si non fourni, `spanName` par défaut au nom de la fonction tracée.
...
{{% tab "Node.js" %}}
Pour tracer un appel d'outil, spécifiez le type de span comme `tool`, et spécifiez éventuellement des arguments sur l'objet options.
...
{{< tabs >}}
{{% tab "Python" %}}
Pour tracer un intervalle de tâche, utilisez le décorateur de fonction `LLMObs.task()`.
...
{{% tab "Node.js" %}}
Pour tracer un intervalle de tâche, spécifiez le type d'intervalle comme `task`, et spécifiez éventuellement des arguments dans l'objet options.
...
{{% /tab %}}
{{% tab "Java" %}}
Pour tracer un intervalle de tâche, importez et appelez la méthode suivante avec les arguments listés ci-dessous :
...
**Remarque** : Annoter l'entrée d'un intervalle d'embedding nécessite un formatage différent des autres types d'intervalles. Voir [Enrichir les intervalles](#enriching-spans) pour plus de détails sur la façon de spécifier les entrées d'embedding.
...
{{% tab "Node.js" %}}
Pour tracer une opération d'incorporation, spécifiez le type de portée comme `embedding`, et optionnellement spécifiez des arguments dans l'objet d'options.
...
**Remarque**: Annoter la sortie d'une portée de récupération nécessite un formatage différent des autres types de portée. Voir [Enrichir les portées](#enriching-spans) pour plus de détails sur la façon de spécifier les sorties de récupération.
...
Pour tracer une portée de récupération, spécifiez le type de portée comme `retrieval`, et optionnellement spécifiez les arguments suivants dans l'objet d'options.
...
Ce qui suit inclut également un exemple d'annotation d'une portée. Voir [Enrichir les portées](#enriching-spans) pour plus d'informations.
...
Commencer une nouvelle portée avant que la portée actuelle ne soit terminée trace automatiquement une relation parent-enfant entre les deux portées. La portée parent représente l'opération plus large, tandis que la portée enfant représente une sous-opération imbriquée plus petite à l'intérieur.
...
<br />La portée à annoter. Si `span` n'est pas fourni (comme lors de l'utilisation de décorateurs de fonction), le SDK annote la portée active actuelle.

`input_data`
: optionnel - _type sérialisable JSON ou liste de dictionnaires_
<br />Soit un type sérialisable JSON (pour les portées non-LLM) ou une liste de dictionnaires avec ce format : `{"content": "...", "role": "...", "tool_calls": ..., "tool_results": ...}`, où `"tool_calls"` sont une liste optionnelle de dictionnaires d'appels d'outils avec des clés requises : `"name"`, `"arguments"`, et des clés optionnelles : `"tool_id"`, `"type"`, et `"tool_results"` sont une liste optionnelle de dictionnaires de résultats d'outils avec une clé requise : `"result"`, et des clés optionnelles : `"name"`, `"tool_id"`, `"type"` pour les scénarios d'appel de fonction. **Remarque** : Les portées d'incorporation sont un cas spécial et nécessitent une chaîne ou un dictionnaire (ou une liste de dictionnaires) avec ce format : `{"text": "..."}`.

`output_data`
: optionnel - _type sérialisable JSON ou liste de dictionnaires_
<br />Soit un type sérialisable JSON (pour les portées non-LLM) ou une liste de dictionnaires avec ce format : `{"content": "...", "role": "...", "tool_calls": ...}`, où `"tool_calls"` sont une liste optionnelle de dictionnaires d'appels d'outils avec des clés requises : `"name"`, `"arguments"`, et des clés optionnelles : `"tool_id"`, `"type"` pour les scénarios d'appel de fonction. **Remarque** : Les portées de récupération sont un cas spécial et nécessitent une chaîne ou un dictionnaire (ou une liste de dictionnaires) avec ce format : `{"text": "...", "name": "...", "score": float, "id": "..."}`.
...
<br />Un dictionnaire de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter comme informations de métadonnées pertinentes à l'opération d'entrée ou de sortie décrite par l'intervalle (`model_temperature`, `max_tokens`, `top_k`, etc.).

`metrics`
: optionnel - _dictionnaire_
<br />Un dictionnaire de clés sérialisables en JSON et de valeurs numériques que les utilisateurs peuvent ajouter comme métriques pertinentes à l'opération décrite par l'intervalle (`input_tokens`, `output_tokens`, `total_tokens`, `time_to_first_token`, etc.). L'unité pour `time_to_first_token` est en secondes, similaire à la métrique `duration` qui est émise par défaut.

`tags`
: optionnel - _dictionnaire_
<br />Un dictionnaire de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter comme étiquettes sur l'intervalle. Exemples de clés : `session`, `env`, `system` et `version`. Pour plus d'informations sur les étiquettes, voir [Introduction aux étiquettes](/getting_started/tagging/).
...
{{% tab "Node.js" %}}
Le SDK fournit la méthode `llmobs.annotate()` pour annoter les intervalles avec des entrées, des sorties et des métadonnées.
...
`span`
: optionnel - _Portée_ - **par défaut** : la portée active actuelle
<br />La portée à annoter. Si `span` n'est pas fourni (comme lors de l'utilisation d'enveloppes de fonction), le SDK annote l'intervalle actif actuel.

`annotationOptions`
: requis - _objet_
<br />Un objet de différents types de données pour annoter l'intervalle.

L'objet `annotationOptions` peut contenir les éléments suivants :

`inputData`
: optionnel - _type sérialisable en JSON ou liste d'objets_
<br />Soit un type sérialisable en JSON (pour les intervalles non-LLM) ou une liste de dictionnaires avec ce format : `{role: "...", content: "..."}` (pour les intervalles LLM).  **Remarque** : Les intervalles d'incorporation sont un cas particulier et nécessitent une chaîne ou un objet (ou une liste d'objets) avec ce format : `{text: "..."}`.

`outputData`
: optionnel - _type sérialisable en JSON ou liste d'objets_
<br />Soit un type sérialisable en JSON (pour les intervalles non-LLM) ou une liste d'objets avec ce format : `{role: "...", content: "..."}` (pour les intervalles LLM). **Remarque** : Les intervalles de récupération sont un cas spécial et nécessitent une chaîne ou un objet (ou une liste d'objets) avec ce format : `{text: "...", name: "...", score: number, id: "..."}`.

`metadata`
: optionnel - _objet_
<br />Un objet de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter comme informations de métadonnées pertinentes à l'opération d'entrée ou de sortie décrite par l'intervalle (`model_temperature`, `max_tokens`, `top_k`, etc.).

`metrics`
: optionnel - _objet_
<br />Un objet de clés sérialisables en JSON et de valeurs numériques que les utilisateurs peuvent ajouter comme métriques pertinentes à l'opération décrite par l'intervalle (`input_tokens`, `output_tokens`, `total_tokens`, etc.).

`tags`
: optionnel - _objet_
<br />Un objet de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter comme étiquettes concernant le contexte de l'intervalle (`session`, `environment`, `system`, `versioning`, etc.). Pour plus d'informations sur les étiquettes, voir [Commencer avec les étiquettes](/getting_started/tagging/).
...
{{% /tab %}}
{{% tab "Java" %}}
Le SDK fournit plusieurs méthodes pour annoter les intervalles avec des entrées, des sorties, des métriques et des métadonnées.
...
<br />Soit une chaîne (pour les intervalles non-LLM) ou une liste de `LLMObs.LLMMessage` pour les intervalles LLM.
...
Pour plus d'informations sur les balises, voir [Démarrer avec les balises][1].

#### Ajout en masse de balises

La méthode membre `setTags()` de l'interface `LLMObsSpan` accepte les arguments suivants pour attacher plusieurs balises en masse :

##### Arguments

`tags`
: requis - _Map<String, Object>_
<br /> Une carte de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter comme balises pour décrire le contexte de l'étendue (par exemple, `session`, `environment`, `system` ou `version`).

#### Ajouter une seule balise

La méthode membre `setTag()` de l'interface `LLMObsSpan` accepte les arguments suivants pour attacher une seule balise :
...
`metadata`
: requis - _Carte<String, Objet>_
<br />Une carte de paires clé-valeur sérialisables en JSON contenant des métadonnées pertinentes pour l'opération d'entrée ou de sortie décrite par l'étendue.
...
### Annotation des étendues auto-instrumentées
...
`name`
: optionnel - _str_
<br />Nom qui remplace le nom de l'étendue pour toutes les étendues auto-instrumentées démarrées dans le contexte d'annotation.

`prompt`
: optionnel - _dictionnaire_
<br />Un dictionnaire qui représente l'invite utilisée pour un appel LLM. Voir la documentation de l'objet [Prompt](#prompt-tracking-arguments) pour le schéma complet et les clés prises en charge. Vous pouvez également importer l'objet `Prompt` depuis `ddtrace.llmobs.utils` et le passer en tant qu'argument `prompt`. **Remarque** : Cet argument ne s'applique qu'aux intervalles LLM.

`tags`
: optionnel - _dictionnaire_
<br />Un dictionnaire de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter en tant qu'étiquettes sur l'intervalle. Clés d'exemple : `session`, `env`, `system` et `version`. Pour plus d'informations sur les étiquettes, voir [Commencer avec les étiquettes](/getting_started/tagging/).
...
Le `llmobs.annotationContext()` du SDK accepte une fonction de rappel qui peut être utilisée pour modifier tous les intervalles auto-instrumentés démarrés pendant la portée de la fonction de rappel.
...
`tags`
: optionnel - _objet_
<br />Un objet de paires clé-valeur sérialisables en JSON que les utilisateurs peuvent ajouter en tant qu'étiquettes sur l'intervalle. Clés d'exemple : `session`, `env`, `system` et `version`. Pour plus d'informations sur les étiquettes, voir [Commencer avec les étiquettes](/getting_started/tagging/).
...
Attachez des métadonnées d'invite structurées à l'intervalle LLM afin de pouvoir reproduire les résultats, auditer les changements et comparer les performances des invites entre les versions. Lors de l'utilisation de modèles, l'observabilité LLM fournit également un [suivi des versions](#version-tracking) basé sur les changements de contenu des modèles.

{{< tabs >}}
{{% tab "Python" %}}
Utilisez `LLMObs.annotation_context(prompt=...)` pour attacher des métadonnées d'invite avant l'appel LLM. Pour plus de détails sur l'annotation des intervalles, voir [Enrichir les intervalles](#enriching-spans).
...
- `id` (str) : Identifiant logique pour cette invite. Doit être unique par `ml_app`. Par défaut `{ml_app}-unnamed_prompt`
...
Utilisez `llmobs.annotationContext({ prompt: ... }, () => { ... })` pour attacher des métadonnées d'invite avant l'appel LLM. Pour plus de détails sur l'annotation des intervalles, voir [Enrichir les intervalles](#enriching-spans).
...
`prompt`
: requis - objet
<br />Un objet qui suit le schéma de prompt ci-dessous.
...
- `template` (chaîne | Liste<Message>) : Chaîne de modèle avec espaces réservés (par exemple, `"Traduire {{text}} en {{lang}}"`). Alternatively, a list of `{ "role": "<role>", "content": "<template string with placeholders>" }` objets.
- `tags` (Enregistrement<string, chaîne>) : Étiquettes à attacher à l'exécution du prompt.
- `contextVariables` (tableau de chaînes[]) : Clés de variables contenant le contenu de vérité de base/contexte. Utilisé pour [la détection d'hallucinations](/llm_observability/evaluations/managed_evaluations/?tab=openai#hallucination).
- `queryVariables` (tableau de chaînes[]) : Clés de variables contenant la requête de l'utilisateur. Utilisé pour [la détection d'hallucinations](/llm_observability/evaluations/managed_evaluations/?tab=openai#hallucination).
...
#### Notes
- L'annotation d'un prompt n'est disponible que sur les intervalles LLM.
...
- Pour plusieurs appels LLM auto-instrumentés dans un bloc, utilisez un contexte d'annotation pour appliquer les mêmes métadonnées de prompt à travers les appels. Voir [Annoter les intervalles auto-instrumentés](#annotating-auto-instrumented-spans).

### Suivi des versions

LLM Observabilité fournit un versionnage automatique pour vos prompts lorsque aucune version explicite n'est spécifiée. Lorsque vous fournissez un `template` ou `chat_template` dans vos métadonnées de prompt sans un tag `version`, le système génère automatiquement une version en calculant un hachage du contenu du modèle. Si vous fournissez un tag `version`, LLM Observabilité utilise votre étiquette de version spécifiée au lieu d'en générer une automatiquement.

Le système de versionnage fonctionne comme suit :
- **Versionnage automatique** : Lorsque aucun tag `version` n'est fourni, LLM Observabilité calcule un hachage du contenu `template` ou `chat_template` pour générer automatiquement un identifiant de version numérique.
- **Versionnage manuel** : Lorsque un tag `version` est fourni, LLM Observabilité utilise votre étiquette de version spécifiée exactement comme fournie.
...
## Suivi des coûts
Attachez des métriques de tokens (pour le suivi automatique des coûts) ou des métriques de coûts (pour le suivi manuel des coûts) à vos intervalles LLM/embedding. Les métriques de tokens permettent à Datadog de calculer les coûts en utilisant les prix du fournisseur, tandis que les métriques de coûts vous permettent de fournir vos propres prix lors de l'utilisation de modèles personnalisés ou non pris en charge. Pour plus de détails, voir [Coûts][14].

Si vous utilisez une instrumentation automatique, les métriques de tokens et de coûts apparaissent automatiquement sur vos intervalles. Si vous instrumentez manuellement, suivez les conseils ci-dessous.

<div class="alert alert-info">Dans ce contexte, "métriques de tokens" et "métriques de coûts" se réfèrent à des paires clé-valeur numériques que vous attachez aux intervalles via le paramètre <code>metrics</code> de la méthode <code>LLMObs.annotate()</code>. Ceci est distinct des <a href="/llm_observability/monitoring/metrics/">métriques LLM Observabilité de la plateforme Datadog</a>. Pour les clés reconnues telles que <code>input_tokens</code>, <code>output_tokens</code>, <code>input_cost</code> et <code>output_cost</code>, Datadog utilise ces attributs de span pour générer des métriques de plateforme correspondantes (telles que <code>ml_obs.span.llm.input.cost</code>) à utiliser dans les tableaux de bord et les moniteurs.</div>
...
- _Jointure basée sur des tags_ - Joindre une évaluation en utilisant une paire clé-valeur de tag unique qui est définie sur un seul span. L'évaluation échouera à se joindre si la paire clé-valeur de tag correspond à plusieurs spans ou à aucun span.
- _Référence directe au span_ - Joindre une évaluation en utilisant la combinaison de l'ID de trace unique et de l'ID de span du span.

### Exporter un span
{{< tabs >}}
{{% tab "Python" %}}
`LLMObs.export_span()` peut être utilisé pour extraire le contexte du span d'un span. Cette méthode est utile pour associer votre évaluation au span correspondant.
...
`span`
: optionnel - _Span_
<br />L'étendue pour extraire le contexte de l'étendue (identifiants d'étendue et de trace). S'il n'est pas fourni (comme lors de l'utilisation de décorateurs de fonction), le SDK exporte l'étendue active actuelle.
...
{{% tab "Node.js" %}}
`llmobs.exportSpan()` peut être utilisé pour extraire le contexte du span d'un span. Vous devrez utiliser cette méthode pour associer votre évaluation à l'étendue correspondante.
...
`span`
: optionnel - _Span_
<br />L'étendue pour extraire le contexte de l'étendue (identifiants d'étendue et de trace). S'il n'est pas fourni (comme lors de l'utilisation d'enveloppes de fonction), le SDK exporte l'étendue active actuelle.
...
`span`
: optionnel - _dictionnaire_
<br />Un dictionnaire qui identifie de manière unique l'étendue associée à cette évaluation. Doit contenir `span_id` (chaîne) et `trace_id` (chaîne). Utilisez [`LLMObs.export_span()`](#exporting-a-span) pour générer ce dictionnaire.

`span_with_tag_value`
: optionnel - _dictionnaire_
<br />Un dictionnaire qui identifie de manière unique l'étendue associée à cette évaluation. Doit contenir `tag_key` (chaîne) et `tag_value` (chaîne).
...
`llmobs.submitEvaluation()` peut être utilisé pour soumettre votre évaluation personnalisée associée à une plage donnée.
...
<br />Le contexte de portée à associer à l'évaluation. Cela devrait être la sortie de `LLMObs.export_span()`.
...
`value`
: requis - _chaîne ou type numérique_
...
`tags`
: optionnel - _dictionnaire_
<br />Un dictionnaire de paires clé-valeur de chaînes que les utilisateurs peuvent ajouter en tant que tags concernant l'évaluation. Pour plus d'informations sur les tags, voir [Guide de démarrage avec les tags](/getting_started/tagging/).
...
`llmObsSpan`
: requis - _LLMObsSpan_
<br />Le contexte de portée à associer à l'évaluation.
...
<br />Un dictionnaire de paires clé-valeur de chaînes utilisé pour taguer l'évaluation. Pour plus d'informations sur les balises, voir [Introduction aux balises](/getting_started/tagging/).
{{% /collapse-content %}}
...
Pour modifier les données d'entrée et de sortie sur les spans, vous pouvez configurer une fonction de traitement. La fonction de traitement a accès aux balises des spans pour permettre la modification conditionnelle des entrées/sorties. Les fonctions de traitement peuvent soit renvoyer le span modifié pour l'émettre, soit renvoyer `None`/`null` pour empêcher le span d'être émis entièrement. Ceci est utile pour filtrer les spans contenant des données sensibles ou répondant à certains critères.
...
### Exemple : empêcher les spans d'être émis
...
| `user_handle` | Le nom d'utilisateur de la session de chat. |
| `user_name` | Le nom pour l'utilisateur de la session de chat. |
| `user_id` | L'ID pour l'utilisateur de la session de chat. |
{{% /tab %}}

{{% tab "Node.js" %}}
Lors du démarrage d'un span racine pour un nouveau trace ou span dans un nouveau processus, spécifiez l'argument `sessionId` avec l'ID de chaîne de la session utilisateur sous-jacente :
...
Le SDK prend en charge le traçage à travers des services ou hôtes distribués. Le traçage distribué fonctionne en propageant les informations de span à travers les requêtes web.
...
<br />Le span dont le contexte doit être injecté dans les en-têtes de requête fournis. Tous les spans (y compris ceux avec des décorateurs de fonction), cela par défaut au span actif actuel.
...
**Remarque** : Vous devez appeler `LLMObs.activate_distributed_headers()` avant de commencer tout span dans votre service en aval. Les spans commencés auparavant (y compris les spans de décorateur de fonction) ne sont pas capturés dans le traçage distribué.
...
La bibliothèque `dd-trace` fournit des intégrations prêtes à l'emploi qui prennent en charge le traçage distribué pour les [frameworks web populaires][1]. Exiger le traceur active automatiquement ces intégrations, mais vous pouvez les désactiver optionnellement avec :
...
### Persister un span à travers les contextes

Pour démarrer et arrêter manuellement un span à travers différents contextes ou portées :

1. Démarrer un span manuellement en utilisant les mêmes méthodes (par exemple, la méthode `LLMObs.workflow` pour un span de flux de travail), mais comme un appel de fonction ordinaire plutôt qu'en tant que gestionnaire de contexte.
2. Passez l'objet span comme argument à d'autres fonctions.
...
`LLMObs.flush()` est une fonction bloquante qui soumet toutes les données d'observabilité LLM mises en mémoire tampon au backend Datadog. Cela peut être utile dans les environnements sans serveur pour empêcher une application de se fermer jusqu'à ce que toutes les traces d'observabilité LLM soient soumises.
...
Pour remplacer cette configuration et utiliser un nom d'application LLM différent pour un span racine donné, passez l'argument `ml_app` avec le nom de chaîne de l'application LLM sous-jacente lors du démarrage d'un span racine pour un nouveau traçage ou d'un span dans un nouveau processus.
...
Le SDK d'observabilité LLM de Node.js propose une fonction `llmobs.decorate` qui sert de décorateur de fonction pour les applications TypeScript. Le comportement de traçage de cette fonction est le même que `llmobs.wrap`.
...
Pour remplacer cette configuration et utiliser un nom d'application LLM différent pour un span racine donné, passez l'argument `mlApp` avec le nom de chaîne de l'application LLM sous-jacente lors du démarrage d'un span racine pour un nouveau traçage ou d'un span dans un nouveau processus.
...
- Peut avoir jusqu'à 193 caractères de long
