...
- link: https://www.datadoghq.com/blog/monitor-connection-churn-datadog/
  tag: Blog
  text: Bonnes pratiques pour surveiller et remédier au churn de connexions.
...
Pour des informations sur les plateformes et protocoles pris en charge, et pour des instructions sur la façon de commencer, lisez [Setting Up Universal Service Monitoring][7].

<div class="alert alert-info"><strong>Aperçu : Protocoles supplémentaires et méthodes de cryptage</strong><p>L'USM est en Aperçu pour découvrir des services cloud et pour décoder des protocoles supplémentaires et des méthodes de cryptage de trafic. Pour plus d'informations et pour demander l'accès, lisez <a href="/universal_service_monitoring/additional_protocols/">Cloud Service Discovery and Additional Protocols</a>.</p></div>

## Tagging automatique des services {#automatic-service-tagging}

La Surveillance Universelle des Services détecte automatiquement les services fonctionnant dans votre infrastructure. S'il ne trouve pas [unified service tags][1], il leur attribue un nom basé sur l'un des tags : `app`, `short_image`, `kube_container_name`, `container_name`, `kube_deployment`, `kube_service`.
...
- Cliquez sur des pages de Service spécifiques pour voir les métriques des signaux d'or (demandes, erreurs et durée), et corrélez-les avec les récents changements de code grâce à [Deployment Tracking][2].

- Créez [moniteurs][4], [tableaux de bord][5] et [SLOs][6] en utilisant les métriques `universal.http.*`.
