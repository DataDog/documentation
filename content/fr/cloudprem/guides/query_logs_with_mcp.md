...
- link: /ide_plugins/vscode/?tab=cursor#installation
  tag: Documentation
  text: Extension Datadog pour VS Code et Curseur.
...
---
{{< callout url="https://www.datadoghq.com/product-preview/cloudprem/" btn_hidden="false" header="CloudPrem est en aperçu." >}}
  Rejoignez l'aperçu de CloudPrem pour accéder à de nouvelles fonctionnalités de gestion des journaux auto-hébergées.
{{< /callout >}}
...
- Un déploiement CloudPrem en cours avec des journaux ingérés.
- Accès au [serveur Datadog MCP][1].
- Le nom de votre index CloudPrem (visible dans l'[Explorateur de Journaux Datadog][2] sous **INDEX CLOUDPREM**).

## Interroger les journaux CloudPrem
...
- (Requis) **`indexes`** : Le(s) nom(s) de votre/vos index CloudPrem.
- (Requis) **`storage_tier`** : Doit être défini sur `"cloudprem"`.

Sans les deux paramètres, les requêtes par défaut rechercheront les index de journaux Datadog standard au lieu de CloudPrem.
...
- (Recommandé) Filtres de requête (service, statut, contenu des journaux).

### Paramètres de requête
Le tableau suivant décrit les paramètres clés utilisés lors de la requête de journaux avec le serveur MCP :

| Paramètre | Description | Exemple |
|-----------|-------------|---------|
| `query` | Requête de recherche de journaux utilisant la syntaxe de requête Datadog | `"*"` (tous les journaux), `"service:web"`, `"status:error"` |
...
| `sort` | Ordre de tri des résultats | `"-timestamp"` (décroissant), `"timestamp"` (croissant) |

Pour des exemples de paramètres et de requêtes en langage naturel, voir [ Exemples de requêtes avancées ](#advanced-query-examples).
...
2. Recherchez la section **INDEXES CLOUDPREM** dans le panneau de facette gauche.
3. Vos index CloudPrem y sont listés, généralement au format `cloudprem-<cluster_id>`.
...
### Rechercher un contenu de journal spécifique
**Invite** :
"Trouvez les journaux contenant 'délai de connexion' du service API dans cloudprem-prod au cours des dernières 24 heures."
...
**Invite** :
"Obtenez tous les journaux de code de statut 500 de l'index cloudprem-prod au cours de la dernière journée."
...
- **À la fois `storage_tier` et `indexes` sont requis** lors de la requête des journaux CloudPrem. Sans ces paramètres, les requêtes rechercheront plutôt dans les index standard de Datadog.
...
- Lorsque vous utilisez des requêtes en langage naturel, mentionnez explicitement le nom de votre index CloudPrem dans votre invite.
- Les journaux CloudPrem sont interrogeables en temps réel dès qu'ils sont indexés.
- La syntaxe de la requête suit la syntaxe standard de recherche de journaux [Datadog][4].
