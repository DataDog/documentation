...
**Agent n'envoie pas logs** :
- Vérifiez que la variable d'environnement `DD_LOGS_CONFIG_LOGS_DD_URL` est correctement définie.
- Vérifier Agent pod logs : `kubectl logs <datadog-agent-pod>`
- Assurez-vous que la collecte de log est activée : `DD_LOGS_ENABLED=true`

**CloudPrem ne reçoit pas logs** :
- Vérifiez l'indexeur CloudPrem logs : `kubectl logs -n <NAMESPACE_NAME> -l app=<RELEASE_NAME>-indexer`
- Vérifier la connectivité réseau entre Agent et l'indexeur CloudPrem
