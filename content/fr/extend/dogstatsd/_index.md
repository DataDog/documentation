...
3. Mettez à jour vos pods d'application : votre application a besoin d'un moyen fiable pour déterminer l'adresse IP de son hôte. Cela est simplifié dans Kubernetes 1.7, qui élargit l'ensemble des attributs que vous pouvez passer à vos pods en tant que variables d'environnement. Dans les versions 1.7 et supérieures, vous pouvez passer l'IP de l'hôte à n'importe quel pod en ajoutant une variable d'environnement au PodSpec. Par exemple, votre manifeste d'application pourrait ressembler à ceci :
...
L'Agent Datadog v6.10.0 prend en charge _la détection d'origine_, ce qui permet à DogStatsD de détecter d'où proviennent les métriques des conteneurs et de les taguer automatiquement. Lorsque la détection d'origine est activée, toutes les métriques reçues par UDP sont taguées par le même pod 
Ces tags sont considérés comme des métriques Autodiscovery.
...
Le client DogStatsD attache une étiquette interne, `entity_id`. La valeur de cette étiquette est le contenu de la variable d'environnement `DD_ENTITY_ID`, qui représente l'UID du pod. 
...
**Remarque** : En tant que meilleure pratique, Datadog recommande d'utiliser le marquage de service unifié lors de l'attribution des étiquettes. Le balisage de service unifié relie la télémétrie de Datadog grâce à l'utilisation de trois étiquettes standard : `env`, `service` et `version`. Pour apprendre à unifier votre environnement, consultez [le marquage de service unifié][8].
...
| `statsd_socket_path`   | Chaîne          | `null`      | Le chemin vers le socket UNIX de DogStatsD (remplace `host` et `port`, uniquement pris en charge avec l'Agent v6+). |
...
| `socket_path`   | Chaîne          | `null`      | Le chemin vers le socket UNIX de DogStatsD (remplace `host` et `port`, uniquement pris en charge avec l'Agent v6+). |
| `tags`          | Liste de chaînes | `null`      | Étiquettes à appliquer à toutes les métriques, événements et vérifications de service.                                                      |
| `namespace`     | Chaîne          | `null`      | Espace de noms à utiliser comme préfixe pour toutes les métriques, événements et vérifications de service.                                                |
...
| ----------------- | --------------- | --------------------------------------------------------------------------- |
| `WithNamespace()` | Chaîne          | Utilisez cette chaîne pour définir un espace de noms servant de préfixe à toutes les métriques, événements et vérifications de service. |
...
| `blocking(boolean val)`                      | Booléen        | false     | Le type de client à instancier : bloquant vs non-bloquant.                       |
...
| `socket_path`      | Chaîne          | `null`      | Le chemin vers le socket UNIX de DogStatsD (remplace `host` et `port`). Ceci n'est pris en charge qu'avec l'Agent v6+. Si cela n'est pas défini, l'Agent consulte la variable d'environnement `DD_DOGSTATSD_URL`. |
| `global_tags`      | Liste de chaînes | `null`      | Étiquettes à appliquer à toutes les métriques, événements et vérifications de service. L'étiquette `@dd.internal.entity_id` est ajoutée aux global_tags provenant de la variable d'environnement `DD_ENTITY_ID`.                                    |
...
| `ContainerID`      | Chaîne          | `null`      | Un identifiant de conteneur pour taguer toutes les métriques pour la détection d'origine.         |

{{< /programming-lang >}}
{{< /programming-lang-wrapper >}}

## Découvrez DogStatsD {#dive-into-dogstatsd}
