...
<div class="alert alert-info">L'emplacement d'installation par défaut pour l'Agent est <code>%ProgramFiles%\Datadog\Datadog Agent</code>. Si vous choisissez d'utiliser un emplacement d'installation personnalisé, assurez-vous de spécifier un <code>Datadog</code> sous-répertoire pour les fichiers Datadog.</div>
...
- L'option `/qn` effectue une installation silencieuse. Pour voir les invites de l'interface graphique, retirez-le.
...
Lors de la modification de la configuration, assurez-vous de redémarrer l'Agent pour que les changements prennent effet.

L'[interface graphique du gestionnaire d'Agent Datadog][6] peut être utilisée pour activer, désactiver et configurer des vérifications. Vous devez redémarrer l'Agent pour que vos changements prennent effet.
...
| lancer-gui      | Démarre le gestionnaire de l'Agent Datadog.                                                |
| redémarrer-service | Redémarre l'Agent dans le gestionnaire de contrôle des services.                           |
| exécuter | Démarre l'Agent.                                                                |
| démarrer | Démarre l'Agent. (Étant déprécié, mais accepté. Utilisez `run` comme alternative.) |
...
Pour vérifier que l'Agent fonctionne, vérifiez si le service `DatadogAgent` dans le panneau des Services est répertorié comme *Démarré*. Un processus appelé *Agent de métriques Datadog* (`agent.exe`) doit également exister dans le Gestionnaire des tâches.

Pour obtenir davantage d'informations sur l'état de l'agent, démarrez Datadog Agent Manager :

* Cliquez avec le bouton droit sur l'icône de la barre d'état système de l'Agent Datadog > {{< ui >}}Configure{{< /ui >}}, ou
* Exécutez `launch-gui` la commande depuis une **invite de commande élevée (exécuter en tant qu'administrateur)**
	- PowerShell : `& "<PATH_TO_AGENT.EXE>" launch-gui`
	- cmd : `"<PATH_TO_AGENT.EXE>" launch-gui`

Ensuite, ouvrez la page d'état en allant à {{< ui >}}Status{{< /ui >}} > {{< ui >}}General{{< /ui >}}.
