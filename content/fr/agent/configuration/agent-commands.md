...
| macOS      | `launchctl start com.datadoghq.agent` *ou* via l'application de la barre d'état système |
...
| macOS      | `launchctl stop com.datadoghq.agent` *ou* via l'application de la barre d'état système                |
...
| Docker     | Utilisez la [commande d'installation][2].                                               |
| Kubernetes | `kubectl delete pod <AGENT POD NAME>`—note : le pod est automatiquement replanifié |
| macOS      | Arrêtez puis redémarrez l'Agent avec : <br>`launchctl stop com.datadoghq.agent`<br>`launchctl start com.datadoghq.agent`<br> Ou utilisez l'application de la barre d'état système |
...
| macOS           | `launchctl list com.datadoghq.agent` * ou * via l'application de la barre d'état système             |
...
| macOS      | `datadog-agent status` ou via la [web GUI][6]   |
...
Certaines options comportent des flags et des options détaillées sous `--help`. Par exemple, utilisez l'aide avec la sous-commande `check` :
...
| `stream-logs`     | Affichez les journaux traités par un Agent en cours d'exécution.                         |
