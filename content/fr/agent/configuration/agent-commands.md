...
| Kubernetes | `kubectl delete pod <AGENT POD NAME>`—note : le pod est automatiquement reprogrammé |
...
| Docker     | Utilisez la [commande d'installation][2].                                               |
| Kubernetes | `kubectl delete pod <AGENT POD NAME>`—note : le pod est automatiquement reprogrammé |
| macOS      | Arrêtez puis démarrez l'Agent avec :<br>`launchctl stop com.datadoghq.agent`<br>`launchctl start com.datadoghq.agent`<br>Ou utilisez l'application systray |
...
| macOS      | `datadog-agent status` ou via l'[web GUI][6]   |
...
| `restart-service` | Redémarrez l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.         |
| `start-service`   | Démarrez l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.           |
| `stream-logs`     | Diffusez les journaux traités par un agent en cours d'exécution.                         |
| `stopservice`     | Arrêtez l'Agent dans le gestionnaire de contrôle des services. Uniquement sous Windows.            |
