...
L'Agent Datadog vous aide à gérer vos secrets de manière sécurisée en s'intégrant aux solutions de gestion des secrets suivantes :
 ...
 [Fichier Texte](#idforjsonyamltext)
...
### Option 1 : Utiliser le support natif de l'Agent pour récupérer des secrets

**Note** : À partir de la version `7.76` de l'Agent et des versions ultérieures, la gestion des secrets natifs est disponible pour les Agents activés FIPS.
...
`secret_backend_type` est utilisé pour spécifier quelle solution de gestion des secrets utiliser, et `secret_backend_config` contient des configurations supplémentaires pertinentes pour cette solution.
...
**Note** : Si vous exécutez Datadog dans un environnement conteneurisé, le [Cluster Agent](/containers/cluster_agent/) nécessite l'Agent 7.77 ou une version ultérieure pour prendre en charge la récupération native des secrets. Pour les versions antérieures, utilisez [Option 2](#option2usingthebuiltinscriptforkubernetesanddocker) ou [Option 3](#option3creatingacustomexecutable) à la place.
...
Datadog recommande d'utiliser la [méthode de profil d'instance][1006] pour récupérer les secrets, car AWS gère toutes les variables d'environnement et les profils de session pour vous. Plus d'instructions sur la façon de procéder peuvent être trouvées dans la [documentation officielle d'AWS Secrets Manager][1000].
...
* `secretId` : soit le "nom amical" du secret (par exemple, `/DatadogAgent/Production`) ou l'ARN (par exemple, `arn:aws:secretsmanager:useast1:123456789012:secret:/DatadogAgent/ProductionFOga1K`).
   **Remarque** : Le format ARN complet est requis lors de l'accès à des secrets d'un compte différent où les informations d'identification AWS ou `sts:AssumeRole` sont définies.
* `secretKey` : la clé JSON du secret AWS que vous souhaitez utiliser.


Le gestionnaire de secrets AWS peut stocker plusieurs paires clé-valeur au sein d'un seul secret. Une configuration backend utilisant Secrets Manager a accès à toutes les clés définies dans un secret.

Par exemple, en supposant que l'ID de secret `MySecrets` contient les 3 valeurs suivantes :
...
Ce qui suit est un exemple complet du fichier de configuration `datadog.yaml` utilisant les AWS Secrets pour extraire sa clé API de `MySecrets` :
...
<div class="alert alert-info"> Vous devez inclure le <code>serviceAccountAnnotations</code> pour accorder à l'Agent les permissions d'accéder au secret AWS. </div>

<br>


##### Vérification de cluster : sans les runners de vérification de cluster activés
```sh
datadog:
  env:
   - name: DD_SECRET_BACKEND_TYPE
...
##### Vérification de cluster : avec les coureurs de vérification de cluster activés
```sh
datadog:
  env:
   - name: DD_SECRET_BACKEND_TYPE
...
| valeur du type secret_backend_type                            | Service Azure          |
| ||
| `azure.keyvault` | [Azure Keyvault][2000] |

##### Authentification Azure

Datadog recommande d'utiliser des identités gérées pour s'authentifier avec Azure. Cela vous permet d'associer des ressources cloud avec des comptes AMI et supprime le besoin de mettre des informations sensibles dans votre fichier de configuration `datadog.yaml`.
...
La configuration backend pour les secrets Azure Key Vault est structurée en YAML suivant ce schéma :
...
Le secret backend est référencé dans votre fichier de configuration de l'Agent Datadog avec `ENC[ ]`. Voici un exemple où un secret en texte clair doit être récupéré :
...
| valeur du type secret_backend_type                               | Service GCP                    |
|  |  |
...
Cela peut être accordé avec le rôle prédéfini **Accesseur de secret du gestionnaire de secrets** (`roles/secretmanager.secretAccessor`) ou un rôle personnalisé avec un [accès][5002] équivalent.

Sur les environnements GCE ou GKE, ADC est configuré automatiquement via le compte de service attaché à l'instance ou au pod. Le compte de service attaché doit avoir les rôles appropriés pour accéder à GCP Secret Manager. De plus, l'environnement GCE ou GKE nécessite le `cloudplatform` [portée d'accès OAuth][5003].
...
Après avoir configuré l'Agent pour utiliser GCP Secret Manager, référencez les secrets dans vos configurations avec `ENC[secretname]` ou `ENC[secretname;key;version;]`.

La notation ENC est composée de :

 `secret` : le nom du secret dans GCP Secret Manager (par exemple, `datadogapikey`).
 `key` : (optionnel) la clé à extraire d'un secret au format JSON. Si vous utilisez des secrets en texte clair, vous pouvez omettre cela (exemple : `ENC[secretname;;version]`).
 `version` : (optionnel) le numéro de version du secret. Si non spécifié, la version `latest` est utilisée.
  + Exemples de syntaxe de version :
     `secretkey`  Version implicite `latest`
     `secretkey;;latest`  Version explicite `latest`
     `secretkey;;1`  Numéro de version spécifique

Par exemple, en supposant des secrets GCP nommés `datadogapikey` avec deux versions et `datadogappkey` :
...
Pour les secrets au format JSON, en supposant qu'un secret nommé `datadogkeys` contient :
...
##### Versionnage des secrets

GCP Secret Manager prend en charge les versions de secrets. L'implémentation de l'Agent prend également en charge la version secrète en utilisant le délimiteur `;`. Si aucune version n'est spécifiée, la version `latest` est utilisée.


##### Support JSON pour les secrets
...
2. Écrivez une politique qui donne la permission de récupérer des secrets de votre coffre. Créez un fichier `*.hcl` et incluez la permission suivante si vous utilisez la version 1 du moteur de secrets :
```
...
4. Choisissez la méthode d'authentification pour votre coffre. Si vous utilisez la méthode de profil d'instance AWS, exécutez `vault auth enable aws`.

##### Instructions pour le profil d'instance AWS

Datadog recommande d'authentifier en utilisant la [méthode de profil d'instance][3003] si vous exécutez votre HashiCorp Vault depuis une machine connectée à AWS.

Après cela, rédigez une [politique de coffre spécifique à l'authentification][3004].

##### Exemple de configuration

Dans l'exemple suivant, supposons que le préfixe de chemin secret de HashiCorp Vault soit `/Datadog/Production` avec une clé de paramètre de `apikey`:
 ...
 **Identifiants ServiceAccount**: Par défaut, utilise des jetons ServiceAccount montés automatiquement (`automountServiceAccountToken: true`, voir [documentation Kubernetes](https://kubernetes.io/docs/tasks/configurepodcontainer/configureserviceaccount/#optoutofapicredentialautomounting)). Des chemins personnalisés peuvent être configurés si nécessaire.
...
Pour chaque espace de noms contenant des secrets, créez un `Rôle` et un `RoleBinding` en utilisant l'exemple suivant avec le nom d'espace de noms correct :
...
Le format de notation ENC est `namespace/secretname;key` :
...
**Exemple :** Étant donné un Secret dans l'espace de noms `secretsns` :
...
**Remarque :** Un espace réservé `apiKey` est requis pour la validation du graphique Helm lors de l'utilisation d'un backend de secret pour résoudre la clé API. La variable d'environnement `DD_API_KEY` la remplace. Vous devez créer manuellement RBAC (Rôle + RoleBinding) pour chaque espace de noms contenant des secrets. Pour plus d'informations, consultez la section [configuration RBAC](#rbacsetup).

<div class="alert alert-info"> Helm n'a pas de configuration native <code>secretBackend.type</code>. Utilisez des variables d'environnement. </div>
...
**Remarque :** Une clé API de remplacement satisfait à la validation de l'Opérateur lors de l'utilisation d'un backend secret pour résoudre la clé API. La variable d'environnement `DD_API_KEY` la remplace. Vous devez créer manuellement RBAC (Rôle + RoleBinding) pour chaque espace de noms contenant des secrets. Pour plus d'informations, consultez la section [configuration RBAC](#rbacsetup).

<div class="alert alert-info"> L'Opérateur n'a pas de configuration native <code>secretBackend.type</code>. Utilisez des variables d'environnement dans <code>override.nodeAgent.env</code>. </div>

{{% /tab %}}
{{< /tabs >}}

##### Configuration de chemin personnalisée
...
Si votre configuration n'expose pas les variables d'environnement par défaut `KUBERNETES_SERVICE_HOST` et `KUBERNETES_SERVICE_PORT`, vous pouvez fournir une URL `api_server` pour interagir avec l'API REST Kubernetes.
...
Le backend des secrets Docker prend en charge à la fois les [secrets Docker Swarm][6002] et les [secrets Docker Compose][6003]. Par défaut, Swarm et Compose montent automatiquement les secrets dans le conteneur sous forme de fichiers à `/run/secrets` (Linux) ou `C:\ProgramData\Docker\secrets` (Windows).
...
**Chemin des secrets personnalisé :**
...
[Créer][6003] un `dockercompose.yml` avec des secrets basés sur des fichiers :
...
{{% collapse-content title="Backends de secrets de fichier JSON, YAML ou TEXT" level="h4" expanded=false id="id-for-json-yaml-text" %}}

| valeur du type secret_backend_type                                 | Service de fichier                             |
|||
|`file.json`           |[JSON][4001]                             |
|`file.yaml`          |[YAML][4002]                        |                            |
|`file.text`          |[TEXT][4003]                        |                            |
...
Le backend de fichier nécessite uniquement **des** permissions de lecture pour les fichiers JSON, YAML ou TEXT configurés. Ces permissions doivent être accordées à l'utilisateur local de l'Agent Datadog (`ddagent` sur Linux, `ddagentuser` sur Windows).
...
Par exemple, si nous avons un fichier YAML dans `/path/to/secret.yaml` contenant :

```yaml
datadog_api_key: your api key
```

Vous pouvez utiliser la configuration suivante pour extraire des secrets :
...
##### Sécurité des chemins :
 ...
 Les chemins relatifs dans `ENC[]` sont résolus par rapport à `secrets_path` (par exemple, `ENC[dd_api_key]` avec `secret_path: /chemin/vers/secrets` sera résolu en `/chemin/vers/secrets/dd_api_key`)
 Les chemins absolus dans `ENC[]` doivent être dans `secrets_path` (par exemple, `ENC[/chemin/vers/secrets/dd_api_key]` avec `secret_path: /chemin/vers/secrets` fonctionnera)
...
**Remarque :** Certains outils ajoutent automatiquement des sauts de ligne lors de l'exportation de secrets vers des fichiers. Voir [Supprimer les sauts de ligne finaux](#removetrailinglinebreaks) pour savoir comment gérer cela.
...
* Fichiers : en utilisant `ENC[file@/chemin/vers/fichier]`
* Secrets Kubernetes : en utilisant `ENC[k8s_secret@namespace/secretname/key]`
...
**Notes** :
 ...
 Le script peut accéder à tous les sous-dossiers, y compris le `/var/run/secrets/kubernetes.io/serviceaccount/token` sensible. En conséquence, Datadog recommande d'utiliser un dossier dédié au lieu de `/var/run/secrets`.

[Les secrets de Docker swarm][3] sont montés dans le dossier `/run/secrets`. Par exemple, le secret Docker `db_prod_passsword` se trouve dans `/run/secrets/db_prod_password` dans le conteneur de l'Agent. Cela serait référencé dans la configuration avec `ENC[file@/run/secrets/db_prod_password]`.
...
Configurez RBAC pour permettre au compte de service de l'Agent de lire le Secret. Le rôle suivant accorde l'accès en lecture au Secret `databasesecret` dans l'espace de noms `database` :
{{< tabs >}}
{{% tab "Opérateur Datadog" %}}
```yaml
apiVersion: datadoghq.com/v2alpha1
...
Alternativement, vous pouvez définir des ressources RBAC directement :
```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
...
Ce `Rôle` donne accès au `Secret : databasesecret` dans le `Namespace : database`. Le `RoleBinding` relie cette permission au `ServiceAccount : datadogagent` dans le `Namespace : default`. Cela doit être ajouté manuellement à votre cluster en tenant compte de vos ressources déployées.

### Option 3 : Création d'un exécutable personnalisé

Pour récupérer des secrets, l'Agent utilise un exécutable externe que vous fournissez. L'exécutable est utilisé lorsque de nouveaux secrets sont découverts et sont mis en cache pour la durée de vie de l'Agent. Si vous devez mettre à jour ou faire tourner un secret, vous devez redémarrer l'Agent pour le recharger.
...
* `secrets` (liste de chaînes) : Chaque chaîne est un gestionnaire pour un secret à récupérer.
...
Si un secret ne peut pas être résolu (soit en renvoyant un code de sortie non nul, soit un message d'erreur non nul), la configuration associée est ignorée par l'Agent.
...
Vous pouvez également créer votre propre exécutable de récupération de secret en utilisant n'importe quel langage. La seule exigence est qu'il suive le format d'entrée/sortie décrit précédemment.
...
* Appartenir au même utilisateur exécutant l'Agent (`ddagent` par défaut, ou `root` à l'intérieur d'un conteneur).
* Ne pas avoir de droits pour `groupe` ou `autre`.
...
**Note** : Votre exécutable partage les mêmes variables d'environnement que l'Agent.

## Actualisation des secrets à l'exécution

À partir de la version 7.67 de l'Agent, vous pouvez configurer l'Agent pour actualiser les secrets résolus sans nécessiter un redémarrage.
...
Les clés API/APP extraites en tant que secrets prennent en charge l'actualisation à l'exécution.
...
Par défaut, l'Agent randomise l'actualisation initiale dans la fenêtre `secret_refresh_interval` pour éviter qu'une flotte d'
Agents ne s'actualise simultanément. La clé est résolue au démarrage, puis actualisée une fois dans le premier intervalle
...
### Actualisation des secrets de vérification de l'autodécouverte
À partir de l'Agent v7.76, les vérifications programmées [Autodécouverte][1] peuvent actualiser les secrets à l'exécution si le modèle utilise la syntaxe `ENC[]`.
...
L'Agent peut alors déclencher l'actualisation des secrets soit à l'intervalle défini dans `secret_refresh_interval`, soit manuellement avec `datadogagent secret refresh`.

### Actualisation automatique des secrets en cas d'échec / d'invalidation de la clé API

À partir de la version v7.74 de l'Agent, l'Agent peut automatiquement actualiser les secrets lorsqu'il détecte une clé API invalide. Cela se produit lorsque l'Agent reçoit une réponse 403 Interdit de Datadog ou lorsque le contrôle de santé périodique détecte une clé API invalide ou expirée.

Pour activer cette fonctionnalité, définissez `secret_refresh_on_api_key_failure_interval` à un intervalle en minutes dans votre fichier `datadog.yaml`. Définissez sur `0` pour désactiver (par défaut).

Cet intervalle est le temps minimum entre 2 actualisations pour éviter de spammer votre solution de gestion des secrets lorsqu'une clé API invalide est détectée.
...
La commande `secret` dans l'interface CLI de l'Agent affiche toutes les erreurs liées à votre configuration. Par exemple, si les droits sur l'exécutable sont incorrects. Elle liste également tous les handles trouvés et leur emplacement.
...
### Voir les configurations après l'injection des secrets
...
L'utilisateur `ddagent` est créé lorsque vous installez l'Agent Datadog.
...
2. Si `ddagentuser` n'a pas les droits de lecture et d'exécution sur le fichier, une erreur similaire est enregistrée :
   ```
...
##### Test de votre exécutable

Votre exécutable est exécuté par l'Agent lors de la récupération de vos secrets. L'Agent Datadog s'exécute en utilisant le `ddagentuser`. Cet utilisateur n'a pas de droits spécifiques, mais il fait partie du groupe `Performance Monitor Users`. Le mot de passe de cet utilisateur est généré aléatoirement au moment de l'installation et n'est jamais enregistré nulle part.

Cela signifie que votre exécutable peut fonctionner avec votre utilisateur par défaut ou utilisateur de développement, mais pas lorsqu'il est exécuté par l'Agent, puisque `ddagentuser` a des droits plus restreints.
...
1. Retirez `ddagentuser` de la liste `Local Policies/User Rights Assignement/Deny Log on locally` dans la `Local Security Policy`.
2. Définissez un nouveau mot de passe pour `ddagentuser` (puisque celui généré au moment de l'installation n'est jamais enregistré nulle part). Dans PowerShell, exécutez :
    ```powershell
...
Considérez l'exemple précédent [Secrets Kubernetes](#examplereadingakubernetessecretacrossnamespaces), où le Secret `Secret:databasesecret` existe dans le `Namespace: database`, et le Compte de Service `ServiceAccount:datadogagent` existe dans le `Namespace: default`.
...
[7]: /fr/agent/configuration/agentcommands/#restarttheagent
[8]: /fr/agent/configuration/agentconfigurationfiles/
...
[6002]: https://docs.docker.com/engine/swarm/secrets/#commentdockermanagelessecrets
[6003]: https://docs.docker.com/compose/howtos/utiliserlessecrets/
