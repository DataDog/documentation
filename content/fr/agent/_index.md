...
Datadog recommande de mettre à jour l'Agent Datadog à chaque version mineure ou de correction, ou, au minimum, mensuellement. </p>
<p>
La mise à niveau vers une version majeure de l'Agent Datadog et le maintien à jour de l'Agent est le seul moyen pris en charge d'obtenir les dernières fonctionnalités et corrections de l'Agent.</p>
...
[Fleet Automation][15] est le flux de travail principal dans l'application pour installer, mettre à niveau, configurer et dépanner l'Agent Datadog à grande échelle.
...
- **{{< ui >}}Voir la configuration et l'historique{{< /ui >}}**: Consultez chaque Agent de votre flotte, sa version, les produits activés, les fichiers de configuration et les modifications historiques depuis une seule page.
- **[Mettre à jour les Agents obsolètes][13]**: Déclenchez des mises à jour à distance pour vos Agents afin de maintenir votre flotte à jour en quelques clics.
- **[Envoyer un signal de demande d'assistance][14]**: Depuis le {{< ui >}}Support{{< /ui >}} onglet d'un hôte, générez un signal et attachez-le à un cas de support existant ou nouveau sans avoir à utiliser la ligne de commande.
- **Audit de l'utilisation de la clé API**: Identifiez quels Agents utilisent une clé API spécifique et remplacez les clés en toute sécurité.
...
- Consultez les informations de statut de votre Agent
...
À partir de l'Agent 6 et ultérieur, l'interface de ligne de commande de l'Agent est basée sur des sous-commandes. Pour une liste complète des sous-commandes de l'Agent, voir [Agent Commands][2].
...
**Remarque** : Consultez la documentation [Getting Started with Datadog Sites][11] pour plus de détails sur le paramètre `site`.
...
* Bande passante du réseau : ~ 200 Ko/s ▲
...
{{< whatsnext desc="Cette section comprend les sujets suivants :">}}
  ...
  {{< nextlink href="/getting_started/observability_pipelines">}}<u>Configure Observability Pipelines and Datadog</u> : Déployez le Observability Pipelines Worker en tant qu'agrégateur pour collecter, transformer et acheminer tous vos journaux et métriques vers n'importe quelle destination.{{< /nextlink >}}
{{< /whatsnext >}}
