...
Datadog recommande de mettre à jour l'Agent Datadog à chaque nouvelle version mineure et de patch, ou, au minimum, mensuellement. </p>
<p>
...
[Automatisation de Flotte][15] est le flux de travail principal dans l'application pour installer, mettre à niveau, configurer et dépanner l'Agent Datadog à grande échelle.
...
- **{{< ui >}}View configuration & history{{< /ui >}}**: Voir chaque Agent dans votre flotte, sa version, les produits activés, les fichiers de configuration et les modifications historiques depuis une seule page.
- **[Mettre à niveau les Agents obsolètes][13]** : Déclenchez des mises à niveau à distance pour vos Agents afin de maintenir votre flotte à jour en quelques clics.
...
- **Audit de l'utilisation de la clé API** : Identifiez quels agents utilisent une clé API spécifique et effectuez une rotation des clés en toute sécurité.
...
- Consultez tous les contrôles en cours d'exécution
- Consultez le journal de l'Agent
- Modifiez le fichier de configuration de l'Agent (`datadog.yaml`)
- Ajoutez ou modifiez les contrôles de l'Agent
- Send flares → Envoyez des flares
...
- Pour des raisons de sécurité, l'interface graphique ne peut être accédée que **depuis** l'interface réseau locale (`localhost`/`127.0.0.1`), vous devez donc être sur l'hôte où l'Agent est en cours d'exécution. Vous ne pouvez pas exécuter l'Agent sur une machine virtuelle ou un conteneur et y accéder depuis la machine hôte.

### Interface de ligne de commande {#command-line-interface}

À partir de l'Agent 6 et des versions ultérieures, l'interface de ligne de commande de l'Agent est basée sur des sous-commandes. Pour une liste complète des sous-commandes de l'Agent, voir [Commandes de l'Agent][2].
...
Un exemple de la consommation de ressources de l'Agent Datadog est ci-dessous. Des tests ont été effectués sur une instance de machine Amazon EC2 `c5.xlarge` (4 VCPU/ 8 Go de RAM) et des performances comparables ont été observées pour les instances basées sur ARM64 avec des ressources similaires. La version vanilla `datadog-agent` fonctionnait avec un contrôle de processus pour surveiller l'Agent lui-même. Activer plus d'intégrations peut augmenter la consommation de ressources de l'Agent.
Activer les vérifications JMX oblige l'Agent à utiliser plus de mémoire en fonction du nombre de beans exposés par les JVM surveillées. Activer les Agents de trace et de processus augmente également la consommation de ressources.
...
Les résultats ci-dessous sont obtenus à partir d'une collecte de *110 Ko de journaux par seconde* d'un fichier avec le [transmetteur HTTP][6] activé. Cela montre l'évolution de l'utilisation des ressources pour les différents niveaux de compression disponibles.

{{< tabs >}}
{{% tab "Niveau de compression HTTP 6" %}}

* Version de l'agent Test : 6.15.0
* CPU : ~ 1,5 % du CPU utilisé en moyenne
* Mémoire : ~ 95 Mo de RAM utilisée.
* Bande passante réseau : ~ 14 Ko/s ▲
...
{{< whatsnext desc="Cette section comprend les sujets suivants :">}}
  ...
  {{< nextlink href="/agent/guide">}}<u>Guides</u> : Ce sont des tutoriels détaillés, étape par étape, pour utiliser l'Agent.{{< /nextlink >}}
