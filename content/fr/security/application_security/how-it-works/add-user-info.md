...
Le package de traceur Go fournit la fonction `SetUser()`, qui vous permet de surveiller les requêtes authentifiées en ajoutant des informations utilisateur à la trace. Pour plus d'options, voir [la documentation du traceur Go][2] (ou [documentation v1][1]).
...
À partir de dd-trace-py v3.7, vous pouvez utiliser le nouveau SDK du traceur Python pour suivre les utilisateurs et les événements des utilisateurs.
...
À partir de dd-trace-py v3.7, cet exemple montre comment définir des balises de surveillance des utilisateurs et activer la capacité de blocage des utilisateurs :
...
#### Migration vers les nouvelles méthodes de connexion pour le succès et l'échec
...
1. L'identifiant de connexion (email, nom d'utilisateur) est le premier paramètre et est obligatoire.
2. La liste/ID utilisateur est optionnelle dans les événements de succès et a été supprimée des événements d'échec.
3. Les métadonnées ont été simplifiées et ne nécessitent plus le champ `usr.login`.
...
Disponible depuis la version 3.7 de dd-trace-py, `track_user_sdk` fournit 5 fonctions :
...
Disponible depuis la version 3.17 de dd-trace-py, `track_user_sdk` fournit cette fonction supplémentaire :
...
La méthode préférée est d'utiliser le nouveau SDK de suivi des utilisateurs (disponible depuis la version 1.9 de dd-trace-py) au lieu de l'API héritée.
...
Si votre service a l'AAP activé et que la [Configuration à distance][1] est également activée, vous pouvez créer une règle WAF personnalisée pour signaler toute requête correspondant à un tag de logique métier personnalisé. Cela ne nécessite aucune modification de votre application et peut être fait entièrement depuis Datadog.

Pour commencer, accédez à la [page de règle WAF personnalisée][2] et cliquez sur "Créer une nouvelle règle".

{{< img src="security/application_security/threats/custom-waf-rule-menu.png" alt="Accédez au menu de règles WAF personnalisées depuis la page d'accueil de l'AAP en cliquant sur Protection, puis sur In-App WAF et Règles personnalisées" style="width:100%;" >}}

Cela ouvrira un menu dans lequel vous pourrez définir votre règle WAF personnalisée. En sélectionnant la catégorie "Logique métier", vous pourrez configurer un type d'événement (par exemple, `users.password_reset`). Vous pouvez ensuite sélectionner le service que vous souhaitez suivre et un point de terminaison spécifique. Vous pouvez également utiliser la condition de règle pour cibler un paramètre spécifique afin d'identifier le flux de code que vous souhaitez _instrumenter_. Lorsque la condition correspond, la bibliothèque ajoute un tag à la trace et la signale pour être transmise à l'AAP. Si vous n'avez pas besoin d'une condition spécifique, vous pouvez en définir une générale qui correspond à tous les cas.
...
Le suivi automatique de l'activité des utilisateurs offre les modes suivants :

- `identification` mode (nom court : `ident`) :
  - Ce mode par défaut collecte toujours l'identifiant de l'utilisateur, selon le principe du meilleur effort.
  - L'identifiant de l'utilisateur est collecté lors de la réussite ou de l'échec de la connexion. En cas d'échec, l'identifiant de l'utilisateur est collecté, que l'utilisateur existe ou non.
  - Lorsque le cadre instrumenté ne fournit pas clairement un identifiant d'utilisateur, mais plutôt un objet utilisateur structuré, l'identifiant de l'utilisateur est déterminé au mieux en fonction des noms de champs de l'objet. Cette liste de noms de champs est considérée, classée par ordre de priorité :
    - `id`
    - `email`
    - `username`
    - `login`
    - `user`
  - Si aucun identifiant d'utilisateur n'est disponible ou trouvé, l'événement utilisateur n'est pas émis.
- `anonymization` mode (nom court : `anon`) :
  - Ce mode est le même que `identification`, mais anonymise l'identifiant de l'utilisateur en le hachant (SHA256) et en tronquant le hachage résultant.
- `disabled` mode :
...
- `extended` mode : La bibliothèque de trace essaie de collecter l'identifiant utilisateur et l'email de l'utilisateur. Dans ce mode, Datadog ne vérifie pas que le type de l'identifiant utilisateur soit un GUID. La bibliothèque de trace rapporte toute valeur pouvant être extraite de l'événement.
