...
Pour correctement classifier les destinations du trafic, la solution NPM fait appel au concept d'ID réseau. Un ID réseau est représenté par un tag et correspond à un identifiant alphanumérique pour un ensemble d'adresses IP qui peuvent communiquer entre elles. Lorsque d'une adresse IP mappée vers plusieurs hosts ayant des ID réseau différents est détectée, cet identifiant est utilisé pour déterminer avec précision le host vers lequel le trafic réseau est acheminé ou duquel il provient.
