...
このデータを収集することを選択した場合、いつでもサポートチケットを送信することにより、今後収集するデータからこのデータを削除するようリクエストすることができます。スクラビングを開始する前に、収集したデータから IP を削除することはできません。IP のスクラビングは組織レベルで行われるため、特定のアプリケーションやサービスなどに対してこのデータを削除することはできません。これはバックエンドで実行されるため、Browser SDK はデータを送信しますが、IP アドレスは Datadog のバックエンドパイプラインによってスクラビングされ、処理時に削除されます。

#### 位置情報
クライアント IP の削除に加えて、今後収集するすべてのデータからジオロケーション (国、都市、郡) または GeoIP をスクラビングするよう選択することも可能です。クライアント IP を削除せずに、GeoIP データのみをスクラビングすることはできません。IP アドレスと同様に、ジオロケーションのスクラビングは組織レベルで行われます。このデータは、特定のアプリケーションやサービスなどに対して削除することはできません。これはバックエンドレベルで行われます。つまり、Browser SDK はデータを送信しますが、ジオロケーションデータはバックエンドパイプラインによってスクラビングされ、処理時に削除されます。
