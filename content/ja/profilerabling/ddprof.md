...
kind: ドキュメント
title: コンパイル済み言語のプロファイラーを有効にする
type: multi-code-lang
---

<div class="alert alert-warning">
コンパイル言語用プロファイラー、<code>ddprof</code> は公開ベータ版です。Datadog では、本番環境にデプロイする前に重要でない環境でプロファイラを評価することを推奨しています。
</div>

コンパイル済み言語用 Datadog Continuous Profiler (`ddprof`) は、OS レベルの API を使用してプロファイリングデータを収集します。C、C++、Rust などのコンパイルされた言語で書かれたアプリケーションに最適です。

## 要件

対応 OS
: Linux kernel v4.17+、glibc 2.18+ または musl libc
...
1. お使いの Linux ディストリビューションの最新の [`ddprof` リリース][2]をダウンロードします。例えば、`amd64` (別名 `x86_64`) プラットフォーム用の `v0.9.3` リリースを取得する一つの方法は以下の通りです。
