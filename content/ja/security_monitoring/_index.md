...
Datadog は、アプリケーションやインフラストラクチャーに対するさまざまな種類の脅威を検出します。たとえば、標的型攻撃、脅威のインテルリストに一致するシステムと通信する IP、安全でない構成などがあります。こうした脅威は、Datadog でセキュリティシグナルとして表面化され、[セキュリティエクスプローラー][1]で相関およびトリアージできます。
