...
1. Upload the IdP Metadata from your SAML Identity provider by clicking the "Choose File" button. <br><br>
  <img src="/static/images/saml_choose_file.png" style="width:50%; border:1px solid #777777"/> <br><br>
  After you've chosen the file, click "Upload File". <br>
...
* Datadog supports the **HTTP-POST** binding for **SAML2**: <br>`urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST`.
...
* Datadog expects that Attributes use the NameFormat<br> `urn:oasis:names:tc:SAML:2.0:attrname-format:uri` or `urn:oasis:names:tc:SAML:2.0:attrname-format:basic`.  The name used for each attribute will depend on the NameFormat that your IdP uses.
