...
1.  拡張機能をインストールします（`--profile` とプロファイル名を省略すると、デフォルトのCursorプロファイルにインストールされます）：
    ```shell
...
1.  Datadog MCP サーバーを HTTP トランスポートとサイトのエンドポイント URL で追加するには、`~/.codex/config.toml`（または Codex CLI 構成ファイル）を編集します。たとえば、以下のとおりです。
...
1. HTTPトランスポートとあなたのサイトのエンドポイントURLを使用して、クライアントの設定ファイルにDatadog MCPサーバーを追加してください。たとえば、以下のとおりです。
...
|  クライアント |  開発者 |  ノート |
|--------|------|------|
| [Cursor][3] |  Cursor |  Datadog [Cursor & VS Code extension][15]を推奨します。|
...
|  [Warp][28] |  Warp | |
|  [VS Code][7] |  マイクロソフト |  Datadog [Cursor & VS Code extension][16]を推奨します。|
