...
4. [RDS][6] または [Aurora][7] インスタンスの Agent セットアップ手順を完了します。
...
## Microsoft Entra ID マネージド ID 認証の構成
...
PostgreSQL Flexible または Single Server インスタンスへの認証を構成するには、以下の手順を実行します。


1. Azure ポータルで[マネージドアイデンティティ][11]を作成し、Agent がデプロイされる Azure 仮想マシンに割り当てます。
...
```
4. Azure の通常の [Agent セットアップ手順][14]を実行します。例:
...
Azure SQL DB または Azure Managed Instance への認証を構成するには、以下の手順を実行します。
...
```
4. Azure の通常の Agent セットアップ手順を実行します。例えば、[Azure Managed Instance][17] の場合:
...
[Azure SQL DB][19] を使用している場合は、`master` データベースから以下を実行します。
...
5. `managed_identity` 構成ブロックでインスタンス構成を更新します。


**注**: この機能を使用するには、[ODBC Driver 17 for SQL Server][18] 以上が必要です。
