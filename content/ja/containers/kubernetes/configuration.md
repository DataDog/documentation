...
description: Datadog Agent インストール後の APM、ログ、プロセス、イベント、およびその他の機能の追加構成オプション
...
### Datadog による以下の収集を有効にします。{#enable-datadog-to-collect}
...
### その他の機能 {#other-capabilities}
- [Datadog Cluster Agent](#datadog-cluster-agent)
- [Integrations](#integrations)
- [コンテナビュー](#containers-view)
- [オーケストレーターエクスプローラー](#orchestrator-explorer)
- [外部メトリクスサーバー](#custom-metrics-server)

### その他の構成 {#more-configurations}
- [環境変数](#environment-variables)
- [カスタムメトリクスの DogStatsD](#configure-dogstatsd)
...
- [クラスター名の設定](#set-cluster-name)
- [その他](#miscellaneous)

## APM とトレースの有効化 {#enable-apm-and-tracing}
...
## Kubernetes イベント収集の有効化 {#enable-kubernetes-event-collection}
...
## CNM 収集の有効化 {#enable-cnm-collection}

{{< tabs >}}
{{% tab "Datadog Operator" %}}

ご使用の `datadog-agent.yaml` で、`features.npm.enabled` を `true` に設定します。
...
詳細については、[Cloud Network Monitoring][18] を参照してください。

## ログ収集の有効化{#enable-log-collection}

{{< tabs >}}
{{% tab "Datadog Operator" %}}
ご使用の `datadog-agent.yaml` で、`features.logCollection.enabled` および `features.logCollection.containerCollectAll` を `true` に設定します。
...
## プロセス収集の有効化{#enable-process-collection}
...
Cluster Agent は、クラスターレベルのモニタリングデータを収集するための合理化された一元的なアプローチを提供します。Datadog は、Kubernetes のモニタリングに Cluster Agent を使用することを強く推奨します。

Datadog Operator v1.0.0+ と Helm チャート v2.7.0+ では、**デフォルトで Cluster Agent が有効になります**。それ以上の構成は必要ありません。
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
`datadog-agent.yaml` では、`spec.global.credentials.appKey` の下にアプリケーションキーを提供し、`features.externalMetricsServer.enabled` を `true` に設定します。
...
{{% /tab %}}
{{% tab "Helm" %}}
`datadog-values.yaml` では、`datadog.appKey` の下にアプリケーションキーを提供し、`clusterAgent.metricsProvider.enabled` を `true` に設定します。
...
クラスター内で Agent が実行されたら、[Datadog の Autodiscovery 機能][5]を使用して、Pod からメトリクスとログを自動的に収集します。
...
Datadog Operator では、Process Agent がデフォルトで有効になります。
...
spec:
  ...
  features:
    liveContainerCollection:
      enabled: true
```
一部のセットアップでは、Process Agent と Cluster Agent で Kubernetes クラスター名が自動検出されません。この場合、機能は起動せず、Cluster Agent ログで以下のような警告が表示されます。`Orchestrator explorer enabled but no cluster name set: disabling`。この場合、`spec.global.clusterName` を `datadog-agent.yaml` でクラスター名に設定する必要があります。
...
一部のセットアップでは、Process Agent と Cluster Agent で Kubernetes クラスター名が自動検出されません。この場合、機能は起動せず、Cluster Agent ログで以下のような警告が表示されます。`Orchestrator explorer enabled but no cluster name set: disabling.`。この場合、`datadog.clusterName` を `datadog-values.yaml` でクラスター名に設定する必要があります。
...
有効なクラスター名の制限については、[クラスター名を設定する](#set-cluster-name)を参照してください。

詳細については、[コンテナビュー][15]のドキュメントを参照してください。

## オーケストレーターエクスプローラー {#orchestrator-explorer}

Datadog Operator と Helm チャートでは、デフォルトで **Datadog の[オーケストレーターエクスプローラー][20]が有効になります**。それ以上の構成は必要ありません。

{{< tabs >}}
{{% tab "Datadog Operator" %}}

Datadog Operator では、オーケストレーターエクスプローラーがデフォルトで有効になります。
...
Helm チャートでは、デフォルトでオーケストレーターエクスプローラーが有効になります。

検証のため、`datadog-values.yaml` フィルターで、`orchestratorExplorer.enabled` パラメーターが `true` に設定されていることを確認してください。
...
詳細については、[オーケストレーターエクスプローラーのドキュメント][21]を参照してください。
...
| `global.credentials.appKey` |  Datadog アプリケーションキーを構成します。外部メトリクスサーバーを使用している場合は、メトリクスへの読み取りアクセス用に Datadog アプリケーションキーを設定する必要があります。|
...
| `global.logLevel` | ロギングの冗長性を設定します。これはコンテナによってオーバーライドできます。有効なログレベルは、`trace`、`debug`、`info`、`warn`、`error`、`critical` および `off` です。デフォルト: `info`。|
...
Datadog Operator の構成フィールドの完全なリストについては、[Operator v2alpha1 の仕様][2]を参照してください。旧バージョンについては、[DatadogAgent CRD を v2alpha1 へ移行する][3]を参照してください。また、`kubectl explain datadogagent --recursive` を使用して構成フィールドをクエリすることもできます。

[1]: /ja/getting_started/
[2]: https://github.com/DataDog/datadog-operator/blob/main/docs/configuration.v2alpha1.md
[3]: /ja/containers/guide/v2alpha1_migration/
{{% /tab %}}
{{% tab "Helm" %}}
|  Helm | 説明 |
|  ---- | ----------- |
|  `datadog.apiKey` | Datadog API キーを構成します。|
| `datadog.apiKeyExistingSecret` | `datadog.apiKey` の代わりに、Datadog API キーを含む既存の Kubernetes `Secret` の名前を指定し、キー名 `api-key` で設定します。|
|  `datadog.appKey` | Datadog アプリケーションキーを構成します。外部メトリクスサーバーを使用している場合は、メトリクスへの読み取りアクセス用に Datadog アプリケーションキーを設定する必要があります。|
| `datadog.appKeyExistingSecret` | `datadog.appKey` の代わりに、Datadog API キーを含む既存の Kubernetes `Secret` の名前を指定し、キー名 `app-key` で設定します。|
| `datadog.logLevel` | ロギングの冗長性を設定します。これはコンテナによってオーバーライドできます。有効なログレベルは、`trace`、`debug`、`info`、`warn`、`error`、`critical` および `off` です。デフォルト: `info`。|
...
| `datadog.tags` | 収集されるすべてのメトリクス、イベント、サービスチェックにアタッチされるタグのリスト。|
...
| 環境変数         | 説明                                                                                                                                                                                                                                                                                                                                      |
|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DD_API_KEY`         | ご使用の Datadog API キー (**必須**)                                                                                                                                                                                                                                                                                                              |
| `DD_ENV`             | 出力されるすべてのデータにグローバルタグの `env` を設定します。                                                                                                                                                                                                                                                                                                 |
| `DD_HOSTNAME`        | メトリクスに使用するホスト名 (自動検出が失敗した場合)                                                                                                                                                                                                                                                                                             |
| `DD_TAGS`            | ホストタグはスペースで区切ります。例: `simple-tag-0 tag-key-1:tag-value-1`                                                                                                                                                                                                                                                                 |
| `DD_SITE`            | メトリクス、トレース、およびログの送信先サイト。ご使用の `DD_SITE` は {{< region-param key="dd_site" code="true">}}です。デフォルトは `datadoghq.com` です。                                                                                                                                                                                              |
| `DD_DD_URL`          | メトリクス送信用 URL を上書きするためのオプションの設定。                                                                                                                                                                                                                                                                                     |
| `DD_URL` (6.36+/7.36+)            | `DD_DD_URL` のエイリアス。`DD_DD_URL` がすでに設定されている場合は無視されます。                                                                                                                                                                                                                                                                                   |
| `DD_CHECK_RUNNERS`   | Agent はデフォルトですべてのチェックを同時に実行します (デフォルト値は `4` ランナーです)。チェックを順次実行する場合は、値を `1` に設定します。多数のチェック (または時間のかかるチェック) を実行する必要がある場合、`collector-queue` コンポーネントが遅延してヘルスチェックに失敗する可能性があります。ランナーの数を増やすと、チェックを並行して実行できます。|
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
Datadog Operator を使用する場合、`override` でコンポーネントの環境変数を `[key].env []object` で、コンテナの環境変数を `[key].containers.[key].env []object` で追加設定できます。以下のキーがサポートされています。
...
## DogStatsD の構成 {#configure-dogstatsd}
...
| 環境変数                     | 説明                                                                                                                                                |
|----------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DD_DOGSTATSD_NON_LOCAL_TRAFFIC` | ほかのコンテナからの DogStatsD パケットをリスニングします (Custom Metrics の送信に必要)。                                                                      |
...
## タグマッピングの構成 {#configure-tag-mapping}
...
インテグレーションの資格情報を Docker や Kubernetes のシークレットに格納し、Autodiscovery テンプレートで使用できます。詳細については、[シークレット管理][12]を参照してください。
...
例については、[コンテナのディスカバリー管理][13]ページを参照してください。

**注**: `kubernetes.containers.running`、`kubernetes.pods.running`、`docker.containers.running`、`.stopped`、`.running.total`、および `.stopped.total` メトリクスはこれらの設定の影響を受けません。すべてのコンテナを対象とします。

## Kubernetes API サーバーのタイムアウト {#kubernetes-api-server-timeout}

デフォルトでは、[Kubernetes ステートメトリクスコアチェック][25]は、Kubernetes API サーバーからの応答を 10 秒間待ちます。大規模なクラスターでは、リクエストがタイムアウトし、メトリクスが欠落する可能性があります。
...
| `DD_SKIP_SSL_VALIDATION` | Agent と Datadog との接続時に問題が発生した場合にテストを実施するオプションです。|

## クラスター名の設定 {#set-cluster-name}

一部の機能は Kubernetes クラスター名を設定する必要があります。有効なクラスター名は一意でドットで区切られている必要があり、次の制限があります。

- アルファベットの小文字、数字、ハイフンのみを含めることができる
- 文字で始まる必要がある
- 全体の長さは 80 文字以内である必要がある

{{< tabs >}}
{{% tab "Datadog Operator" %}}
`datadog-agent.yaml` のクラスター名を `spec.global.clusterName` に設定します。
...
{{% tab "Helm" %}}
`datadog-values.yaml` のクラスター名を `datadog.clusterName` に設定します。
...
| 環境変数                 | 説明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DD_LISTENERS`               | 実行する Autodiscovery リスナー。                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `DD_EXTRA_LISTENERS`         | 実行する追加の Autodiscovery リスナー。これは `datadog.yaml` 構成ファイルの `listeners` セクションで定義された変数に加えて追加されます。                                                                                                                                                                                                                                                                                                                                   |
| `DD_CONFIG_PROVIDERS`        | Agent がチェック構成を収集するために呼び出す必要があるプロバイダー。利用可能なプロバイダーは次のとおりです。<br>`kubelet` - Pod アノテーションに埋め込まれたテンプレートを処理します。<br>`docker` - コンテナラベルに埋め込まれたテンプレートを処理します。<br>`clusterchecks` - Cluster Agent からクラスターレベルのチェック構成を取得します。<br>`kube_services` - クラスターのチェックのために Kubernetes サービスをモニタリングします。|
| `DD_EXTRA_CONFIG_PROVIDERS`  | 使用する追加の Autodiscovery 構成プロバイダー。これは `datadog.yaml` 構成ファイルの `config_providers` セクションで定義された変数に加えて追加されます。|

## その他 {#miscellaneous}

| 環境変数                        | 説明                                                                                                                                                                                                                                                         |
|-------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DD_PROCESS_AGENT_CONTAINER_SOURCE` | コンテナソースの自動検出を上書きして、1 つのソースに制限します。たとえば、`"docker"`、`"ecs_fargate"`、`"kubelet"` です。Agent v7.35.0 以降では不要になりました。                                                                                                     |
| `DD_HEALTH_PORT`                    | これを `5555` に設定すると、Agent のヘルスチェックをポート `5555` で公開します。                                                                                                                                                                                                |
| `DD_CLUSTER_NAME`                   | カスタム Kubernetes クラスター識別子を設定して、ホストエイリアスの衝突を回避します。クラスター名は最大 40 文字で、小文字、数字、およびハイフンのみという制限があります。文字で始める必要があります。数字または文字で終わる必要があります。|
...
[18]: /ja/network_monitoring/cloud_network_monitoring/
[19]: /ja/extend/dogstatsd
