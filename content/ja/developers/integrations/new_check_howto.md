...
[integrations-extras リポジトリ][3]を複製します。デフォルトでは、このツールは `$HOME/dd/` ディレクトリで作業することを想定しています。ただし、これは任意で、後でコンフィギュレーションによって調整できます。
...
`setup.py` は、Wheel のパッケージ化とビルドを行う `setuptools` セットアップスクリプトを提供します。Python パッケージの詳細については、[Python プロジェクトのパッケージ][14]を参照してください。
