...
| フィールド   | タイプ   | 説明        |
|---------|--------|--------------------|
| message | 文字列 | The error message. |
| stack   | 文字列 | The stack trace.   |
| type    | 文字列 | The error type.    |

#### IO
| フィールド   | タイプ   | 説明  |
|---------|--------|--------------|
| 値   | 文字列 | Input or output value. This should be used for all spans except for LLM spans. |
| messages| [[Message](#message)] | List of messages. This should only be used for LLM spans. |

#### メッセージ
| フィールド                | タイプ   | 説明              |
|----------------------|--------|--------------------------|
| content [*required*] | 文字列 | The body of the message. |
| ロール                 | 文字列 | The role of the entity.  |

#### Meta
| フィールド       | タイプ              | 説明  |
|-------------|-------------------|--------------|
| kind [*required*]    | 文字列 | The [span kind][2]: `"agent"`, `"workflow"`, `"llm"`, `"tool"`, `"task"`, `"embedding"`, or `"retrieval"`.      |
...
| フィールド       | タイプ              | 説明         |
|-------------|-------------------|---------------------|
| name [*required*]       | 文字列            | The name of the span.          |
| span_id [*required*]     | 文字列            | An ID unique to the span.       |
| trace_id  [*required*]   | 文字列            | A unique ID shared by all spans in the same trace.     |
| parent_id  [*required*]    | 文字列 | ID of the span's direct parent. If the span is a root span, the `parent_id` must be `undefined`. |
...
| status      | 文字列            | Error status (`"ok"` or `"error"`). Defaults to `"ok"`.      |
| モニター     | [メトリクス](#metrics)           | Datadog metrics to collect.         |
| session_id  | 文字列     | The span's `session_id`. Overrides the top-level `session_id` field.    |
| usage-metering-get-hourly-usage-for-lambda-traced-invocations        | [[Tag](#tag)] | A list of tags to apply to this particular span.       |

#### SpansRequestData
| フィールド      | タイプ                          | 説明                                |
|------------|-------------------------------|--------------------------------------------|
| type [*required*]        | 文字列                        | Identifier for the request. Set to `span`. |
| attributes [*required*]  | [SpansPayload](#spanspayload) | The body of the request.  |

#### SpansPayload
| フィールド    | タイプ                | 説明  |
|----------|---------------------|--------------|
| ml_app [*required*] | 文字列              | The name of your LLM application. See [Application naming guidelines](#application-naming-guidelines).     |
| spans [*required*]  | [[Span](#span)] | A list of spans.           |
| usage-metering-get-hourly-usage-for-lambda-traced-invocations                | [[Tag](#tag)]   | A list of top-level tags to apply to each span.        |
| session_id          | 文字列              | The session the list of spans belongs to. Can be overridden or set on individual spans as well. |
...
| usage-metering-get-hourly-usage-for-lambda-traced-invocations        | [[Tag](#tag)] | A list of tags to apply to all the evaluations in the payload.       |

#### EvalMetric

| フィールド                  | タイプ   | 説明  |
|------------------------|--------|--------------|
| ID                     | 文字列 | Evaluation metric UUID (generated upon submission). |
| span_id [*required*]    | 文字列 | The ID of the span that this evaluation is associated with. |
| trace_id [*required*]   | 文字列 | The ID of the trace that this evaluation is associated with. |
| timestamp_ms [*required*] | int64  | A UTC UNIX timestamp in milliseconds representing the time the request was sent. |
| ml_app [*required*] | 文字列 | The name of your LLM application. See [Application naming guidelines](#application-naming-guidelines). |
| metric_type [*required*]| 文字列 | The type of evaluation: `"categorical"` or `"score"`. |
| label [*required*]      | 文字列 | The unique name or label for the provided evaluation . |
| categorical_value [*required if the metric_type is "score"*]    | 文字列 | A string representing the category that the evaluation belongs to. |
| score_value [*required if the metric_type is "score"*]    | 数値 | A score value of the evaluation. |
| usage-metering-get-hourly-usage-for-lambda-traced-invocations        | [[Tag](#tag)] | A list of tags to apply to this particular evaluation metric.       |
