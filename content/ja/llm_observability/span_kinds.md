...
| Kind      | Represents   | Root span?   | Can have child spans? | 例 |
|-----------|--------------|--------------|-------------|----|
| [LLM](#llm-span)      | A call to an LLM. | はい | いいえ | A call to a model, such as OpenAI GPT-4. |
| [Workflow](#workflow-span)  | Any predetermined sequence of operations which include LLM calls and any surrounding contextual operations. | はい | はい | A service that takes a URL and returns a summary of the page, requiring a tool call to fetch the page, some text processing tasks, and an LLM summarization. |
| [Agent](#agent-span)     | A series of decisions and operations made by an autonomous agent, which usually consist of nested workflows, LLMs, tools, and task calls. | はい | はい  | A chatbot that answers customer questions.
| [Tool](#tool-span)      | A call to external programs or services. | いいえ | いいえ | A call to a web search API or calculator. |
| [Task](#task-span)      | A standalone step that does not involve a call to an external service. | いいえ | いいえ | A data preprocessing step. |
| [Embedding](#embedding-span) | A call to a model or function that returns an embedding. | いいえ  | はい | A call to text-embedding-ada-002. |
| [Retrieval](#retrieval-span) | A data retrieval operation from an external knowledge base. | いいえ | いいえ | A call to a vector database that returns an array of ranked documents. |
