...
特定の AWS クラウドサービスでフィルタリングすることで、レイテンシーの特定、データベースのパフォーマンス評価、ネットワークのより完全な視覚化などを便利に行うことができます。たとえば、検索クエリをサービスでフィルタリングしてネットワークマップ内のサービスを表示し、該当するノード上のコミュニケーションをトレースして影響を受けたサービスを確認することができます。

{{< img src="network_performance_monitoring/network_page/cloud-service-hero-docs.png" alt="クラウドサービスマップ" >}}

- **クエリをフィルタリングする**: 検索バーに `service:s3`、`service:kinesis`、`service:elb` などのタグを入力します。サービスの中には、レイテンシーを分割して、`s3_bucket` や `rds_instance_type` といったその他の既定タグで再度絞り込めるものもあります。
- **特定の管理型サービスを視覚化する**: [ネットワークマップ][2]で *View* の横にあるドロップダウンをクリックし、使用したいタグを入力または選択します。マップ内でノードをクリックしてトラブルシューティングのオプションを表示します。
- **サービスのインテグレーションメトリクスを表示する**: ネットワークページで[依存関係テーブル](#table)の行をクリックします。サイドパネルが開くので、*Integration Metrics* タブでクラウドサービスのパフォーマンスを分析し、クライアントサイドとクラウドプロバイダー間の問題を区別します。

NPM は S3、RDS、Kinesis、ELB、Elasticache およびその他の [AWS サービス][3]を自動で検知します。Agent がインストールできないその他のエンドポイント (公開 API など) を監視するには、ネットワーク概要で [`domain` タグ](#dns-resolution)を使用して宛先をグループ化します。
...
詳しくは、[保存済みビュー][4]のドキュメントを参照してください。
