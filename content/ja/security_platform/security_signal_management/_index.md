...
Datadog が何らかのルールに基づいて脅威を検出した場合、セキュリティシグナルが作成されます。すべてのセキュリティシグナルはシグナルエクスプローラーで表示でき、専用のクエリ言語を習得しなくても、簡単に検索、フィルタリング、関連付けを行うことができます。Datadog Cloud Security Platform からシグナルを監視するか、[通知ルール][3]を構成してサードパーティツールにシグナルを送信することが可能です。シグナルのステータスは、次のいずれかになります。
...
3. シグナルのサイドパネルの右上に移動し、ドロップダウンメニューから希望のステータスを選択します。デフォルトのステータスは **Open** です。
