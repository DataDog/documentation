...
セキュリティモニタリング製品は、ログを脅威インテリジェンスと照合したり、[セキュリティルール][1]を適用して攻撃や異常を検出したりするなどして、取り込んだログを分析して脅威をリアルタイムで検出します。

Datadog は、Datadog セキュリティモニタリングサービスによって取り込まれ分析されたギガバイトの総数に基づいて、分析されたログの料金を請求します。デフォルトでは、セキュリティモニタリングは、取り込んだすべてのログを分析して、検出範囲を最大化します。ただし、[セキュリティモニタリング API][2] を使用すると、プログラムでセキュリティフィルターを設定して、取り込んだログのどのサブセットを分析するかを構成できます。
...
**注**: `version` は、更新するフィルターの現在のバージョンを示します。
...
[1]: /ja/security_platform/detection_rules/security_monitoring
