...
The instructions below show you how to configure the task using the [Amazon Web Console][3], [AWS CLI tools][4], or [AWS CloudFormation][5].
...
1. Download [datadog-agent-ecs-fargate.json][1]. **Note**: If you are using Internet Explorer, this may download as a gzip file, which contains the JSON file mentioned below.
...
3. Add your other application containers to the task definition. For details on collecting integration metrics, see [Integration Setup for ECS Fargate][2].
...
For more information on CloudFormation templating and syntax, see the [AWS CloudFormation task definition documentation][2].
...
3. Whenever a Fargate task runs, Fluent Bit sends the container logs to Datadog with information about all of the containers managed by your Fargate tasks. You can see the raw logs on the [Log Explorer page][24], [build monitors][25] for the logs, and use the [Live Container view][26].
...
**Note**: Use a [TaskDefinition secret][27] to avoid exposing the `apikey` in plain text.
...
3. Finally, use the [Datadog Lambda Log Forwarder function][30] to collect logs from CloudWatch and send them to Datadog.
...
2. Instrument your application based on your setup:
   ...
   **Note**: With Fargate APM applications, do **not** set `DD_AGENT_HOST` - the default of `localhost` works.
   ...
   See more general information about [Sending Traces to Datadog][40].
