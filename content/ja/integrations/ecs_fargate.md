...
1. [datadog-agent-ecs-fargate.json][8] をダウンロードします。
...
ECS Fargate のメトリクスを収集するには、次のアクセス許可を [Datadog IAM ポリシー][9]に追加します。ECS ポリシーの詳細については、[AWS Web サイトのガイドを参照][10]してください。
...
ECS Fargate では、タスクを [Replica サービス][11]として実行するオプションしかありません。Datadog Agent は、アプリケーションやインテグレーションコンテナと同じタスク定義内で実行されます。
...
上述のように Datadog Agent をセットアップすると、オートディスカバリーを有効にした状態で [ecs_fargate チェック][12]がメトリクスを収集します。その他のメトリクスを収集するには、同じタスク内の他のコンテナに Docker ラベルを追加します。
...
Datadog Agent コンテナ内の環境変数 `DD_PROCESS_AGENT_ENABLED` を `true` に設定して、Datadog の[プロセス Agent][14] を有効にします。Amazon が Fargate の基底ホストを制御するため、ライブプロセスは Datadog Agent コンテナからしか収集できません。

#### その他の環境変数

Docker Agent コンテナと共に使用できる環境変数については、[Docker Agent][15] ページを参照してください。**注**: 一部の変数は Fargate では使用できません。

グローバルなタグ付けには、`DD_DOCKER_LABELS_AS_TAGS` の使用をお勧めします。Agent は、この方法を使用して、Docker コンテナラベルからタグを取得します。それには、他の Docker コンテナに適切なラベルを追加する必要があります。ラベルは、[タスク定義][16]で直接追加できます。
...
Datadog Agent によって収集されるメトリクスのほかに、Datadog には CloudWatch ベースの ECS インテグレーションがあります。このインテグレーションは、[Amazon ECS CloudWatch メトリクス][17]を収集します。
...
この方法は Datadog Agent を使用しないため、インテグレーションタイルで **ECS** をチェックすることで、Datadog の AWS インテグレーションを構成する必要があります。これで、自動的に Datadog アプリケーションが CloudWatch メトリクス (Datadog 内の `aws.ecs.*` ネームスペースを使用) を取得します。ドキュメントの [収集データ][18] セクションを参照してください。
...
Datadog のデフォルトの CloudWatch クローラーは、10 分ごとにメトリクスをポーリングします。クローリングスケジュールを速くする必要がある場合は、それが可能かどうかを [Datadog のサポートチーム][19]にお問い合わせください。**注**: CloudWatch の API 呼び出しは課金対象なので、AWS 側のコストが増大します。
...
1. Fargate タスクの FireLens ログルーターコンテナで Fluent Bit を有効化します。 FireLens の有効化については、専用の [AWS Firelens ドキュメント][20]を参照してください。Fargate コンテナの定義については、[AWS コンテナ定義ドキュメント][21]を参照してください。AWS では、[リージョン別 Docker イメージ][22]の使用を推奨しています。下記に、Fluent Bit イメージが構成されたタスク定義の例を示します。
...
2. 次に、同じ Fargate タスクで、AWS FireLens をログドライバーとして使用し、Fluent Bit にデータが出力されるようにログコンフィギュレーションを定義します。下記に、ログドライバーに FireLens が使用され、Fluent Bit にデータを出力しているタスク定義の例を示します。
    ...
    **注**: Datadog EU サイトのオーガニゼーションをご利用の場合は、代わりに `Host` オプションとして `http-intake.logs.datadoghq.eu` を使用してください。ご利用可能なパラメーターの全リストは [Datadog Fluentbit ドキュメント][25]に記載されています。

3. これで、Fargate タスクの実行中は常に、Fargate タスクが管理するコンテナの情報を監視しながら、Fluent Bit から Datadog にコンテナログが送信されるようになります。生ログは [Log Explorer ページ][26]や [Live Container ビュー][28]から確認できるほか、[モニターを作成][27]して確認することもできます。

#### AWS LogDriver

`awslogs` ログドライバーと Lambda 関数を使用して Fargate ログを監視し、Datadog にルーティングします。

1. タスクで Fargate AwsLogDriver を定義します。手順については、[AWS Fargate 開発者ガイド][29]を参考にしてください。
...
[9]: https://docs.datadoghq.com/ja/integrations/amazon_web_services/#installation
...
[25]: https://docs.datadoghq.com/ja/integrations/fluentbit/#configuration-parameters
[26]: https://app.datadoghq.com/logs
[27]: https://docs.datadoghq.com/ja/monitors/monitor_types/
[28]: https://docs.datadoghq.com/ja/infrastructure/livecontainers/?tab=linuxwindows
