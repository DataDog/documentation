...
Fastly のログを Datadog アプリケーションに転送するには、Syslog エンドポイントを構成します。
...
4. **Logging** リンクをクリックします。ログエンドポイントページが表示されます。**Syslog** エンドポイントを選択します。
...
3. **Syslog Address** を `intake.logs.datadoghq.com` に設定します。
4. **Port** を `10516` に設定します。
5. **TLS** を `yes` に設定します。
