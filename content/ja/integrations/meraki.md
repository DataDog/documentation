...
このインテグレーションは、[ネットワークデバイスモニタリング][1]、ネットワークイベントログ、および[Cloud SIEM][2] のセキュリティイベントログのメトリクスを収集することで、Cisco Meraki プラットフォームの包括的な可視性を提供します。ログ、メトリクス、推奨モニター、インテグレーションダッシュボードの詳細については、[Meraki インテグレーションタイル][3]を参照してください。
...
デバイスのタグ付けにより、Datadog でデータをフィルタリングして整理する機能が強化され、特定のデバイスグループ、ロケーション、またはデータタイプに掘り下げることができます。これにより、すぐに使えるダッシュボードを使用しながら、トラブルシューティング、パフォーマンスの最適化、およびレポート作成を支援することができます。
