...
Agent v6.18.0/7.18.0 に付属のチェック v5.0.0 より、新しい実装が導入されたことに伴い、コンフィギュレーションの変更が必要になりました。後方互換を維持するため、`use_legacy_implementation` というコンフィギュレーションパラメーターが一時的に利用可能になっています。
