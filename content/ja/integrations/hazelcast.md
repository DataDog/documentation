...
1. Agent のコンフィギュレーションディレクトリのルートにある `conf.d/` フォルダーの `hazelcast.d/conf.yaml` ファイルを編集して、
   ...
   使用可能なすべてのコンフィギュレーションオプションについては、[サンプル hazelcast.d/conf.yaml][3] を参照してください。
...
1. Hazelcast は数々の多様な[ロギングアダプター][7]をサポートします。これは、`log4j2.properties` ファイルの例です。
...
2. Datadog のインテグレーションパイプラインは、デフォルトで次の変換[パターン][8]をサポートします。
...
4. 次のコンフィギュレーションブロックを `hazelcast.d/conf.yaml` ファイルに追加します。環境に基づいて、`path` パラメーターと `service` パラメーターの値を変更してください。使用可能なすべてのコンフィギュレーションオプションの詳細については、[サンプル hazelcast.d/conf.yaml][3] を参照してください。
...
[Agent の status サブコマンドを実行][12]し、**JMXFetch** セクションで `hazelcast` を探します。
