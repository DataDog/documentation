...
- billing_type: flat_fee
  ...
  product_id: steadybit-professional-plan-3-teams
...
[Steadybit][1] は、ネットワークのレイテンシーやコンテナのクラッシュなどの乱流状態を制御された方法でシミュレーションし、システムの信頼性を向上させ、組織をより良いインシデント管理に導くことができるカオスエンジニアリングプラットフォームです。ドラッグアンドドロップで簡単に操作できるユーザーインターフェイスにより、過去のインシデントを再構築することができます。ネットワークの遅延、リソースのストレス、利用できない DNS をシステムにシミュレートし、システムがそれにどう反応するか、観測可能性ソリューションがこれらの障害に気づくかどうかを確認します。
...
このMarketplace 製品は、Steadybit の[プロフェッショナルプラン][3]ライセンス (3 チーム分) です。価格は 1 か月あたりです。

異なるニーズをお持ちの方は、[office@steadybit.com][4] までご連絡いただき、プライベートオファーについてご相談ください。
