...
assets:
  dashboards:
    NGINX Plus base overview: assets/dashboards/istio_overview.json
  monitors: {}
...
short_description: 接続およびリクエストのメトリクスを監視。NGINX Plus でさらに多くのメトリクスを取得可能 Plus.
support: コア
supported_os:
  - linux
  - mac_os
...
server {
  ...
  location /nginx_status {
    ...
    # オープンソース NGINX では、以下を無料で使用できます
    stub_status;

    # バージョン 1.7.5 より前のオープンソース NGINX では、以下を使用します
    # stub_status on;

    # 以下は NGINX Plus でのみ使用できます。
    # status;
  }
}
```
...
2. NGINX のログの収集を開始するには、次の構成ブロックを `nginx.d/conf.yaml` ファイルに追加します。
    ...
    使用可能なすべての構成オプションの詳細については、[サンプル nginx.d/conf.yaml][6] を参照してください。
...
Datadog Agent で、ログの収集はデフォルトで無効になっています。これを有効にするには、[Docker log collection][9] を参照してください。
...
[9]: https://docs.datadoghq.com/ja/agent/docker/log
