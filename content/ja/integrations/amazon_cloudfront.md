...
{{< img src="integrations/amazon_cloudfront/cloudfront_logging_2.png" alt="Cloudfront ロギング 1" popup="true" style="width:70%;">}}

**重要**: 複数のディストリビューションのログファイルは同じバケットに格納してください。ログを有効にする場合は、[どのログファイルがどのディストリビューションに関連付けられているかを追跡できるように][5]、`cloudfront` をファイル名のプレフィックスとして指定します。
