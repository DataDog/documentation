...
"short_description": "Monitors the exposed GPU metrics leveraged by the Nvidia DCGM Exporter"
"supported_os":
- linux
- windows
- macos
...
This check submits metrics exposed by the [NVIDIA DCGM Exporter][1] in Datadog Agent format. For more information on NVIDIA Data Center GPU Manager (DCGM), see [NVIDIA DCGM][2].

## セットアップ

### インストール

Starting from Agent release 7.47.0, the DCGM check is included in the [Datadog Agent][3] package. However, you need to spin up the DCGM Exporter container to expose the GPU metrics in order for the Agent to collect this data. As the default counters are not sufficient, Datadog recommends using the following DCGM configuration to cover the same ground as the NVML integration in addition to having useful metrics.
...
1. Create the file `$PWD/default-counters.csv` which contains the default fields from NVIDIA `etc/default-counters.csv` as well as additional Datadog-recommended fields. To add more fields for collection, follow [these instructions][1]. For the complete list of fields, see the [DCGM API reference manual][2].
...
The DCGM exporter can quickly be installed in a Kubernetes environment using the NVIDIA DCGM Exporter Helm chart. The instructions below are derived from the template provided by NVIDIA [here][1].
...
The DCGM exporter can be installed in a Kubernetes environment by using NVIDIA GPU Operator. The instructions below are derived from the template provided by NVIDIA [here][1].

1. Add the NVIDIA GPU Operator Helm repository and ensure it is up-to-date :
   ```shell
   helm repo add nvidia https://helm.ngc.nvidia.com/nvidia && helm repo update
   ```
2. Follow the [Custom Metrics Config][2] instructions with the CSV from [Installation](#installation) :
...
1. Edit the `dcgm.d/conf.yaml` file (located in the `conf.d/` folder at the root of your Agent's configuration directory) to start collecting your GPU Metrics. See the [sample dcgm.d/conf.yaml][1] for all available configuration options.
...
Use the `extra_metrics` configuration field to add metrics that go beyond the ones Datadog [supports out of the box][2]. See the [NVIDIA docs][3] for the full list of metrics that dcgm-exporter can collect. Make sure to [enable these fields in the dcgm-exporter configuration][4] as well.
...
Set [Autodiscovery Integrations Templates][1] as Docker labels on your DCGM exporter container:
...
The out-of-the-box monitors that come with this integration have some default values based on their alert thresholds. For example, the GPU temperature is determined based on an [acceptable range for industrial devices][6].
...
If you have added some metrics that don't appear in the [metadata.csv][8] above but appear in your account with the format `DCGM_FI_DEV_NEW_METRIC`, remap these metrics in the [dcgm.d/conf.yaml][9] configuration file:
```yaml
...
If a field is not being collected even after enabling it in `default-counters.csv` and performing a `curl` request to `host:9400/metrics`, the [dcgm-exporter developers recommend][10] looking at the log file at `var/log/nv-hostengine.log`.
