...
{{< get-service-checks-from-git "gitlab" >}}
 `gitlab.readiness.*` のサービスチェックについての詳細は、公式の [GitLab ドキュメント][4]に記載されています。
...
GitLab Runner と Prometheus とのインテグレーションについては、[GitLab Runner ドキュメント][6]を参照してください。
...
Runner の Prometheus メトリクスエンドポイントおよびサービスチェックを持つ GitLab マスターを指定するには、[Agent のコンフィギュレーションディレクトリ][8]のルートにある `conf.d/` フォルダーの `gitlab_runner.d/conf.yaml` ファイルを編集します。使用可能なすべてのコンフィギュレーションオプションについては、[サンプル gitlab_runner.d/conf.yaml][9] を参照してください。
...
1. `gitlab_runner` [コンフィギュレーションファイル][10]で、ログフォーマットを `json` に変更します (_GitLab Runner のバージョン 11.4.0 以降で利用可能_) :
   ```toml
   log_format = "json"
   ```
...
4. GitLab Runner のログの収集を開始するには、次の構成ブロックを `gitlab_runner.d/conf.yaml` ファイルに追加します。
   ...
   ```
    ...
    使用可能なすべてのコンフィギュレーションオプションの詳細については、[サンプル gitlab_runner.d/conf.yaml][9] を参照してください。
...
[4]: https://docs.gitlab.com/ee/user/admin_area/monitoring/health_check.html#readiness
[5]: https://docs.datadoghq.com/ja/help/
[6]: https://docs.gitlab.com/runner/monitoring/
...
[9]: https://github.com/DataDog/integrations-core/blob/master/gitlab_runner/datadog_checks/gitlab_runner/data/conf.yaml.example
[10]: https://docs.gitlab.com/runner/configuration/advanced-configuration.html
