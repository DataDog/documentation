...
name: amazon_vpn
public_title: Datadog-AWS VPN Integration
short_description: Track key AWS VPN metrics.
version: '1.0'
---
...
1. In the [AWS integration page][2], ensure that `VPN` is enabled under the `Metric Collection` tab.
2. Install the [Datadog - AWS VPN integration][3].
...
**Note**: If you log to a S3 bucket, make sure that `amazon_vpn` is set as _Target prefix_.

#### Send logs to Datadog

1. If you haven’t already, set up the [Datadog Forwarder Lambda function][4].
2. Once the Lambda function is installed, manually add a trigger on the S3 bucket or CloudWatch log group that contains your AWS VPN logs in the AWS console:
