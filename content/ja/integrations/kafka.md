...
- Kafka ブローカーや Java ベースのコンシューマー/プロデューサーから JMX メトリクスを収集したい場合は、[Kafka Broker インテグレーション][16]を参照してください。
...
Agent の Kafka コンシューマーチェックは、[Datadog Agent][5] パッケージに含まれています。Kafka ノードに追加インストールする必要はありません。
