...
2. Datadog のトリガーを受信すると実行するアクションをフローに入力します。


この例では、"RestartHost" というタイトルのワークフローで、トリガーされると、Datadog がこのワークフローに送信した情報からホストを再起動します。

  - Datadog から送信されたデータで生成される入力変数を追加するには、ワークフローの "Start" にある "Expand" アイコンをクリックして変数パネルを開きます。一致する **Input** 変数を作成するには、すべての入力変数を同等の空要素 `""` に設定します。Datadog では、デフォルトで以下のデータ情報を送信します。
`body`
`last_updated`
`event_type`
`title`
...
エンドポイントが次のように表示されます: ***https://<YOUR_PLIANT_INSTANCE>/api/v1/trigger/<YOUR_PLIANT_USERNAME>/User/<PATH_TO_WORKFLOW>/<WORKFLOW_NOW>?sync=true&api_key=<YOUR_API_KEY>***

![エンドポイント][8]

***https*** で始まり API キーで終わるこのエンドポイントをコピーします。引用符は含めません。
