...
コマンドの出力にリストされる値を `win32_event_log.yaml` で設定して、同種のイベントをキャプチャできます。
...
1 - イベントログに対して 1 つ以上のフィルターを構成します。フィルターを使用して、どのログイベントを Datadog に取り込むかを選択できます。

以下のプロパティで絞り込むことができます。
...
`conf.d/win32_event_log.yaml` の構成ファイルに、フィルターごとにインスタンスを追加します。

いくつかのフィルターの例を示します。

```yaml
instances:
    # 以下は、MSSQLSERVER ソースにすべてのイベントを置く SQL サーバーから
    # エラーと警告をキャプチャし、それらを #sqlserver でタグ付けします。
    -   tags:
            - sqlserver
        type:
            - Warning
    ...
    # 次のインスタンスはすべてのシステムエラーをキャプチャし、#system でタグ付けします。
    -   tags:
            - system
        type:
            - Error
