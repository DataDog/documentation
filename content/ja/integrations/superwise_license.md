...
Superwise のモデル観測性プラットフォームにより、本番用 ML をあらゆる規模で監視することができます。Datadog Marketplace を通じて Superwise のサブスクリプションを購入すると、モデル数無制限の 14 日間無料トライアルが提供されます。トライアル終了後は、最初の 3 モデルは永久に無料です。また、Superwise の料金は使用ベースなので、いつでも監視の規模を拡大または縮小することができます。詳細については、sales@superwise.ai までお問い合わせください。

このタイルでは、Superwise のサブスクリプションを購入することができます。既に Superwise のアカウントをお持ちの場合は、[Superwise インテグレーションタイル](https://app.datadoghq.com/account/settings#integrations/superwise)をクリックして Superwise Datadog インテグレーションをセットアップします。
...
ヘルプが必要な場合は、[Superwise Knowledge Hub](https://docs.superwise.ai) をご覧いただくか、Superwise サポートチームまでお問い合わせください。
