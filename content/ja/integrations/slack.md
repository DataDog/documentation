...
name: slack
public_title: Slack
short_description: Slack は、チームのやり取りを 1 か所に集約できる、ホスト型で全文検索可能なコミュニケーション基盤です。
supported_os: []
tile:
  ...
  description: Slack は、チームのやり取りを 1 か所に集約できる、ホスト型で全文検索可能なコミュニケーション基盤です。
  media: []
...
[4]: https://docs.datadoghq.com/ja/events/explorer/notifications/
{{% /tab %}}
{{< /tabs >}}
...
[Workflow Automation の Slack trigger][10] を使うと、Slack から workflow を直接実行できます。

### Workflow コマンド

Slack から workflow を実行するには:
...
| `remote_files:read`      | 構成済みの Slack チャンネルに共有された、ダッシュボードなどの Datadog 上の PDF レポートを読み取れるようにします。 |
| `remote_files:share`     | ダッシュボードなどの Datadog 上の PDF レポートを、構成済みの Slack チャンネルに共有できるようにします。                  |
| `remote_files:write`     | ダッシュボードなどの Datadog 上の PDF レポートへのリンクを、構成済みのワーク スペースに投稿できるようにします。             |
...
| `files:read`       | incident チャンネルの画像を incident タイムラインに同期します。                               |
| `calls:write`      | incident meeting が開始されたときに、そのチャンネルへ通知を送信します。        |
...
- [Datadog の Slack インテグレーションで incident をスムーズに管理][14]
...
[10]: https://docs.datadoghq.com/ja/service_management/workflows/trigger/#slack-triggers
