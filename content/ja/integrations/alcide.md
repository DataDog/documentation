...
2. URL 欄に `https://http-intake.logs.<DATADOG_SITE>/v1/input/<DATADOG_API_KEY>?ddsource=alcide` と入力し、プレースホルダー `<DATADOG_SITE>` の値を米国サイトは `datadoghq.com`、ヨーロッパサイトは `datadoghq.eu` に置き換えます。プレースホルダー `<DATADOG_API_KEY>` の値はお使いの [Datadog API キー][1]に変更してください。
...
6. _Entities Matching_ および _Entities Not Matching_ ボックスを使用して、エンティティ上にホワイトリストとブラックリストを作成することができます (オプション) 。
