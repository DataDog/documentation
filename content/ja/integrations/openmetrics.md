...
[Agent の構成ディレクトリ][6]の root にある `conf.d/openmetrics.d/conf.yaml` ファイルを編集します。利用可能なすべての構成オプションについては、[サンプル openmetrics.d/conf.yaml][7] を参照してください。これは、Datadog Agent バージョン 7.32.0 時点での最新の OpenMetrics チェック例です。以前にこのインテグレーションを実装していた場合は、[レガシー例][8]を参照してください。
