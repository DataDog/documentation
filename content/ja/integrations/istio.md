...
それぞれのエンドポイントはオプションですが、少なくとも 1 つ構成する必要があります。Prometheus アダプターの詳細については、[Istio ドキュメント][6]を参照してください。

注: `connectionID` Prometheus ラベルは除外されます。

##### サイドカーインジェクションの無効化

[コンテナに Datadog Agent を][7]インストールする場合は、その前に Istio のサイドカーインジェクションを無効にすることをお勧めします。
...
Istio には 2 種類のログがあります。[Envoy インテグレーション][8]で収集される Envoy アクセスログと、[Istio ログ][9]です。 
...
[Agent の `info` サブコマンドを実行][11]し、Checks セクションで `istio` を探します。
...
`istio.prometheus.health`: Agent がメトリクスのエンドポイントに到達できない場合は `CRITICAL` を返します。それ以外の場合は、`OK` を返します。

Istio の他のすべてのバージョンの場合

`istio.pilot.prometheus.health`: Agent がメトリクスのエンドポイントに到達できない場合は `CRITICAL` を返します。それ以外の場合は、`OK` を返します。

`istio.galley.prometheus.health`: Agent がメトリクスのエンドポイントに到達できない場合は `CRITICAL` を返します。それ以外の場合は、`OK` を返します。

`istio.citadel.prometheus.health`: Agent がメトリクスのエンドポイントに到達できない場合は `CRITICAL` を返します。それ以外の場合は、`OK` を返します。
...
- [Datadog を使用した Istio サービスメッシュの監視][14]
- [Datadog でキーメトリクスを収集して Istio を監視する][15]
...
[6]: https://istio.io/docs/tasks/telemetry/metrics/querying-metrics
[7]: https://docs.datadoghq.com/ja/agent/kubernetes/
[8]: https://docs.datadoghq.com/ja/integrations/envoy/#log-collection
[9]: https://istio.io/docs/tasks/telemetry/logs/collecting-logs/
...
[12]: https://github.com/DataDog/integrations-core/blob/master/istio/metadata.csv
[13]: https://docs.datadoghq.com/ja/help/
[14]: https://www.datadoghq.com/blog/monitor-istio-with-datadog
[15]: https://www.datadoghq.com/blog/istio-metrics/