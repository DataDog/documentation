...
1. [Agent のコンフィギュレーションディレクトリ][4]のルートにある `conf.d/` フォルダーの `sqlserver.d/conf.yaml` ファイルを編集します。
    ...
    **注**: (デフォルトの) プロバイダー `SQLOLEDB` は、非推奨になります。新しい `MSOLEDBSQL` プロバイダーを使用するには、[Microsoft][6] からこのプロバイダーをダウンロードし、`sqlserver.d/conf.yaml`ファイルで `adoprovider` 変数を `MSOLEDBSQL` に設定します。また、以下のように指定することで、Windows 認証を使用する際に、ユーザー名とパスワードの入力を求めないようにすることもできます。
...
2. SQL Server のログの収集を開始するには、次の構成ブロックを `sqlserver.d/conf.yaml` ファイルに追加します。
    ...
    `path` パラメーターと `service` パラメーターの値を環境に合わせて変更してください。使用可能なすべてのコンフィギュレーションオプションの詳細については、[sqlserver.d/conf.yaml のサンプル][5]を参照してください。
...
`<UNIQUEPASSWORD>` をラベルではなく環境変数として渡す方法について、詳細は[オートディスカバリーテンプレート変数][10]を参照してください。
...
[Agent の `status` サブコマンドを実行][11]し、Checks セクションの `sqlserver` を探します。
...
Agent ベースのインテグレーションのテストおよび開発方法については、[メインドキュメント][14]を参照してください。
...
Linux では、MSSQL インスタンスを実行する Docker コンテナが、テストの実行前に自動的に起動されます。データベースとの対話には unixODBC と [FreeTDS][15] が使用されます。このため、Linux ディストリビューションによっては、テストを実行する前に、ローカルの開発環境に追加の依存関係をインストールする必要があります。たとえば、Ubuntu 14.04 の場合のインストール手順は以下のとおりです。
...
Linux と同様に、MSSQL は Docker コンテナで実行され、データベースとの対話には unixODBC と [FreeTDS][15] が使用されます。[homebrew][16] を使用して必要なパッケージをインストールできます。
...
- [Datadog を使用した Azure SQL Database の監視][17]
- [SQL Server 監視のためのキーメトリクス][18]
- [SQL Server 監視ツール][19]
- [Datadog を使用した SQL Server パフォーマンスの監視][20]
- [カスタム SQL Server メトリクスによる詳細な監視][21]
...
[10]: https://docs.datadoghq.com/ja/agent/faq/template_variables/
...
[14]: https://docs.datadoghq.com/ja/developers/integrations/
