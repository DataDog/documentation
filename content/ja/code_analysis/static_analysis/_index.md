...
Datadog Static Analysis では、2 種類の修正提案があります。

1. **Default Suggested Fix:** lint の問題のような単純な違反については、ルールアナライザーが自動的にテンプレート化された修正を提供します。
2. **AI Suggested Fix:** 複雑な違反の場合、通常は事前に修正することはできません。代わりに、OpenAI の GPT-4 を使って、修正提案を生成する AI Suggested Fix を使うことができます。"Text" と "Unified Diff" の修正が選択でき、それぞれ違反を解決するためのプレーンテキストの指示またはコード変更が出力されます。

この 2 種類の修正は、UI 上では異なるラベルで視覚的に区別されます。
