...
Python と Node.js のサーバーレスアプリケーションでは、Datadog は [Datadog のトレーシングライブラリをインストールする][5]ことを推奨しています。
...
[Datadog のトレーシングライブラリ][5]を使って、Datadog でサーバーレス関数をトレースすることができます。
...
Go サーバーレスアプリケーションでは、Datadog は [Datadog のトレーシングライブラリ][5]のインストールを推奨しています。
...
Java サーバーレスアプリケーションでは、Datadog は [Datadog のトレーシングライブラリ][5]のインストールを推奨しています。
...
.NET サーバーレスアプリケーションでは、Datadog は [Datadog のトレーシングライブラリ][5]のインストールを推奨しています。
...
<div class="alert alert-info">ベータ期間中は、追加料金なしでプロファイリングをご利用いただけます。</div>

Datadog の [Continuous Profiler][27] はバージョン 4.62.0 とレイヤーバージョン 62 以上の Python でベータ版が利用可能です。このオプション機能は `DD_PROFILING_ENABLED` 環境変数を `true` に設定することで有効になります。

Continuous Profiler は、周期的に起動し、実行中のすべての Python コードの CPU とヒープのスナップショットを取得するスレッドを生成することで機能します。これにはプロファイラー自身も含まれる場合があります。プロファイラー自身を無視したい場合は、 `DD_PROFILING_IGNORE_PROFILER` を `true` に設定します。
...
## X-Ray インテグレーションで Datadog へトレースを送信する

すでに X-Ray でサーバーレスアプリケーションをトレースしており、引き続き X-Ray を利用したい場合は、[AWS X-Ray インテグレーションをインストール][2]して、X-Ray から Datadog にトレースを送信することができます。
