...
**注**: Lambda 関数名は [Datadog のタグ付け規則][2]に準拠する必要があります。これにより、関数のすべてのトレース、ログ、メトリクスがシームレスに結び付けられます。
...
`version` タグを追加して、[デプロイ追跡][3]を有効にします。

## サービスマップを整理する

[サービスマップ][4]では、`env` タグによりサービスがマップにグループ化されます。そして `service` タグを使用して、サービスとモニターの正常性との関係を表示します。サービスは、サービスマップ上の個々のノードとして表されます。ノードの色は、関連付けられたモニターの正常性を示しています。関連付けるには、モニターに同じ `service` タグを付けます。
...
[3]: /ja/tracing/deployment_tracking/
[4]: /ja/tracing/visualization/services_map/