...
Datadog に取り込む前のカスタムメトリクスから特定のタグを削除するには、メトリクスを操作するドメイン固有の言語を備えた観測可能性パイプラインの [`remap` 変換][7] を使用できます。
...
- `limit_exceeded_action`: カーディナリティの構成された制限を超えるタグを持つメトリクスが来た場合の対応を制御します。設定可能なオプションは以下の通りです。
