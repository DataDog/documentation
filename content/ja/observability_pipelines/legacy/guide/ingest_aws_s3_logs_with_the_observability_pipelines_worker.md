...
title: (LEGACY) Observability Pipelines Worker で Amazon S3 Logs を取り込む
---

## 概要

[Observability Pipelines Worker][1] は、さまざまなソースからログを取り込むことができます。AWS CloudTrail や CloudWatch などの外部システムからログを受信している Amazon S3 バケットがある場合、ログを取り込むように Worker を構成することができます。この設定では、Observability Pipelines Worker の Amazon S3 ソースを使用します。これにより、S3 バケットからのイベント通知を受信するために Amazon SQS キューを設定する必要があります。そしてイベント通知は、S3 バケットに新しいログイベントを収集するように Worker に通知します。 
...
Amazon SQS コンソールで、この構成専用の新しいキューをプロビジョニングします。これにより、他に使用しているログ分析ツールの設定とは分離され、そこで行った変更が独立して管理されます。
...
1. [Amazon S3 コンソール][5]で、Worker に取り込みませたいログを収集している S3 バケットに移動します。
...
Amazon S3 ソースのセットアップ完了後、データを操作するための [変換][8]の追加、およびユースケースに基づいたログを出力するための[シンク][9]を追加することができます。ソース、変換、シンクの詳細については、[Configurations][3] を参照してください。 

## バッチされた Amazon S3 ログイベントを分離するように Worker を構成する
