...
パイプラインはデータ収集から始まります。サービスやシステムが生成するデータ[*](#support) は、収集してダウンストリームの送信先へ送ることができます。データ収集はエージェントによって行われます。どのエージェントを使うべきかを理解することで、必要なデータを確実に収集できます。
...
Observability Pipelines Worker を使用して、データ[*](#support) を処理することができます。しかし、連続的なプロファイリングデータのような、リアルタイムのベンダー特有データは相互運用性がなく、一般的に処理するメリットはありません。
...
<a name="support"></a> * Observability Pipelines はログをサポートします。メトリクスのサポートはベータ版です。