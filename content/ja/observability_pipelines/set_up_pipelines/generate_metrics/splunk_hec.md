---
disable_toc: false
title: Splunk HTTP Event Collector (HEC) のログからメトリクスを生成する
---

## 概要

Splunk HTTP Event Collectors (HEC) を構成してログを Observability Pipelines Worker に送信し、これらのログからメトリクスを生成できるようにします。

{{% observability_pipelines/use_case_images/generate_metrics %}}

このドキュメントでは、デュアル シッピングを設定するための以下の手順を説明します:
...
[1]: /ja/observability_pipelines/dual_ship_logs/splunk_tcp/
