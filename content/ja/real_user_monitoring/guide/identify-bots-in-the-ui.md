...
ボットセッションをフィルターするために、アプリケーションが RUM ブラウザ SDK を初期化するときに、`user-agent` を[既知のロボットリスト][3]に対してチェックすることによって、条件付きで `sampleRate` を `0` に設定することが可能です。
...
// RUM ブラウザ SDK を初期化し、sampleRate を conditionalSampleRate に設定します
datadogRum.init({
 // ... 構成オプション
