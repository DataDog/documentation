...
- リクエスト本文にはバイナリデータを含めることができ、文字列に変換してはいけません。プロキシの実装が変換なしで生のボディを転送することを確認してください。
- プロキシの実装が、悪意のある利用者によって別のサーバーにリクエストが送信されることを防ぐようにしてください。たとえば: `https://browser-intake-datadoghq.com.malicious.com`。
{% /alert %}
...
### 初期化パラメーターに関数を渡す `proxy`{% #passing-a-function-to-the-proxy-initialization-parameter %}
