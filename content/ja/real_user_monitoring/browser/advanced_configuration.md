...
RUM によって[収集されたデータおよびコンテキスト][1]を変更して、次のニーズをサポートするには、さまざまな方法があります。
...
### React ルーターインスツルメンテーション

React アプリケーション内で定義した通りにデフォルトの RUM ビュー名をオーバーライドするには、以下の手順に従う必要があります。

**注**: これらの手順は、**React Router v6** ライブラリ固有のものです。

1. [上記](#override-default-rum-view-names)にあるように、RUM ブラウザ SDK を初期化する際に `trackViewsManually` を `true` に設定します。

2. 各ルート変更ごとにビューの追跡を開始します。
