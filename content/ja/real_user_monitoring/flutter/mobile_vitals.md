...
| ウィジェットビルド時間             | これは UI スレッドでフレームを構築するのにかかる時間です。スムーズなアニメーションを保証するために、60 FPS の場合は 16ms、120 FPS の場合は 8ms を超えないようにしてください。<br /><br />この値が高い場合は、このビューのビルド方法を最適化する必要があることを意味します。Flutter ドキュメントの[ビルドコストの管理][3]を参照してください。 |
| ラスタ時間             | これは、ラスタスレッドでフレームをラスタライズするのにかかる時間です。スムーズなアニメーションを保証するために、60 FPS の場合は 16ms、120 FPS の場合は 8ms を超えないようにしてください。<br /><br />この値が高い場合は、ビューのレンダリングが複雑であることを意味します。Flutter ドキュメントの [GPU グラフの問題の特定][4]を参照してください。 |
