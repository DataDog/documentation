...
1. 子組織にログインした状態で、[Plan & Usage][2] に移動します。
1. **Usage** タブをクリックします。
...
推定コストデータは当月分です。前月の履歴コストデータがない場合は、前月の推定コストデータも表示されます。
...
Datadog の請求は月によって異なります。各製品の請求に使用される集計の種類によって、そのコストが安定したままか、月が進むにつれて増加するかが決まります。最適な視覚化については、[コスト サマリー][5]機能チャートをご覧ください。各 **Products** フィルターには、製品名の横に関連する請求集計方法が含まれています。

### パーセンタイルおよび平均使用量請求

月間使用量の下位 99 パーセントの最大カウントで請求される製品には、インフラストラクチャーホストおよび APM ホストが含まれます。月間平均で請求される製品には、カスタムメトリクスと Fargate タスクが含まれます。これら 2 つのタイプの製品については、コストは月を通して比較的安定していると予想されます。ただし、コストは変動する可能性があります。

### 使用量請求の合計

月間使用量の合計で請求される製品には、インデックス化ログと取り込みログが含まれます。これらのタイプの製品については、月が進むにつれてコストが着実に増加することが予想されます。
