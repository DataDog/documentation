...
description: ロールベースのアクセス権限、カスタムロール、およびダッシュボード、モニター、その他の Datadog リソースに対する詳細なアクセス制御により、ユーザーアクセスを管理します。
further_reading:
- link: /api/v2/roles/
  tag: ドキュメント
...
ロールによりユーザーを分類し、各ユーザーが読み取ることができるデータや変更できるアカウントアセットなど、アカウントのアクセス許可を定義します。Datadog ではデフォルトで 3 つのロールを提供していますが、[カスタムロール](#custom-roles)を作成すると、ユーザーとアクセス許可のマッピングをより適切に定義できます。
...
## Datadog のデフォルトのロール {#datadog-default-roles}
...
Datadog Read Only ロール
Datadog システム内での編集権限を持たないユーザーです。特定の機能を読み取り専用ビューでクライアントに共有したり、ある部門のメンバーから外部ユーザーに[ダッシュボード][5]を共有したりする必要がある場合に便利です。

## カスタムロール {#custom-roles}

カスタムロール機能を使用すると、オーガニゼーションで一意の権限セットを持つ新規ロールを作成できます。Datadog サイト、[Datadog ロール API][8]、または SAML から直接カスタムロールを管理できます。下記からロールの作成、更新、削除方法をご確認ください。利用可能なアクセス許可の詳細については、[Datadog ロールのアクセス許可][1]を参照してください。Datadog でロールを作成または編集できるのは、ユーザーアクセス管理のアクセス許可を持つユーザーのみです。

### カスタムロールを有効にする {#enable-custom-roles}

1. [組織の設定][9]に移動します。
2. ページ左側の {{< ui >}}Roles{{< /ui >}} を選択します。
3. 右上隅のギアアイコンをクリックすると、Custom Roles のポップアップウィンドウが開きます。
4. Custom Roles のポップアップで {{< ui >}}Enable{{< /ui >}} をクリックします。

{{< img src="account_management/rbac/enable_custom_roles.png" alt="Custom Roles ポップアップウィンドウの Enable ボタン" style="width:90%;">}}

代替として、POST 呼び出しを [ロールの作成 API エンドポイント][10]宛てにすると、自動的にオーガニゼーションに対するカスタムロールが有効になります。

### カスタムロールを作成する {#create-a-custom-role}

{{< tabs >}}
{{% tab "Datadog アプリケーション" %}}

カスタムロールを作成するには

1. [Datadog ロールページ][1]に移動します。
2. ページ右上隅の{{< ui >}}New Role{{< /ui >}}を選択します。
3. ロールに名前を付けます。
...
ロールが削除されると、そのロールを持つすべてのユーザーのアクセス許可が更新されます。ロールのないユーザーは、Datadog を効果的に使用できませんが、制限付きのアクセスを保持します。
