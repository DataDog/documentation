...
* **サーバーレス関数**は、定義されたイベントまたはアクションに応答して、クラウドプラットフォームのサーバーレスコンピューティングサービス上で実行するように設定されたアプリケーションコードです (AWS Lambda、Google Cloud Function、Azure Function など)。Datadog では、1 か月の Lambda 関数呼び出しの総数に基づいて課金されます。
...
* ホスト（[インフラストラクチャーモニタリング](#infrastructure-monitoring)で定義）がインスツルメントされたアプリケーションからトレースを受信する場合、Datadog で 1 件の **APM ホスト**としてカウントされます。
  * Datadog の最高水準プラン (HWMP) では、月末にこの 1 時間ごとの測定結果が最高から最低まで順序付けられ、そのうち 8 番目に高い測定値に基づいて課金されます。
  * 月単位/時間単位混合プラン (MHP) では、月単位の最低契約量に対して課金され、ホスト数がその契約量を超えた時間については、時間単位の料金が課金されます。
...
* Datadog は、お客様が Datadog データベースモニタリングで監視しているデータベースのユニークホスト数を 1 時間ごとに記録します。
  * Datadog の最高水準プラン (HWMP) では、月末にこの 1 時間ごとの測定結果が最高から最低まで順序付けられ、そのうち 8 番目に高い測定値に基づいて課金されます。
  * 月単位/時間単位混合プラン (MHP) では、月単位の最低契約量に対して課金され、ホスト数がその契約量を超えた時間については、時間単位の料金が課金されます。
* クエリダイジェストとも呼ばれる**正規化されたクエリ**は、クエリのパラメーターのみが異なる同様の構造を持つクエリの集合を表します。Datadog では、指定時間に追跡された、構成済みの正規化されたクエリの合計数に基づき請求が発生します。
