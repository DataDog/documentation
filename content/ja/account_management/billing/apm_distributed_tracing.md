...
| 請求パラメーター  | 料金                                      | インデックス化されたスパン                                                                 | 課金                                                                                                                                                                                                                                                                                                                          |
|--------------------|--------------------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [APM ホスト][4]      | 基底の [APM ホスト][4] あたり 31 ドル / 月 | 各 APM ホストに対し、100 万件 / 月の Indexed Span を含む   | お客様が Datadog APM サービスで同時に監視している [APM ホスト][5]数を 1 時間に一度記録します。最高水準プラン (HWMP) では、月末にこの 1 時間ごとの測定結果が最高から最低まで順序付けられ、そのうち 8 番目に高い測定値に基づき課金されます。[詳しくはこちら。][5] |
| [Fargate][4]       | 1 件の同時タスクあたり 2 ドル / 月           | Indexed Span は料金に含まれません                                        | お客様が Datadog APM サービスで監視しているタスクのインスタンス数を 5 分間隔で記録します。月末にこの間隔に基づく測定値を集計し、アプリケーションが実行されて監視された合計時間数に基づいて課金されます。[詳しくはこちら][4]              |
...
| インデックス化されたスパン | 3,000 万件 | 5 つの APM ホストに 500 万件の Indexed Span が含まれるため、残りの 2,500 万件に対し 1.70 ドル / 100 万件 | 25 × 1.70 ドル    | 42.50 ドル                |
...
| インデックス化されたスパン | 2,000 万件 | 5 つの APM ホストに 500 万件の Indexed Span が含まれるため、残りの 1,500 万件に対し 1.70 ドル / 100 万件 | 15 × 1.70 ドル          | 25.50 ドル                |
...
| インデックス化されたスパン | 2,000 万件 | 1 つの APM ホストに 100 万件の Indexed Span が含まれるため、残りの 1,900 万件に対し 1.70 ドル / 100 万件 | 19 × 1.70 ドル   | 32.30 ドル               |
...
| インデックス化されたスパン    | 2,000 万件 | 20 の APM ホスト (ノード) に 2,000 万件の Indexed Span が含まれるため、追加はなし | 0 × 1.70 ドル | 0                  |
...
Lambda 関数を 1 か月間毎時間継続して呼び出し、2,000 万件の Indexed Span を送信。

| 対象ユニット     | 数量   | 料金                                                                       | 計算式   | 小計           |
|-------------------|------------|-----------------------------------------------------------------------------|-----------|--------------------|
| Lambda 関数   | 1          | [月額 5 ドル][8]                                               | 1 × 5 ドル  | 5 ドル               |
| インデックス化されたスパン    | 2,000 万件 | Indexed Span 100 万件あたり 1.70 ドル | 20 × 1.70 ドル | 34 ドル               |
