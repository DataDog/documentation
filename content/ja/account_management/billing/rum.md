...
Datadog は、エンドユーザーが訪問したすべてのページを、読み込みリソース (XHR、イメージ、CSS ファイル、JS スクリプト等)、フロントエンドエラー、クラッシュレポート、長時間のタスクなど、重要なテレメトリーと一緒に収集します。これらはすべてユーザーセッションに含まれるデータです。Datadog リアルユーザーモニタリング (RUM) サービスで収集されたセッション 1 万件ごとに課金されます。
