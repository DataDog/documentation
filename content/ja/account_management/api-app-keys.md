...
組織の API キーと組み合わせて[アプリケーションキー][1]を使用すると、Datadog のプログラム API に完全にアクセスできます。アプリケーションキーは、これを作成したユーザーアカウントに関連付けられており、名前が必要です。アプリケーションキーは、API に対して行われたすべてのリクエストをログ出力するために使用されます。
...
Datadog API キー、アプリケーションキー、またはクライアントトークンを追加するには、[Integration-> APIs][1] に移動し、キーまたはトークンの名前を入力して、**Create API key**、**Create Application Key**、または、**Create Client Token** をクリックします。
...
Datadog API キー、アプリケーションキー、またはクライアントトークンを削除するには、[Integration-> APIs][1] に移動し、削除するキーまたはトークンの横にある **Revoke ** ボタンを選択します。
...
API キーが定められた上限の 5 つを超えて必要な場合は、上限の引き上げについて[サポートチーム][4]までお問い合わせください。
