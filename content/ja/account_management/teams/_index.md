---
description: チーム資産を整理し、Datadogの体験をフィルタリングし、チームハンドル、通知、リソースの関連付けを使ってチームメンバーシップを管理します。
further_reading:
- link: https://www.datadoghq.com/blog/datadog-teams-github-integration
  tag: ブログ
  text: Datadog TeamsのGitHub統合を使用して、サービスの所有権を最新の状態に保ちます。
title: Teams
---
## 概要 {#overview}
Datadog Teams は、ユーザーグループが Datadog 内でチームのアセットを整理し、Datadog 全体のエクスペリエンスに自動的にフィルターをかけて、これらのアセットに優先順位をつけることができるようにします。

Teamsを使用して、ダッシュボード、サービス、モニター、インシデントなどのリソースをユーザーグループにリンクします。チーム固有のリンクをSlackチャンネル、Jiraボード、GitHubリポジトリなどに追加することもできます。

チームメンバーシップは柔軟です。ユーザーはチームに参加したり、他のメンバーによって追加されたり、管理者によって追加されたりできます。ユーザーは複数のチームに所属できます。

## セットアップ {#setup}

### ナビゲーション {#navigation}

[組織設定][1]からチームディレクトリページにアクセスするか、[**Teams**][2]に移動します。[チームディレクトリページ][1]には、組織内のすべてのチームが一覧表示されます。

### チームの作成 {#create-team}

1. [チームディレクトリページ][1]で、右上の {{< ui >}}New Team{{< /ui >}} をクリックします。
1. {{< ui >}}Team Name{{< /ui >}} を選択します。
1. チームハンドルは、チーム名に基づいて入力されます。
1. ドロップダウンメニューを使用してチームメンバーおよびチームマネージャーを選択します。
1. オプションの {{< ui >}}Description{{< /ui >}} を提供します。
1. {{< ui >}}Create{{< /ui >}} をクリックします。

**注**: 

-  チーム名に使用できる文字は `a-z`、 `A-Z`、 `0-9`、および `._-:/` です。スペースをアンダースコアに置き換えます。
-  チームハンドルに使用できる文字は `a-z`、 `0-9`、および `._-:/` です。最後の文字はアンダースコアにできません。

###  チームを修正 {#modify-team}

1.  [チームディレクトリページ][1]で、修正したいチームをクリックします。[チーム詳細ページ][3]が表示されます。
1. 画面の上部にある {{< ui >}}Settings{{< /ui >}} 歯車をクリックします。ポップアップウィンドウが表示されます。
1.  修正したい項目を選択します。
1.  変更を行い、次に {{< ui >}}Save{{< /ui >}} をクリックします。

###  プロビジョニングソースを選択 {#choose-provisioning-source}

管理者とチームマネージャーがチームメンバーシップを更新する方法を 3 つのオプションから選択します。

UI と API
: UI アクションと API 呼び出しのみでメンバーシップを更新します。

SAML
: アイデンティティプロバイダーのデータがチームメンバーシップを決定するように、*SAML厳格*モデルを使用します。

すべてのソース
: SAML を出発点として使用し、UI および API によるオーバーライドを許可します。

1. [チームディレクトリページ][1]で、{{< ui >}}Teams Settings{{< /ui >}}をクリックします。
1. {{< ui >}}Team Provisioning Sources{{< /ui >}}の下のオプションのいずれかを選択します。

既存のメンバーを持つチームがある場合、SAML厳格オプションを選択すると設定が上書きされ、これらのチームからメンバーが削除されます。すべてのソースオプションを選択すると、既存のメンバーシップが保持されます。SAML 属性を使用してチームおよびチームメンバーシップを管理するには、[SAML 属性のチームへのマッピング][4]を参照してください。

## チームハンドル {#team-handle}

チームハンドルは、チームをDatadogリソースにリンクします。チームハンドルは、`team:<team-handle>`または`teams:<team-handle>`の形式で検索バーやファセットに表示されます。

チームハンドルを探すには
1. チームディレクトリページで、チームの名前をクリックします。チーム詳細ページが表示されます。
1. チームハンドルはページ上部の名前の右側に表示されます。

定義されたチームにリソースを関連付けるには、Datadogに一致するチームハンドルを持つチームが存在する必要があります。定義されたチームに関連付けられたリソースをクリックすると、チームハンドルと追加情報を含む小さなウィンドウが表示されます。定義されたチームは、下のチームフィルターなどの追加機能を提供します。

Datadogで定義されたチームに関連付けられていないチームハンドルは、タグと同様に動作します。未定義のチームハンドルを定義されたチームに変換して、Teams の機能を活用してください。

### リソースとチームハンドルを関連付ける {#associate-resources-with-team-handles}

Datadog は、以下のリソースをチームハンドルに関連付けることをサポートしています。

- [ダッシュボード][5]
- [Incidents][6]
- [Monitors][7]
- [Resource Catalog][8]
- [Software Catalog][9]
- [SLO][10]
- Synthetic テスト、グローバル変数、プライベートロケーション

### 特定のコミュニケーションチャネルに通知を送信します {#send-notifications-to-a-specific-communication-channel}

チームに通知チャネルを追加して、SlackやMicrosoft Teamsなどのコミュニケーションチャネルにアラートをルーティングします。`@team-<handle>`をターゲットにしたアラートを監視し、選択したチャンネルにリダイレクトされます。

1. [チームディレクトリページ][1]で、修正したいチームをクリックします。
1. 画面の上部にある{{< ui >}}Settings{{< /ui >}}の歯車をクリックします。ポップアップウィンドウが表示されます。
1. {{< ui >}}Notifications{{< /ui >}}を選択します。
1. チャンネルを追加し、次に{{< ui >}}Save{{< /ui >}}をクリックします。

## チームフィルター {#team-filter}

チームフィルターは、あなたのチームに関連するコンテンツを表示することで、Datadog全体での体験を調整します。{{< ui >}}My Teams{{< /ui >}}リストには、あなたがメンバーであるチームと、お気に入りとして選択したチームが含まれています。

{{< img src="/account_management/teams/team-filter.png" alt="チームフィルターの周りに赤いボックスがあるモニターリストページ。選択された「マイチーム」のうち、3つ中2つ。">}}

チームフィルターを有効にすると、あなたのチームに関連するリソースや、あなたのチームが所有するサービスに関連するリソースのみが表示されます。チームフィルターの状態はグローバルで持続的であるため、Datadogは異なる製品をナビゲートする際にあなたのチームコンテキストを適用します。

チームフィルターは、検索クエリにチームベースの検索用語を追加することで機能します。チームフィルターを有効にすると、検索バーに追加されるチームベースの検索用語を見ることができます。

### お気に入りのチーム {#favorite-teams}

特定のチームのリソースに興味があるかもしれませんが、そのチームのメンバーではない場合があります。お気に入りのチームにチームを追加すると、そのチームのリソースに対するフィルタービューを取得でき、チームに参加することなく利用できます。

お気に入りにしたチームは、自分が所属するチームとともにチームディレクトリページの上部やチームフィルター内に表示されます。

#### お気に入りのチームの追加または削除 {#add-or-remove-favorite-teams}

チームをお気に入りに追加または削除するには、チームディレクトリページまたはチームフィルターから行えます。

[チームディレクトリページ][1]から、
1. お気に入りとして追加したいチームをクリックします。[チーム詳細ページ][3]が表示されます。
1. 右上の {{< ui >}}Add Favorite{{< /ui >}} または {{< ui >}}Remove Favorite{{< /ui >}} をクリックします。

あるいは、同じくチームディレクトリページで、
1. 追加または削除したいチームにカーソルを合わせます。チーム名の右側にインラインアイコンが表示されます。
1. 星 ({{< ui >}}Add to Favorites{{< /ui >}} または {{< ui >}}Remove from Favorites{{< /ui >}}) アイコンをクリックします。

チームフィルターから、
1. フィルターが折りたたまれている場合、{{< ui >}}My Teams{{< /ui >}} をクリックして展開します。
1.  {{< ui >}}Add Favorites{{< /ui >}} をクリックします。検索ボックスとチームのリストが表示されます。
1. チームのリストを絞り込むには、検索ボックスにチーム名を入力し始めます。
1. 目的のチームの横にある星をクリックして、お気に入りに追加または削除します。

### 対応製品 {#supported-products}

以下の表は、チームフィルターを使用できる製品を示します。

| 製品リストページ | フィルターの基準 |
|--------------------------------|------------------------------------------------------------------------------------|
| [APM Error Tracking][15]       | チームが所有するサービス（[Software Catalog][12]内の所有権によって決定されます） |
| [Apps][21]                     | チームハンドル |
| [Case Management projects][22] | チームハンドル |
| [Connections][23]              | チームハンドル |
| [Connection Groups][24]        | チームハンドル |
| [Cross Org Connections][25]    | チームハンドル |
| [Datastores][26]               | チームハンドル |
| [Data Streams Monitoring][18]  | チームハンドル                                                                        |
| [Dashboards][11]               | チームハンドル                                                                        |
| [インシデント][13]                | チームハンドル                                                                        |
| [Integrations][27]             | チームハンドル                                                                        |
| [ログエラートラッキング][16]      | チームが所有するサービス ([ソフトウェアカタログ][12]内での所有権により決定) |
| [ログパイプライン][28]           | チームハンドル                                                                        |
| [モニター][14]                 | チームハンドル                                                                        |
| [ノートブック][20]                | チームハンドル                                                                        |
| [Observability Pipelines][29]  | チームハンドル                                                                        |
| [オンコール][30]                  | チームが所有するサービス ([ソフトウェアカタログ][12]内での所有権により決定) |
| [パワーパック][32]               | チームハンドル                                                                        |
| [プライベートアクションランナー][31]    | チームハンドル                                                                        |
| [リファレンステーブル][33]         | チームハンドル                                                                        |
| [リソースカタログ][8]          | チームハンドル                                                                        |
| [RUMアプリ][34]                 | チームハンドル                                                                        |
| [セキュリティルール][35]           | チームハンドル                                                                        |
| [セキュリティ抑制][36]    | チームハンドル                                                                        |
| [サービスレベル目標][17] | チームハンドル                                                                        |
| [Sheets][37]                   | チームハンドル                                                                        |
| [ソフトウェアカタログ][12]         | チームハンドル                                                                        |
| [Synthetic テスト][19]          | チームハンドル                                                                        |
| [ワークフロー][38]                | チームハンドル                                                                        |


## 権限 {#permissions}

チーム管理権限を持つ役割のユーザーは、チームを作成、チーム名の変更、チームの削除、およびチームハンドルの変更ができます。`user_access_manage`を持つユーザーは、チームメンバーやマネージャーを追加、削除、昇進させることができます。

## チームを管理する {#manage-teams}

チームをカスタマイズする方法については、[Team Management][3] を参照してください。


[1]: https://app.datadoghq.com/organization-settings/teams
[2]: https://app.datadoghq.com/teams
[3]: /ja/account_management/teams/manage/
[4]: /ja/account_management/saml/mapping/#map-saml-attributes-to-teams
[5]: /ja/dashboards/#dashboard-details
[6]: /ja/incident_response/incident_management/
[7]: /ja/monitors/configuration/?tab=thresholdalert#add-metadata
[8]: https://app.datadoghq.com/infrastructure/catalog
[9]: /ja/tracing/software_catalog/adding_metadata/#add-metadata-from-the-datadog-ui
[10]: /ja/service_level_objectives/#slo-tags
[11]: https://app.datadoghq.com/dashboard/lists
[12]: https://app.datadoghq.com/services
[13]: https://app.datadoghq.com/incidents
[14]: https://app.datadoghq.com/monitors/manage
[15]: https://app.datadoghq.com/apm/error-tracking
[16]: https://app.datadoghq.com/logs/error-tracking
[17]: https://app.datadoghq.com/slo/manage
[18]: https://app.datadoghq.com/data-streams
[19]: https://app.datadoghq.com/synthetics
[20]: https://app.datadoghq.com/notebook/list/
[21]: https://app.datadoghq.com/app-builder/apps/list
[22]: https://app.datadoghq.com/cases
[23]: https://app.datadoghq.com/actions/connections
[24]: https://app.datadoghq.com/actions/connections?sort=-updated_at&tab=groups
[25]: https://app.datadoghq.com/organization-settings/cross-org-visibility
[26]: https://app.datadoghq.com/actions/datastores
[27]: https://app.datadoghq.com/integrations
[28]: https://app.datadoghq.com/logs/pipelines
[29]: https://app.datadoghq.com/observability-pipelines
[30]: https://app.datadoghq.com/on-call/summary
[31]: https://app.datadoghq.com/actions/private-action-runners
[32]: /ja/dashboards/widgets/powerpack/#powerpack-permissions
[33]: https://app.datadoghq.com/reference-tables
[34]: https://app.datadoghq.com/rum/list
[35]: https://app.datadoghq.com/security/configuration/notification-rules
[36]: https://app.datadoghq.com/security/configuration/suppressions
[37]: https://app.datadoghq.com/sheets
[38]: https://app.datadoghq.com/workflow