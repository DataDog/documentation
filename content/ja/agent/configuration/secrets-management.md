...
- [ファイルJSON](#id-for-json-yaml-text)
- [ファイルYAML](#id-for-json-yaml-text)
...
**注**: `ENC[]`の構文を`secret_*`の設定で`secret_backend_command`のように使用することはできません。

## シークレットを取得するためのオプション {#options-for-retrieving-secrets}

### オプション 1: シークレットを取得するために、ネイティブな Agent サポートを使用する{#option-1-using-native-agent-support-for-fetching-secrets}

**注**: Agent バージョン `7.76` 以降、ネイティブなシークレット管理が FIPS 対応 Agent で利用可能です。

Agent バージョン `7.70` から、Datadog Agent では複数のシークレット管理ソリューションをネイティブにサポートしています。`datadog.yaml`に新しい2つの設定が導入されました: `secret_backend_type` と `secret_backend_config`。
...
**注**: コンテナ化された環境で Datadog を実行している場合、[Cluster Agent](/containers/cluster_agent/) は、ネイティブなシークレットの取得をサポートするために Agent 7.77 以降が必要です。以前のバージョンでは、代わりに [オプション 2](#option-2-using-the-built-in-script-for-kubernetes-and-docker) または [オプション 3](#option-3-creating-a-custom-executable) を使用してください。
...
* `secretId`: シークレットの「フレンドリ名」（例えば、`/DatadogAgent/Production`）または ARN（例えば、`arn:aws:secretsmanager:us-east-1:123456789012:secret:/DatadogAgent/Production-FOga1K`）のいずれかです。
  - **注**: AWS 資格情報または `sts:AssumeRole` 資格情報が定義されている異なるアカウントからシークレットにアクセスする場合、完全な ARN 形式が必要です。
...
次に示すのは、`datadog.yaml` 構成ファイルの完全な例で、AWS Secrets を使用して `My-Secrets` から API キーを取得します。
...
##### インテグレーション チェック {#integration-check}
...
<div class="alert alert-info"> 必ずそれを含めてください <code>serviceAccountAnnotations</code> Agent に AWS シークレットへアクセスする権限を付与するために。 </div>

<br>


##### クラスター チェック: クラスター チェックランナーが有効でない場合 {#cluster-check-without-cluster-check-runners-enabled}
...
##### クラスター チェック: クラスター チェックランナーが有効な場合 {#cluster-check-with-cluster-check-runners-enabled}
...
**または**、Helm チャート v3.171.0+ および Agent v7.70+ を使用している場合は、環境変数の代わりにネイティブの `secretBackend.type` および `secretBackend.config` フィールドを使用できます。たとえば：`datadog.secretBackend.type: "aws.secrets"` と `datadog.secretBackend.config.aws_session.aws_region: "<AWS_REGION>"`。
...
##### インテグレーション チェック {#integration-check-1}
...
##### クラスター チェック: クラスター チェックランナーが有効でない場合 {#cluster-check-without-cluster-check-runners-enabled-1}
...
##### クラスター チェック: クラスター チェックランナーが有効な場合 {#cluster-check-with-cluster-check-runners-enabled-1}
...
**または**、Datadog Operator v1.25.0+ および Agent v7.70+ を使用している場合は、環境変数の代わりにネイティブの `secretBackend.type` および `secretBackend.config` フィールドを使用できます。たとえば：`spec.global.secretBackend.type: "aws.secrets"` と `spec.global.secretBackend.config` を `aws_session.aws_region: "<AWS_REGION>"` とともに。
...
Datadog では、Azure での認証にマネージドアイデンティティを使用することを推奨しています。これにより、クラウドリソースを AMI アカウントに関連付けることができ、`datadog.yaml` 構成ファイルに機密情報を記載する必要がなくなります。
...
これは、事前定義されたロール {{< ui >}}Secret Manager Secret Accessor{{< /ui >}} (`roles/secretmanager.secretAccessor`) または同等の [アクセス][5002] を持つカスタムロールで付与できます。

GCE または GKE ランタイムでは、ADC はインスタンスまたは Pod に添付されたサービスアカウントを通じて自動的に設定されます。添付されたサービスアカウントには、GCP Secret Manager にアクセスするための適切なロールが割り当てられている必要があります。さらに、GCE または GKE ランタイムは、`cloud-platform` [OAuth スコープ][5003] を必要とします。

##### GCP 構成例 {#gcp-configuration-example}
...
Agent を GCP Secret Manager を使用するように構成した後、構成内でシークレットを `ENC[secret-name]` または `ENC[secret-name;key;version;]` で参照します。
...
例えば、`datadog-api-key` という名前の GCP シークレットが2つのバージョンを持ち、`datadog-app-key` という名前のシークレットが存在すると仮定します:
...
JSON 形式のシークレットの場合、`datadog-keys` という名前のシークレットが含まれていると仮定します:
...
- `datadog;api_key` - 暗黙の `latest` バージョンを持つ `datadog` シークレットから `api_key` フィールドを抽出します。
...
```
3. 実行 `vault policy write <policy_name> <path_to_*.hcl_file>`

4. Vault の認証方法を選択します。AWS インスタンスプロファイルメソッドを使用している場合は、`vault auth enable aws`を実行します。
...
- **ServiceAccount 認証情報**: デフォルトでは、自動的にマウントされた ServiceAccount トークン (`automountServiceAccountToken: true`, [Kubernetes ドキュメント](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/#opt-out-of-api-credential-automounting)を参照) を使用します。必要に応じてカスタムパスを設定できます。
...
**注:** シークレットバックエンドを使用して API キーを解決する際、Helm チャートの検証にはプレースホルダー `apiKey` が必要です。これは `DD_API_KEY` 環境変数によりオーバーライドされます。シークレットを含む各ネームスペースに対して手動でRBAC (ロール + RoleBinding) を作成する必要があります。詳細については、[RBAC のセットアップ](#rbac-setup)セクションを参照してください。

**代わりに**、Helm チャート v3.171.0+ および Agent v7.70+ を使用する場合、環境変数の代わりにネイティブ `datadog.secretBackend.type` フィールドを使用できます。
...
**注:** プレースホルダー API キーでは、シークレットバックエンドを使用して API キーを解決する際に Operator の検証が満たされます。これは `DD_API_KEY` 環境変数によりオーバーライドされます。シークレットを含む各ネームスペースに対して手動でRBAC (ロール + RoleBinding) を作成する必要があります。詳細については、[RBAC のセットアップ](#rbac-setup)セクションを参照してください。

**代わりに**、Datadog Operator v1.25.0+ および Agent v7.70+ を使用する場合、環境変数の代わりにネイティブ `spec.global.secretBackend.type` フィールドを使用できます。
...
**代わりに**、Helm チャート v3.171.0+ を使用する場合、次のように使用できます: `datadog.secretBackend.type: "k8s.secrets"` と `datadog.secretBackend.config` を `token_path` および `ca_path` キーと共に。
...
**代わりに**、Datadog Operator v1.25.0+ を使用する場合、次のように使用できます: `spec.global.secretBackend.type: "k8s.secrets"` と `spec.global.secretBackend.config` を `token_path` および `ca_path` キーと共に。
...
デフォルトの `KUBERNETES_SERVICE_HOST` および `KUBERNETES_SERVICE_PORT` 環境変数が公開されていない場合は、Kubernetes REST API と対話するための `api_server` URL を提供できます。
...
**代わりに**、Helm チャート v3.171.0+ を使用する場合、次のように使用できます: `datadog.secretBackend.type: "k8s.secrets"` と `datadog.secretBackend.config` を `api_server` キーと共に。
...
**代わりに**、Datadog Operator v1.25.0+ を使用する場合、次のように使用できます: `spec.global.secretBackend.type: "k8s.secrets"` と `spec.global.secretBackend.config` を `api_server` キーと共に。
...
Docker Secrets バックエンドでは、[Docker Swarm のシークレット][6002] と [Docker Compose のシークレット][6003] の両方がサポートされています。デフォルトでは、Swarm と Compose は、コンテナ内にシークレットを `/run/secrets` (Linux) または `C:\ProgramData\Docker\secrets` (Windows) のファイルとして自動的にマウントします。
...
[作成][6003] ファイルベースのシークレットを持つ `docker-compose.yml`:
...
ファイルバックエンドには、設定された JSON、YAML、または TEXT ファイルに対する読み取り**権限**のみが必要です。これらのアクセス許可は、ローカルの Datadog Agent ユーザー (`dd-agent` は Linux、`ddagentuser` は Windows) に付与する必要があります。
...
例えば、次の内容を含む `/path/to/secret.json` の JSON ファイルを使用する場合:
...
**注**: YAML の深度は 1 つのレベルのみサポートされています (例えば、`key: value`)

##### 構成例 {#configuration-example-7}

YAML ファイルを使用してシークレットをローカルに保存できます。

例として、次の内容を含む `/path/to/secret.yaml` の YAML ファイルがある場合:
...
例えば、次の内容を含む `/path/to/secrets/` のテキストファイルを使用する場合:
...
- 絶対パスは `ENC[]` 内で`secrets_path`なければなりません (例: `ENC[/path/to/secrets/dd_api_key]` と `secret_path: /path/to/secrets` は動作します)
- パストラバーサルの試み (例: `ENC[../etc/passwd]`) はブロックされ、「許可されたディレクトリ外のパス」として失敗します。

**注:** 一部のツールでは、シークレットをファイルにエクスポートする際に自動的に改行が追加されます。[末尾の改行を削除する方法](#remove-trailing-line-breaks)を参照してください。
...
この実行可能ファイルを使用するには、環境変数 `DD_SECRET_BACKEND_COMMAND` を次のように設定してください:
...
**注**:
- シークレットは、マウント先のポッドと同じネームスペースに存在する必要があります。
- スクリプトは、機密の `/var/run/secrets/kubernetes.io/serviceaccount/token` を含むすべてのサブフォルダーにアクセスできます。そのため、Datadog では `/var/run/secrets` の代わりに専用のフォルダーを使用することを推奨しています。

[Docker Swarm シークレット][3] は `/run/secrets` フォルダーにマウントされます。例えば、Docker シークレット `db_prod_passsword` は エージェント コンテナの `/run/secrets/db_prod_password` にあります。これは、設定で `ENC[file@/run/secrets/db_prod_password]` を参照します。

#### 例: ネームスペースをまたいで Kubernetes シークレットを読み取る {#example-reading-a-kubernetes-secret-across-namespaces}

エージェントが異なるネームスペースからシークレットを読み取る必要がある場合は、`k8s_secret@` プレフィックスを使用します。たとえば、以下のとおりです。

```
password: ENC[k8s_secret@database/database-secret/password]
```

Agent のサービスアカウントがシークレットを読み取れるように RBAC を設定します。次のロールは、`database-secret` シークレットが存在する `database` ネームスペースへの読み取りアクセスを付与します:
{{< tabs >}}
{{% tab "Datadog Operator" %}}
  ...
  global:
    secretBackend:
      ...
      - namespace: database
        ...
        - "database-secret"
```
***注***: ロールリスト内の各ネームスペースは、Datadog Operator のデプロイメントで `WATCH_NAMESPACE` または `DD_AGENT_WATCH_NAMESPACE` 環境変数に設定されている必要があります。
{{% /tab %}}
{{% tab "Helm" %}}
...
この `Role` は `Secret: database-secret` 内の `Namespace: database` へのアクセスを提供します。`RoleBinding` は `ServiceAccount: datadog-agent` 内の `Namespace: default` にこの権限をリンクします。これは、デプロイされたリソースに関して手動でクラスターに追加する必要があります。
...
* `secrets`(文字列のリスト): 各文字列はシークレットを取得するためのハンドルです。
...
* `value` (文字列): 設定で使用されるシークレットの値です。エラーの場合、これは `null` になる可能性があります。
* `error`(文字列): エラーメッセージまたは `null` です。

シークレットの解決に失敗した場合（非ゼロの終了コードまたは非 null のエラーが返された場合）、関連する設定はエージェントによって無視されます。

**機密情報は `stderr`** に出力しないでください。バイナリが `0` 以外のステータスコードで終了した場合、エージェントはトラブルシューティングのために実行ファイルの標準エラー出力をログに記録します。
...
## エージェントのセキュリティ要件 {#agent-security-requirements}

エージェントは提供された実行ファイルをサブプロセスとして実行します。実行パターンは Linux と Windows で異なります。
...
* エージェントを実行しているのと同じユーザーに属していること (`dd-agent` がデフォルト、または `root` コンテナ内)。
*  `group` または `other` に対する権利を持っていないこと。
...
* は、**読み取り**または**実行**の権限があること (`ddagentuser` はエージェントの実行に使用するユーザー)。
* は、`Administrators` グループ、ビルトイン **ローカルシステム** アカウント、またはエージェントユーザーコンテキスト (`ddagentuser` がデフォルト) 以外のユーザーまたはグループの権限を持っていないこと。
* エージェントが実行できるように、有効な Win32 アプリケーションである必要があります（例えば、PowerShell や Python のスクリプトは動作しません）。

{{% /tab %}}
{{< /tabs >}}

**注**: 実行ファイルは、エージェントと同じ環境変数を共有します。
...
これを有効にするには、`secret_refresh_interval`（秒単位）を `datadog.yaml` に設定します。
...
デフォルトでは、エージェントは初期リフレッシュを `secret_refresh_interval` ウィンドウ内でランダム化して、
...
[Fleet Management](https://app.datadoghq.com/fleet) ページでの使用を防ぎます。
...
エージェント v7.76 以降、スケジュールされた [Autodiscovery][1] チェックでは、テンプレートが `ENC[]` 構文を使用している場合、実行時にシークレットをリフレッシュできます。
...
その後、エージェントは `secret_refresh_interval` に設定された間隔または `datadog-agent secret refresh` で手動でシークレットのリフレッシュをトリガーできます。
...
この機能を有効にするには、`secret_refresh_on_api_key_failure_interval` を `datadog.yaml` ファイルの分単位の間隔に設定します。無効にするには `0` に設定します (デフォルト)。
...
これにより、シークレットがリフレッシュされた後、DDOT コレクターとエージェントの同期が維持されます。Agent で設定状態が定期的に確認されるのと同様の方法で、DDOT コレクターではこの設定を使用して Agent で更新された値を定期的にチェックします。

##  トラブルシューティング {#troubleshooting}

###  検出されたシークレットのリスト {#listing-detected-secrets}

エージェント CLI の `secret` コマンドでは、セットアップに関連するエラーが表示されます。たとえば、実行ファイルの権限が不正確な場合です。また、検出されたすべてのハンドルとその場所がリストされます。
...
###  シークレットが挿入された後の構成の表示 {#seeing-configurations-after-secrets-were-injected}
...
**注**: 構成ファイルの変更を取得するには、エージェントを [再起動][7] する必要があります。
...
2.  `ddagentuser` がファイルに対する読み取りおよび実行権限を持っていない場合、同様のエラーがログに記録されます。
   ```
...
これは、実行可能ファイルがデフォルトのユーザーまたは開発ユーザーで動作する可能性があることを意味しますが、`ddagentuser` にはより制限された権限があるため、エージェントによって実行されるときは動作しません。

エージェントと同じ条件で実行ファイルをテストするには、開発用マシンの `ddagentuser` のパスワードを更新してください。この方法で、`ddagentuser` として認証し、エージェントと同じコンテキストで実行ファイルを実行できます。
...
### エージェントが起動を拒否しています {#agent-refusing-to-start}

エージェントの起動時の最初の処理は、`datadog.yaml` をロードし、その中のシークレットを復号化することです。これは、ログのセットアップの前に行われます。そのため、Windows のようなプラットフォームでは、`datadog.yaml` のロード時に発生するエラーがログに書き込まれず、`stderr` に書き込まれます。これは、シークレットのために Agent に渡された実行ファイルがエラーを返すときに発生する可能性があります。

`datadog.yaml` にシークレットがあり、エージェントが起動を拒否した場合:

*  `stderr` を表示できるように、手動でエージェントを起動してみてください。
...
前の [ Kubernetes Secrets の例](#example-reading-a-kubernetes-secret-across-namespaces) を考慮してください。ここでは、シークレット `Secret:database-secret` が `Namespace: database` に存在し、サービスアカウント `ServiceAccount:datadog-agent` が `Namespace: default` に存在します。
...
###  行末の改行を削除します {#remove-trailing-line-breaks}

一部のシークレット管理ツールでは、ファイルを通じてシークレットをエクスポートする際に自動的に改行を追加します。これらの改行は、[datadog.yaml 構成ファイル][8] で `secret_backend_remove_trailing_line_break: true` を設定するか、環境変数 `DD_SECRET_BACKEND_REMOVE_TRAILING_LINE_BREAK` を使用して同様のことを行うことで削除できます。特にコンテナ化された環境では。

### シークレットハンドル内のAutodiscovery変数 {#autodiscovery-variables-in-secret-handles}
