...
**注**: `ENC[]`構成のような設定では、`secret_*`構文を使用することはできません。
...
Agent バージョン `7.70` から、Datadog Agent では複数のシークレット管理ソリューションをネイティブにサポートしています。`datadog.yaml` に新しい 2 つの設定が導入されました: `secret_backend_type` と `secret_backend_config`。
...
**または**、Helm チャート v3.171.0+ および Agent v7.70+ では、環境変数の代わりにネイティブな `secretBackend.type` および `secretBackend.config` フィールドを使用できます。例: `datadog.secretBackend.type: "aws.secrets"` と `datadog.secretBackend.config.aws_session.aws_region: "<AWS_REGION>"`。
...
**代わりに**、Datadog Operator v1.25.0+ および Agent v7.70+ を使用する場合、環境変数の代わりにネイティブ `secretBackend.type` および `secretBackend.config` フィールドを使用できます。例: `spec.global.secretBackend.type: "aws.secrets"` と `spec.global.secretBackend.config` と `aws_session.aws_region: "<AWS_REGION>"`。
...
Agent を GCP Secret Manager を使用するように構成した後、構成内で `ENC[secret-name]` または `ENC[secret-name;key;version;]` を使用してシークレットを参照します。
...
例として、`datadog-api-key` という名前の GCP シークレットが 2 つのバージョンを持つと仮定します。`datadog-app-key`:
...
JSON 形式のシークレットについて、`datadog-keys` という名前のシークレットが次の内容を含むと仮定します:
...
- `datadog;api_key` - `datadog` シークレットから `api_key` フィールドを暗黙の `latest` バージョンで抽出します
...
2.2. Vault からシークレットを取得するための権限を付与するポリシーを記述します。`*.hcl` ファイルを作成し、Secrets Engine バージョン 1 を使用している場合は、次の権限を含めます:
...
- **ServiceAccount 認証情報**: デフォルトでは、自動的にマウントされた ServiceAccount トークン (`automountServiceAccountToken: true`、[Kubernetes ドキュメントを参照) を使用します。必要に応じてカスタムパスを設定できます。
...
******注:****** プレースホルダー API キーでは、シークレットバックエンドを使用して API キーを解決する際に Operator の検証が満たされます。`DD_API_KEY` 環境変数はそれをオーバーライドします。シークレットを含む各ネームスペースに対して手動でRBAC (ロール + RoleBinding) を作成する必要があります。詳細については、[RBAC のセットアップ](#rbac-setup) セクションを参照してください。
...
############### カスタムパスの設定
セットアップが ServiceAccount ベースの認証のデフォルトの場所に従わない場合は、代わりに `token_path` と `ca_path` を指定できます。
...
**代わりに**、Helm チャート v3.171.0+ を使用する場合、`datadog.secretBackend.type: "k8s.secrets"` と `datadog.secretBackend.config` を `token_path` および `ca_path` キーと共に使用できます。
...
**代わりに**、Datadog Operator v1.25.0+ を使用する場合、`spec.global.secretBackend.type: "k8s.secrets"` と `spec.global.secretBackend.config` を `token_path` および `ca_path` キーと共に使用できます。
...
セットアップがデフォルトの `KUBERNETES_SERVICE_HOST` および `KUBERNETES_SERVICE_PORT` 環境変数を公開しない場合は、Kubernetes REST API と対話するために `api_server` URL を提供できます。
...
**代わりに**、Helm チャート v3.171.0+ を使用する場合、`datadog.secretBackend.type: "k8s.secrets"` と `datadog.secretBackend.config` を `api_server` キーと共に使用できます。
...
**代わりに**、Datadog Operator v1.25.0+ を使用する場合、`spec.global.secretBackend.type: "k8s.secrets"` と `spec.global.secretBackend.config` を `api_server` キーと共に使用できます。
...
Docker Secrets バックエンドでは、[Docker Swarm のシークレット][6002] と [Docker Compose のシークレット][6003] の両方がサポートされています。デフォルトでは、Swarm と Compose は、コンテナ内で `/run/secrets` (Linux) または `C:\ProgramData\Docker\secrets` (Windows) のファイルとしてシークレットを自動的にマウントします。
...
シークレット `dd_api_key` は `/run/secrets/dd_api_key` に自動的にマウントされ、エージェントは `docker.secrets` バックエンドを使用してそれを読み取ります。

########## Docker Compose の例

[ファイルベースのシークレットを使用して] `docker-compose.yml` を作成します:
...
##### ファイルのアクセス許可
ファイルバックエンドには、設定された JSON、YAML、または TEXT ファイルに対する**読み取り**アクセス許可のみが必要です。これらのアクセス許可を、ローカルの Datadog Agent ユーザー (`dd-agent` は Linux、`ddagentuser` は Windows) に付与する必要があります。
...
たとえば、次の内容を含む `/path/to/secret.json` の JSON ファイルを使用します:
...
例として、`/path/to/secret.yaml` に YAML ファイルがある場合:
...
たとえば、`/path/to/secrets/` にテキストファイルがある場合:

`/path/to/secrets/dd_api_key` に含まれる:

```
your_api_key_value
```

`/path/to/secrets/dd_app_key` に含まれる:
...
- `ENC[]` の絶対パスは `secrets_path` 内でなければなりません (例: `ENC[/path/to/secrets/dd_api_key]` と `secret_path: /path/to/secrets` は動作します)
- パストラバーサルの試み (例: `ENC[../etc/passwd]`) はブロックされ、「許可されたディレクトリ外のパス」として失敗します

**注:** 一部のツールでは、シークレットをファイルにエクスポートする際に自動的に改行が追加されます。[末尾の改行を削除する](#remove-trailing-line-breaks) については、こちらを参照してください。
...
* Kubernetes シークレット: `ENC[k8s_secret@namespace/secret-name/key]` を使用
...
-このスクリプトは、機密性の高い `/var/run/secrets/kubernetes.io/serviceaccount/token` を含むすべてのサブフォルダーにアクセスできます。そのため、Datadog では `/var/run/secrets` の代わりに専用のフォルダーを使用することが推奨されています。
...
Agent のサービスアカウントがシークレットを読み取れるように RBAC を設定します。以下のロールは、`database-secret` ネームスペース内の `database` シークレットへの読み取りアクセスを付与します。
{{< tabs >}}
{{% tab "Datadog OperatorDatadog オペレーター" %}}
...
この `Role` は、`Secret: database-secret` に対するアクセスを `Namespace: database` に付与します。`RoleBinding` は、この権限を `ServiceAccount: datadog-agent` にリンクします。これは、デプロイされたリソースに関して手動でクラスターに追加する必要があります。
...
シークレットの解決に失敗した場合（非ゼロの終了コードまたは非 null のエラーが返された場合）、関連する設定は Agent によって無視されます。
...
* Agent を実行しているのと同じユーザーに属しています（デフォルトでは `dd-agent`、またはコンテナ内の `root`）。
*`group` または `other` に対する権利を持っていません。
...
* **読み取り**または**実行**権限が `ddagentuser` にあります（Agent の実行に使用するユーザー）。
*`Administrators` グループ、ビルトインローカルシステムアカウント、または Agent ユーザーコンテキスト（デフォルトでは `ddagentuser`）以外のユーザーまたはグループの権限を持っていません。
...
[フリート管理](https://app.datadoghq.com/fleet) ページでの使用を防ぎます。
...
その後、Agent は `secret_refresh_interval` に設定された間隔または `datadog-agent secret refresh` を使用して手動でシークレットのリフレッシュをトリガーできます。
...
この機能を有効にするには、`datadog.yaml` ファイル内の `secret_refresh_on_api_key_failure_interval` を分単位の間隔に設定してください。無効にするには `0` に設定します（デフォルト）。
...
チェックの設定がどのように解決されるかを素早く確認するには、`configcheck` コマンドを使用できます。
...
3.3.3. 実行ファイルは有効な Win32 アプリケーションである必要があります。そうでない場合、次のエラーがログに記録されます。
   ```
   error while running 'C:\decrypt.py': fork/exec C:\decrypt.py: %1 is not a valid Win32 application.
   ```

Datadog には、実行ファイルに正しい権限を設定するのに役立つ [PowerShell スクリプト][9] があります。使用方法の例
...
2.`ddagentuser` に新しいパスワードを設定します（インストール時に生成されたパスワードはどこにも保存されないため）。PowerShell で、次を実行します。
    ...
    ```
3. サービスコントロールマネージャーで使用されるパスワードを `DatadogAgent` に更新します。PowerShell で、次を実行します。
    ```powershell
...
`datadog.yaml` にシークレットがあり、Agent が開始を拒否した場合:

* `stderr` を表示できるように、Agent を手動で起動してみてください。
...
一部のシークレット管理ツールでは、ファイルを通じてシークレットをエクスポートする際に自動的に改行を追加します。これらの改行は、[datadog.yaml 構成ファイル][8] に `secret_backend_remove_trailing_line_break: true` を設定するか、環境変数 `DD_SECRET_BACKEND_REMOVE_TRAILING_LINE_BREAK` を使用して同様のことを行うことで削除できます。特にコンテナ化された環境では。
