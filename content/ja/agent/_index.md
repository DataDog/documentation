...
## エージェントの管理 {#managing-the-agent}

### Fleet Automation を使用したエージェントの管理 (推奨) {#managing-the-agent-with-fleet-automation-recommended}
...
- すべての実行中のチェックを表示する
- エージェントログを表示する
- エージェント構成ファイル (`datadog.yaml`) を編集する
- エージェントチェックを追加または編集する
- フレアを送信する
...
- セキュリティ上の理由から、GUI はローカルネットワークインターフェース (`localhost`/`127.0.0.1`) から**のみ**アクセス可能であるため、エージェントが実行されているホストにいる必要があります。Agent を VM またはコンテナで実行し、ホストマシンからアクセスすることはできません。
