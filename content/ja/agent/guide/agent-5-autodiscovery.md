...
# どの key-value ストアを使用して構成テンプレートを探すかを定義します。
# デフォルトは etcd です。Consul もサポートされています。
sd_config_backend: etcd

# バックエンドに接続するための設定です。以下はデフォルト値です。異なる構成を使用する場合は、編集してください。
sd_backend_host: 127.0.0.1
sd_backend_port: 4001

# デフォルトでは、Agent は、バックエンドの `/datadog/check_configs` キーの
# 下で構成テンプレートを探します。
