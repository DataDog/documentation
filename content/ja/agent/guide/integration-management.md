...
これらのコマンドの使用方法と説明は、`datadog-agent integration --help` を使用して出力できます。
Linux では `dd-agent` ユーザーとして、Windows では `administrator` として、コマンドを実行してください。
...
以下の条件を満たす場合、インテグレーションは互換性があり、インストール可能です。

1. [Agent に付属しているバージョン][2]より新しいバージョンである。
2. インストールされている Agent 内の [datadog_checks_base][3] のバージョンと互換性がある。
3. `datadog_checks_base` ではない。ベースチェックをアップグレードするには、Agent をアップグレードする必要があります。

このコマンドの構文は `datadog-agent integration install <integration_package_name>==<version>` です。`<integration_package_name>` は、先頭に `datadog-` を付けたインテグレーション名です。
...
インテグレーションを削除するには、`datadog-agent integration remove` コマンドを使用します。このコマンドの構文は `datadog-agent integration remove <integration_package_name>` です。`<integration_package_name>` は、先頭に `datadog-` を付けたインテグレーション名です。
...
インストールされているインテグレーションのバージョンなどの情報を取得するには、`datadog-agent integration show` コマンドを使用します。このコマンドの構文は `datadog-agent integration show <integration_package_name>` です。`<integration_package_name>` は、先頭に `datadog-` を付けたインテグレーション名です。
