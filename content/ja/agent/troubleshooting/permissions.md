...
Datadog Agent をホストで実行している際、Agent の適切なロギングを妨げかねない、アクセス許可に関連した下記のような問題が発生することがあります。
...
Linux ホストの場合、正常に起動するためには、`/opt/datadog-agent/run` ディレクトリのオーナーは `dd-agent` である必要があります。まれに、このディレクトリのオーナーシップが `dd-agent` 以外に変更されてしまうことがあります。これにより、Agent の起動時に上記のエラーが発生します。次のコマンドを実行して、このディレクトリのオーナーシップを再度確認します。
...
Linux OS で実行している Agent の[プロセスチェック][7]を有効化している場合、デフォルトでは `system.processes.open_file_descriptors` メトリクスが収集または報告されないことに気付いているかもしれません。
...
**注意**: Agent を `root` として実行することは推奨されません。Datadog Agent に限ったことではなく、また、何らかの形で信用性の低い現象が起こるという懸念からでもなく、デーモンを `root` として実行することは推奨されておらず、Linux のほとんどのプロセスでのベストプラクティスであるためです。個人的に懸念事項がある場合は、Agent はオープンソースであるため、 [GitHub レポジトリ][1]からご自身やチームで監査できます。
...
Agent v5 を実行している場合は、 [最新バージョンの Agent 6][1] へアップデートし、`try_sudo` オプションを使用するようにしてください。アップデートできない場合は、この問題の回避策として、Agent を `root` として実行します。

**注意**: Agent を `root` として実行することは推奨されません。Datadog Agent に限らず、または何らかの形で信用性の低い現象が起こるという懸念からでもなく、デーモンを `root` として実行することは推奨されておらず、Linux のほとんどのプロセスのベストプラクティスであるためです。個人的に懸念される場合は、Agent はオープンソースであるため、 [GitHub レポジトリ][2]からご自身やチームで調査を実施することができます。
