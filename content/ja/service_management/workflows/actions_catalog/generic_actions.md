...
{{< img src="service_management/workflows/http-request-options-tab.png" alt="ワークフローキャンバスは、HTTP リクエスト アクションが選択され、構成タブが開かれています。Request Options タブが選択され、アクションは Connection という名前のヘッダーと keep-alive という値で構成され、Error on Status フィールドは 202, 300-305 という値で埋められています" >}}
...
ワークフローの実行経路を、定義した 1 つまたは複数のステートメントの評価に基づいて分岐させることができます。例えば、以下のスクリーンショットでは、HTTP リクエストのステータスコードが `200` で、かつ関数が `true` を返した場合に、ワークフローのアクションを分岐させることを検討しています。ステートメントは、`Join operand` ドロップダウンで `and` と `or` ロジックで評価することができます。

{{< img src="service_management/workflows/branch-workflow-configuration.png" alt="条件分岐ワークフローが選択され、構成タブが開かれたワークフロー キャンバス。Statements セクションは、前のリクエストのステータスが 200 であること、前の関数の戻り値が true であることを指定する 2 つのステートメントで強調表示されています。" >}}
