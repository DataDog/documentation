...
グローバルタイムセレクターは、同一のダッシュボード上で `Global Time` オプションを使用するすべてのウィジェットに対して同一のタイムフレームを設定します。過去の移動ウィンドウ (`Past 1 Hour`、`Past 1 Day` など) を選択するか、固定期間を選択します。固定期間は、`Select from calendar...` オプションを使用します。または、[カスタムタイムフレームを入力][11]します。移動ウィンドウを選択した場合、ウィジェットはタイムウィンドウに沿って移動します。
...
ダッシュボードから、ページ右上のバージョン履歴アイコンをクリックすると、バージョン履歴のサイドパネルが表示されます。ダッシュボードのバージョン履歴をプレビュー、復元、複製することができます。詳細については、[バージョン履歴ガイド][12]を参照してください。
...
アクセス制御ルールを設定するユーザーは、組織のロックアウトを防ぐために、自分が所属する 1 つまたは複数のロールを含める必要があります。ロールの詳細については、[RBAC ドキュメント][15]を参照してください。
...
**注:** 個々のダッシュボードの表示制限は、Enterprise ティアプランを利用している人であれば誰でも利用できます。この機能を有効にするには、アカウントチームまたは [Datadog サポート][17]にご連絡ください。
...
個々のダッシュボードで、ダッシュボードタイトルの横にあるキャレットアイコンをクリックすると、ダッシュボードの詳細を表示および編集することができます。ドロップダウンパネルが開きます。

{{< img src="dashboards/suggested_dashboards.png" alt="推奨ダッシュボード" >}}

タイトルの下には、ダッシュボードの作成者を示す傍点が表示されます。

### 推奨されるダッシュボードとアクティブユーザー

Datadog は、個々のダッシュボードから、関連するダッシュボードの閲覧を提案します。これらのダッシュボードは、組織内のユーザーアクティビティと、ユーザーがこのダッシュボードから他の既存のダッシュボードに移動する頻度に基づいて推奨されます。ダッシュボードの詳細ビューの右端のセクションには、このダッシュボードの最もアクティブなユーザーのリストが表示されます。

### 詳細を編集する

Markdown に対応するダッシュボードの説明を更新したり、[チーム][18]をダッシュボードと関連付けたりします。

1. 編集したいダッシュボードを開きます。
1. ダッシュボードタイトルの横にあるキャレットアイコンをクリックします。ドロップダウンパネルが開きます。
1. **Edit** をクリックします。パネルが編集モードに切り替わります。
1. **Teams** ドロップダウンから最大 5 チームを選択します。
1. テキストボックスに説明を入力します。Markdown でテキストをフォーマットします。
1. **保存**をクリックします。
...
**My Teams** トグルを使用して、すべてのダッシュボードを表示するか、自分の[チーム][18]が所有するダッシュボードのみを表示するかを切り替えることができます。
...
ダッシュボードページでは、すべてのダッシュボードを表示、検索し、Datadog Web アプリで設定したものと同じテンプレート変数を使用してそれらをフィルタリングできます。個々のダッシュボードをクリックして表示します。テンプレート変数の保存済みビューを使用して、ダッシュボードをすばやくフィルタリングします。テンプレート変数の保存済みビューの詳細については、[ダッシュボードの保存済みビュー][19]を参照してください。個々のダッシュボードをクリックして表示します。
...
[17]: /ja/help/
[18]: /ja/account_management/teams/
[19]: /ja/dashboards/template_variables/#saved-views