...
この関数は `<METHOD>` と `<TIME>` の 2 つのパラメーターを受け取り、`.rollup(<METHOD>,<TIME>)` のように記述します。
...
`.moving_rollup()` 関数をクエリに適用することで、「最後の X 秒」など最も直近の特定の時間範囲にあるポイントを結合することができます。この関数には `<TIME>` および `<METHOD>`: `.moving_rollup(<TIME>,<METHOD>)` という 2 種類のパラメーターがあります。
