...
**注**: `DISTRIBUTION` メトリクスタイプにはロールアップの `aggregator` はありません。このメトリクスタイプは時間と空間の両方で集計されます (詳細は[パーセンタイル付きのディストリビューション向けロールアップ][5]についてのドキュメントを参照してください)。

この関数には `<AGGREGATOR>` およびオプションの `<INTERVAL>` という 2 つのパラメーターがあり、`.rollup(<AGGREGATOR>,<INTERVAL>)` または `.rollup(<AGGREGATOR>)` の形で使用されます。

| パラメーター  | 説明                                                                                                     |
|------------|-----------------------------------------------------------------------------------------------------------------|
| `<AGGREGATOR>` | `sum`、`min`、`max`、`count`、または `avg` で、指定された時間間隔の範囲におけるデータポイントの集計方法を定義します。 |
...
[5]: /ja/metrics/faq/rollup-for-distributions-with-percentiles/