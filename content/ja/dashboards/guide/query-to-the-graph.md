...
Datadog はデータを 1 秒の粒度で保存するため、グラフに実際のデータをすべて表示することはできません。グラフのデータがどのように集計されるかについては、[こちらの記事をご参照ください][5]。
...
注: もちろん、ソースが 1 つしかない場合 (たとえば、クエリのスコープとして `{host:moby, device:/dev/disk}` を選択した場合)、空間集計を行う必要がないため、`sum`/`avg`/`max`/`min` を使用しても影響はありません。[詳細はこちらをご参照ください][8]。
...
詳細は、[こちらのブログ記事][9]
および [StatsD/DogStatsD に関するドキュメント][10]をご参照ください。
