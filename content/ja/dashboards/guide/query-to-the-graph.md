...
この例では、`host:bubs` に 5 つのデバイスがあります。したがって、以下の 5 つの時系列 (各ソースから送信されたすべてのデータポイントの履歴) が保存されます。
...
**関連パラメーター: スコープ**
複数のタグを使用できます。たとえば、2 つのタグに対応するデータを取得する場合は、`{host:bubs, device:udev}` のようにします。
...
**注**: もちろん、ソースが 1 つしかない場合 (たとえば、クエリのスコープとして `{host:bubs, device:/dev/disk}` を選択した場合)、空間集計を行う必要がないため、`sum`/`avg`/`max`/`min` を使用しても影響はありません。[sum/min/max/avg 集計間の切り替えについて][8]のよくある質問もご覧ください。
