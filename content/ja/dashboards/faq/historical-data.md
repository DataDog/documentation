...
ダッシュボードの場合、メトリクスまたはタグはエイジアウト期間後に視覚化されますが、UI エディタを使用してグラフを作成するためのドロップダウンでは使用できません。対応するメトリクスまたはタグは、[JSON][1] メソッドを使用してグラフを表示するために利用可能なままです。
