...
Usage Attribution ページでは、コストや使用状況属性を分解するためのタグを最大 3 つまで設定できます。**Enforce tags for usage attribution on all tests** (すべてのテストに対して使用状況帰属タグを必須にする) を選択すると、Synthetic テストを新規作成または編集する際、構成済みの Usage Attribution タグをすべて入力しなければなりません。この設定が有効な場合、必要なタグがすべて入力されるまでテストを保存できません。
