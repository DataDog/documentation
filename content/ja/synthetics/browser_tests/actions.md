...
操作した最後のページの URL を取得し、特定の値 (`string`、`number`、`regex` など) がその中に存在するかどうかをアサートします。
...
メールが送信されたこと、および特定の値 (`string`、`number`、`regex` など) がメールの件名または本文内にあるかどうかをアサートします。このアサーションは、[メール変数][4]を利用します。このため、`Test that an email was received` アサーションを使用できるようにするために、最初にメール変数を作成する必要があります。
...
デフォルトでは、Datadog はページが完全にロードされるのを待ってから、ステップまたは次のステップを実行します。タイムアウトは 60 秒です。ページまたはページ要素のロードに 60 秒以上かかることがわかっている場合は、ステップの[詳細オプション][2]でそのタイムアウトをカスタマイズするか、ハードコードされた待機ステップ (最大値 300 秒) を追加できます。
...
変数を作成するには、変数に名前を付けて、以下のいずれかの方法を使用して値を定義します。ステップ内で変数を使用する方法については、[変数の使用](#use-variables)を参照してください。
...
`span` や `div` などのコンテンツから、この要素のテキストを抽出して変数を作成します。
...
ブラウザテストは、ページにスクリプトを追加することで外部の JavaScript を読み込むため、Web サイトが外部の JavaScript を受け入れる場合にのみ機能します。
...
このタイプのグローバル変数は、時間ベースのワンタイムパスワード (TOTP) シークレットキーを格納し、MFA モジュールと MFA で保護されたワークフローをテストすることができます。ブラウザテストにおける TOTP ベースの MFA の詳細については、こちらの[ブラウザテストにおける多要素認証 (MFA) のための TOTP][8] を参照してください。
...
2. オプションで、**Advanced Options** を指定します。
     ...
     * Body: リクエストの本文と本文のタイプ（`text/plain`、`application/json`、`text/xml`、`text/html`、`application/x-www-form-urlencoded`、`GraphQL` または `None`）。**注**: リクエスト本文の最大サイズは 50 キロバイトに制限されています。
...
2. レスポンスヘッダーから変数を抽出するか、レスポンス本文から抽出するか、レスポンス本文をすべて使用するかを決定します。
...
作成されたこの変数は、ブラウザテストの[次の手順](#use-variables)で使用できます。

## 変数を使用する

手動で追加したステップで利用可能なすべての変数を表示するには、入力フィールドに `{{` と入力します。

自動記録されたステップで変数を使用したい場合は、**Inject this variable** アイコンをクリックします。
