...
* `vars`: ブラウザテスト[変数][5]を含む文字列。JavaScript スニペットでブラウザテスト変数を参照するには、`vars.<YOUR_VARIABLE>` を使用します。たとえば、ブラウザテストに `USERNAME` 変数が含まれている場合は、`vars.USERNAME` を使用して JavaScript スニペットでそれを呼び出します。
...
**注**: `{{` 表示のあるすべてのフィールドは、変数のオートコンプリートをサポートしています。
