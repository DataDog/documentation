...
ローカル変数を作成するには、右上の **Create Local Variable** をクリックします。以下の利用可能なビルトインのいずれかから値を定義することができます。
...
`{{ date(n unit, format) }}` 
: テストが +/- `n` 選択単位で開始された日付に対応する値を使用して、許容される形式のいずれかで日付を生成します。

`{{ timestamp(n, unit) }}` 
: テストが +/- `n` 選択単位で開始されたタイムスタンプに対応する値を使用して、許容される単位のいずれかでタイムスタンプを生成します。

名前と値を入力したら、**Add Variable** をクリックします。
