...
description: CI/CD パイプラインで、オンデマンドまたは事前定義された間隔で Synthetic テストを実行します。
further_reading:
...
<div class="alert alert-info">このページでは、継続的インテグレーション (CI) パイプラインの Synthetic テストの実行について説明します。CI のメトリクスやデータを Datadog のダッシュボードに取り込みたい場合は、<a href="/continuous_integration/" target="_blank">継続的インテグレーションの可視性</a>のセクションを参照してください。</div>
...
[`@datadog/datadog-ci` パッケージ][1]を使うと、CI/CD パイプラインの中で Synthetics テストを直接実行することができるようになります。

[`@datadog/datadog-ci` NPM パッケージ][2]を使用するには、[コンフィギュレーション][3]を参照してください。
...
[3]: /ja/synthetics/cicd_integrations/configuration
[4]: /ja/api/latest/synthetics/#trigger-tests-from-cicd-pipelines
[5]: /ja/api/latest/synthetics/#get-details-of-batch
[6]: /ja/synthetics/cicd_integrations/configuration#configure-tests