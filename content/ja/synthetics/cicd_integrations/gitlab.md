---
description: CI/CD パイプラインで Synthetic テストを実行するために、GitLab インスタンスを構成します。
further_reading:
...
[GitLab][1] パイプラインで Synthetic テストを実行し、デプロイをブロックし、ロールバックをトリガーして、重要なビジネスワークフローが期待通りに機能するときに、コードが本番環境に追加されていることを確認します。

Synthetic テストを [GitLab パイプライン][2]にインテグレーションするには、[datadog-ci npm パッケージ][3]を使用します。
...
もし、CI/CD 環境に固有のテストユーザーやデータがある場合は、`-v` コマンドでこれらの変数をオーバーライドすることができます。詳しくは、NPM パッケージ `datadog-ci` の [Synthetics コマンド][5]を参照してください。
