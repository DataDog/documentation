...
ダウンタイムの期限が切れたときにモニターのステータスがアラート対象（`ALERT`、`WARNING`、`NO DATA`）の場合、モニターは新しい通知をトリガーします。これは、ダウンタイム中にモニターのステータスが（たとえば `OK` から `ALERT`、`WARNING`、または `NO DATA` に）変わったモニターや、ダウンタイムの開始時にすでにアラート対象のステータスであったモニターも同様です。
