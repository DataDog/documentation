...
ダウンタイム履歴は、[Monitor Status][4] ページにグループ遷移履歴のオーバーレイとして表示されます。または、[イベントエクスプローラー][5]内で `tags:audit,downtime` を検索するか、`tags:audit,downtime_id:<DOWNTIME_ID>` を使用して特定のダウンタイムを ID で検索できます。
