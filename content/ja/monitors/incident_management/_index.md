...
タイムラインのデータは自動的に分類されるため、ファセットを使用してタイムラインの内容にフィルターを設定できます。この機能は、調査が長期におよぶ長期的なインシデントの場合、特に便利です。これにより、IC や対応者は関係者、進捗状況、調査済みの事柄についてフィルターを設定しやすくなります。タイムスタンプや作成されたメッセージメモは編集できます。また、タイムラインコールにフラグを立てて、インシデントをモニタリングしている人に対して強調できます。
...
軽減タスクまたはインシデント発生後の修復タスクを作成します。テキストフィールドにタスクを追加することで、ここのタスクをすべて追跡できます。タスクが終了したら、ボックスにチェックを入れて完了します。
