...
季節性 
: メトリクスを分析する `agile` または `robust` アルゴリズムの[季節性](#seasonality) (`hourly`、`daily`、`weekly`) サイクル。`agile` アルゴリズムでは `monthly` も利用可能です。
...
### 季節性
<div class="alert alert-info">**注**: 機械学習アルゴリズムが完全に効率的であるためには、選択された季節性の時間の少なくとも 2 倍の履歴データ時間が必要です。例えば、1 週間ごとの季節性の場合、最低でも 2 週間のデータが必要です。</div>
...
Monthly
: このアルゴリズムは、月のある日が過去の月のある日のように振る舞うことを期待します。例えば、月の初日は過去の月の初日のように振る舞います。このアルゴリズムは `agile` で利用可能です。
...
`interval`
: ロールアップ間隔の秒数を表す正の整数。`interval` は少なくとも `alert_window` 期間の 5 分の 1 にするべきです。

`count_default_zero`
: ほとんどのモニターでは `true` を使用します。値の欠如をゼロとして解釈してはならないカウントメトリクスを送信する場合にのみ、`false` に設定します。

`seasonality`
: `hourly`、`daily`、`weekly`、または `monthly`このパラメーターは `basic` アルゴリズムを使用する場合には除外してください。`monthly` パラメーターは `agile` アルゴリズムを使用している場合に使用できます。
...
[2]: /ja/monitors/create/types/metric/#define-the-metric
...
[8]: /ja/monitors/create/configuration/#advanced-alert-conditions
[9]: /ja/monitors/create/types/metric/#data-window
