...
複合条件は 4 つの異なるステータスを監視し、そのうち 3 つがアラート対象です。
...
### New Group Delay と複合条件

[new_group_delay][4] は複合条件モニターで設定可能で、設定した値が子モニターの値よりも大きい場合は、子モニターの設定値をオーバーライドします。

**例:**

1. 子モニターに異なる New Group Delay がある複合条件:

    * モニター A: new_group_delay=120s
    * モニター B: new_group_delay=60s
    * 複合条件: `A&&B`

    新しいグループが現れると、すぐに複合条件モニターはこの新しいグループを OK の状態にします。60 秒後、新しいグループは複合条件モニターで B からの状態になリマス。120 秒後、新しいグループは複合条件内の A、B の中で最も悪いステータスになります。

2. New Group Delay がある複合条件

    * モニター A: new_group_delay=120s
    * モニター B: new_group_delay=60s
    * 複合条件: new_group_delay=200s
    * 複合条件: `A&&B`

    新しいグループが現れると、すぐに複合条件モニターはこの新しいグループを OK の状態にします。200 秒後、新しいグループは複合条件内の A、B の中で最も悪いステータスになります。
...
[2]: /ja/monitors/create/configuration/#advanced-alert-conditions
[3]: /ja/monitors/notify/
[4]: /ja/monitors/create/configuration/?tab=thresholdalert#new-group-delay