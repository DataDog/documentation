---
description: 指定した種類の監査ログが閾値を超えた場合に警告を出します。
further_reading:
...
監査ログは、指定された種類の監査ログが、ユーザー定義のしきい値を一定時間超えた場合にアラートを生成します。

## モニターの作成

Datadog で[監査ログモニター][1]を作成するには、メインナビゲーションで *Monitors --> New Monitor --> Audit Logs* の順に進みます。

### 検索クエリを定義する

監査ログの検索クエリを定義します。検索クエリは、ログエクスプローラーと同じ[検索構文][2]に従います。
