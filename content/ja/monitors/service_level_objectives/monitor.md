...
SLO で単一のマルチアラートモニターのみを使用する場合、オプションで “Calculate on selected groups” (選択したグループ で計算) を選択し、最大 20 グループを選択することができます。デフォルトでは、SLO のステータスが最も悪い 5 つのグループを使用して、SLO が計算されます。グループ選択は、複数のモニターを含む SLO には対応していません。
...
Datadog は、1 つのマルチアラートモニターの**すべての**モニターまたは**すべての**計算されたグループが、`ALERT` 状態でない時間の割合として、全体のステータスを計算します。Datadog は、集計されたモニターまたは集計されたグループの平均を計算しません。
...
### データがない
モニターを作成するときに、データが欠落したときにアラートを送信するかどうかを選択します。この構成は、モニターベースの SLO 計算が欠落データをどのように解釈するかに影響します。

欠落データを無視するように構成されたモニターの場合、欠落データのある時間帯は SLO によって OK として扱われます。欠落データに警告するように構成されたモニターの場合、欠落データのある時間帯は、SLO によって ALERT として扱われます。
...
[5]: /ja/monitors/service_level_objectives/#slo-status-corrections