---
aliases:
- /ja/monitors/guide/suppress-alert-with-downtimes
further_reading:
...
すべての `<DATADOG_API_KEY>` および `<DATADOG_APP_KEY>` を、Datadog API キーと Datadog Application キーにそれぞれ置き換えてください。
...
`start` パラメーターと `end` パラメーターは、繰り返しルールの最初の日の予想される開始時刻と終了時刻に一致させる必要があります。つまり、このルールの最初の第 2 火曜日が 7 月 11 日 (火) であるとすると、開始日は 7 月 11 日 (火) 午前 8:00 でなければならず、2 時間の期間を設定しなければなりません。
...
[1]: https://docs.datadoghq.com/ja/api/v2/downtimes/
