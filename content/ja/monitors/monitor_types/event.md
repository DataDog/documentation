...
* `<TEXT>` を含むイベントと一致:
    * 複数の用語には、暗黙の `AND` があります。たとえば、`a b` は、`a` と `b` の両方が含まれるイベントを検索します。
    * 正確な文字列 `a b` のイベントを検索するには、引用符 `"a b"` を使用します。
    * 複数の用語の少なくとも 1 つを含むイベントを検索するには、`OR` を使用します。たとえば、`a OR b` は、`a` と `b` のいずれかを含むイベントを検索します。
