...
### テストスイートレベルの可視化互換性
[テストスイートレベルの可視化][1]は `dd-sdk-swift-testing>=2.1.0` からのみサポートされます。
...
Cocoapods や Framework のリンクなどの他の方法と比較して、Swift のテストアクションオプションは構成と実行の柔軟性に欠けるかもしれませんが、コードの変更は必要ではありません。
...
テスト対象のアプリケーションが RUM を使用してインスツルメントされている場合、UI テストの結果と生成された RUM セッションは自動的にリンクされます。RUM の詳細については、[RUM iOS インテグレーション][4]ガイドを参照してください。iOS RUM バージョン &gt;= 1.10 が必要です。
...
さらに、[Intelligent Test Runner][7] を有効にすると、プロジェクトから以下のデータが収集されます。
