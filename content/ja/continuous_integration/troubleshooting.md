...
- デフォルトブランチリスト: デフォルトブランチのみがそこに表示されるため、そうでないブランチは、このテーブルから項目が欠落するか、間違ったブランチが表示される結果となります。

- デフォルトでないブランチのウォールタイム比較: ブランチ一覧ページで、**VS Default** 列は、現在のブランチのウォールタイムとデフォルトブランチのウォールタイムを比較して計算されます。
