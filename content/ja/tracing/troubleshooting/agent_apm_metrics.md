...
| メトリクス名                                         | タイプ      | 説明                                                                                              |
| --------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------- |
| datadog.trace_agent.cpu_percent                     | Gauge     | CPU 使用量 (コア単位): 例えば 50 (1 コアの半分)、200 (2 コア)、250 (2.5 コア)                            |
| datadog.trace_agent.events.max_eps.current_rate     | Gauge     | Agent が受信した 1 秒あたりの APM イベント数                                                     |
...
| datadog.trace_agent.panic                           | Gauge     | コードパニックごとに 1 増加。                                                                    |
| datadog.trace_agent.ratelimit                       | Gauge     | `1` 未満の場合、リソース使用量 (CPU またはメモリ) が多いためにペイロードが拒否されていることを意味します。       |
...
| datadog.trace_agent.receiver.out_chan_fill          | Gauge     | 内部メトリクス。受信機の出力チャンネルのフィルの割合。                                    |
...
| datadog.trace_agent.receiver.traces_received        | カウント     | 受信済みおよび承認済みのトレース数。                                                                  |
...
| datadog.trace_agent.stats_writer.connection_fill    | Histogram | 使用する発信接続の割合。                                                                 |
...
| datadog.trace_agent.stats_writer.encode_ms          | Histogram | 統計データペイロードのエンコード時間。                                                                  |
| datadog.trace_agent.stats_writer.errors             | カウント     | リトライできなかったエラー。                                                                        |
| datadog.trace_agent.stats_writer.queue_fill         | Histogram | いっぱいになったキューの割合。                                                                              |
...
| datadog.trace_agent.trace_writer.compress_ms        | Gauge     | エンコードされたトレースペイロードの圧縮にかかったミリ秒数。                                      |
| datadog.trace_agent.trace_writer.connection_fill    | Histogram | トレースライターが使用する発信接続の割合。                                             |
...
| datadog.trace_agent.trace_writer.encode_ms          | Gauge     | トレースペイロードのエンコードにかかったミリ秒数。                                                 |
| datadog.trace_agent.trace_writer.errors             | カウント     | リトライできなかったエラー。                                                                        |
| datadog.trace_agent.trace_writer.events             | カウント     | 処理されたイベント数。                                                                              |
| datadog.trace_agent.trace_writer.flush_duration     | Gauge     | ペイロードを Datadog API にフラッシュするのにかかった時間。                                                      |
| datadog.trace_agent.trace_writer.payloads           | カウント     | 処理されたペイロード数。                                                                            |
| datadog.trace_agent.trace_writer.payloads           | カウント     | 発信ペイロード数。                                                                                 |
| datadog.trace_agent.trace_writer.queue_fill         | Histogram | 発信ペイロードキューがいっぱいになる割合。                                                               |
