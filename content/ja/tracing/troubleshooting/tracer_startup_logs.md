...
PHP トレーサーの診断は、トレーサーが DEBUG モードの場合に出力されます。これは、以下の他のオプションからも利用できます。
...
#### PHP 情報
PHP 情報ページから JSON 文字列を取得します。診断情報は独立した表に表示され、一般的な問題の診断に役立ちます。
...
`\DDTrace\startup_logs()` を使用して、ランタイム時に JSON 文字列にアクセスします。
