...
1. お使いの Linux ディストリビューションに適した [`ddprof` リリース][2]をダウンロードします。例えば、`amd64` (別名 `x86_64`) プラットフォーム用の最新リリースを取得する一つの方法は以下の通りです。
...
| DD_PROFILING_NATIVE_SHOW_CONFIG | show_config     | a          | いいえ        | プロファイラーの構成パラメーターをログに記録するかどうか。                                                                                    |
...
[1]: /ja/tracing/profiler/profiler_troubleshooting
