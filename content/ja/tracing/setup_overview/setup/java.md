...
- `-javaagent` 引数を `java -jar` コマンドに追加する場合は、`-jar` 引数の_前_に追加する必要があります。つまり、アプリケーション引数としてではなく、JVM オプションとして追加する必要があります。例:
  ...
  詳細については、[Oracle のドキュメント][5]を参照してください。

- classpath に `dd-java-agent` を追加しないでください。予期せぬ挙動が生じる場合があります。

## 自動でデータと収集

Java の自動インスツルメンテーションは、[JVM によって提供される][6] `java-agent` インスツルメンテーション機能を使用します。`java-agent` が登録されている場合は、ロード時にクラスファイルを変更することができます。
...
**デフォルト**: `localhost`<br>
トレースの送信先のホスト名。コンテナ化された環境を使う場合は、これを構成してホスト IP にします。詳しくは、[Docker アプリケーションのトレース][8]を参照してください。
...
- デフォルトで、アプリケーションからの JMX メトリクスは、DogStatsD によりポート `8125` で Datadog Agent に送信されます。[DogStatsD がエージェントに対して有効になっている][10]ことを確認してください。

  - Agent をコンテナとして実行している場合は、`DD_DOGSTATSD_NON_LOCAL_TRAFFIC` が [`true` に設定されている][11]ことと、Agent コンテナでポート `8125` が開いていることを確認してください。
  - Kubernetes の場合は、[DogStatsD ポートをホストポートにバインドします][12]。ECS の場合は、[タスク定義で適当なフラグを設定します][13]。

### 統合

インテグレーションを無効にする方法については、[インテグレーション][14]の互換性セクションを参照してください。
...
JMX フェッチを使った Java メトリクス収集についての詳細は [Java インテグレーションドキュメント][15]を参照してください。
...
[10]: /ja/developers/dogstatsd/#setup
[11]: /ja/agent/docker/#dogstatsd-custom-metrics
[12]: /ja/developers/dogstatsd/
[13]: /ja/integrations/amazon_ecs/?tab=python#create-an-ecs-task
[14]: /ja/tracing/compatibility_requirements/java#disabling-integrations
[15]: /ja/integrations/java/?tab=host#metric-collection
