...
- [すべてのトレース相関識別子をログ コンテキストに追加する](#add-all-trace-correlation-identifiers-to-the-log-context)
- [メッセージにプレースホルダーを使用する](#use-placeholders-in-your-message)

#### オプション 1: すべてのトレース相関識別子をログ コンテキストに追加する {#add-all-trace-correlation-identifiers-to-the-log-context}

PHP トレーサーのデフォルトの動作は、すべてのトレース相関識別子をログ コンテキストに追加することです。
...
PHP トレーサーは、利用可能なトレース相関識別子を JSON 形式でログ コンテキストに追加します。上記のログ メッセージは次のように変換されます。
...
#### オプション 2: メッセージ内でプレースホルダーを使用する {#use-placeholders-in-your-message}

メッセージ内のプレースホルダーを使用して、どのトレース相関識別子を自動的にログに挿入するかを選択できます。PHP トレーサーは次のプレースホルダーをサポートしています。
- `%dd.trace_id%`: トレース ID
- `%dd.span_id%`: スパン ID
- `%dd.service%`: サービス名
...
PHP トレーサーは、プレースホルダーを対応する値に置き換えます。たとえば、上記のログ メッセージは次のように変換されます。
