...
**注意**: トレースメトリクスは、`trace.*` [ネームスペース][1]を基に問い合わせることができます。

## サービスのプライマリオペレーションを定義する
...
1  つのサービスに複数のプライマリオペレーションが定義されている場合は、最も高いリクエストスループットによってオペレーションが自動的に選択され、サービスのエンドポイントとなります。管理者ユーザーはこれを手動で設定できます。
...
## 手動インスツルメンテーションでプライマリオペレーションを構成する
