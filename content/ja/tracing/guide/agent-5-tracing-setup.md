...
| `extra_sample_rate`     | `DD_EXTRA_SAMPLE_RATE`     | この設定によりトレースサンプリングレートを調整します。値は `0` (サンプリングなし) から `1` (標準サンプリング) までの浮動小数点数にする必要があります。デフォルト値は `1` です。 |
| `max_traces_per_second` | `DD_MAX_TRACES_PER_SECOND` | 1 秒間にサンプリングするトレースの最大数。制限を解除にするには `0` に設定します (*非推奨*)。デフォルト値は `10` です。                              |
