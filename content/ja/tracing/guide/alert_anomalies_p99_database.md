...
2. Primary Tags に **環境を選択**し、Service に **監視するデータベースを選択**します。
   ...
   [Resource][4] にデータベースで実行される特定のクエリーを選択して監視することができますが、この例では全体的なパフォーマンスを確認するため、`*` のままにします。
...
4. ***Alert when* フィールドの値を「100%」に設定します**。
   ...
   これで、選択期間のすべてのイベントがアラートをトリガーする異常になります。これは、異常検知を開始する場合のベストプラクティスです。時間の経過とともに、状況に応じた適正値がわかってきます。異常検知モニターの詳細については、[FAQ][6] をご確認ください。
...
7. ***Edit* タブから *Status* タブに切り替えます**。
   ...
   このタブでは、モニターの現在の状況の確認やミュート設定を行ったり、発せられたアラートの詳細をより深く調査したりできます。
