...
インテリジェント保持は、サービスに対して常にアクティブであり、アプリケーションの健全性を監視するのに役立つトレースの割合を保持します。すべての[トップレベルスパン][12]は、インテリジェント保持フィルターによって保持されるトレースに対してインデックス化されます。
...
インデックス化されて 15 日間保持されるスパンをカスタマイズするには、タグに基づいて追加のフィルターを作成、変更、無効化し、保持する各フィルターに一致するスパンの割合を設定します。保持されているスパンには、対応するトレースも保存され、表示されると、完全なトレースが利用可能になります。ただし、[検索と分析][4]でタグで検索するには、検索対象のタグを直接含むスパンが保持フィルターによってインデックス化されている必要があります。

1. フィルターに名前を付けます。
2. **すべて**に一致するスパンをインデックス化するタグを設定します。
3. このフィルターが基準に一致するスパンを保持するか、[トップレベルスパン][5]のみを保持するかを選択します。
4. インデックス化するこれらのタグに一致するスパンの割合を設定します。
5. 新しいフィルターを保存します。

**注:** "Top-Level Spans for Services Only" を選択すると、保持フィルターはサービスの[トップレベルスパン][5]の選択された割合のみを保持し、それらをインデックス化します。一致するタグを持つトップレベルスパンのみをインデックス化する場合は、これを使用します。"All Spans" が選択されている場合、保持フィルターは、階層に関係なく、分散トレースのすべてのスパンの選択された割合を保持し、それらをインデックス化します。これは請求に影響を与える可能性があり、保持フィルターを設定している間のアプリ内の視覚的なインジケーターは、期間中に検出された一致するスパンの数を通知します。
...
- **トレーサーレートリミッターによってドロップされた完全なトレース** (オレンジ): [サービスの取り込み率を構成する](#change-the-default-ingestion-rate)ことを選択すると、サービスが持つべき取り込み率を明示的に定義します。ただし、保護メカニズムとして、デフォルトで 100 トレース/秒に設定されているレートリミッターが自動的に有効になります。このレートリミッターを構成するには、[サポートチケットを開いてください][7]。こちらからプロセスをガイドします。

- **Agent の CPU 制限のために削除されたトレース** (赤): Agent には、ユーザーが CPU の使用を制限できるコンフィギュレーションオプションがあります。この制限に達すると、Agent はトレーサーからのトレースの受け入れを停止します。[Agent コンフィギュレーション][8]を変更して、Agent に割り当てる CPU の量を構成します。
...
Datadog 内で取り込み率が 100% 未満であり、すべてのトレースを送信したい場合は、上記のように環境変数を設定して、制限なしのトレースを有効にします。ご不明な点がございましたら、[サポートチーム][7]までお問い合わせください。
...
2020 年 10 月 20 日以前は、Datadog は分析を実行するためのインデックススパンに App Analytics を提供していました。これは推奨されるセットアップコンフィギュレーションではなくなり、[トレースと分析][9]を使用する必要はありませんが、従来の手順は [App Analytics][10] のセットアップページで利用できます。
...
**注:** 既存の App Analytics フィルターは、Datadog 内で編集できますが、移行された[保持フィルター][2]を編集することによってのみ編集できます。レガシーフィルターは、アプリ内の[設定][11]ページでのみ読み取られるようになりました。
...
[4]: /ja/tracing/trace_search_and_analytics/#historical-search-mode
[5]: /ja/tracing/visualization/#top-level-span
...
[9]: /ja/tracing/trace_search_and_analytics
[10]: /ja/tracing/legacy_app_analytics/
[11]: https://app.datadoghq.com/apm/settings
[12]: /ja/tracing/visualization/#top-level-span