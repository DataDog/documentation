...
**注**: Intelligent Retention は保持するスパンを無差別に選択するものではないため、インテリジェントフィルターによってのみ保持されたスパンはトレース分析には_含まれません_。トレース分析は、[カスタムリテンションフィルター](#create-your-own-retention-filter)で保持されたスパンに対してのみ利用できます。
