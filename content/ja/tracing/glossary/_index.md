...
各リソースには、特定のエンドポイントにスコープされた[トレースメトリクス](#trace-metrics)を持つ独自の[リソースページ][7]があります。トレースメトリクスは、他の Datadog メトリクスと同様に、ダッシュボードにエクスポートしたり、モニターを作成するために使用することができます。リソースページには、すべての[トレース](#trace)の[スパン](#span)、リクエストの分散型トレーシング、およびこのエンドポイントへのリクエストを示すトレースの集計を表示するスパン要約ウィジェットもあります。
...
サブレイヤーメトリクスは、サービスが下流に依存する場合にのみ利用可能で、サンプリング後に計算されます。
