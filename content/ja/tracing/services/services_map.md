...
{{< img src="tracing/visualization/services_map/service_map_overview-2.png" alt="サービスマップの概要" >}}
...
サービスマップは、[ファセット][2]またはサービス名のあいまい文字列マッチを使用してフィルタリングすることができます。ファセットとは、Datadog が自動的にサービスデータに適用するタグで、サービスタイプ (Web サーバ、データベース、キャッシュなど)、最終デプロイ時間、モニターステータスなどが含まれます。フィルタリングは、数百、数千のノードが存在するマイクロサービス環境で特に有効です。さらに、サービスマップを特定の時間範囲にスコープすることができ、進化するアーキテクチャを追跡するのに役立ちます。
...
{{< img src="tracing/visualization/services_map/service_map_collapsed.png" alt="サービスマップ折りたたみノード" style="width:50%;">}}

## "service" タグ

サービスをクリックすると、さらに絞り込みが可能です。

{{< img src="tracing/visualization/services_map/servicetag-2.png" alt="サービスマップのタグ" style="width:40%;">}}
...
ノードは、サービスを APM でインスツルメントされたとおりに表し、[サービス][4]ページのサービスと一致します。エッジは、あるサービスから別のサービスへの aggregate 呼び出しを表します。これらの相互作用は、各[トレース][5]のフレームグラフに表示されます。
...
サービスマップは、ルートスパンを含む完全なトレースに基づいてレンダリングされます。指定したクエリウィンドウで一部のスパンが欠落した場合、その時間帯はマップビューを利用できないことがあります。これは、[APM 接続エラー][6]が発生し、スパンがドロップされた場合に発生する可能性があります。
...
[2]: /ja/tracing/services/services_list/#facets
