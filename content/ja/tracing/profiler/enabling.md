...
| `DD_PROFILING_HEAP_ENABLED` |                            | Boolean                    | メモリヒーププロファイリングを有効にするには、`true` に設定します。(ddtrace 0.50+)      |
...
[1]: https://app.datadoghq.com/account/settings#agent/overview
[2]: https://app.datadoghq.com/account/settings?agent_version=6#agent
[3]: https://pkg.go.dev/gopkg.in/DataDog/dd-trace-go.v1/profiler#pkg-constants
...
Datadog Profiler には MRI Ruby 2.1+ が必要です。**ウォールタイムプロファイリングは、すべてのプラットフォーム（macOS および Windows を含む）のユーザーに利用可能ですが、CPU タイムプロファイルは、現在 Linux プラットフォームでのみご利用いただけます**。アプリケーションのプロファイリングを開始するには:
