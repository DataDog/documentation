...
これらの標準ラベルは、[統合サービスタグ付け][3]の一部です。ベストプラクティスとして、DatadogDatadog ではタグおよび環境変数の構成には統合サービスタグ付けを使用することをおすすめします。
...
2. 監視するサービスごとに[インテグレーション固有のコンフィギュレーションテンプレート](#インテグレーションテンプレート)を作成します。**注**: Datadog は、Apache や Redis を含む[いくつかの一般的なコンテナ化されたサービス][4]の自動コンフィギュレーションテンプレートを提供します。
...
オートディスカバリーが有効になると、Datadog Agent は、デフォルトのオートディスカバリーコンフィギュレーションファイルに基づいて、Apache や Redis を含む複数の[サービス][4]に対してオートディスカバリーを自動的に試行します。
