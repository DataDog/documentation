---
description: 技術的な質問や緊急の問題について、チャットまたはサポート チケットで Datadog サポートに連絡する際のベスト プラクティスをご確認ください。
further_reading:
- link: https://docs.datadoghq.com/agent/troubleshooting/
  tag: Documentation
  text: Agent のトラブルシューティング
- link: /account_management/guide/access-your-support-ticket
  tag: ドキュメント
  text: サポートチケットにアクセスする
title: Datadog サポートの概要
---

## 概要

Datadog は、サポートを求めるお客様に 2 つの主要チャンネルを提供しています。
   - [サポートチケットを開く][1]ことができます。
   - テキストチャットで Datadog テクニカルサポートエンジニアとライブで会話することができます。

どちらも迅速かつ効果的な解決策を提供することに重点を置いていますが、リクエストのタイミングや性質、緊急性によって、どちらかのプラットフォームが適している場合もあります。

このガイドでは、サポートチームに連絡するためのベストプラクティスと、お客様に適したサポートチャンネルを決定するためのガイドラインを提供します。

## 前提条件

最も効率的に問題を解決するために、以下の関連情報やリソースを提供できるように準備してください。

   - 組織名 ([複数][2]にアクセスできる場合)
   - [Datadog サイト][3]
   - Datadog アカウントで問題を実証しているページへのリンク
   - 当該動作を撮影したスクリーンショットまたは画面記録
   - 問題を再現するために取ることのできる手順
   - 問題が Datadog Agent の機能に関連している場合、[Agent フレア][4]
   - 使用したドキュメントページへのリンク

注： 問題を迅速かつ正確に解決するため、サポートケースは1件につき1つの問題またはご質問に限定していただきますようお願いいたします。サポートエンジニアはケースを1件ずつ対応しており、複数の無関係な問題を1件のケースにまとめていただいた場合、解決までにお時間を要する場合がございます。複数の問題がある場合には、それぞれ個別にサポートケースをオープンいただきますようお願いいたします。

例： AWSインテグレーションのエラーに関するトラブルシューティングと、モニターのしきい値の設定方法についてご確認したい場合は、それぞれ別のサポートケースをオープンしてください。

## チャットかサポートチケットか？

以下の表を参照して、[サポート チケット][1] を作成するか、[チャットで問い合わせる](#reaching-out-on-chat) かを判断してください。

| サポートチケット                  | チャット        |
| ------------------------------- | ----------- |
| 緊急の問題                   | 製品構成サポート
| 通話/スクリーンシェアのリクエスト       | セットアップに関する質問
| インシデントと障害           | ドキュメントの明確化
| 多くの構成ファイル、ログ、クエリにまたがる複雑なサポートトラブルシューティング | 特定のコンフィギュレーションファイル、ログ、またはクエリに関するマイナーな問題

どちらのオプションが最適かわからない場合は、どちらのチャンネルでも Datadog のサポートに接続できますので、ご自由にお使いください。チャットが閉じられると自動的にサポートチケットが作成されるため、チャットで解決できなかった場合でも、問題の調査を継続することができます。

## チャットでのお問い合わせ

<div class="alert alert-danger">チャットは米国東部時間 (ET) の 10:00 から 19:00 まで、営業日にご利用いただけます。HIPAA 対応アカウントではチャットをご利用いただけません。</a></div>

開始するには、ナビゲーションメニューの左下にある **Support** をクリックしてください。

{{< img src="getting_started/support/support_chat_nav.png" alt="アプリ下部の左ナビゲーションメニューにある Support ボタン" style="width:40%" >}}

新しいチャットが開かれると、**技術的な**質問か**販売に関する**質問か尋ねられます。
   - 技術的な質問の場合、最初に対応可能なテクニカルサポートエンジニアに転送されます。Datadog の使用や構成に関する質問には最適です。
   - 販売に関する質問の場合、請求やアカウント管理に関する質問に答えることができる Datadog の営業チームの担当者に案内されます。

**注**: 緊急の問題は、サポート チケットからの問い合わせがおすすめです。その際、なぜ緊急なのかを明記してください。こうすることで、サポートの責任者がケースを適切な専門家へ直ちに振り分けられます。

### 技術的な質問に対するベストプラクティス

関連する[前提条件](#prerequisites)をできるだけ多くご用意ください。チャットで接続した Datadog のチームメンバーは、問題についての情報を収集し、解決するために最善を尽くします。すべての問題がチャットセッションで解決できるわけではありません。より詳細な調査が必要な場合、Datadog のサポートは、チャット終了後に調査に必要な情報を収集することに重点を置きます。

### フォローアップ

チャットが終了すると、自動的にサポート チケットが作成されます。チャットで解決した場合は、そのチケットをクローズできます。

追加調査が必要な場合、チケットは適切な分野の専門家に転送され、チャット中に提供された詳細とコンテキストが完全に転送されます。このチケットは、Datadog サポートチームとさらに連絡を取るために使用します。

## 言語別サポート

日本語でのサポートは、現地の祝日と 12 月 29 日から 1 月 3 日を除き、月曜日から金曜日の日本標準時 (JST) 9:00 から 17:00 まで対応しています。日本語のチャット サポートは、営業日の日本標準時 (JST) 10:00 から 16:00 までご利用いただけます。

韓国語でのサポートは、現地の祝日を除き、月曜日から金曜日の韓国標準時 9:00 から 17:00 まで対応しています。韓国語のチャット サポートは提供していません。

ご希望の言語でのサポートが利用できない場合でも、Datadog サポートとは英語で引き続きやり取りできます。

## 参考資料

{{< partial name="whats-next/whats-next.html" >}}

[1]: https://help.datadoghq.com/hc/
[2]: /ja/account_management/org_switching/
[3]: /ja/getting_started/site/
[4]: /ja/agent/troubleshooting/send_a_flare/
