...
Test Optimization を設定した後、パイプライン実行のサイドパネルの Test Runs タブで、テストのステータス (Failed、New Flaky、Passed、Skipped) を含む CI パイプラインで実行されたテストに関する情報にアクセスできます。詳細については、[Flaky Test Management ドキュメント][10]を参照してください。
...
1. `Configure notifications and automations` セクションで、モニターの通知設定を構成します。
1. モニターの権限を設定します。
1. **Create** をクリックします。
...
[10]: /ja/tests/flaky_test_management/
