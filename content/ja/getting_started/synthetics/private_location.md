...
プライベートロケーションは、プライベートネットワーク内のどこででもインストールできる Docker コンテナです。[プライベートロケーションワーカーイメージ][2]には、Docker Hub からアクセスできます。
...
2. Datadog アプリケーションで **[UX Monitoring][5]** にカーソルを合わせ、**Settings** > **Private Locations** の順に選択します。
...
これで、管理ロケーションのように新しいプライベートロケーションを使用して、Synthetics テストを実行できます。
