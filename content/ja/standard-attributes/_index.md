...
- description: 呼び出されるメソッドの名前。
  ...
  - icon-apm
  type: string
content: 以下の表は、エージェントによってDatadogに送信されるデータに自動的に適用される属性を、データドメインに応じてRUM、ログ、およびAPM製品ごとに示しています。必要に応じて、製品別にリストをフィルタリングしたり、キーワードや説明文で検索して、興味のある属性を見つけることができます。
