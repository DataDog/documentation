...
- description: 送信元デバイスのタイプ。
  ...
  - roku
  type: string
- description: これは、データの発生元技術である統合名に対応します。統合名と一致する場合、Datadog は対応するパーサーとファセットを自動的にインストールします。例えば、`nginx`、`postgresql`
    などです。
  domain: Reserved
  name: source
  product_source:
...
- description: 処理を実行するユーザー。
  ...
  - icon-log
  type: string
- description: '**nanoseconds** 単位の任意の種類の時間。HTTP 応答時間、データベースクエリ時間、レイテンシーなどがあります。[リマップ](/logs/log_configuration/processors/#remapper)して、ログ内のすべての期間をこの属性に割り当ててください。Datadog
...
- description: ユーザーアクションのタイプ (`tap` または `application_start` など)。[カスタムブラウザユーザーアクション](/real_user_monitoring/application_monitoring/browser/tracking_user_actions/?tab=npm#custom-actions)の場合には、`custom`
  ...
  - roku
  type: string
- description: 'わかりやすい名前 (例: `チェックアウトをクリック`)。[カスタムブラウザユーザーアクション](/real_user_monitoring/application_monitoring/browser/tracking_user_actions/?tab=npm#custom-actions)の場合には、APIコールで指定されたアクション名。'
  domain: Action (Browser events, Android events, iOS events, Roku events)
  name: action.name
  product_source:
  - icon-rum
...
- description: 呼び出されるメソッドの名前。
  ...
  - icon-apm
  type: string
content: 以下のテーブルは、AgentによってDatadogに送信されるデータに自動的に適用される属性を、データドメインに応じてRUM、ログ、およびAPM製品ごとに示しています。オプションで、製品別にリストをフィルタリングしたり、キーワードや説明文で検索して興味のある属性を見つけることができます。
