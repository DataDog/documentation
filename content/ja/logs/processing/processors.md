...
ルックアッププロセッサーを使用して、ログ属性と、[エンリッチメントテーブル (ベータ版)][10] またはプロセッサーのマッピングテーブル内に保存されている視認可能な値のマッピングを定義します。
...
1. [トレース ID をアプリケーションログに挿入する方法][11]のドキュメントを参照してください。セットアップの大半は、ログのインテグレーションによってデフォルトで行われます。
...
[10]: /ja/logs/guide/enrichment-tables/
