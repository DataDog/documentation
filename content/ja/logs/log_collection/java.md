...
- `<API_KEY>` を Datadog API キー値に置き換えてください。
...
[キー/値パーサー][6]は、ログイベント内で認識された `<key>=<value>` パターンを抽出します。
...
**MDC は素晴らしい機能ですが、なぜか文字列型しか使用できません。したがって、MDC を使用してメトリクス用の数値を提供するという方法はとれません。**
