...
<div class="alert alert-info">セキュリティ上の理由から、<a href="https://docs.datadoghq.com/account_management/api-app-keys/#api-keys">API キー</a>はブラウザログ SDK の設定に使用できません。理由は、JavaScript コード内でクライアント側に公開されるためです。Web ブラウザからログを収集するには、クライアントトークン<a href="https://docs.datadoghq.com/account_management/api-app-keys/#client-tokens">を使用する必要があります。</a> 
...
[`@datadog/browser-logs`][13] を `package.json` ファイルに追加してください。たとえば、npm cli を使う場合です。 
...
**注**: 初期の API 呼び出しは `window.DD_LOGS.onReady()` コールバックでラップする必要があります。これにより、SDK が正しくロードされた後にのみコードが実行されるようになります。
...
| プレースホルダー             | 説明                                                                                                                                                                                                            |
| ------------------- | --------------------------------------------------------------------------------------- |
Datadog によって完全にインデックス化されたログメッセージ。                              |
| `` | `` に付随するすべての属性を含む有効な JSON オブジェクト。        |
| `<STATUS>`          | ログのステータス; 受け入れられるステータス値は `debug`、`info`、`warn`、または `error` です。|
[JavaScript Error][10] オブジェクトのインスタンス。                                        |
...
`beforeSend` コールバック関数は、`log` イベントと `context` の 2 つの引数で呼び出すことができます。この関数を使用すると、Datadog に送信される前に Browser SDK によって収集された各ログにアクセスでき、コンテキストを使用してログのプロパティを調整できます。コンテキストにはイベントに関連する追加情報が含まれていますが、必ずしもイベントに含まれているわけではありません。通常、この情報を使用してイベントを [強化][11] したり、[破棄][12] したりできます。
...
**注意**: これらのパラメータは、[setLevel](#filter-by-status)、[setHandler](#change-the-destination)、および [setContext](#overwrite-context) API で設定できます。
...
たとえば、他のロガーと共に定義された `signupLogger` があるとします:
...
- API を使用して、すべてのロガーのために全体のコンテキストを設定します。
-`setGlobalContextProperty (key: string, value: any)` API を使用して、すべてのロガーにコンテキストを追加します。
-`getGlobalContext ()` API を使用して、全体のグローバルコンテキストを取得します。
-`removeGlobalContextProperty (key: string)` API を使用して、コンテキストプロパティを削除します。
-`clearGlobalContext ()` API を使用して、すべての既存のコンテキストプロパティをクリアします。

>> Log Browser SDK v4.17.0 では、いくつかの API の名前が更新されています。
>
> - `getGlobalContext` `getLoggerGlobalContext` の代わりに
> - `setGlobalContext` `setLoggerGlobalContext` の代わりに
> - `setGlobalContextProperty` `addLoggerGlobalContext` の代わりに
> - `removeGlobalContextProperty` `removeLoggerGlobalContext` の代わりに
...
-`setAccountProperty (key: string, value: any)` API を使用して、すべてのロガーにアカウントプロパティを追加または変更します。
...
ブラウザ SDK の v4.49.0 で `storeContextsAcrossPages` 構成オプションが導入され、これらのコンテキストを [`localStorage`][9] に格納できるようになり、以下の動作が可能になりました:
...
- これらのコンテキストで個人を特定できる情報 (PII) を設定することは推奨されません。なぜなら `localStorage` に格納されたデータはユーザーセッションよりも長持ちするからです。
...
- にログを送信し、`console` および Datadog (`http`) に送信します。
- にログを送信し、`console` のみに送信します。
- ログをまったく送信しない (`silent`)。
