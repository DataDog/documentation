...
Datadog Agent の設定ができたら、API を使用してインテグレーションデータの Datadog への送信を始めます。Agent に追加のセットアップを行うと、API を使用して Synthetics、ログ、トレースを Datadog へ送信することも可能です。
...
- [イベント][13]のエンドポイントを使用すると、[Datadog のイベントストリーム][14]との間でイベントを送受信できます。
- [Synthetics][15] エンドポイントを使用して、[Synthetics テスト][16]を作成、開始、終了し結果を確認します。
...
- [モニター][24]の作成
- [サービスチェック][25] - モニターで使用されるチェックステータスをポスト
...
- [Query a Timeseries API][41] 呼び出しのレート制限値は、組織当たり `600`/時間です。これは、オンデマンドで増やすことができます。
- [Log Query API][42] 呼び出しのレート制限値は、組織当たり `300`/時間です。これは、オンデマンドで増やすことができます。
