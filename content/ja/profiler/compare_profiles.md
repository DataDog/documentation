...
{{< img src="profiler/compare_time_periods.mp4" alt="比較ビューを開いて、プロファイルをタイムフレームの集計と比較します。" video="true">}}
...
{{< img src="profiler/compare_legend.png" alt="プロファイル比較の凡例。" >}}

プロファイル内のメソッドにカーソルを合わせると、比較対象のプロファイルよりも時間が長いか短い、または割り当てが少ないか多いメソッドに関する特定のメトリクスが表示されます。

{{< img src="profiler/compare_tooltip.png" alt="プロファイル内のメソッドにカーソルを合わせると、メトリクスの比較が表示されます" >}}

### 最近のバージョンの比較

集計ビューで、サービスを選択して、選択したタイムフレームでの特定のメトリック (たとえば、実時間) の集計プロファイルを表示します。次に、**Compare** をクリックして、別のバージョンの集計プロファイルと比較します。

{{< img src="profiler/compare_versions.mp4" alt="2 つのバージョンの比較ビューを開きます。" video="true">}}
