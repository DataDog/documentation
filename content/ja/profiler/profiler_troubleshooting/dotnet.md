...
{{% tab "Linux での Single Step APM インスツルメンテーション" %}}
...
5. 次の環境変数が設定されていないことを確認します:
   - `CORECLR_ENABLE_PROFILING`
   - `CORECLR_PROFILER`
   - `CORECLR_PROFILER_PATH`
   - `LD_PRELOAD` に `Datadog.Linux.ApiWrapper.x64.so` を指す値が設定されていないこと

   これらの変数が設定されている場合は、スクリプトまたは Dockerfile から削除してください。そうしないと、以前に手動でインストールしたプロファイラのバージョンが使用されます。

6. `DD_PROFILING_ENABLED` の値が `1` または `Auto` に設定されていることを確認します。`Auto` に設定されている場合、次の両方の条件を満たす場合にのみプロファイラがプロファイルを生成します:
   - トレースが作成されていること
   - アプリケーションが 30 秒を超えて実行されていること
他の値に設定されている、または設定されていない場合、プロファイラは無効化されます。
...
2. ローダーログからプロファイラーが読み込まれたことを確認します。
   ...
   1. `/var/log/datadog` フォルダ内の `dotnet-native-loader-dotnet-<PID>` ログ ファイルを開きます。
...
3. `DD_PROFILING_ENABLED` の値が `1` に設定されていることを確認します。設定されていない、または別の値の場合、プロファイラは無効化されます。

4. プロファイルのエクスポート結果を確認します。

   1. ステップ 2.b でデバッグ ログを有効にしていない場合は、アプリケーションに対して環境変数 `DD_TRACE_DEBUG` を `true` に設定し、再起動します。
...
5. CPU または Wall タイムのプロファイルがない場合のみ、スタックウォーク用の Datadog シグナルハンドラーが置き換えられていないことを確認します。
