...
| Accessibility |                              定義                               |
|:-------------:|:---------------------------------------------------------------------:|
|    プライベート    |     The vulnerable component or resource is in a private network.     |
|    公開     | The vulnerable component or resource is accessible from the internet. |
...
|  影響  |                                                                                                                 定義                                                                                                                |
|:--------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|    小   | This misconfiguration is related to security hardening, hygiene, resource metadata, or industry best practice configurations. By itself, this misconfiguration represents little to no impact to the environment.                                                                                                             |
|  中  | Abusing this misconfiguration results in an impact to the confidentiality, integrity, or availability of the vulnerable component or its directly associated resources.                                                                   |
|   大   | Abusing this misconfiguration results in an impact to the confidentiality, integrity, or availability of the vulnerable component and impacts a significant number of other resources. For example, an identity with the `S3FullAccess` policy attached. |
| クリティカル | Abusing this misconfiguration results in complete control of all resources in the account. For example, an identity with the `AdministratorAccess` policy attached. |
...
| **Improbable**      | 小     | Low        | 中   | Medium       |
| **Possible**        | 小     | 中     | 大     | High         |
| **Probable**        | 中  | 大       | High     | クリティカル     |
| **Highly Probable** | 中  | 大       | クリティカル | Critical     |
...
- **Severity score**: Highly Probable x Medium = High
  - 最終的な重大度スコアは「高」です。これは、「可能性がとても高い」の可能性と「中」の影響が混在しているためで、総合スコアは「高」となります。
