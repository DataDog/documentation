...
cascade:
  banner:
    ...
    title: <span>シームレスなインテグレーション。</span>Datadog Code Security をお試しください
  modal:
    bottom_boxes:
    ...
    footer:
      link:
        name: Datadog Code Security
        url: https://www.datadoghq.com/product/code-security/
      text: Datadog Code Security を使用して、開発プロセスの各ステップでコードの問題を検出します
    title: このルールを試して、Datadog Code Security でコードを分析してみましょう
    top_box:
      footer: 詳細については、<a href="/security/code_security/">Code Security ドキュメント</a>をご覧ください
      steps:
      - リポジトリのルートに、上記内容を含む static-analysis.datadog.yml を作成します
      - 無料の IDE プラグインを使用するか、Code Security スキャンを CI パイプラインに追加します
      - コードに関するフィードバックを受け取る
      title: このルールの使用方法
description: Static Code Analysis の複数言語のルールを表示します。
further_reading:
...
rulesets:
  apex-code-style:
    description: '確立されたコーディング規約に従う Apex のルールを記述するための Code Security のルールです。
  ...
  bash-code-quality:
    description: 'Bash スクリプトのコード品質を強制するためのルールです。
  ...
  github-actions:
    description: 'GitHub Actions をチェックし、権限やバージョン固定などの安全でないパターンを検出するためのルールです。
  ...
  java-inclusive:
    description: 'Java のコードとコメントで不適切な表現を避けるためのルールです。
      ...
      '
    title: Java におけるインクルーシブな表現を使用する
  java-security:
  ...
  javascript-inclusive:
    description: 'コードやコメントにおける不適切な表現を避けるための JavaScript のルールです。

      '
    title: 表現に問題がないか JavaScript コードをチェックする
  javascript-node-security:
    description: 'Node における潜在的なセキュリティホットスポットを特定するためのルールです。これには、さらなるトリアージが必要な偽陽性が含まれる場合があります。
  ...
  kotlin-security:
    description: 'Kotlin コードのセキュリティ問題を発見することに焦点を当てたルール。
  ...
  ruby-code-style:
    description: '確立されたコーディング規約に従う Ruby のルールを記述するための Code Security のルールです。
  ...
  swift-code-style:
    description: '確立されたコーディング規約に従う Swift のルールを記述するための Code Security のルールです。

      '
    title: Swift コードスタイルとベストプラクティスを強制するためのルール。
  swift-security:
    description: 'Swift コードのセキュリティ問題を発見することに焦点を当てたルール。
  ...
  typescript-code-style:
    description: '現代の TypeScript コードベースにおけるベストプラクティスと見なされるルールですが、プログラムのロジックには影響を与えません。これらのルールは、一般的によりシンプルなコードパターンを強制することに関して意見が分かれています。
...
<div class="alert alert-warning">
    コードセキュリティは利用できません。 {{< region-param key="dd_site_name" >}} `site`
</div>
{{% /site-region %}}

概要

Datadog Static Code Analysis は、コードベース内のセキュリティ脆弱性、バグ、および保守性の問題を検出するのに役立つ、すぐに使えるルールを提供します。詳細については、[セットアップドキュメント][1]を参照してください。
