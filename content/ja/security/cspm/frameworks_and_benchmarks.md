...
Cloud Security Posture Management (CSPM) には、クラウドリソースの構成を評価し、潜在的な誤構成を特定する 400 以上のコンプライアンスルールがすぐに使える状態で提供されています。各[コンプライアンスルール][1]は、以下のコンプライアンス基準および業界ベンチマーク内の 1 つまたは複数のコントロールに対応します。
...
**注**:
- CSPM は、リソースが特定のコンプライアンスルールに基づいて構成されているかを可視化します。これらのルールは、さまざまな規制フレームワーク、ベンチマーク、標準 (セキュリティポスチャフレームワーク) などに対応します。CSPM はセキュリティポスチャフレームワークへの実際のコンプライアンスを評価するものでありません。また、コンプライアンスルールは特定のフレームワークに関するコンフィギュレーション設定のすべてに対応するわけではありません。CSPM を導入する際には、法律顧問やコンプライアンスの専門家と相談することが推奨されます。
