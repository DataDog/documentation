---
aliases:
- /ja/security_platform/cspm/custom_rules/gcp_kms_crypto_key
kind: documentation
...
## `next_rotation_time`
**タイプ**: `TIMESTAMP`<br>
    **説明**: `next_rotation_time` になると、キー管理サービスは自動的にこの CryptoKey の新しいバージョンを作成し、新しいバージョンをプライマリーとしてマークします。`CreateCryptoKeyVersion` と `UpdateCryptoKeyPrimaryVersion` を使って手動で行ったキーローテーションは `next_rotation_time` に影響を与えません。目的 `ENCRYPT_DECRYPT` を持つキーは自動ローテーションをサポートします。それ以外のキーでは、このフィールドは省略しなければなりません。<br>
    **GCP 名**: `nextRotationTime`<br>
## `organization_id`
**タイプ**: `STRING`<br>
## `parent`
**タイプ**: `STRING`<br>
## `primary`
  ...
  **GCP 名**: `primary`
   ...
   - `attestation`<br>
      ...
      **GCP 名**: `attestation`
        ...
        **タイプ**: `STRING`<br>
          ...
          - `CAVIUM_V1_COMPRESSED` - Cavium HSM 認証は、gzip で圧縮されています。このフォーマットは Cavium によって定義され、いつでも変更される可能性があることに注意してください。[ソフトウェアキー認証][1]を参照してください。<br>
   ...
   - `external_protection_level_options`<br>
      ...
      **GCP 名**: `externalProtectionLevelOptions`
    ...
    **タイプ**: `BOOLEAN`<br>
        ...
        **GCP 名**: `reimportEligible`<br>
   - `state`<br>
    **タイプ**: `STRING`<br>
        **説明**: CryptoKeyVersion の現在の状態。<br>
        **GCP name**: `state`<br>
            **Possible values**:<br>
