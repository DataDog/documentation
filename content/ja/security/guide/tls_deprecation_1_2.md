...
選択したクライアントで、(`https://www.howsmyssl.com/s/api.html`[8]) にアクセスし、 `tls_version` キーを見て、サポートされている最新のバージョンを判断します。
## ブラウザサポート

最近のブラウザは、以前から TLS v1.2 をサポートしています。「〜を使用できますか」[互換性マトリックス][2]を参照して、お使いの特定のブラウザとバージョンが影響を受けるかどうかを判断してください。
## Agent サポート
...
[ソースインストールスクリプト][3]でインストールした場合、Agent v5 はシステムの Python と OpenSSL に依存します。したがって、TLS v1.2 のサポートは、システムにインストールされた Python と OpenSSL のバージョンに依存します。
...
OpenSSL は、Python、Ruby、PHP、Curl など多くのツールで使用されている汎用暗号およびセキュア通信のためのライブラリです。TLS v1.2 は OpenSSL 1.0.1 からサポートされています。詳細は [OpenSSL の変更点][4]を参照してください。
...
組み込みの .NET クライアントを使用している場合は、[さまざまなバージョンの .NET フレームワークで TLS v1.2 にアップグレードする方法][5]に関する Microsoft ガイドをお読みください。

### Powershell

TLS v1.2 の Powershell サポートは、システムにインストールされている .NET のバージョンに依存します。正確な要件は、Microsoft の [.NET による TLS のベストプラクティス][6]ガイドをお読みください。
...
また、Github で公開されている[コミュニティ Powershell モジュール][7]には、それを実現するものもあります。

この設定を永続化するには、[Office Online Server の TLS の有効化][8]に関する Microsoft のドキュメントに従って、レジストリを編集します。
