...
カーネルレベルでファイルやプロセスのアクティビティを監視し、AWS EC2 インスタンス、Docker コンテナ、Kubernetes クラスターなどのインフラストラクチャーへの脅威を検出します。CWS を[ネットワークパフォーマンスモニタリング][9]と組み合わせ、ワークロードが侵害される前にネットワークレベルで疑わしいアクティビティを検出します。
