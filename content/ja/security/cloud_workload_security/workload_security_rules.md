...
これは、クラウドワークロードセキュリティをそのまま使用した場合のデフォルトルールの一部で、様々なシェル、シェルユーティリティ、Web サーバー、言語エンジンをリストでチェックするものです。等号の右辺は、`["a", "b", "c", ...]` という形式のリストにすることができます。
...
1. Datadog で、[Agent コンフィギュレーションページ][4] に移動します。
