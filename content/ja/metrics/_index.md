...
次に、時間ロールアップを使用してデータの粒度を選択します。この例では、1 時間 (3600 秒) ごとに 1 つのデータポイントがあると定義しました。各タイムバケットのデータをどのように集計するかを選択することもできます。デフォルトでは、_平均_ が適用されますが、他の使用可能なオプションは _合計_、_最小_、_最大_、_カウント_ です。最大を適用する場合は、`.rollup(max, 60)` を使用します。
