...
Metrics without LimitsTM は、アプリ内ですべてのメトリクスタイプのタグを構成する機能を提供します。また、カウント、レート、ゲージの集計を、再デプロイやコード変更の必要なくカスタマイズできます。Metrics without LimitsTM では、アプリ内でタグの許可リストを構成し、Datadog プラットフォーム全体でクエリ可能な状態を維持することができます。これは、アプリケーションレベルまたはビジネスメトリクス (例えば、`host`) に付けられた非必須タグを自動的に除外します。この機能は、[Metrics Summary][1] ページにあります。

このページでは、観測可能な予算内でカスタムメトリクス量を管理するのに役立つ Metrics without LimitsTM の主要コンポーネントを確認します。

### タグの構成

任意のメトリクス名をクリックすると、その詳細サイドパネルが表示されます。次に、**Manage Tags** -> **“Custom”** をクリックして、ダッシュボードやモニターでクエリ可能にするタグを構成します。**Save** を選択する前に、この潜在的なタグコンフィギュレーションから生じる、インデックス化されたカスタムメトリクスの新しいボリュームの概算が表示されます。

{{< img src="metrics/mwl_tag_config.mp4" alt="タグのコンフィギュレーション" video=true >}}

Metrics without Limits のベータ版機能として、過去 30 日間にダッシュボード、ノートブック、またはモニターでアクティブにクエリされたタグの許可リスト (アイコンがついた青色) を、デフォルトでタグ構成モーダルに事前入力することが可能です。その他のタグも含めることができます。
...
Metrics without Limits のベータ版機能として、過去 30 日間にダッシュボード、ノートブック、またはモニターでアクティブにクエリされた集計の許可リスト (アイコンがついた青色) を、タグ構成モーダルに事前入力することが可能です。



### 複数のメトリクスを一度に構成する

[メトリクスタグの一括構成機能][7]を利用して、カスタムメトリックのボリュームを最適化することができます。Metrics Summary で **Configure Tags** をクリックすると、メトリクスのネームスペースを指定することができます。そして、そのネームスペースの接頭辞に一致するすべてのメトリクスに、同じ許可リストのタグを構成することができます。
...
- **Ingested Custom Metrics**: インジェストされたすべてのタグに基づくカスタムメトリクスの元となる量 (コードを使用して送信されます)。
