---
description: Amazon RDDS에서 Postgres용 데이터베이스 모니터링을 설치하고 설정합니다.
further_reading:
- link: /integrations/postgres/
  tag: 설명서
  text: 기본 Postgres 통합
title: Amazon RDS 관리형 Postgres에서 데이터베이스 모니터링 설정
---
...
[Amazon Web Services 통합 타이틀][3]에 있는 **리소스 수집** 섹션에서 **표준 수집**을 활성화합니다.

## Postgres 설정 구성

[DB 파라미터 그룹][5]에서 다음 [파라미터][4]를 설정합니다. **서버를 재시작**해야 설정이 적용됩니다. 이 파라미터에 관한 더 자세한 정보는 [Postgres 설명서][6]를 참고하세요.

| 파라미터 | 값 | 설명 |
| --- | --- | --- |
|`shared_preload_libraries` | `pg_stat_statements` | `postgresql.queries.*` 메트릭에 필요. [pg_stat_statements][6] 확장을 통해 쿼리 메트릭 수집 활성화. |
...
Postgres가 복제되면 다음 SQL 명령을 **주** 데이터베이스 서버(쓰기 권한자)에서 실행해야 합니다. 서버에서 에이전트를 연결할 PostgreSQL 데이베이스를 선택하세요. 어떤 데이터베이스에 연결하든 에이전트는 서버에 있는 모든 데이터베이스에서 원격 분석 데이터를 수집하기 때문에 기본값인 `postgres` 데이터베이스에 연결하는 것이 좋습니다. 에이전트에서 [특정 데이터베이스의 커스텀 쿼리][7]를 실행하고 싶을 경우에만 다른 데이터베이스를 선택하세요.

선택한 데이터베이스에 슈퍼 사용자(또는 충분한 권한이 있는 다른 사용자)로 연결합니다. 예를 들어 선택한 데이터베이스가 `postgres`면 다음을 실행하여 [psql][8]을 이용해 `postgres` 사용자로 연결합니다.
...
**참고:** IAM 인증도 지원됩니다. RDS 인스턴스에서 IAM 인증을 설정하는 방법은 [가이드][9]를 참고하세요.
...
RDS 호스트를 모니터링하려면 인프라에서 Datadog 에이전트를 설치하고 설정하여 각 인스턴스 엔드포인트가 원격으로 연결되도록 합니다. 에이전트는 데이터베이스에서 실행될 필요가 없고 연결하기만 하면 됩니다. 여기에서 언급되지 않은 부수적인 에이전트 설치 메서드는 [에이전트 설치 지침][10]을 참조하세요.

{{< tabs >}}
{{% tab "Host" %}}

호스트에서 실행하는 데이터베이스 모니터링 메트릭 수집을 설정하려면(예: 에이전트가 RDS 데이터베이스에서 수집할 수 있도록 소규모 EC2 인스턴스를 프로비저닝할 때) 다음을 따릅니다.
...
쿠버네티스 클러스터에서 클러스터 확인을 아직 활성화하지 않은 경우 [클러스터 확인 활성화][2] 지침에 따라 활성화합니다. 클러스터 에이전트 컨테이너에 있는 정적 파일을 이용하거나 서비스 주석을 이용해 Postgres 설정을 선언할 수 있습니다.
...
[에이전트 상태 하위 명령을 실행][11]하고 점검 섹션에서 `postgres`를 찾으세요. 또는 [데이터베이스][12] 페이지에서 시작할 수도 있습니다.
...
AWS에서 좀 더 포괄적인 데이터베이스를 수집하려면 [RDS 통합][1](선택 사항)을 설치하세요.
