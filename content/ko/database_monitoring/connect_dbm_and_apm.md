...
kind: 설명서
title: 데이터베이스 모니터링 및 트레이스 연결
---
{{< site-region region="gov" >}}
<div class="alert alert-warning">이 사이트에는 데이터베이스 모니터링이 지원되지 않습니다.</div>
{{< /site-region >}}

이 가이드는 [데이터베이스 모니터링][1]과 [APM][2]을 사용한다고 가정합니다. APM과 DBM에 연결하면 APM 트레이스 식별자가 DBM 데이터 수집에 삽입되고, 두 데이터 소스에 상관 관계가 생성됩니다. 이를 통해 APM 제품에 데이터베이스 정보를 표시하는 제품 기능을 사용할 수 있고, DBM 제품에 APM 데이터를 표시하는 기능을 사용할 수 있습니다.
...
- `service` 모드에서는 서비스 이름을 전송하며, 데이터베이스 로드에 기여하는 서비스가 무엇인지 알 수 있습니다. 이 모드는 SQL Server 애플리케이션에서만 사용할 수 있습니다.
- `none` 모드에서는 전파를 비활성화하고 애플리케이션에서 어떤 정보도 전송하지 않습니다.
...
| 언어                                 | 프레임워크 라이브러리 | Postgres  |   MySQL   |                SQL Server              |
|:-----------------------------------------|:---------------------|:---------:|:---------:|:--------------------------------------:|
| **Go:** [dd-trace-go][3] >= 1.44.0       |                      |           |           |                                        |
|                                          | [database/sql][4]    | {{< X >}} | {{< X >}} | `service` 모드에서만 지원                    |
|                                          | [sqlx][5]            | {{< X >}} | {{< X >}} | `service` 모드에서만 지원                    |
| **Java** [dd-trace-java][23] >= 1.11.0   |                      |           |           |                                        |
|                                          | [jdbc][22]           | {{< X >}} | {{< X >}} | `service` 모드에서만 지원                    |
...
`jdbc-datastore` [계측][2] 또한 활성화해야 합니다.
...
2. `dbmPropagationMode` 옵션(기본값: `ENV['DD_DBM_PROPAGATION_MODE']`):
   ```javascript
...
{{< img src="database_monitoring/explain_plans_in_traces.png" alt="실행 계획을 이용해 트레이스에 있는 데이터베이스 쿼리에서 비효율적 요소 파악.">}}
