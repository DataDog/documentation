...
애플리케이션의 보안을 유지하고 애플리케이션을 더 잘 보호하려면, 애플리케이션 키에 권한 범위를 지정하여 보다 세분화된 권한을 정의하고 애플리케이션이 Datadog 데이터에 액세스할 수 있는 권한을 최소화합니다. 이렇게 하면 애플리케이션에 대한 세분화된 액세스 제어가 가능하고 외부 액세스를 제한하여 보안 취약성을 크게 줄일 수 있습니다. 예를 들어 대시보드만 읽는 애플리케이션에는 조직 데이터를 삭제하거나 사용자를 관리하기 위한 관리자 권한이 필요하지 않습니다.
...
- 새로 생성된 API 키가 유효하려면 일반적으로 몇 초의 시간이 소요됩니다.
...
- 새로 생성된 애플리케이션 키가 유효하려면 일반적으로 몇 초의 시간이 소요됩니다.
...
애플리케이션 키에 대한 권한 범위를 지정하려면 [Datadog API][5] 또는 UI에서 애플리케이션 키를 만들거나 편집하세요. 범위는 [현재 사용자][14] 또는 [서비스 계정][15]이 소유한 애플리케이션 키에 지정할 수 있습니다. 이 필드를 지정하지 않으면 애플리케이션 키는 기본적으로 애플리케이션 키를 생성한 사용자와 동일한 범위 및 권한을 갖습니다.
