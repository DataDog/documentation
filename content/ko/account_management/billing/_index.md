...
비보고 호스트([인프라스트럭처 목록](2)에서 상태 `???`)는 청구 금액으로 계산되지 않습니다. 이러한 호스트가 [인프라스트럭처 목록][2]에서 드롭되는 데에는 최대 2시간이 소요될 수 있습니다. Datadogs에서는 이러한 호스트(유료 계정)의 기록 데이터를 보관합니다. 특정 호스트 이름 또는 태그와 함께 메트릭을 대시보드에 그래프화할 수 있습니다.
...
[**Payment Method**][8] 섹션에는 결제 옵션에 대한 세부정보가 포함되어 있습니다.
...
[**Billing Contact Details**][8] 섹션에서 빌링 연락처 세부정보를 확인할 수 있습니다.
...
[구독 세부 정보][8] 섹션에는 모든 약정 제품의 수량, 계약 가격, 주문형 가격이 포함됩니다.
...
신용카드로 결제하는 경우, [Billing History][10]에서 이전 달에 대한 영수증을 [Administrators][9]가 확인할 수 있습니다. 청구서 사본을 받으려면 [Datadog Billing][11]으로 이메일을 보내주세요.

자세한 내용은 [신용카드 결제][12]를 참고하세요.

### 인보이싱

수표, ACH, 또는 전신환으로 결제하는 경우, 매월 10일 영업일에 빌링 이메일 주소로 청구서가 발송됩니다. 추가 사본을 요청하려면 [Datadog 빌링][11]으로 이메일을 보내세요. 지불 송금처에 대한 자세한 내용은 청구서에서 확인할 수 있습니다.
...
[9]: /ko/account_management/rbac/#datadog-default-roles
