---
aliases:
- /ko/serverless/datadog_lambda_library/java/
further_reading:
...
kind: 설명서
title: 자바(Java) 서버리스 애플리케이션 계측하기
---

배포된 트레이싱을 사용해 서버리스 애플리케이션을 완전히 계측하려면 자바 람다 함수가 자바 8 코렉토(`java8.al2`), 자바 11(`java11`) 또는 자바 17(`java17`) 런타임을 사용하고 최소 1024MB의 메모리가 있어야 합니다.

람다 함수가 공개 인터넷에 액세스할 수 없는 VPC에 구축된 경우 `datadoghq.com`[Datadog 사이트][7]를 위한 [AWS PrivateLink][6]나 모든 기타 사이트를 위한 [프록시][8]을 사용하여 데이터를 보낼 수 있습니다.

이전에 Datadog 포워더(Forwarder)를 사용하여 람다 함수를 설정한 경우 [Datadog 포워더(Forwarder)를 사용해 계측하기][9]를 참조하세요. 아니면 이 안내서의 지침에 따라 Datadog 람다 확장을 사용하여 계측하세요.

Datadog 람다 레이어 `dd-trace-java:4`(또는 이전) 및 `Datadog-Extension:24`(또는 이전)을 사용하는 경우 [업그레이드를 위한 특별 지침][10]을 따르세요.
...
2. Datadog 서버리스 모니터링을 처음 시도한다면 양방향 모드에서 Datadog CLI를 시작하여 빠른 시작을 위한 첫 번째 설치 가이드를 따르세요. 이 페이지의 남은 단계는 무시해도 됩니다. 영구적으로 프로덕션 애플리케이션에 Datadog를 설치하려면 이 단계를 건너뛰고 남은 단계에 따라 일반적인 구축 _이후) CI/CD 파이프라인에서 Datadog CLI 명령을 실행하세요.
...
6. Lambda 함수 계측
   ...
   **참고**: 개발 또는 스테이징 환경에서 먼저 람다 함수를 계측할 수 있습니다. 계측 결과가 만족스럽지 않다면 동일한 인수로 `uninstrument`를 실행하여 변경을 원래대로 복원하세요.
...
<div class="alert alert-info">Datadog CDK 구문을 통해 자바 함수를 계측하는 것은 Node.js 및 파이쎤에서 작성된 AWS CDK 앱에서만 가능합니다.</div>

[Datadog CDK 구문]은 자동으로 람다 레이어를 사용해 함수에서 Datadog를 설치합니다. 함수를 설정하여 Datadog 람다 확장을 통해 Datadog에 메트릭, 트레이스 및 로그를 전송합니다.
...
2. Lambda 함수 계측
   ...
   자리 표시자를 채우는 방법:
    - `<DATADOG_SITE>`를 {{< region-param key="dd_site" code="true" >}} (정확한 사이트가 정확한 곳에 선택되었는지 확인하세요)로 대체합니다. 
    - `<DATADOG_API_KEY_SECRET_ARN>`을 [Datadog API 키][2]가 안전하게 저장된 AWS 암호 ARN으로 대체합니다. 키는 일반 텍스트 문자열로로 저장해야 합니다(JSON blob 안 됨). 또한, `secretsmanager:GetSecretValue` IAM 권한이 있어야 합니다. 빠른 테스트를 위해 대신 `apiKey`를 사용하고 Datadog API 키를 일반 텍스트로 설정할 수 있습니다.
...
- 이제 [서버리스 홈페이지][1]에서 메트릭, 로그, 트레이스를 조회할 수 있습니다.
- 귀하의 서비스를 목표로 하는 공격자에 대한 알림을 받으려면 [위협 모니터링][11]을 켭니다.
...
[3]: /ko/tracing/custom_instrumentation/java/
[4]: /ko/serverless/guide/troubleshoot_serverless_monitoring/
[5]: /ko/serverless/configuration/
[6]: /ko/agent/guide/private-link/
[7]: /ko/getting_started/site/
[8]: /ko/agent/proxy/
[9]: /ko/serverless/guide/datadog_forwarder_java
[10]: /ko/serverless/guide/upgrade_java_instrumentation
[11]: /ko/security/application_security/enabling/serverless/?tab=serverlessframework