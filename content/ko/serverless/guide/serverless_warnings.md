...
**해결 방법:** 콜드 스타트는 서버리스 애플리케이션에 트래픽이 급증할 때 발생합니다. 함수가 이전에 비활성 상태였거나 수신 요청 수가 비교적 일정했던 경우에도 발생할 수 있습니다. 사용자는 응답 시간이 느려지거나 지연 시간이 발생하는 것으로 콜드 스타트를 경험합니다. 콜드 스타트를 방지하려면 관련 Lambda 함수에 [provisioned concurrency][8]를 활성화하는 것을 고려해 보세요. 이는 AWS 청구서에 영향을 줄 수 있으니 참고하세요.
...
**해결 방법:** Lamda 함수에 할당된 메모리를 줄이는 것을 고려해 보세요. 이는 AWS 청구 금액에 영향을 줄 수 있으니 참고하세요. 
