...
람다 함수에서 환경 변수 `DD_LOG_LEVEL`을 `debug`로 설정하여 디버깅 로그를 활성화합니다. 로그에서 데이터 포워딩을 위해 [Datadog 포워더 람다 함수][2]를 사용하는 경우 포워더 람다 함수에서 `DD_LOG_LEVEL`을 `debug`로 설정합니다.
...
Datadog는 [AWS 통합][10](부수적)을 통해 AWS에서 메트릭과 리소스 태그를 수집할 수 있습니다. 클라우드와치(CloudWatch) 및 람다 리소스 태그에서 특정 메트릭을 놓치고 있다면 AWS 통합이 제대로 설정되었는지 확인하세요.
...
위에 나와 있는 모든 트러블슈팅 단계를 따랐고 [Datadog 지원][12]을 받은 경우, 티켓에

1. 람다 함수(ARN, 런타임, 핸들러, 레이어, 환경 변수 및 태그)에 대한 기본 정보를 포함하세요. 다수의 함수에 대해 동일한 이슈가 있는 경우 먼저 단일 함수에 집중하세요.
1. 람다 함수가 Datadog 포워더 람다 함수를 사용해 로그로 데이터를 전송하도록 설정된 경우, 포워더 람다 함수에 대한 기본 정보와 람다 함수 로그 그룹에 설정된 구독 필터에 대한 기본 정보를 포함하세요.
2. 사용한 설치 방법(_서버리스 프레임워크_ 또는 _AWS CDK_ 등)
3. 시도한 대체 설치 방법(_Datadog CLI_ 또는 _커스텀_)
4. 자체적인 람다 함수의 디버깅 로그
5. Datadog 포워더 람다 함수의 디버깅 로그(사용한 경우)
6. `serverless.yaml`, `package.json`, `package-lock.json`, `yarn.lock`, `tsconfig.json` 및 `webpack.config.json` 등 **삭제된 하드코드된 암호**를 포함하는 프로젝트 설정 파일
