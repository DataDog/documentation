...
프로파일러를 설정하였고 프로필 검색 페이지에서 프로필을 확인할 수 없는 경우, 운영 체제에 따라 다음 사항을 확인해 보세요.
...
3. 프로필 내보내기 결과를 확인합니다.
   ...
   4. `Failed to send profile` 메시지가 보이지 않으면 `The profile was sent. Success?` 입력 항목을 찾습니다.
      ...
      다음 메시지는 프로필이 성공적으로 전송되었음을 나타냅니다.
      ```
      true, Http code: 200
      ```
...
4. 누락된 CPU 시간 또는 경과 시간(Wall time) 프로필의 경우에만 스택 워크에 대한 Datadog 신호 핸들러가 교체되지 않았는지 확인합니다. 
   ...
   다음 메시지가 표시될 수 있지만 Datadog 프로필링에는 영향을 미치지 않으니 참고하세요. `Profiler signal handler has been replaced. Restoring it.` 이 메시지는 Datadog 신호 핸들러가 재정의되었을 때 다시 설치되었음을 알려주는 역할만 합니다.

5. 다음 환경 변수를 설정하지 않아야 합니다. 다시 확인하세요.
   - `CORECLR_ENABLE_PROFILING`
   - `CORECLR_PROFILER`
   - `CORECLR_PROFILER_PATH`
   - `LD_PRELOAD`에서 `Datadog.Linux.ApiWrapper.x64.so`를 가리키는 값

   이러한 변수가 설정되어 있다면 스크립트나 Dockerfile에서 제거하세요. 그러지 않으면 이전에 수동으로 설치한 프로파일러 버전이 사용됩니다.

6. `DD_PROFILING_ENABLED` 값이 `1` 또는 `Auto`로 설정되어 있는지 확인하세요. `Auto`로 설정된 경우,  프로파일러는 다음 두 조건이 모두 충족되는 경우에만 프로필을 생성합니다.
   - 트레이스가 생성되었습니다.
   - 해당 애플리케이션은 30초 이상 실행되었습니다.
다른 값으로 설정하거나 전혀 설정하지 않으면 프로파일러가 비활성화됩니다.
...
3. `DD_PROFILING_ENABLED` 값이 `1`로 설정되어 있는지 확인하세요. 설정되어 있지 않거나 다른 값이 있는 경우 프로파일러가 비활성화됩니다.

4. 프로필 내보내기 결과를 확인합니다.

   1. 2.b 단계에서 디버그 로그가 활성화되지 않은 경우, 애플리케이션의 `DD_TRACE_DEBUG` 환경 변수를 `true`로 설정하고 다시 시작합니다.
...
5. 누락된 CPU 시간 또는 경과 시간(Wall time) 프로필의 경우에만 스택 워크에 대한 Datadog 신호 핸들러가 교체되지 않았는지 확인합니다. 
...
3. `DD_PROFILING_ENABLED` 값이 `1`로 설정되어 있는지 확인하세요. 설정되어 있지 않거나 다른 값으로 설정된 경우 프로파일러가 비활성화됩니다.
...
[각기 다른 프로필 유형][3]에는 고정 CPU 및 메모리 오버헤드가 있으므로 *애플리케이션이 더 많을 수록 오버헤드가 높아집니다.*
...
- 직접 애플리케이션을 실행하는 대신 배치 파일로 환경을 설정하여 특정 애플리케이션만 프로필링합니다.
- 프로필링되는 IIS 풀의 개수를 줄입니다(IIS 10 이상에서만 가능).
- `DD_PROFILING_WALLTIME_ENABLED=0` 설정을 사용해 경과 시간 프로필링을 비활성화합니다.
...
애플리케이션 성능 모니터링(APM) 추적은 또한 CLR 프로필링 API에 의존하므로, .NET 프로필 수집을 중단하지만 계속 .NET 트레이스를 수집하려면 다음 환경 변수를 설정하여 프로필링만 비활성화합니다.
