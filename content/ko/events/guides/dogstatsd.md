...
[DogStatsD 설치][1] 후 다음 기능을 사용하여 [Datadog event explorer][2]에 이벤트를 내보낼 수 있습니다.
...
|----------------------|-----------------|----------|--------------------------------------------------------------------------------------------|
| `<title>`            | 스트링          | 예      | 이벤트 타이틀                                                                     |
| `<message>`          | 스트링          | 예      | 이벤트 텍스트 본문                                                                 |
| `<alert_type>`       | 스트링          | 아니요       | `error`, `warning`, `success` 또는 `info`(기본값은 `info`)                              |
| `<aggregation_key>`  | 스트링          | 아니요       | 이벤트 집계에 사용할 키                                                        |
| `<source_type_name>` | 스트링          | 아니요       | 소스 유형 이름(기본값은 `my_apps`)                                               |
| `<date_happened>`    | 정수         | 아니요       | 이벤트의 epoch 타임스탬프(기본값은 DogStatsD 서버의 현재 시간) |
| `<priority>`         | 스트링          | 아니요       | 이벤트의 우선도를 지정합니다(`normal` 또는 `low`).                                    |
| `<tags>`             | 스트링 목록 | 아니요       | 이 이벤트와 연결된 태그 목록                                                  |
| `<hostname>`         | 스트링          | 아니요       | 호스트 이름                                                                       |
