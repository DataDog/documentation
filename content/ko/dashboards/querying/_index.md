...
- 템플릿 변수를 사용해 동적으로 쿼리를 편집하세요. 태그 키와 그래프를 사용해 `$`을 추가하면 자동으로 템플릿 변수 드롭다운에서 선택한 태그가 적용됩니다. 자세한 정보는 [템플릿 변수 설명서][8]를 참조하세요.
...
실제로 에이전트는 15~20초마다 메트릭을 수집합니다. 그러므로 데이터 하루 집계량은 4320개의 데이터 요소입니다. 단일 그래프에 하루 집계량을 표시하는 경우 Datadog는 자동으로 데이터를 롤업합니다. 시간 집계에 대한 자세한 내용은 [메트릭 소개][10]를 참조하세요. [롤업][11] 설명서를 참조해 롤업 간격과 Datadog가 자동으로 데이터 요소를 롤업하는 방법에 대한 자세한 정보를 알아보세요.

수동으로 데이터를 롤업하려면 [롤업 함수][12]를 사용하세요. 시그마 아이콘을 클릭해 함수를 추가하고 드롭다운 메뉴에서 `rollup`을 선택합니다. 그런 다음 데이터 집계를 원하는 방법과 간격(초)를 선택하세요.
...
Datadog의 중첩된 쿼리 기능을 사용하면 기존 메트릭 쿼리 결과의 시간 및/또는 공간 누계에 추가 레이어를 더할 수 있습니다. 이와 같은 고급 쿼리 기능으로 개수/속도/게이지 유형 메트릭의 누적된 쿼리로 백분율과 표준편차를 구할 수 있고, 과거 시간대에서 더 세밀한 쿼리에 액세스할 수 있습니다.
...
분석 요구 사항에 따라 쿼리에 다른 수학 함수를 적용할 수 있습니다. 그 예로는 등급, 미분, 평활화 등이 있습니다. 이와 관련해서는 [사용 가능한 함수 목록][14]을 참조하세요.
...
**참고**: 쉼표를 사용하는 쿼리는 시각화에서만 지원되며 모니터에서는 작동하지 않습니다. [부울 연산자][15] 또는 산술 연산을 사용하여 모니터에서 여러 메트릭을 결합합니다.
...
[시계열][16] 시각화를 위한 그래프 편집기의 **이벤트 오버레이** 섹션을 사용하여 이벤트 상관관계를 봅니다. 검색 필드에 텍스트 또는 구조화된 검색 쿼리를 입력합니다. 이벤트 검색은 [로그 검색 구문][17]을 사용합니다.
