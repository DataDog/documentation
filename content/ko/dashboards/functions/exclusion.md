...
참고: `clamp_min(values, threshold)`와 `clamp_max(values, threshold)`를 사용하면 값이 `NaN`일 경우 `threshold`로 변합니다.
...
임계값을 추가합니다. `cutoff_min()`는 이 임계값보다 낮은 모든 메트릭 값을 `NaN`으로 바꾸고, `cutoff_max()`는 이 임계값보다 높은 모든 메트릭 값을 `NaN`으로 바꿉니다. 컷오프 함수는 임계값과 **같은** 값은 교체하지 않습니다.
