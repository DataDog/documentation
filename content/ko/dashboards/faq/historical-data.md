...
대시보드의 경우 만료 기간 이후에 메트릭 또는 태그가 시각화에 표시됩니다. 그러나 UI 편집기를 사용해 그래프화하기 위한 드롭다운 패널에서는 사용할 수 없습니다. 해당 메트릭 또는 태그는 [JSON][1] 메서드를 사용하여 그래프로 표시할 수 있습니다.

### 호스트

Agent를 실행 중이고 의도적으로 호스트를 [중지][4] 또는 [제거][5]했다면 2시간 동안 새로운 데이터를 확인하지 못한 모든 호스트가 UI에서 사라집니다. 이들을 계속 쿼리할 수는 있습니다. 그러나 드롭다운, 인프라스트럭처 목록 또는 호스트 맵에는 표시되지 않습니다.

[1]: /ko/dashboards/graphing_json/
[2]: /ko/agent/
[3]: /ko/infrastructure/
[4]: /ko/agent/configuration/agent-commands/#start-stop-restart-the-agent
