...
**모니터** > **검사 요약**을 클릭하여 [검사 요약][1] 페이지를 확인합니다.

{{< img src="developers/service_checks/check_summary.png" alt="검사 요약" >}}

지난 하루 동안 인프라스트럭처 전반에 걸쳐 보고된 모든 검사 및 해당 상태의 목록입니다. 검사를 선택하면 연결된 각 태그의 개별 검사 수에 대한 인사이트를 얻을 수 있습니다.
...
메트릭처럼 시간 경과에 따른 서비스 검사를 그래프로 표시할 수 없더라도, 계속 모니터링할 수 있습니다.
**모니터** > **새 모니터**로 가서 **서비스 검사**를 선택합니다:
...
- **팀에 알림**: 해당 모니터가 누구에게 알려야 할지 선택합니다.
- **진행 상황 전달**: 전송된 알림을 편집합니다 ([Datadog 알림][4] 참고).

서비스 검사 생성에 대한 자세한 내용은 [서비스 검사 모니터][5]를 참조하세요. 
