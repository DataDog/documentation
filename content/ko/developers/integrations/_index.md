...
이 페이지에서는 기술 파트너가 [Datadog Agent][11] 또는 [Datadog API][12]를 사용하여 어떻게 [통합을 구축](#create-a-datadog-integration)하고 제품을 **Integrations** 또는 **Marketplace** 페이지에 등록하는지 확인할 수 있습니다.
...
Datadog에 통합을 등록하기 전에 먼저 [Datadog Partner Network][5]의 **Technology Partner** 트랙에 지원하세요. 신청서가 승인되면 통합 개발을 시작할 수 있습니다.
...
**Technology Partner** 트랙에 가입하고 샌드박스 계정을 요청하면 다음 문서를 통해 제품 개발에 대해 자세히 알아볼 수 있습니다.

* [Datadog 학습 센터][8]에서 온디맨드 [**Datadog 통합 소개**][7] 과정을 수료합니다.
* [API 기반 통합][1] 생성 및 [API 기반 통합을 위한 OAuth 2.0 클라이언트][9] 설정 관련 문서
* [Agent 기반 통합][2] 생성에 대한 문서.

Datadog 통합 또는 기타 유형의 제품 판매에 대한 자세한 내용은 [마켓플레이스 제품 구축][4]을 참조하세요.
...
Agent 기반 통합은 [Datadog Agent][11]를 사용하여 기술 파트너가 작성한 검사를 통해 데이터를 제출합니다. 이러한 통합을 위한 구현 코드는 Datadog에서 호스팅됩니다.

Agent 통합은 LAN(Local Area Network) 또는 VPC(Virtual Private Cloud)에 있는 시스템이나 애플리케이션에서 데이터를 수집하는 데 가장 적합합니다. [Agent 통합을 생성][2]하려면 솔루션을 Python 휠(`.whl`)로 게시하고 배포해야 합니다.

### API 기반 통합

API 기반 통합은 [Datadog API][12]를 사용하여 외부 플랫폼에서 메트릭, 트레이스, 로그 등과 같은 원격 분석을 제출합니다. 그런 다음 고객은 이 데이터를 나머지 스택의 데이터와 함께 시각화하고 상호 연관시켜 문제를 신속하게 분석하고 해결할 수 있습니다. API 기반 통합은 고객이 [OAuth를 사용하여 액세스를 승인][13]하면 Datadog에서 데이터를 읽을 수도 있습니다.

기술 파트너는 통합을 구성하는 구현 코드를 작성하고 호스팅합니다. [API 통합 생성][1]은 Datadog과 다른 SaaS 플랫폼 간의 커넥터를 구축하는 기술 파트너에게 적합합니다.
