...
에이전트 기반 통합을 사용해 [모니터][25], [대시보드][26] 및 [로그 파이프라인][27]과 같은 자산을 바로 포함할 수 있습니다. 사용자가 통합 타일에서 **설치**를 클릭하면 설치 지침이 안내되고 계정에 바로 사용 가능한 모든 대시보드가 표시됩니다. 로그 파이프라인과 같은 기타  자산은 통합이 적절하게 설치 및 설정된 후 사용자에게 표시됩니다.
...
5. 개발 작업 테스트가 완료되면 [타일 생성][20] 단계를 따라 **통합** 또는 **마켓플레이스** 페이지에 통합을 표시합니다.
