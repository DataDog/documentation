...
description: Datadog 통합 준비 시 필요한 다양한 에셋에 대해 알아봅니다.
further_reading:
- link: https://github.com/DataDog/documentation/blob/master/CONTRIBUTING.md
  tag: GitHub
  text: 설명서 사이트에 대한 지침 제공
- link: /developers/integrations/
  tag: 설명서
  text: 에이전트 또는 API 기반 통합 생성에 대해 알아보기
- link: /developers/integrations/oauth_for_integrations/
  tag: 설명서
  text: 통합을 위한 OAuth 사용에 대해 알아보기
kind: 설명서
title: 통합 에셋 참조
---
## 개요

이 페이지에서는 [**통합**페이지][12] 또는 [**마켓플레이스**페이지][9]에서 오퍼링을 생성하기 위해 필요한 파일에 대해 설명합니다.

## 설정 파일

새로운 통합을 준비할 때 필요한 옵션과 적절한 기본값을 포함하는 예제 설정을 포함해야 합니다. 다음 예제 설정 파일은 `<CHECK_NAME>/datadog_checks/<CHECK_NAME>/data/conf.yaml.example`에 위치합니다. 이는 두 개의 최상위 요소 `init_config`및 `instances`를 가지고 있습니다.
...
설정 블록은 몇 가지 가이드라인을 따릅니다:

- 설명은 비워둘 수 없습니다.
- 플레이스홀더를 위해서는 항상 `<THIS_IS_A_PLACEHOLDER>` 형식을 사용하세요. 자세한 내용은 [설명서 사이트 가이드라인][1]을 참조하세요.
- 모든 필수 매개 변수는 기본적으로 주석 처리되지 **않습니다**.
- 모든 부수적 매개 변수는 기본적으로 주석 처리됩니다.
- 플레이스홀더에 통합에 대한 기본값 (예: 통합의 상태 엔드포인트)이 있는 경우, 일반 플레이스홀더 대신 해당 값을 사용할 수 있습니다.
...
- `name`: `search_string`와 같은 매개 변수의 이름 (필수).
- `type`: 매개 변수 값의 데이터 유형 (필수).
          사용 가능한 값: _boolean_, _string_, _integer_, _double_, _float_, _dictionary_, _list\*_, and _object\*_.
- `defval`: 매개 변수의 기본값으로, 비워둘 수 있음 (선택 사항).

`list` 및 `object` 변수는 여러 줄에 걸쳐 있고 특별한 규칙이 있습니다.

- `list`에서 개별 요소는 `@param` 사양과 함께 문서화되지 않습니다.
- `object`에서는 `@param` 사양을 사용하여 요소를 개별적으로 문서화하거나 객체 자체의 사양에 따라 공통 최상위 설명을 갖도록 선택할 수 있습니다.

### 부수적 매개 변수

부수적인 매개 변수는 기본적으로 주석을 달아야 합니다. 매개 변수가 걸쳐있는 모든 줄 앞에 `@param` 사양과 동일한 들여쓰기와 함께 `#`를 추가합니다.
...
YAML 구문에 대한 자세한 내용은 [YAML에 대한 위키피디아 문서][2]를 참조하세요. [온라인 YAML 파서][3]를 살펴볼 수도 있습니다.

## 매니페스트 파일

[**통합**페이지][4] 또는 [**마켓플레이스**페이지][11]의 모든 오퍼링에는 오퍼레이팅 매개 변수, 우수한 Datadog 통합 에코시스템 내에서의 포지셔닝 및 추가 메타데이터를 정의하는 `manifest.json` 파일이 포함되어 있습니다.

{{% integration-assets-reference %}}

### Classifier 태그

`classifier_tags` 매개 변수를 사용하여 여러 카테고리를 설정하고 통합에 대해 전송되거나 쿼리된 데이터 유형을 정의할 수 있습니다.
...
`service_check.json` 파일은 통합이 수행하는 서비스 검사에 대해 설명합니다.

아래에서 `service_checks.json` 파일에 대한 필수 특성 전체 목록을 확인할 수 있습니다:
...
| `groups`        | [태그][8]가 서비스 검사와 함께 전송됩니다.                                                                                     |
...
`metadata.csv` 파일에 대한 필수 및 부수적인 특성의 전체 목록은 아래에서 확인할 수 있습니다:

| 열 이름     | 필수 사항 또는 선택 사항 | 설명                                                                                                                                                                                                                                                                                                                             |
| --------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `metric_name`   | 필수 사항          | 메트릭의 이름입니다.                                                                                                                                                                                                                                                                                                                     |
| `metric_type`   | 필수 사항          | 메트릭의 유형입니다. 사용 가능한 메트릭 제출 유형 목록은 [메트릭 유형][6]을 참조하세요.                                                                                                                                                                                                                                                                                                                |
| `interval`      | 부수적인           | 메트릭의 수집 간격(초)입니다.                                                                                                                                                                                                                                                                                            |
| `unit_name`     | 부수적인           | 메트릭의 단위입니다. 지원되는 단위의 전체 목록은 [메트릭 단위][7]를 참조하세요.                                                                                                                                                                                                                                                                              |
| `per_unit_name` | 부수적인           | `request per second`와 같은 단위 하위 구분이 있는 경우입니다.                                                                                                                                                                                                                                                                               |
| `description`   | 부수적인           | 메트릭에 대한 설명입니다.                                                                                                                                                                                                                                                                                                              |
