...
실시간으로 EventStore에서 메트릭을 받아 다음을 수행할 수 있습니다.

- EventStore 대기열 시각화 및 모니터링
- 통계, 노드 정보, 비트랜잭션 예측, 구독, 클러스터 가십(스크랩할 엔드포인트 목록은 구성 가능) 등 다음 API 엔드포인트 내에서 사용 가능한 모든 메트릭 캡처
...
EventStore 점검은 이벤트를 포함하지 않습니다.

### 서비스 점검

EventStore 점검은 서비스 점검을 포함하지 않습니다.
