...
custom_kind: 통합
dependencies: []
description: 게이트웨이 오류, 캐시 히트 및 미스, 요청 지연 시간을 추적하세요.
...
2. 원하는 API를 선택하고 Stages 섹션으로 이동합니다.
3. **Logs** 탭에서 **Enable CloudWatch Logs** 및 **Enable Access Logging**을 활성화합니다.
4. `INFO` 수준을 선택하여 모든 요청이 있는지 확인합니다.
5. **CloudWatch Group** 이름이 `api-gateway`로 시작하는지 확인합니다.
6. JSON 형식(CLF 및 CSV도 지원됨)을 선택하고 **Log format** 상자에 다음을 추가합니다.
