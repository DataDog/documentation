...
프라이빗 위치 컨테이너가 `Out Of Memory`로 작동하지 않는 것은 일반적으로 프라이빗 위치 작업자의 리소스 고갈 문제를 보여줍니다. 프라이빗 위치 컨테이너에 [충분한 메모리 리소스][13]가 프로비저닝되는지 확인하세요.

### 내 브라우저 테스트 결과가 때로 `Page crashed` 오류를 표시함

이는 프라이빗 위치 작업자의 리소스 고갈 문제를 보여주는 것일 수 있습니다. 프라이빗 위치 컨테이너에 [충분한 메모리 리소스][13]가 프로비저닝되는지 확인하세요.

### 때로 테스트 실행 속도가 저하됨

프라이빗 위치 작업자의 리소스 고갈 문제를 보여주는 것일 수 있습니다. 프라이빗 위치 컨테이너에 [충분한 CPU 리소스][13]가 프로비저닝되는지 확인하세요.

### 브라우저 테스트 실행에 너무 오랜 시간이 걸림

프라이빗 위치 구현 환경에 [메모리 없음 문제][14]가 발생하지 않았는지 확인하세요. 이미 [디멘저닝 가이드라인][15]에 따라 컨테이너 인스턴스 크기 확장을 시도한 경우 [Datadog 지원][1]에 문의하세요.
...
윈도우즈(Windows) 기반 컨테이너에 단일 파일을 마운트하려고 시도할 때 발생합니다. 이러한 기능은 지원되지 않습니다. 자세한 정보는 [도커(Docker) 마운트 볼륨 설명서][16]를 참조하세요. 바인드 마운트 소스가 로컬 디렉터리에 있는지 확인하세요.
...
또한, 프라이빗 위치 버전 `>v1.27`에서 Datadog는 `clone3` 시스템 호출 사용에 의존합니다. 이전 버전의 컨테이너 런타임 환경(도커(Docker) 버전 20.10.10 이하)에서 `clone3`은 기본 `seccomp` 정책에서 지원되지 않습니다. 컨테이너 런타임 환경의 `seccomp` 정책이 `clone3`을 포함하는지 확인하세요. 사용 중인 런타임 버전을 업데이트하고 `clone3`을 `seccomp` 정책에 추가하거나 `unconfined` seccomp 정책을 사용하여 이를 수행할 수 있습니다. 자세한 정보는 [도커 `seccomp` 설명서][17]을 참조하세요.
...
[13]: /ko/synthetics/private_locations#private-location-total-hardware-requirements
[14]: https://docs.docker.com/config/containers/resource_constraints/
[15]: /ko/synthetics/private_locations/dimensioning#define-your-total-hardware-requirements
