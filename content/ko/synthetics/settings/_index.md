...
사용량 속성 페이지에서 비용 및 사용량 속성을 세분화할 태그를 최대 3개까지 설정할 수 있습니다. 모든 테스트에 사용량 속성 태그 적용**을 선택하면 사용자가 신서틱(Synthetic) 테스트를 만들거나 편집할 때 설정된 모든 사용량 속성 태그를 입력해야 합니다. 이 설정을 활성화하는 경우, 사용자가 필요한 태그 을 모두 입력하지 않으면 테스트를 저장할 수 없습니다.
...
브라우저 옵션에는 Google Chrome, Mozilla Firefox, Microsoft Edge가 있습니다. 장치 옵션에는 대형 노트북, 태블릿, 소형 모바일 장치 등이 있습니다.
...
Datadog 애플리케이션 성능 모니터링(APM) 통합 헤더를 사용하면 Datadog 브라우저 테스트를 애플리케이션 성능 모니터링(APM) 와 연결할 수 있습니다. 

**값** 목록 에 URL을 추가하여 애플리케이션 성능 모니터링(APM) 헤더를 전송할 엔드포인트를 정의합니다. 엔드포인트가 추적 중이고 허용되는 경우 브라우저 테스트 결과는 해당 트레이스에 자동으로 연결됩니다.
...
Datadog가 테스트 실행에서 RUM 데이터를 수집하도록 허용하려면 **신서틱(Synthetic) RUM 데이터 수집 활성화**를 클릭합니다. 비활성화하면 브라우저 테스트 레코더에서 RUM 설정을 편집할 수 없습니다. 변경 사항을 적용하려면 **RUM 데이터 수집 저장**을 클릭합니다.

데이터를 전송할 새 브라우저 테스트의 기본 애플리케이션을 선택합니다. **기본 애플리케이션** 드롭다운 메뉴를 사용하여 브라우저 테스트 데이터를 수집하는 RUM 애플리케이션을 선택합니다. 변경 사항을 적용하려면 **RUM 데이터 애플리케이션 저장**을 클릭합니다.
...
기본적으로 [Datadog 관리자 및 Datadog 표준 역할][11]을 가진 사용자만 신서틱(Synthetic) 모니터링 **글로벌 변수** 페이지에 액세스할 수 있습니다. 사용자를 이 두 [기본 역할][11] 중 하나로 업그레이드하면 **글로벌 변수** 페이지에 액세스할 수 있습니다.

[커스텀 역할 기능][12]을 사용하는 경우 `synthetics_default_settings_read` 및 `synthetics_default_settings_write` 권한이 포함된 커스텀 역할에 사용자를 추가합니다.
