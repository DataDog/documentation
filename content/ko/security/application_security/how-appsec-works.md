...
보안 추적을 위한 데이터는 90일 동안 보관됩니다. 기본 추적 데이터는 15일 동안 보관됩니다.

## 데이터 개인 정보 보호

기본적으로 ASM은 보안 추적에서 정보를 수집하여 의심스러운 것으로 분류된 요청을 이해하는 데 도움을 줍니다. 데이터를 보내기 전에 ASM은 데이터가 민감한 데이터임을 나타내는 패턴과 키워드를 검색합니다. 데이터가 민감한 것으로 간주되면 해당 데이터는 플래그로 대체됩니다. 이는 요청이 의심스럽지만 데이터 보안 문제로 인해 요청 데이터를 수집할 수 없음을 나타냅니다.
