...
다음은 특정 트레이서 버전에 대해 Java 라이브러리가 지원하는 애플리케이션 보안 기능입니다.
...
Java에서 지원하는 모든 애플리케이션 보안 기능을 사용하는 데 필요한 최소 트레이서 버전은 1.31.0입니다.
...
**참고**: Azure App Service는 **웹 애플리케이션만** 지원됩니다. Application Security은 Azure 기능을 지원하지 않습니다.
...
**참고**: 많은 애플리케이션 서버가 Servlet과 호환되며 Websphere, Weblogic, JBoss와 같이 해당 계측에 자동 포함됩니다. 아울러, Spring Boot(버전 3)와 같은 프레임워크는 대개 Tomcat, Jetty 또는 Netty 등 지원되는 임베디드 애플리케이션 서버를 사용하기 때문에 기본 작동합니다.
...
- 사용 중인 프레임워크가 하단에서 지원되지 않는 경우, **Code Security**는 SQL Injection 취약점을 탐지하지 않지만 [여기][3]에 명시된 나머지 취약점 유형은 계속 탐지합니다.
