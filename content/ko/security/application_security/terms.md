...
- link: /security/application_security/how-it-works
  tag: 설명서
  text: App and API Protection 작동 방식
...
Datadog App and API Protection는 위협을 모니터링하며, 코드 수준의 취약점을 악용하려는 애플리케이션 수준의 공격에 대한 보호 기능을 제공합니다. 런타임 코드 실행 컨텍스트, 트레이스, 오류 데이터, 사용자 속성을 활용합니다.

## 일반 App and API Protection 약관
...
Datadog 라이브러리
: _또는_ 트레이서, 추적 라이브러리
: 웹 애플리케이션에 내장된 프로그래밍 언어별 라이브러리입니다. Datadog App and API Protection는 라이브러리를 사용하여 모니터링하고 예측합니다. APM은 동일한 라이브러리를 사용하여 원격 텔레메트리용 코드를 계측합니다.
...
허용 목록(이전의 제외 필터)
: Datadog App and API Protection 라이브러리와 인앱 WAF 규칙을 통해 플래그가 지정된 보안 추적을 삭제하는 메커니즘입니다. 허용 목록은 요청이 Datadog로 수집되어 적용됩니다. 허용 목록은 오탐 및 수집 비용을 관리하는 데 도움이 됩니다.
: 앱의 [제외 필터][11]를 참조하세요.
...
원격 구성
: Agent 구성을 원격으로 업데이트할 수 있는 Datadog 플랫폼 메커니즘입니다. Datadog App and API Protection에서 앱 내 WAF 규칙을 업데이트하고 제품을 활성화하며 공격자를 차단하는 데 사용됩니다.
: [원격 구성 작동 방식][8]을 참조하세요.
...
의심스러운 공격자
: 플래그 지정된 IP의 전구체입니다. 의심스러운 IP는 의심 대상으로 분류되기 위한 공격 트래픽의 최소 임계값을 충족했지만 플래그 지정 임계값은 충족하지 못했습니다. 임계값은 사용자가 구성할 수 없습니다.
: [공격자 탐색기][17]를 참조하세요.

플래그가 지정된 공격자
: 대량의 공격 트래픽을 전송하는 IP입니다. 플래그 지정된 IP를 검토하고 차단하는 것이 좋습니다. 임계값은 사용자가 구성할 수 없습니다.
: [공격자 탐색기][17]를 참조하세요.
...
OWASP(Open Web App and API Protection Project)
: 웹 애플리케이션 보안을 강화하기 위해 여러 프로젝트를 진행하는 비영리 재단입니다. OWASP는 웹 애플리케이션의 가장 중요한 보안 위험에 대한 광범위한 합의안인 [OWASP Top 10][2]으로 가장 잘 알려져 있습니다.
