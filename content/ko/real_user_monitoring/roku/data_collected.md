...
RUM Roku SDK는 연관된 메트릭 및 속성이 있는 이벤트를 생성합니다. 메트릭은 이벤트와 관련된 측정에 사용할 수 있는 정량화 가능한 값입니다. 속성은 분석에서 메트릭 데이터를 분류(그룹화)하는 데 사용되는 정량화할 수 없는 값입니다.

모든 RUM 이벤트에는 [기본 속성](#default-attributes)이 있습니다. 예를 들어, 디바이스 유형(`device.type`), 이름(`usr.name`) 및 국가(`geo.country`)와 같은 사용자 정보입니다.

추가적으로 [주어진 이벤트 유형에 특정 메트릭 및 속성](#event-specific-metrics-and-attributes)이 있습니다. 예를 들어, 메트릭 `view.time_spent`은 "보기" 이벤트와 연결되고 속성 `resource.method`는 "리소스" 이벤트와 연결됩니다.

| 이벤트 유형 | 보존 | 설명                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 세션    | 30일   | 세션은 Roku 채널에서 실제 사용자 여정을 나타냅니다. 이 세션은 사용자가 채널을 실행할 때 시작되며, 사용자가 활성 상태를 유지하는 한 세션은 활성 상태로 유지됩니다. 사용자 여정 중에 세션의 일부로 생성된 모든 RUM 이벤트는 동일한 `session.id` 속성을 공유합니다. **참고:** 세션은 15분 동안 활동이 없으면 재설정됩니다. 채널이 충돌하거나 Roku OS에 의해 종료되면 세션이 재설정됩니다. |
| 보기       | 30일   | 보기는 Roku 채널의 고유한 화면(또는 화면 세그먼트)을 나타냅니다. 보기는`startView` 함수를 호출할 때 시작되고 새로운 보기가 시작될 때 중지됩니다. 각 발생은 고유한 보기로 분류됩니다. 사용자가 보기를 사용하는 동안 RUM 이벤트 속성(오류, 리소스, 액션)이 고유한 `view.id`와 함께 보기에 첨부됩니다.                                                                               |
| 리소스   | 15일   | 리소스는 Roku 채널에서 퍼스트 파티 호스트, API 및 타사 공급자에 대한 네트워크 요청을 나타냅니다. 사용자 세션 중에 생성된 모든 요청은 고유한 `resource.id`와 함께 보기에 첨부됩니다.                                                                                                                                                                                                                        |
| Error      | 30일   | 오류는 생성된 보기에 첨부된 Roku 채널에서 발생하는 예외 또는 충돌을 나타냅니다.                                                                                                                                                                                                                                                                                                                               |
| 액션     | 30일   | 액션은 Roku 채널에서의 사용자 활동(예: 원격 클릭)을 나타냅니다. 각 액션은 생성되는 보기에 고유한 `action.id`와 함께 첨부됩니다.                                                                                                                                                                                                                                                               |
| 긴 작업  | 15일   | 지정된 기간 임계값을 초과하여 메인 스레드를 차단하는 애플리케이션의 모든 작업에 대해 긴 작업 이벤트가 생성됩니다.                                                                                                                                                                                                                                                                                                   |

다음 다이어그램은 RUM 이벤트 계층을 보여줍니다:

{{< img src="real_user_monitoring/data_collected/event-hierarchy.png" alt="RUM 이벤트 계층 구조" style="width:50%;" >}}

## 기본 속성

RUM은 모든 이벤트에 대한 공통 속성과 아래 나열된 각 이벤트에 대한 특정 속성을 자동으로 수집합니다. 또한 [추가 이벤트][2]를 추적하거나 애플리케이션 모니터링 및 비즈니스 분석 요구 사항에 맞는 기본 이벤트에 [커스텀 속성을 추가][2]하여 사용자 세션 데이터를 강화시킬 수 있습니다.

### 공통 핵심 속성

| 속성 이름   | 유형    | 설명                                                                         |
| ---------------- | ------- | ----------------------------------------------------------------------------------- |
| `date`           | 정수 | 에포크에서 이벤트 시작 시간(밀리초).                                      |
| `type`           | 스트링  | 이벤트 유형 (예: `view` 또는 `resource`).                          |
| `service`        | 스트링  | 이 애플리케이션의 [통합되지 않은 서비스 이름][4]은 사용자 세션을 상관시키는 데 사용됩니다. |
...
**참고:** 지리적 위치 속성 수집을 중지하려면 [애플리케이션 상세정보][9]에서 설정을 변경하세요.
...
[사용자 정보 추적][5]을 전역적으로 활성화하여 사용자 속성을 수집하고 모든 RUM 이벤트에 적용할 수 있습니다.
...
| `session.is_active`         | 부울 | 세션이 현재 활성 상태인지 여부를 나타냅니다. 사용자가 애플리케이션에서 벗어나 탐색하거나 브라우저 창을 닫으면 세션이 종료되며, 4시간 동안 활동 후 또는 15분 동안 활동하지 않을 시 만료됩니다. |
...
프론트엔드 오류는 실제 사용자 모니터링(RUM)을 사용하여 수집됩니다. 오류 메시지와 스택 트레이스는 사용 가능한 경우 포함됩니다.

| 속성        | 유형   | 설명                                                                       |
| ---------------- | ------ | --------------------------------------------------------------------------------- |
| `error.source`   | 스트링 | 오류가 발생하는 곳 (예: `webview`, `logger`, 또는 `network`). |
| `error.type`     | 스트링 | 오류 유형(또는 경우에 따라 오류 코드).                                     |
| `error.message`  | 스트링 | 이벤트를 설명하는 간결하고 사람이 읽을 수 있는 한 줄 메시지.                  |
| `error.stack`    | 스트링 | 스택 트레이스 또는 오류에 대한 보완 정보.                     |
| `error.issue_id` | 스트링 | 스택 트레이스 또는 오류에 대한 보완 정보.                     |
...
| `error.resource.provider.name`   | 스트링 | 리소스 공급자 이름. 기본값은 `unknown`.                                     |
...
| `action.target.name` | 스트링 | 사용자가 상호 작용한 요소. 자동으로 수집된 액션에만 해당합니다. |

## 데이터 저장 공간

데이터가 Datadog에 업로드되기 전에 채널의 [캐시 디렉토리][6]에 일반 텍스트로 저장되므로 다른 애플리케이션에서 이 데이터를 읽을 수 없습니다. OS는 언제든지 데이터를 제거할 수 있으며, 이로 인해 드물게 데이터가 손실될 수 있습니다.

## 참고 자료

{{< partial name="whats-next/whats-next.html" >}}

[2]: /ko/real_user_monitoring/roku/advanced_configuration/#enrich-user-sessions
[4]: /ko/getting_started/tagging/unified_service_tagging/
[5]: /ko/real_user_monitoring/roku/advanced_configuration/#identifying-your-users
