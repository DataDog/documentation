...
| 텍스트 마스킹 전략 | 설명 | 예시 |
|-----------------------|-------------|---------|
| 마스킹 없음 | 텍스트가 세션 재생에 표시됨 | `"Hello world"` → `"Hello world"` |
| 공간 유지 마스킹 | 눈에 보이는 각 문자가 소문자 "x"로 바뀜 | `"Hello world"` → `"xxxxx xxxxx"` |
| 고정 길이 마스킹 | 전체 텍스트 필드가 세 개의 별표(***)로 구성된 정수로 바뀜 | `"Hello world"` → `"***"`
...
|------|-------------|------------|-------------------|
| [민감한 텍스트](#sensitive-text) | 고정 길이 마스킹 | 고정 길이 마스킹 | 고정 길이 마스킹 |
| [입력 및 옵션 텍스트](#input-and-option-text) | 마스킹 없음 | 고정 길이 마스킹 | 고정 길이 마스킹 |
| [정적 텍스트](#static-text) | 마스킹 없음 | 공간 유지 마스킹 | 마스킹 없음 |
| [힌트 텍스트](#hint-text) | 마스킹 없음 | 고정 길이 마스킹 | 마스킹 없음 |
...
**텍스트 속성**
- 날짜 선택기에서 선택한 날짜를 렌더링하는 레이블의 색상 (iOS)
