...
| `view.loading_time`             | 숫자 (ns) | 페이지가 준비될 때까지의 시간이며 현재 네트워크 요청이나 DOM 변이가 발생하지 않습니다. 자세한 내용은 [페이지 성능 모니터링][10]을 참조하세요.                                                                          |
| `view.first_contentful_paint`   | 숫자 (ns) | 브라우저가 텍스트, 이미지(배경 이미지 포함), 흰색이 아닌 캔버스 또는 SVG를 처음 렌더링하는 시간입니다. 브라우저 렌더링에 대한 자세한 내용은 [w3c 정의][11]를 참조하세요.                                         |
| `view.dom_interactive`          | 숫자 (ns) | 파서(Parser)가 메인 문서에 대한 작업을 완료하는 순간입니다. 자세한 내용은 [MDN 설명서][12]를 참조하세요.                                                                                                        |
| `view.dom_content_loaded`       | 숫자 (ns) | 최초의 HTML 문서가 비렌더링 블로킹 스타일 시트, 이미지, 서브프레임의 로드 완료를 기다리지 않고 완전하게 로드되어 해석될 때 발생하는 이벤트입니다. 자세한 내용은 [MDN 설명서][13]를 참조하세요. |
| `view.dom_complete`             | 숫자 (ns) | 페이지와 모든 하위 리소스가 준비되었습니다. 사용자 측에서는 로딩 스피너가 회전을 멈췄습니다. 자세한 내용은 [MDN 설명서][14]를 참조하세요.                                                                     |
| `view.load_event`               | 숫자 (ns) | 페이지가 완전히 로드될 때 발생하는 이벤트입니다. 일반적으로 추가 애플리케이션 로직을 위한 트리거입니다. 자세한 내용은 [MDN 설명서][15]를 참조하세요.                                                                            |
...
Datadog은 페이지 로딩에 필요한 시간을 계산하는 고유한 성능 메트릭인 `loading_time`을 제공합니다. 이 메트릭은 `initial_load` 및 `route_change` 탐색 모두에서 작동합니다.

### 로딩 시간 계산 방법

최신 웹 애플리케이션을 고려하기 위해 로딩 시간은 네트워크 요청과 DOM 변형을 감시합니다.

- **Initial Load**: 로딩 시간은 _더 긴 값_과 같습니다:

  - `navigationStart`와 `loadEventEnd`의 차이, 또는
  - `navigationStart`와 페이지에 활동이 없는 첫 시간 사이의 차이. 자세한 내용은 [페이지 활동 계산 방법](#how-page-activity-is-calculated)을 참조하세요.

- **SPA Route Change**: 로딩 시간은 URL 변경과 페이지에 활동이 없는 첫 시간 사이의 차이와 같습니다. 자세한 내용은 [페이지 활동 계산 방법](#how-page-activity-is-calculated)을 참조하세요.

### 페이지 활동 계산 방법

RUM Browser SDK는 페이지 활동을 추적하여 인터페이스가 다시 안정될 때까지의 시간을 추정합니다. 다음과 같은 경우 페이지에 활동이 있는 것으로 간주됩니다:
...
마지막 요청 또는 DOM 변형 이후 100ms라는 기준은 다음과 같은 시나리오에서 활동을 정확하게 판단하지 못할 수 있습니다:
...
## 자체 성능 타이밍 추가
RUM의 기본 성능 타이밍 외에도 애플리케이션이 시간을 소비하는 위치를 보다 유연하게 측정할 수 있습니다. `addTiming` API는 성능 타이밍을 추가할 수 있는 간단한 방법을 제공합니다.
...
타이밍이 전송되면 타이밍은 `@view.custom_timings.<timing_name>`과 같이 액세스할 수 있습니다. 예를 들어, `@view.custom_timings.first_scroll`입니다. RUM 탐색기 또는 대시보드에서 시각화를 만들기 전에 [측정값을 만들기][18]를 수행해야 합니다.

단일 페이지 애플리케이션의 경우 `addTiming` API는 현재 RUM 보기의 시작을 기준으로 타이밍을 발행합니다. 예를 들어 사용자가 애플리케이션에 도착한 후(초기 로드) 5초 후에 다른 페이지로 이동하고(경로 변경) 8초 후에 최종적으로 `addTiming`를 트리거하는 경우, 타이밍은 `8-5 = 3`초와 같습니다.

비동기 설정을 사용하는 경우 두 번째 파라미터로 고유한 타이밍(현재 RUM 보기의 시작을 기준으로 한 밀리초 수 또는 UNIX 에포크 타임스탬프)을 제공할 수 있습니다.
