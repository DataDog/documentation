...
Datadog는 자동으로 JSON 형식의 로그를 [파싱][1]합니다. 그런 다음 처리 파이프라인을 통해 모든 로그(원시 및 JSON)를 전송하여 부가가치를 창출할 수 있습니다. 파이프라인은 다양한 형식의 로그를 가져와 Datadog에서 공통 형식으로 변환할 수 있습니다. 로그 파이프라인과 처리 전략을 구현하면 조직을 위한 [속성 명명 규칙][2]을 사용할 수 있어 도움ㅇ ㅣ됩니다.
...
6. (선택 사항) 파이프라인에 태그 및 설명을 추가합니다. 설명과 태그를 사용하여 파이프라인의 목적과 해당 파이프라인을 소유한 팀을 명시할 수 있습니다.
7. **만들기**를 누릅니다.
