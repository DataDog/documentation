...
title: Amazon Data Firehose로 Amazon EKS Fargate Log 전송하기
---
...
{{< img src="logs/guide/aws-eks-fargate-logs-with-kinesis-data-firehose/log_streaming_diagram.png" alt="Fargate EKS 클러스터가 Fluent Bit 로그 라우터를 통해 Amazon Data Firehose와 AWS 내의 S3 백업 버킷으로 컨테이너 로그를 전송한 후 다시 Datadog으로 전달하는 것을 보여주는 로그 플로 다이어그램" responsive="true">}}
...
1. 로그와 로그 전송 실패 S3 백업을 Datadog에 전달하는 [Amazon Data Firehose 전달 스트림을 생성](#create-an-amazon-data-firehose-delivery-stream)합니다.
...
[Datadog Amazon Firehose 대상으로 AWS 서비스 로그 전송하기][4] 가이드를 참조하여 Amazon Data Firehose 전송 스트림을 설정하세요.
**참고**: **Source**를 `Direct PUT`로 설정합니다.
...
<div class="alert alert-info">보다 성능이 뛰어난 신규 <a href="https://docs.fluentbit.io/manual/pipeline/outputs/firehose">Kinesis Firehose 플러그인</a>의 경우 플러그인 이름을 <code>amazon_data_firehose</code> 대신 <code>kinesis_firehose</code>로 사용합니다.</div>
...
4. IAM 정책을 생성하고 이를 포드 실행 역할에 연결하여 AWS Fargate에서 실행되는 로그 라우터가 Amazon Data Firehose를 작성할 수 있도록 합니다. 아래 예를 사용하여 **Resource** 필드의 ARN을 전송 스트림의 ARN으로 바꾸고 지역 및 계정 ID를 지정할 수 있습니다.
...
4. 로그가 Datadog에 있는지 확인합니다. [Datadog log 탐색기][10]에서 `@aws.firehose.arn:"<ARN>"`를 검색하고 `<ARN>`를 Amazon Data Firehose ARN으로 교체하여 Amazon Data Firehose의 로그를 필터링합니다.
...
[4]: /ko/logs/guide/send-aws-services-logs-with-the-datadog-kinesis-firehose-destination/?tab=amazondatafirehosedeliverystream#setup
