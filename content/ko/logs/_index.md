...
---
{{< learning-center-callout header="지원 웨비나 세션 참여" hide_image="true" btn_title="Sign Up" btn_url="https://www.datadoghq.com/technical-enablement/sessions/?tags.topics-0=Logs">}}
  초급 또는 중급 지원 세션에 참여하여 Datadog Log Management가 로그, 메트릭 및 트레이스를 단일 뷰로 통합함으로써 로그 데이터를 분석하기 위한 풍부한 컨텍스트를 제공하는 방법을 알아보세요.
{{< /learning-center-callout >}}

## 개요 {#overview}

시스템 운영의 중요한 부분을 로깅하는 작업은 인프라 상태를 양호하게 유지하는 데 매우 중요합니다. 최신 인프라는 1분당 수천 개의 로그 이벤트를 생성할 수 있습니다. 이런 상황에서는 사용자가 어느 로그를 로그 관리 솔루션으로 보내고, 어느 로그를 보관할지 선택해야 합니다. 그러나 로그를 보내기 전에 필터링하면 커버리지에 빈틈이 생기거나, 귀중한 데이터를 우발적으로 제거하는 사태가 발생할 수 있습니다.
...
Logging without Limits\*는 [Log Explorer][1]에서 간소화된 문제 해결 경험을 제공합니다. 따라서 사용자와 사용자 팀원들이 인프라 문제를 신속하게 평가하고 해결할 수 있습니다. 또한 직관적인 보관 기능을 제공하여 감사 및 평가 시 보안팀과 IT 팀을 지원합니다. Logging without Limits*는 [Datadog Cloud SIEM][2]의 기반이기도 합니다. 이 기능은 사용자가 로그를 인덱싱할 필요 없이 사용자 환경의 보안 위협을 탐지합니다.
...
Datadog Log Management로 호스트, 컨테이너, 클라우드 공급자 및 기타 소스에서 [로그 수집][4]을 시작해 보세요.

## 구성 {#configure}

{{< img src="logs/lwl_marketecture_20231030.png" alt="한 곳에서 모든 로그 구성" >}}

로그가 수집되고 나면 [Log Configuration 옵션][5]을 사용해 파이프라인 및 프로세서로 모든 로그를 처리 및 보강하거나, 인덱스로 로그 관리 예산을 제어하거나, 수집된 로그에서 메트릭을 생성하거나, 스토리지 최적화 아카이브에서 로그를 관리하세요.

## 연결 {#connect}

{{< img src="/logs/connect.png" alt="로그와 메트릭 또는 트레이스의 상관관계 정립" style="width:80%;">}}

로그를 메트릭 및 트레이스에 연결하여 가시성의 핵심 요소를 활용하세요.

- [로그와 트레이스를 연결][6]하여 애플리케이션 가시성을 확보하세요.
...
- [검색][8]: 로그 전체를 검색합니다.
- [Live Tail][9]: 모든 환경에서 수집한 로그를 실시간으로 확인합니다.
- [분석][10]: 인덱싱된 로그에 대해 로그 분석을 수행합니다.
- [패턴][11]: 인덱싱된 로그를 함께 클러스터링하여 로그 패턴을 확인합니다.
- [Saved Views][12]: 저장된 뷰를 사용하여 Log Explorer를 자동으로 구성합니다.


{{< learning-center-callout header="학습 센터에서 Log Management 소개 읽기" btn_title="Enroll Now" btn_url="https://learn.datadoghq.com/courses/intro-to-log-management">}}
  실제 클라우드 컴퓨팅 용량과 Datadog 평가판 계정을 사용하여 무료로 학습하세요. 지금 등록해 로그 수집, 쿼리, 분석, 메트릭, 모니터링, 처리, 스토리지 및 액세스 제어에 대해 자세히 알아보세요.
{{< /learning-center-callout >}}
