...
이 옵션을 `DEBUG`로 설정하면 작업자의 내부 프로세스 (HTTP 요청 및 수신된 응답 포함)에 대한 자세한 정보를 얻을 수 있습니다. Datadog 지원팀에 트러블슈팅에 도움이 되는 `DEBUG` 로그를 요청할 수 있습니다. 이러한 로그는 로그 탐색기 및 [진단 로그](#investigate-diagnostic-logs)에도 나타납니다.
