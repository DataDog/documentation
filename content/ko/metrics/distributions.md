...
* **백분위수 에그리게이터 계산**: 분포는 전체적으로 정확한 백분위수 집계(p50, p75, p90, p95, p99 또는 소수점 두 개까지 포함한 선택한 모든 백분위수)를 계산할 수 있도록호스트에서 집계되지 않은 원시 데이터를 나타내는 DDSketch 데이터 구조로 저장됩니다. 백분위수 에그리게이터를 활성화하면 다음과 같은 고급 쿼리 기능을 잠금 해제할 수 있습니다:
  ...
  * **타임프레임 전반의 표준 편차**:
     ...
     _"지난 한 달 동안 애플리케이션의 CPU 사용량 표준편차(stddev) 값은 어떻게 되나요?"_
...
**참고:** 분포 메트릭 데이터는 기타 유형과 다르게 저장되므로 `distribution`에 사용한 메트릭 이름을 다른 메트릭 유형에 사용하지 마시기 바랍니다.
...
그러나 메트릭 요약 페이지에서 분포에 있는 모든 쿼리 가능 태그에 대해 정확한 전역 백분위수 집계 계산과 같은 고급 쿼리 기능을 사용하도록 설정할 수 있습니다. `p50`, `p75`, `p90`, `p95`, `p99` 또는 사용자가 정의한 백분위수(99.99와 같이 최대 소수점 두 자리까지)에 대한 집계가 제공됩니다. 고급 쿼리를 활성화하면 임계값 쿼리 및 표준 편차 잠금도 해제됩니다.

{{< img src="metrics/distributions/metric_detail_enable_percentiles.mp4" alt="메트릭 세부 패널의 고급 섹션에서 설정을 클릭하여 고급 백분위수 및 임계값 쿼리 기능을 활성화하는 사용자" video=true width=80% >}}
...
1. [메트릭 요약 페이지][4]로 이동하여 **메트릭 설정** 드롭다운을 클릭합니다.
1. **백분위수 활성화**를 선택합니다.
1. 메트릭 네임스페이스 접두사를 지정하여 해당 네임스페이스와 일치하는 메트릭을 모두 선택합니다.
1. (옵션) 네임스페이스에서 모든 메트릭에 대한 백분위수를 비활성화하려면 **백분위수 집계** 토글을 클릭합니다.

{{< img src="metrics/summary/percentile_aggregations_toggle.png" alt="백분위수 집계 관리 토글" style="width:100%;" >}}
