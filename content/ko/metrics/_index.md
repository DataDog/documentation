...
{{< whatsnext desc="This section includes the following topics:">}}
    ...
    {{< nextlink href="/metrics/advanced-filtering" >}}<u>고급 필터링</u> - 데이터를 필터링하여 반환되는 메트릭 범위를 좁힙니다.{{< /nextlink >}}
    {{< nextlink href="/metrics/summary" >}}<u>메트릭 요약</u> - 활성 보고 Datadog 메트릭을 이해합니다.{{< /nextlink >}}
    {{< nextlink href="metrics/distributions/" >}}<u>배포 메트릭</u> - 배포 메트릭과 전 세계적으로 정확한 백분위수에 대해 알아봅니다.{{< /nextlink >}}
    {{< nextlink href="metrics/metrics-without-limits/" >}}<u>제한없는 메트릭 수집TM</u> - 제한없는 메트릭 수집TM을 사용해 태그 및 집계 설정으로 커스텀 메트릭 볼륨을 제어하는 방법을 알아봅니다.{{< /nextlink >}}
{{< /whatsnext >}}
...
Datadog는 다양한 시각화 옵션을 제공해 사용자가 메트릭을 손쉽게 그래프화하고 표시할 수 있도록 돕습니다. 메트릭 사용자가 유용하다고 생각하는 두 가지 시각화 옵션은 다음과 같습니다.

메트릭 쿼리는 두 개의 동일한 평가 단계로 구성됩니다. 바로 1) 시간 집계와 2) 공간 집계입니다. [메트릭 쿼리 분석][6]에서 자세한 정보를 참조하세요.

{{< whatsnext desc="메트릭 사용자가 유용하다고 생각하는 시각화 옵션:">}}
    {{< nextlink href="dashboards/widgets/query_value/" >}}<u>쿼리 값 위젯</u> - 해당 두 단계의 결과를 단일 값으로 줄여줍니다.{{< /nextlink >}}
    {{< nextlink href="/dashboards/widgets/top_list/" >}}<u>상위 목록</u> - 그룹별 단일 값을 반환합니다.{{< /nextlink >}}
{{< /whatsnext >}}
...
다음으로 시간을 사용해 데이터를 세분화합니다. 예를 들어 매 시간(3600초) 동안 하나의 데이터 요소가 존재하도록 정의할 수 있습니다. 또한, 각 시간 범위마다 데이터를 집계할 방법을 선택합니다. 기본적으로 _평균(avg)_이 적용됩니다. 하지만 다른 옵션으로 _합계(sum)_, _최소(min)_, _최대(max)_ 및 _개수(count)_가 제공됩니다. 최대를 적용하려는 경우 `.rollup(max, 60)`을(를) 사용할 수 있습니다.

#### 공간 집계 설정

Datadog에서 "공간"이란 각기 다른 호스트와 태그에서 메트릭이 배포되는 방법을 가리킵니다. 공간에는 제어할 수 있는 두 가지 요소가 있습니다. 애그리게이터(aggregator)와 그룹화입니다.
...
Datadog는 다양한 메트릭 유형을 통해 고유한 사용 사례를 지원합니다. 개수, 게이지, 비율, 히스토그램, 배포가 그것입니다. 메트릭 유형은 앱에서 메트릭으로 활용할 수 있는 그래프와 함수를 결정합니다.
...
**_배포_**는 히스토그램과 비슷하지만 환경 내 모든 호스트에서 특정 시간 간격 동안 제출된 값을 요약합니다. 또한 다수의 백분위 수를 보고하도록 선택할 수 있습니다(p50, p75, p90, p95 및 p99). [배포 설명서][19]에서 이 강력한 기능에 대해 자세히 알아볼 수 있습니다.
...
{{< whatsnext desc="메트릭을 계속하려면 체크아웃하세요.">}}
    {{< nextlink href="/metrics/advanced-filtering" >}}<u>고급 필터링</u> - 데이터를 필터링하여 반환되는 메트릭 범위를 좁힙니다.{{< /nextlink >}}
    {{< nextlink href="/metrics/distributions" >}}<u>배포 메트릭</u> - 전체 데이터세트 전반에서 전 세계 백분위수를 계산합니다.{{< /nextlink >}}
    {{< nextlink href="metrics/metrics-without-limits/" >}}<u>제한없는 메트릭 수집TM</u> - 제한없는 메트릭 수집TM을 사용해 태그와 집계를 설정하여 커스텀 메트릭 볼륨을 제어하는 방법을 알아봅니다.{{< /nextlink >}}
{{< /whatsnext >}}
...
[21]: /ko/account_management/billing/usage_details/
