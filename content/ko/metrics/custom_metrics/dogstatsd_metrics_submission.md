...
호스트 태그는 메트릭을 집계하는 Datadog Agent에 의해 자동으로 할당됩니다. Agent 호스트 이름과 일치하지 않는 호스트 태그와 함께 제출된 메트릭은 원래 호스트에 대한 레퍼런스를 잃습니다. 제출된 호스트 태그는 Agent에서 수집하거나 설정한 모든 호스트 이름을 재정의합니다.
