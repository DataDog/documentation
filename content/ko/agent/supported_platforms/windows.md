...
description: Windows 플랫폼의 Datadog Agent 기본 기능입니다.
further_reading:
- link: /logs/
  tag: Documentation
  text: 로그 수집
...
- link: /agent/guide/windows-agent-ddagent-user
  tag: Documentation
  text: Datadog Windows Agent User에 대해 자세히 알아보기
platform: Windows
title: Windows
---
## 개요 {#overview}

이 페이지에서는 Windows용 Datadog Agent의 기본 기능에 대해 설명합니다. 아직 Agent를 설치하지 않은 경우 아래 설치 지침을 확인하거나 [앱 지침을 따르세요][1].

지원되는 Windows 버전의 전체 목록은 [지원되는 플랫폼][15]을 참조하세요.

## 설치 {#installation}

Windows 호스트에 Datadog Agent를 설치하려면 [Fleet Automation 내의 인앱 흐름][16]을 따른 다음 설치 명령을 복사하고 실행하세요. Datadog Agent는 `ddagentuser`로 실행됩니다. 자세한 내용은 [Datadog Windows Agent User][17] 설명서를 참조하세요.
...
### Agent Manager GUI로 설치 {#install-with-the-agent-manager-gui}
...
2. `datadog-agent-7-latest.amd64.msi`를 열어 설치 프로그램을 실행합니다. 메시지가 표시되면 관리자 자격 증명을 입력합니다.
3. 메시지에 따라 라이선스 계약에 동의하고 [Datadog API 키][500]를 입력합니다.

설치가 완료되면 Datadog Agent Manager를 시작할 수 있는 옵션이 제공됩니다.


#### 설치 구성 옵션 {#installation-configuration-options}

Windows에 Agent를 설치할 때 다음 각 구성 옵션을 명령줄에 속성으로 추가할 수 있습니다. 추가 Agent 구성 옵션은 [추가 Agent 구성 옵션](#more-agent-configuration-options)을 참조하세요.


| 변수                                    | 유형    | 설명                                                                                                                                                                                                                         |
|----------------------------                 |---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `APIKEY`                                    | 문자열  | 구성 파일에 Datadog API 키를 추가합니다.                                                                                                                                                                                 |
...
| `DDAGENTUSER_NAME`                          | 문자열  | Agent 설치_(v6.11.0 이상)_ 중에 사용되는 기본 `ddagentuser` 사용자 이름을 재정의합니다. [Datadog Windows Agent User에 대해 자세히 알아보세요][3].                                                                                      |
| `DDAGENTUSER_PASSWORD`                      | 문자열  | Agent 설치_(v6.11.0 이상)_ 중에 `ddagentuser` 사용자에 대해 생성된 암호화된 보안 비밀번호를 재정의합니다. 도메인 서버 설치를 위해 반드시 제공되어야 합니다. [Datadog Windows Agent User에 대해 자세히 알아보세요][3].  |
| `APPLICATIONDATADIRECTORY`                  | 경로    | 구성 파일 디렉터리 트리에 사용할 디렉터리를 재정의합니다. 최초 설치 시에만 제공될 수 있으며, 업그레이드에는 유효하지 않습니다. 기본값은 `C:\ProgramData\Datadog`입니다. _(v6.11.0 이상)_                                           |
| `PROJECTLOCATION`                           | 경로    | 바이너리 파일 디렉터리 트리에 사용할 디렉터리를 재정의합니다. 최초 설치 시에만 제공될 수 있으며, 업그레이드에는 유효하지 않습니다. 기본값은 `%ProgramFiles%\Datadog\Datadog Agent`입니다. _(v6.11.0 이상)_<br><br>기본 디렉터리를 재정의하려는 경우, Datadog 파일의 `Datadog` 하위 디렉터리를 지정해야 합니다.                                    |

**참고**

- `/qn` 옵션은 자동 설치를 실행합니다. GUI 프롬프트를 보려면 이를 제거하세요.
...
- 유효한 `datadog.yaml`이 발견되면 해당 파일이 지정된 모든 명령줄 옵션보다 우선합니다.

#### 추가 Agent 구성 옵션 {#more-agent-configuration-options}

Windows에 Agent를 설치할 때 다음 각 구성 옵션을 명령줄에 속성으로 추가할 수 있습니다.

**참고**: 유효한 `datadog.yaml`이 발견되면 해당 파일이 지정된 모든 명령줄 옵션보다 우선합니다.
...
| `HOSTNAME_FQDN_ENABLED`                     | 문자열  | Agent 호스트 이름에 대한 FQDN 사용을 활성화(`"true"`)하거나 비활성화(`"false"`)합니다. 이는 Agent 구성 파일에서 `hostname_fqdn`을 설정하는 것과 동일합니다. 호스트 이름에 대한 FQDN 사용은 기본적으로 비활성화되어 있습니다. _(v6.20.0 이상)_ |
...
| `PROXY_PORT`                                | 숫자  | (프록시를 사용하는 경우) 프록시 포트를 설정합니다. [Datadog Agent를 프록시와 함께 사용하는 방법에 대해 자세히 알아보세요][4].                                                                                                                                 |
...
| `EC2_USE_WINDOWS_PREFIX_DETECTION`          | 부울 | EC2의 Windows 호스트에는 EC2 인스턴스 ID를 사용합니다. _(v7.28.0 이상)_                                            |

#### 설치 로그 파일 {#installation-log-files}

설치 로그 파일을 구성하려면 `/log <FILENAME>` msiexec 옵션을 설정하세요. 이 옵션을 설정하지 않으면 msiexec는 기본적으로 로그를 `%TEMP%\MSI*.LOG`에 작성합니다.


## 구성 {#configuration}

주요 Agent 구성 파일은
`C:\ProgramData\Datadog\datadog.yaml`에 있습니다. 이 파일은 API 키, 선택한 Datadog 사이트, 프록시 파라미터, 호스트 태그 및 로그 레벨과 같은 호스트 전체 설정에 사용됩니다.

동일한 디렉터리에 `datadog.yaml.example` 파일도 있습니다. 이 파일은 사용 가능한 모든 구성 옵션이 포함된 완전 주석형 참조 파일로, 참조 및 특정 설정 복사에 유용합니다.


통합용 구성 파일은
`C:\ProgramData\Datadog\conf.d\`에 있습니다. 대체 레거시 위치인 `C:\Documents and Settings\All Users\Application Data\Datadog\conf.d\`가 있을 수도 있습니다.

각 통합에는 `<INTEGRATION>.d\`라는 하위 디렉터리가 있으며, 여기에는 다음이 포함됩니다.
- `conf.yaml`: 통합에 대한 활성 설정
* `conf.yaml.example`: 지원되는 구성 키를 보여주는 샘플 파일

구성을 변경할 때는 변경 사항이 적용되도록 Agent를 다시 시작해야 합니다.

[Datadog Agent Manager GUI][6]를 사용하여 검사를 활성화, 비활성화 및 구성할 수 있습니다. 변경 사항이 적용되려면 Agent를 다시 시작해야 합니다.

**참고**: `ProgramData`는 숨겨진 폴더입니다.

## Agent 명령 {#agent-commands}
...
* `<PATH_TO_AGENT.EXE> <COMMAND>` 구문을 사용하여 **승격된(관리자로 실행)** 명령줄(PowerShell 또는 명령 프롬프트)에서 명령을 실행할 수 있습니다.
* 명령줄 옵션은 다음과 같습니다.
...
| restart-service | 서비스 제어 관리자 내에서 Agent를 다시 시작합니다.                           |
| run             | Agent를 시작합니다.                                                                |
| start           | Agent를 시작합니다. 더 이상 사용되지 않지만 허용됩니다. 대안으로 `run`을 사용하세요. |
| start-service   | 서비스 제어 관리자 내에서 Agent를 시작합니다.                             |
| status          | 현재 상태를 출력합니다.                                                        |
| stopservice     | 서비스 제어 관리자 내에서 Agent를 중지합니다.                              |
...
### 프로그램 추가 또는 제거 {#add-or-remove-programs}

1. **Ctrl**+**Esc**를 누르거나 Windows 키를 사용하여 Windows 검색을 실행합니다.
1. `add`를 검색하고 **프로그램 추가/제거**를 클릭합니다.
...
문제 해결 단계를 보려면 [Agent 문제 해결 설명서][18]를 참조하세요.


### Agent 상태 및 정보 {#agent-status-and-information}

Agent가 실행 중인지 확인하려면 Services 패널의 `DatadogAgent` 서비스가 *Started*로 나열되어 있는지 확인합니다. *Datadog Metrics Agent*(`agent.exe`)라는 프로세스도 Task Manager에 있어야 합니다.

Agent 상태에 대한 자세한 정보를 얻으려면 Datadog Agent Manager를 시작하세요.

* Datadog Agent 시스템 트레이 아이콘을 오른쪽 버튼으로 클릭하고 Configure를 클릭합니다. 또는
* **승격된(관리자로 실행)** 명령줄에서 `launch-gui` 명령을 실행합니다.
...
###  Windows 서비스 모니터링 {#monitoring-a-windows-service}

대상 호스트에서 Datadog Agent Manager를 실행하고 목록에서 'Windows Service' 통합을 선택합니다. 즉시 사용 가능한 예시가 있으나 이 예시에서는 DHCP를 사용합니다.
...
자체 서비스를 추가할 때는 표시된 형식을 정확하게 따라야 합니다. 형식이 올바르지 않으면 통합이 실패합니다. **참고**: 서비스 이름의 특수 문자는 이스케이프되어야 합니다. 예를 들어, 이름 `MSSQL$BILLING`은 `MSSQL\$BILLING`으로 추가할 수 있습니다.
...
Services의 경우 Datadog은 메트릭을 추적하지 않고 가용성만 추적합니다 (메트릭의 경우 [Process](#monitoring-windows-processes) 또는 [WMI][7] 통합 사용). 모니터를 설정하려면 [Integration monitor type][8]을 선택한 다음 **Windows Service**를 검색합니다. *Integration Status -> Pick Monitor Scope*에서 모니터링할 서비스를 선택합니다.

### Windows용 시스템 로드 모니터링 {#monitoring-system-load-for-windows}
...
### Windows 프로세스 모니터링 {#monitoring-windows-processes}

[Live Process Monitoring][9]을 통해 Windows 프로세스를 모니터링할 수 있습니다. Windows에서 이 기능을 활성화하려면 다음 파라미터를 true로 설정하여 [Agent 기본 구성 파일][10]을 수정하세요.
