...
한 가지 주의할 점은 Google CloudSQL은 [performance_schemas][5]를 지원하지 않으므로 Datadog Agent 사용자에게는 `GRANT SELECT ON performance_schema.*`를 사용할 수 없다는 것입니다. 이 때문에 MySQL 검사의 추가/선택적 메트릭 중 두 개는 Google CloudSQL 인스턴스에 사용할 수 없습니다. 그렇지 않은 경우, 통합은 로컬로 호스팅된 MySQL 인스턴스에서와 마찬가지로 작동해야 합니다.
