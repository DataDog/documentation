...
{{< whatsnext desc="클라우드 인프라스트럭처 가이드:" >}}
    {{< nextlink href="agent/guide/can-i-set-up-the-dd-agent-mysql-check-on-my-google-cloudsql/" >}}Google CloudSQL에서 dd-agent mysql 검사를 설정할 수 있나요?{{< /nextlink >}}
    {{< nextlink href="/agent/guide/heroku-ruby" >}}Datadog으로 Heroku에서 Ruby on Rails 애플리케이션 계측{{< /nextlink >}}
    {{< nextlink href="agent/guide/heroku-troubleshooting/" >}}Datadog-Heroku 빌드팩 트러블슈팅{{< /nextlink >}}
    {{< nextlink href="agent/guide/private-link" >}}AWS PrivateLink를 통해 텔레메트리를 Datadog에 안전하게 전달{{< /nextlink >}}
    {{< nextlink href="agent/guide/azure-private-link" >}}Azure Private Link를 통해 Datadog에 연결{{< /nextlink >}}
    {{< nextlink href="agent/guide/why-should-i-install-the-agent-on-my-cloud-instances" >}}왜 클라우드 인스턴스에 Datadog Agent를 설치해야 하나요?{{< /nextlink >}}
{{< /whatsnext >}}

{{< whatsnext desc="통합 가이드:" >}}
    {{< nextlink href="agent/guide/use-community-integrations" >}}커뮤니티 통합 사용{{< /nextlink >}}
    {{< nextlink href="agent/guide/integration-management" >}}통합 관리{{< /nextlink >}}
{{< /whatsnext >}}

{{< whatsnext desc="Agent 버전 관리 가이드:" >}}
    {{< nextlink href="agent/guide/agent-v6-python-3" >}}Python 버전 관리: Datadog Agent v6과 함께 Python 3 사용{{< /nextlink >}}
    {{< nextlink href="agent/guide/python-3" >}}Python 2에서 3으로 커스텀 검사 마이그레이션{{< /nextlink >}}
{{< /whatsnext >}}
