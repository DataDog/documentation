...
트래픽은 항상 Agent에서 Datadog으로 시작됩니다. Datadog에서 Agent로 다시 시작되는 세션은 없습니다.
</div>

모든 Agent 트래픽은 SSL을 통해 전송됩니다. Datadog 서비스 및 사이트에 따라 대상이 결정됩니다. [Datadog 사이트][11]에 따른 대상을 보려면 오른쪽의 `DATADOG SITE` 선택기를 클릭하세요.
...
이 동작은 버전 7.72.0 이상에서 구성의 <code>convert_dd_site_fqdn.enabled</code>를 <code>false</code>로 설정하거나 환경 변수 <code>DD_CONVERT_DD_SITE_FQDN_ENABLED=false</code>를 사용하여 비활성화할 수 있습니다.
</div>
...
예를 들어, Agent v7.31.0은 `7-31-0-app.agent.`에{{< region-param key="dd_site" code="true" >}}보고합니다. `*.agent.`를{{< region-param key="dd_site" code="true" >}} 방화벽의 포함 목록에 추가해야 합니다.<br>
...
예를 들어, Agent v7.31.0은 플레어 데이터를 `7-31-0-flare.agent.`로{{< region-param key="dd_site" code="true" >}}전송합니다. `*.agent.`를{{< region-param key="dd_site" code="true" >}} 방화벽의 포함 목록에 추가해야 합니다.<br>

### 고정 IP 주소 {#static-ip-addresses}

이 도메인은 모두 **CNAME** 레코드로, 고정 IP 주소 세트를 가리킵니다. 이 주소는 `https://ip-ranges.`에서 찾을 수 있습니다{{< region-param key="dd_site" code="true" >}}.
...
각 섹션에는 전용 엔드포인트가 있습니다. 예:

- `https://ip-ranges.{{< region-param key="dd_site" >}}/logs.json` - TCP를 통해 로그 데이터를 수신하는 데 사용되는 IP의 경우
- `https://ip-ranges.{{< region-param key="dd_site" >}}/apm.json` - APM 데이터를 수신하는 데 사용되는 IP의 경우

### 포함 {#inclusion}

포함 목록에 `ip-ranges`를 모두 추가합니다. 특정 순간에는 일부만 활성화되지만 정기적인 네트워크 운영 및 유지 관리로 인해 전체 세트 내에서 시간이 지남에 따라 변동이 발생합니다.
...
<br><br>
방화벽 규칙 또는 이와 유사한 네트워크 제한을 사용하여 애플리케이션 또는 신뢰할 수 있는 네트워크 소스에서만 Agent에 액세스할 수 있게 하세요. 악의적 공격자가 신뢰할 수 없는 액세스를 통해 Datadog 계정에 트레이스 및 메트릭을 기록하거나 구성 및 서비스에 대한 정보를 얻는 등 여러 가지 침입 행위를 할 수 있습니다.
</div>
...
| 로그 수집                                                                                                                                                           | {{< region-param key=web_integrations_port >}} | (지원 중단됨) TCP | TCP를 통해 로그를 작성합니다. <br>**참고**: TCP 로그 수집은 **지원되지 않습니다**. Datadog은 TCP를 사용할 때 **전달 또는 안정성을 보장하지 않으며**, 로그 데이터는 예고 없이 손실될 수 있습니다. 안정적인 수집을 위해 HTTP 수집 엔드포인트, 공식 Datadog Agent 또는 포워더 통합을 대신 사용하세요. 다른 연결 유형은 [로그 엔드포인트][21]를 참조하세요. |
| NTP                                                                                                                                                                      | 123                                            | UDP              | NTP(Network Time Protocol). [기본 NTP 대상][20]을 참조하세요.<br>NTP 문제 해결에 대한 정보는 [NTP 문제][19]를 참조하세요.                                                                |
...
| [DogStatsD][18]              | 8125 | UDP      | `dogstatsd_non_local_traffic`이 true로 설정된 경우를 제외하고 DogStatsD의 포트입니다. 이 포트는 IPv4 로컬 호스트(`127.0.0.1`)에서 사용 가능합니다. |
| go_expvar 서버(APM)       | 5012 | TCP      | 자세한 내용은 [go_expar 통합 설명서][15]를 참조하세요.                                                        |
| go_expvar 통합 서버 | 5000 | TCP      | 자세한 내용은 [go_expar 통합 설명서][15]를 참조하세요.                                                        |
...
기본 포트가 네트워크의 기존 서비스에서 이미 사용 중이어서 인바운드 포트를 변경해야 하는 경우 `datadog.yaml` 구성 파일을 수정하세요. 대부분의 포트는 파일의 **Advanced Configuration** 섹션에서 찾을 수 있습니다.
...
APM 수신기와 DogStatsD 포트는 각각 `datadog.yaml` 구성 파일의 **Trace Collection Configuration** 및 **DogStatsD Configuration** 섹션에 있습니다.
...
<div class="alert alert-danger">여기에서 DogStatsD 포트나 APM 수신기 포트 값을 변경하면 해당 포트의 APM 트레이싱 라이브러리 구성도 변경해야 합니다. 포트 구성과 관련된 정보는 <a href="/tracing/trace_collection/library_config/">내 언어에 맞는 라이브러리 구성 설명서</a>를 참조하세요.</div>
...
메트릭을 저장하기 위한 최대 메모리 사용량은 `forwarder_retry_queue_payloads_max_size` 구성 설정으로 정의됩니다. 이 한도에 도달하면 메트릭이 삭제됩니다.
...
메트릭은 `forwarder_storage_path` 설정으로 정의된 폴더에 저장됩니다. Unix 시스템에서는 `/opt/datadog-agent/run/transactions_to_retry`가 기본값이고, Windows에서는 `C:\ProgramData\Datadog\run\transactions_to_retry`가 기본값입니다.
...
연결성이 제한된 Kubernetes 환경에서 Datadog Operator를 설치하는 경우, 레지스트리에 따라 TCP 포트 443에 대해 다음 엔드포인트를 허용 목록에 추가해야 합니다.

- `registry.datadoghq.com`(Datadog 컨테이너 레지스트리)
  - `us-docker.pkg.dev/datadog-prod/public-images`(`registry.datadoghq.com`에서 리디렉션을 받을 수 있음)
