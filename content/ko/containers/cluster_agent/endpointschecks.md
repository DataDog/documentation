...
클러스터 점검 기능은  [자동탐지][1]가 가능하며 쿠버네티스(Kubernetes) 서비스와 같은 로드 밸런스 클러스터 서비스 점검을 합니다. _엔드포인트 점검_해당 장치를 연장하여 쿠버네티스 서비스로 관리되는 각 엔드포인트를 모니터링합니다. 

[클러스터 에이전트][2]는 쿠버네티스 서비스의 [자동탐지][1] 주석을 기반으로 엔드포인트 점검 설정을 찾습니다. 그 후에 클러스터 에이전트는 이러한 설정을 개별적으로 작동하기 위해 노드 관련 에이전트에게 보냅니다. 엔드포인트 점검은 모니터링되는 쿠버네티스 서비스의 엔드포인트를 지원하는 파드와 같은 노드를 활용하는 에이전트에 보냅니다. 이 디스패칭 로직을 통해 에이전트는 이미 수집된 각각의 파드에 파드 및 컨테이너 태그를 추가할 수 있습니다.

에이전트는 10초마다 클러스터 에이전트와 연결하며 작동할 설정 점검을 검색합니다. 엔드 포인트 점검으로부터 나온 메트릭은 평가된 IP 주소를 기반으로 한 서비스 태그, [쿠버네티스 태그][3], 호스트 태그 그리고 `kube_endpoint_ip` 태그와 함께 제출합니다. 

**버저닝**;
이 기능은 Datadog 에이전트 v6.12.0+ 그리고 Datadog 클러스터 에이전트 v1.3.0+ 위한 쿠버네티스를 지원합니다. 클러스터 에이전트 v1.4.0를 먼저 살펴보면, 클러스터 에이전트는 파드를 지원하지 않는 엔드포인드의 모든 엔드포인트 점검을 규칙적인 클러스터 점검으로 바꿉니다. 이 기능을 활용하기 위해 [클러스터 점검][4] 기능(엔드포인트 점검 기능 추가 외)을 사용하세요. 

**참고:** 만약 서비스 뒤 파드가 정적이라면, `ad.datadoghq.com/endpoints.resolve` 주석을 추가해야 합니다. Datadog 클러스터 에이전트는 점검을 엔드포인트 점검으로 예약하고 [클러스터 점검 실행자][5]로 보냅니다. 쿠버네티스 API 서버로 주석을 추가하는 [예시는 여기에서 참조하세요][6].

### 예: 엔드포인트 관련 서비스
아래 예시에서,  NGINX용 쿠버네티스 배포는 3개의 파드로 생성되었습니다.
...
서비스도 만들어졌습니다. 이 세 가지 엔드포인트를 통해 파드에 연결됩니다.
...
서비스 기반 클러스터 점검이 서비스의 하나의 IP 주소를 테스트하는 동안, 이 서비스와 관련된 **각각** 세 개의 엔드포인트의 점검이 예약됩니다.

설계상 엔드포인트 점검은 `nginx`서비스의 엔드포인트를 지원하는 파드와 동일한 노드에서 작동되는 에이전트로 보냅니다. 이 예시에서 에이전트는 `gke-cluster-default-pool-4658d5d4-k2sn`와 `gke-cluster-default-pool-4658d5d4-p39c`노드에서 작동되고 `nginx` 파드에 대해 점검을 합니다.
...
`datadog.clusterChecks.enabled` 설정 키를 통해 클러스터 에이전트의 키(Helm) 배포에서 엔드포인트 점검 디스패칭을 기본적으로 사용할 수 있습니다.
```yaml
datadog:
clusterChecks:
enabled: true
...
**참고**: 모니터링되는 엔드포인트가 파드에서 지원되지 않는 경우 [클러스터 점검을 활성화][1]해야 합니다. `kube_services` 설정 프로바이더와 리스너를 추가하세요.
...
**참고**: 모니터링되는 엔드포인트가 파드에서 지원되지 않는 경우 [클러스터 점검검을 활성화][1]해야 합니다. 이것은 `clusterchecks` 설정 프로바이더를 추가하여 작동할 수 있습니다.
...
클러스터 에이전트 v1.18.0+에서는 쿠버네티스 엔드포인트를 대상으로 지정하기 위해 점검 설정에서`advanced_ad_identifiers`와  [자동탐지 템플릿 변수][7]를 사용할 수 있습니다([예시 보기][8]).

#### 예: 쿠버네티스 엔드포인트 HTTP 점검 

쿠버네티스 서비스의 엔드포인트에 대해 [HTTP 점검][9]을 실행하려면,
...
**참고**: 커스텀 엔드포인트 로그 설정은 쿠버네티스 로그 파일 수집이 아닌 도커(Docker) 소켓 로그 수집 중에만 지원됩니다.

#### 예: 서비스의 엔드포인트 NGINX 점검으로 NGINX 지원 서비스 HTTP 점검

이 서비스는 `nginx` 배포 파드와 연결됩니다. 이 설정을 기반으로:

- 이 서비스를 지원하는 각 NGINX 파드에 대해 [`nginx`][12] 기반 엔드포인트 점검을 보냅니다. 이 점검은 NGINX 파드와 같은 각 노드의 에이전트에 의해 작동됩니다(파드 IP를 `%%host%%`로 사용).
- [`http_check`][9] 기반 클러스터 점검은 클러스터의 단일 에이전트로 보냅니다. 이 점검은 서비스의 IP를 `%%host%%`로 사용하여 각 엔드포인트에 자동으로 로드 밸런스를 맞춥니다.
- 점검은 [합쳐진 서비스 태깅][13] 레이블에 해당하는 `env:prod`, `service:my-nginx` 그리고`version:1.19.0` 태그와 함께 보냅니다.
