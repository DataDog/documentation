...
<div style="overflow-x: auto;">
  ...
  | `oshift_deployment`           | 오케스트레이터 | 포드 주석 `openshift.io/deployment.name`                                                                                 | OpenShift 환경 및 포드 주석 필요 |
  | `kube_ownerref_name`          | 오케스트레이터 | 포드 소유자 참조                                                                                                                  | 포드에 소유자가 있어야 함                              |
  | `kube_job`                    | 오케스트레이터 | 포드 소유자 참조                                                                                                                  | 포드가 CronJob에 연결되어 있어야 함                  |
  | `kube_job`                    | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 Job에 연결되어 있어야 함                       |
  | `kube_replica_set`            | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 ReplicaSet에 연결되어 있어야 함               |
  | `kube_service`                | 낮음          | Kubernetes 서비스 검색                                                                                                  | 포드가 Kubernetes 서비스 뒤에 있어야 함                  |
  | `kube_daemon_set`             | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 DaemonSet에 연결되어 있어야 함                 |
  | `kube_container_name`         | 낮음          | 포드 상태                                                                                                                    | 해당 없음                                                 |
  | `kube_namespace`              | 낮음          | 포드 메타데이터                                                                                                                  | 해당 없음                                                 |
  | `kube_app_name`               | 낮음          | 포드 레이블 `app.kubernetes.io/name`                                                                                            | 포드 레이블 존재 필요                               |
  | `kube_app_instance`           | 낮음          | 포드 레이블 `app.kubernetes.io/instance`                                                                                        | 포드 레이블 존재 필요                                |
  | `kube_app_version`            | 낮음          | 포드 레이블 `app.kubernetes.io/version`                                                                                         | 포드 레이블 존재 필요                                |
  | `kube_app_component`          | 낮음          | 포드 레이블 `app.kubernetes.io/component`                                                                                       | 포드 레이블 존재 필요                                |
  | `kube_app_part_of`            | 낮음          | 포드 레이블 `app.kubernetes.io/part-of`                                                                                         | 포드 레이블 존재 필요                                |
  | `kube_app_managed_by`         | 낮음          | 포드 레이블 `app.kubernetes.io/managed-by`                                                                                      | 포드 레이블 존재 필요                                |
  | `env`                         | 낮음          | 포드 레이블 `tags.datadoghq.com/env` 또는 컨테이너 환경 변수(`DD_ENV` 또는 `OTEL_RESOURCE_ATTRIBUTES`)                               | [unified service tagging][2] 활성화 필요                |
  | `version`                     | 낮음          | 포드 레이블 `tags.datadoghq.com/version` 또는 컨테이너 환경 변수(`DD_VERSION` 또는 `OTEL_RESOURCE_ATTRIBUTES`)                       | [unified service tagging][2] 활성화 필요                |
  | `service`                     | 낮음          | 포드 레이블 `tags.datadoghq.com/service` 또는 컨테이너 환경 변수(`DD_SERVICE`, `OTEL_RESOURCE_ATTRIBUTES`, 또는 `OTEL_SERVICE_NAME`) | [unified service tagging][2] 활성화 필요                |
  | `pod_phase`                   | 낮음          | 포드 상태                                                                                                                    | 해당 없음                                                 |
  | `oshift_deployment_config`    | 낮음          | 포드 주석 `openshift.io/deployment-config.name`                                                                          | OpenShift 환경 및 포드 주석 필요 |
  | `kube_ownerref_kind`          | 낮음          | 포드 소유자 참조                                                                                                                  | 포드에 소유자가 있어야 함                              |
  | `kube_deployment`             | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 Deployment에 연결되어 있어야 함                |
  | `kube_argo_rollout`           | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 Argo Rollout에 연결되어 있어야 함             |
  | `kube_replication_controller` | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 Replication Controller에 연결되어 있어야 함    |
  | `kube_stateful_set`           | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 StatefulSet에 연결되어 있어야 함               |
  | `persistentvolumeclaim`       | 낮음          | 포드 사양                                                                                                                      | 포드에 PVC가 연결되어 있어야 함                   |
  | `kube_cronjob`                | 낮음          | 포드 소유자 참조                                                                                                                  | 포드가 CronJob에 연결되어 있어야 함                   |
  | `image_name`                  | 낮음          | 포드 사양                                                                                                                      | 해당 없음                                                 |
  | `short_image`                 | 낮음          | 포드 사양                                                                                                                      | 해당 없음                                                 |
  | `image_tag`                   | 낮음          | 포드 사양                                                                                                                      | 해당 없음                                                 |
  | `eks_fargate_node`            | 낮음          | 포드 사양                                                                                                                      | EKS Fargate 환경                             |
  | `kube_runtime_class`          | 낮음          | 포드 사양                                                                                                                      | 포드가 RuntimeClass에 연결되어 있어야 함             |
  | `gpu_vendor`                  | 낮음          | 포드 사양                                                                                                                      | 컨테이너가 GPU 리소스에 연결되어 있어야 함        |
  | `image_id`                    | 낮음          | 컨테이너 이미지 ID                                                                                                            | 해당 없음                                                 |
  | `kube_autoscaler_kind`        | 낮음          | Kubernetes 오토스케일러 유형                                                                                                    | Kubernetes 오토스케일러 사용 필요                  |
  | `kube_priority_class`         | 낮음          | 포드 Priority Class                                                                                                            | 포드에 우선순위 클래스가 설정되어 있어야 함                    |
...
<div style="overflow-x: auto;">
  ...
  | 태그                 | 카디널리티 | 소스                                                 | 요구 사항                                                    |
  |---------------------|-------------|--------------------------------------------------------|----------------------------------------------------------------|
  | `kube_cluster_name` | 낮음         | `DD_CLUSTER_NAME` 환경 변수 또는 클라우드 공급자 통합 | `DD_CLUSTER_NAME` 환경 변수 또는 클라우드 공급자 통합 활성화 |
  | `kube_node_role`    | 낮음         | 노드 레이블 `node-role.kubernetes.io/<role>`            | 노드 레이블이 존재해야 함                                          |
  | `kube_node`         | 낮음         포드 사양의 | `NodeName` 필드             |                                                              |
  | `orch_cluster_id`   | 낮음         | 오케스트레이터 클러스터 메타데이터                          |  오케스트레이터 환경                                    |
  | `kube_distribution` | 낮음         | 노드 레이블 및 NodeSystemInfo                         |  |
</div>

## 태그 Autodiscovery {#tag-autodiscovery}
...
컨테이너화된 환경에서는 Datadog에서 태그 통합에 도움이 되는 unified service tagging 사용을 권장합니다. unified service tagging은 Datadog `env`, `service`, `version`의 세 가지 표준 태그를 사용하여 Datadog 텔레메트리를 연결합니다. 환경에서 unified service tagging을 구성하는 방법은 [unified service tagging][2] 설명서를 참조하세요.
...
이 옵션은 기존 Agent 옵션인 `podLabelsAsTags`, `nodeLabelsAsTags`, `namespaceLabelsAsTags` 및 모든 KSM 구성 재정의 대신 사용해야 합니다.
...
예를 들어, `kubectl api-resources`를 실행하여 정보를 확인할 수 있습니다.
...
- 태그는 워크로드와 하위 리소스 간에 상속되지 *않습니다*. 예를 들어, 배포에 설정된 레이블이 그 하위 포드의 로그에 자동으로 적용되지 않습니다. 포트 데이터를 태깅하려면 레이블 추출을 포드에서 직접 구성해야 합니다.
- 태그는 네임스페이스에서 해당 네임스페이스 내부의 포드 및 컨테이너로 *실제로* 상속됩니다.
- KSM Metrics의 태그 추출 규칙에서 와일드카드를 사용하려면 Datadog Agent 7.73 이상을 사용하세요.

### Kubernetes 리소스 레이블을 태그로 사용하기 {#kubernetes-resources-labels-as-tags}
...
지정된 리소스 레이블 `<LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
지정된 리소스 레이블 `<LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
지정된 리소스 레이블 `<LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Agent 및 Cluster Agent 컨테이너 **모두**에 다음 환경 변수를 추가합니다.
...
#### 레거시 구성과 병합하기 {#merging-with-legacy-configurations}
...
### Kubernetes 리소스 주석을 태그로 사용하기 {#kubernetes-resources-annotations-as-tags}
...
지정된 리소스 주석 `<ANNOTATION>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
지정된 리소스 주석 `<ANNOTATION>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
지정된 리소스 주석 `<ANNOTATION>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Agent 및 Cluster Agent 컨테이너 **모두**에 다음 환경 변수를 추가합니다.
...
#### 노드 레이블을 태그로 사용하기 {#node-labels-as-tags}

<div class="alert alert-info">

Agent 버전 7.58.0 이상에서는 <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">Kubernetes 리소스 레이블을 태그로 사용</a>하여 노드 레이블을 태그로 구성하는 것이 좋습니다.
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 노드 레이블 `<NODE_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 노드 레이블 `<NODE_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
#### 포드 레이블을 태그로 사용하기 {#pod-labels-as-tags}
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 포드 레이블 `<POD_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 포드 레이블 `<POD_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
#### 포드 주석을 태그로 사용하기 {#pod-annotations-as-tags}

<div class="alert alert-info">

Agent 버전 7.58.0 이상에서는 <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">Kubernetes 리소스 레이블을 태그로 사용</a>하여 포드 주석을 태그로 구성하는 것이 좋습니다.
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 포드 주석 `<POD_ANNOTATION>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 포드 주석 `<POD_ANNOTATION>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
#### 네임스페이스 레이블을 태그로 사용하기 {#namespace-labels-as-tags}

<div class="alert alert-info">

Agent 버전 7.58.0 이상에서는 <a href="/containers/kubernetes/tag/#kubernetes-resources-labels-as-tags">Kubernetes 리소스 레이블을 태그로 사용</a>하여 네임스페이스 레이블을 태그로 구성하는 것이 좋습니다.

</div>

Agent v7.55.0 이상부터는 지정된 네임스페이스에 대한 레이블을 수집하여 해당 네임스페이스의 모든 포드에서 내보내는 모든 메트릭, 트레이스 및 로그에 붙일 태그로 사용할 수 있습니다.

{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 네임스페이스 레이블 `<NAMESPACE_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 네임스페이스 레이블 `<NAMESPACE_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
### 컨테이너 환경 변수를 태그로 사용하기 {#container-environment-variables-as-tags}

Agent v7.32 이상부터는 컨테이너 환경 변수를 수집하여 컨테이너에 해당하는 모든 메트릭, 트레이스 및 로그에 붙일 태그로 사용할 수 있습니다. `docker` 및 `containerd` 컨테이너 모두 지원됩니다.

{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 환경 변수 `<ENV_VAR>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 환경 변수 `<ENV_VAR>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
### 컨테이너 레이블을 태그로 사용하기 {#container-labels-as-tags}

Agent v7.33 이상부터는 컨테이너 레이블을 수집하여 태그로 사용할 수 있습니다. Agent는 컨테이너와 관련된 모든 메트릭, 트레이스 및 로그에 태그를 붙입니다.
...
{{< tabs >}}
{{% tab "Datadog Operator" %}}
지정된 컨테이너 레이블 `<CONTAINER_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면, Operator의 `datadog-agent.yaml` 파일에 있는 `DatadogAgent` 구성에 다음 구성을 추가합니다.
...
{{% tab "Helm" %}}
지정된 컨테이너 레이블 `<CONTAINER_LABEL>`을 추출하여 Datadog 내에서 태그 키 `<TAG_KEY>`로 변환하려면 Helm `datadog-values.yaml` 파일에 다음 구성을 추가합니다.
...
{{% tab "수동(DaemonSet)" %}}
지정된 컨테이너 레이블 `<CONTAINER_LABEL>`을 추출하여 태그 키 `<TAG_KEY>`로 변환하려면 Datadog Agent에 다음 환경 변수를 추가합니다.
...
[4]: /ko/getting_started/tagging/assigning_tags/?tab=containerizedenvironments#tags-cardinality
