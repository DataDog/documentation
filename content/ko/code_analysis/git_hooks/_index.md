...
[Git hook](https://git-scm.com/docs/githooks)은 사용자가 코드를 리포지토리에 커밋하거나 원격 위치에 푸시하기 전에 실행되는 프로그램입니다. Git hook은 일반적으로 원격 브랜치에 푸시하기 전에 코드 검증을 실행하고 요구 사항을 적용하는 데 사용됩니다.

Datadog Code Analysis는 코드를 푸시하거나 커밋하기 전에 정적 분석 위반 사항이나 보안 비밀을 확인하는 Git hook을 제공합니다. Datadog Code Analysis Git hook은 최신 커밋과 기본 브랜치의 코드를 검사하고 감지된 오류를 표시합니다.

Datadog Git hook은 개발자가 코딩 오류, 취약점, 비밀이 포함된 코드를 푸시하기 전에 경고합니다. 오류가 있는 코드를 커밋하면 다음과 같은 메시지가 사용자 터미널에 나타납니다.

{{< img src="code_analysis/git_hooks/git_hook.png" alt="취약점을 감지한 Datadog Git Hook" style="width:100%;">}}
...
이 프로그램은 다음과 같은 파라미터를 허용합니다.
 ...
 - `--static-analysis`: 정적 분석을 활성화합니다.
 - `--secrets`: 비밀 감지(비공개 베타)를 활성화합니다.
