...
스테이지, 작업, 단계는 상위 파이프라인과 정확하게 같은 파이프라인 이름이어야 합니다. 이름이 일치하지 않는 경우, 일부 파이프라인에 스테이지, 작업, 단계 정보가 누락될 수 있습니다. 예를 들어, 작업 요약 표에 작업이 누락될 수 있습니다.
...
서로 다른 타임스탬프를 가진 ID를 반복해서 전송하면 사용자 인터페이스에서 원치 않는 동작이 발생할 수 있습니다. 예를 들어, 플레임 그래프에 다른 파이프라인 실행의 스팬 태그가 표시될 수 있습니다. 동일한 타임스탬프를 가진 중복 ID가 전송되면 마지막으로 수신된 파이프라인 실행의 값만 저장됩니다.
...
파이프라인 실행을 트리거한 커밋과 관련한 Git 정보를 제공하는 것이 좋습니다. Git 정보가 없는 파이프라인 실행은 [Recent Code Changes 페이지][4]에 표시되지 않습니다. 최소한 리포지토리 URL, 커밋 SHA, 작성자 이메일이 필요합니다. 자세한 내용은 [공개 API 엔드포인트 사양][3]을 참고하세요.
