...
| New Flaky   | `@test.is_new_flaky:true` | 테스트 실행은 커밋에서 실패했으며 (위의 "Flaky"의 정의에 따라) 이전에는 테스트 실행의 브랜치 또는 리포지토리 기본 브랜치의 "Flaky tests"테이블에 존재하지 않았습니다. 즉, 테스트가 "Flaky tests" 테이블에 추가되었음을 의미합니다. |
