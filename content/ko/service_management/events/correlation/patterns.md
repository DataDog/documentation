...
1. 위에 정의된 소스에서 이벤트를 제외하려면 **Filter by these events or tags**에 이벤트 쿼리를 추가해 필터링하세요.

### 고급 설정(선택 사항)
1. **Show Advanced Settings**을 클릭하세요.
1. 비알림 소스를 추가하고 그룹화 태그를 추가해 이벤트 상관 관계를 수립할 수 있습니다.

   비알림 소스
   비알림 소스는 Event Explorer에서 사용할 수 있는 이벤트이나 모니터나 알림과 연결되지 않은 이벤트를 뜻합니다.

   그룹화 태그 추가
   새 그룹화 태그를 추가하는 방법은 [새 이벤트 패싯][3]을 추가하는 방법과 동일합니다.
1. **Advanced correlation logic** 아래에서 사례를 만들 때 필요한 최소 상관 관계 이벤트를 지정하고 시간대를 업데이트할 수 있습니다.
   ...
   해당 알림의 이벤트 중복 제거 시간
   상관 관계가 수립되었으나 계속 플래핑되거나 확인되지 않은 현재 알림의 최대 지속 시간. 새 사례가 열리기 전에 기존 사례 내 해당 알림에 따라 이벤트가 중복 해제됩니다.


## 패턴 출력 미리 보기

내 구성에서 가능한 패턴과 사례를 미리 보세요. 미리 보기 패널에서 다음을 표시합니다.
- 수집된 이벤트 총 수(첫 1000개 이벤트로 제한)
- 이벤트에서 중복 해제될 알림 수
- 구성에 따라 생성될 사례 수
