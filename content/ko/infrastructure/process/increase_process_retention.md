...
<div class="alert alert-danger">프로세스 기반 메트릭은 <a href="/metrics/custom_metrics/">커스텀 메트릭</a>으로 간주되며 그에 따라 요금이 청구됩니다. 청구에 영향을 미치지 않게 하려면 <code>명령</code> 및 <code>사용자</code>와 같은 제한이 없거나 카디널리티가 매우 높은 태그를 기준으로 그룹을 생성하지 않아야 합니다.
