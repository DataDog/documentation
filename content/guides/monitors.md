...
* The set of [reserved characters](https://www.elastic.co/guide/en/elasticsearch/reference/2.4/query-dsl-query-string-query.html#_reserved_characters) for monitor search is more limited. The characters are `-`, `(`, `)`, `"`, `~`, `*`, `:`, `.`, and whitespace. Reserved characters other than `*` and `.` may only be used inside of double quotes, e.g. `title:chef-client*` is not valid; use `title:"chef-client*"`. The quote-less queries `chef*` and `metric:system.cpu.idle` are valid.
