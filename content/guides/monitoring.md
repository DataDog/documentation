...
***Note:*** *Notifications sent out within 20 seconds of each other via email, Hipchat, and Slack are rolled up together. For example, if there are 10 monitors that fire with the same monitor ID and alert type within 20 seconds, instead of receiving 10 notifications you will only receive one listing out all 10 in one message.*
