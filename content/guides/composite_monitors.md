...
An individual monitor is unaffected by the behavior and configuration of composite monitors which use it. If you'd like to turn off their notifications or make any other changes to them, modify them independently.

## Creating a Composite Monitor

In Datadog, go to the New Monitor page and select 'Composite Monitor' as the monitor type.

![choose-composite-type](/static/images/composite_monitors/select-monitor-type.png)

### Choose individual monitors

Choose up to 10 individual monitors to add to the new composite monitor. The individual monitors cannot themselves be composite monitors (i.e. you cannot create composite monitors from composite monitors). 
...
1. Populate the trigger condition field with the default trigger condition `a && b`,
...
### How Datadog computes composite statuses

### Composite monitor notifications
