...
Thus, from the user's perspective, the different status exposed by the composite monitor will be `Ok`, `Alert` and `No Data` (if not configured to alert in the individual monitors).
