...
If RUM sessions and test results are not linked, check the following possible causes:

1. The RUM sessions generated by the tests might be sampled out. Review [Best Practices for RUM Sampling][24].
2. The RUM session might not flush before the wait period expires. Set the `DD_CIVISIBILITY_RUM_FLUSH_WAIT_MILLIS` environment variable to increase the wait period. The default is 500 milliseconds, so try a higher value.
