...
- [The permissions required to view and set up scanning rules](#permissions).
- [Setting up scanning for sensitive data](#set-up-scanning-for-sensitive-data).
- [Investigating sensitive data issues using the Summary page](#investigate-sensitive-data-issues-in-the-summary-page).
...
## Set up scanning for sensitive data

This section walks through the following so that you can scan your data for sensitive information:
...
- [Redacting sensitive data in tags](#redact-sensitive-data-in-tags)
...
1. Navigate to [Sensitive Data Scanner][4]. Alternatively, go to Organization Settings > Sensitive Data Scanner.
...
1. In the **Add library rules to the scanning group**, select the library rules you want to use.
1. In the **Define rule target and action**, select if you want to scan the **Entire Event** or **Specific Attributes**.
...
1. In the **Define match conditions** section, specify the regex pattern to use for matching against events. Test with sample data to verify that your regex pattern is valid.   
...
1. Navigate to your [scanning group][4].
