...
NOTE: If the pods behind your service are static pods, you'll need another annotation ad.datadoghq.com/endpoints.resolve. The Datadog Cluster Agent schedules the checks as endpoint checks and dispatches them to Cluster Check Runners. See an example of this annotation with the kube API server here - https://docs.datadoghq.com/containers/kubernetes/control_plane/?tab=helm#api-server-2
...
Starting Datadog Agent 1.18.0 you can use `advanced_ad_identifiers` and [Autodiscovery template variables][5] in you check configuration to target Kubernetes endpoints ([see example][6]).
...
Similar to how [Kubernetes Pods][7] are annotated, services can be annotated with the following syntax:
...
The `%%host%%` [template variable][8] is supported and is replaced by the endpoints' IPs. The `kube_namespace`, `kube_service`, and `kube_endpoint_ip` tags are automatically added to the instances.
...
Optionally, to leverage [Unified Service Tagging][9], set the `env`, `service`, and `version` tags on data generated by these checks.
```yaml
...
- An [`nginx`][10]-based Endpoints Check is dispatched for each NGINX pod backing this service. This check is run by Agents on the same respective nodes as the NGINX pods (using the pod IP as `%%host%%`).
- An [`http_check`][11]-based Cluster Check is dispatched to a single Agent in the cluster. This check uses the IP of the service as `%%host%%` , automatically getting load balanced to the respective endpoints.
...
Troubleshooting Endpoints Checks is similar to [troubleshooting Cluster Checks][12]. The only difference is on Node-based Agents, where scheduled Endpoints Checks appear alongside Cluster Checks.
...
[5]: /agent/guide/template_variables/
[6]: /agent/cluster_agent/endpointschecks/#example-http_check-on-kubernetes-endpoints
[7]: /agent/kubernetes/integrations/?tab=kubernetes#template-source-kubernetes-pod-annotations
[8]: /agent/kubernetes/integrations/?tab=kubernetes#supported-template-variables
[9]: /getting_started/tagging/unified_service_tagging
[10]: /integrations/nginx/
[11]: /integrations/http_check/
[12]: /agent/cluster_agent/troubleshooting/
