...
| `DD_CONFIG_PROVIDERS`        | The providers the Agent should call to collect checks configurations. Available providers are: <br> * `kubelet` - The kubelet provider handles templates embedded in pod annotations. <br> * `docker` - The Docker provider handles templates embedded in container labels. <br> * `clusterchecks` - The clustercheck provider retrieves cluster-level check configurations from the cluster-agent. <br> * `kube_services` - The kube_services provider watches Kubernetes services for cluster-checks. |
