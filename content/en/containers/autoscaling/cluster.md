...
3. Run the `kubectl` plugin and pass in AWS credentials for the AWS project that the cluster is running:
   ...
   For more detailed command options, run [`kubectl datadog autoscaling cluster install --help`][4].

## Autoscale a cluster

In Datadog, navigate to [Kubernetes Autoscaling][5]. You can review all your clusters that are available for autoscaling on the [Autoscaling Settings][7] page. Toggle the {{< ui >}}Enabled{{< /ui >}} column for your cluster of choice.
...
Use the [Cluster Scaling][6] page and select a cluster to review its recommendations. Identify a cluster you want to begin optimizing.
...
Click {{< ui >}}Start Autoscaling{{< /ui >}} to apply recommendations.
