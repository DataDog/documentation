...
- If your containers are using the `awslogs` [logging driver to send the logs to CloudWatch][3], then those logs are not be visible to the Agent. Instead, use one of the [AWS log collection integrations][4] in order to collect those logs.

## Installation

### ECS task definition

To collect all logs from your running ECS containers, update your Agent's Task Definition from the [original ECS Setup][5] with the environment variables and mounts below.
...
If you have a local file for your Agent's Task Definition you can repeat the steps to [register your updated Task Definition][2]. This creates a new revision for you. You can then reference this updated revision in the Daemon Service for the Datadog Agent.
...
You can consult [the Docker Log Collection setup instructions][6] for information on how to use Autodiscovery configurations. For example, the following log configuration overrides the `source` and `service` of the logs collected:
...
You can further customize this by [adding tags to your log configuration][7] or any `log_processing_rules` for [advanced log collection options][8].
...
The log configuration below tells the Agent to [collect this custom log file][9] at the `/var/log/example/app.log` path.
```json
[{
  "type": "file",
...
See the [AWS Bind mounts documentation][10] for additional details on volume management with ECS.
...
The `source` attribute is used to identify the integration to use for each container. Override it directly in your containers labels to start using [the Datadog log integrations][11]. Read Datadog's [Autodiscovery guide for logs][12] to learn more about this process.
...
[4]: /integrations/amazon_web_services/?tab=allpermissions#log-collection
[5]: /containers/amazon_ecs/?tab=awscli#setup
[6]: /containers/docker/log/?tab=dockerfile#log-integrations
[7]: /getting_started/tagging/assigning_tags/?tab=noncontainerizedenvironments#methods-for-assigning-tags
[8]: /agent/logs/advanced_log_collection?tab=docker
[9]: /agent/logs/?tab=tailfiles#custom-log-collection
[10]: https://docs.aws.amazon.com/AmazonECS/latest/developerguide/bind-mounts.html
[11]: https://app.datadoghq.com/logs/pipelines/pipeline/library
[12]: /agent/docker/log/?tab=containerinstallation#log-integrations
