...
{{< whatsnext desc="Source code management (SCM) integrations check for Static Analysis violations during code reviews in pull requests for repos that have at least one ruleset applied. Violations are flagged with a comment on the relevant line of code. The comment includes the name, ID, severity, and description of the violation. Certain violations will also include suggested changes that can be applied directly in the UI of the SCM tool." >}}
