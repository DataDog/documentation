...
`--metrics`
: Key-value pairs in the form `key:123` to be attached to all tests (the `--metrics` parameter can be specified multiple times). When specifying tags using `DD_METRICS`, separate them using commas (for example, `coverage:80,priority:3`).<br/>
...
[Test suite level visibility][10] is supported from `datadog-ci>=2.17`.
...
[10]: /continuous_integration/tests/#test-suite-level-visibility
