...
In addition to tests, you have visibility over the whole testing phase of your project. On the [Test Runs][4] page, you can filter by test level: session, module, suite and test. Each represent an different level of aggregation of tests.

{{< img src="ci/ci-test-suite-visibility.png" alt="Test Suite Visibility" style="width:100%;">}}

#### Sessions
Test sessions are the highest level of aggregation. They map one to one to a test command:
...
#### Module
The definition of module changes slightly per language: for .NET, they represent every test that is run under the same [unit test project][8] and for Swift, it's every test that is run for a given bundle.

An example of module is `SwiftLintFrameworkTests`, which corresponds to a test target in [`SwiftLint`][9]
...
Test execution data is available when you create widgets in [Dashboards][11] and [Notebooks][12].
...
[12]: https://app.datadoghq.com/notebook/list
