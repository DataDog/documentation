...
| --- | --- | --- |
| `shared_preload_libraries` | includes `pg_stat_statements` | Required for `postgresql.queries.*` metrics. Enables collection of query metrics using the [pg_stat_statements][5] extension. If you have other values in `shared_preload_libraries`, append `,pg_stat_statements` |
