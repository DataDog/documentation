...
This page details common issues with setting up and using Database Monitoring with MongoDB, and how to resolve them. Datadog recommends staying on the latest stable Agent version and adhering to the latest [setup documentation][1], as it can change with Agent version releases.
...
If you are using MongoDB Atlas, you may encounter a `ServerSelectionTimeoutError` when connecting to the cluster. This error occurs when the Agent is unable to connect to the MongoDB Atlas cluster due to a network issue or TLS misconfiguration. To resolve this issue, ensure `tls` is set to `true` in the integration configuration file. If you are still encountering issues, check the MongoDB Atlas cluster's network access configuration to ensure the Agent's IP address is allowed to connect to the cluster.
