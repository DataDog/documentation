...
Use these events to confirm what happened to a message, investigate failures, and get alerted when delivery breaks down. To run a query from this guide, click a situation to open it prefilled in the [Audit Trail Explorer][1], or query for email events manually using the attributes in [Event attributes][2].

**Note**: Enable Audit Trail to keep email delivery events for 90 days and to build monitors and dashboards on them. Events are recorded from the date you enable Audit Trail and are not retroactive. To diagnose and fix a specific reported failure, see [Email suppressions][3].
