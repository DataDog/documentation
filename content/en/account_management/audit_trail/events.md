...
| [Password][9] | A user modified or reset their password in the org. These events are delivered to all orgs that user is active in, even if the org does not have password authentication configured | `@evt.name:"Access Management" @asset.type:password @action:modified` |
