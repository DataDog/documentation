...
- How to create a [Dashboard and Notebook widget](#create-a-widget-with-cross-organization-data) or API query with data from your other organizations
...
Cross-organization data can be selected in Dashboard and Notebook widget JSON. The same per-query field is also available in the Metrics API formula query endpoints:
- [Query timeseries data across multiple products][8]
- [Query scalar data across multiple products][15]

When you define a widget in the Dashboard API, or send a Metrics API formula query request, use the `cross_org_uuids` parameter in each query object to identify the source organization in a cross-organization query.
