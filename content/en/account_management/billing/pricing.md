...
* A **serverless function** is application code set up to execute on a cloud platform's serverless compute service in response to defined events or actions (for example, AWS Lambda, Google Cloud Function, and Azure Function). Datadog charges per average on the number of Lambda functions per hour across the month.
