---
title: Setting AI Credit Limits
...
To view and set AI Credit limits, a user needs the following permissions:
...
|---|---|
| Org-wide limit | Caps total AI Credit usage across the organization for the month. No org-wide limit is set by default. |
...
- Raising a limit unblocks users who had reached the previous limit. Lowering a limit blocks users whose usage already exceeds the new value.
...
| Org limit | Per-user limit | Users | Max hypothetical usage | Credits charged if limits are reached | Result |
|---|---|---|---|---|---|
...
| 2,000 Credits | 200 Credits per user | 20 | 200 × 20 = 4,000 Credits<sup>*</sup> | 2,000 Credits | Each user can use up to 200 AI Credits **as long as the org limit of 2,000 Credits has not been reached.** |
...
Attribution determines whose limit applies to a given unit of AI usage, and who the AI Credit spend is assigned to.

| Product | Attribution |
|---|---|
| [Bits Chat][1] | All Bits Chat usage rolls up to the user email interacting with Bits Chat. |
...
| [Bits Agent Builder][4] | Agent runs for workflows roll up to the user email that created the workflow.<br>Agent runs for workflows that roll up to service accounts roll up to **Autonomous Agents**. |
