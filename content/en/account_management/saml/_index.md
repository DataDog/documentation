...
You can assign or remove Datadog roles based on a user's SAML-assigned role:
...
When a user who has the specified identity provider attribute logs in, they will automatically be assigned the Datadog role. Likewise, if someone has that identity provider attribute removed, they will also lose access to the role (unless another mapping adds it).
