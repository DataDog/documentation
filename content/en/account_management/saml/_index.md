...
When a user who has the specified identity provider attribute logs in, they will automatically be assigned the Datadog role. Likewise, if someone has that identity provider attribute removed, they will also lose access to the role (unless another mapping adds it). It is important to note: if a user does not match any mappings they will lose any roles they had previously and will be prevented from logging into your account with SAML.
