...
`spec.yaml`
: This is used to generate the `conf.yaml.example` using the `ddev` tooling (see the **Configuration template** tab below). For more information, see [Configuration specification][13].
...
The wheel is installed using the Agent `integration` command, available in [Agent v6.10.0 and up][21]. Depending on your environment, you may need to execute this command as a specific user or with specific privileges:
