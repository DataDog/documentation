...
On any platform with Python (on Windows, the Agent's embedded Python interpreter can be used,
which is located at `C:\Program Files\Datadog\Datadog Agent\embedded\python.exe` for agent versions <= 6.12
and in `C:\Program Files\Datadog\Datadog Agent\embedded2\python.exe` for agent versions >= 6.13):
