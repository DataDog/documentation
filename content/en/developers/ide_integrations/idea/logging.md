...
1. If a bulb icon appears, click on the on the icon and select **Show Logs View...**.
1. (Alternatively) Right-click the line and select **Datadog** -> **View Logs...**.
  ...
  {{< img src="/developers/ide_integrations/idea/log-navigation-dialog1.png" alt="A bulb icon indicates an intention action" style="width:90%;" >}}
...
To quickly open a log query in Datadog without first modifying the query:
...
1. Select on of the following log actions:
   ...
   - **Tail Live Logs**: Opens a logs view showing the latest logs ingested for a service. The view is filtered by logger name if one is detected.
