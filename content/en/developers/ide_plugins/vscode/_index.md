...
Use **Log Annotations** to gauge the volume of logs generated by a given log line in your code. The extension adds CodeLens annotations to detected logging patterns that match log records in Datadog. Click an annotation to open the [Log Explorer][4] in Datadog and view the matching logs.
...
Datadog anonymously collects information about your usage of this IDE, including how you interact with it, whether errors occurred while using it, and what caused those errors, in accordance with the [Datadog Privacy Policy][13] and Datadog's [VS Code extension EULA][12].

If you don't wish to send this data to [Datadog][3], you can opt out at any time in the extension's settings: `Datadog > Telemetry > Setup > Enable Telemetry` and select `disabled`.
