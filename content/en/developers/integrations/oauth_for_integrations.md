...
You can include OAuth in a new integration on the [Marketplace][2] or [Integrations][3] page by following the steps below. These steps also apply to adding OAuth to an existing integration, but note that there's no need to change your `app_uuid` in the `manifest.json`. 
