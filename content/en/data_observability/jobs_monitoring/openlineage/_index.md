...
The `tags` facet must be an array of objects, each with a `key`, `value`, and `source`. Datadog only applies tags where `source` is exactly `USER`; tags sent as a single object instead of an array, or with a missing or different `source`, are ignored.
