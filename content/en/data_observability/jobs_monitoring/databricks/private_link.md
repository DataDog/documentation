...
1. Assign an identity ([Workload Identity][5] or [IAM Role][6]) to the pod. This identity **must** have permissions to read the secret created in the Databricks prerequisites.
...
1. Assign an identity ([Managed Identity][3] or [IAM Role][4]) to the instance. This identity **must** have permissions to read the secret created in the Databricks prerequisites.
...
1. Attach a connection to the Private Action Runner:
   ...
   1. Add the service account created in the Datadog prerequisites with **Resolver** access.
   1. Select **Create and Test**. Verify the test results do not return an error.
   1. Note the connection ID.
...
1. Set up the Databricks integration:
   ...
   1. Under **Credentials**, select **Private Action Runner**.
      1. Enter the **Connection ID** noted above.
      1. Under **Service Account**, select the service account created in the Datadog prerequisites.
      1. Enter the secret path noted in the Databricks prerequisites under **Secret Path**. Alternatively, set this as an environment variable on the container or pod using `DATABRICKS_SECRET_PATH`.
