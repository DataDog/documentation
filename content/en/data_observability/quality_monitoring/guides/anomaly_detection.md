...
The model often reuses prior predictions rather than generating new ones — for example, when a value hasn't changed, or when the monitor is in an alerting state. As a result, adding [annotations][1] is the primary way to get an alerting monitor back to a normal state and resume learning from new values.
...
Row count monitors alert when a table's row count has flatlined — that is, when it has not changed for longer than normal. A stalled row count may indicate a broken pipeline.
