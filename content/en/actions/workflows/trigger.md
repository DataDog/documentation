...
1. From the workflow page, click {{< ui >}}Run{{< /ui >}}.
1. Enter the values for existing trigger variables.
1. When you're ready to run the workflow, click {{< ui >}}Save & Run{{< /ui >}}.

## Dashboard triggers

To trigger a workflow from a dashboard, add the {{< ui >}}Run Workflow{{< /ui >}} widget:
1. From your dashboard, click {{< ui >}}Add Widget{{< /ui >}}.
1. Search for `workflows` and add the {{< ui >}}Run Workflow{{< /ui >}} widget.
1. Under {{< ui >}}Select the workflow{{< /ui >}}, find your workflow in the dropdown menu. Only published workflows with dashboard triggers appear in the list.
...
1. In the {{< ui >}}Recipient{{< /ui >}} section, use the workflow mention name to find your workflow. For example, `@workflow-my-workflow`.
...
1. Click {{< ui >}}Run Workflow{{< /ui >}} in the {{< ui >}}Next Steps{{< /ui >}} box at the top of the side panel.
...
You can see the workflow run status in the {{< ui >}}Workflows{{< /ui >}} section of the side panel.
...
1. Add a GitHub trigger to your workflow:
   - If your workflow doesn't have any triggers, click {{< ui >}}Add Trigger{{< /ui >}} > {{< ui >}}GitHub{{< /ui >}}.
   - If your workflow already has one or more triggers and you're adding GitHub as an additional trigger, click the {{< ui >}}Add Trigger{{< /ui >}} (lightning bolt) icon and select {{< ui >}}GitHub{{< /ui >}}.
1. Navigate to the GitHub repo you want to use to trigger your workflow.
1. In GitHub, click {{< ui >}}Settings{{< /ui >}}, click {{< ui >}}Webhooks{{< /ui >}}, and then click {{< ui >}}Add webhook{{< /ui >}}.
1. In the {{< ui >}}Configure{{< /ui >}} tab of your workflow, copy the {{< ui >}}Payload URL{{< /ui >}}. Paste it into the {{< ui >}}Payload URL{{< /ui >}} field on the GitHub webhook creation page.
...
1. Click {{< ui >}}Add Trigger{{< /ui >}} > {{< ui >}}API{{< /ui >}}.
1. On the workflow canvas, click {{< ui >}}API{{< /ui >}} and note the example workflow cURL request, which includes the required headers and data to trigger your workflow.
...
1. Click {{< ui >}}Save{{< /ui >}}.
1. Click {{< ui >}}Publish{{< /ui >}}. Scheduled workflows don't run until you've published them. Published workflows accrue costs based on workflow executions. For more information, see the [Datadog Pricing page][11].

## Trigger a workflow from a workflow

You can trigger a child workflow from another workflow using the {{< ui >}}Trigger Workflow{{< /ui >}} action. For example, if you have a complex series of steps that you need to reuse in several workflows, there's no need to recreate those steps for all of your workflows. Instead, add the steps to a new workflow and trigger it in your other workflows using the Trigger Workflow action.
...
You can pass the result of a child workflow back to the parent workflow by defining {{< ui >}}Output parameters{{< /ui >}} in the child workflow. Use the `WorkflowOutputs` context variable in the parent workflow to retrieve the output parameters of the child workflow. For example, given a child workflow named `Example_workflow` with an output parameter named `exampleList`, use `Steps.Example_workflow.workflowOutputs.exampleList` to access the result of the child workflow.

## Run history

After you trigger a workflow, the workflow page switches to the workflow's {{< ui >}}Run History{{< /ui >}}. Click {{< ui >}}Configuration{{< /ui >}} or {{< ui >}}Run History{{< /ui >}} in the top-left to switch between the configuration and run history views. Use run history to watch the progress of a triggered workflow or [debug a failed step][13].
