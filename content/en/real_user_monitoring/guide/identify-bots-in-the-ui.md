...
To filter out bot sessions, you can conditionally set a `sampleRate` of `0` for bot sessions by checking the `user-agent` against a [list of known robots][3] when the application initializes the RUM Browser SDK. 
...
// initialize the RUM Browser SDK and set sampleRate to conditionalSampleRate
datadogRum.init({
 // ... config options
