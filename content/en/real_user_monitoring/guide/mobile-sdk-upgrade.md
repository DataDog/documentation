...
We following Google's [AndroidX library version policy](https://developer.android.com/jetpack/androidx/versions#version-table) policy for the `AndroidX` libraries so the minimum Android API level supported by SDK v3 is `23` now.

The `OpenTracing` dependency was removed because it is obsolete.
...
To enable Crash Reporting, make sure to enable RUM.
...
Learn more about [privacy levels][2].
...
[2]: /real_user_monitoring/session_replay/mobile/privacy_options?platform=ios
...
The [`OpenTracing`](https://opentracing.io/) project has been marked as archived and it is no longer supported. The `OpenTracing` dependencies on has been removed from SDK v3.

The Datadog SDK already supports [`OpenTelemetry`](https://opentelemetry.io/), which is now the recommended way to use the tracing feature API.

**Note** that the `OpenTelemetry` specification library [requires](https://github.com/open-telemetry/opentelemetry-java?tab=readme-ov-file#requirements) desugaring to be enabled for projects with a `minSdk` < 26.
...
SDK v2 enables the usage of [Mobile Session Replay][4] on Android and iOS applications.
...
Add the following rules to your build script (more details in the relevant [Stack Overflow issue][5]):
...
See the [Android sample application][6] for an example of how to set up the SDK.

[5]: https://stackoverflow.com/a/75298544
[6]: https://github.com/DataDog/dd-sdk-android/tree/develop/sample
...
[7]: /real_user_monitoring/session_replay/mobile/setup_and_configuration/?tab=android
...
For instructions on setting up Mobile Session Replay, see [Mobile Session Replay Setup and Configuration][8].

[8]: /real_user_monitoring/session_replay/mobile/setup_and_configuration/?tab=ios
