...
`setFirstPartyHosts()` 
: Defines hosts that have tracing enabled and have RUM resources categorized as `first-party`. **Note**: If you defined custom tracing header types in Datadog configuration and you are using a tracer registered with `GlobalTracer`, it's necessary to ensure the same tracing header types are set for the tracer in use.
