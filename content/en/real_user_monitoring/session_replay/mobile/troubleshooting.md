...
On iOS, vector images are not supported in Session Replay. These appear as "Unsupported image type" placeholders in the replay. To work around this, you can disable the "Preserve Vector Data" option in your asset catalog so that Xcode rasterizes the images, allowing them to be captured correctly. Note that SF Symbols are also not supported.
