...
Starting with [version 2.13.0][4], `beforeSend` takes two arguments: the `event` generated by the RUM Browser SDK, and the `context` that triggered the creation of the RUM event.
...
For more information, see the [Enrich and control RUM data guide][13].
...
The RUM Browser SDK ignores modifications made to event properties not listed above. For more information about event properties, see the [RUM Browser SDK GitHub repository][14].
...
Follow the [Datadog naming convention][15] for a better correlation of your data across the product.
