...
RUM is compliant with many compliance frameworks, including, but not limited to:
...
The data we track automatically contains primarily technical information, much of which doesn't include personal identifying information. If you leave automatic tracking on, you can still opt to redact the data that is captured by RUM before it is sent and stored in Datadog through advanced configuration options for the following methods:
...
- Automatically captured data, such as action names on buttons
- Automatically captured data, such as view names shown in URLs
