...
To use Multisource Querying, ensure you have configured [Cloud Cost Management][10] and are actively ingesting costs in Datadog. Multiple currencies are now supported, with your costs automatically being converted to and displayed in USD.
...
| `billingcurrency` | The currency in which a cloud bill was ingested in. |
...
## Currency conversion
Cloud Cost Management retrieves the billing currency from each cloud provider's bill. When processing multisource costs from multiple providers with different currencies, each provider’s cost charges are converted to USD. This conversion is performed using the average monthly exchange rate, which is fetched and updated daily from the NetSuite API. This ensures that Cloud Cost Management can conssitently and accurately represent all cost data, regardless of its original currency.
