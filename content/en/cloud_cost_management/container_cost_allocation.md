...
Datadog consolidates and applies additional tags from various sources to cost metrics. Conflicts are resolved by favoring higher-specificity tags such as pod tags over lower-specificity tags such as host tags. For example, if a k8s pod has the tag `service:datadog-agent` and the node it runs on has the tag `service:aws-node`, the resulting final tag will just show `service:datadog-agent`.
