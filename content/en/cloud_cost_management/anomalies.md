...
Our algorithm takes into account seasonality, distinguishing true anomalies from expected fluctuations. For example, if your infrastructure typically scales up every Monday, Datadog recognizes this pattern and won't flag it as a cost anomaly. The algorithm also filters into just usage charges only (excludes taxes, credits, refunds, RIFee, etc) so that anomalies are based on engineering driven usage that can be actioned on.
