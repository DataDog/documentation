...
recommendations:
    ...
    - category: Previous generation resource
      cloud_provider: AWS
      resource_type: Extended Support for RDS
      recommendation_type: An RDS running an engine version that is no longer supported and incurring [extended support charges][7]
    ...
    - category: Over-provisioned resource
      cloud_provider: AWS
      resource_type: EC2 instances not in an auto-scaling group, with less than 50% CPU and memory utilization.
      recommendation_type: '[Datadog agent](/agent/)'
      recommendation_description: ""
      recommendation_prerequisites: ""
    - category: Over-provisioned resource
      cloud_provider: AWS
    ...
    - category: Over-provisioned resource
      ...
      recommendation_prerequisites: '[Also requires Amazon EC2 integration](/integrations/amazon_ec2/)'
    - category: Over-provisioned resource
      cloud_provider: AWS
    ...
    - category: Architecture
      cloud_provider: NAT Gateway
      resource_type: NAT Gateway within VPC transfer charges
      recommendation_type: Resources in the same VPC should avoid communicating with each other through a NAT gateway because that incurs unnecessary NAT gateway processing charges.
      recommendation_description: '[NPM](/network_monitoring/performance/setup/)'
      recommendation_prerequisites: ""
    - category: Architecture
      cloud_provider: NAT Gateway
      resource_type: NAT Gateway cross-zone transfer charges
      recommendation_type: Resources that need a NAT gateway should use one that is in the same availability zone, or they can incur unnecessary cross-zone transfer charges.
      recommendation_description: ""
      recommendation_prerequisites: ""
...
1. Install the '[Datadog Agent](/agent/)' (required for over-provisioned resource recommendations).
...
| ---------------------------- | -------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| Unused resource              | AWS            | EC2                                                                                        | Unused EC2 Instances                                                                                                                                       | EC2 instances with less than 5% CPU utilization, and less than 10% memory utilization.                                                                                                              | '[Datadog agent](/agent/)'                          |
...
| Unused resource              | AWS            | S3                                                                                         | Abandoned S3 Multipart Uploads                                                                                                                             | Incomplete multipart uploads.                                                                                                                                                                       | [Storage Lens](/integrations/amazon_s3_storage_lens/)                           |
...
| Unused resource              | AWS            | Autoscaling groups (ASG)                                                                   | ASGs with legacy instance types                                                                                                                            | An autoscaling group that includes legacy instance types.                                                                                                                                           | '[Datadog agent](/agent/)'                          |
...
| Previous generation resource | AWS            | EC2                                                                                        | Legacy EC2 Instance                                                                                                                                        | EC2 Instances that are previous generation, and can be upgraded to a newer instance type.                                                                                                           | '[Datadog agent](/agent/)'                          |
...
| Over-provisioned resource    | AWS            | EC2 instances not in an auto-scaling group, with less than 50% CPU and memory utilization. | '[Datadog agent](/agent/)'                                                                                                                                         |
| Over-provisioned resource    | AWS            | Kubernetes containers                                                                      | Over-provisioned Kubernetes Containers                                                                                                                     | Containers with less than 30% CPU and memory utilization.                                                                                                                                           | [Datadog profiling agent](/profiler/enabling/)                |
| Over-provisioned resource    | AWS            | EBS                                                                                        | Over-provisioned EBS Volume IOPS                                                                                                                           | EBS Volumes where the amount of IOPS exceeds what is being used.                                                                                                                                    | [Also requires Amazon EC2 integration](/integrations/amazon_ec2/) |
...
| Over-provisioned resource    | AWS            | EBS IOPS                                                                                   | Over-provisioned EBS IOPS                                                                                                                                  | An EBS volume using less than 80% of the provisioned IOPS for reads and writes.                                                                                                                     | \*[Also requires Amazon EC2 integration](/integrations/amazon_ec2/) |
| Over-provisioned resource    | AWS            | EBS Storage                                                                                | Over-provisioned EBS Storage                                                                                                                               | An EBS volume with less than 20% of its storage capacity used.                                                                                                                                      | \*[Also requires Amazon EC2 integration](/integrations/amazon_ec2/) |
| Over-provisioned resource    | AWS            | EBS Throughput                                                                             | Over-provisioned EBS Throughput                                                                                                                            | An EBS volume using less than 80% of the provisioned throughput for reads and writes.                                                                                                               | \*[Also requires Amazon EC2 integration](/integrations/amazon_ec2/) |
...
| Rate optimization            | AWS            | S3                                                                                         | S3 Non-current Version Expiration Lifecycle Rule                                                                                                           | A standard S3 bucket without a non-current version expiration lifecycle and that does not serve a website contains non-current version storage bytes older than 30 days.                            | [Storage Lens](/integrations/amazon_s3_storage_lens/)                           |
