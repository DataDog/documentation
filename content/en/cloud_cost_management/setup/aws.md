...
* **Export Path Prefix**: The S3 path prefix where report files are stored.
  * **Note:** The following prefix formats are not supported: empty prefix, prefix starting with `/` (such as `/` or `/cost`), or prefix ending with `/` (such as `cost/`). Prefixes containing `/` in the middle are supported (such as `cost/hourly`).
