...
- link: "/mobile_testing/mobile_tests//"
  tag: "Documentation"
  text: "Learn how to create mobile tests"
- link: "/mobile_testing/mobile_tests/steps/"
  tag: "Documentation"
  text: "Learn how to create steps in mobile tests"
...
When the test runs successfully, the mobile test recomputes (or "self heals") any broken locators with updated values, ensuring your tests do not break from simple UI updates and that your tests are automatically adapting to your mobile application's UI. 
...
If a mobile test cannot locate an element, it retries the step for 60 seconds by default.
...
In some cases, such as in the event of a pop-up, you may want to make some steps optional. To configure this option, select **Continue with test if this step fails**. If the step fails after the amount of minutes specified on the timeout option, then the mobile test moves on and executes the next step.
