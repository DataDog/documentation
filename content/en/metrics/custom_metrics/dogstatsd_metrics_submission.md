...
tracer.metrics.increment('example_metric.increment', 1, ['environment:dev']);
tracer.metrics.decrement('example_metric.decrement', 1, ['environment:dev']);
```
{{< /programming-lang >}}
...
while(true) {
  i++;
  tracer.metrics.gauge('example_metric.gauge', i, ['environment:dev']);
}
```
{{< /programming-lang >}}
...
while(true) {
  tracer.metrics.distribution('example_metric.distribution', Math.random() * 20, ['environment:dev']);
...
{{< programming-lang lang="nodejs" >}}
```javascript
tracer.metrics.increment('example_metric.increment', 1, ['environment:dev', 'account:local']);
```
{{< /programming-lang >}}
