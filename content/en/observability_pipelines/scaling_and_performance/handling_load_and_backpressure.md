...
If you enable disk buffering for destinations, you must enable Kubernetes [persistent volumes][1] in the Observability Pipelines helm chart to handle back pressure when a destination is unavailable or can't keep up with the volume of data that the Worker is sending.
