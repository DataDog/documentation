...
[global_acks]: https://vector.dev/docs/reference/configuration/global-options/#acknowledgements</td></tr><tr><td>api_version</td><td>The API version of Elasticsearch.</td></tr><tr><td>auth</td><td>Elasticsearch Authentication strategies.</td></tr><tr><td>auth.access_key_id</td><td>The AWS access key ID.</td></tr><tr><td>auth.assume_role</td><td>The ARN of an [IAM role][iam_role] to assume.
...
Relevant when the default credentials chain is used or `assume_role`.</td></tr><tr><td>auth.password</td><td>Basic authentication password.</td></tr><tr><td>auth.profile</td><td>The credentials profile to use.
...
[aws_region]: https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints</td></tr><tr><td>auth.secret_access_key</td><td>The AWS secret access key.</td></tr><tr><td>auth.strategy</td><td>The authentication strategy to use.</td></tr><tr><td>auth.user</td><td>Basic authentication username.</td></tr><tr><td>aws</td><td>Configuration of the region/endpoint to use when interacting with an AWS service.</td></tr><tr><td>aws.endpoint</td><td>Custom endpoint for use with AWS-compatible services.</td></tr><tr><td>aws.region</td><td>The [AWS region][aws_region] of the target service.

[aws_region]: https://docs.aws.amazon.com/general/latest/gr/rande.html#regional-endpoints</td></tr><tr><td>batch</td><td>Event batching behavior.</td></tr><tr><td>batch.max_bytes</td><td>The maximum size of a batch that will be processed by a sink.

This is based on the uncompressed size of the batched events, before they are
serialized / compressed.</td></tr><tr><td>batch.max_events</td><td>The maximum size of a batch before it is flushed.</td></tr><tr><td>batch.timeout_secs</td><td>The maximum age of a batch before it is flushed.</td></tr><tr><td>bulk</td><td>Elasticsearch bulk mode configuration.</td></tr><tr><td>bulk.action</td><td>Action to use when making requests to the [Elasticsearch Bulk API][es_bulk].

Currently, Vector only supports `index` and `create`. `update` and `delete` actions are not supported.

[es_bulk]: https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-bulk.html</td></tr><tr><td>bulk.index</td><td>The name of the index to write events to.</td></tr><tr><td>compression</td><td>Compression configuration.

All compression algorithms use the default compression level unless otherwise specified.</td></tr><tr><td>data_stream</td><td>Elasticsearch data stream mode configuration.</td></tr><tr><td>data_stream.auto_routing</td><td>Automatically routes events by deriving the data stream name using specific event fields.
...
set here in the configuration will be used.</td></tr><tr><td>data_stream.dataset</td><td>The data stream dataset used to construct the data stream at index time.</td></tr><tr><td>data_stream.namespace</td><td>The data stream namespace used to construct the data stream at index time.</td></tr><tr><td>data_stream.sync_fields</td><td>Automatically adds and syncs the `data_stream.*` event fields if they are missing from the event.

This ensures that fields match the name of the data stream that is receiving events.</td></tr><tr><td>data_stream.type</td><td>The data stream type used to construct the data stream at index time.</td></tr><tr><td>distribution</td><td>Options for determining the health of an endpoint.</td></tr><tr><td>distribution.retry_initial_backoff_secs</td><td>Initial delay between attempts to reactivate endpoints once they become unhealthy.</td></tr><tr><td>distribution.retry_max_duration_secs</td><td>Maximum delay between attempts to reactivate endpoints once they become unhealthy.</td></tr><tr><td>doc_type</td><td>The [`doc_type`][doc_type] for your index data.
...
[doc_type]: https://www.elastic.co/guide/en/elasticsearch/reference/6.8/actions-index.html</td></tr><tr><td>encoding</td><td>Transformations to prepare an event for serialization.</td></tr><tr><td>encoding.except_fields</td><td>List of fields that will be excluded from the encoded event.</td></tr><tr><td>encoding.only_fields</td><td>List of fields that will be included in the encoded event.</td></tr><tr><td>encoding.timestamp_format</td><td>Format used for timestamp fields.</td></tr><tr><td>endpoint</td><td>The Elasticsearch endpoint to send logs to.

The endpoint must contain an HTTP scheme, and may specify a
hostname or IP address and port.</td></tr><tr><td>endpoints</td><td>A list of Elasticsearch endpoints to send logs to.

The endpoint must contain an HTTP scheme, and may specify a
hostname or IP address and port.</td></tr><tr><td>id_key</td><td>The name of the event key that should map to Elasticsearch’s [`_id` field][es_id].
...
[perf_doc]: https://www.elastic.co/guide/en/elasticsearch/reference/master/tune-for-indexing-speed.html#_use_auto_generated_ids</td></tr><tr><td>metrics</td><td>Configuration for the `metric_to_log` transform.</td></tr><tr><td>metrics.host_tag</td><td>Name of the tag in the metric to use for the source host.
...
[global_log_schema_host_key]: https://vector.dev/docs/reference/configuration//global-options#log_schema.host_key</td></tr><tr><td>metrics.metric_tag_values</td><td>Controls how metric tag values are encoded.
...
[vector_native_json]: https://github.com/vectordotdev/vector/blob/master/lib/codecs/tests/data/native_encoding/schema.cue</td></tr><tr><td>metrics.timezone</td><td>The name of the timezone to apply to timestamp conversions that do not contain an explicit
time zone.
...
[tz_database]: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones</td></tr><tr><td>mode</td><td>Elasticsearch Indexing mode.</td></tr><tr><td>pipeline</td><td>The name of the pipeline to apply.</td></tr><tr><td>query</td><td>Custom parameters to add to the query string for each HTTP request sent to Elasticsearch.</td></tr><tr><td>query.*</td><td>A query string parameter.</td></tr><tr><td>request</td><td>Outbound HTTP request settings.</td></tr><tr><td>request.adaptive_concurrency</td><td>Configuration of adaptive concurrency parameters.
...
create orphaned requests, pile on retries, and result in duplicate data downstream.</td></tr><tr><td>request_retry_partial</td><td>Whether or not to retry successful requests containing partial failures.

To avoid duplicates in Elasticsearch, please use option `id_key`.</td></tr><tr><td>suppress_type_name</td><td>Whether or not to send the `type` field to Elasticsearch.

The `type` field was deprecated in Elasticsearch 7.x and removed in Elasticsearch 8.x.

If enabled, the `doc_type` option will be ignored.</td></tr><tr><td>inputs</td><td>A list of upstream [source][sources] or [transform][transforms] IDs.
