...
serialized / compressed.</td></tr><tr><td>batch.max_events</td><td>The maximum size of a batch before it is flushed.</td></tr><tr><td>batch.timeout_secs</td><td>The maximum age of a batch before it is flushed.</td></tr><tr><td>bucket</td><td>The name of the bucket to write into.

Only relevant when using InfluxDB v2.x and above.</td></tr><tr><td>consistency</td><td>The consistency level to use for writes.

Only relevant when using InfluxDB v0.x/v1.x.</td></tr><tr><td>database</td><td>The name of the database to write into.

Only relevant when using InfluxDB v0.x/v1.x.</td></tr><tr><td>default_namespace</td><td>Sets the default namespace for any metrics sent.
...
This should be a full HTTP URI, including the scheme, host, and port.</td></tr><tr><td>org</td><td>The name of the organization to write into.

Only relevant when using InfluxDB v2.x and above.</td></tr><tr><td>password</td><td>The password to authenticate with.

Only relevant when using InfluxDB v0.x/v1.x.</td></tr><tr><td>quantiles</td><td>The list of quantiles to calculate when sending distribution metrics.</td></tr><tr><td>request</td><td>Middleware settings for outbound requests.
...
create orphaned requests, pile on retries, and result in duplicate data downstream.</td></tr><tr><td>retention_policy_name</td><td>The target retention policy for writes.

Only relevant when using InfluxDB v0.x/v1.x.</td></tr><tr><td>tags</td><td>A map of additional tags, in the form of key/value pairs, to add to each measurement.</td></tr><tr><td>tags.*</td><td>A tag key/value pair.</td></tr><tr><td>tls</td><td>TLS configuration.</td></tr><tr><td>tls.alpn_protocols</td><td>Sets the list of supported ALPN protocols.
...
Do NOT set this to `false` unless you understand the risks of not verifying the remote hostname.</td></tr><tr><td>token</td><td>The [token][token_docs] to authenticate with.

Only relevant when using InfluxDB v2.x and above.

[token_docs]: https://v2.docs.influxdata.com/v2.0/security/tokens/</td></tr><tr><td>inputs</td><td>A list of upstream [source][sources] or [transform][transforms] IDs.
...
Must be at least ~256 megabytes (268435488 bytes).</td></tr><tr><td>buffer.type</td><td>The type of buffer to use.</td></tr><tr><td>buffer.when_full</td><td>Event handling behavior when a buffer is full.</td></tr><tr><td>username</td><td>The username to authenticate with.

Only relevant when using InfluxDB v0.x/v1.x.</td></tr><tr><td>healthcheck</td><td>Healthcheck configuration.</td></tr><tr><td>healthcheck.enabled</td><td>Whether or not to check the health of the sink when Vector starts up.</td></tr><tr><td>type</td><td>The component type. This is a required field for all components and tells Vector which component to use.</td></tr></tbody></table>
