...
metric.  When set to `full`, all metric tags will be exposed as separate assignments.</td></tr><tr><td>encoding.only_fields</td><td>List of fields that will be included in the encoded event.</td></tr><tr><td>encoding.timestamp_format</td><td>Format used for timestamp fields.</td></tr><tr><td>framing</td><td>Framing configuration.</td></tr><tr><td>framing.character_delimited</td><td>Options for the character delimited encoder.</td></tr><tr><td>framing.character_delimited.delimiter</td><td>The ASCII (7-bit) character that delimits byte sequences.</td></tr><tr><td>framing.method</td><td>The framing method.</td></tr><tr><td>headers</td><td>A list of custom headers to add to each request.</td></tr><tr><td>headers.*</td><td>An HTTP request header and it's value.</td></tr><tr><td>method</td><td>The HTTP method to use when making the request.</td></tr><tr><td>payload_prefix</td><td>A string to prefix the payload with.

This option is ignored if the encoding is not character delimited JSON.

If specified, the `payload_suffix` must also be specified and together they must produce a valid JSON object.</td></tr><tr><td>payload_suffix</td><td>A string to suffix the payload with.

This option is ignored if the encoding is not character delimited JSON.

If specified, the `payload_prefix` must also be specified and together they must produce a valid JSON object.</td></tr><tr><td>request</td><td>Outbound HTTP request settings.</td></tr><tr><td>request.adaptive_concurrency</td><td>Configuration of adaptive concurrency parameters.
...
[cidr]: https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing</td></tr><tr><td>uri</td><td>The full URI to make HTTP requests to.

This should include the protocol and host, but can also include the port, path, and any other valid part of a URI.</td></tr><tr><td>healthcheck</td><td>Healthcheck configuration.</td></tr><tr><td>healthcheck.enabled</td><td>Whether or not to check the health of the sink when Vector starts up.</td></tr><tr><td>healthcheck.uri</td><td>The full URI to make HTTP healthcheck requests to.

This must be a valid URI, which requires at least the scheme and host. All other
components -- port, path, etc -- are allowed as well.</td></tr><tr><td>type</td><td>The component type. This is a required field for all components and tells Vector which component to use.</td></tr></tbody></table>
