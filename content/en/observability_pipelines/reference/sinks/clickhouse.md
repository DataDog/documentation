...
[global_acks]: https://vector.dev/docs/reference/configuration/global-options/#acknowledgements</td></tr><tr><td>auth</td><td>Configuration of the authentication strategy for HTTP requests.

HTTP authentication should almost always be used with HTTPS only, as the authentication credentials are passed as an
HTTP header without any additional encryption beyond what is provided by the transport itself.</td></tr><tr><td>auth.password</td><td>The basic authentication password.</td></tr><tr><td>auth.strategy</td><td>The authentication strategy to use.</td></tr><tr><td>auth.token</td><td>The bearer authentication token.</td></tr><tr><td>auth.user</td><td>The basic authentication username.</td></tr><tr><td>batch</td><td>Event batching behavior.</td></tr><tr><td>batch.max_bytes</td><td>The maximum size of a batch that will be processed by a sink.
...
All compression algorithms use the default compression level unless otherwise specified.</td></tr><tr><td>database</td><td>The database that contains the table that data will be inserted into.</td></tr><tr><td>date_time_best_effort</td><td>Sets `date_time_input_format` to `best_effort`, allowing ClickHouse to properly parse RFC3339/ISO 8601.</td></tr><tr><td>encoding</td><td>Transformations to prepare an event for serialization.</td></tr><tr><td>encoding.except_fields</td><td>List of fields that will be excluded from the encoded event.</td></tr><tr><td>encoding.only_fields</td><td>List of fields that will be included in the encoded event.</td></tr><tr><td>encoding.timestamp_format</td><td>Format used for timestamp fields.</td></tr><tr><td>endpoint</td><td>The endpoint of the ClickHouse server.</td></tr><tr><td>request</td><td>Middleware settings for outbound requests.
...
create orphaned requests, pile on retries, and result in duplicate data downstream.</td></tr><tr><td>skip_unknown_fields</td><td>Sets `input_format_skip_unknown_fields`, allowing ClickHouse to discard fields not present in the table schema.</td></tr><tr><td>table</td><td>The table that data will be inserted into.</td></tr><tr><td>inputs</td><td>A list of upstream [source][sources] or [transform][transforms] IDs.
