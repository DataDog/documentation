...
# Configuration
<table><thead><tr><td>Param</td><td>Description</td></tr></thead><tbody><tr><td>auth</td><td>Configuration of the authentication strategy when interacting with NATS.</td></tr><tr><td>auth.credentials_file</td><td>Credentials file configuration.</td></tr><tr><td>auth.credentials_file.path</td><td>Path to credentials file.</td></tr><tr><td>auth.nkey</td><td>NKeys configuration.</td></tr><tr><td>auth.nkey.nkey</td><td>User.
...
If there is a risk of processing malformed data, such as logs with user-controlled input,
consider setting the maximum length to a reasonably large value as a safety net. This will
ensure that processing is not truly unbounded.</td></tr><tr><td>framing.octet_counting</td><td>Options for the octet counting decoder.</td></tr><tr><td>framing.octet_counting.max_length</td><td>The maximum length of the byte buffer.</td></tr><tr><td>queue</td><td>NATS Queue Group to join.</td></tr><tr><td>subject</td><td>The NATS [subject][nats_subject] to pull messages from.

[nats_subject]: https://docs.nats.io/nats-concepts/subjects</td></tr><tr><td>subject_key_field</td><td>The `NATS` subject key.</td></tr><tr><td>tls</td><td>Configures the TLS options for incoming/outgoing connections.</td></tr><tr><td>tls.alpn_protocols</td><td>Sets the list of supported ALPN protocols.
