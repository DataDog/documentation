...
# Configuration
<table><thead><tr><td>Param</td><td>Description</td></tr></thead><tbody><tr><td>decoding</td><td>Configures how events are decoded from raw bytes.</td></tr><tr><td>decoding.codec</td><td>The codec to use for decoding events.</td></tr><tr><td>fd</td><td>The file descriptor number to read from.</td></tr><tr><td>framing</td><td>Framing configuration.
...
By default, the [global `host_key` option](https://vector.dev/docs/reference/configuration//global-options#log_schema.host_key) is used.</td></tr><tr><td>max_length</td><td>The maximum buffer size, in bytes, of incoming messages.
...
## Context
By default, the `file_descriptor` source augments events with helpful
context keys.
