 ...
 **Note**: When `DD_OP_API_ENABLED` is set to `true`, the `/health` API endpoint is exposed. Configure load balancers to use the `/health` endpoint to check that the Worker is up and running.
