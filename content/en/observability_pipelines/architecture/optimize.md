...
In addition, network file systems like Amazon's EFS are usable, but only if sufficient throughput is provisioned; burst throughput modes will not suffice. We recommend configuring at least 2x the maximum expected throughput to give headroom for spikes in demand. The recommended disks above all have sufficient throughput that this is not a concern.
