...
### Generated metrics are timestamped with the Worker processing time instead of the log timestamp

If metrics generated by the Generate Metrics processor are timestamped with the time the log was processed by the Worker rather than the log's own timestamp, the `timestamp` field on the log is likely in string format. The Generate Metrics processor requires the `timestamp` field to be a parsed timestamp type. See [Convert string timestamp to timestamp format][23] for instructions.
