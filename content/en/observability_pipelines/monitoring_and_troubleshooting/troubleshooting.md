...
If you are seeing error response codes such as `4xx` or `5xx`, check the Worker debug logs for more information.
...
### Generated metrics are timestamped with the processing time instead of the log timestamp

If metrics generated by the Generate Metrics processor are timestamped with the log's processing time rather than the timestamp on the log, check if the log `timestamp` is in string format. The Generate Metrics processor requires the `timestamp` field to be a parsed timestamp type. See [Convert string timestamp to timestamp format][23] for instructions.
