...
If metrics generated by the Generate Metrics processor are timestamped with the log's processing time rather than the timestamp on the log, check to see if the `timestamp` field on the log is in string format. The Generate Metrics processor requires the `timestamp` field to be a parsed timestamp type. See [Convert string timestamp to timestamp format][23] for instructions.
