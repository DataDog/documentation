...
<div class="alert alert-info">Local firewalls like Uncomplicated Firewall (UFW) may block traffic even when configured with permissive settings. Check system logs for blocked packet entries, which typically indicate traffic that reached the network interface but was blocked before reaching the operating system.</div>
