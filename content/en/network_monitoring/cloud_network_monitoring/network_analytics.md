...
Neutral tags are tags that are not specific to a client or server but apply to an entire flow. You can search for and filter on traffic with these neutral tags, for example, you can use these tags to filter for traffic that is TLS encrypted.
...
The following is the list of neutral tags available to be used:
| Tag                     | Description                                                                                   |
|-------------------------|-----------------------------------------------------------------------------------------------|
| is_agent_traffic        | Indicates whether the traffic was generated by the Datadog Agent.                             |
| tls_encrypted           | Specifies if the connection is encrypted using TLS.                                           |
| tls_cipher_suite        | Identifies the specific TLS cipher suite used (example `TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256`).  |
| tls_cipher_insecure     | Identifies if the cipher used in the connection is generally considered to be secure or not.  |
| tls_version             | The version of TLS negotiated and used by the connection (`tls_1.2`, `tls_1.3`).            |
| tls_client_version      | The version(s) of TLS that the client supports (`tls_1.2`, `tls_1.3`).                      |
| gateway_id              | Unique identifier for the AWS gateway resource.                                               |
| gateway_type            | Specifies the type of AWS gateway (Internet, NAT, Transit).                             |
| gateway_region          | The AWS region in which the gateway is deployed (example `us-east-1`).                            |
| gateway_availability-zone | The Availability Zone hosting the gateway (example `us-east-1a`).                               |
...
| tgw_attachment_type     | Type of the AWS Transit Gateway attachment (example VPC, VPN, Direct Connect).                  |
