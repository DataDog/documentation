...
NPM automatically detects S3, RDS, Kinesis, ELB, Elasticache, and and others listed in the [supported services][3]. To monitor other endpoints where an Agent cannot be installed (such as public APIs), group the destination in the Network Overview by  the [`domain` tag](#dns-resolution).
