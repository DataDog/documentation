...
Notebooks are a collaborative text editor that gives you all the power of Datadog graphs inside your documents. Multiple users can work together to put together an investigation or postmortem featuring live data from your incident. Notebooks are also great for runbooks and documentation featuring real insights into your systems alongside your content.

## Creating a notebook

You can create a notebook in two places.

- From the lefthand navigation bar click **Dashboards > New Notebook**.
...
With Notebooks, you can start typing using markdown shortucts such as `#` for headers or ``` for code blocks. Notebooks comprise cells that can contain text or graphs. A new cell is created every time you hit `enter`. Notebooks automatically save your text content as you type. For graph cells, save all edits in the graph editor for the widget to be saved in the notebook.
...
| Feature      | Description                                                                                          |
|--------------|------------------------------------------------------------------------------------------------------|
| **Bold**     | To bold text, select it and press cmd + b.                                                           |
| *Italics*    | To italicize text, select it and press cmd + i.                                                      |
...
| Quotes       | Insert a quote block by typing >, or use the slash command menu.                                     |

### Slash commands

Slash commands are an interface for creating graphs or cells. On a new line, type `/` to open the slash command menu. Continue typing the name of the desired cell type to search for and select the appropriate option.
...
To create a markdown cell type `/markdown` and hit enter.

### Moving cells

You can move cells by either cutting and pasting them (`cmd + x`, `cmd + v`) or by hovering over the cell and using the drag handle on the left to move it to the desired location.
...
| **Tag by Team**        | Tagging a Notebook with a team allows you to use it as a filter when searching for a Notebook. You can tag a Notebook with up to 5 teams. To tag, click on the "team" option in the Notebook header and select the desired teams.  |
| **Tag by Type**        | You can tag your notebooks with type tags for easier searching, such as: Postmortem, Runbook, Investigation, Documentation, Report. To tag, select Type at the top of the Notebook. |
...
- Hyperlink an external image using the image widget in the header or by referencing the [official Markdown guide][16]. **Note**: This option does not upload the image to be hosted by Datadog. 
...
| Feature                  | Description                                                                                                          |
|--------------------------|----------------------------------------------------------------------------------------------------------------------|
| **Navigating to Comments** | Saved comments appear in the right margin of the notebook. Click a comment highlight in the text to align it, or click a comment in the margin to scroll to its location. |
...
Notebooks are collaborative, allowing multiple users to edit simultaneously. If a collaborator opens your document, their cursor will appear inside the document. Hover over a cursor to see the collaborator's name.

<!-- TODO Add updated image from new notebooks -->

### Widgets

When another user is editing a widget, an outline appears to indicate this. Since widgets are saved as last write wins, avoid editing a widget that someone else is working on.

<!-- TODO Add updated image from new notebooks -->

#### Presence

At the top of the document you can see a a row avatar images of all the users currently viewing the document. Hover over an avatar to see the name of the associated collaborator.
