...
Under **Evaluation Scope**, define where and how your evaluation runs. This helps control coverage (which spans are included) and cost (how many spans are sampled).
   ...
   - **Query**: (Optional) Enter a query to filter which spans are evaluated. This uses the standard Datadog query syntax. You can filter by span name, tags, or any combination. For example:
      - `span_name:agent.workflow` to filter by span name
      - `env:prod` to filter by tag
      - `span_name:agent.workflow AND env:prod` to combine multiple filters
