...
- Did the user leave the session with their problem solved?
- Did the agent remember context from earlier turns?
...
|---|---|
| Goal completeness | A user may ask follow-up questions across several traces before their task is truly done. |
| Conversation quality | Coherence, memory, and tone are properties of the whole chat, not one turn. |
| User intent resolution | Multiple distinct intents can appear in one session; the judge can assess whether each was addressed. |
