...
- Set API keys via `client_options={"api_key": "..."}` or environment variables (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`)
- Use `reasoning=True` in structured outputs to include explanation in results
- Define pass/fail criteria with `pass_when` (boolean), `pass_values` (categorical), or `min_threshold`/`max_threshold` (score)

## Built-in evaluators

The SDK provides built-in evaluators for common evaluation patterns.
