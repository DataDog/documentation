...
5. Under **What should we detect Patterns on?**, enter a template that defines what gets sent to the model for analysis. Use `{{variable}}` syntax to reference any span field — for example, `{{meta.input.value}}` to analyze patterns by user input, or `{{meta.span.kind}}` to analyze by span kind. To access array elements, use bracket notation: `{{meta.input.messages[0].content}}`. Dotted array indices such as `messages.0.content` are not supported and cause an error when you save the configuration. Click {{< ui >}}Template Examples{{< /ui >}} to see common configurations. As you type, the right panel previews matching spans and shows what percentage of interactions have values for the variables you've referenced.
6. Click **Save**

<div class="alert alert-info">When you click <strong>Save</strong>, Datadog validates your configuration before saving it. If the template contains invalid syntax — for example, dotted array indices instead of bracket notation — the save is rejected and you are prompted to fix the variable paths. If the span filter matches no processed spans in the selected time window at the moment you save, the save is also rejected with an actionable error, which prevents creating configurations that would produce empty runs. If the event processing service is temporarily unavailable, this second validation check is skipped and the configuration is saved normally so availability is preserved.</div>
