...
**Note**: When only `input.messages` is set for an LLM span, Datadog infers `input.value` by concatenating the content fields of all messages, regardless of their roles.
