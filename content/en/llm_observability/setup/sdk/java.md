...
Spans can be finishe by calling `finish()` on a span object instance.
...
`inputData`
: optional - `String` or `List<Map<String, Object>>`
<br />Either a String (for non-LLM spans) or a list of maps' with this format: `{role: "...", content: "..."}` (for LLM spans).
...
public class MyJavaClass {
  public String invokeChat(String userInput) {
    ...
    llmSpan.annotateIO(
      ...
      Arrays.asList(
        Map.of("role", "assistant", "content", chatResponse)
      )
    );
    llmSpan.finish();
    return chatResponse;
