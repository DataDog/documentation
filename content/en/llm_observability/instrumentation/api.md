...
| metadata                 | Dict[key (string), value] where the value is a float, bool, or string | Data about the span that is not input or output related. For example, you can pass `temperature` and `max_tokens` for LLM spans. If `model_name` or `model_provider` keys are present here, they will supercede whatever is passed in the top-level fields. |
