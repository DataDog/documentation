...
For example it sends APM payloads to `trace.agent.datadoghq.com.`.

This behavior can be disabled in version 7.72.0 or more recent by setting `convert_dd_site_fqdn.enabled` to `false` in the configuration, or with the environment variable `DD_CONVERT_DD_SITE_FQDN_ENABLED=false`.
</div>
