...
A special case are YAML files with the suffix `.default`. These files are loaded by the Agent by default and help define the core set of checks that are always enabled. To turn off one of these checks, change the suffix to `.disabled` or remove the file altogether.  To customize these checks, copy the `conf.yaml.default` file to `conf.yaml` and make any desired changes, and the `.default` file will be ignored.
