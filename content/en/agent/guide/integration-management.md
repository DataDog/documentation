...
**Note:**: Starting with Agent version 7.68, Community, Partner, and Marketplace integrations are automatically retained during upgrades. To opt out of this functionality, create an empty file named `.skip_install_python_third_party_deps` in the  `/etc/datadog-agent/` directory prior to upgrading. If this file is present, integrations will not be retained and must be manually reinstalled after the upgrade.
