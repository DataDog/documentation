...
Datadog exposes AWS PrivateLink endpoints in **ap-northeast-1**.

However, to route traffic to Datadog's PrivateLink offering in `ap-northeast-1` from other regions, use inter-region [Amazon VPC peering][2]. Inter-region VPC peering enables you to establish connections between VPCs across different AWS regions. This allows VPC resources in different regions to communicate with each other using private IP addresses. For more details, see [Amazon VPC peering][2].

{{< tabs >}}
{{% tab "ap-northeast-1" %}}

1. Connect the AWS Console to region **ap-northeast-1** and create a VPC endpoint.
...
| Datadog                   | PrivateLink service name                                  | Private DNS name                           |
|---------------------------|-----------------------------------------------------------|--------------------------------------------|
| Logs (Agent HTTP intake)  | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-03e139d1f2766685b`                                                    | `agent-http-intake.logs.ap1.datadoghq.com` |
| Logs (User HTTP intake)   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-08799aabff1cfd8a3` | `http-intake.logs.ap1.datadoghq.com`       |
| API                       | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-008cd79a7a09e0a1e` | `api.ap1.datadoghq.com`                    |
| Metrics                   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-002d904d5e69340ad` | `metrics.agent.ap1.datadoghq.com`          |
| Containers                | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-03ffd8d552f0d957d` | `orchestrator.ap1.datadoghq.com`           |
| Process                   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0e86b29a0dc2c8a26` | `process.ap1.datadoghq.com`                |
| Profiling                 | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0d598057ecde10596` | `intake.profile.ap1.datadoghq.com`         |
| Traces                    | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0a5d94b2e8f6e70fc` | `trace.agent.ap1.datadoghq.com`            |
...
| Datadog                   | PrivateLink service name                                  |
|---------------------------| --------------------------------------------------------- |
| Metrics                   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-002d904d5e69340ad` |
| Logs (Agent HTTP intake)  | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-03e139d1f2766685b` |
| Logs (User HTTP intake)   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-08799aabff1cfd8a3` |
| API                       | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-008cd79a7a09e0a1e` |
| Process                   | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0e86b29a0dc2c8a26` |
| Profiling                 | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0d598057ecde10596` |
| Traces                    | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-0a5d94b2e8f6e70fc` |
| Containers                | `com.amazonaws.vpce.ap-northeast-1.vpce-svc-03ffd8d552f0d957d` |
...
This returns `metrics.agent.ap1.datadoghq.com`, the private hosted zone name that you need in order to associate with the VPC which the Agent traffic originates in. Overriding this record grabs all Metrics-related intake hostnames.

2. Within each new Route53 private hosted zone, create an A record with the same name. Toggle the **Alias** option, then under **Route traffic to**, choose **Alias to VPC endpoint**, **ap-northeast-1**, and enter the DNS name of the VPC endpoint associated with the DNS name.

   **Notes**:
  - To retrieve your DNS name, see the [View endpoint service private DNS name configuration documentation.][4]
  - The Agent sends telemetry to versioned endpoints, for example, `<version>-app.agent.ap1.datadoghq.com` which resolves to `metrics.agent.ap1.datadoghq.com` through a CNAME alias. Therefore, you only need to set up a private hosted zone for `metrics.agent.ap1.datadoghq.com`.

{{< img src="agent/guide/private_link/create-an-a-record.png" alt="Create an A record" style="width:90%;" >}}

3. Configure VPC peering and routing between the VPC in `ap-northeast-1` that contains the Datadog PrivateLink endpoints and the VPC in the region where the Datadog Agents run.
...
The DNS names should resolve to IP addresses contained within the CIDR block of the VPC in `ap-northeast-1`, and connections to `port 443` should succeed.
