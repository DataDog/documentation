...
**Note**: The container `<NAME>` is the one in your manifest `.spec.containers[0].name`.
...
Datadog Agent excludes Kubernetes pause containers by default. They are still counted in the container count like excluded containers.

To disable this behavior and include pause containers in the Autodiscovery perimeter, set the environment variable `DD_EXCLUDE_PAUSE_CONTAINER` to `false`.
