...
This feature requires a running [Cluster Agent][2].
...
**Note**: The [Datadog Helm Chart][3] offers the possibility to deploy, via the `clusterChecksRunner` field, a set of Datadog Agents configured to run cluster checks only.

### Custom checks

Running [custom Agent checks][4] as cluster checks is supported, as long as all node-based Agents are able to run it. This means your checks' code:
...
In addition to the steps mentioned in the [Cluster Agent Setup][2] section, you must set `DD_CLUSTER_CHECKS_ADVANCED_DISPATCHING_ENABLED` to `true`.
...
After setting up a CloudSQL instance and a [Datadog user][5], mount a `/conf.d/mysql.yaml` file in the Cluster Agent container with the following content:
...
The `%%host%%` [template variable][6] is supported and is replaced by the service's IP. The `kube_namespace` and `kube_service` tags are automatically added to the instance.
...
These standard labels are part of [Unified Service Tagging][7].
