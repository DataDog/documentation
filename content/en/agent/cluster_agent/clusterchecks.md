...
It is not recommended to perform cluster checks on the Agents from the DaemonSet. Instead, have a side deployment with three replicas that run the Agent without any metadata that hits the Datadog Cluster Agent for cluster-wide configurations.

## Setting up check configurations

### Static configurations in files

When the IP of a given resource is constant (for example: external service endpoint, public URL, etc.), a static configuration can be passed to the Cluster Agent as YAML files. The file name convention and syntax are the same as the static configurations on the node-based Agent, with the addition of the `cluster_check: true` line.
...
The `%%host%%` [template variable][5] is supported and is replaced by the service's IP. The `kube_namespace` and `kube_service` tags are automatically added to the instance.

#### Example: HTTP check on an NGINX-backed service

The following Service definition exposes the Pods from the `my-nginx` deployment and runs an [HTTP check][6] to measure the latency of the load balanced service:
...
In addition, each pod should be monitored with the [NGINX check][7], as it enables the monitoring of each worker as well as the aggregated service. If a pod has annotations or is Autodiscovered, it is monitored.
...
Alternatively `exec`ing into any Datadog Cluster Agent and running `agent status` shows the leader's details:

```
Leader Election
===============
Leader Election Status: Running
Leader Name is: datadog-cluster-agent-5c9754447-pmvlj
Last Acquisition of the lease: Wed, 19 Feb 2020 21:19:57 UTC
Renewed leadership: Mon, 24 Feb 2020 21:54:02 UTC
Number of leader transitions: 103 transitions
```
...
[6]: /integrations/http_check/
[7]: /integrations/nginx/
