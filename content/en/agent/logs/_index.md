...
| `start_position` | No       | If `type` is **file**, set the position from where the agent will start reading the file. Valid values are `beginning` and `end`. If `path` contains wildcard character `beginning` is not supported.                                                                                                                                                   |
