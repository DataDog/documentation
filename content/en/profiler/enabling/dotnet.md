...
<div class="alert alert-warning">
  ...
  Because APM tracing also relies on the CLR Profiling API, if you want to stop collecting .NET profiles but continue receiving .NET traces, set `DD_PROFILING_ENABLED=0` but leave `CORECLR_ENABLE_PROFILING=1`.
</div>
