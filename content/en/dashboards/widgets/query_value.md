...
Enable the {{< ui >}}Change Indicator{{< /ui >}} to highlight how the current value compares to a previous time frame.
...
* {{< ui >}}Display{{< /ui >}}: `Relative Change` (percentage), `Absolute Change` (raw value), `Both`, or `Off`
* {{< ui >}}Color{{< /ui >}}: `Increases as better` (green ↑, red ↓), `Decreases as better` (red ↑, green ↓), or `Neutral`
* {{< ui >}}Compared to{{< /ui >}}: `Previous Period`, `Previous Day/Week/Month`, or `Custom` time frame
