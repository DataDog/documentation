...
* **Name**: A unique name for the template variable. Template variables are automatically named after the selected tag or attribute. This name is how the variable will appear in queries on the dashboard.
* **Default Value**: The tag or attribute value that appears automatically when the dashboard is loaded. Defaults to `*`.
* **Available Values**: The tag or attribute values available for selection in the dropdown. Defaults to `(all)`. The list of available values will always include `*`, which queries all values of the tag or attribute.
...
{{< img src="dashboards/template_variables/stats_tv_modal.png" alt="template variable modal with several variables set" style="width:90%;">}}
