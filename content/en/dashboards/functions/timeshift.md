...
To access the `calendar_shift()` function click the {{< ui >}}Add function{{< /ui >}} button, select {{< ui >}}Timeshift > Month before{{< /ui >}}. The calendar shift allows you to compare the same metric across equivalent time frames. Below is an example of cloud cost metric `aws.cost.net.amortized` with the calendar_shift() value from two weeks ago compared to the current value.
