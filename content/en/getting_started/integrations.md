...
* Authentication (crawler) based integrations are set up in the [Datadog App][7] where you provide credentials for obtaining metrics with the API.
...
**conf.yaml** - You can find the `conf.yaml` in the `conf.d/` folder at the root of your [Agent's configuration directory][8]. Use this file to connect integrations to your system, as well as configure their settings. Nonrequired parameters are usually commented out.

**instance** - You define and map the instance of whatever you are monitoring in the `conf.yaml` file. For example, in the [`http_check` integration][9], you're defining the name associated with the instance of the HTTP endpoint you are monitoring up and downtime. You can monitor multiple instances in the same integration, and you do that by defining all of the instances in the `conf.yaml` file.

**metrics** - The list of what is collected from your system by each integration. You can find the metrics for each integration in that integrations `metadata.csv` file. For more information about metrics, see the [Metrics][10] developer page. You can also set up [custom metrics][11], so if the integration doesn't offer a metric out of the box, you can usually add it.

**metadata.csv** - The file that lists and stores the metrics collected by each integration.

**custom check** - If you have a unique system that you want to monitor, or if you're going to expand the metrics already sent by an integration, you can build a [custom check][12] to define and send metrics to Datadog. However, if you want to monitor a generally available application, public service, or an open source project, and we don't have an integration that already exists for it, you should consider [building a new integration][1] instead of a custom check.
...
In order to download the the Datadog Agent package, you need an [API and Application key][15]. You can manage your accounts API and Application keys in the [API Settings page][16] of the UI.

### Installation

If you want to connect with a cloud service provider, navigate to that provider on the [Integrations page][17] for specific instructions on how to connect. For most other integrations, start by installing the [Datadog Agent][18]. Most integrations are supported on our containerized agents: [Docker][19], and [Kubernetes][20]. After you've downloaded the Agent, go to the [Integrations page][17] section to find specific configuration instructions for individual integrations.

### Configuration

Configurations are more specifc to [individual integrations][17]. Configure most integrations by editing the `conf.yaml` file, in the `conf.d/` folder at the root of your Agent's configuration directory. Most integration have a sample `conf.yaml` that lists all available configuration options.
...
To validate your configuration, [run the Agent's `status` subcommand][21], and look for new configuration under the Checks section.
...
For information on how Datadog handles your data, and other security considerations, see our [Security documentation][22].

## What's next?

Now that you have your first integrations set up, you can start [exploring all of the metrics][23] being sent by Datadog to your application, and use these metrics to begin setting up [graphs][24] and [alerts][25] to monitor your data.

Also check out our [Logs management][14], [APM][26], and [Synthetics][27] solutions.

## Troubleshooting

The first step to troubleshooting an integration is to use a plugin in your code editor or use one of the many online tools to verify that the YAML is valid. The next step is to run through all of the [Agent troubleshooting][28] steps.

If you continue to have problems, reach out to [our awesome Support team][29].
...
[8]: /agent/guide/agent-configuration-files/?tab=agentv6#agent-configuration-directory
[9]: https://github.com/DataDog/integrations-core/blob/master/http_check/datadog_checks/http_check/data/conf.yaml.example#L13
[10]: /developers/metrics
[11]: /developers/metrics/custom_metrics
[12]: /developers/write_agent_check/?tab=agentv6
[13]: /tagging
[14]: /logs
[15]: /account_management/faq/api-app-key-management
...
[21]: /agent/guide/agent-commands/?tab=agentv6#agent-status-and-information
[22]: /security
[23]: /graphing/metrics/explorer
[24]: /graphing
[25]: /monitors
[26]: /tracing
[27]: /synthetics
[28]: /agent/troubleshooting/?tab=agentv6
[29]: /help
