...
Now that the test is completed, if you did not create any [assertions][11], they are automatically populated. API tests require at least one assertion to be monitored by Datadog. An assertion is defined by a parameter, an optional property, a comparator, and a target value.
...
Once alert conditions are set, create a message for the alert and specify what services and team members to receive the alert notification email and click **Save Test**. You can also use [integrations][4], such as Slack, Pagerduty, webhooks, etc., to receive alert notifications.
...
Datadog also has an [APM integration with Synthetics][8] which allows you to go from a test run that failed to the root cause of the issue by looking at the trace generated by that test run.
...
1. Set up a [Vagrant Ubuntu 16.04 virtual machine][12].
2. Install [Docker][13] on that machine.
3. In the Datadog app, hover over **UX Monitoring** and select *Settings* -> *Private Locations*. Create a new private location:
...
7. If your private location reports correctly to Datadog, you will see the corresponding health status displayed if the private location polled your endpoint less than 5 seconds before loading the settings or create test pages:
  ...
  * In the form when creating a test, below the private locations section:
    ...
    ```
You are now able to use your new private location as any other Datadog managed locations for your [Synthetics API tests][9]. This is specifically useful to monitor any internal endpoints you might have.
...
[10]: /api/?lang=bash#create-a-test
[11]: /synthetics/api_tests/?tab=httptest#assertions
[12]: https://app.vagrantup.com/ubuntu/boxes/xenial64
[13]: https://docs.docker.com/install/linux/docker-ce/ubuntu/#install-docker-ce
