...
API tests **proactively monitor** that your **most important services** are available at anytime and from anywhere. [Single API tests][1] come in six different subtypes that allow you to launch requests on the different network layers of your systems (`HTTP`, `SSL`, `DNS`, `TCP`, `UDP`, and `ICMP`). [Multistep API tests][2] enable you to run HTTP tests in sequence to monitor the uptime of key journeys at the API level.
