...
3. Tags are normalized to lowercase across all integrations. Avoid `CamelCase` in tag keys: cloud providers normalize camelCase inconsistently. For example, AWS converts `TestTag` to `testtag`, while Alibaba Cloud converts `TestTag` to `test_tag`.
