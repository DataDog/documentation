...
| `bearer_token_auth`         | boolean                                 | optional  | false         | Set `bearer_token_auth` to `true` to add bearer token authentication header. Note: if `bearer_token_path` is not set, `/var/run/secrets/kubernetes.io/serviceaccount/token` is used as a default path                                                                |
| `bearer_token_path`         | string                                  | optional  | none          | The path to a kubernetes service account bearer token file, make sure the file exists and mounted correctly. Note: bearer_token_auth should be set to true to enable adding the token to HTTP headers for authentication.                                            |
