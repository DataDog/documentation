...
1. **Confirm Agent configurations, including if APM is enabled**
    ...
    While APM is enabled automatically for Agent 6+, in containerized environments there is an additional configuration step for [non local traffic][8] that can be the solution to traces not being received.
