...
Your logger needs to have a `source` that sets the `trace_id` mapping correctly. The default source for .NET Applications, `csharp`, does this automatically. For more information [see the FAQ on correlation troubleshooting](/tracing/faq/why-cant-i-see-my-correlated-logs-in-the-trace-id-panel.md#trace_id-option).
