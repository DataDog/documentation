...
| `DD_TRACE_QUEUE_SIZE`                               | Sets the maximum number of traces that can be queued between flushes to the Datadog Agent. Once the size limit of the queue is reached, traces are discarded. Default is `1000` |
...
| `DD_TRACE_ADONET_EXCLUDED_TYPES`<br/><br/>`AdoNetExcludedTypes` | Sets a list of `AdoNet` types that will be excluded from automatic instrumentation. (e.g. : `DD_TRACE_ADONET_EXCLUDED_TYPES=System.Data.SqlClient.SqlCommand`) |
