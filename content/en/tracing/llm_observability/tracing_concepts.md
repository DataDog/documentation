...
- Inputs and Outputs to the traced operation (e.g. prompts and completions generated by an LLM)
- Operation metadata (e.g. LLM parameters such as `temperature`, `max_tokens`)
- Operation metrics (e.g. `prompt_tokens`, `completion_tokens`)
- Tags

In addition, LLM Observability categorizes spans by their span kind, which defines the type of work the span is performing. This can give you more granular insights on what operations are being performed by your LLM application. LLM Observability currently supports the following span kinds:
- LLM: A call to an LLM.
- Workflow: A sequence of operations which include LLM calls and any surrounding contextual operations.
