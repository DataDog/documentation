...
* **[Span tags](#span-tags-attributes)**: Also called span attributes.

* **[Infrastructure tags](#infrastructure-tags)**: Spans inherit tags from hosts and integrations that generate them.
...
- Tag searches are case sensitive. Use free text search to get case insensitive results.

- Searching for a tag value that contains special characters requires escaping or double quotes.
For example, for a tag `resource_name` with the value `/test/test`, search using: `resource_name:\/test\/test` or `resource_name:"/test/test"`. To match a single special character or space, use the `?` wildcard. For example, for a facet `resource` with the value `GET /test`, search using: `resource:GET?\/test`.
...
If your tags don't follow [tags best practices][1] and don't use the `key:value` syntax, use this search query:
...
[Saved views][2] contain your search query, columns, time range and facets.
...
**Note**: Select the **Live Explorer** view (`15 minutes`) to view all [ingested][4] spans. For longer time frames, view spans that are [indexed][5] by retention filters.

### Displaying a full trace

Click on any span to see open the [Trace Panel][6] and access the request context.
...
Visualize the top values from a facet according to a chosen [measure][7] (the first measure you choose in the list), and display the value of additional measures for elements appearing in this top list. Update the search query or investigate logs corresponding to either dimension.
...
Select or click on a section of the graph to either zoom in the graph or see the list of [traces][8] corresponding to your selection:
...
* To a new [APM monitor][9]
* To an existing [Dashboard][10]. 

**Note:** Analytics can be exported only when powered by [indexed spans][11].

## Traces in dashboard

Export [Analytics][4] from the trace search or build them directly in your [Dashboard][10] alongside metrics and logs.
...
[1]: /getting_started/tagging/#tags-best-practices
[2]: /tracing/trace_explorer/saved_views/
[3]: /dashboards/guide/custom_time_frames/
[4]: /tracing/trace_explorer/#live-search-for-15-minutes
[5]: /tracing/trace_explorer/#indexed-spans-search-with-15-day-retention
[6]: /tracing/trace_explorer/trace_panel
[7]: /tracing/trace_search_and_analytics/query_syntax/#measures
[8]: /tracing/visualization/#trace
[9]: /monitors/create/types/apm/
[10]: /dashboards/
[11]: /tracing/visualization/#indexed-span
