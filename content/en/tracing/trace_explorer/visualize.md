...
For example, you can use request flow maps to quickly identify high-traffic services or track the number of database calls generated by a request to a specific endpoint. If you use [shadow deployments][9] or feature flags set as custom span tags, you can use request flow maps to compare request latencies between requests and anticipate how code changes will impact perforamnce. 
...
{{< img src="tracing/live_search_and_analytics/request_flow_map/RUMService.mp4" alt="A video showing how to nvagiate into the service details of a RUM application in the request flow map" video=true style="width:100%;">}}
