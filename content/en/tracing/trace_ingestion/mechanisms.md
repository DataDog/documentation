...
A request from a web or mobile application generates a trace when the backend services are instrumented. Thanks to the [APM integration with Real User Monitoring][5], link your web and mobile application requests to their corresponding backend traces. This combination enables you to see your full frontend and backend data through one lens.

From the RUM browser SDK `v4.10.0`, you are able to control ingested volumes and only keep a share of the backend traces by configure the `tracingSampleRate` initialization parameter. `tracingSampleRate` should be a number between `0` and `100`.
By default, 100% of the traces coming from the browser requests are sampled.
...
| reactnative | `tracingSamplingRate` | [1.0.0-rc6][10]   |

### Synthetic Traces
`ingestion_reason:synthetics` and `ingestion_reason:synthetics-browser`

HTTP and browser tests generate traces when the backend services are instrumented. Thanks to the [APM integration with synthetics][11], link your synthetic tests with the corresponding backend traces. Seamlessly navigate from a test run that potentially failed to the root cause of the issue by looking at the trace generated by that test run.

By default, 100% of the HTTP and browser tests generate backend traces.
