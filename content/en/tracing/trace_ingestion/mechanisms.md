...
|------------|-------------------------------------|---------------------------------|
| Synthetic Monitoring | `synthetics` and `synthetics-browser` | An HTTP or browser test generates a trace when the backend services are instrumented. [Find the backend trace][4] from the synthetic test run.
| Real User Monitoring        | `RUM`                               | A browser request from a web or mobile application generates a trace when the backend services are instrumented. [Find the backend trace][5] from the RUM browser sessions and resources. |
| Serverless | `lambda` and `xray`                   | Your traces received from the [Datadog Lambda Extension][6] or AWS X-Ray integration |
