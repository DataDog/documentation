...
<div class="alert alert-warning">Version tags like <code>v1.11.3</code> are rolling tags. The underlying Datadog NGINX module may be updated while the tag stays the same. To pin an exact version, reference the image by its digest (for example, <code>datadog/ingress-nginx-injection@sha256:...</code>).</div>
