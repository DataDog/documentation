...
- [Python][18]
- [.NET][19]
  - **Note**: For .NET library injection, if the application container uses a musl-based Linux distribution (such as Alpine), and you want to use version 2.60 and lower, you must specify a tag with the `-musl` suffix for the pod annotation. For example, to use library version `v2.29.0`, specify container tag `v2.29.0-musl`. For later versions in V2 and V3, no `-musl` suffix is required.
- [Ruby][20]
