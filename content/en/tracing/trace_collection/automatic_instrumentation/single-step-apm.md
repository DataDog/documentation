...
1. Run the one-line installation command:
   ```shell
   DD_APM_INSTRUMENTATION_ENABLED=docker DD_INSTALL_ONLY=true DD_NO_AGENT_INSTALL=true bash -c "$(curl -L https://install.datadoghq.com/scripts/install_script_agent7.sh)"
   ```
...
```shell
DD_APM_INSTRUMENTATION_ENABLED=docker DD_INSTALL_ONLY=true DD_NO_AGENT_INSTALL=true DD_APM_INSTRUMENTATION_LIBRARIES="java:1.25.0,python" bash -c "$(curl -L https://install.datadoghq.com/scripts/install_script_agent7.sh)"
```
