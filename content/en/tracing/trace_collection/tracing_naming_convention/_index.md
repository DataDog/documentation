...
| `http.url`                                  | `string` | The URL of the HTTP request, including the obfuscated query string. For more informations on obfuscation refer to /tracing/setup_overview/configure_data_security/                                                           |
...
| `http.request.headers.*`                    | `string` | The request HTTP headers. None are collected by default but can optionally be configured with `DD_TRACE_HEADER_TAGS`.<br>To know more about how tho collect headers, refer to the corresponding [Library configuration][5].  |
| `http.response.headers.*`                   | `string` | The response HTTP headers. None are collected by default but can optionally be configured with `DD_TRACE_HEADER_TAGS`.<br>To know more about how tho collect headers, refer to the corresponding [Library configuration][5]. |
