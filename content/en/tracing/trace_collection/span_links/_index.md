...
``datadog-agent>=7.52.0`` is required to generate Span Links using [Datadog tracing libraries][7]. Support for span links was introduced in the following releases:
...
Note: This section documents minimum support for generating span links with Datadog APM client libraries. Span links generated by the OpenTelemetry SDK are sent to Datadog through [OTLP Ingest][8].
