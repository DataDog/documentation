...
The following configuration options are uniformly supported across SDKs where implemented:
...
**Description**: Enables or disables instrumentation for the specified `<INTEGRATION>`. he integration name must be in uppercase (e.g., `DD_TRACE_KAFKA_ENABLED=true`)
...
**Description**: Applies a regex to redact sensitive data from query strings on incoming HTTP requests. Matches are replaced with `<redacted>`. If an empty string is passed, no obfuscation occurs. The resulting value is reported in the `http.url` tag.
...
**Description**: Specifies the directory where tracer log files should be routed. If the directory does not exist, the SDK will fallback to its default method of logging diagnostics.

### Agent

`DD_TRACE_AGENT_URL`
: **Default**: `http://localhost:8126` <br>
**Supported Input**: A string representing a host name <br>
...
**Description**: Sets the default service name used for most spans. In some cases, tracers may set a different service name for downstream services. Integration spans may utilize their own default names, which can differ from the value specified in `DD_SERVICE`

`DD_ENV`
: **Default**: `null` <br>
**Supported Input**: A string representing an application environment name (e.g., `prod`, `dev`) <br>
...
: **Default**: `400-499` <br>
**Supported Input**: A comma-separated string of the form `from-to`, where `from` and `to` are integers. Singular values are also accepted (e.g., `400-403,405,410-499`). <br>
**Not Supported In**: Node.js SDK<br>
**Description**: Defines inclusive the range of status codes to be considered as errors on automatically collected HTTP client spans. Only the values within the specified range are considered errors.
