...
Starting in Agent [v7.60+][1], Datadog uses dynamic resource calculation for init containers that inject SDKs. Instead of using fixed values, the init containers temporarily request all available CPU and memory for the pod, without affecting how the pod is scheduled. (Prior to v7.60, init containers used conservative defaults: `50m` for CPU and `20Mi` for memory.)
