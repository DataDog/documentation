...
**Note**: If your applications and services are instrumented with OpenTelemetry libraries and you set up sampling at the SDK level and/or at the collector level, APM metrics are based on the **sampled** set of data by default. Use the [Datadog processor][17] to keep the metrics accuracy while using collector-level sampling. Please read [Ingestion Sampling with OpenTelemetry][4] for more information.
