...
**Note**: If your applications and services are instrumented with OpenTelemetry libraries and you set up sampling at the SDK level and/or at the collector level, APM metrics are based on the **sampled** set of data by default. Use the [Datadog Processor][4] to keep the metrics accuracy, while using collector-level sampling. See [Ingestion Sampling with OpenTelemetry][5] for more information.
...
[Metrics from spans][6] are based on ingested spans.
...
Count-based [**Trace analytics**][7] monitors are impacted as well. Check if you have trace analytics monitors created by looking for `type:trace-analytics` monitors in the manage monitors page.
...
- **Top sampling decision makers**: which upstream services are taking sampling decisions for the spans ingested in regards to the [default ingestion mechanism][8]

An [out-of-the-box dashboard][9] is also available to get more insights on historical trends related to your ingestion usage and volume. Clone this dashboard to be able to edit widgets and perform further analysis.
...
To reduce the ingestion volume at the Agent level, configure `DD_APM_MAX_TPS` (set to `10` by default) to reduce the share of head-based sampling volume. Read more about the [default sampling mechanism][8].
...
Additionally, to reduce the volume of [error][10] and [rare][11] traces:
...
**Note:** The application needs to be redeployed in order to apply the configuration changes. Datadog recommends applying the changes by setting [environment variables][12].
...
- [TraceIdRatioBased][13] and [ParentBased][14] are 2 built-in samplers that allow you to implement deterministic head-based sampling based on the trace_id at the **SDK** level.
- The [Tail Sampling Processor][15] and [Probabilistic Sampling Processor][16] allow you to sample traces based on a set of rules at the **collector** level.
...
| `analytics`          | Agent and Tracing Libraries | [Deprecated ingestion mechanism][17] that samples single spans without the full trace.   | null                 |
...
[4]: https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/datadogprocessor
[5]: /opentelemetry/guide/ingestion_sampling_with_opentelemetry/
[6]: /tracing/trace_pipeline/generate_metrics/
[7]: /monitors/types/apm/?tab=analytics
[8]: /tracing/trace_pipeline/ingestion_mechanisms/#head-based-sampling
[9]: /tracing/trace_pipeline/metrics/
[10]: /tracing/trace_pipeline/ingestion_mechanisms/#error-traces
[11]: /tracing/trace_pipeline/ingestion_mechanisms/#rare-traces
[12]: /tracing/trace_pipeline/ingestion_mechanisms/?tab=environmentvariables#in-tracing-libraries-user-defined-rules
[13]: https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/trace/sdk.md#traceidratiobased
[14]: https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/trace/sdk.md#parentbased
[15]: https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/tailsamplingprocessor/README.md
[16]: https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/probabilisticsamplerprocessor/README.md
[17]: /tracing/legacy_app_analytics
