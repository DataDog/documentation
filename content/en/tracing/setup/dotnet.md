...
The following table lists the supported configuration variables. Tracer settings can be accessed through properties on `Tracer.Settings`.
...
In addition to the settings listed in [Getting Started][3], the following tables list configuration variables specific to automatic instrumentation.

Tracer settings can be accessed through properties on `Tracer.Settings`:
...
Integration settings can be accessed through properties on `Tracer.Settings.Integrations["<INTEGRATION>"]`. Integration names are listed in the [Integrations][4] section below.
