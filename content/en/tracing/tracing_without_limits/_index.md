...
Live data is all [Ingested spans](#ingestion-controls) and is available in real-time for the past 15 minutes.  The Datadog UI will also have a 'Live' indicator next to the time selector whenever you are in Live mode.  Historical data is all [Indexed spans](#indexing-controls).  Both Ingestion and Indexing can be customized via the [Trace Ingestion and Indexing](#ingestion-and-indexing-overview) features.
...
The root span of any trace with at least one indexed span is automatically indexed.  This means that tags on the root span, or the indexed child spans can be used to power your search. These spans are kept by Datadog for 15 days after being indexed.
...
In app, on the ['Data Ingestion' view][4], you can see the below information:
...
If the numbers you are seeing are not in line with your expectations, or you want to change your ingestion or indexing rates, refer to the [Indexing](#indexing-controls) or [Ingestion](#ingestion-controls) documentation.
...
Each retention filter set on your services, including the default [Datadog Intelligent Sampling Filter](#datadog-intelligent-sampling-filter) will result in an increase to the number of Indexed Spans for your Datadog account.
