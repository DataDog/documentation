...
1. Download `dd-java-agent.jar`, which contains the Java Agent class files, and add the `dd-trace-java` version to your `pom.xml` or equivalent:
     ...
     **Note**: Profiling is available in the `dd-java-agent.jar` library in version 0.44+.
...
2. Update your service invocation to look like:
    ...
    **Note**: If you are on our EU site, please add `-Ddd.site=datadoghq.eu` or set `DD_SITE` in environment variables. This option is available in the `dd-java-agent.jar` library in version 0.48+.
...
| `-Ddd.site`                   | DD_SITE                   | Destination site for your profiles. Valid options are datadoghq.com for the Datadog US site (default), and datadoghq.eu for the Datadog EU site. |
