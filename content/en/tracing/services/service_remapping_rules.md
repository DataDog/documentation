...
- **Service naming hierarchy:** The full service naming hierarchy is as follows: service remapping rules > service defined in code (tracer.Start(WithService(xx))) > service defined in the system property (-Ddd.service={}) > service defined in env var (DD_SERVICE) > service defined in the config file.
