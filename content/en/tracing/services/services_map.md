...
A node is collapsed when there are two services in the filter (applied through the search bar or facets) that are connected by 1+ services which are not in the filter.
