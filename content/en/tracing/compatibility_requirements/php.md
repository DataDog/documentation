...
Starting with ddtrace version `0.48.0`, the PHP tracer uses the `zend_execute_ex` hook on PHP 5. The PHP tracer will emit a warning when the call stack reaches `512` frames deep. The warning can be disabled by setting the environment variable `DD_TRACE_WARN_CALL_STACK_DEPTH=0`.
...
For example: if the existing stack size is `8192`, you may which to double it by setting a new stack size of `16384`.
