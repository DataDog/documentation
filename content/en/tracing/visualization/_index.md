...
| [Trace metrics](#trace-metrics)         | Trace metrics are automatically collected and kept with a 15-month retention policy similar to other [Datadog metrics][17]. They can be used to identify and alert on hits, errors, or latency.                                                                                                    |
...
Trace metrics are automatically collected and kept at a 15-month retention policy similar to any other [Datadog metric][17]. They can be used to identify and alert on hits, errors, or latency. Trace metrics are tagged by the host receiving traces along with the service or resource. For example, after instrumenting a web service trace metrics are collected for the entry-point span `web.request` in the [Metric Summary][20]. 
...
Trace metrics are useful for monitoring. APM monitors can be set up on the [New Monitors][18], [Service][4], or [Resource][5] page. A set of suggested monitors is available on the [Service][4], or [Resource][5] page.
...
To get started tagging spans in your application, check out this [walkthrough][15].
