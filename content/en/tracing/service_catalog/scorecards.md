...
Service scorecards help you monitor, prioritize, plan, and communicate effectively to take informed actions that improve your service's health and performance. Each scorecard shows status for Production Readiness, Observability Best Practices, and Documentation & Ownership. All eligible services in Service Catalog are automatically evaluated against a set of pass-fail criteria.

## How services are evaluated

On the [Service Catalog][1], click **Scorecards**. Click the section titles to see the rules that services are evaluated for.
...
Specified on-call
: Improve the on-call experience for everyone by establishing clear ownership of your services. This will give your on-call engineers the correct point of contact during incidents, reducing the time it takes to resolve your incidents. 
...
The scorecard summary is accessible in the Service Catalog **Explore** page under the **Score** column in the **Ownership** tab. Here, you can see how your specific service or subset of services is doing per each scorecard, and the rules within each. 

Click **View Details** from the scorecard hover, or open the Service details side panel to see the **Scorecards** tab, which lists all the scorecards, the rules, and that service's pass-fail score for each.
