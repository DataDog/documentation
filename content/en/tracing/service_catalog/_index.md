...
The second dropdown allows you to rescope any APM data you have in the Performance view to the [second primary tag][12] on APM [trace metrics][13]. This dropdown does not affect how many services you see in the list. RUM applications are not associated any specific environments (unlike APM or USM) and are only viewable when you select the `env*` option in the environment dropdown.  
