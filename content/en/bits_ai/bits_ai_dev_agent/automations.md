...
## Overview
Create an automation to have Bits Code start a [session][1] when a trigger fires, such as a new Code Security finding or a recurring schedule, then deliver the results as a pull/merge request or Slack notification.
...
- Route the resulting code changes directly to a pull/merge request or notify a team in Slack
...
- **Create PRs based on APM Recommendations**: Generates pull/merge requests based on APM Recommendations for a specific service.
- **Fix frequent errors for a repo**: Uses the [**Custom Prompt**](#custom-prompt-trigger) trigger to instruct Bits Code to scan the last 24 hours of logs, find the most frequent error, and open a pull/merge request with a fix.
...
To limit how many Bits Code sessions the automation can create in a given period (for example, `5 runs per Week`), click **Add Trigger** > **Set max runs**. One automation execution may produce more than one session. Use this setting to control the volume of pull/merge requests or notifications an automation produces.
...
<div class="alert alert-warning">Each finding that triggers an automation is tied to a single session. Multiple findings cannot be fixed in a single session or pull/merge request.</div>
...
## Outputs
An output defines what Bits Code does after a [session][1] completes. An automation can have one or more outputs, including [opening a pull/merge request](#pull-merge-request-output) and [generating a Slack notification](#slack-message-output).

### Pull/merge request output
You can configure your automation to:
- **Create a PR/MR**: Open a pull/merge request with the proposed changes
- **Draft a PR/MR**: Open a draft pull/merge request with the proposed changes

As the author of a Bits Code automation, you are the author of all pull/merge requests it generates.

### Slack message output
You can configure your automation to send a Slack message summarizing the [session][1] and code changes. If you use a pull/merge request output in addition to a Slack output, Bits Code includes a link to the pull/merge request in the Slack message.
