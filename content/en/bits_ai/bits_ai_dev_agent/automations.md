...
## Overview
Create an automation to have Bits Code start a [code session][1] when a trigger fires, such as a new Code Security finding or a recurring schedule, then deliver the results as a pull request or Slack notification.
...
You can [create a custom automation][13], or [use a Datadog-provided automation template][14].
...
1. In the **Trigger** section, configure a [trigger][2].
1. In the **Output** section, configure one or more [outputs][3].
...
- **Fix frequent errors for a repo**: Uses the [**Custom Prompt**][9] trigger to instruct Bits Code to scan the last 24 hours of logs, find the most frequent error, and open a pull request with a fix.
...
To limit how many code sessions the automation can create in a given period (for example, `5 runs per Week`), click **Add Trigger** > **Set max runs**. One automation execution may produce more than one code session. Use this setting to control the volume of pull requests or notifications an automation produces.

### Product source trigger
A product source trigger runs the automation in response to new findings in another Datadog product (for example, Error Tracking or Code Security). You can use a product source trigger by itself, which runs the automation whenever there is a new finding, or with a [schedule][10] and lookback window you define (in the **New findings within** field).

Setting up a product source trigger requires configuring additional filters, which vary by product. For example:
...
### Schedule trigger
A schedule trigger controls when an automation runs. It can be used in combination with a [product source][8] or a [custom prompt][9]. When setting a schedule, you can choose from:
...
## Outputs
An output defines what Bits Code does after a [code session][1] completes. An automation can have one or more outputs, including [opening a pull request][12] and [generating a Slack notification][7].
...
You can configure your automation to **Send Slack message**, which sends a Slack message about the [code session][1].

When you add a Slack message output, by default, Bits Code sends the message to the channel configured for the affected service in [Catalog][11]. You can set a fallback Slack channel, which is used when no channel is set in Catalog.

## Manage automations
On [**Automations**][5], view the automations you created on the **My Automations** tab. Switch to **All** to see automations created by anyone in your organization.
...
- You cannot define Bits Code automations in code; they must be created through the form builder in Datadog.
...
[7]: #slack-message-output
[8]: #product-source-trigger
[9]: #custom-prompt-trigger
[10]: #schedule-trigger
[11]: /internal_developer_portal/catalog/
[12]: #pull-request-output
[13]: #create-a-custom-automation
[14]: #create-an-automation-from-a-template
