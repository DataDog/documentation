...
For structured or JSON-like data, use `GetObjectValue(key, defaultValue)`. This method returns an `object`, which can be cast to the appropriate type (such as a dictionary or custom class). Object flags are useful for remote configuration scenarios where multiple properties need to be provided together. For example:
