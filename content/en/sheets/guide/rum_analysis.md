...
1. **Filter RUM Sessions**:
   - Start from the [RUM Sessions Explorer][3] and build a query to filter sessions with two or more errors.

     ```
     @session.error.count:>=2 @session.type:user
     ```
     {{< img src="/sheets/guide/rum_analysis/rum_sessions_explorer.png" alt="RUM Session Explorer showing a query filtering @session.error.count:>=2 and @session.type:user" style="width:80%;" >}}
   - Click **Open in Sheets** to create a table with the filtered sessions.
2. **Enrich Data**:
   - Upload a CSV file to a Reference Table with additional user metadata (such as premium status or team).
   - Use the Lookup function in Sheets to add the premium status to your RUM sessions table.
...
3. **Analyze with Pivot Tables**:
   - Create a pivot table to count the number of errors per premium user.
   - Summarize the data to understand the distribution of errors among premium and non-premium users.
...
By following these steps, you can effectively analyze RUM sessions, enrich your data with additional user information, and gain insights into how your premium users are impacted by errors. Use sheets to help you make data-driven decisions to improve user experience and address issues more effectively.
