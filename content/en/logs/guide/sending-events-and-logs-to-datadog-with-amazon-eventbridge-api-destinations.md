...
1. Follow the steps in the [Amazon Create an API destination docs][5] to add Datadog as an API destination.
    ...
    - For your destination endpoint, use `https://http-intake.logs.datadoghq.com/v1/input` or `https://http-intake.logs.datadoghq.eu/v1/input`   for logs and `https://api.datadoghq.com/api/v1/events` or `https://api.datadoghq.eu/api/v1/events` for events, and set `POST` as the HTTP method. For more information about the differences between logs and events, see the [logs section][6], and the [events section][7] of the [Categories of Data docs page][8].
