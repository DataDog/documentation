---
title: Forward Google Cloud logs to Datadog
kind: documentation
further_reading:
- link: "/logs/explorer/"
  tag: "Documentation"
...
Log events forwarded to Datadog from a Google Cloud Pub/Sub topic with the Datadog Dataflow template is in beta. Datadog recommends using this approach for compression and batching of events forwarded to Datadog. Use the <a href="https://cloud.google.com/products/calculator" target="_blank">Google Cloud Pricing Calculator</a> to calculate any additional costs you may incur.
</div>

For applications running in GCE or GKE, the [Datadog Agent][13] can be used to collect logs locally. Google Cloud service logs are collected with Google Cloud Logging and sent to your Cloud Pub/Sub topic. 

The previous method of using a **Push** subscription with a [Google Cloud Pub/Sub][10] topic is no longer recommended due to the compression and batching enhancements provided by the **Pull** subscription used with the [Datadog Dataflow template][11] (beta).
...
1. Go to the [Cloud Pub/Sub console][2] and create a topic.
...
3. Create a secret in Secret Manager with your valid Datadog API Key.
