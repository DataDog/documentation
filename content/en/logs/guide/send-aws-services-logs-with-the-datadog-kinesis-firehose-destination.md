...
1. Check the `Subscriptions` column on the [log groups index page][1] to see current subscriptions to your relevant log groups. Add the new Kinesis stream as a subscriber.    
   * **Note**: CloudWatch Log groups can only have two subscriptions each. If you have more than two sources you want to subscribe to, you can subscribe to the new Kinesis stream after completing this setup.
