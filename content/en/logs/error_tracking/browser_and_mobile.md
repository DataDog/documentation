...
An essential attribute for log errors is the stack trace in a log's `error.stack`. If you are sending stack traces to Datadog but they are not in `error.stack`, you can set up a [generic log remapper][1] to remap the stack trace to the correct attribute in Datadog.
...
1. Download the latest version of the [Datadog Android SDK for Logs][2].
2. Configure your application's `version`, `env`, and `service` when [initializing the SDK][3].
...
[3]: /logs/log_collection/android/?tab=kotlin#setup
{{% /tab %}}
{{% tab "iOS" %}}
