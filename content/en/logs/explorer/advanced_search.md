...
1. Optionally, reduce the number of subquery attribute values to match against. The default value and maximum value is `1000`. Choose between {{< ui >}}top{{< /ui >}} (highest frequency values) or {{< ui >}}bottom{{< /ui >}} (lowest frequency values).
