...
*The [Python logger][4] has an `extra` parameter for adding custom attributes. Use `DJANGO_DATADOG_LOGGER_EXTRA_INCLUDE` to specify a regex that matches the name of the loggers for which you want to add the `extra` parameter.

## Configure the Datadog Agent

Once [log collection][5] is enabled, set up [custom log collection][6] to tail your log files and send them to Datadog by doing the following:
...
4. Run the [Agent's status subcommand][8] and look for `python` under the `Checks` section to confirm that logs are successfully submitted to Datadog.
...
If APM is enabled for this application, connect your logs and traces by automatically adding trace IDs, span IDs, `env`, `service`, and `version` to your logs by [following the APM Python instructions][11].
...
[4]: https://docs.python.org/3/library/logging.html#logging
