...
5. Run the [Agent's status subcommand][7] and look for `ruby` under the `Checks` section to confirm that logs are successfully submitted to Datadog.
...
If APM is enabled for this application, you can improve the connection between application logs and traces by following the [APM Ruby logging instructions][10] to automatically add trace and span IDs in your logs.
...
[8]: /logs/log_configuration/parsing
[9]: /logs/explorer/
