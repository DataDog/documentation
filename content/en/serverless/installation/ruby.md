...
Configure your Lambda functions. Enable Datadog APM and wrap your Lambda handler function using the wrapper provided by the Datadog Lambda library:
...
Then, use this format for your [Terraform resource][1]:
```sh
...
1. Replace `<DATADOG_TRACER_ARN>` with the ARN of the appropriate Datadog tracer for your type of region:
   ...
   In each ARN, replace `<AWS_REGION>` with a valid AWS region, such as `us-east-1`. Replace `<RUNTIME>` with `Ruby2-7` or `Ruby3-2`.
...
resource "aws_lambda_function" "lambda" {
  ...
  layers = [
    "arn:aws:lambda:us-east-1::464622532012:layer:Datadog-Ruby3-2:20",
