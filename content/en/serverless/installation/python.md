...
If your Python Lambda functions are written in [Python 3.6 or less][2], or you previously set up Datadog Serverless using the Datadog Forwarder, see the [installation instructions here][3]. Currently, Lambda only supports sending data to Datadog in the `us-east-1`, `eu-west-1`, and `eu-south-1` regions.
