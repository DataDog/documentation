...
- `Wait`, `Choice`, `Success`, `Fail`, and `Pass` are currently supported. Support for `Map`, and `Parallel` flows is still in progress. For now, you would be able to see parallel executing spans stacking on top of each other, but there  will be no `Parallel` spans showing on the flame graph.
