...
### Requirements
Node.JS (layer v94+) or Python (layer v75+) runtime 

### Setup

On the Lambda Task, set the `OutputPath` and `Parameters` keys with the following: 
...
The `JsonMerge` [intrinsic function][3] merges the [Step Functions Context Object][4] (`$$`) with the original Lambda's input payload (`$`). Fields of the original payload overwrite the Step Functions context object if their keys are the same.
