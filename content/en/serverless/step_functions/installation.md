...
2. Ensure you have deployed the [Datadog Lambda Forwarder][1], a Lambda function that ships logs from AWS to Datadog, and that you are using v3.74.0+. You may need to [update your Forwarder][2]. When deploying the Forwarder, you can also set the `DdStepFunctionTraceEnabled` parameter in CloudFormation to enable tracing for all your Step Functions at the Forwarder-level.
...
5. Enable tracing on your Step Function by adding a `DD_TRACE_ENABLED` tag. Set the value to `true`. This is only needed if `DD_STEP_FUNCTION_TRACE_ENABLED` hasn't been set by the Forwarder.
