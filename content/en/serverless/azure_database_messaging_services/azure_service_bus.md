...
| .NET (Producer & consumer) | dd-trace-dotnet v3.28.0 | N/A (NuGet) | Azure.Messaging.ServiceBus >= 7.14.0 |

**Note**: dd-trace-dotnet v3.28.0-v3.29.x requires opt-in with an environment variable; v3.30.0 and later enables Service Bus tracing by default.</br>
To enable Service Bus tracing through the OpenTelemetry integration with dd-trace-dotnet v2.38.0 or later, set `DD_TRACE_OTEL_ENABLED=true`.
