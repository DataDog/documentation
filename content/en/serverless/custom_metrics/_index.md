...
When Datadog receives multiple count or gauge metric points that share the same timestamp and same set of tags, only the last-arrived one counts. This works fine for a host-based application because metric points get aggregated by the Datadog agent and tagged with a unique `host` tag.

A Lambda function may launch many concurrent execution environments when traffic increases and they may submit count or gauge metric points that overwrite each other and cause under-counted results. To avoid this problem, custom metrics generated by Lambda functions are submitted as [distributions][2], because distribution metric points are aggregated on the datadog backend, and every metric point counts.

Distributions provide `avg`, `sum`, `max`, `min`, `count` aggregations by default. You can also enable percentile aggregations (p50, p75, p90, p95, p99), and [manage tags][3] on the Metric Summary page. To monitor a distribution like a gauge, use `avg` for both the [time and space aggregations][4]. To monitor a distribution like a count, use `sum` for both the [time and space aggregations][4]. Refer to the guide [Query to the Graph][5] for how time and space aggregations work.

### Submitting historical metrics

To submit historical metrics with timestamps of the past (only <20 mins old is allowed), you must use the [Datadog Forwarder](#with-the-datadog-forwarder). The [Datadog Lambda Extension](#with-the-datadog-lambda-extension) can only submit metrics with the current timestamp due to the limitation of the statsd protocol.

### Submitting many data points

Submitting many data points for the same metric and the same set of tags, for example, inside a big "for" loop, may cause noticeable performance impact to Lambda and cost impact to CloudWatch when using the Forwarder. You can aggregate the data points in your application to avoid the overhead. The idea is better illustrated by the sample code in Python below. 
...
With log-based metrics, you can record a count of logs that match a query or summarize a numeric value contained in a log, such as request duration. Log-based metrics are a cost-efficient way to summarize log data from the entire ingest stream. [Learn more about creating log-based metrics][6].

You can also generate metrics from 100% of ingested spans, regardless of whether they are indexed by a retention filter. Learn more about creating span-based metrics [here][7].
...
- `<METRIC_NAME>` uniquely identifies your metric and adheres to the [metric naming policy][12].
