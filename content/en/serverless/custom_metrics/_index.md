...
Install the Datadog Lambda Library to collect and send custom metrics. Metrics sent from the Datadog Lambda Library are automatically aggregated into [distributions][1], so you can graph the `avg`, `sum`, `max`, `min`, and `count`. You can also calculate aggregations over a set of tags for the 50th, 75th, 95th, and 99th percentile values on the [Distribution Metrics][1] page.
...
2. Update your [Datadog Forwarder][2] to at least version 1.4.0.

If you are not using Datadog Logs, you can still use asynchronous custom metric submission. Set the environment variable `DD_FORWARD_LOG` to `False` on the [Datadog log collection AWS Lambda function][2]. This intelligently forwards only custom metrics to Datadog, and not regular logs.
...
You should tag your custom metrics when submitting them with the [Datadog Lambda Library][4]. Use the [Distribution Metrics][1] page to [customize the set of tags][5] applied to your custom metrics.
...
There are a number of open source libraries that make it easy to submit custom metrics to Datadog. However, many have not been updated to use [Distribution metrics][1], which are optimized for Lambda. Distribution metrics allow for server-side aggregations independent of a host or locally-running [agent][7]. In a serverless environment where there is no agent, Distribution metrics give you flexible aggregations and tagging.
...
**This method of submitting custom metrics is no longer supported, and is disabled for all new customers.** The recommended way to submit custom metrics from Lambda is with a [Datadog Lambda Library][4].

This requires the following AWS permissions in your [Datadog IAM policy][8].
