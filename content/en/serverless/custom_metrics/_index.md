...
Distributions provide `avg`, `sum`, `max`, `min`, `count` aggregations by default. On the {{< ui >}}Metric Summary{{< /ui >}} page, you can enable percentile aggregations (p50, p75, p90, p95, p99) and also [manage tags][3]. To monitor a distribution for a gauge metric type, use `avg` for both the [time and space aggregations][4]. To monitor a distribution for a count metric type, use `sum` for both the [time and space aggregations][4]. Refer to the guide [Query to the Graph][5] for how time and space aggregations work.
