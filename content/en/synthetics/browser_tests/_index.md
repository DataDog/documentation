...
6. **Locations**: The Datadog managed locations to run the test from. Many AWS locations from around the world are available. You can also set up [private locations][2] to run a your browser test from custom locations or from inside private networks.
7. **How often should Datadog run the test?** Intervals are available between every 5 minutes to once per week. The 1 minute frequency is available [upon request][3].
