...
6. Select **managed and private locations**: Set locations around the world that are managed by Datadog to run your test from. You can also create [private locations][1] to run your browser test from custom locations or inside private networks. For a full list of locations, navigate to the [UI][2] or use the [API][3]. {{< site-region region="gov" >}}**Note**: The West US (AWS GovCloud) location is supported on the Datadog for Government site.{{< /site-region >}}
