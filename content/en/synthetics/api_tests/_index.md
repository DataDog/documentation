...
2. Choose the **Method** and **URL** to query. Available methods are: `GET`, `POST`, `PATCH`, `PUT`, `HEAD`, `DELETE`, and `OPTIONS`.
    ...
    **Note**: You can also set up a [Private Location][2] to run a Synthetics API test on a private endpoint not accessible from the public internet.
3. **Name**: The name of your API test.
4. **Select your tags**: The tags attached to your browser test. Use the `<KEY>:<VALUE>` format to filter on a `<VALUE>` for a given `<KEY>` on the [Synthetics page][3].
5. **Locations**: The Datadog managed locations to run the test from. Many AWS locations from around the world are available. The full list is retrievable through the [Datadog API][4]. You can also set up a [Private Location][2] to run the Synthetics API test on a location of your choice.
...
[3]: /synthetics
[4]: /api/?lang=bash#get-available-locations
{{% /tab %}}
...
2. Specify the `Host` and the SSL `Port`. By default, the port is set to _443_. You can also set up a [Private Location][1] to run a Synthetics API test on a private endpoint not accessible from the public internet.
3. **Name**: The name of your API test.
...
5. **Locations**: The Datadog managed locations to run the test from. Many AWS locations from around the world are available. The full list is retrievable through the [Datadog API][3]. You can also set up a [Private Location][1] to run the Synthetics API test on a location of your choice.
