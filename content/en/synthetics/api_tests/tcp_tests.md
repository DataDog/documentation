...
- **Build a test from scratch**:
    ...
    1. To build a test from scratch, click the {{< ui >}}+ Start from scratch{{< /ui >}} template, then select the `TCP` request type.
    1. Specify the {{< ui >}}Host{{< /ui >}} and the {{< ui >}}Port{{< /ui >}} to run your test on.
    1. Decide whether or not to {{< ui >}}Track number of network hops (TTL){{< /ui >}}. This option allows you to assert on the number of network hops and to have access to a TCP Traceroute in your test results.
    1. Specify the amount of time in seconds before the test times out (optional).
    1. {{< ui >}}Name{{< /ui >}} your TCP test.
    1. Add Environment {{< ui >}}Tags{{< /ui >}} as well as any other tag to your TCP test. You can then use these tags to filter through your Synthetic tests on the [{{< ui >}}Synthetic Monitoring & Continuous Testing{{< /ui >}} page][3]. 
...
Assertions define what an expected test result is. When you click {{< ui >}}Test URL{{< /ui >}}, basic assertions on `response time` are added. You must define at least one assertion for your test to monitor.
...
Select the {{< ui >}}Locations{{< /ui >}} to run your TCP test from. TCP tests can run from both managed and [private locations][1] depending on your preference for launching the connection from outside or inside your network.
...
You can use the [global variables defined on the {{< ui >}}Settings{{< /ui >}} page][8] in the URL, advanced options, and assertions of your TCP tests.
