...
1. **Record the step that generates the file download** in your browser test. The example below shows how to record a click on a button that triggers the download of a `.docx` file. The file size must be below 100Mb for tests running on Managed Locations and below 250Mb for tests running on Private Locations.
