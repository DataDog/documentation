...
Synthetic browser tests automatically handle [JavaScript modals][1]:
 ...
 - `prompt` modals are answered with the text returned or `null` if the Cancel button or `Esc` is clicked. `prompt` modals offer a text input field that is filled with `Lorem Ipsum` for tests on Google Chrome or Microsoft Edge.
