...
2. Run this command to boot your private location worker by mounting your configuration file to the container. Ensure that your `<MY_WORKER_CONFIG_FILE_NAME>.json` file is accessible to mount to the container:
   ...
   ```go
   podman run –cap-add=NET_RAW --rm -v $PWD/<MY_WORKER_CONFIG_FILE_NAME>.json:/etc/datadog/synthetics-check-runner.json datadog/synthetics-private-location-worker:latest
   ```

   If you have blocked reserved IP addresses, add the `NET_ADMIN` Linux capabilities to your private location container.
