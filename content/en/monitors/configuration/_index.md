...
Alerts are grouped automatically based on your selection of the aggregation selected for your query (for exanple, `avg by service`). If the query has no grouping, it defaults to `Simple Alert`. If the query is grouped by any dimension, grouping changes to `Multi Alert`.
