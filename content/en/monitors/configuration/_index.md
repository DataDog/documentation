...
- **Evaluate as zero:** This option is available for monitors using `Count` queries without the `default_zero()` function. For Metrics monitors, this option is available for queries where each metric either uses the `sum` space aggregation, is a count of distribution values, or uses one of those functions: `default_zero()`, `count_nonzero()` or `count_not_null()`. 
