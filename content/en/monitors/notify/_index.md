...
But if that creates too much noise, you can control on which dimension you would like to receive notifications on. **Example**: you are measuring CPU usage on your Kubernetes deployed application and currently alert on `cluster`, `namespace`, and `pod`. However, you don't want to be alerted on each pod level issue. You can remove the `pod` tag from the `Separate Notification` section and you will start to receive notifications on `cluster` and `namespace` level. Read more about Notification Grouping in this guide.
