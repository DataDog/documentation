...
`Separate Notification` mode applies the alert to each source according to your group parameters. By default, you receive **an alert for each group** that meets the set conditions. For example, you could group a query looking at a capacity metric by `host` and `device` to receive a separate alert for each host device that is running out of space. Note that if your metric is only reporting by `host` with no `device` tag, it would not be detected by a monitor group by both `host` and `device`. [Tag Variables][2] are available for every group evaluated in the multi-alert to dynamically fill in notifications with useful context.

Furthermore, you can control the granularity on which you want to receive notificatoins on by removing or re-adding tags from your `group by` section. **Example**: you are measuring CPU usage on your Kubernetes deployed applications and currently alert on `cluster`, `namespace`, and `pod`. However, you don't want to be alerted on each pod level issue anymore as you feel like it creates too much noise. You can remove the `pod` tag from the `Separate Notification` section and you will start to receive notifications aggregated on `cluster` and `namespace`. You can find more information on Notification Grouping in this guide.
