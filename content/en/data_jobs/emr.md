...
1. Save the following script to an S3 bucket that your EMR cluster can read. Take note of the path to this script.
   ...
   The script above sets the required parameters, and downloads and runs the latest init script for Data Jobs Monitoring in EMR. If you want to pin your script to a specific version, you can replace the file name in the URL with `install-emr-0.9.1.sh` to use the last stable version. The source code used to generate this script can be found on the [Datadog Agent repository][12].
