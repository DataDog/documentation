...
1. Install the [Datadog Agent][1] (version 6.41 or 7.41 and later) with the network kernel device driver component enabled.
   For Agent version 7.44 or earlier, you must pass `ADDLOCAL="MainApplication,USM"` to the `msiexec` command during installation, or select **Universal Service Monitoring** when running the Agent installation through the GUI.
