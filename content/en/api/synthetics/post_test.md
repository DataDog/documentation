...
*   **`operator`** - _required_ - The action that you want tested. Valid operators depend on the property being tested.
...
*   **`locations`** - _optional_ - A list of the locations that you want the tests to be sent from. Valid values are `aws:` followed by any valid AWS region, for example, `aws:ap-south-1`. For a full list of valid AWS regions, see [the AWS documentation][1]. [do we really support all of them?]
