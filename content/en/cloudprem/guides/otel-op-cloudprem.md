---
title: Send OpenTelemetry Logs to CloudPrem via Observability Pipelines
...
2. Create an Observability Pipeline with a custom processor to add tags
3. Run the Observability Pipelines Worker
4. Send OpenTelemetry logs using the Python SDK
5. View tagged logs in Datadog
...
**Note**: The custom processor adds a `ddtags` field with custom tags to all logs via the `remaps` configuration.
