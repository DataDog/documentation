...
For more information, see [Search Syntax][2].

## Explore test runs

The Explorer displays all of your test runs from [Synthetic Monitoring][3] and [Continuous Testing][4]. Every test corresponds to a test run for a particular test subtype, including fast retries. Click on a test in the Explorer to access the test run page.
...
For more information about test runs, see [Search Test Runs][5].

## Explore test batches

The Explorer displays batches of tests run by [Continuous Testing and your CI/CD provider][6]. Every batch corresponds with a call to the Datadog API (through one of your [CI/CD integrations][6], the [datadog-ci][7] NPM package, or directly through the API endpoint) and triggers one or several test executions. 
...
For more information about test batches, see [Search Test Batches][8].
