...
**Note**: Security Filters are only required to control logs analyzed by the Cloud SIEM product. You do not need to write Security Filters to exclude logs generated by the Datadog Agent as part of the Cloud Workload Security (`source:runtime-security-agent`) and Cloud Security Posture Management (`source:compliance-agent`) products, as they're not billed as analyzed logs regardless.
...
[1]: /security_platform/default_rules#cat-cloud-siem
