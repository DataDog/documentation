...
To manually add or update rules, load a new `default.policy` file to the Agent. Follow the instructions below:

1. Navigate to the [Rules page][4] in Datadog.
2. Click **New Rule** button in the top right.
...
5. Define the logic for when this rule triggers a security signal. For example, `a>0` means a security signal triggers as long as the rule condition set in step 4 is met at least once in the sliding time window. Select a severity to associate the rule with and select all relevant parties you to notify.
...
6. Name the rule and add the actual notification message in markdown format. It is recommended to include a remediation runbook in the body. As noted in the template, use substitution variables to dynamically generate contextualized content at runtime. After the message, add multiple tags to give more context to the signals generated by your custom rule.

7. Save the rule. This automatically navigates you back to the main Rules page. Click the **Download Workload Security Policy** button in the top right corner to download a `default.policy` file to your local machine.
...
9. Add the configMap (`jdefaultpol`) into values.yaml, `datadog.securityAgent.runtime.policies.configMap`:
...
10. Upgrade the helm chart with the following command: `helm upgrade <RELEASENAME> -f values.yaml --set datadog.apiKey=<APIKEY> datadog/datadog`
