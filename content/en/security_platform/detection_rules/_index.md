...
The [Security Rules][6] page lets you search all Detection Rules. Quickly enable, disable, edit, delete, clone, or view signals generated by any of these detection rules. To create a custom [security][3] rule, click on the **New Rule** button in the top right corner of the page.

**Note**: Custom detection rules are only available for Cloud SIEM.
...
Use facets in the left panel to scope a search query by value. For example, if you have several rule sources and need to troubleshoot on detection rules provided by one source, hover over a source value in the panel, such as `cloudtrail` or `kubernetes`, and click **only** to narrow the search to that source.
...
Rules are displayed in the detection rules table.

Columns can be added or removed with the options menu.

Rules are sorted alphabetically—ascending by default (A-Z). The detection rules can be inverse-sorted by name, query name, creation date, or rule ID.
