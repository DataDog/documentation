...
By using the existing [role-based access control (RBAC) authorization method provided by Kubernetes](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) and authorizing [our existing read-only IAM entity role](https://help.cloudcraft.co/article/108-how-cloudcraft-connects-to-aws), Cloudcraft requires no special software or agent to look inside your clusters.
...
- [Create your first live AWS diagram](https://help.cloudcraft.co/article/99-create-your-first-cloudcraft-diagram)
...
Our IAM role is now allowed to have view-only access, but that alone is not enough, you still need to bind the IAM role to a Kubernetes role with a [ClusterRoleBinding](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding).
