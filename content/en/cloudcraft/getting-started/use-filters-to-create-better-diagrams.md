...
To avoid these issues, Cloudcraft recommends that you use the {{< ui >}}Filtered layout{{< /ui >}} feature to apply filters, or exclude services when placing live components.
...
The search box on the {{< ui >}}Live{{< /ui >}} tab allows you to enter patterns that affect your scan result.
...
After scanning your AWS account, click {{< ui >}}Auto Layout{{< /ui >}} on the {{< ui >}}Live{{< /ui >}} tab to view a two-column list with services from your AWS environment.

You can move services from the {{< ui >}}Included services{{< /ui >}} to the {{< ui >}}Excluded services{{< /ui >}} column—or vice versa—by clicking them.
...
1. On the {{< ui >}}Live{{< /ui >}} tab, type the search pattern that corresponds to your query in the search box. In this example, the pattern is `service=wirecraft AND (type=ec2 running OR type=ebs)`. Notice that the button {{< ui >}}Auto Layout{{< /ui >}} now says {{< ui >}}Filtered Layout{{< /ui >}}.
2.  Click {{< ui >}}Filtered Layout{{< /ui >}}.
3. Click {{< ui >}}Layout{{< /ui >}}. The components in the diagram now match the pattern in Step 1.

Other alternatives include:

- Running the same query on another AWS region. Before you click {{< ui >}}Layout{{< /ui >}}, select {{< ui >}}Include existing components{{< /ui >}} from the {{< ui >}}Options{{< /ui >}} dropdown. Doing so would perform a filtered layout on all the components for the secondary region currently in your inventory and all the components already on the diagram.
- Combining {{< ui >}}Filtered layout{{< /ui >}} with the {{< ui >}}Blueprint link{{< /ui >}} feature to break down larger environments into multiple diagrams that link to each other. You can also have an overview diagram that provides a quick glance of your whole cloud architecture with no performance penalties.
