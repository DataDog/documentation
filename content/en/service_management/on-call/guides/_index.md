---
title: On-Call Guides
private: false
disable_toc: true
---

{{< whatsnext desc="General guides:">}}
    {{< nextlink href="/service_management/on-call/guides/configure-mobile-device-for-on-call" >}}Set up your mobile device for Datadog On-Call{{< /nextlink >}}
    {{< nextlink href="/service_management/on-call/guides/migrating-from-your-current-providers" >}}Migrate from your current on-call provider{{< /nextlink >}}
    {{< nextlink href="/service_management/on-call/guides/migrate-your-pagerduty-resources-to-on-call" >}}Migrate PagerDuty resources to Datadog On-Call{{< /nextlink >}}
    {{< nextlink href="/service_management/on-call/guides/offboarding-teams-and-users" >}}Offboard teams and users from Datadog On-Call{{< /nextlink >}}
{{< /whatsnext >}}
