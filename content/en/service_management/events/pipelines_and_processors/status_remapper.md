...
**Note**: If multiple event status remapper processors are applied to a given event within a pipeline, only the first one in the pipeline’s order is considered. Additionally, across all pipelines that match the event, only the first status remapper encountered (from all applicable pipelines) will be applied.
