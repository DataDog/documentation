...
This guide is intended to help you resolve issues with third-party integrations. If you continue to have trouble, reach out to [Datadog support][1] for further assistance.

## Jira

Jira issue types with custom fields, private Jira projects, and on-premises Jira instances are not supported. For automatic Jira creation with syncing:

### Configuration

1. If Jira projects are not populating the dropdown on the Jira integration configuration screen, check that you have `manage_integrations` permission. 
...
1. If you're updating the status of a case in a way that is disallowed by a Jira workflow, the case will fall out of sync with the status mapping. 
1. Updates to comments, including deletions, on either Case Management or Jira are not reflected on the other side. 
