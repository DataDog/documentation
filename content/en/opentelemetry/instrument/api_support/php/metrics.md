...
{{% otel-overview-native lang="PHP" signal="Metrics" sdk_name="dd-trace-php" %}}

## Prerequisites

- **Datadog SDK**: dd-trace-php version [x.y.z] or later.
- **OpenTelemetry API**: `open-telemetry/api` version [x.y.z] or later. (The Datadog SDK provides the implementation for this API).
...
1. Add the Datadog SDK (`dd-trace-php`) to your project and enable its instrumentation.
...
3. Remove the `open-telemetry/sdk` and `open-telemetry/exporter-otlp` packages from your project's dependencies.
