...
- **Datadog SDK**: dd-trace-go version [x.y.z] or later.
- **OpenTelemetry SDK**: The standard opentelemetry-sdk package for Go.
- **OTLP Exporter**: The standard `opentelemetry-exporter-otlp` package for Go.
...
2. Install the OTel SDK and Exporter:
   ```sh
   go get go.opentelemetry.io/otel/sdk go.opentelemetry.io/otel/exporters/otlp
   ```
...
1. Add the Datadog SDK (`dd-trace-go`) to your project and enable its instrumentation.
