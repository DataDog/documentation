...
## Traces and metrics correlation

Traces and metrics can be correlated using container tags obtained from attributes generated by the [`k8sattributesprocessor`][19]. For an example configuration on setting it up, please take a look at our [example file][8] in the exporter repository. To ensure that the attributes are correctly added to both the Collector and application pods, it is important to set the `OTEL_RESOURCE_ATTRIBUTES` environment variable to specify the `k8s.pod.ip` attribute pointing to the pod IP obtained from the downwards API (such as for example [here][20]). For a full example, check our [example Kubernetes chart][21].
...
Container metrics in Datadog are generated by the [dockerstats][27] receiver. When used in conjunction with the Datadog Exporter, it translates them to their Datadog counterparts. Besides the default attributes, some additional ones need to be enabled. To do so, use the following configuration:
