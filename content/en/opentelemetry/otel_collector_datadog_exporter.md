...
Traces and metrics can be correlated using container tags obtained from attributes generated by the [`k8sattributesprocessor`][19]. For an example setup configuration, see the [example file][8] in the exporter repository.

To ensure that the attributes are correctly added to both the Collector and application pods, set the `OTEL_RESOURCE_ATTRIBUTES` environment variable to specify the `k8s.pod.ip` attribute pointing to the pod IP obtained from the downwards API (such as this [example][20]). For a full example, see the [example Kubernetes chart][21].
