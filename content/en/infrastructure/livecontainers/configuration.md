...
**Note**: The additional sensitive words needs to be in lower-case as the agent compares the text with the pattern in lower-case as well. This means `password` would scrub `MY_PASSWORD` to `MY_*******` while `PASSWORD` would not.
