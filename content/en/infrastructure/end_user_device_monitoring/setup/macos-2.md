...
- **Manual install**: Run a one-line script directly on the device. Use this method to test a single machine or for devices that are not managed by a mobile device management (MDM) solution.
...
1. In Datadog, go to [Fleet Automation][2] and select **Install Agents** > **macOS**.
1. Select an existing API key or create a new one.
1. Copy the install command that Datadog displays. It is similar to the following, with your API key in place of the placeholder:

   {{< code-block lang="shell" >}}
sudo DD_API_KEY=<YOUR_API_KEY> DD_INFRASTRUCTURE_MODE="end_user_device" DD_SITE="datadoghq.com" bash -c "$(curl -L https://install.datadoghq.com/scripts/install_mac_os.sh)"
{{< /code-block >}}

   If your Datadog site is not US1, update `DD_SITE` to match. For the list of sites, see [Datadog sites][3].

1. Open **Terminal**. Find it in **Applications** > **Utilities** > **Terminal**, or search for it with Spotlight (**Cmd + Space**).
1. Paste and run the install command. When prompted, enter your Mac password. The script installs the Agent, creates the `ddagent` system user, and registers the Agent as a background launch daemon. Installation takes one to two minutes.
...
<!-- TODO: Confirm macOS Network Path support with Erica. The manual-install config comment in the source doc says macOS is not supported yet, while the Jamf config comment lists macOS as supported. -->

1. Enable the `traceroute` system-probe module by adding the following to `/opt/datadog-agent/etc/system-probe.yaml`:
...
1. Locate `/etc/datadog-agent/conf.d/network_path.d/conf.yaml.example` and remove `.example` from the filename.
1. Open `conf.yaml` and add the endpoints you want to monitor. For example:
...
1. Restart the Agent and system-probe to apply the changes:
...
- A valid Datadog API key. For instructions, see [API and Application Keys][1].
- Devices running macOS 12 or later.
...
1. To avoid storing your API key in the script body, pass it as a Jamf script parameter. In the **Options** tab, label parameter 4 as `DD_API_KEY`. The script reads this value from `$4`, and you enter the actual key when you create the policy.
...
[2]: https://app.datadoghq.com/fleet
