...
The sections below list every metric under `aws.elb.*`: 68 to update, 9 to verify, and 22 unaffected.
...
{{% collapse-content title="View all 22 unaffected metrics" level="h4" expanded=false %}}
...
<!-- DRAFT NOTE: Two additional metrics (`aws.elb.healthy_host_count_deduped` and `aws.elb.un_healthy_host_count_deduped`) are generated by Datadog rather than sourced directly from CloudWatch. Doug is confirming their reporting behavior; add them here once categorized. -->
