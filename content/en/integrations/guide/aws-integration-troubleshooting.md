...
1. Datadog displays raw data from AWS in per second values, regardless of the time frame selected in AWS. This is why you might see Datadog's value as lower.

2. `min`, `max`, and `avg` have a different meaning within AWS than in Datadog. In AWS, average latency, minimum latency, and maximum latency are three distinct metrics that AWS collects. When Datadog pulls metrics from AWS CloudWatch, the average latency is received as a single timeseries per Elastic Load Balancer. Within Datadog, when you are selecting `min`, `max`, or `avg`, you are controlling how multiple timeseries are combined. For example, requesting `system.cpu.idle` without any filter returns one series for each host that reports that metric, and those series need to be combined to be graphed. On the other hand, if you requested `system.cpu.idle` from a single host, no aggregation is necessary and switching between average and max yields the same result.
...
When installing the Agent on an AWS host, you might see duplicated hosts on the Datadog infrastructure page for a few hours if you manually set the hostname in the Agent's configuration. This second host disappears a few hours later, and does not affect your billing.
...
You can verify by checking the “Apps Running” for that host via the infrastructure list or checking the metrics summary and creating a notebook scoped to that host.

If you prefer to have these AWS host tags permanently removed from a host, you can do this by using the [Remove host tags API endpoint][7].
