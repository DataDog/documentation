...
| `send_distribution_buckets`             | boolean                                 | optional  | false         | Set `send_distribution_buckets` to `true` to send and convert OpenMetrics histograms to [Distribution metrics][8]. <br>`collect_histogram_buckets` must be set to `true` (default value).                                                                              |
