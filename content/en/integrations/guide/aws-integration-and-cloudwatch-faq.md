...
- Datadog collects a single CloudWatch statistic for the equivalent CloudWatch metric name in Datadog. If you are looking at `Sum` in CloudWatch but Datadog is pulling the `Average` CloudWatch statistic, this will result in discrepencies. For some CloudWatch metrics, multiple statistics can be useful and Datadog creates different metric names for the same CloudWatch metric with different statistics. Ex. `aws.elb.latency` and `aws.elb.latency.maximum`.
