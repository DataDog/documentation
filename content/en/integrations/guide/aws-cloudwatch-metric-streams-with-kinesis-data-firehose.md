...
4. If Firehose receives records but does not deliver them, verify the S3 backup configuration and IAM role. Confirm that Firehose can assume the configured role and write to the backup bucket. Check that the bucket policy, permissions boundary, service control policies (SCPs), and KMS key policy do not deny the required access. To check if records are reaching S3, look in the backup bucket under the prefix configured in your Firehose delivery stream's S3 backup settings. If no objects are being written, it confirms a permissions or configuration issue with the S3 backup path. If the logs show an S3 permissions error, correct it before continuing.
