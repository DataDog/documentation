 ...
 * Gathering information about the status / health of resources, such as automuting
...
AWS CloudWatch reports metrics at 1-minute granularity normalized to per minute data. Datadog reports metrics at 1-minute granularity normalized to per second data. To conform the data in Datadog, multiply by 60.

#### Will a rollup() conform my data?

Rollups don't display similar results. For a rollup call of `rollup(sum, 60)`, the server groups all data points in minute bins and returns the sum of each bin as a datapoint. However, the granularity of AWS metrics is 1-minute, so there is only one datapoint per bin leading to no change.
