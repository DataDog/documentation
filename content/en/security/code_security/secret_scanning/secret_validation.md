...
For secret types with validation available, Datadog displays the validation status in the explorer as "{{< ui >}}Active{{< /ui >}}" or "{{< ui >}}Inactive{{< /ui >}}". You can also filter or query detections by their {{< ui >}}Validation Status{{< /ui >}}.
