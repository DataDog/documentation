...
The schema for this field is described in the [Software Catalog entity model][23].
...
<div class="alert alert-danger">
Services defined with a Software Catalog schema v2.x do not support `codeLocations`. Existing definitions can be upgraded to the v3 schema in the [Software Catalog][24]. Once the migration is complete, changes can take up to 24 hours to apply to findings. If you are unable to upgrade to v3, Datadog will fall back to alternative linking techniques (described below). These rely on less precise heuristics, so accuracy might vary depending on the Code Security product and your use of other Datadog features.
</div>
...
Datadog attempts to map a single finding to as many services as possible. If no matches are found, Datadog continues onto our next linking method.

### Software Catalog cannot determine the service

If the Software Catalog does not provide a match, because the finding’s file path does not match any `codeLocations`, or because the service uses the v2.x schema, Datadog evaluates whether Error Tracking can identify the service associated with the code. Datadog uses only the last 30 days of Error Tracking data due to product [data-retention limits][25].
...
Datadog evaluates the following:

  - the **file path** contains identifiers matching a known service
  - the **repository name** corresponds to a service name

When using the finding’s file path, Datadog performs a reverse search on each path segment until it finds a matching service or exhausts all options. For example, if a finding occurs in `github.com/org/checkout-service` at `/foo/bar/baz/main.go`, Datadog uses the last path segment, `main`, to determine if any Software Catalog service uses that name. If a match exists, the finding is attributed to that service. If not, the process continues with `baz`, then `bar`, and so on.
...
**Note**: You must accurately map your Git provider teams to your [Datadog Teams][25] for team attribution to function properly.
