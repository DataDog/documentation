...
1. On the [{{< ui >}}Notification Rules{{< /ui >}}][1] page, click {{< ui >}}New Notification Rule{{< /ui >}}.
1. Enter a {{< ui >}}Name{{< /ui >}} for the notification rule.
1. Select the source type for the notification rule:
    - {{< ui >}}Finding{{< /ui >}}: A potential security flaw in your infrastructure.
    - {{< ui >}}Signal{{< /ui >}}: Suspicious activity that poses an active threat against your infrastructure.
...
1. If you selected {{< ui >}}Finding{{< /ui >}} in step 3, select the frequency of the notifications:
   - {{< ui >}}Aggregate results over{{< /ui >}}: Select this option, followed by a time frame from the list, to only get one notification for detections that occurred over that time frame.
   - {{< ui >}}Trigger immediately for each individual issue meeting the criteria{{< /ui >}}: Select this option to get one notification for each detection.<br />**Note**: Selecting this option can result in a large number of notifications.
1. Under {{< ui >}}Destination{{< /ui >}}, select a routing mode:
    - {{< ui >}}Manual routing{{< /ui >}}: Click {{< ui >}}Add Recipient{{< /ui >}} and specify the recipients you want to notify. You can notify individuals or teams, create Jira issues, and more. See [Notification channels][2] for more information.
    - {{< ui >}}Dynamic routing{{< /ui >}} (Preview): Automatically route notifications to the responsible team based on the `team` tag on findings. Specify a {{< ui >}}Fallback Channel{{< /ui >}} for findings that cannot be dynamically routed. See [Dynamic routing](#dynamic-routing) for requirements.<br />**Note**: Dynamic routing is only available when {{< ui >}}Trigger immediately for each individual issue meeting the criteria{{< /ui >}} is selected in step 6.
