...
**Note**: Security Filters are only required to control logs analyzed by the Cloud SIEM product. You do not need to write Security Filters to exclude logs generated by the Datadog Agent as part of the Workload Protection Threats (`source:runtime-security-agent`) and Workload Protection Misconfigurations (`source:compliance-agent`) products, as they're not billed as analyzed logs regardless.
