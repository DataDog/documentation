...
At some point, you may want to write your own [custom Cloud Workload Security (CWS) rules][1] for the Agent to use. When writing your own rules, there are a few strategies you can use to optimize for efficiency.
...
CWS uses the concept of approvers and discarders to filter out events which will not trigger any rules in your policy. Approvers and discarders do not act on individual rules; they allow or deny events.
...
Approvers and discarders are generated based on your entire policy. Because of this, if a single rule does not make use of approvers for a given event (for example, `open` or `exec`), approvers cannot be used for that event for the entire policy, making every rule that uses that event less efficient.
