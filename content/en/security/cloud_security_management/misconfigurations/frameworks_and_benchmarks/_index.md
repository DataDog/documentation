...
- {{< ui >}}Star{{< /ui >}}: Pin a framework to the top of your table.
- {{< ui >}}Score{{< /ui >}}: The [posture score][3] for the rules in the given framework.
- {{< ui >}}Change{{< /ui >}}: The difference in posture score over the chosen time range (defaults to 1 month). Hover to see details.
- {{< ui >}}Failing Rules{{< /ui >}}: All the rules failing in the framework. Hover for more details.
- {{< ui >}}Resources Passing{{< /ui >}}: Of all the resources evaluated by rules in the framework, the percentage of which pass all the rules in the framework.
- {{< ui >}}Framework Overview{{< /ui >}}: A [detailed report](#explore-compliance-framework-reports) that gives you insight into how you score against a framework's requirements and rules.
- {{< ui >}}Explore Resources{{< /ui >}}: A filtered view of the {{< ui >}}Misconfigurations{{< /ui >}} page that shows resources with misconfigurations for the selected framework.
- {{< ui >}}Configure Rules{{< /ui >}}: Customize how your environment is scanned and set notification targets by modifying the compliance rules for each framework.
