...
1. Find the triage status dropdown for the resource.
   - In the misconfiguration, identity risk, or vulnerability explorers, the dropdown is in the {{< ui >}}Triage{{< /ui >}} column for each resource. Alternatively, you can select one or more resources, then click the {{< ui >}}Set State{{< /ui >}} dropdown that appears, so you can mute your entire selection at once.
   - When you're viewing a resource in a side panel, under {{< ui >}}Next Steps{{< /ui >}}, the dropdown is under {{< ui >}}Triage{{< /ui >}}.
2. Open the dropdown with the current triage status and click {{< ui >}}Muted{{< /ui >}}. The {{< ui >}}Mute issue{{< /ui >}} window opens.
3. Select a reason for the mute; for example, it's a false positive, it's an accepted risk, or a fix is pending.
4. Enter an optional {{< ui >}}Description{{< /ui >}}.
5. Select the duration of the mute.
6. Click {{< ui >}}Mute{{< /ui >}}. The {{< ui >}}Mute issue{{< /ui >}} window closes.
...
1. Find the triage status dropdown for the resource.
   - In the misconfiguration, identity risk, or vulnerability explorers, the dropdown is in the {{< ui >}}Triage{{< /ui >}} column for each resource. Alternatively, you can select one or more resources, then click the {{< ui >}}Set State{{< /ui >}} dropdown that appears, so you can unmute your entire selection at once.
   - When you're viewing a resource in a side panel, under {{< ui >}}Next Steps{{< /ui >}}, the dropdown is under {{< ui >}}Triage{{< /ui >}}.
2. Click {{< ui >}}Muted{{< /ui >}} to open the dropdown, then select a new triage status. The triage status updates immediately for the selected resources.
...
1. Depending on the issue explorer you're using, sort or filter the issues:
   - On the Misconfigurations issue explorer, sort by the {{< ui >}}Muted{{< /ui >}} column.
   - On the Misconfigurations or Identity Risks issue explorers, filter issues using the {{< ui >}}Muted{{< /ui >}} facet.
   - On the Vulnerabilities issue explorer, click the {{< ui >}}Muted{{< /ui >}} tab.
...
3. Click the {{< ui >}}Timeline{{< /ui >}} tab to view a chronological history of the misconfiguration. Hover over a mute or unmute action to view additional details, such as the reason for the mute, how long the mute is intended to last, and who muted it.
