...
1. In the {{< ui >}}Match Conditions{{< /ui >}} section, click {{< ui >}}Custom Keywords{{< /ui >}}.
    ...
    - To remove keywords, click the {{< ui >}}X{{< /ui >}} next to the keyword you want to remove.
...
- When you add rules to a scanning group with sampling enabled, you will not be able to select the {{< ui >}}redact{{< /ui >}}, {{< ui >}}partially redact{{< /ui >}}, or {{< ui >}}hash{{< /ui >}} actions. For complete obfuscation, disable sampling in your scanning group settings.

See [Investigate Sensitive Data Findings][7] for details on triaging sensitive data using the [{{< ui >}}Findings{{< /ui >}}][8] page.
...
To redact sensitive data contained in tags, you must [remap][10] the tag to an attribute and then redact the attribute. Uncheck {{< ui >}}Preserve source attribute{{< /ui >}} in the remapper processor so that the tag is not preserved during the remapping.
