...
If an event matches an Agent detection rule *and* a backend Threat detection rule, a signal is created and displayed in [{{< ui >}}Signals{{< /ui >}}][11] (`Agent detection rule + backend Threat detection rule = Signal`).
