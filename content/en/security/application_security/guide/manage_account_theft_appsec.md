...
5. Collect the tags required by detection rules. The most important tag is `usr.login`. Assuming the login was provided in the request, you can add a condition and set {{< ui >}}store value as tag{{< /ui >}} as the operator.
...
7. Set the {{< ui >}}Tag{{< /ui >}} field to the name of the tag where you want to save the value captured using `usr.login`.
...
In a **Distributed Credential Stuffing campaign** signal, Datadog automatically identifies clear traits and presents them as {{< ui >}}Attacker Attributes{{< /ui >}}. 
...
If the rule is no longer relevant, you can delete it by clicking more options (**...**) and selecting {{< ui >}}Delete{{< /ui >}}.
