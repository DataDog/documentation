...
- {{< ui >}}Vulnerabilities{{< /ui >}}: Patch any vulnerable libraries surfaced by SCA or Runtime Code Analysis, then redeploy the service.
- {{< ui >}}API findings discovered{{< /ui >}}: Review each issue in context of the traced service, fix any code or configurations, and then validate using new traces.
- {{< ui >}}Processing sensitive data{{< /ui >}}: Confirm data handling complies with policy, sanitize or encrypt PII, and limit access to necessary services.
- {{< ui >}}Unauthenticated endpoint{{< /ui >}}: If the endpoint is not intentionally public, enforce authentication and update service configurations.
