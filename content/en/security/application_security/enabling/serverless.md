...
You can monitor your functions running in AWS Lambda with Datadog Application Security Management (ASM). See [Setup and Configure][1] for information about what ASM features are supported for serverless functions.

In general, setting up ASM for AWS Lambda involves:

1. Identifying functions that are vulnerable or are under attack, which would most benefit from ASM. Find them on [the Security tab of your Service Catalog][2].
2. Setting up ASM instrumentation by using the [Datadog Serverless Framework plugin][3] or manually setting the different layers.
3. Triggering security signals in your application and seeing how Datadog displays the resulting information.

## Prerequisites

- [Serverless APM][4] is configured on the Lambda function to send traces directly to Datadog. The X-Ray integration for sending trace data to APM does not support the data ASM needs to monitor functions.
...
3. Enable ASM by adding the following environment variables on your function deployment:
   ...
   environment:
     ...
     AWS_LAMBDA_EXEC_WRAPPER: /opt/datadog_wrapper
   ```
4. Redeploy the function and invoke it. After a few minutes, it appears in [ASM views][2].


[1]: https://docs.aws.amazon.com/lambda/latest/dg/configuration-layers.html
[2]: https://app.datadoghq.com/security/appsec?column=time&order=desc
{{% /tab %}}
{{< /tabs >}}
...
To see Application Security Management threat detection in action, send known attack patterns to your application. For example, send an HTTP header with value `acunetix-product` to trigger a [security scanner attack][5] attempt:
   ...
   ```
A few minutes after you enable your application and exercise it, **threat information appears in the [Application Signals Explorer][6]**.
...
[1]: /security/application_security/threats/setup_and_configure/?code-lang=serverless
[2]: https://app.datadoghq.com/services?env=prod&hostGroup=%2A&lens=Security
[3]: /serverless/libraries_integrations/plugin/
[4]: /serverless/distributed_tracing/?tab=python
