---
title: Enabling Application & API Protection for Envoy
code_lang: envoy
...
To try the preview of Application & API Protection for Envoy, follow the setup instructions below.
{{< /callout >}}
...
The Application & API Protection Envoy integration uses the Envoy external processing filter.
...
## Datadog Go Tracer and Envoy integration
  ...
  <div class="alert alert-warning">
    <strong>Note:</strong> The Application & API Protection Envoy integration is built on top of the Datadog Go Tracer. It follows the same release process as the tracer, and its Docker images are tagged with the corresponding tracer version.
  </div>

  The Envoy integration uses the [Datadog Go Tracer][6] and inherits all environment variables from the tracer. You can find more information in [Configuring the Go Tracing Library][7] and [Application & API Protection Library Configuration][8].
...
[2]: https://docs.datadoghq.com/tracing/guide/remote_config
