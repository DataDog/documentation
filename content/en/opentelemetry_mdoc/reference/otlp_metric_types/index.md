...
sourceUrl: >-
  https://docs.datadoghq.com/opentelemetry/reference/otlp_metric_types/index.html
---
...
1. For cumulative monotonic sums, the delta between consecutive points is calculated and reported to Datadog as a count. The first point is stored but omitted. To recover the value in the OTLP payload, use the [`cumsum` arithmetic function](https://docs.datadoghq.com/dashboards/functions/arithmetic/#cumulative-sum).
...
1. Delta histograms are reported as Datadog distributions. [Read more about distributions](https://docs.datadoghq.com/metrics/distributions) to understand the available aggregations. Histograms with a count of 0 are dropped.
1. For cumulative histograms, the delta between consecutive points is calculated and reported to Datadog as a distribution. Deltas with a count of 0 are not reported. You may use the [`cumsum` arithmetic function](https://docs.datadoghq.com/dashboards/functions/arithmetic/#cumulative-sum) on individual aggregations to recover the value in the OTLP payload.
...
[Read more about distributions](https://docs.datadoghq.com/metrics/distributions) to understand how to configure further aggregations.
...
- [Producing delta temporality metrics with OpenTelemetry](https://docs.datadoghq.com/opentelemetry/guide/otlp_delta_temporality/)
