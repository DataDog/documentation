...
If you use Docker containers in a Beanstalk environment, and want to monitor you Docker usage, the containerized Datadog agent is the perfect tool!

Read on to understand how to configure your Beanstalk environment to integrate the Datadog agent container.

#### Task definition
To run docker environments with multiple containers per instance, Elastic Beanstalk relies on EC2 Container Service (ECS).
...
Once the container definition is ready, we need to ship it to Beanstalk. [The multicontainer Docker tutorial](http://docs.aws.amazon.com/elasticbeanstalk/latest/dg/create_deploy_docker_ecstutorial.html) of the Elastic Beanstalk documentation explains how to do so easily.
