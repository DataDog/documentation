...
: <YOUR_USERNAME>     # if the status and ping URLs require HTTP basic auth
: <YOUR_PASSWORD> # if the status and ping URLs require HTTP basic auth
: <HOST>         # if your FPM pool is only accessible via a specific HTTP vhost 
:
#     - instance:foo
```
