...
Cette page couvre la configuration d'AWS ECS avec [l'Agent v6 de Datadog][1]. Si vous souhaitez effectuer la configuration avec l'Agent v5 de Datadog, [consultez la page de documentation relative à la configuration d'AWS ECS avec l'Agent v5][2].
...
Nous partons du principe que vous avez déjà configuré un cluster EC2 Container Service. Si ce n'est pas le cas, consultez la [section de mise en route de la documentation ECS][3].
...
Vous pouvez configurer la tâche à l'aide des [outils d'AWS CLI] ou de la console Web d'Amazon.

##### AWS CLI

1. Téléchargez [datadog-agent-ecs.json][5].
...
1. Ajoutez les autorisations suivantes à votre [stratégie IAM Datadog][8] afin de recueillir des métriques Amazon ECS. Pour en savoir plus sur les stratégies ECS, [consultez la documentation du site Web d'AWS][9].
    ...
    |----------------------------------|------------------------------------------------------------------------------------------------------|
    | `ecs:ListClusters`               | Répertorie tous les clusters disponibles.                                                                             |
    | `ecs:ListContainerInstances`     | Répertorie les instances d'un cluster.                                                                         |
...
La fonction [Autodiscovery][23] de Datadog peut être utilisée avec ECS et Docker afin de découvrir et de surveiller automatiquement des tâches s'exécutant dans votre environnement.
...
1. Suivez les [instructions ci-dessous](#aws-cli) pour installer l'Agent Datadog.
...
Pour supprimer ce filtre et recevoir tous les événements de votre intégration Datadog/AWS ECS, contactez [l'équipe d'assistance Datadog][15].
