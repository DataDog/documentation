...
- data store
ddtype: check
doc_link: https://docs.datadoghq.com/integrations/tokumx/
...
short_description: Track metrics for opcounters, replication lag, cache table size,
  and more.
support: core
supported_os:
- linux
...
[Restart the Agent](https://docs.datadoghq.com/agent/faq/start-stop-restart-the-datadog-agent) to start sending TokuMX metrics to Datadog.

### Validation

[Run the Agent's `status` subcommand](https://docs.datadoghq.com/agent/faq/agent-status-and-information/) and look for `tokumx` under the Checks section:
