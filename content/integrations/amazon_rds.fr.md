...
name: amazon_rds
public_title: Datadog-AWS RDS Integration
...
6.  Encrypt the key you just created by using the [AWS CLI](http://aws.amazon.com/documentation/cli/), replacing \<KMS_KEY_NAME\> with the alias of the key you just created: <br>
`aws kms encrypt --key-id alias/<KMS_KEY_NAME> --plaintext "{'api_key':'<YOUR_API_KEY>', 'app_key':'<YOUR_APP_KEY>'}"`. <br>
...
1. In the [AWS integration tile](https://app.datadoghq.com/account/settings#integrations/amazon_web_services), ensure that `RDS` is checked under metric collection.

2. Add those permissions to your [Datadog IAM policy](https://docs.datadoghq.com/integrations/amazon_web_services/#installation) in order to collect Amazon RDS metrics: 
  ...
  * `rds:DescribeEvents`: Add events related to RDS databases.
  ...
  For more information on RDS policies, review [the documentation on the AWS website](https://docs.aws.amazon.com/IAM/latest/UserGuide/list_rds.html).

3. Install the [Datadog - AWS RDS integration](https://app.datadoghq.com/account/settings#integrations/amazon_rds).
...
After a few minutes, RDS metrics and [metrics from MySQL, Aurora, MariaDB, SQL Server, or PostgreSQL](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Aurora.Monitoring.html) are accessible in Datadog in the Metrics Explorer, in [Graphs](https://docs.datadoghq.com/graphing/) and in [Alerts](https://docs.datadoghq.com/monitors/).  
Here's an example of an Aurora dashboard displaying a number of metrics from both RDS and the MySQL integration. Metrics from both integrations on the instance `quicktestrds` are unified using the `dbinstanceidentifier` tag.
...
### Events
The AWS RDS integration does not include any event at this time.

### Service Checks
The AWS RDS integration does not include any service check at this time.
