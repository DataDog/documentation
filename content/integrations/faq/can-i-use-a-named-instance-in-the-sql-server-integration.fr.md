...
Le service SQL Server Browser est requis pour utiliser des instances nommées et ce service est désactivé par défaut. Nous recommandons fortement l'approbation d'un administrateur système avant d'activer [ce service][2].aspx).

Une fois le service SQL Server Browser activé, vous pouvez configurer le fichier [sqlserver.yaml][3] pour se connecter à une instance nommée en désignant l'instance nommée dans la valeur de l'host. Par exemple:
