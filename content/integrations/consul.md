...
instances:
 the Consul HTTP Server Lives
 'https' if Consul is configured for SSL
    - url: http://localhost:8500
, if Consul is talking SSL
: '/path/to/client.concatenated.pem'
 per-service node status and per-node service status?
      catalog_checks: yes
