...
2. Rollups will not display similar results; the attempted rollup call would be rollup(sum, 60), where the server will group all data points in minute bins and return the sum of each bin as a datapoint. However, the granularity of AWS metrics is 1 minute, so there will only be one datapoint per bin leading to no change.
 