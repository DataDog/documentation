...
Datadog Agent default configuration targets Kubernetes 1.7.6 and later, as the Datadog Agent relies on features and endpoints introduced in this version. More installation steps are required for older versions:
...
- [RBAC objects][1] (`ClusterRoles` and `ClusterRoleBindings`) are available since Kubernetes 1.6 and OpenShift 3.3, but are available under different `apiVersion` prefixes:
  ...
  * `v1` in Openshift 3.3 to 3.6
    ...
    Apply Datadog Agent yaml manifests with the following `sed` invocations:
...
- Datadog Agent default daemonset makes use of the [downward API][3] to pass the kubelet's IP to the agent. This only works on versions 1.7 and up. For older versions, here are other ways to enable kubelet connectivity:
...
- Datadog Agent default configuration relies on [bearer token authentication][4] to the APIserver and kubelet. On 1.3, the kubelet does not support bearer token auth, setup client certificates for the `datadog-agent` serviceaccount and pass them to the agent.
