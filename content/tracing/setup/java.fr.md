...
| priority.sampling  | dd.priority.sampling  | DD_PRIORITY_SAMPLING      | `false`            | Change la façon dont l'agent et le client échantillonnent les traces. Consultez la section [Echantillonnage/traçage distribué](#sampling-distributed-tracing) pour plus de détails. |
...
L'échantillonnage via priorité garantit que les traces distribuées sont complètes en affectant et en propageant une valeur de priorité le long de chaque trace.
...
Une fois activé, l'échantillonneur attribue automatiquement une priorité aux traces, en fonction de leur service et de leur volume.

Vous pouvez également définir cette priorité manuellement pour supprimer une trace non intéressante ou en conserver une importante.
```java
