...
Si un utilisateur décide de suivre les modifications pour un Timeboard, les modifications du Timeboard sont signalées à l'utilisateur via un événement dans [l'event stream][2]:
...
Un utilisateur peut trouver tous les événements liés aux changements d'un Timeboard sur le Timeboard qu'ils suivent en recherchant: `tags:audit, <Screenboard_name>` dans le [flux d'événements][3], chaque événement de notification est taggé avec ces deux tags.

## Audit de dashboards

Dans les dashboard, les notifications permettent de suivre les modifications à des fins d'audit. Toute modification effectuée crée un événement dans le [flux d'événements][3] qui explique la modification et affiche l'utilisateur qui a effectué la modification.
...
En utilisant notre [APIs][4], il est possible d'écrire un script pour sauvegarder vos définitions de Timeboard en tant que code. Consultez les projets suivants comme exemples de la façon dont ces sauvegardes peuvent être accomplies:

* https://github.com/brightcove/dog-watcher
* https://github.com/Shopify/doggy
* https://github.com/grosser/kennel

Un grand merci à  [Brightcove][5], [Shopify][6], et [Zendesk][7] pour partager ces projets!
