---
title: Graphing FAQ
kind: faq
customnav: graphingnav
---

## Dashboards

* [How to Transform a Timeboard to a Screenboard or vice versa ?](/graphing/faq/how-to-transform-a-timeboard-to-a-screenboard-or-vice-versa)
* [Dashboard loads very slowly/displays no data, but I'm correctly sending data to Datadog](/graphing/faq/dashboard-loads-very-slowly)
* [Why am I unable to iFrame certain https:// URLs?](/graphing/faq/why-am-i-unable-to-iframe-certain-https-urls)
* [Why isn't my AWS Trusted Advisor Dashboard showing any data?](/graphing/faq/why-isn-t-my-aws-trusted-advisor-dashboard-showing-any-data)
* [How do I see the number of hosts I'm monitoring?](/graphing/faq/how-do-i-see-the-number-of-hosts-i-m-monitoring)
* [How do I change the name of a Dashboard?](/graphing/faq/how-do-i-change-the-name-of-a-dashboard)
* [Treemap graph/visualization - can I use this elsewhere?](/graphing/faq/treemap-graph-visualization-can-i-use-this-elsewhere)
* [Is there a way to share (or revoke previously shared) graphs?](/graphing/faq/is-there-a-way-to-share-or-revoke-previously-shared-graphs)

## Graphing

* [Is there a way to share graphs?](/graphing/faq/is-there-a-way-to-share-graphs)
* [How do I do arithmetic with grouped metrics?](/graphing/faq/how-do-i-do-arithmetic-with-grouped-metrics)
* [What's the syntax to sum multiple datapoints into a single line?](/graphing/faq/what-s-the-syntax-to-sum-multiple-datapoints-into-a-single-line)
* [How do I track cron jobs?](/graphing/faq/how-do-i-track-cron-jobs)
* [Correlate metrics and events using dashboard template variables](/graphing/faq/correlate-metrics-and-events-using-dashboard-template-variables)
* [I see unexpected drops to zero on my graph, why?](/graphing/faq/i-see-unexpected-drops-to-zero-on-my-graph-why)
* [Interpolation & the fill modifier explained](/graphing/faq/interpolation-the-fill-modifier-explained)
* [ How can I graph the percentage change between an earlier value and a current value?](/graphing/faq/how-can-i-graph-the-percentage-change-between-an-earlier-value-and-a-current-value)
* [False spikes sometimes appear on my `sum:` graph](/graphing/faq/false-spikes-sometimes-appear-on-my-sum-graph)
* [Graphing with Datadog: from the query to the graph](/graphing/faq/graphing-with-datadog-from-the-query-to-the-graph)
* [There are too many lines on my graph, can I only display the most important ones?](/graphing/faq/there-are-too-many-lines-on-my-graph-can-i-only-display-the-most-important-ones)
* [How does Datadog render graphs? My graph doesn't show the values I'm expecting.](/graphing/faq/how-does-datadog-render-graphs-my-graph-doesn-t-show-the-values-i-m-expecting)
* [What is the granularity of my graphs? Am I seeing raw data or aggregates on my graph?](/graphing/faq/what-is-the-granularity-of-my-graphs-am-i-seeing-raw-data-or-aggregates-on-my-graph)
* [How can I set up custom units for custom metrics?](/graphing/faq/how-can-i-set-up-custom-units-for-custom-metrics)
* [How can I graph host uptime percentage?](/graphing/faq/how-can-i-graph-host-uptime-percentage)
* [Inconsistent sum aggregation between different time windows on graph](/graphing/faq/inconsistent-sum-aggregation-between-different-time-windows-on-graph)
* [Why does zooming out a timeframe also smooth out my graphs?](/graphing/faq/why-does-zooming-out-a-timeframe-also-smooth-out-my-graphs)
* [Why is a counter metric being displayed as a decimal value?](/graphing/faq/why-is-a-counter-metric-being-displayed-as-a-decimal-value)
* [The same color is used twice in my graph!](/graphing/faq/the-same-color-is-used-twice-in-my-graph)
* [Is there a way for me to set the maximum and minimum values on the y-axis of a graph?](/graphing/faq/is-there-a-way-for-me-to-set-the-maximum-and-minimum-values-on-the-y-axis-of-a-graph)
* [Is it possible to adjust the y-axis for my graphs?](/graphing/faq/is-it-possible-to-adjust-the-y-axis-for-my-graphs)
* [How do I overlay events onto my dashboards?](/graphing/faq/how-do-i-overlay-events-onto-my-dashboards)