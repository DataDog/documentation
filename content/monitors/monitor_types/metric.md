...
7. You can opt to **automatically resolve the monitor from a triggered state**.  
    ...
    The most common use-case for this option is when you have very sparse counters, e.g. for errors. When errors stop occuring the metric stops reporting. This prevents the monitor from resolving because there are no more values to potentially trigger a resolution. You can also set **Recovery thresholds**: thresholds added to your monitor that defines an additional condition to a monitor’s recovery from alert or warning states.  
