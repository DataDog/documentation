...
- link: /agent/basic_agent_usage/
  ...
  text: Agent 기본 사용법
kind: 설명서
title: 서버리스 모니터링 시작하기
---

## 개요

_서버리스(Serverless)_는 개발자가 인프라스트럭처를 자체적으로 관리하는 대신, 클라우드 공급업체를 사용해 애플리케이션과 서비스를 개발하고 실행하는 모델입니다. Datadog [Serverless Monitoring][10]은 서버리스 인프라스트럭처에서 메트릭, 로그, 트레이스를 수집하여 애플리케이션 건전성과 성능을 모니터링하도록 도와줍니다.

이번 가이드에서는 클릭 한 번으로 시작할 수 있는 서버리스 [샘플 앱][1]을 사용해보겠습니다. 이 앱에는 서버리스 모니터링이 사전 설정으로 구성되어 있습니다. 이번 가이드를 따라 샘플 앱에서 문제가 발생했을 때 트러블슈팅하는 방법, 서버리스 모니터링에서 지원하는 시각화 자료의 유형을 알아보시기 바랍니다.

### 샘플 앱 설치하기

1. [CloudFormation Stack을 시작하세요][9]. 이 링크를 사용하면 CloudFormation의 **Create stack** 페이지로 이동합니다.
2. [Datadog API 키][12]와 [Datadog 사이트][13]({{< region-param key="dd_site" code="true" >}})를 입력합니다.
...
[샘플 앱 함수를 서버리스 보기 화면][11]에서 볼 수 있습니다.
...
### 서버리스 인사이트
서버리스 보기 화면에서 가장 오른쪽에 있는 열의 이름은 **Insights**입니다. Datadog는 자동으로 서버리스 애플리케이션에서 발생할 수 있는 문제를 강조 표시해줍니다. [고오류(high error)][3] 및 [고기간(high duration)][4] 등의 문제는 Insights 열에 표시됩니다.

서버리스 샘플 애플리케이션에서는 Datadog에서 [콜드 스타트][5] 문제를 탐지할 가능성이 높습니다. 콜드 스타트는 서버리스 애플리케이션의 트래픽이 갑자기 증가했을 때 발생합니다. 함수에서 이전에 상대적으로 일관성 있는 수량의 요청을 받다가 갑자기 더 많은 요청을 받을 때, 또는 이번 사례와 같이 이전에 비활성화 상태였던 함수가 처음으로 호출되었을 때 발생할 수 있습니다.
...
위에서 볼 수 있듯, 상세 정보 화면에는 세 개의 그래프가 있습니다. 사용 가능한 메트릭을 표시하도록 설정할 수 있는데, 기본적으로는 [확장 람다(Lambda) 메트릭][7](호출, 오류, 기간)을 표시하도록 되어 있습니다.

Datadog는 확장 람다 메트릭을 바로 사용할 수 있도록 생성합니다. 낮은 레이턴시, 몇 초 단위의 입도, 콜드 스타트와 커스텀 태그에 대한 상세 메타데이터가 지원됩니다. 또한 기본 [확장 람다 메트릭 대시보드][8]도 볼 수 있습니다.
...
이러한 로그는 오류만 보도록 필터링하거나, [로그 익스플로러(Log Explorer)][6]에서 조회할 수 있습니다.


[1]: https://github.com/DataDog/serverless-sample-app
[2]: https://docs.datadoghq.com/kr/serverless/libraries_integrations/extension/#tagging
[3]: https://docs.datadoghq.com/kr/serverless/guide/insights/#high-errors
[4]: https://docs.datadoghq.com/kr/serverless/guide/insights/#high-duration
[5]: https://docs.datadoghq.com/kr/serverless/guide/insights/#cold-starts
[6]: https://docs.datadoghq.com/kr/logs/explorer/
[7]: https://docs.datadoghq.com/kr/serverless/enhanced_lambda_metrics
...
[13]: https://docs.datadoghq.com/kr/getting_started/site