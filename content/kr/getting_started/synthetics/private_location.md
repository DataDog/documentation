...
- link: https://www.datadoghq.com/blog/synthetic-private-location-monitoring-datadog/
  tag: 블로그
  text: Datadog에서 신서틱(Synthetic) 프라이빗 위치로 모니터링하기
...
IE11 브라우저 테스트를 실행할 수 있도록 해주는 Windows 프라이빗 위치(Private Location) 베타에 참여하고 싶으신 분은 <a href="https://docs.datadoghq.com/help/">Datadog 지원팀</a>에 문의하시기 바랍니다.
</div>
...
- [CI/CD 파이프라인에서 신서틱(Synthetic) 테스트][1]를 사용하여 실제 환경에 신기능을 출시하기 전에 **내부 테스트 환경에서 애플리케이션 성능을 확인**합니다.
- 내부 네트워크의 내외부에서 **애플리케이션 성능을 비교**합니다.

프라이빗 위치는 비공개(프라이빗) 네트워크 내 어디서나 설치할 수 있는 도커(Docker) 컨테이너입니다. Google Container Registry를 통해 [프라이빗 위치 워커 이미지][2]에 접근할 수 있습니다.
...
1. 머신에 [도커][4]를 설치합니다. 빠르게 시작하려면 도커를 [Vagrant Ubuntu 16.04][2] 같은 가상 머신에 설치하세요.
2. Datadog 사이트에서 **[UX Monitoring][5]**에 커서를 올린 다음 **Settings** > **Private Locations**를 선택합니다. 
3. **Add Private Location**를 클릭합니다.
4. 프라이빗 위치의 상세 정보를 입력합니다. `Name`과 `API key` 필드만 필수 입력 항목입니다. Windows용 프라이빗 위치를 설정하는 경우는 **This is a Windows Private Location**을 선택합니다.
...
6. 프라이빗 위치를 어디에 설치하느냐에 따라 설정 파일에 추가 파라미터를 입력해야 할 수 있습니다.
   ...
   자세한 정보는 [프라이빗 위치 설정 옵션][6] 및 [프라이빗 위치에서 신서틱 테스트 실행하기][7] 가이드를 참조하시기 바랍니다.
...
10. 예를 들어, 도커를 사용 중이라면 도커 `run` 명령어와 설정 파일을 활용하여 도커를 독립형 컨테이너로 시작하세요.
...
[1]: /kr/synthetics/cicd_integrations
[2]: https://console.cloud.google.com/gcr/images/datadoghq/GLOBAL/synthetics-private-location-worker?pli=1
[3]: /kr/getting_started/synthetics/
[4]: https://docs.docker.com/install/linux/docker-ce/ubuntu/#install-docker-ce
[5]: https://app.datadoghq.com/synthetics/list
[6]: /kr/synthetics/private_locations/configuration/#configuration-options
[7]: /kr/synthetics/private_locations/?tab=docker#blocking-reserved-ips
[8]: /kr/getting_started/synthetics/api_test#create-a-multistep-api-test
[9]: /kr/getting_started/synthetics/browser_test