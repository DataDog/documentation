...
[브라우저 테스트][1]는 Datadog가 웹 애플리케이션에서 실행하는 시나리오입니다. 정기적인 간격을 구성하여 여러 장소, 기기 및 브라우저에서 테스트를 실행하거나 CI/CD 파이프라인에서 테스트를 할 수 있습니다.
...
아래의 사례는 장바구니에 상품을 추가하는 단계부터 결제 완료까지 사용자의 여정을 매핑하는 브라우저 테스트 생성 방법을 설명합니다.
...
관리형 위치(Managed locations)를 사용하면 공개용 웹사이트와 엔드포인트를 테스트할 수 있습니다. 내부 애플리케이션을 테스트하거나 별도 지역의 사용자 행동을 시뮬레이션하는 경우에는 [비공개 위치(private locations)][4]를 선택하세요.
...
**참조**: 신서틱 테스트를 일정대로 실행하는 것과 더불어 [CI/CD 파이프라인][5]에서 수동으로 또는 직접 트리거할 수 있습니다.
...
또한 Slack, PagerDuty, Microsoft Teams, 웹훅 등의 [알림 통합][6]을 사용할 수도 있습니다. 이러한 도구에서 신서틱 경고를 트리거하려면 먼저 해당하는 [통합][7]을 설정하셔야 합니다.

테스트를 기록할 준비가 되었다면 **Save Details & Record Test**를 클릭하세요.

### 기록 생성하기

테스트 설정을 저장하면 Datadog에서 [Datadog 테스트 리코더][8] Chrome 확장 프로그램을 다운로드해 설치하라는 안내를 표시합니다.

확장 프로그램을 설치했다면 **Start Recording**을 클릭해 테스트 절차 기록을 시작하세요.

리코더 페이지 오른쪽에 위치한 iframe에서 페이지를 이동할 수 있습니다. div, 이미지 또는 페이지에서 아무 데나 선택하면 Datadog에서 브라우저 테스트와 관련된 단계를 기록하고 작성합니다.
...
4. 결제 완료 후 “Thank you!” 문구가 표시되는지 확인하려면 **Value** 필드에 `Thank you!`를 입력하세요.
5. **Save & Quit**를 누릅니다.
...
**참조**: 예시 웹사이트는 의도적으로 실패하도록 하는 오류를 규칙적으로 발생시킵니다. **Notify your team** 필드에 이메일 주소를 포함시켰다면, 테스트가 실패하고 복원되었을 때 이메일 알림을 받게 됩니다.
...
[실패한 테스트][9]를 트러블슈팅하려면 실패한 테스트 결과를 선택하고 실패한 단계에 이르기까지 스크린샷을 확인하세요. 잠재적인 **[Errors & Warnings][10]**, **[Resources][11]**, 및 **[Core Web Vitals][12]**를 확인하여 문제를 진단할 수도 있습니다.
...
Datadog의 [신서틱 모니터링과의 APM 통합][13]을 이용하면 **Traces** 탭에서 실행된 테스트를 통해 백엔드에서 생성된 트레이스를 확인할 수 있습니다.
...
[1]: /kr/synthetics/browser_tests/
[2]: https://app.datadoghq.com/synthetics/list
[3]: https://app.datadoghq.com/synthetics/browser/create
[4]: /kr/getting_started/synthetics/private_location
[5]: /kr/synthetics/cicd_integrations
[6]: /kr/integrations/#cat-notification
...
[9]: /kr/synthetics/browser_tests/test_results#test-failure
[10]: /kr/synthetics/browser_tests/test_results#errors
[11]: /kr/synthetics/browser_tests/test_results#resources
[12]: /kr/synthetics/browser_tests/test_results#page-performance
