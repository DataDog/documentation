...
클라이언트 토큰을 관리하려면 **Organization Settings**로 이동한 다음 **Client Tokens** 탭을 클릭하세요.

클라이언트 토큰은 조직에 고유하게 할당됩니다. 클라이언트 토큰은 [웹 브라우저 로그 컬렉터][6]가 로그를, 또 [Real User Monitoring][7]이 이벤트와 로그를 Datadog로 제출할 때 필요합니다.

보안상의 이유로 브라우저에서 API 키를 사용하여 데이터를 전송할 수 없습니다. 자바스크립트(Javascript) 코드를 통해 API 키가 클라이언트 측에 노출되기 때문입니다. 웹 브라우저에서 로그를 수집하려면 클라이언트 토큰을 사용해야 합니다.

**참조**: 클라이언트 토큰을 생성한 사용자가 비활성화되어도 해당 토큰의 권한이 취소되지는 않습니다. 여전히 해당 토큰을 RUM 애플리케이션에서 사용 가능하며 로그를 수집할 수 있습니다.
