...
Integrations and Log Collection are intimately tied together, by collecting Logs the right way you make sure to auto-configure all the the subsequent components such as [processing][33], [parsing][29] and [facets][18] in the Explorer. **[Discover all the log integrations currently supported by Datadog][37]**, if not defined no worries you can always define custom log sources. 
