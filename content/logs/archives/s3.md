...
Adding multiple S3 archives is a feature that is in private beta. Ask your [customer success or sales representative][4] if you would like early access. 

Once enabled, admins can add multiple S3 log archives. You can route specific logs to an archive by adding a query in the archive's `filter` field. Logs enter the first archive whose filter they match on, so it is important to order your archives carefully. For example, if you create a first archive filtered to the `env:prod` tag and a second archive without any filter (the equivalent of `\*`), all your production logs would go to one S3 bucket/path and the rest would go to the other.
