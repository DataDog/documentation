...
breadcrumbs: >-
  Docs > OpenTelemetry in Datadog > OpenTelemetry Configuration > Hostname and
  Tagging
---
...
{% image
   source="http://localhost:1313/images/opentelemetry/collector_exporter/hostname_tagging.7a4413449f667cb67c92e808cc795941.png?auto=format"
   alt="Hostname information collected from OpenTelemetry" /%}
...
{% tab title="Kubernetes DaemonSet -> Gateway" %}
Add the following lines to `values.yaml`:
...
For a full working example configuration with the Datadog exporter, see [`k8s-values.yaml`](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/exporter/datadogexporter/examples/k8s-chart/k8s-values.yaml). This example is for Amazon EKS.
...
### Custom host tags{% #custom-host-tags %}

#### In the Datadog exporter{% #in-the-datadog-exporter %}
...
See all configurations options [here](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/exporter/datadogexporter/examples/collector.yaml).

#### As OTLP resource attributes{% #as-otlp-resource-attributes %}

Custom host tags can also be set as resource attributes that start with the namespace `datadog.host.tag`.

This can be set as an env var `OTEL_RESOURCE_ATTRIBUTES=datadog.host.tag.<custom_tag_name>=<custom_tag_value>` in an [OTel SDK](https://opentelemetry.io/docs/languages/js/resources/). Or this can be set in a processor:
...
**Note:** This is only supported if you have opted-in as described [here](https://docs.datadoghq.com/opentelemetry/schema_semantics/host_metadata/).
...
### Custom container tags{% #custom-container-tags %}
...
This can be set as an env var `OTEL_RESOURCE_ATTRIBUTES=datadog.container.tag.<custom_tag_name>=<custom_tag_value>` in an [OTel SDK](https://opentelemetry.io/docs/languages/js/resources/). Or this can be set in a processor:
