...
- **Trace ID format**: The Datadog Agent requires the trace ID to be in the `dd.trace_id` field.
  ...
  - If you are generating logs with OpenTelemetry `trace_id` and `span_id` (as shown in the file-scraping example), you must use a [Log Processing Rule](http://localhost:1313/logs/log_configuration/processors) in Datadog to remap your `trace_id` attribute to `dd.trace_id`.
...
- [Ease troubleshooting with cross product correlation.](http://localhost:1313/logs/guide/ease-troubleshooting-with-cross-product-correlation/)
