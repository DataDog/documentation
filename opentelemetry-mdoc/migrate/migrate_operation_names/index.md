...
When using OpenTelemetry with Datadog, you might see unclear or lengthy operation names in your traces, and some traces might not appear in your service pages. This happens because of missing mappings between OpenTelemetry SDK information and Datadog operation names, which are span attributes that classify [entry points into a service](http://localhost:1313/tracing/guide/configuring-primary-operation/#primary-operations).
...
## Disabling the new logic (opt-out){% #disabling-the-new-logic-opt-out %}
...
- [Mapping Datadog and OpenTelemetry Conventions](http://localhost:1313/opentelemetry/schema_semantics/)
