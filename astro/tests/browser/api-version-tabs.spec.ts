...
const MULTI_VERSION_URL =
  "/api/latest/aws-integration/list-all-aws-integrations/";
...
test.describe("API operation page version tabs", () => {
  test("multi-version operation renders one pill per version with v2 active", async ({
    page,
  }) => {
  ...
  test("clicking v1 swaps the visible panel and activates the v1 button", async ({
    page,
  }) => {
    await page.goto(MULTI_VERSION_URL);
  ...
  test("loading the page with #...-v1 in the URL opens the v1 tab", async ({
    page,
  }) => {
  ...
  test('single-version operation renders one disabled "v1 (latest)" pill', async ({
    page,
  }) => {
  ...
  test("legacy -v1 URL redirects to the unified page with the v1 tab active", async ({
    page,
  }) => {
    await page.goto(
      "/api/latest/aws-integration/list-all-aws-integrations-v1/",
    );
    await page.waitForURL(
      /list-all-aws-integrations\/#list-all-aws-integrations-v1$/,
    );
