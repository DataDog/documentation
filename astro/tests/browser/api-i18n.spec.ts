...
test.describe("API docs i18n routing", () => {
  ...
  test.skip("Japanese category page is served at /ja/api/latest/<slug>/", async ({
    page,
  }) => {
  ...
  test.skip("language selector is present in the API toolbar", async ({
    page,
  }) => {
  ...
  test("switching to Japanese stays on the same endpoint page", async ({
    page,
  }) => {
    await page.goto("/api/latest/aws-integration/");
    await expect(
  ...
  test.skip("side-nav links carry the locale prefix on Japanese pages", async ({
    page,
  }) => {
  ...
  test("unknown locale prefix returns 404", async ({ page }) => {
    const response = await page.goto("/zh/api/latest/aws-integration/", {
      waitUntil: "load",
    });
  ...
  test("default locale prefix /en/... is not served", async ({ page }) => {
    const response = await page.goto("/en/api/latest/aws-integration/", {
      waitUntil: "load",
    });
    expect(response?.status()).toBe(404);
  });
