  ...
  listSections: async () => [
    {
      ...
      pages: [
        ...
        stubPage("/api/latest/metrics/secret.md", "Secret op", {
          isPrivate: true,
        }),
      ],
    },
...
const call = () =>
  sidecarGET({} as Parameters<typeof sidecarGET>[0]) as Promise<Response>;
...
describe("GET /llms-index.json (structure sidecar)", () => {
  ...
  it("carries each source heading, its root pages, and its sections", async () => {
    ...
    expect(source.rootPages.map((page) => page.urlPath)).toEqual([
      "/api/latest.md",
    ]);
...
describe("tree built from the served sidecar", () => {
  ...
  it("produces a detail file per section, listing its pages", async () => {
    ...
    expect(metrics).toContain(
      `- [Get a metric](${SITE}/api/latest/metrics/get-a-metric.md): Fetch a metric.`,
    );
  });
