...
const renderDropdown = (
  props: Partial<Parameters<typeof NavDropdown>[0]> = {},
) =>
  render(
    h(NavDropdownComponent, {
      ...
      children: h(
        "a",
        { href: "/product/apm", "data-testid": "dropdown-child" },
        "APM",
      ),
      ...props,
    }),
...
describe("NavDropdown — interactivity", () => {
  it("is closed by default (no open modifier classes)", () => {
    ...
    expect(getMenu().classList.contains("header__dropdown-menu--open")).toBe(
      false,
    );
  });
