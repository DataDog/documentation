...
describe("getMdUrl", () => {
  ...
  it("handles locale-prefixed paths", () => {
    setLocation({
      origin: "https://docs.datadoghq.com",
      pathname: "/ja/api/latest/metrics/",
    });
    expect(getMdUrl()).toBe(
      "https://docs.datadoghq.com/ja/api/latest/metrics.md",
    );
  });
  ...
  it("uses the current origin for preview deployments", () => {
    ...
    expect(getMdUrl()).toBe(
      "https://preview.datadoghq.example/api/latest/metrics.md",
    );
  });
...
describe("loadPageText", () => {
  ...
  it("caches the result on subsequent calls", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue(new Response("cached text", { status: 200 }));
  ...
  it("deduplicates concurrent calls", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue(new Response("deduped", { status: 200 }));
