...
test.describe("CopyPageButton component", () => {
  ...
  test("shows Copied feedback after click and resets", async ({
    page,
    context,
  }) => {
    ...
    await expect(btn.locator(".copy-page-button__label")).toContainText(
      "Copied",
    );
...
test.describe("CopyPageButton responsive collapse", () => {
  ...
  test("hides the text label but keeps the icon below the xl breakpoint", async ({
    page,
  }) => {
...
test.describe("CopyPageButton (icon variant in toolbar)", () => {
  test("is hidden while the page is scrolled to the top", async ({ page }) => {
    ...
    await expect(icon).toHaveAttribute("data-hydrated", "true", {
      timeout: 5000,
    });
    await expect(icon).toBeHidden();
  });

  test("reveals itself once the main button scrolls behind the sticky toolbar", async ({
    page,
  }) => {
    await page.goto("/api/latest/metrics/edit-metric-metadata/");
    const icon = page.locator(ICON);
  ...
  test('has a "Copy page" tooltip via the title attribute', async ({
    page,
  }) => {
    await page.goto("/api/latest/metrics/edit-metric-metadata/");
    const icon = page.locator(ICON);
