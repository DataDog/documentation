...
describe("RegionSelector — help link", () => {
  it("renders a help link between the label and the select", () => {
    ...
    const helpLink = document.querySelector<HTMLAnchorElement>(
      ".region-selector__help",
    );
    expect(helpLink).toBeTruthy();
    expect(helpLink!.getAttribute("href")).toBe(
      `${HUGO_ORIGIN}/getting_started/site/`,
    );
  ...
  it("help link appears after the label and before the select in DOM order", () => {
    ...
    expect(
      label.compareDocumentPosition(help) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      help.compareDocumentPosition(select) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
});
