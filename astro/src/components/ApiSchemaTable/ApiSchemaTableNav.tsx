...
export function ApiSchemaTableNav({ externalContext }: ApiSchemaTableNavProps) {
  ...
  useEffect(() => {
    ...
    const setRowExpanded = (toggleBtn: HTMLElement, expanded: boolean): void => {
    ...
    const allToggles = (): HTMLElement[] =>
      Array.from(tableEl.querySelectorAll<HTMLElement>('.schema-table__toggle'));

    const allExpanded = (): boolean => {
      const toggles = allToggles();
      return (
        toggles.length > 0 &&
        toggles.every((t) => t.classList.contains('schema-table__toggle--expanded'))
      );
    };
