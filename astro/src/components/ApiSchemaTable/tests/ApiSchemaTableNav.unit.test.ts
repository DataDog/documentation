...
describe("ApiSchemaTableNav — single-row toggle", () => {
  it("clicking a toggle expands its children container (visibility + BEM + aria)", async () => {
    ...
    const toggles = root.querySelectorAll<HTMLButtonElement>(
      ".schema-table__toggle",
    );
    const childrenContainers = root.querySelectorAll<HTMLElement>(
      ".schema-table__children",
    );

    await user.click(toggles[0]);

    expect(
      toggles[0].classList.contains("schema-table__toggle--expanded"),
    ).toBe(true);
    expect(toggles[0].getAttribute("aria-expanded")).toBe("true");
    expect(childrenContainers[0].hidden).toBe(false);
    // Sibling row stays collapsed
    expect(
      toggles[1].classList.contains("schema-table__toggle--expanded"),
    ).toBe(false);
  ...
  it("clicking an expanded toggle collapses it", async () => {
    ...
    const children = root.querySelector<HTMLElement>(
      ".schema-table__children",
    )!;
    ...
    expect(toggle.classList.contains("schema-table__toggle--expanded")).toBe(
      false,
    );
...
describe("ApiSchemaTableNav — expand-all", () => {
  it("clicking expand-all expands every row and flips the label", async () => {
    ...
    const expandAll = root.querySelector<HTMLButtonElement>(
      ".schema-table__expand-all",
    )!;
    const expandLabel = root.querySelector<HTMLElement>(
      ".schema-table__expand-all-label--expand",
    )!;
    const collapseLabel = root.querySelector<HTMLElement>(
    ...
    root
      .querySelectorAll<HTMLButtonElement>(".schema-table__toggle")
      .forEach((t) => {
        expect(t.classList.contains("schema-table__toggle--expanded")).toBe(
          true,
        );
        expect(t.getAttribute("aria-expanded")).toBe("true");
      });
    root
      .querySelectorAll<HTMLElement>(".schema-table__children")
      .forEach((c) => {
