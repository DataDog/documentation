...
export default function FooterAccordion({
  ...
  const [openSection, setOpenSection] = useState<SectionKey | false>(
    "resources",
  );
  const [isDesktop, setIsDesktop] = useState(false);

  useEffect(() => {
    const desktopBreakpoint = getComputedStyle(document.documentElement)
      .getPropertyValue("--hugo-breakpoint-desktop")
      .trim();
...
function AccordionSection({
  ...
  return (
    <div
      class={`${cl("footer__column")} ${cl(`footer__column--${s.id}`)} footer-section footer-section--${s.id}`}
    >
      <p
      ...
      <div
        class={`${cl("footer__divider")} ${cl("footer__divider--desktop")}`}
        aria-hidden="true"
      />
      <div
        class={`${cl("footer__section-body")} ${isOpen ? cl("footer__section-body--open") : ""}`}
      >
