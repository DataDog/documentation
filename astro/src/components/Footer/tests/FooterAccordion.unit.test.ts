...
const renderAccordion = () => render(h(FooterAccordionComponent, { sections }));

beforeEach(() => {
  document.documentElement.style.setProperty(
    "--hugo-breakpoint-desktop",
    "992px",
  );
  mockMobileViewport();
});
...
describe("FooterAccordion — default state", () => {
  ...
  it("opens the resources section by default (BEM + aria)", () => {
    ...
    const resourcesBody = resourcesSection.querySelector(
      ".footer__section-body",
    )!;
    const resourcesHeader = resourcesSection.querySelector(
      ".footer__section-header",
    )!;

    expect(resourcesBody.classList.contains("footer__section-body--open")).toBe(
      true,
    );
...
describe("FooterAccordion — interactivity", () => {
  it("clicking a closed section opens it and closes the previously open one", async () => {
    ...
    const productHeader = productSection.querySelector(
      ".footer__section-header",
    )! as HTMLElement;
    ...
    expect(productBody.classList.contains("footer__section-body--open")).toBe(
      true,
    );
  ...
  it("supports keyboard activation with Enter", async () => {
    ...
    expect(aboutBody.classList.contains("footer__section-body--open")).toBe(
      true,
    );
  ...
  it("supports keyboard activation with Space", async () => {
    ...
    const blogHeader = blogSection.querySelector(
      ".footer__section-header",
    )! as HTMLElement;
    ...
    expect(blogBody.classList.contains("footer__section-body--open")).toBe(
      true,
    );
