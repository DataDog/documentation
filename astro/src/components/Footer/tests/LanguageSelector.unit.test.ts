...
const renderSelector = (
  props: Partial<Parameters<typeof LanguageSelector>[0]> = {},
) =>
  render(
...
describe("LanguageSelector — interactivity", () => {
  ...
  it("renders current language and all alternates inside the open popup", async () => {
    ...
    expect(items[1].querySelector(".footer__lang-primary")!.textContent).toBe(
      "日本語",
    );
    expect(items[1].querySelector(".footer__lang-secondary")!.textContent).toBe(
      "Japanese",
    );
  });
