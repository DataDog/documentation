...
test.describe("CodeBlock component", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/dd_e2e/components/codeblock");
  });
  ...
  test("renders code block without language label", async ({ page }) => {
    ...
    await expect(plainBlock.locator("code")).toContainText(
  ...
  test("each code block without disable_copy has a copy button", async ({
    page,
  }) => {
  ...
  test("copy button appears on hover and shows Copied! after click", async ({
    page,
    context,
  }) => {
    await context.grantPermissions(["clipboard-read", "clipboard-write"]);
    const firstBlock = page
      .locator('.code-block:has([data-hydrated="true"])')
      .first();
  ...
  test("toggle button collapses and expands code", async ({ page }) => {
    const collapsibleBlock = page
      .locator('[data-collapsible]:has([data-hydrated="true"])')
      .first();
  ...
  test("wrapped code block applies word-wrap styling", async ({ page }) => {
    ...
    const whiteSpace = await codeEl.evaluate(
