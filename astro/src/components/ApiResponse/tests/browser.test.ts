...
test.describe("ApiResponse component — visual", () => {
  ...
  for (const code of statusCodes) {
    test(`status ${code} tab matches screenshot`, async ({ page }) => {
      const response = page.locator(".api-response").first();
      await expect(
        response.locator('[role="tablist"][data-hydrated="true"]').first(),
      ).toBeVisible();
      await response.getByRole("tab", { name: code, exact: true }).click();
      await expect(
        response.getByRole("tab", { name: code, exact: true }),
      ).toHaveAttribute("aria-selected", "true");
      await expect(response).toHaveScreenshot(
        `api-response-status-${code}.png`,
      );
    });
  }
  ...
  test("Model view matches screenshot", async ({ page }) => {
    ...
    await expect(
      response.locator('[role="tablist"][data-hydrated="true"]'),
    ).toHaveCount(await response.locator('[role="tablist"]').count());
    await response.getByRole("tab", { name: "200", exact: true }).click();
    await expect(
      response.getByRole("tab", { name: "Model", exact: true }),
    ).toHaveAttribute("aria-selected", "true");
  ...
  test("Example view matches screenshot", async ({ page }) => {
    ...
    await expect(
      response.getByRole("tab", { name: "Example", exact: true }),
    ).toHaveAttribute("aria-selected", "true");
