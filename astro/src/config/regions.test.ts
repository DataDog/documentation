...
describe("config/regions", () => {
  ...
  it("uses Hugo-compatible region keys (lowercase short form)", () => {
    for (const region of getAllowedRegions()) {
      ...
      expect(region.domain, `domain on ${region.key}`).toMatch(
        /^[a-z0-9.-]+\.[a-z]+$/,
      );
    }
  ...
  it("maps every region key → datacenter label", () => {
    for (const region of getAllowedRegions()) {
      expect(
        datacenterLabel(region.key),
        `dd_datacenter for ${region.key}`,
      ).toBe(region.label);
    }
  });
  ...
  it("maps every region key → API site domain", () => {
    for (const region of getAllowedRegions()) {
      expect(siteDomain(region.key), `dd_site for ${region.key}`).toBe(
        region.domain,
      );
    }
  });
  ...
  it("maps every region key → an app host under its own domain", () => {
    for (const region of getAllowedRegions()) {
      ...
      expect(
        host!.endsWith(region.domain),
        `${host} should sit under ${region.domain}`,
      ).toBe(true);
    }
  });
  ...
  it("validates region keys", () => {
    for (const region of getAllowedRegions()) {
      expect(
        isAllowedRegionKey(region.key),
        `${region.key} should be allowed`,
      ).toBe(true);
    }
