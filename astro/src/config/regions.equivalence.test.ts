...
describe('shared/regions.yaml matches the Hugo sources', () => {
  it('has the same regions as params.yaml, in the same order', () => {
    ...
    const sharedByWeight = [...shared.regions].sort((a, b) => a.weight - b.weight);
  ...
  it('carries the same identity fields as params.yaml', () => {
    const hugoByKey = new Map(hugoParams.allowedRegions.map((r) => [r.value, r]));

    for (const region of shared.regions) {
      const hugo = hugoByKey.get(region.key);
      expect(hugo, `region ${region.key} missing from params.yaml`).toBeDefined();
      expect({
        label: region.label,
    ...
    for (const region of shared.regions) {
      for (const valueKey of Object.keys(region.values)) {
        ...
        if (perRegion === undefined) {
          extras.push(`${region.key}.${valueKey} (key absent from regions.config.js)`);
          continue;
        }
  ...
  it('defines every value key for every region', () => {
    ...
    for (const region of shared.regions) {
      for (const valueKey of valueKeys) {
        if (!(valueKey in region.values)) gaps.push(`${region.key}.${valueKey}`);
      }
    }
    expect(gaps).toEqual([]);
  });
