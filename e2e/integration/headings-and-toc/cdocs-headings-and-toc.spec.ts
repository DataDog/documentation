...
const SHARED_H2S = ['Overview', 'Prerequisites'];

const DATABASE_H2S: Record<string, string[]> = {
    postgres: [...SHARED_H2S, 'Installation', 'Configuration', 'Vacuuming and maintenance', 'Troubleshooting'],
    mysql: [...SHARED_H2S, 'Installation', 'Configuration', 'InnoDB monitoring', 'Troubleshooting'],
    mongo_db: [...SHARED_H2S, 'Installation', 'Configuration', 'Sharding metrics', 'Troubleshooting']
};
...
test.describe('Cdocs headings and TOC', () => {
    ...
    test.describe('TOC shows correct H2s for each filter', () => {
        for (const db of ['postgres', 'mysql', 'mongo_db'] as const) {
            test(`TOC for ${db}`, async ({ page }) => {
                ...
                await expect(page.locator(TOC_CONTAINER)).toHaveScreenshot(`toc-${db}.png`);
            });
        }
    });
    ...
    test('clicking Installation in TOC scrolls to heading and shows H3s', async ({ page }) => {
