...
test.describe('Cdocs content filtering', () => {
    ...
    test('initial page snapshot', async ({ page }) => {
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('content-filtering-defaults.png');
    });
    ...
    test('snapshot after changing both filters', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('content-filtering-go-mysql.png');
    });
});
