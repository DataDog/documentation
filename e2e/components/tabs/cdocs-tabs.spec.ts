...
test.describe('Cdocs tabs component', () => {
    ...
    test('page renders as expected', async ({ page }) => {
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('tabs-initial.png');
    });

    test('first tab in each group is selected on initial load', async ({ page }) => {
        ...
        for (const [groupIdx, lang] of expected) {
            const activeLink = tabGroup(page, groupIdx).locator('.nav-tabs li.active a');
            await expect(activeLink).toHaveAttribute('data-lang', lang);
        }
    });

    test('only the active tab content is visible in each group', async ({ page }) => {
    ...
    test('tab buttons display the correct labels', async ({ page }) => {
        ...
        const threeTabLinks = tabGroup(page, GROUP.THREE_TABS).locator('.nav-tabs a');
    ...
    test('clicking a tab reveals its content and hides the previous tab', async ({ page }) => {
    ...
    test('many-tabs group switches to pill layout', async ({ page }) => {
        await expect(tabGroup(page, GROUP.MANY_TABS)).toHaveClass(/tabs-wrap-layout/);
    });
});
