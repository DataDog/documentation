...
test.describe('Cdocs closed stepper', () => {
    ...
    test('initial state: only first step content is visible and finished is hidden', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-initial.png');
    });
    ...
    test('Next button advances to the next step', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-step2.png');
    });
    ...
    test('clicking a step title skips to that step', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-skip-to-step3.png');
    });
    ...
    test('Expand all shows all step contents', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-expanded.png');
    });
    ...
    test('Finish shows stepper-finished content', async ({ page }) => {
        ...
        await expect(page.locator(FINISHED_SECTION)).toContainText('Happy databasing');
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-finished.png');
    });

    test('rewinding from finished state hides stepper-finished content', async ({ page }) => {
    ...
    test('Start over resets the stepper to step 1', async ({ page }) => {
        ...
        await expect(page.locator(CONTENT_AREA)).toHaveScreenshot('stepper-closed-after-reset.png');
    });
    ...
    test.describe('keyboard navigation', () => {
        ...
        test('Tab through the stepper using only keyboard', async ({ page }) => {
            ...
            await focusNavBtn(page, PREV_BTN);
            await page.keyboard.press('Tab');
            await expect(page.locator(ACTIVE_STEP).locator(FINISH_BTN)).toBeFocused();
            await page.keyboard.press('Enter');
            await expect(page.locator(FINISHED_SECTION)).toBeVisible();
