...
func get_integration(integration string, integration_path string) string {
	...
	if _, err := os.Stat(integrations_core_dir + "/" + integration + "/metadata.csv"); err == nil {
		...
		//f, _, err := m.Parse(strings.NewReader(string(dat)))
		//if err != nil {
		//	panic(err)
		//}
		...
		//		break
		//	} else if err != nil {
		//		fmt.Println("error here in func")
		...
		//
		//	// Loop over metadata.cs
		//	if lineCount > 1 {
		//		var shown = ""
		//
		//		if record[3] != "" {
		//			shown = "shown as " + record[3]
		//		} else {
		//			shown = ""
		//		}
		//		buffer.WriteString("<tr><td>" + record[0] + "<br>" + record[1] + "</td><td>" + record[5] + "<br>" + shown + "</td></tr>")
		//		//fmt.Println("<tr><td>" + record[0] + "<br>" + record[1] + "</td><td>" + record[5] + "<br>" + shown + "</td></tr>")
		//	}else {
		//		// Only a single header row exists so check dogweb or skip
		//		continue
		//	}
		//
		//	lineCount += 1
	...
	//fmt.Println(integration+" has no matching csv in integrations-core")
	// Checck dogweb repo for metadata.csv file and run through function to parse metrics
